...
const WRONG_WEIGHT: &[u8] = include_bytes!("../verus/negative/wrong_weight_index.rs");
...
fn proof_and_negative_mutations_have_exact_source_pins() {
    let pins = [
        ...
        (
            ...
            WRONG_WEIGHT,
            340,
            "be2a047f64d480d7c8fc3b5cb28990c681b105cc6ed42722e473cb440e7f571d",
        ),
    ];
...
fn proof_names_all_layers_and_has_no_shortcuts() {
    ...
    for source in [PROOF, DUPLICATE_WRITER, LANE_PLUS_ONE, WRONG_WEIGHT] {
...
fn documentation_states_the_numeric_and_artifact_boundaries() {
    for statement in [
        ...
        "to refine `f32`.",
        "do **not** bind the proof",
        "generated LLVM IR, ISA, HSACO, loading, launch",
    ] {
        assert!(
...
#[test]
fn pinned_verus_identity_matches_the_reviewed_examples() {
    assert_eq!(
