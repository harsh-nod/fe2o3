...
struct SourceFacts {
    ...
    checked_slice_get_receivers: Vec<String>,
...
impl<'ast> Visit<'ast> for SourceFacts {
    ...
    fn visit_expr_method_call(&mut self, call: &'ast syn::ExprMethodCall) {
        ...
        if call.method == "get"
            && let syn::Expr::Path(path) = call.receiver.as_ref()
            && let Some(identifier) = path.path.get_ident()
        {
            self.checked_slice_get_receivers
...
fn source_forbids_unsafe_and_contains_matrix_tiling_and_epilogue() {
    ...
    for input in ["a", "b"] {
        assert_eq!(
            facts
                .checked_slice_get_receivers
                .iter()
                .filter(|receiver| *receiver == input)
                .count(),
            4,
            "input `{input}` must use four checked slice loads"
        );
        assert!(
