...
proof="$script_dir/verus/moe_expert_memory_v1.rs"
kernel="$script_dir/src/kernel.rs"
negative_dir="$script_dir/verus/negative"
negative_manifest="$script_dir/verus/NEGATIVE_SHA256"
...
expected_transcript=$(read_pin "$script_dir/verus/TRANSCRIPT_SHA256")
...
if [ "$negative_count" -ne 6 ]; then
    printf 'FAIL: expected exactly six pinned proof mutations\n' >&2
    exit 1
fi
...
for marker in \
    ...
    'pub proof fn route_id_is_bounded_v1' \
    'pub proof fn token_activation_index_is_bounded_v1' \
    'pub proof fn expert_weight_index_is_bounded_v1' \
    'pub proof fn expert_tile_index_is_bounded_v1' \
    'pub proof fn compact_output_index_is_bounded_v1' \
    'pub proof fn combined_output_index_is_bounded_v1' \
    'pub proof fn accepted_inverse_value_is_a_compact_slot_v1' \
    'pub proof fn padding_rows_are_disjoint_from_active_rows_v1' \
...
tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/fe2o3-moe-expert-verus.XXXXXX")
...
if ! run_verus "$proof" >"$positive_log" 2>&1; then
    printf 'FAIL: positive exact MoE expert proof did not verify\n' >&2
    cat "$positive_log" >&2
    exit 1
fi
if ! grep -Fq 'verification results:: 15 verified, 0 errors' "$positive_log"; then
...
check_negative route_bound_wrong mutated_route_id_is_bounded_v1
check_negative weight_bound_wrong mutated_weight_index_is_bounded_v1
check_negative padding_wrong mutated_padding_is_active_v1
check_negative inverse_wrong mutated_drop_inverse_is_compact_v1
check_negative combine_bound_wrong mutated_combined_index_is_bounded_v1
check_negative phase_order_wrong mutated_combine_precedes_gemm_v1

check_digest "$expected_model" "$proof"
check_digest "$expected_kernel" "$kernel"
check_digest "$expected_closure" "$closure_manifest"
(cd "$script_dir/verus" && "$sha256_path" -c NEGATIVE_SHA256 >/dev/null)
transcript='FE2O3_MOE_EXPERT_MEMORY_V1_VERUS_OK mutations=6 obligations=15'
