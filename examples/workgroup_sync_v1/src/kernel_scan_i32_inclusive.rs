//! Attributed `i32` inclusive scans using the shared capability source shape.
...
macro_rules! inclusive_scan_i32_body {
...
pub fn lds_inclusive_scan_i32_v1(
    mut context: KernelContext<'_>,
    values: Global<'_, i32, ReadOnly>,
    mut output: Global<'_, i32, DisjointWrite<Index1D>>,
) {
    inclusive_scan_i32_body!(context, values, output, 3);
}

#[cfg(feature = "lds-scan-i32-inclusive-65-kernel")]
...
pub fn lds_inclusive_scan_i32_65_v1(
    mut context: KernelContext<'_>,
    values: Global<'_, i32, ReadOnly>,
    mut output: Global<'_, i32, DisjointWrite<Index1D>>,
) {
    inclusive_scan_i32_body!(context, values, output, 65);
}

#[cfg(feature = "lds-scan-i32-inclusive-255-kernel")]
/// Computes the inclusive prefix sum of one exact 255-element `i32` row.
#[kernel(typed, launch(required = [255, 1, 1], max = [255, 1, 1], max_grid = [1, 1, 1], static_shared_memory_bytes = 1020))]
pub fn lds_inclusive_scan_i32_255_v1(
    mut context: KernelContext<'_>,
    values: Global<'_, i32, ReadOnly>,
    mut output: Global<'_, i32, DisjointWrite<Index1D>>,
) {
    inclusive_scan_i32_body!(context, values, output, 255);
}
