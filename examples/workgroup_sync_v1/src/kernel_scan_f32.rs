//! Attributed `f32` inclusive scans using the shared capability source shape.
...
pub fn lds_inclusive_scan_f32_65_v1(
    mut context: KernelContext<'_>,
    values: Global<'_, f32, ReadOnly>,
    mut output: Global<'_, f32, DisjointWrite<Index1D>>,
) {
    inclusive_scan_f32_body!(context, values, output, 65);
}
...
pub fn lds_inclusive_scan_f32_v1(
    mut context: KernelContext<'_>,
    values: Global<'_, f32, ReadOnly>,
    mut output: Global<'_, f32, DisjointWrite<Index1D>>,
) {
    inclusive_scan_f32_body!(context, values, output, 255);
}
