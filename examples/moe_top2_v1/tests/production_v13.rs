...
#[test]
#[ignore = "requires a genuine compiler-produced MoE top-2 Bundle V8"]
fn compiler_bundle_is_exact_final_graph_v13() {
    let path = std::env::var_os("FE2O3_MOE_TOP2_BUNDLE_V8")
        .map(PathBuf::from)
        .expect("FE2O3_MOE_TOP2_BUNDLE_V8 must name compiler-produced bytes");
    ...
    let kernel = module
        .kernels
        .iter()
        .find(|kernel| kernel.id.as_str() == "moe_top2_route_f32_t8_e4_k2_c4_v1")
