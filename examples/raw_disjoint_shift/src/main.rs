...
#[kernel]
pub fn raw_disjoint_shift(x: &[f32], mut out: DisjointSlice<f32, Shifted<Index1D, 1>>) {
...
fn main() -> fe2o3_core::Result<()> {
    ...
    let x_host: Vec<f32> = (0..(N - 1)).map(|i| i as f32 * 0.03125 + 0.5).collect();
    ...
    // SAFETY: `raw_disjoint_shift.hsaco` is compiler-generated for the
    ...
    let module = unsafe {
        context.load_module_from_file_unchecked(hsaco_dir.join("raw_disjoint_shift.hsaco"))
    }?;
    ...
    for i in 0..N {
        let expected = if i == 0 { 0.0 } else { x_host[i - 1] * 3.0 };
        assert!(
    ...
    println!("raw_disjoint_shift passed for {N} elements");
    Ok(())
}
