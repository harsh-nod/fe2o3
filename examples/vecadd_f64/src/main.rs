...
fn main() -> fe2o3_core::Result<()> {
    ...
    // SAFETY: `vecadd_f64.hsaco` is compiler-generated for the `vecadd_f64`
    ...
    let module =
        unsafe { context.load_module_from_file_unchecked(hsaco_dir.join("vecadd_f64.hsaco")) }?;

    // SAFETY: `vecadd_f64` expects three f64 slice ABIs; a, b, and c are
    // distinct N-element allocations kept alive until stream synchronization.
    unsafe {
        launch! {
            kernel: vecadd_f64,
            stream: stream,
