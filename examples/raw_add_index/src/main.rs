...
fn main() -> fe2o3_core::Result<()> {
    ...
    let x_host: Vec<f32> = (0..(N * 2)).map(|i| i as f32 * 0.015625 + 2.0).collect();
    ...
    // SAFETY: `raw_add_index.hsaco` is compiler-generated for the
    // `raw_add_index` kernel here. The subsequent launch remains independently unsafe.
    // This example requires that output to target this device and contain no init/fini kernels.
    let module =
        unsafe { context.load_module_from_file_unchecked(hsaco_dir.join("raw_add_index.hsaco")) }?;
    ...
    println!("raw_add_index passed for {N} elements");
    Ok(())
}
