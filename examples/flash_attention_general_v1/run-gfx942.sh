...
WORK_DIR=$(mktemp -d "${TMPDIR:-/tmp}/fe2o3-flash-attention.XXXXXX")
trap 'rm -rf -- "$WORK_DIR"' EXIT

SYSROOT=$(rustup run "$TOOLCHAIN" rustc --print sysroot)
EXTRACTOR="$ROOT_TARGET/debug/fe2o3-rustc-extract"
LLVM_IR="$WORK_DIR/flash_attention_general_v1.ll"
LINKED_IR="$WORK_DIR/flash_attention_general_v1.bc"
OBJECT="$WORK_DIR/flash_attention_general_v1.o"
HSACO="$OUTPUT_DIR/flash_attention_general_v1.hsaco"
...
(
    cd -- "$EXAMPLE_DIR"
    FE2O3_EXTRACT_CRATE_V1=fe2o3_flash_attention_general_v1 FE2O3_CARGO_METADATA_BUILD_OBSERVATION_V2=1111111111111111111111111111111111111111111111111111111111111111 FE2O3_EXTRACT_GFX942_LLVM_PATH_V1="$LLVM_IR" RUSTC_WORKSPACE_WRAPPER="$EXTRACTOR" CARGO_TARGET_AMDGCN_AMD_AMDHSA_RUSTFLAGS='-Zalways-encode-mir -Ctarget-cpu=gfx942 -Ctarget-feature=-xnack,+wavefrontsize64,-wavefrontsize32' LD_LIBRARY_PATH="$ROOT_TARGET/debug/deps:$SYSROOT/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" rustup run "$TOOLCHAIN" cargo check --release --locked -Zbuild-std=core --target amdgcn-amd-amdhsa --target-dir "$AMD_TARGET" --lib
)
...
(
    cd -- "$EXAMPLE_DIR"
    FE2O3_FLASH_ATTENTION_HSACO="$HSACO" CARGO_TARGET_DIR="$ROOT_TARGET" rustup run "$TOOLCHAIN" cargo run --release --locked --bin fe2o3-flash-attention-general-v1
)
