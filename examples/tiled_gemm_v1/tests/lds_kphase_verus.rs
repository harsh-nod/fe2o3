...
#[test]
fn authenticated_runner_checks_kphase_proof_and_mutations() {
    ...
    assert!(stdout.contains("PASS: Slice 2 LDS K-phase model verified"));
    assert!(
        stdout.contains("XFAIL: lds_kphase_reuse_wrong rejected at the expected proof obligation")
    );
    assert!(stdout.contains(
        "XFAIL: lds_kphase_accumulator_reset_wrong rejected at the expected proof obligation"
    ));
}
