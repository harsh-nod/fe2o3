...
//!
//! This capability-safe functional control replaces BF16 MFMA with scalar
//! widening while leaving routing, expert compute, and stores unchanged. It is
//! not production-qualified and carries no current HSACO or performance claim.
...
use crate::capability_views::{
    compute_expert_streamed, compute_parallel_router_logits, compute_scalar_attention,
...
pub fn gfx950_gpt_oss_120b_decode_megakernel_v1(
    ...
    let [attention0, attention1, attention2, attention3] = compute_scalar_attention(
        &mut context,
        &query_bf16,
        &key_transposed_bf16,
        &value_f32,
