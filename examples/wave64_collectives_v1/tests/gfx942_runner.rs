...
#[test]
fn host_binds_every_argument_through_the_generated_adapter() {
    assert_eq!(HOST.matches("GeneratedHostReadSliceV1::new").count(), 1);
    assert_eq!(HOST.matches("GeneratedHostWriteSliceV1::new").count(), 3);
    assert!(HOST.contains("wave64_collectives_v1_gpu::Arguments::new"));
    assert!(HOST.contains("run_generated_application_v1"));
    assert!(HOST.contains("prepare_generated_application_invocation_v1"));
    assert!(HOST.contains("wave64_collectives_oracle_v1"));
...
fn runner_is_executable_and_rejects_an_unbounded_timeout_before_launch() {
    ...
    let output = Command::new(path)
        .env("FE2O3_HARDWARE_TIMEOUT_SECONDS", "unbounded")
        .output()
        .unwrap();
