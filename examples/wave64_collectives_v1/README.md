...
oracle, formal model, and reviewed source/CPU correspondence for one fixed masked
...
## Historical Source-model-to-Kernel-IR refinement

Current source-to-KIR qualification remains unsupported. No current source-to-KIR
receipt is available. The deleted compiler workload-profile schema is not restored
and the general compiler does not select this example's old template.

The former Rust adapter and its two unit plus nine integration cases are retained
byte-for-byte under `historical/` as noncompiled text, with exact hashes and retirement
status. Those 11 retired Rust cases are not current passing coverage. The old
source identity was 7c6ead1e7c01a61a8f31a010c9e8cb9bd1c21a905ba61e9d90c6c077c748ffd4
and the absent historical schema identity was
da2722bd3ce349228644300b13bb45d4683d1ebd60f8b7749e7764ec6569e894.
These are historical pins, not identities of current source or general Kernel IR.

`verus/wave64_source_kir_refinement_v1.rs` remains unchanged as a historical
abstract-model theorem suite with 10 obligations and three expected negatives.
Its runner labels it historical; it does not establish a current source-to-KIR
proof or bind the present compiler output. Current source/CPU proof identities
are updated separately. Fresh Verus qualification of those current identities
remains pending; earlier proof execution is not inherited.

The live exact-syntax source/CPU checker now requires the current trap-only
spelling. Reinserting the retired `return` or substituting its exact old source
digest rejects. Every existing source/CPU structural mutation and finite-F32
corpus check remains, as does the exact committed-tree process check. The latter
must run after the final files are committed, not against uncommitted edits.
...
refinement. No current source-model-to-Kernel-IR relation is supplied.
It does not prove compiler causality.
Device compilation must use the one general production transaction and its
actual semantic MIR/SSA and Kernel IR. Unsupported behavior is terminal; there
is no workload-profile fallback. Host model tests do not qualify that device path.
