#!/usr/bin/env python3
"""Fail-closed lexical audit for the bounded Wave64 Verus sources."""
    ...
    while cursor < len(source):
        if source.startswith("//", cursor):
            ...
            if raw_end is not None:
                ...
                # Verus proof files use no Rust lifetimes. Treat every quote as
                # a character literal and reject an unterminated one.
                cursor = quoted_end(source, cursor, "'")
            else:
...
def scan(path: Path) -> None:
    ...
    for character in source:
        ...
        if (category.startswith("C") and character in {"\u200e", "\u200f", "\u202a", "\u202e"}) or category.startswith("Z") and character not in {" ", "\n"}:
