...
fn runner_enters_only_the_bounded_production_application_route() {
    ...
    assert!(RUNNER.contains(
        "fe2o3_tutorial_hardware_authority_run_gfx942 \"$example_dir\" fe2o3-row-softmax-general-v1"
    ));
}

#[test]
fn host_exposes_only_typed_arguments_and_dynamic_values() {
    for required in [
        "GeneratedHostReadSliceV1::new(&input)",
        "GeneratedHostReadWriteSliceV1::new",
        "row_softmax_general_v1_gpu::Arguments::new",
        "run_generated_application_v1",
        "prepare_generated_application_invocation_v1",
        "evaluate_reference_v1",
