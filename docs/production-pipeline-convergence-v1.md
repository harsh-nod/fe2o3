...
4. **Rust and verification:** current gfx942 receipts bind exact semantic MIR,
   ...
   across replay. Singleton Worker V3 admission independently decodes the
   target-binding, AMDHSA-layout, and semantic-to-LLVM records; checks every
   associated capsule receipt; owns exact KIR-to-LLVM replay; cross-binds final
   LLVM to handoff and finalizer state; and matches COV6/workgroup facts to the
   exact descriptor. The multi-root target-binding codec is still producer
   private. Formal KIR-to-LLVM semantic preservation and LLVM-to-machine
   refinement remain open; no
