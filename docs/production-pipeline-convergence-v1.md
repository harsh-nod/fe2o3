...
| Compiler module handoff | Production has one mandatory protected-custody path and one V3 publication/consumption transaction. A device transaction must retain an exact managed build attempt before collection; the attempt and protected rustc invocation move as one publication custody value, so no optional or late direct-publication branch remains. The ordinary publication branch and runtime schema selector are deleted. | Keep V1/V2 consumers confined to explicit qualification code until their oracles retire. |
