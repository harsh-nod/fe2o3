...
| Inspect source, work item, wave, workgroup, KIR, schedule, LLVM, and ISA as one identity graph | Native source and machine state, without fe2o3 semantic identities | Hardware timelines and samples, without fe2o3 source/KIR identities | Source and CUDA-GDB focus, without fe2o3 IR identities | The [production multi-function contract](production-multifunction-semantic-debug-v1.md) preserves exact singleton entry/helper owners, KIR ordinals, roles, and identities through source maps, finalizer replay, simulator helper stops, stacks, and KIR-to-source queries. A supplied or strictly collected rocprof PC can join through exact artifact/target/kernel-symbol identity to every matching sparse source/MIR/KIR/LLVM/ISA characteristic occurrence. Additive Variant V3 joins Bundle V4's exact V7 claim through a finalizer-replayed production bridge to a unique V8 catalog occurrence, binding source-map, artifact, structural, catalog, correlation, semantic-map, function, block, and operation identities. A self-contained archive and authority-free fresh-process JSONL route re-establish those query owners. Multi-root custody, function-qualified helper synthetics, external provenance, schedules, complete ISA coverage, and cross-capture execution identity remain open |
