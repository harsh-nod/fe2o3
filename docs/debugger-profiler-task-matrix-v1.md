...
| Inspect source, work item, wave, workgroup, KIR, schedule, LLVM, and ISA as one identity graph | Native source and machine state, without fe2o3 semantic identities | Hardware timelines and samples, without fe2o3 source/KIR identities | Source and CUDA-GDB focus, without fe2o3 IR identities | The [production multi-function contract](production-multifunction-semantic-debug-v1.md) now preserves exact singleton entry/helper owners, KIR ordinals, roles, and identities through source maps, finalizer replay, simulator helper stops, stacks, and KIR-to-source queries. A supplied or strictly collected rocprof PC can join through exact artifact/target/kernel-symbol identity to every matching sparse source/MIR/KIR/LLVM/ISA characteristic occurrence. Multi-root custody, function-qualified helper synthetics, schedules, producer authentication, complete ISA coverage, and cross-capture identity remain open |
