...
| Collect dispatch intervals, counters, PC samples, runtime/copy events, and ATT | Not its primary profiling role | Native collector; rocprofv3 documents each class and Compute Viewer visualizes decoded ATT | Not documented as one comparable profiler contract | Strict dispatch/counter/PC and ATT-reference admission exists; direct-KFD runtime lifecycle, host staging, AQL publication/completion, and host durations are observable through a separate bounded protocol; rocprof correlation, device copy-engine events, counters, PC samples, and decoded ATT remain unavailable |
