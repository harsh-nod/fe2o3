...
| --- | --- | --- | --- | --- |
| Stop a live AMD GPU wave and inspect PC, registers, lanes, and target memory | Native hardware debugger; documented | Not an interactive debugger | Unsupported on AMD; documented GPU path uses CUDA-GDB on NVIDIA | V4 strictly correlates cooperative KFD publication with the five structured ROCgdb hierarchy commands and redacted lane/workgroup coordinates. V5 additionally admits same-stop, wave-scoped register values and simple scalar locals while withholding native selectors and addresses. The installed direct-KFD target still cannot produce a physical GPU stop, and lane values, source, ISA, memory, and broad live-kernel acceptance remain unavailable |
...
| Inspect source, work item, wave, workgroup, KIR, schedule, LLVM, and ISA as one identity graph | Native source and machine state, without fe2o3 semantic identities | Hardware timelines and samples, without fe2o3 source/KIR identities | Source and CUDA-GDB focus, without fe2o3 IR identities | The [production multi-function contract](production-multifunction-semantic-debug-v1.md) preserves exact singleton entry/helper owners, KIR ordinals, roles, and identities through source maps, finalizer replay, simulator helper stops, stacks, and KIR-to-source queries. A supplied or strictly collected rocprof PC can join through exact artifact/target/kernel-symbol identity to every matching sparse source/MIR/KIR/LLVM/ISA characteristic occurrence. Additive Variant V3 can join Bundle V4's exact V7 claim through an already-admitted production bridge to a unique V8 catalog occurrence, binding source-map, artifact, structural, catalog, correlation, semantic-map, function, block, and operation identities. Variant V3 is currently an in-process production-owner API; multi-root custody, function-qualified helper synthetics, schedules, complete ISA coverage, and cross-capture execution identity remain open |
...
| Compare two captures without confusing process-local IDs | Manual debugger analysis | Collector output contains capture-local records; higher-level comparison is external | No documented cross-capture evidence contract | Bundle V4 retains exact same-KIR/artifact joins; Variant V1 content-binds each treatment and compatibility axes. Variant V2 pairs unique positive PC/ATT observations at the same exact source/MIR site. Variant V3 optionally requires each side to resolve through its admitted V7-to-V8 bridge and one exact catalog record before reporting the V2 changed axes as structural positive co-observations. Partial or sampled absence cannot mean added/removed; schedule execution and causality remain typed unavailable |
...
| Let a non-privileged agent query evidence without gaining launch, attach, pause, or recapture authority | MI can be automated but debugger control carries process authority | CLI/output can be automated; collection is a separate privileged action | CLI or VS Code workflow | A deterministic fresh-process reference client discovers capabilities, validates exact cited simulator and Variant V1/V2 evidence, pages dispatches, and requests the minimum ambiguous capture plan over pipes. Variant V2 is independently replayed against exact retained artifact and schedule bytes while absent PC/ATT sessions remain typed unavailable. It has no execution/replay/file/network/patch/attach/scheduling/collection/decoder/launch operation; OS sandbox deployment remains external |
