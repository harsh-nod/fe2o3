...
| Collect dispatch intervals, counters, PC samples, runtime/copy events, and ATT | Not its primary profiling role | Native collector; rocprofv3 documents each class and Compute Viewer visualizes decoded ATT | Not documented as one comparable profiler contract | Strict dispatch/counter/PC and ATT-reference admission exists. The PC producer has a non-executing first plan, a separate content-bound authorization for its bounded external capability probe, exact KFD agent and interval admission, independent collection and beta-risk acknowledgements, immediate pre-spawn KFD/capability re-observation, strict process/agent projection, and canonical Capture V3/relation/query publication; the freeze-risking hardware capture remains unrun. Direct-KFD runtime lifecycle, host staging, AQL publication/completion, and host durations are observable separately; device copy-engine events, decoded ATT, cross-capture identity, direct-KFD/rocprof common runtime-dispatch identity, and the residual device-wide post-observation race remain unavailable |
