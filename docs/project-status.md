...
> This document preserves the detailed implementation and evidence narrative
> that previously formed the project README. It is an engineering status
> archive, not a support or compatibility promise. Start with the
> [documentation index](README.md), [getting-started guide](getting-started.md),
...
[living v2 architecture](architecture-v2.md),
[production compiler convergence design](production-pipeline-convergence-v1.md),
[compiler execution subject V1](compiler-execution-subject-v1.md),
[compiler execution attestation protocol V1](compiler-execution-attestation-v1.md),
[receipt-bearing Worker V3 load envelope V2](worker-v3-receipt-bearing-load-envelope-v2.md),
[gfx942 production LDS reduction](gfx942-production-lds-reduction-v1.md),
[workspace ownership policy](workspace-layers-and-ownership.md),
[Pliron Wave 0 architecture](pliron-wave0-architecture.md),
[cuda-oxide parity matrix](cuda-oxide-parity-matrix.md),
[evidence-backed parity dashboard](generated/cuda-oxide-parity-dashboard.md),
[debugger and profiler architecture V1](debugger-profiler-architecture-v1.md),
[debugger and profiler task matrix V1](debugger-profiler-task-matrix-v1.md),
[verification model](verification-model.md),
[GPU safety contract v1](gpu-safety-contract-v1.md), and
[implementation roadmap](implementation-roadmap-v2.md). The
[testing guide](testing.md) defines the generic, Verus, ROCm compile, and
hardware execution lanes.

CPU-only source extraction is available through the explicit, authority-free
[`fe2o3-export-sim` bundle workflow](simulation-bundle-v1.md). It reuses
...
[`fe2o3-simulation-schedule-v1`](semantic-schedule-v1.md) decision record
...
[protected issuer admission V1](compiler-execution-issuer-admission-v1.md)
and [durable issuer state V2](compiler-execution-issuer-durable-v2.md).
The transport records are specified in
[receipt publication V1](compiler-execution-receipt-publication-v1.md).
The durable consumer is specified in
[Worker receipt ledger V1](compiler-execution-worker-ledger-v1.md).
The bounded transport is specified in
[compiler execution service V1](compiler-execution-service-v1.md).
...
[gfx942 wave/LDS V2 slice](gfx942-wave-lds-v2.md) additionally carries one
...
The [bounded MoE V1 checkpoint](bounded-moe-v1.md) retains a standalone
...
- Pliron framework: `fe2o3-pliron` is a bounded D0 context, registration,
  ...
  `7ebf6e6638c2a3bcec179423993b01211a9689b4`, a strict descendant of the
...
[device memory safety contract](device-memory-safety.md). `DeviceCopy`
...
- Default Worker V3 singleton and multi-root roster decisions now consume
  move-only target-lineage owners produced by an independent verifier. The
  verifier uses shared strict codecs to decode target binding, AMDHSA data
  layout, and the semantic-to-LLVM association; rederives the semantic
  target-layout identity; checks every associated compiler receipt; and retains
  the exact KIR-to-LLVM replay. Final promotion byte-matches those records to
  the capsule, binds the associated final LLVM identity to both the compiler
  handoff and independently reconstructed finalizer module, and binds
  code-object V6 plus every canonical root and exact workgroup to the admitted
  descriptor roster and physical symbols. Missing, duplicate, reordered,
  foreign, or rehashed cross-compilation substitutions fail closed. This is
  exact target association and replay, not semantic or LLVM-to-machine
  refinement, proof-to-executable authority, or runtime authority.
...
[parity matrix](cuda-oxide-parity-matrix.md) and the generated
[evidence dashboard](generated/cuda-oxide-parity-dashboard.md). The
...
See [implementation-plan.md](implementation-plan.md) for the original
compiler/runtime plan and
[implementation-roadmap-v2.md](implementation-roadmap-v2.md) for the
...
VERUS=/absolute/path/to/mir-pliron/verus scripts/ci-local.sh verus
