#!/usr/bin/env python3
"""Reject corrupted diagnostics, relocated exits, source injections and rehashed packets."""
import argparse
import copy
...
def main():
    ...
    spec = importlib.util.spec_from_file_location('shared_retained_diagnostic_selftest', path)
    ...
    for item in mutations:
        case_inputs(item)
    mutation = next(m for m in mutations if m.name == 'chain_guard')
    ...
    def relocate(rows):
        # Valid coordinates/excerpts/expansion, but $count is not a return point.
        secondary = target(rows, 'exit')
        start = body.index('$count', checker.macro_bounds(body)[mutation.macro]['start'])
        forged = module.retained_parent.exact_span(body, Path(secondary['file_name']), start,
    ...
    try:
        ...
        checker.need(str(error) == 'exact intended shared return or block exit', 'relocation rejected for exact exit identity')
    else:
        raise ValueError('accepted source-consistent non-exit span')
    invalid.append((1, out, relocated))
    changed = copy.deepcopy(diagnostics)
    secondary = target(changed, 'exit')
    start, end = checker.invocation(source, mutation.macro)
    secondary.clear()
    secondary.update(module.retained_parent.exact_span(source, original, start, end, False, 'at this exit'))
    invalid.append((1, out, '\n'.join(map(json.dumps, changed))))
    changed = copy.deepcopy(diagnostics)
    ...
    for item in mutations:
        if not item.assertion:
            continue
        text, shared, location, stdout, stderr = case_inputs(item)
        rows = [base.unique_json(line) for line in stderr.splitlines()]
        _, begin, _ = base.function_bounds(text, item.function)
        start = text.index('result', begin)
        rows[0]['spans'] = [module.retained_parent.exact_span(text, location, start, start + 6, True, 'assertion failed')]
        try:
            checker.check_result(module, 1, stdout, '\n'.join(map(json.dumps, rows)), text, shared, location, item)
        except ValueError as error:
            checker.need(str(error) == 'exact supporting assertion span', 'exact assertion target rejection')
        else:
            raise ValueError('accepted relocated supporting assertion')
        assertions_rejected += 1

    good = args.proof / 'positive_before'
    good_source = (good / 'verus' / checker.SOURCE).read_text()
    good_body = (good / checker.BODY).read_text()
    rejected_sources = 0
    with tempfile.TemporaryDirectory(prefix='fe2o3-shared-retained-source-selftest-') as temporary:
        for index in range(10):
            folder = Path(temporary) / str(index)
            shutil.copytree(good, folder)
            changed_source, changed_body = good_source, good_body
            if index == 1:
                changed_source = changed_source.replace('../src/context_version_journal/retained_bodies.rs', 'foreign.rs')
            elif index == 2:
                changed_source += '\ninclude!("foreign.rs");\n'
            elif index == 3:
                changed_body = changed_body.replace('($left:ident', '($left:expr', 1)
            elif index == 4:
                changed_body = changed_body.replace('        Ok(member)', '        assume(false);\n        Ok(member)')
            elif index == 5:
                changed_body = changed_body.replace('        Ok(member)', '        #[cfg(test)] let bypass = 0;\n        Ok(member)')
            elif index == 6:
                changed_body += '\nmacro_rules! bypass { () => { true }; }\n'
            elif index == 7:
                path = folder / 'verus' / module.retained_parent.SOURCE
                path.write_bytes(path.read_bytes() + b'\n')
            elif index == 8:
                changed_source = changed_source.replace(checker.ANNOTATIONS, 'let bypass = true;')
            elif index == 9:
                changed_source = changed_source.replace('retained_chain_body!(verus_exec_expr,', 'retained_chain_body!(foreign_expr,')
            try:
    ...
    if args.packet:
        ...
        expected_failures = ['exact staged nested inputs', 'exact retained exit and expansion tree',
            'owned solver completion', 'owned solver completion', 'exact whole-crate result', 'proof roster',
            'proof input identities', 'owned command completion', 'exact intended shared return or block exit',
            'exact supporting assertion span']
        with tempfile.TemporaryDirectory(prefix='fe2o3-shared-retained-packet-selftest-') as temporary:
            for index, failure in enumerate(expected_failures):
                packet = Path(temporary) / str(index)
                shutil.copytree(args.packet, packet)
                if index == 0:
                    path = packet / 'proof/chain_guard' / checker.BODY
                    path.write_text(path.read_text().replace('return Err(ReadErrorV1::InvalidReference);', 'return Err(ReadErrorV1::InvalidState);'))
                elif index in (1, 8):
                    path = packet / 'proof/chain_guard/solver/stderr.log'
                    ...
                    path = packet / 'proof/chain_guard/solver/record.json'
                    ...
                    path = packet / 'proof/unknown_variant/solver/stderr.log'
                    rows = [json.loads(line) for line in path.read_text().splitlines()]
                    rows[0]['spans'][0]['label'] = 'foreign assertion'
                    path.write_text('\n'.join(map(json.dumps, rows)) + '\n')
                ...
                rejected_packets += 1
    print(json.dumps({'adverse_solver_results_rejected': len(invalid) + assertions_rejected,
