...
class EvidenceTests(unittest.TestCase):
    ...
    def test_focused_qualification_is_required(self):
        changes = [
            ("missing", lambda f: shutil.rmtree(f / "qualification/scalar-wait")),
            ("missing-bracketed", lambda f: shutil.rmtree(f / "qualification/scalar-wait-bracketed")),
            ("missing-before", lambda f: shutil.rmtree(f / "qualification/source-before")),
            ("wrong-source", lambda f: edit(f, "qualification/source-after/receipt.json",
                 lambda r: r["command"].append("unqualified"))),
            ("wrong-bracket-order", lambda f: edit(f, "qualification/source-before/receipt.json",
                 lambda r: r.update(started_ns=0))),
            ("wrong-filter", lambda f: edit(f, "qualification/scalar-wait/receipt.json",
                 lambda r: r["command"].append("different"))),
            ("wrong-count", lambda f: (f / "qualification/scalar-wait/stdout").write_text(
                 "\nrunning 0 tests\n\ntest result: ok. 0 passed; 0 failed; 0 ignored; 0 measured; 1533 filtered out; finished in 0.00s\n\n")),
            ("runtime-omitted", lambda f: shutil.rmtree(f / "qualification/ordered-runtime")),
        ]
        for name, mutate in changes:
            with self.subTest(case=name), tempfile.TemporaryDirectory(prefix="fe2o3-scalar-evidence-") as tmp:
                folder = Path(tmp) / "packet"
                shutil.copytree(HERE, folder)
                mutate(folder)
                rehash(folder)
    ...
    def test_prior_packet_is_not_scalar_qualification(self):
        previous = HERE.parent / "dev-xgmi-ordered-segments-mi300x-2026-09-20"
