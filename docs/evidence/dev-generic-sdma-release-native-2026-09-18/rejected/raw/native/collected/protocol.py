...
COMMIT = "a0ab05f38ab28b81f8a3080268d07421f118ac87"
BINARY_SHA = "9fa74e6b09b972546e63bf31e29be58f262b2eb54e80caa261c0088a7b2c8dcd"
CPU_SEAL = "5e80d1c2b3c3f3937039e69c68e2df99832451bc0e7987bc23684fc8ca792664"
COHORT_SHA = "ef20cc26a5b32d265f891e517e6cf9037bf3b63a77bb142c0c6f8cc3df43c44c"
...
def transcript(case, stdout, stderr):
    ...
    prefix = (
        "retained_single_sdma_release=complete engine=" + TESTS[case]
    ...
    match = re.fullmatch(
        re.escape(prefix) + r"([0-9]+)" + re.escape(suffix) + r"([0-9]+)"
        + re.escape(tail) + r"([0-9]+)" + re.escape(end), stdout
    )
    need(match is not None, "exact whole two-line example transcript")
    sdma, event, offset = map(int, match.groups())
    need(0 < sdma <= 0xffffffff and 1 <= event <= 255 and offset < 8192 and offset % 8 == 0,
         "native queue, event and doorbell observations")
    return {"case": case, "engine": TESTS[case], "primary_queue_id": 0,
            "sdma_queue_id": sdma, "event_id": event, "doorbell_byte_offset": offset,
            "released_resources": 8, "public_root_drop_completed": True}
