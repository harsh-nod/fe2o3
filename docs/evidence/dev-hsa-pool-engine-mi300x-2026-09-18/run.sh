...
cd "$owned/source"
[[ $(git rev-parse HEAD) == 3da2d25ac965afa9845b8ab1246e5fdf13c5d821 ]]
...
phase() {
    local repetition=$1 cell=$2 status=0 post_status=0 grain engine
    printf 'phase repetition=%s cell=%s\n' "$repetition" "$cell"
    admit
    case "$cell" in
        before|after)
            /usr/bin/timeout --signal=TERM --kill-after=5s 180s prlimit --core=0:0 -- /usr/bin/numactl --physcpubind=48-95 --membind=1 "$owned/target/release/examples/gfx942-runtime-directional-window-benchmark" 0x54f88318ca05093d 268435456 3 10 || status=$?
            ;;
        A|B|C|D)
            case "$cell" in A) grain=fine; engine=engine0 ;; B) grain=fine; engine=engine1 ;; C) grain=coarse; engine=engine0 ;; D) grain=coarse; engine=engine1 ;; esac
            HSA_XNACK=0 ROCR_VISIBLE_DEVICES=4 /usr/bin/timeout --signal=TERM --kill-after=5s 180s prlimit --core=0:0 -- /usr/bin/numactl --physcpubind=48-95 --membind=1 "$owned/hsa-copy-pool-engine" 0 0 268435456 3 10 0x54f88318ca05093d "$grain" "$engine" || status=$?
            ;;
        *) return 64 ;;
    esac
...
printf 'context diagnostic=shared-host-hsa-pool-engine git_commit=3da2d25ac965afa9845b8ab1246e5fdf13c5d821 gpu=4 unique_id=0x54f88318ca05093d bytes=268435456 depth=1 warmups=3 samples=10 repetitions=4 optional_context_journal=disabled placement=cpu48-95-memory1 hsa_cpu_index=0\n'
for repetition in 1 2 3 4; do
    phase "$repetition" before
