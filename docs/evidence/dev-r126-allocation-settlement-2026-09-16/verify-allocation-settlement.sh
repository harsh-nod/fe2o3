...
root=${1:?usage: verify-allocation-settlement.sh CHECKOUT OUTPUT_DIRECTORY}
out=${2:?usage: verify-allocation-settlement.sh CHECKOUT OUTPUT_DIRECTORY}
out=$(realpath -- "$out")
cd -- "$root"
...
for target in gnu musl; do
    ...
    for crate in fe2o3_kfd fe2o3_runtime; do
        ...
        if [[ $crate == fe2o3_runtime ]]; then
            ...
            run "$target-kfd-allocation" prlimit --core=0:0 -- "${executables[0]}" constructed_sdma_allocation --test-threads=1
            grep -Eq 'test result: ok\. [1-9][0-9]* passed;' "$out/$target-kfd-allocation.log"
        fi
    done
done
run no-default cargo check --locked --offline -p fe2o3-runtime --no-default-features
run resource-accounting cargo test --locked --offline -p fe2o3-resource-accounting --lib
