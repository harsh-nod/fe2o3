...
const baseline = JSON.parse(fs.readFileSync(path.join(root, 'r93-clippy-corrected-source.json')));
...
for (const mutation of JSON.parse(fs.readFileSync(path.join(root, 'r93-mutations.json')))) {
  const stem = path.join(root, 'r93-mutation-' + mutation.name);
...
console.log('PASS: 3 compiled negative mutations rejected; ' + names.length + ' source identities exactly restored');
