...
assert digest("copy_accounting.rs.txt") == "bded7480cdf174d91756b5cbf1566aee8acd7da9af122cbc9bbea0ba5925cdf8"
...
local = {
    ...
    "upload": 0, "remote-inputs": 0, "campaign": 1, "early-native": 0, "collect": 0,
...
assert read("raw/early-native.stdout") == read("returned/results/native.stdout") + stderr
assert shlex.split(read("raw/early-native.command")) == ["ssh", "-o", "BatchMode=yes", "-o", "ConnectTimeout=10", "mi300x", "cat", OWNED + "/results/native.stdout", OWNED + "/results/native.stderr"]
assert timestamp(read("returned/results/native.finished")) <= timestamp(read("raw/early-native.started"))
...
print("PASS: rejected corrected candidate; 12 original local and 7 remote closed receipts; frozen identities, early native output, sticky refusal and owned cleanup verified; explicit copy/readback/refund not reached")
