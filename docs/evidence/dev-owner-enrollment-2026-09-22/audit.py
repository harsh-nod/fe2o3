...
SOURCE = 'e8d5c3cc1e23ffced85fbd95feac4b6b745502ae'
CHECK = Path('crates/fe2o3-runtime-model/verus/check-owner-enrollment.py')
...
def audit(repo, packet, quiet=False):
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-enrollment-audit-') as temporary:
        ...
        previous = module(tree / check.PREVIOUS, 'enrollment_record_previous')
        legacy = module(tree / check.LEGACY, 'enrollment_record_legacy')
        base = module(tree / legacy.BASE, 'enrollment_record_base')
        expected = set(legacy.SOURCES) | set(previous.NEW) | set(check.NEW) | set(legacy.PINS) | {
            check.LEGACY, check.PREVIOUS, CHECK, check.CRATE / 'verus/context_version_journal_settlement_v1.rs',
        ...
        need(parse((records / 'test/stdout.log').read_text()) == [(942, 0, 18, 0, 0), (27, 0, 0, 0, 0)], 'unit and doctest results')
...
def selftest(repo, packet):
    for case in ('probe', 'source_roster', 'positive_count', 'positive_type', 'negative_scope', 'cpu_count',
        ...
        with tempfile.TemporaryDirectory(prefix='fe2o3-enrollment-audit-test-') as temporary:
            ...
            if case in ('probe', 'source_roster'):
                ...
                row['command'][index] = 'enrollment_journal_exec_v1'
                path.write_text(json.dumps(row))
            elif case == 'cpu_count':
                path = records / 'test/stdout.log'
                text = path.read_text()
                need(text.count('942 passed;') == 1, 'CPU selftest anchor')
                path.write_text(text.replace('942 passed;', '941 passed;'))
