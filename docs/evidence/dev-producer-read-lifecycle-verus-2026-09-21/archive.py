...
CAMPAIGN = Path('/home/harsh/.codex-tmp/fe2o3-producer-lifecycle-verus-20260921-a5')
ARCHIVE = ROOT / 'docs/evidence/dev-producer-read-lifecycle-verus-2026-09-21'
...
QUALIFY_SHA = '7ffde4c98f18367cec159b4e2cb66894e17d36a21353880ba9b1b04c590e4122'
PINNED = {
    PROOF / 'context_producer_read_lifecycle_v1.rs': 'CONTEXT_PRODUCER_READ_LIFECYCLE_SHA256',
    PROOF / 'check-producer-read-lifecycle.py': 'PRODUCER_READ_LIFECYCLE_CHECKER_SHA256',
...
COMMANDS = [
    ...
    ('lint', ['/home/harsh/.local/bin/ruff', 'check', str(PROOF / 'check-producer-read-lifecycle.py'), str(PROOF / 'check-negative-quality.py')], 60),
...
def signed_sources(repo, temporary):
    ...
    checker = temporary / PROOF / 'check-producer-read-lifecycle.py'
...
def validate_campaign(campaign, module, contents, temporary):
    ...
    expected_result = {'qualified': True, 'obligations': 254, 'inherited': 180,
                       'executable_mutations': 19, 'inputs': inputs}
    ...
    source = contents[PROOF / 'context_producer_read_lifecycle_v1.rs'].decode('ascii')
    mutations = module.mutations()
    need(len(mutations) == 19 and len({item.name for item in mutations}) == 19,
         'exact unique mutation inventory')
    cases = [('positive_before', None), *[(item.name, item) for item in mutations], ('positive_after', None)]
    need(len(cases) == 21, 'exact dedicated case count')
    ...
    for phase in ['before', 'after']:
        ...
        for name, mutation in cases:
            ...
            candidate = original_folder / 'context_producer_read_lifecycle_v1.rs'
            expected = module.BASE.mutate(source, mutation) if mutation else source
            generated = {'context_producer_read_lifecycle_v1.rs': expected.encode('ascii'),
            ...
            rows.append({'case': name, 'exit': record['status'], 'verified': 253 if mutation else 254,
