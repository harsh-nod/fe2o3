...
const contract = 'r111-exact-observed-utc-floor-v1';
...
function predecessor(name, field, expected, io = fs) {
  assert(/^r111-exact-clock-[a-z0-9-]+\.json$/.test(name), 'clock cohort predecessor');
  ...
  if (expected.kind === 'run') {
    ...
    assert.strictEqual(record.source_unchanged, true);
    assert.strictEqual(record.source_identities, 5676);
  }
...
function plan(name) {
  const manifestName = 'r111-exact-clock-manifest.json';
