...
const mutation = read('r111-exact-mutations.json').find(m => m.name === name);
assert(mutation, 'known mutation');
const run = read('r111-' + prefix + 'mut-' + name + '.json');
...
assert.strictEqual(run.source_head, '356523e6ea8c61c70c6812762aeaab33506de6e2');
...
const inputs = read('r111-' + prefix + 'mut-' + name + '-source.json');
