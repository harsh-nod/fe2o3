...
const helperNames = ['r111-exact-clock-check-mutation.js', 'r111-exact-clock-evidence-tests.js', 'r111-exact-clock-evidence.js',
...
(async () => {
  ...
  await test('foreign cohort and missing predecessors reject', async () => {
    assert.throws(() => c.predecessor('r111-repeat-mut-slot-only.json', 'finished_at'), /clock cohort/);
    assert.throws(() => c.predecessor('r111-exact-clock-does-not-exist.json', 'finished_at'), /ENOENT/);
    assert.throws(() => c.predecessor('r111-exact-clock-does-not-exist.json', 'anything'), /predecessor field/);
  });
  const predecessorName = 'r111-exact-clock-test-parent.json';
  ...
  await test('wrong event source cohort bytes or outcome rejects', async () => {
    for (const patch of [{name: 'r111-exact-clock-other.json'}, {field: 'verified_at'}, {kind: 'restoration'},
