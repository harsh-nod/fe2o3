...
[[ $expected =~ ^[0-9a-f]{64}$ ]]
case "$test_name" in
    kfd_backend::generated_adoption::tests::native::generated_native_cold_full_roster_readback|\
    kfd_backend::generated_adoption::tests::native::generated_native_bootstrap_full_roster_readback|\
...
[[ $scratch == /tmp/fe2o3-c4-harsh-20260917.* && -O $scratch && ! -L $scratch ]]
printf '%s  %s\n' "$expected" "$scratch/runtime" | sha256sum --check
date -u +%FT%T.%NZ
df -B1 /tmp
status=$(timeout --kill-after=5s 20s rocm-smi --showuse --showmeminfo vram --showuniqueid --showbus --json)
printf '%s\n' "$status"
...
pids=$(timeout --kill-after=5s 20s rocm-smi --showpidgpus)
printf '%s\n' "$pids"
awk '
...
timeout --signal=TERM --kill-after=5s 45s env FE2O3_TEST_NATIVE_ISOLATED=1 \
...
date -u +%FT%T.%NZ
timeout --kill-after=5s 20s rocm-smi --showuse --showmeminfo vram --showuniqueid --showbus --json
