 ...
 completion_input_release_body {
    ($syntax:ident, $context:ident, $id:ident, $producer:ident,
     [$($stable_done:tt)*], [$($producer_done:tt)*]) => {
        $syntax!({
            match $context.release_validated_submission_producer_readers_v1($id) {
                Ok(()) => (),
            ...
            let result = $context.release_prevalidated_submission_readers_v1($id);
            $($producer_done)*
            result
        })
    };
...
// The adapter owns prevalidation, absence handling and the unwind boundary.
 completion_selected_reader_release_body {
