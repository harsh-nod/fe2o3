...
def run_owned(command: list[str], timeout: int, out: Path,
    ...
    try:
        ...
        # A second termination signal must not interrupt cleanup of our own group.
        ...
        record.update(finished_ns=time.time_ns(), status=process.returncode)
    ...
    try:
        ...
        os.killpg(process.pid, signal.SIGKILL)
        raise ValueError("command left live process-group members")
