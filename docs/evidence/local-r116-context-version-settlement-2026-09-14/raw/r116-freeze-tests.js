...
const script = path.join(root, 'r116-freeze.js');
...
function fixture(name, mutate, rejects, openingHead = parent, prerequisite = value => value) {
  ...
  for (const [index, [run, command, previous]] of [...p.initialRuns, ...p.prerequisites].entries()) {
    ...
    const record = prerequisite({source_head: parent, source_map_sha256: hash(JSON.stringify(expectedMap)), cwd: repo, command,
      ...
      runner: {name: 'r116-run-v1.js', sha256: hash(helpers.get('r116-run-v1.js'))},
      process_group_cleanup: {timeout: null, close: {status: 'absent', pgid: 42, error: 'Error: kill ESRCH', live_members: [], observation: close}},
      clock: {contract: 'r116-raw-utc-boot-monotonic-v1', error: null, start, finish, source_verified: verified,
  ...
  const fakeFs = {
    ...
    readFileSync: file => {
      if (file.includes('/raw/r115-environment.json')) return JSON.stringify({binaries});
      if (file.includes('/raw/r115-frozen-source.json')) return JSON.stringify(previousMap);
  ...
  function localRequire(name) {
    ...
    if (!name.startsWith('./r116-')) return require(name);
  ...
  if (rejects) {
    ...
    assert.deepStrictEqual(writes.map(([file]) => file), ['frozen-source', 'environment', 'issue-status'].map(n => path.join(root, 'r116-' + n + '.json')));
