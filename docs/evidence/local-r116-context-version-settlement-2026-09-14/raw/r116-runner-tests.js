...
const runner = path.join(root, 'r116-run-v1.js');
...
async function mocked(name, mode) {
  ...
  const fakeProcess = {
    argv: ['node', runner, 'r116-mocked-' + name, '-', 'fixture'], env: {},
  ...
  const record = JSON.parse(writes.get(path.join(root, 'r116-mocked-' + name + '.json')));
...
(async () => {
  ...
  try {
    ...
    const cases = [
      ...
      ['spawn', '/no-such-r116-qualification-command', [], 127],
    ];
    for (const [name, command, args, expected] of cases) {
      const label = 'r116-runner-v1-fixture-' + name;
      let result;
      try {
