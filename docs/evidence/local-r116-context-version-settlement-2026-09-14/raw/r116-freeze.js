...
const accepted = '4756971168f6b4f2c33d217f5d476ccde8ea2740';
...
const environment = {
  ...
  recorded_phase: 'After separately recorded reviewed-source GNU/musl prerequisites; before remaining source/auxiliary/mutation gates. Original isolated and integrated candidate histories are preserved under their original source maps. No native or formal qualification.',
  ...
  prior_artifacts: fs.readdirSync(root).filter(name => /^r116-.*\.(json|log)$/.test(name)).concat(p.isolatedArtifacts).sort()
    .map(name => ({name, sha256: hash(fs.readFileSync(path.join(root, name)))})),
};
const previous = JSON.parse(fs.readFileSync(path.join(repo,
  'docs/evidence/local-r115-returning-control-cleanup-2026-09-14/raw/r115-environment.json')));
...
const previousSource = JSON.parse(fs.readFileSync(path.join(repo,
  'docs/evidence/local-r115-returning-control-cleanup-2026-09-14/raw/r115-frozen-source.json')));
...
for (const [name, value] of [['frozen-source', frozen], ['environment', environment], ['issue-status', issue]]) {
  fs.writeFileSync(path.join(root, 'r116-' + name + '.json'), JSON.stringify(value, null, 2) + '\n', {flag: 'wx'});
}
