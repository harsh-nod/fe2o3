...
spec fn begin_roster_view(roster: Seq<AllocationWriteV1>) -> Seq<logical::AllocationWriteV1> {
    roster.map(|_i, value: AllocationWriteV1| logical::AllocationWriteV1 { allocation: allocation_reference_view(value.allocation), device: device_key_view(value.device), byte_extent: 0 })
}
...
{
    match value { Ok(entry) => Ok(allocation_entry_from(entry)), Err(error) => Err(read_error_embed(error)) }
}
