...
def audit(repo, packet):
    ...
    need(not git(repo, 'diff', '--name-only', '30bc6fdbd493a82bc699ef1af99adc907e99109b', source, '--',
                 'crates/fe2o3-runtime-model/src', 'crates/fe2o3-runtime'), 'unchanged production runtime source')
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-audit-') as temporary:
        ...
        for name, command in cpu.commands():
            ...
            if environment is None:
                ...
                need(target.is_absolute() and target.name == 'target' and target.parent.name.startswith('fe2o3-begin-correspondence-'), 'owned CPU target')
...
def main():
    ...
    if args.selftest:
        ...
        print('PASS: standalone Begin correspondence proof and CPU packet reconstructed from Git objects')
