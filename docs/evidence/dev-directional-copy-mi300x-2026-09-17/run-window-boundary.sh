...
phase() {
    local repetition=$1 backend=$2 bytes=$3 status=0 post_status=0
    printf 'phase repetition=%s backend=%s bytes=%s\n' "$repetition" "$backend" "$bytes"
    admit
    case "$backend" in
        kfd)
            /usr/bin/timeout --signal=TERM --kill-after=5s 180s prlimit --core=0:0 -- "$owned/target/release/examples/gfx942-runtime-directional-window-benchmark" 0xab83d2ffef0d3cdf "$bytes" 3 10 || status=$?
            ;;
        hsa)
            HSA_XNACK=0 ROCR_VISIBLE_DEVICES=1 /usr/bin/timeout --signal=TERM --kill-after=5s 180s prlimit --core=0:0 -- "$owned/async-copy-hsa" 0 "$bytes" 1 3 10 0xab83d2ffef0d3cdf || status=$?
            ;;
        hip)
            HSA_XNACK=0 HIP_VISIBLE_DEVICES=1 /usr/bin/timeout --signal=TERM --kill-after=5s 180s prlimit --core=0:0 -- "$owned/async-copy-hip" 0 "$bytes" 1 3 10 0xab83d2ffef0d3cdf || status=$?
            ;;
        *) return 64 ;;
    esac
    printf 'completed repetition=%s backend=%s bytes=%s exit=%s\n' "$repetition" "$backend" "$bytes" "$status"
    # HSA's untimed allocation/free probe can leave deferred VRAM reclamation.
    sleep 20
    admit || post_status=$?
    printf 'postflight repetition=%s backend=%s bytes=%s exit=%s\n' "$repetition" "$backend" "$bytes" "$post_status"
    ((status == 0)) || return "$status"
    return "$post_status"
}

printf 'context diagnostic=shared-host-window-boundary git_commit=ed5b5d64bf95116c21e9bf350c30132ff2bbb524 gpu=1 unique_id=0xab83d2ffef0d3cdf byte_sizes=264239136,264239137 depth=1 warmups=3 samples=10 repetitions=3 optional_context_journal=disabled\n'
for repetition in 1 2 3; do
    case "$repetition" in
        1) sizes=(264239136 264239137); order=(kfd hsa hip) ;;
        2) sizes=(264239137 264239136); order=(hsa hip kfd) ;;
        3) sizes=(264239136 264239137); order=(hip kfd hsa) ;;
    esac
    for bytes in "${sizes[@]}"; do
        for backend in "${order[@]}"; do
            phase "$repetition" "$backend" "$bytes"
        done
    done
done
