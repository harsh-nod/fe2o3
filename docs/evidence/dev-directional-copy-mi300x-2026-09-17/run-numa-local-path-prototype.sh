...
phase() {
    ...
    case "$backend" in
        kfd)
            /usr/bin/timeout --signal=TERM --kill-after=5s 180s prlimit --core=0:0 -- numactl --physcpubind=0-47 --membind=0 "$owned/target/release/examples/gfx942-runtime-directional-window-benchmark" 0xab83d2ffef0d3cdf 268435456 3 10 || status=$?
            ;;
        hsa)
            HSA_XNACK=0 ROCR_VISIBLE_DEVICES=1 /usr/bin/timeout --signal=TERM --kill-after=5s 180s prlimit --core=0:0 -- numactl --physcpubind=0-47 --membind=0 "$owned/async-copy-hsa" 0 268435456 1 3 10 0xab83d2ffef0d3cdf || status=$?
            ;;
        hip)
            HSA_XNACK=0 HIP_VISIBLE_DEVICES=1 /usr/bin/timeout --signal=TERM --kill-after=5s 180s prlimit --core=0:0 -- numactl --physcpubind=0-47 --membind=0 "$owned/async-copy-hip" 0 268435456 1 3 10 0xab83d2ffef0d3cdf || status=$?
            ;;
        *) return 64 ;;
    esac
...
numactl --version
numactl --physcpubind=0-47 --membind=0 numactl --show
