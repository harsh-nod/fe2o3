...
bundle = root / (label + '-source.bundle')
assert bundle.is_file()
status = root / (label + '-controller.json')
...
status.write_text(json.dumps(record, indent=2) + '\n')
print(json.dumps(record), flush=True)
with (root / (label + '-capture.tar')).open('xb') as capture, (root / (label + '-transport.log')).open('xb') as errors:
