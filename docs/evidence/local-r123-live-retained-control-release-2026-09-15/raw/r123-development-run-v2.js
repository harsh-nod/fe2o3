...
const parent = 'd1c87064bd10e513f5aad328e0b3445bd78c296a';
const [name, previousName, command, ...args] = process.argv.slice(2);
assert(/^r123-development-[a-z0-9-]+$/.test(name) && previousName && command);
...
(async () => {
  ...
  let previous = null;
  if (previousName !== '-') {
    assert(/^r123-development-[a-z0-9-]+\.json$/.test(previousName));
    ...
    assert.strictEqual(record.clock.contract, 'r123-development-raw-utc-boot-monotonic-v1');
  ...
  const declaredFullCommand = name === 'r123-development-gnu-all-v1' ? fullCommand
    : name === 'r123-development-musl-all-v1' ? [...fullCommand, '--target', 'x86_64-unknown-linux-musl']
    : null;
  if (declaredFullCommand) {
    ...
    assert.strictEqual(hash(JSON.stringify(before)),
      '8380746325d6717df78a608eb5be8f3ff3c48e42f32eec19aba2ab0677fdcb08', 'declared full-test source');
  }
  const deadline = declaredFullCommand ? 3600000
    : ['r123-development-source-campaign', 'r123-development-auxiliary-campaign'].includes(name) ? 7200000 : 1800000;
  ...
  const record = {source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    clock: {contract: 'r123-development-raw-utc-boot-monotonic-v1', predecessor: previous, start, finish: result.finish, source_verified: verified, error: clockError}};
