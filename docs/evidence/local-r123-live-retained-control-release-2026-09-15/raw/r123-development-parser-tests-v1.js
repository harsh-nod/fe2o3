...
const helper = root + 'r123-development-check-v1.js';
...
const validate = (record, run = spec, runner = C.p.runner) => C.E.checkRecord(record, run.name,
  run.command, C.map, run.predecessor, 0, {read: C.json, bytes: C.bytes}, {
    head: C.p.source_parent, cwd: C.repo, contract: 'r123-development-raw-utc-boot-monotonic-v1',
    runner, after: C.map,
  });
...
for (const [name, edit] of [
  ...
  ['wrong runner version', value => value.runner = {name: 'r123-development-run-v1.js', sha256: C.p.runners['r123-development-run-v1.js']}],
...
const focused = C.p.anchors.find(anchor => anchor.passed === 128);
