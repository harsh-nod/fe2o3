// Read-only R123 CPU/test qualification. Native execution and formal correspondence remain separate.
...
const q = JSON.parse(initialPin('r123-development-qualification-plan-v1.json',
...
assert.strictEqual(q.source_identities, 5703);
...
assert.strictEqual(bundle.accepted_baseline.path,
  'docs/evidence/local-r122-live-data-release-2026-09-15/raw/r122-development-clippy-v2-source.json');
const baselineMap = JSON.parse(C.git(['show', p.source_parent + ':' + bundle.accepted_baseline.path]));
assert.strictEqual(hash(JSON.stringify(baselineMap)), '178fa1fb2c9d3267440dbf36c50e4d79916e66fb2765446d7e1fa8acb8fc90c4');
assert.strictEqual(bundle.accepted_baseline.source_map_sha256, hash(JSON.stringify(baselineMap)));
assert.strictEqual(Object.keys(baselineMap).length, 5700);
...
assert.strictEqual(delta.length, 19);
...
assert.deepStrictEqual(JSON.parse(snapshot.log), {source_snapshot_only: true,
  output: root + q.source_bundle, files: 19, new_files: 3, sha256: hash(bytes(q.source_bundle)),
...
const gateCalibration = record(q.gate_calibration); calibration(q.gate_calibration, gateCalibration);
assert.strictEqual(calibrationCount, q.helper_calibrations);
assert.strictEqual(calibrationCount, 134);
let previousGate = gateCalibration.record;
...
for (const item of gateReport.artifacts) {
  const flat = path.dirname(item.file) === root.slice(0, -1) &&
    (path.basename(item.file).startsWith('r123-development-') || Object.hasOwn(q.inputs, path.basename(item.file)));
...
assert.strictEqual(q.mutations.length, 10);
...
assert.strictEqual(distinctMaps.size, 10);
assert.deepStrictEqual(q.restored.map(spec => spec.passed), [128, 17]);
const restoredNames = [];
...
assert.strictEqual(restoredNames.length, 145);
assert.strictEqual(new Set(restoredNames).size, 145);
...
console.log(JSON.stringify({development_only: true, publication_pending: true, scope: q.scope,
  ...
  source_bundle: {name: q.source_bundle, sha256: hash(bytes(q.source_bundle)), changed: 19, added: 3},
  gnu_passed: 2865, musl_passed: 2865, ignored_each: 5, libtest_targets_each: 48, csv_targets_each: 1,
  source_gates: 17, auxiliary_checks: 10, compiled_runtime_negatives: 10, distinct_mutant_maps: 10,
  helper_calibrations: 134, restored_suites: [128, 17], runs, mutations, raw_artifacts: raw,
