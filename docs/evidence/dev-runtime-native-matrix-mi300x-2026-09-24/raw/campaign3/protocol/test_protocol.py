...
def transcript(case):
    summary = "test result: ok. 1 passed; 0 failed; 0 ignored; 0 measured; 1384 filtered out; finished in 0.01s\n"
...
class ProtocolTests(unittest.TestCase):
    def test_suffix_is_only_the_sixteen_unexecuted_cases(self):
        self.assertEqual(N.selected_cases(P), P.CASE_NAMES[6:])
        self.assertEqual(len(N.selected_cases(P)), 16)
        self.assertFalse({name for name, _ in N.selected_cases(P)} & {name for name, _ in P.CASE_NAMES[:6]})
    ...
    def test_exact_roster_and_case_environments(self):
        self.assertEqual((len(P.CASE_NAMES), len(P.TESTS), len(set(P.TESTS.values()))), (22, 22, 20))
        source = (REPO / "docs/evidence/dev-topology-link-directory-cpu-2026-09-24/raw/cpu2/commands/musl-fe2o3-runtime/stdout").read_text()
        ...
        for case in P.TESTS:
            ...
            self.assertEqual("FE2O3_TEST_NATIVE_ISOLATED" in env, case.startswith(("generated-", "cold-")))
    ...
    def test_all_22_transcripts(self):
        self.assertEqual(sum(P.transcript(case, *transcript(case))["harness_passes"] for case in P.TESTS), 24)

    def test_transcript_mutations(self):
        for case in P.TESTS:
            stdout, stderr = transcript(case)
            for text, err in ((stdout.replace("1384 filtered", "1383 filtered"), stderr),
    ...
    def test_historical_output_as_parser_calibration_only(self):
        ...
        for case, number in (("cold-device", 0), ("cold-host", 1), ("device-promotion", 6), ("zero-cache", 7)):
            ...
            text = re.sub(r"[0-9]+ filtered out", "1384 filtered out", text)
