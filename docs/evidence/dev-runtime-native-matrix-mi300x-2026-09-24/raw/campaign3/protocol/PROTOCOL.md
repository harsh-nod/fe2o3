...
This frozen controller runs only the sixteen previously unexecuted cases,
`protocol.py`'s ordered roster starting at index six. Its `complete_matrix`
field refers to that explicit suffix, not the original 22-invocation matrix.
The earlier `campaign2` remains rejected: six test commands passed, but the
primary-panic immediate observer refused its first 1%-busy sysfs sample.
The delayed pass does not override that refusal. There is no retry of that
cell, no relaxation of observation policy and no aggregate full-matrix claim.
`campaign` separately retains the initial local prelaunch helper-pin failure.
...
auxiliary-allocation failure expands to prefixes 0, 1 and 2 in the original
22-command plan. This separate suffix contains sixteen top-level commands and
sixteen libtest frames; all terminal-retention cases remain in `campaign2`.
