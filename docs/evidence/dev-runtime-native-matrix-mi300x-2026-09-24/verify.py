#!/usr/bin/env python3
"""Replay native assertions, the rejected first cohort and accepted suffix offline."""
...
ORIGINAL_HERE = ORIGINAL_REPO / "docs/evidence/dev-runtime-native-matrix-mi300x-2026-09-24"
PRIVATE = Path("/home/harsh/.codex-tmp/fe2o3-runtime-native-20260924-6Bl3TK87")
...
def signed_source(build, p, m):
    ...
    for name, path in (("cpu-source", m.CPU + "/raw/cpu2/inputs-before.json"),
                       ("cpu-roster", m.CPU + "/raw/cpu2/commands/musl-fe2o3-runtime/stdout")):
...
def verify_build(build, campaign, p, m):
    ...
    need(value["commit"] == p.COMMIT and value["cpu_source_inputs"] == 3956, "qualified current build")
    ...
    need(read(build / "commands/cpu-source/stdout")["source"] == inputs and len(inputs) == 3956, "complete prior CPU source map")
    ...
    specs = [
        ...
        ("cpu-source", [*m.GIT, "show", p.COMMIT + ":" + m.CPU + "/raw/cpu2/inputs-before.json"], REPO, m.GIT_ENV, 30),
        ("cpu-roster", [*m.GIT, "show", p.COMMIT + ":" + m.CPU + "/raw/cpu2/commands/musl-fe2o3-runtime/stdout"], REPO, m.GIT_ENV, 30),
...
def verify_campaign(root, name, rejected):
    ...
    if not rejected:
        need(n.CASE_START == 6 and n.EXPECTED_CASES == 16, "separate explicit unexecuted suffix")
...
def verify_prelaunch(root):
    attempt = root / "campaign"
    before = read(attempt / "protocol-before.json")
    for name, digest in before.items():
        need(sha(attempt / "protocol" / name) == digest, "retained prelaunch protocol")
    p, m, n, c = [module(attempt / "protocol" / (name + ".py"), before[name + ".py"], "prelaunch_" + name)
                   for name in ("protocol", "prepare", "native", "campaign")]
    need(set(before) == set(c.PROTOCOL) and set(inventory(attempt / "payload")) == n.PAYLOAD,
         "complete prelaunch captures")
    need({path.name for path in attempt.iterdir()} == {"protocol", "protocol-before.json", "payload", "commands"}
         and {path.name for path in (attempt / "commands").iterdir()} == {"protocol-tests"},
         "prelaunch rejection preceded owner creation and remote commands")
    command(attempt / "commands/protocol-tests", ["/usr/bin/python3", "-I", "-B", str(ORIGINAL_HERE / "test_protocol.py")],
            ORIGINAL_REPO, m.GIT_ENV, 120)
    payload = inventory(attempt / "payload")
    pins = {"native.py": before["native.py"], "protocol.py": before["protocol.py"], "base.py": p.BASE_SHA,
            "recorder.py": p.RECORDER_SHA, "observer.py": p.OBSERVER_SHA,
            "runtime-tests": sha(root / "build/runtime-tests")}
    need(all(payload[name] == digest for name, digest in pins.items()), "original prelaunch payload identities")
    need(n.TOPOLOGY_SHA == "669a25592e8ba72743bed9c0a3ced39ad392f104746c2ca04a7f2867ed8d7ca3"
         and payload["topology.py"] == "f16f8415badc3b44b72b840fe1c3191afc8dea4b577548e45b020f9b99768714",
         "original stale topology pin fails before remote launch")


def verify(root):
    verify_prelaunch(root)
    prefix = verify_campaign(root, "campaign2", True)
    suffix = verify_campaign(root, "campaign3", False)
    need(prefix == suffix, "same source, ELF, complete roster, device and shared helper identities across cohorts")
    need(prefix["tests"] == dict(prefix["roster"]), "case roster matches executed test-name map")
    need(len(prefix["roster"]) == 22 and len(set(name for name, _ in prefix["roster"])) == 22
         and len(set(test for _, test in prefix["roster"])) == 20, "exact 22 variants of twenty distinct tests")
    ordered = [root / "build/commands/after-cargo", root / "campaign/commands/protocol-tests",
               root / "campaign2/commands/protocol-tests", root / "campaign2/commands/absence",
               root / "campaign3/commands/protocol-tests", root / "campaign3/commands/absence"]
    ...
    need(all(left["finished_ns"] <= right["started_ns"] for left, right in zip(receipts, receipts[1:])),
         "cold build, retained prelaunch attempt and two cohorts are sequential")
    return {"native_commands": 22, "harness_passes": 24, "endpoint_observations": 66, "strict_admitted": 65, "remote_commands": 92,
            "local_build_commands": 11, "local_campaign_commands": 16, "local_prelaunch_commands": 1,
            "cpu_tests": 1365, "cpu_hardware_ignores": 20,
            "whole_elf_retained": True, "record_replay": "pass", "original_matrix_accepted": False,
            "separate_suffix_accepted": True, "refused_observations": 1, "formal_refinement": False, "performance_acceptance": False}
