...
[[ $(wc -l < "$archive/source-files.list") == 18 ]]
...
for receipt in "$archive"/raw/*.command; do
    ...
    case "$name" in
        exploratory-readiness)
            [[ $(< "$archive/raw/$name.exit") == 134 ]]
            rg -Fq 'assertion `left == right` failed' "$archive/raw/$name.log"
            rg -Fq 'panic in a destructor during cleanup' "$archive/raw/$name.log"
            ;;
        exploratory-domain-faults)
            [[ $(< "$archive/raw/$name.exit") == 101 ]]
            rg -Fq 'field `domain` of struct `reservation::RuntimeAsyncReservedTicketV1` is private' "$archive/raw/$name.log"
            ;;
        exploratory-clippy)
            [[ $(< "$archive/raw/$name.exit") == 101 ]]
            rg -Fq 'clippy::result_large_err' "$archive/raw/$name.log"
            rg -Fq 'clippy::let_underscore_future' "$archive/raw/$name.log"
            ;;
        *) success "$name" ;;
    esac
done
...
for target in gnu musl-no-hip; do
    ...
    for crate in host runtime; do
        ...
        if [[ $crate == host ]]; then count=275; passes=271; ignores=4; else count=932; passes=915; ignores=17; fi
...
awk '/^test result:/ { if ($0 !~ /^test result: ok\. (3|16|39) passed; 0 failed; 0 ignored; 0 measured; 0 filtered out;/ || seen[$4]++) exit 1; rows++ }
    END { if (rows != 3 || !seen[3] || !seen[16] || !seen[39]) exit 1 }' "$archive/raw/doctests.log"
rg -q '^test result: ok\. 5 passed; 0 failed; 1 ignored; 0 measured; 0 filtered out;' "$archive/raw/unsafe-policy.log"
printf 'PASS: exact frozen C5 source and GNU/musl-no-hip CPU rosters; host 271+4 ignored, runtime 915+17 ignored per target; quality gates and 58 doctests. No native integration, formal or milestone acceptance.\n'
