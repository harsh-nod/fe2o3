"""Retain exact borrowed coherent initialization source gates, not native or proof acceptance."""
import hashlib
import json
...
root = repo / 'docs/evidence/local-r78-borrowed-coherent-2026-09-10'
raw = root / 'raw'
attempt = sys.argv[1]
...
assert base == '0193b8fa0c26ecef072acafd72979484d74d44e9'
...
inventory = (temporary / 'r78-proof-inventory.log').read_text()
assert 'PASS:' in inventory and 'FAIL:' not in inventory
metadata = (temporary / 'r78-production-audit.log').read_text()
...
summary.update(base_commit=base, final_gate_attempt=attempt,
    ...
    production_metadata_packages=43, new_cpu_test_functions=10,
    new_runtime_compile_fail_doctests=0, new_host_negative_binaries=0,
    new_host_compile_only_examples=0, borrowed_coherent_initialization_cpu_validated=True,
...
files = list(temporary.glob('r78-*.log'))
files += list(temporary.glob('r78-*-source-gate.json'))
files += list(temporary.glob('r78-*-source-inputs.json'))
files += [temporary / name for name in ('r78-source-gate.py', 'r78-retain-local.py',
    'r78-production-metadata.json', 'r78-auxiliary-gates.py', 'r78-doc-links.py',
    'r78-focused.py')]
for path in files:
