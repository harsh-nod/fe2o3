"""Serial current-source borrowed coherent initialization gates; no hardware execution."""
import hashlib
import json
import os
...
root = repo.parent
attempt = sys.argv[1]
assert re.fullmatch(r'r78-[a-z0-9]+', attempt)
crates = ['fe2o3-completion', 'fe2o3-runtime-model', 'fe2o3-resource-accounting',
          'fe2o3-kfd', 'fe2o3-runtime']
packages = [arg for crate in crates for arg in ('-p', crate)]
tests = ['cargo', '+nightly-2026-04-03', 'test', '--locked', '--offline',
         '--no-fail-fast', *packages, '--all-features']
musl = ['--target', 'x86_64-unknown-linux-musl']
lint = ['cargo', '+nightly-2026-04-03', 'clippy', '--locked', '--offline',
        *packages, '-p', 'fe2o3-host', '-p', 'fe2o3-macros']
gates = [
    ...
    ('musl-host-docs', ['cargo', 'test', '--locked', '--offline', '-p',
        'fe2o3-host', '--doc', *musl], repo),
    ('gnu-host', ['cargo', 'test', '--locked', '--offline', '-p',
        'fe2o3-host', '--all-features', '--lib'], repo),
    ('musl-host', ['cargo', 'test', '--locked', '--offline', '-p',
        'fe2o3-host', '--lib', *musl], repo),
    ('macro-fixtures', ['cargo', 'test', '--locked', '--offline', '-p',
        'fe2o3-macros', '--test', 'typed_kernel_fixtures'], repo),
    ...
    ('python', ['python3', '-B', '-m', 'unittest', 'test_run_r26_inplace_runner',
        ...
        'test_check_scale3_protocol', 'test_check_drain_capture', 'test_run_drain_capture'],
        repo / 'benchmarks/runtime_gfx942'),
...
def source_identities():
    paths = subprocess.check_output(['git', 'ls-files', '--cached', '--others',
        '--exclude-standard', '-z'], cwd=repo).split(b'\0')
    return {name: hashlib.sha256((repo / name).read_bytes()).hexdigest()
        for name in sorted({path.decode() for path in paths if path
            and not path.startswith(b'docs/')})}


before = source_identities()
(root / f'{attempt}-source-inputs.json').write_text(json.dumps(before, indent=2) + '\n')
...
for name, command, cwd in gates:
    ...
    with log.open('xb') as output:
        run = subprocess.run(command, cwd=cwd, env=env, stdout=output,
            stderr=subprocess.STDOUT, timeout=1800)
    result = dict(name=name, command=command, cwd=str(cwd), log=str(log),
...
assert source_identities() == before, 'source changed during final gates'
