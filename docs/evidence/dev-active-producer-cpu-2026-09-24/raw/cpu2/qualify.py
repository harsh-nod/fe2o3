...
BASELINE_SHA = {
    "inputs-before.json": "4ffa4cefdf7c7a9003ec9ac29ad5316dbb858dc1c3916beddba30e246909712d",
    "inputs-after.json": "4ffa4cefdf7c7a9003ec9ac29ad5316dbb858dc1c3916beddba30e246909712d",
    "commands/gnu-fe2o3-runtime/stdout": "e4ecaa9784c6cad57f2be7a35c1a3dcbe2052fdbbfa9e30da6cd858026966cb2",
    "commands/musl-fe2o3-runtime/stdout": "c7536a96cd66acdf9bfc3cf65e2c7f713f59b84215d708c89ed0761e600c0338",
}


def baseline(name):
    path = BASE / "raw/campaign1" / name
    need(R.sha(path) == BASELINE_SHA[name], "baseline input identity")
    return path
...
def verify(output):
    V.inventory(output)
    need({path.name for path in output.iterdir()} == {"inputs-before.json", "inputs-after.json", "commands", "qualify.py"},
         "exact campaign tree")
    ...
    need(prior == V.read(baseline("inputs-after.json"))["source"], "baseline source bracket")
    ...
    for label in ("gnu", "musl"):
        ...
        need(focused == {name: result for name, result in full.items() if "producer_launch" in name}, "exact focused roster")
        old, _ = V.test_roster(baseline("commands/" + label + "-fe2o3-runtime/stdout").read_text(),
                               [1392, 0, 20, 0, 0])
    ...
    print("active-producer CPU replay: pass; 13 stages; 35 focused; 1400 runtime +20 hardware ignores per target; "
          "46 doctests; native/formal/performance: not established", flush=True)
...
def main():
    ...
    before = inputs()
    with (output / "qualify.py").open("xb") as retained:
        retained.write(Path(__file__).read_bytes())
    c = R.helpers()
