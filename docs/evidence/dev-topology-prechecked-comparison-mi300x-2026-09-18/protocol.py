...
PREFIX = "/home/harsh/fe2o3-topology-prechecked-comparison-20260918."
LOCAL_PREFIX = "fe2o3-topology-prechecked-comparison-20260918."
B.PREFIX = PREFIX
...
PROVIDER_SHA = "88648e9aa06e8508d711c20cda0a22f83154cdbcd689b09aa4832cac57f8044f"
T = load_pinned(HERE / "provider.py", PROVIDER_SHA, "topology_provider_parser")
COHORTS = {
    "baseline": {
        "commit": "fa9413425531458f9ac6b1ceb41d96912a9ae2d4",
        ...
        "tree": "8695e89497a4ce89d42c4cc20f9eed9ffe6c4ac9",
        "tar_sha256": "dbcdc6128c2ca97ce5e46b4cfcf3a1e3e0d5fbeca2cb04f455b4ba4e1e787cbc",
        "tar_bytes": 11965419,
        "files": 5572,
    },
    "candidate": {
        "commit": "f43c352602296fa548faaff845477c1526f10aae",
        "cpu": "docs/evidence/dev-topology-prechecked-reads-cpu-2026-09-18",
        "seal": "d40b72e3456b0fc36a2bb4d51cb29549d4dcb3eaf677c4c56b72393b5bae55b0",
        "source_snapshot": "9cf73b317f1e2bc3a357d6f8f156f1792cdface87b0ab8ec94b12eb3e79a0a2b",
        "file_map_sha256": "62e01514b0fdaf5f4685ea5a5ce1cbbbf26d40fbeff3cc7e927a45c4b694d960",
        "modes_sha256": "d1f3346848c7540f477393ba412e5eef68348046313079fa24f2b78ce0cc7edb",
        "tree": "2a4126d59bf99306404a6cb69955c7c272750711",
        "tar_sha256": "9736e2b873127ee416c329780ce1aa1eb2d911a440748b34acf04e95802898f2",
        "tar_bytes": 11968550,
        "files": 5573,
    },
}
...
ADDED = {"crates/fe2o3-kfd/src/topology/tests/prechecked_reads.rs"}
CHANGED = {"crates/fe2o3-kfd/src/topology.rs"}
STATIC_INPUTS = (
    "protocol.py",
    "provider.py",
    "campaign.py",
...
BINARY_PATHS = {
    **{
        name: "target-" + name + "/release/examples/gfx942-runtime-xgmi-peer-benchmark"
        for name in COHORTS
    },
    "topology": "target-candidate/release/examples/kfd-topology",
}


def binary(name):
    return BINARY_PATHS[name]
...
def provider_specs(owned):
    return [
        (
            "topology-probe" + str(index),
            [str(owned / binary("topology"))],
            30,
            owned / "source-candidate",
            environment(owned, "candidate"),
        )
...
def provider_observation(root):
    outputs, observations = [], []
    for name, *_ in provider_specs(Path("/unused")):
        folder = root / name
        need((folder / "stderr").read_bytes() == b"", "empty provider stderr")
        data = (folder / "stdout").read_bytes()
        observations.append(T.parse(data, C.DEVICE_ROSTER))
        outputs.append(data)
    need(outputs[0] == outputs[1], "two identical provider summaries")
    need(observations[0] == observations[1], "same parsed provider observation")
...
def validate_binding(binding):
    ...
    need(
        binding["schema"] == "fe2o3.topology-prechecked-source-comparison.v1",
        "comparison schema",
    )
    ...
    for name in ("protocol.py", "provider.py", "campaign.py"):
        need(
