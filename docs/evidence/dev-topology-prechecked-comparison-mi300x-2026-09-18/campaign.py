...
ROOT = HERE.parents[2]
PROTOCOL_SHA = "8cb41d1d4e0e943c6693b16313bddce23a470e31d57e95922406bd9d20131c7a"
...
def binary_observation(owned):
    return {name: sha(owned / P.binary(name)) for name in P.BINARY_PATHS}


def check_binaries(owned, expected):
    need(binary_observation(owned) == expected, "unchanged complete binary roster")
...
def execute_measurements(rec, owned, selected, identities, results):
    # No queue workload is reachable until both read-only probes agree.
    for name, command, seconds, cwd, env in P.provider_specs(owned):
        identities()
        rec.cwd = cwd
        folder = rec.run(name, command, seconds, env=env)
        need((folder / "stderr").read_bytes() == b"", "empty provider stderr")
        P.T.parse((folder / "stdout").read_bytes(), P.C.DEVICE_ROSTER)
        identities()
    B.write_json(rec.output / "topology.json", P.provider_observation(rec.output))
    specs = {
        name: (command, seconds, cwd, env)
        for name, command, seconds, cwd, env in P.remote_specs(owned, selected)
    }
    for phase in P.PHASES:
        identities()
        execute_phase(rec, specs, selected, phase, results)
...
def run_remote(marker):
    ...
    try:
        ...
        binaries = binary_observation(owned)
        B.write_json(rec.output / "binaries.json", binaries)
        execute_measurements(rec, owned, binding["devices"], identities, results)
    except BaseException as error:
        failure = error
    finally:
