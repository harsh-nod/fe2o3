...
CAMPAIGN_SHA = "f6f79e527249b3c8d4ba02e95644114e930ba9979b6ee5ceb5df088723fb4aba"
CALIBRATION_TESTS = 37
...
def verify(root=HERE):
    ...
    need(
        set(binaries) == set(P.BINARY_PATHS)
        and all(
    ...
    topology = P.provider_observation(remote)
    need(
        P.same_json(topology, read(remote / "topology.json")),
        "independent read-only provider replay",
    )
    parsed = {}
    ...
    return {
        ...
        "binary_digests_distinct": len({binaries[name] for name in P.COHORTS}) == 2,
        "provider_observations": 2,
        "devices": selected,
