"""Serial integration gates for OVL-DIAG-1 and MEM-BASE."""
import json
import os
...
attempt = sys.argv[1] if len(sys.argv) > 1 else 'r66diag'
assert attempt in ('r66diag', 'r66diag-final')
...
gates = [
    ...
    ('python', ['python3', '-B', '-m', 'unittest', 'test_run_r26_inplace_runner',
        ...
        'test_check_scale3_protocol'], repo / 'benchmarks/runtime_gfx942'),
    ('fmt', ['cargo', 'fmt', '--all', '--', '--check'], repo),
    ('whitespace', ['git', 'diff', '--check'], repo),
]
results = []
env = dict(os.environ, CARGO_BUILD_JOBS='4', PYTHONDONTWRITEBYTECODE='1')
