...
commands = [
    ...
    ('production-audit', ['python3', '-B', 'scripts/runtime_pure_rust_audit.py', 'metadata',
        '--input', str(root / 'r103-production-metadata.log'), '--root', 'fe2o3-runtime']),
]
def identities():
...
before = identities()
assert before == json.loads((root / 'r103-frozen-source.json').read_text())
with (root / 'r103-auxiliary-source-inputs.json').open('x') as output:
    json.dump(before, output, indent=2)
results = []
for name, command in commands:
    log = root / f'r103-{name}.log'
    ...
    results.append(row)
    (root / 'r103-auxiliary-results.json').write_text(json.dumps(results, indent=2) + '\n')
    print(json.dumps(row), flush=True)
    if result.returncode:
        raise SystemExit(result.returncode)
after = identities()
with (root / 'r103-auxiliary-source-after.json').open('x') as output:
    json.dump(after, output, indent=2)
assert after == before, 'source changed during auxiliary gates'
with (root / 'r103-auxiliary-complete.json').open('x') as output:
