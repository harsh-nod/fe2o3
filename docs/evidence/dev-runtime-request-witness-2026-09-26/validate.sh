...
run focused timeout 1800 cargo test "${packages[@]}" --all-features --lib --locked --offline \
    -- composed_request composed_policy multi_device_composition required_empty_roster \
    retained_charge_context request_account_variants generated_plan_rejects rooted_n1_constructor \
    context::generated_preparation::tests::shell_tests \
    context::tests::allocation_outcome_tests --test-threads=2
