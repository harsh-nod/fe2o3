...
fn generated_native_inputs_borrow_original_program_and_complete_bytes_across_control_transfer() {
    ...
    let expected_abi =
        fe2o3_amdhsa_loader::validate(&hsaco, fe2o3_amdhsa_loader::AdmittedProfile::Gfx942XnackOffCov6)
            .unwrap()
            .bind_kernel("vecadd")
            .unwrap()
...
fn forbidden_buffer_escape<E>(
    source: &RuntimeGfx942GeneratedSourceV1<'_, E>,
    expected: &crate::generated_source::GeneratedHostRosterV1,
) {
    let mut escaped = None;
    let _ = source.with_native_inputs_v1(7, expected, |_, buffers| {
        escaped = Some(buffers);
        Ok(())
    });
    drop(escaped);
}

fn forbidden_program_escape<E>(
    source: &RuntimeGfx942GeneratedSourceV1<'_, E>,
    expected: &crate::generated_source::GeneratedHostRosterV1,
) {
    let mut escaped = None;
    let _ = source.with_native_inputs_v1(7, expected, |program, _| {
        escaped = Some(program);
        Ok(())
    });
    drop(escaped);
}

fn forbidden_owner_result<E>(
    source: &RuntimeGfx942GeneratedSourceV1<'_, E>,
    expected: &crate::generated_source::GeneratedHostRosterV1,
) {
    let _ = source.with_native_inputs_v1(7, expected, |_, _| Ok(vec![1u8]));
}
