...
root=$(cd -- "$archive/../../.." && pwd)
baseline="$root/docs/evidence/dev-r126-auxiliary-release-2026-09-16/final"
cd -- "$root"
...
for target in gnu musl; do
    ...
    for crate in kfd runtime; do
        ...
        if [[ $crate == kfd ]]; then
            run "$target-kfd-baseline-binary" cmp "$archive/final/$target-kfd-binary.sha256" "$baseline/$target-kfd-binary.sha256"
            run "$target-kfd-baseline-roster" cmp "$archive/final/$target-kfd-roster.log" "$baseline/$target-kfd-roster.log"
        else
