...
const parent = 'f6d3fb50e4fd1eb6949d4a29f55c79432a5989e1';
const [name, previousName, command, ...args] = process.argv.slice(2);
assert(/^r122-development-[a-z0-9-]+$/.test(name) && previousName && command);
...
(async () => {
  ...
  let previous = null;
  if (previousName !== '-') {
    assert(/^r122-development-[a-z0-9-]+\.json$/.test(previousName));
    ...
    assert.strictEqual(record.clock.contract, 'r122-development-raw-utc-boot-monotonic-v1');
  ...
  const deadline = ['r122-development-source-campaign', 'r122-development-auxiliary-campaign'].includes(name) ? 7200000 : 1800000;
  let timedOut = false;
  ...
  const record = {source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    clock: {contract: 'r122-development-raw-utc-boot-monotonic-v1', predecessor: previous, start, finish: result.finish, source_verified: verified, error: clockError}};
