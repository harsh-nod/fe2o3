// Local R122 gate validation only. Mutation, native and formal acceptance remain separate.
const fs = require('fs'), path = require('path'), assert = require('assert'), crypto = require('crypto');
const root = '/home/harsh/.codex-tmp', checker = path.join(root, 'r122-development-check-v2.js');
const sha = value => crypto.createHash('sha256').update(value).digest('hex');
const checkerHash = '3a9c56f38d9ffcb9df09cd4980cab107a4fd536bc24d858dfa520838c519b81e';
...
const gates = JSON.parse(pin('r122-development-gates-v2.json', '3387a723ba5e175d26321f4b965a12fa1b41bc16759f66465cd998435b9c33d9'));
assert.strictEqual(gates.accepted, false);
assert.strictEqual(gates.source_parent, C.p.source_parent);
assert.strictEqual(gates.accepted_commit, gates.source_parent);
assert.strictEqual(gates.source_map_sha256, C.p.source_map_sha256);
assert.strictEqual(gates.source_count, C.p.source_count);
assert.deepStrictEqual(json(gates.source_map), map);
assert.strictEqual(gates.runner, C.p.runner);
assert.strictEqual(gates.runner_sha256, C.p.runner_sha256);
assert.strictEqual(gates.mutation_plan, 'r122-development-mutations-v1.json');
pin(gates.mutation_plan, gates.mutation_plan_sha256);
assert.deepStrictEqual(gates.counts, C.p.counts);
assert.strictEqual(gates.gates.length, 25);
assert.strictEqual(gates.full_runs.length, 2);
for (const [file, digest] of Object.entries(gates.accepted_inputs)) {
  assert.strictEqual(sha(git(['show', gates.accepted_commit + ':' + file])), digest);
}
