...
function negative(mutation) {
  ...
  assert.strictEqual(changed[mutation.patch.path], mutation.expected_file_sha256);
  baseline();
  const name = 'r122-development-final-negative-' + mutation.id + '-v1';
  const {record, log} = completed(name, [...p.test_commands[mutation.kind], mutation.test, '--', '--exact'], p.baseline.name + '.json', changed, 101);
  ...
  assert.strictEqual(header[3], mutation.oracle_location, 'predeclared behavioral assertion');
  ...
  assert.strictEqual((log.match(/^     Running unittests /gm) || []).length, 1);
...
module.exports = {p, E, root, repo, hash, bytes, json, pin, git, map, identities, completed, summaries, assertFull, baseline, negative, stable, artifacts};
