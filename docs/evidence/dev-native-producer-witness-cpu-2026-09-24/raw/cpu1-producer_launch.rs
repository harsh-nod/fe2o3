...
fn native_producer_chain(published: bool) {
    ...
    // Observe only the consumer; Context must reconcile its retained producer.
    while context.query_submission(&producer).unwrap() != RuntimeCompletionStatusV1::Succeeded {
        assert!(
    ...
    loop {
        ...
        match context.poll(&mut consumer).unwrap() {
        ...
        .enumerate()
    {
        ...
        digests.push(format!("{:x}", Sha256::digest(&observed)));
    }
