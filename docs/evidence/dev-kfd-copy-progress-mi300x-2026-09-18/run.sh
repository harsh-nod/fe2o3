...
[[ "$owned" == /tmp/fe2o3-kfd-copy-progress-20260918.* && -O "$owned" && -d "$owned" && ! -L "$owned" ]]
[[ $(cat "$owned/owner") == fe2o3-copy-progress-3e12ef82bbb41fb116afbb7ddf7cffdad735ec7d ]]
...
cd "$owned/source"
[[ $(git rev-parse HEAD) == 3e12ef82bbb41fb116afbb7ddf7cffdad735ec7d ]]
...
phase() {
    local repetition=$1 cell=$2 status=0 post_status=0 policy cpu
    printf 'phase repetition=%s cell=%s\n' "$repetition" "$cell"
    admit
    case "$cell" in
        A|B)
            case "$cell" in A) policy=diagnostic-slice50us ;; B) policy=diagnostic-window-deadline ;; esac
            ...
            ;;
        C|D)
            case "$cell" in C) cpu=0 ;; D) cpu=1 ;; esac
            HSA_XNACK=0 ROCR_VISIBLE_DEVICES=4 /usr/bin/timeout --signal=TERM --kill-after=5s 180s prlimit --core=0:0 -- /usr/bin/numactl --physcpubind=48-95 --membind=1 "$owned/hsa-copy-pool-engine" 0 "$cpu" 268435456 3 10 0x54f88318ca05093d fine engine1 || status=$?
            ;;
        *) return 64 ;;
    esac
...
printf 'context diagnostic=shared-host-kfd-copy-progress git_commit=3e12ef82bbb41fb116afbb7ddf7cffdad735ec7d gpu=4 unique_id=0x54f88318ca05093d bytes=268435456 depth=1 warmups=3 samples=10 repetitions=4 optional_context_journal=disabled placement=cpu48-95-memory1\n'
