...
[[ $(find "$archive/raw" -type f | wc -l) == 100 ]]
for suffix in command started log finished exit; do
    [[ -f "$archive/raw/audit.$suffix" ]]
    [[ -f "$archive/raw/audit-final.$suffix" ]]
done
[[ $(< "$archive/raw/audit.exit") == 0 ]]
[[ $(< "$archive/raw/audit-final.exit") == 0 ]]
cmp <(python3 -I "$archive/audit.py") "$archive/raw/audit-final.log"
cd -- "$archive"
