#!/usr/bin/env python3
"""Bounded development qualification of the scalar XGMI flush correction."""
import hashlib
...
def main():
    ...
    output = ROOT / 'cpu-2'
    output.mkdir()
    spec = importlib.util.spec_from_file_location('flush_probe_checker', CHECK)
    ...
    paths = subprocess.check_output(
        ['/usr/bin/git', 'ls-files', '-z', 'crates', 'Cargo.toml', 'Cargo.lock',
         'rust-toolchain.toml', '.cargo'], env=check.git_environment()).decode().split('\0')
    paths = sorted(path for path in paths if path and
                   (path.endswith(('.rs', '.toml', '.lock', '.json')) or '/fixtures/' in path))
    ...
    env = {'HOME': '/home/harsh', 'USER': 'harsh', 'PATH': '/home/harsh/.cargo/bin:/usr/bin:/bin',
           'LC_ALL': 'C', 'CARGO_TARGET_DIR': str(ROOT / 'target'), 'CARGO_INCREMENTAL': '0',
    ...
    commands = [
        ...
        ('doctests', cargo + ['test', '-p', 'fe2o3-runtime', '--all-features', '--doc'], 600),
