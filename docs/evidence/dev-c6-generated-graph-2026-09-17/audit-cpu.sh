...
[[ $(wc -l < "$archive/source-files.list") == 23 ]]
...
for receipt in "$archive"/raw/*.command; do
    ...
    case "$name" in
        exploratory-graph-expanded)
            [[ $(< "$archive/raw/$name.exit") == 101 ]]
            rg -Fq 'no entry found for key' "$archive/raw/$name.log"
            rg -Fq 'test result: FAILED. 15 passed; 1 failed;' "$archive/raw/$name.log"
            ;;
        *) success "$name" ;;
    esac
done
...
for target in gnu musl-no-hip; do
    ...
    for crate in host runtime; do
        ...
        if [[ $crate == host ]]; then count=275; passes=271; ignores=4; else count=951; passes=934; ignores=17; fi
...
printf 'PASS: exact frozen C6 source and GNU/musl-no-hip CPU rosters; host 271+4 ignored, runtime 934+17 ignored per target; quality gates and 61 doctests. No native integration, formal or milestone acceptance.\n'
