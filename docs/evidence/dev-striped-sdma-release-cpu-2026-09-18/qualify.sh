...
record gnu-roster "${environment[@]}" cargo test --frozen -p fe2o3-kfd --all-features --lib -- --list constructed_generic_release striped_sdma_cases
record gnu-release "${environment[@]}" cargo test --frozen -p fe2o3-kfd --all-features --lib -- constructed_generic_release striped_sdma_cases
record gnu-cleanup "${environment[@]}" cargo test --frozen -p fe2o3-kfd --all-features --lib sdma_cleanup
record musl-roster "${environment[@]}" cargo test --frozen -p fe2o3-kfd --all-features --target x86_64-unknown-linux-musl --lib -- --list constructed_generic_release striped_sdma_cases
record musl-release "${environment[@]}" cargo test --frozen -p fe2o3-kfd --all-features --target x86_64-unknown-linux-musl --lib -- constructed_generic_release striped_sdma_cases
