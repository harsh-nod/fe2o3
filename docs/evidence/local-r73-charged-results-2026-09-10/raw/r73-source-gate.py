"""Serial, retained local gates for GEN-2R. No GPU execution."""
import json
import hashlib
import os
...
attempt = sys.argv[1]
assert re.fullmatch(r'r73-[a-z0-9]+', attempt)
...
tests = ['cargo', 'test', '--locked', '--offline', '--no-fail-fast', *packages,
         '--all-features']
musl = ['--target', 'x86_64-unknown-linux-musl']
lint = ['cargo', 'clippy', '--locked', '--offline', *packages,
        '-p', 'fe2o3-host', '-p', 'fe2o3-macros']
gates = [
    ...
    ('gnu-host', ['cargo', 'test', '--locked', '--offline', '-p', 'fe2o3-host',
        '--all-features', '--lib'], repo),
    ('musl-host', ['cargo', 'test', '--locked', '--offline', '-p', 'fe2o3-host',
        '--lib', *musl], repo),
...
assert source_identities() == before, 'non-documentation source changed during final gates'
print('PASS: exact non-documentation source identities unchanged across final gates', flush=True)
