...
const parent = '4a49234a03fc3759bd1d3ca330197619b4ae8ab0';
const accepted = '4a49234a03fc3759bd1d3ca330197619b4ae8ab0';
...
const initialArtifacts = p.initialRuns.flatMap(([name]) => ['.json', '.log', '-source.json', '-source-after.json'].map(suffix => name + suffix));
const validationInputs = [...new Set([...prerequisiteArtifacts, ...p.prerequisites.map(([, , previous]) => previous),
  ...initialArtifacts, ...p.freezeHelpers])].sort();
...
for (const [name, command, previous] of [...p.initialRuns, ...p.prerequisites]) {
...
const environment = {
  ...
  recorded_phase: 'After separately recorded reviewed-source GNU/musl prerequisites; before remaining source/auxiliary/mutation gates. The isolated detached-control candidate and its failures are preserved under their original source maps. No native or formal qualification.',
  ...
  initial_runs: p.initialRuns.map(([name]) => name),
  ...
  prior_artifacts: fs.readdirSync(root).filter(name => /^r117-.*\.(json|log)$/.test(name)).concat(p.isolatedArtifacts).sort()
    .map(name => ({name, sha256: hash(fs.readFileSync(path.join(root, name)))})),
};
const previous = JSON.parse(fs.readFileSync(path.join(repo,
  'docs/evidence/local-r116-context-version-settlement-2026-09-14/raw/r116-environment.json')));
...
const previousSource = JSON.parse(fs.readFileSync(path.join(repo,
  'docs/evidence/local-r116-context-version-settlement-2026-09-14/raw/r116-frozen-source.json')));
...
for (const name of [...p.initialRuns.map(([name]) => name), ...prerequisiteRuns]) e.ordered(io.read(name + '.json').clock.source_verified, environment.observation);
...
for (const [name, value] of [['frozen-source', frozen], ['environment', environment], ['issue-status', issue]]) {
  fs.writeFileSync(path.join(root, 'r117-' + name + '.json'), JSON.stringify(value, null, 2) + '\n', {flag: 'wx'});
}
