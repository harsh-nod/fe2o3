...
const repo = path.join(root, 'fe2o3-detached-control');
const parent = '4756971168f6b4f2c33d217f5d476ccde8ea2740';
const [name, previousName, command, ...args] = process.argv.slice(2);
assert(/^detachedcandidate-[a-z0-9-]+$/.test(name) && previousName && command);
...
(async () => {
  ...
  let previous = null;
  if (previousName !== '-') {
    assert(/^detachedcandidate-[a-z0-9-]+\.json$/.test(previousName));
    ...
    assert.strictEqual(record.clock.contract, 'detachedcandidate-raw-utc-boot-monotonic-v1');
  ...
  const deadline = ['detachedcandidate-source-campaign', 'detachedcandidate-auxiliary-campaign'].includes(name) ? 7200000 : 1800000;
  let timedOut = false;
  ...
  const record = {source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    clock: {contract: 'detachedcandidate-raw-utc-boot-monotonic-v1', predecessor: previous, start, finish: result.finish, source_verified: verified, error: clockError}};
