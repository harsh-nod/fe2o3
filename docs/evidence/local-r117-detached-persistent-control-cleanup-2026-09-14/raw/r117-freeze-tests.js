...
const script = path.join(root, 'r117-freeze.js');
...
function fixture(name, mutate, rejects, openingHead = parent, prerequisite = value => value) {
  ...
  for (const name of p.isolatedArtifacts) artifacts.set(name, 'retained original-context artifact');
  for (const [index, [run, command, previous]] of [...p.initialRuns, ...p.prerequisites].entries()) {
    const prior = previous === null ? null : JSON.parse(artifacts.get(previous));
    ...
    const record = prerequisite({source_head: parent, source_map_sha256: hash(JSON.stringify(expectedMap)), cwd: repo, command,
      ...
      runner: {name: 'r117-run-v1.js', sha256: hash(helpers.get('r117-run-v1.js'))},
      process_group_cleanup: {timeout: null, close: {status: 'absent', pgid: 42, error: 'Error: kill ESRCH', live_members: [], observation: close}},
      clock: {contract: 'r117-raw-utc-boot-monotonic-v1', error: null, start, finish, source_verified: verified,
        predecessor: previous === null ? null : {name: previous, sha256: hash(artifacts.get(previous)), observation: prior.clock.source_verified}},
    }, run);
    ...
    for (const suffix of ['-source.json', '-source-after.json']) {
      const substituted = run === p.prerequisites[0][0] &&
        ((name === 'prerequisite-after-source' && suffix === '-source-after.json') ||
  ...
  const fakeFs = {
    ...
    readFileSync: file => {
      if (file.includes('/raw/r116-environment.json')) return JSON.stringify({binaries});
      if (file.includes('/raw/r116-frozen-source.json')) return JSON.stringify(previousMap);
  ...
  function localRequire(name) {
    ...
    if (!name.startsWith('./r117-')) return require(name);
  ...
  if (rejects) {
    ...
    assert.deepStrictEqual(writes.map(([file]) => file), ['frozen-source', 'environment', 'issue-status'].map(n => path.join(root, 'r117-' + n + '.json')));
...
fixture('deleted-untracked-member-test', ({files, listed}) => {files.delete(p.addedSource[0]); listed.delete(p.addedSource[0]);}, true);
...
fixture('same-count-replacement', ({files, listed}) => {
  files.delete(p.addedSource[0]); listed.delete(p.addedSource[0]); files.set('scripts/replacement.rs', 'candidate'); listed.add('scripts/replacement.rs');
}, true);
...
for (const [name, modify] of [
  ...
  ['path', r => {r.log += '.other';}],
  ['cwd', r => {r.cwd += '/other';}],
  ['predecessor', r => {if (r.clock.predecessor) r.clock.predecessor.sha256 = '0'.repeat(64); else r.clock.predecessor = {}; }],
...
]) fixture('prerequisite-' + name, () => {}, true, parent, (r, run) => {
  if (run === p.prerequisites[0][0]) modify(r); return r;
});
fixture('prerequisite-initial-anchor-unclosed', () => {}, true, parent, (r, run) => {
  if (run === p.initialRuns[0][0]) r.child_closed = false; return r;
});
...
fixture('prerequisite-artifact-substitution', ({artifacts}) => artifacts.set(p.prerequisites[0][0] + '.log', 'substituted'), true);
