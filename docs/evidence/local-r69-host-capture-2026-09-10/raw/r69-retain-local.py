"""Retain DRN-1A gates, failed attempts and exact changed-source identities."""
import hashlib
import json
...
root = repo / 'docs/evidence/local-r69-host-capture-2026-09-10'
raw = root / 'raw'
attempt = sys.argv[1]
assert attempt in ('r69-final', 'r69-final2', 'r69-final3')
...
proof = (temporary / 'r69-verus-final.log').read_text()
...
assert len(positive) == 58 and sum(map(int, positive)) == 1338
assert len(negative) == 650 and len(set(negative)) == 650
...
summary['verus'] = dict(positive_sources=58, obligations=1338, expected_negatives=650,
                        native_or_whole_executor_proof=False)
git = ['git', '-C', str(repo)]
base = subprocess.check_output(git + ['rev-parse', 'HEAD'], text=True).strip()
assert base == '0d5a2a931dbe94c9432691bd1643c2803482bde3'
...
files = list(temporary.glob('r69-*.log'))
files += list(temporary.glob('r69-*-source-gate.json'))
files += [temporary / name for name in ('r69-source-gate.py',
    'r69-production-metadata.json', 'r69-retain-local.py')]
for path in files:
