...
class NativeTests(unittest.TestCase):
    def test_settled_postflight_orders_delays_and_preserves_first_failure(self):
        for original in (None, RuntimeError("original")):
            for failed_suffix in (None, "-settled", "-delayed"):
                events, secondary = [], RuntimeError("observer")

                def observe(name):
                    events.append(name)
                    if failed_suffix is not None and name.endswith(failed_suffix):
                        raise secondary

                result = N.settled_postflight(
                    observe, "kfd-d1", original, sleep=events.append
                )
                self.assertEqual(events, [2, "kfd-d1-settled", 20, "kfd-d1-delayed"])
                self.assertIs(
                    result,
                    original
    ...
    def test_postflight_timing_rejects_early_settled_or_delayed_samples(self):
        rows = {
            "kfd-d1": {"finished_ns": 100},
            "kfd-d1-settled-gpu1": {"started_ns": 2_000_000_100},
            "kfd-d1-settled-gpu2": {"finished_ns": 5_000_000_100},
            "kfd-d1-delayed-gpu1": {"started_ns": 25_000_000_100},
        }
        V.postflight_timing(rows, "kfd-d1")
        for name in ("kfd-d1-settled-gpu1", "kfd-d1-delayed-gpu1"):
            changed = copy.deepcopy(rows)
            changed[name]["started_ns"] -= 1
            with self.assertRaises(RuntimeError):
                V.postflight_timing(changed, "kfd-d1")
