#!/usr/bin/env python3
"""Bounded, serialized CPU qualification of the native budget witness."""
import hashlib
...
def main():
    ...
    cargo = ['cargo', '--locked']
    packages = ['-p', 'fe2o3-runtime']
    example = 'gfx942-runtime-xgmi-backing-budget-smoke'
    env['FE2O3_BACKING_BUDGET_RUST_BINARY'] = str(TARGET / 'debug/examples' / example)
    commands = [
        ('rustc', ['rustc', '-vV'], 30), ('cargo', ['cargo', '-V'], 30),
        ('format', ['cargo', 'fmt'] + packages + ['--', '--check'], 120),
        ('gnu-example', cargo + ['test'] + packages + ['--all-features', '--example', example], 1200),
        ('musl-example', cargo + ['test'] + packages + ['--all-features', '--example', example, '--target', 'x86_64-unknown-linux-musl'], 1200),
        ('build-cli', cargo + ['build'] + packages + ['--example', example], 1200),
        *[(name, ['/usr/bin/python3', '-I', '-B', str(REPO / 'benchmarks/runtime_gfx942' / (name + '.py'))], 120)
            for name in ['test_xgmi_backing_budget_results', 'test_xgmi_backing_budget_campaign', 'test_xgmi_backing_budget_native']],
        ('clippy', cargo + ['clippy'] + packages + ['--all-features', '--all-targets', '--', '-D', 'warnings'], 1200),
