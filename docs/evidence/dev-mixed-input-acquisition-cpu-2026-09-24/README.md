...
cleanup-test groups and eight new cleanup-gate groups. Optional cleanup requires
the exact scratch/command rosters, terminal recorded/fixed development process
groups and unchanged complete owned-tree identities. Best-effort same-UID
process scans reject observed private references through arguments, cwd,
descriptors or mappings; inaccessible fields are recorded with PID/start-time
identity and errno, never interpreted as absence. Each field is inspected
independently. These scans run before retention and immediately before removal.
They are diagnostics, not proof of global process absence or deletion authority.

The collector copies all development logs before removing only the task-owned
scratch directory, immediately rechecks its complete tree identity, and records
independent ordinary-path and symlink absence in one command. Process and
filesystem scans are observations, not an exclusion lock against a hostile
concurrent creator or new external handle. The preexisting repository target
and unrelated owner-inspection evidence are outside its deletion scope. No
remote workspace is used.

The original `audit1` remains failed: all four prerequisite commands passed,
but global process inspection refused inaccessible `/proc/455/cwd` before
collection or deletion. Its exact controls and unchanged input brackets are
retained. The corrected audit uses the explicit visibility-limited diagnostic
above; it does not promote the earlier refusal or claim inaccessible processes
have no handles.
