...
source_clean() {
    [[ -z $(find . -type l -print) ]] || return 1
    find . -type f -print0 | LC_ALL=C sort -z | xargs -0 sha256sum > "$owned/results/source-current.sha256" || return $?
    cmp --silent "$owned/results/source-files.sha256" "$owned/results/source-current.sha256"
}
source_clean
...
finish() {
    ...
    dirty=
    source_clean || clean_status=$?
    ((clean_status == 0)) || dirty=source-roster-or-content-mismatch
...
printf 'context diagnostic=shared-host-kfd-native-wait git_commit=5d70cb0a6e16fb265fe224690274fdb0be2b0055 gpu=4 unique_id=0x54f88318ca05093d bytes=268435456 depth=1 warmups=3 samples=10 repetitions=4 optional_context_journal=disabled placement=cpu48-95-memory1\n'
