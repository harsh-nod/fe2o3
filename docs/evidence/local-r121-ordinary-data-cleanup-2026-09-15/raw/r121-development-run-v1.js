...
const parent = 'be50052c74a7956ba0a50df0e2efe8d9568de78a';
const [name, previousName, command, ...args] = process.argv.slice(2);
assert(/^r121-development-[a-z0-9-]+$/.test(name) && previousName && command);
...
(async () => {
  ...
  let previous = null;
  if (previousName !== '-') {
    assert(/^r121-development-[a-z0-9-]+\.json$/.test(previousName));
  ...
  const deadline = ['r121-development-source-campaign', 'r121-development-auxiliary-campaign'].includes(name) ? 7200000 : 1800000;
  let timedOut = false;
  ...
  const record = {source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    clock: {contract: 'r121-development-raw-utc-boot-monotonic-v1', predecessor: previous, start, finish: result.finish, source_verified: verified, error: clockError}};
