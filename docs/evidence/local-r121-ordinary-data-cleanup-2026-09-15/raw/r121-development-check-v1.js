...
function completed(name, command, expectedMap) {
  ...
  for (const sample of samples) {
    ...
    assert(/^\d+$/.test(sample.monotonic_ns));
  }
...
if (mode === 'restored') {
  ...
  console.log(JSON.stringify({development_only: true, restored: true, source_map_sha256: plan.source_map_sha256, identities: Object.keys(map).length}));
} else if (mode === 'baseline') {
  const {record, log} = completed('r121-development-kfd-all-v1', ['cargo', 'test', '-p', 'fe2o3-kfd', '--all-features', '--lib'], map);
  assert.strictEqual(record.returncode, 0);
  assert(log.includes('test result: ok. 1172 passed; 0 failed; 0 ignored; 0 measured; 0 filtered out;'));
  assert.strictEqual((log.match(/^test .* \.\.\. ok$/gm) || []).length, 1172);
  assert.deepStrictEqual(identities(), map);
  ...
  assert.deepStrictEqual(record.clock.predecessor, {name: predecessorName, sha256: hash(read(predecessorName)), observation: json(predecessorName).clock.source_verified});
