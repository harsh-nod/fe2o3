// Execute the predeclared leaf gates once, serially, without replacing evidence.
...
const checker = 'r121-development-gate-check-v3.js';
const checkerSha = 'e0652f1c76bfff0afef5401616670e786bcb73841dc4ab6af44272e55f630762';
function run(args) {
...
const full = check('full', 'musl');
assert.deepStrictEqual(full.checked, plan.full_runs.map(s => s.name));
...
console.log(JSON.stringify({development_only: true, packet_accepted: false, completed_leaves: 25,
  next: 'Run the separately declared complete gate checker, calibrations and collector; then assemble and independently review the archive.'}));
