...
const g = pin('r121-development-gates-v2.json', '7db1c9671038939caf37860e43a81fd4ad864ba979babc370f71714c4518f28f');
const gi = pin('r121-development-gate-check-inputs-v3.json', '7d91d2ccaac155f081937d115e455aaa9bf23c0e8a3c289d8adc238e5eb91e2e');
const ci = pin('r121-development-gate-parser-inputs-v3.json', 'f15355e6e883bca002dfc46eba35e8e397c21c0f5176dab8c10277c0f55f5553');
...
const gateName = 'r121-development-gate-check-all-v4';
...
assert.strictEqual(calibration.passed, 30);
assert.strictEqual(calibration.cases.length, 30);
assert.strictEqual(new Set(calibration.cases.map(c => c.name)).size, 30);
assert.strictEqual(calibration.cases.filter(c => c.expected === 'accept').length, 10);
assert.strictEqual(calibration.cases.filter(c => c.expected === 'reject').length, 20);
const expectedCalibration = [
  ...
  ['standalone exact lockfile roster', 'accept'],
  ['standalone observed cache wait', 'accept'],
  ['standalone repeated exact cache waits', 'accept'],
  ['standalone unknown cache wait suffix', 'reject'],
  ['standalone missing lockfile with cache wait', 'reject'],
  ['standalone duplicate lockfile with cache wait', 'reject'],
  ['standalone wrong completion count with cache wait', 'reject'],
  ['standalone completion without roster', 'reject'],
  ['standalone error with cache wait', 'reject'],
  ['standalone warning with cache wait', 'reject'],
...
const ownRun = 'r121-development-qualification-collect-v6';
...
console.log(JSON.stringify({development_only: true, publication_pending: true, scope: 'R121 local CPU/test qualification only',
  ...
  compiled_runtime_negatives: 26, distinct_mutant_maps: 25, helper_calibrations: 30, restored_suites: [59, 17],
