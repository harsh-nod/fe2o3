...
const gates = JSON.parse(pinned('r121-development-gates-v2.json', '7db1c9671038939caf37860e43a81fd4ad864ba979babc370f71714c4518f28f'));
const retry = JSON.parse(pinned('r121-development-musl-retry1-inputs-v1.json', '479bf5f21a311a29361ddcce17139d9e31187bdf568429a63e50a34f46a25aec'));
...
function leaf(spec, log) {
  ...
  if (spec.name in leafCounts || spec.name in auxCounts) {
    ...
    const normalized = log.split('\n')
      .filter(line => line !== '    Blocking waiting for file lock on package cache').join('\n');
    assert.strictEqual(normalized, old, 'standalone-lockfiles exact roster and completion');
...
function rejectedMuslHistory(spec) {
  const h = gates.rejected_musl_attempt;
  const r = JSON.parse(pinned(h.name + '.json', h.record_sha256));
  pinned(h.name + '.log', h.log_sha256);
  assert.deepStrictEqual(JSON.parse(pinned(h.name + '-source.json', h.source_file_sha256)), map);
  assert.deepStrictEqual(JSON.parse(pinned(h.name + '-source-after.json', h.source_after_file_sha256)), map);
  assert.strictEqual(r.source_head, gates.source_parent);
  assert.strictEqual(r.source_map_sha256, gates.source_map_sha256);
  assert.strictEqual(r.cwd, repo);
  assert.strictEqual(r.source_unchanged, true);
  assert.strictEqual(r.returncode, 124); assert.strictEqual(r.child_returncode, null);
  assert.strictEqual(r.child_closed, true); assert.strictEqual(r.spawn_error, null);
  assert.strictEqual(r.signal, 'SIGKILL'); assert.strictEqual(r.timed_out, true);
  assert.strictEqual(r.deadline_ms, 1800000);
  assert.deepStrictEqual(r.runner, {name: gates.runner, sha256: gates.runner_sha256});
  assert.strictEqual(retry.run, spec.name); assert.strictEqual(spec.predecessor, h.name + '.json');
  assert.strictEqual(retry.predecessor, spec.predecessor);
  assert.deepStrictEqual(retry.command, spec.command); assert.deepStrictEqual(r.command, spec.command);
  assert.deepStrictEqual(retry.environment, r.environment);
  assert.strictEqual(retry.deadline_ms, r.deadline_ms);
  assert.strictEqual(retry.source_map_sha256, gates.source_map_sha256);
  assert.deepStrictEqual(retry.rejected_attempt, h);
  assert.strictEqual(r.clock.contract, 'r121-development-raw-utc-boot-monotonic-v1');
  assert.strictEqual(r.clock.error, null);
  const prior = json(h.predecessor);
  assert.deepStrictEqual(r.clock.predecessor, {name: h.predecessor,
    sha256: sha(bytes(h.predecessor)), observation: prior.clock.source_verified});
  E.ordered(prior.clock.source_verified, r.clock.start);
  E.ordered(r.clock.start, r.process_group_cleanup.timeout.observation);
  E.ordered(r.process_group_cleanup.timeout.observation, r.clock.finish);
  E.ordered(r.clock.finish, r.process_group_cleanup.close.observation);
  E.ordered(r.process_group_cleanup.close.observation, r.clock.source_verified);
  assert.strictEqual(r.process_group_cleanup.timeout.status, 'signaled');
  assert.strictEqual(r.process_group_cleanup.close.status, 'signaled');
  assert.strictEqual(r.process_group_cleanup.timeout.pgid, r.process_group_cleanup.close.pgid);
  assert.deepStrictEqual(r.process_group_cleanup.close.live_members, []);
  assert.deepStrictEqual(E.liveGroupMembers(r.process_group_cleanup.close.pgid), []);
}
