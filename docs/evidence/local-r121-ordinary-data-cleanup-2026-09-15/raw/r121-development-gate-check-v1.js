...
function relocate(log) {
  for (const r of gates.doc_relocations) {
    ...
    if (!names.length) continue; // The standalone host doctests have no KFD group.
...
function linux(log) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (partial) {
      ...
      assert.notStrictEqual(lines[i], 'ok', 'no orphan result');
...
full(fullGnu, record(fullGnu)); checked.push(fullGnu.name);
