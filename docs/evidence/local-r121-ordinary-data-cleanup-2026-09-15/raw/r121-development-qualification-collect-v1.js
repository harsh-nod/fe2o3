...
const ownRun = 'r121-development-qualification-collect-v1';
const ownOutputs = new Set(['.json', '.log', '-source.json', '-source-after.json', '-gate-failure.json'].map(s => ownRun + s));
const inventory = fs.readdirSync(root).filter(n => /^r121-development-.*\.(?:json|log|js|md|rs)$/.test(n) && !ownOutputs.has(n)).sort();
