...
Use a fresh mode-0700 marked directory under `/home/harsh`, whose filesystem
must admit executable files. The separately preserved `/tmp` setup failed
with ENOSPC before creating its directory or uploading/executing the payload.
This new path binding is a new payload, not a retry of a started GPU campaign.
The separately sealed GPU 1 campaign passed four cases, then stopped after
new contention caused the fifth delayed observation to refuse. Its successful
native commands do not override that refusal; counts 12/14 never ran there.
This GPU 2 campaign preserves that history and starts with a new frozen payload.
...
GPU 2 is a candidate, not reserved. Its topology was independently checked
before freezing this protocol. Before every native invocation require the
complete pinned observer to admit UID 0xd2e26fef80cf5c33 at BDF 0000:46:00.0,
