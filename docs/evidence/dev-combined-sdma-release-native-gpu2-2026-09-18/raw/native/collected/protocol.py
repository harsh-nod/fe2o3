#!/usr/bin/env python3
"""Exact combined-SDMA teardown native protocol; no device access on import."""
...
BINARY_SHA = "08e0ed94feac79cb8126e536a3926994ebda3c4eff43e3edc3276faf11a1586d"
CPU_SEAL = "529398ddbab4825c6847e918f26e1a0cc6b4554963f20fc37b7b2d9cbb155477"
COHORT_SHA = "9f5447bdbffb3277ff3750f2747a5300d7d69b05065bee149c297a5f4abe08cb"
COHORT_FILE_COUNT = 5557
...
TESTS = {f"combined-{count}": count for count in (2, 4, 6, 8, 10, 12, 14)}
...
def transcript(case, stdout, stderr):
    ...
    count = TESTS[case]
    total = count + 2
    resources = 11 + 3 * count
    engines = ",".join(str(index % 2) for index in range(count))
    prefix = f"retained_combined_sdma_release=complete striped_queue_count={count} total_sdma_queue_count={total} cursor=striped-initial-0-no-advance-source-qualified primary_queue_id=0 directional_h2d_queue_id="
    suffix = (
    ...
    match = re.fullmatch(
        re.escape(prefix)
        + r"([1-9][0-9]*) directional_d2h_queue_id=([1-9][0-9]*) striped_queue_ids="
        + r"([1-9][0-9]*(?:,[1-9][0-9]*)*)"
        + re.escape(suffix)
        + r"([1-9][0-9]*)"
        + re.escape(tail)
    ...
    h2d, d2h = int(match[1]), int(match[2])
    striped = [int(value) for value in match[3].split(",")]
    sdma = [h2d, d2h] + striped
    event, offset = map(int, match.groups()[3:])
    need(
        len(striped) == count
        and len(set(sdma)) == total
    ...
    return {
        "case": case,
        "striped_queue_count": count,
        "total_sdma_queue_count": total,
        "directional_engine_placement": [1, 0],
        "striped_engine_placement": [index % 2 for index in range(count)],
        "admitted_engine_count": 2,
        "admitted_queues_per_engine": 8,
        "maximum_striped_queue_count": 14,
        "cursor_scope": "striped-initial-0-no-advance-source-qualified",
        "host_delta_bytes": 4096 * total,
        "host_delta_records": total,
        "primary_queue_id": 0,
        "sdma_queue_ids": sdma,
        "directional_h2d_queue_id": h2d,
        "directional_d2h_queue_id": d2h,
        "striped_queue_ids": striped,
        "event_id": event,
