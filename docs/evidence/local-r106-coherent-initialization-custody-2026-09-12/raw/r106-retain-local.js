...
const collectorRejection = read('r106-collector-rejection.json');
assert.strictEqual(collectorRejection.returncode, 1);
assert.strictEqual(collectorRejection.actual, 10);
assert.strictEqual(collectorRejection.expected, 8);
assert.strictEqual(collectorRejection.archive_created, false);
assert.strictEqual(hash(fs.readFileSync(path.join(root, 'r106-retain-local-before-roster-fix.js'))),
  '13014bfc576dfe6cdbaceb9ba7fe3ccd7966cec5db6d233b8247537d33ec0773');
...
const borrowed = oldNames.filter(n => n.includes('borrowed_initialization') &&
  (n.startsWith('shared_memory::') || n.startsWith('queue::')));
...
const summary = {
  ...
  collector_rejection_record: 'raw/r106-collector-rejection.json',
  rejected_collector: collectorRejection.rejected_collector,
