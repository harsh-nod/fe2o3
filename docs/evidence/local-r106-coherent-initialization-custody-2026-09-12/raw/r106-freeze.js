...
const acceptedRuntimeCheckpoint = '3ae84f8542ef82d4615f5e5ff377e2a2b076e29a';
...
const environment = {
  ...
  runners: ['r106-run.js', 'r106-source-gate.py', 'r106-auxiliary-gates.py', 'r106-freeze.js']
...
const previous = JSON.parse(fs.readFileSync(path.join(repo,
  'docs/evidence/local-r105-pristine-rebind-custody-2026-09-12/raw/r105-environment.json')));
...
for (const [name, value] of [['frozen-source', frozen], ['environment', environment], ['issue-status', issue]]) {
  fs.writeFileSync(path.join(root, 'r106-' + name + '.json'), JSON.stringify(value, null, 2) + '\n', {flag: 'wx'});
}
