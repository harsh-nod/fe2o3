#!/usr/bin/env python3
"""Record CPU correctness, production enrollment and isolated partition measurements."""
import argparse
import hashlib
...
def commands():
    return [
        ...
        *[(name, ['taskset', '-c', '31', 'cargo', 'test', '--locked', '-p', 'fe2o3-runtime-model', '--release', name + '_performance',
                  '--', '--ignored', '--nocapture', '--test-threads=1']) for name in ('adaptive', 'sort_enrollment')],
        ('partition', ['taskset', '-c', '31', 'cargo', 'test', '--locked', '-p', 'fe2o3-runtime-model', '--release',
                       'adaptive_partition_diagnostics', '--', '--ignored', '--nocapture', '--test-threads=1']),
    ]
