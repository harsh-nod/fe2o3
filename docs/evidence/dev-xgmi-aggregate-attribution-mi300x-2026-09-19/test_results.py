...
def summary(mode: str) -> bytes:
    diagnostic = " diagnostic=aggregate-host-attribution" if mode == "on" else ""
    return (
...
def record(ordinal: int) -> bytes:
    source, destination = (
        (UIDS[0][2:], UIDS[1][2:])
        if ordinal % 2 == 0
        else (UIDS[1][2:], UIDS[0][2:])
    )
    return (
        "schema=fe2o3.xgmi-aggregate-host-attribution.v1 backend=kfd "
        f"ordinal={ordinal} backend_submission={ordinal + 7} "
        f"source_uid={source} destination_uid={destination} "
        "admission_validation_ns=2 preparation_ns=3 opening_currentness_ns=5 "
        "submission_ns=7 wait_ns=11 closing_currentness_ns=13 "
        "settlement_ns=17 total_ns=61 authority=none teardown=explicit "
...
class ResultParserTests(unittest.TestCase):
    ...
    def test_exact_off_and_on_transcripts(self):
        ...
        self.assertEqual(on["summary"]["diagnostic"], "aggregate-host-attribution")
        self.assertEqual(len(on["records"]), 82)
        self.assertEqual(
            [(row["population"], row["round"], row["direction"]) for row in on["records"][:4]],
            [
                ("prime", 0, "forward"),
                ("prime", 0, "reverse"),
                ("warmup", 0, "forward"),
        ...
        for key in (*R._PHASES, "total_ns"):
            for value in ("-1", "+1", "01", "1.0", str(1 << 64), "nan"):
                with self.subTest(key=key, value=value):
                    self.rejected(mutate_row(valid, 0, key, value))
        self.assertEqual(
            R.parse_result(mutate_row(valid, 0, "wait_ns", "0"), "on", UIDS)[
                "records"
            ][0]["durations_ns"]["wait_ns"],
            0,
        )
    ...
    def test_summary_constants_metrics_and_diagnostic_marker_are_strict(self):
        ...
        self.rejected(off[:-1] + b" diagnostic=aggregate-host-attribution\n", "off")
        on_without = on.replace(b" diagnostic=aggregate-host-attribution", b"", 1)
