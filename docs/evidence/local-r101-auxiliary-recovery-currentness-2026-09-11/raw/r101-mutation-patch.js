...
const mutations = JSON.parse(fs.readFileSync(path.join(root, 'r101-mutations.json')));
const mutation = mutations.find(m => m.name === name);
assert(mutation, 'named mutation');
const baseline = JSON.parse(fs.readFileSync(path.join(root, 'r101-frozen-source.json')));
