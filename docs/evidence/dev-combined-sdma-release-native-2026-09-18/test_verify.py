...
class ArchiveTests(unittest.TestCase):
    ...
    def test_complete_historical_audit(self):
        ...
        self.assertEqual(value["native_campaign"], "rejected-delayed-shared-host-observation")
        self.assertEqual(value["profiles"], V.CASES)
        self.assertEqual(value["fully_closed_profiles"], V.CASES[:-1])
        self.assertEqual(value["unexecuted_profiles"], ["combined-12", "combined-14"])
        self.assertEqual(value["native_commands_passed"], 5)
        self.assertEqual(value["strict_endpoints"], 14)
        self.assertEqual(value["refused_endpoints"], 1)
        self.assertEqual(value["setup_rejection"]["native_commands"], 0)
        self.assertEqual(value["setup_rejection"]["path_absence"], "confirmed")
    ...
    def test_observer_command_contract(self):
        value = V.load(self.payload / "results/combined-2-preflight/record.json")
    ...
    def test_endpoint_capture_clock_bracket(self):
        row = V.load(self.payload / "results/combined-2-preflight/record.json")
        value = V.lines(self.payload / "results/combined-2-preflight/stdout.log")[0]
    ...
    def test_controller_requires_preserved_rejection_and_cleanup(self):
    ...
    def test_inventory_cannot_omit_or_replace_retained_files(self):
        ...
        for label in ("missing", "wrong-elf", "extra"):
            with self.subTest(label=label), self.assertRaises(ValueError):
                changed = copy.deepcopy(value)
                if label == "missing":
                    changed["files"].pop("results/combined-10-test/stderr.log")
    ...
    def test_endpoint_rederives_raw_values(self):
        value = V.lines(self.payload / "results/combined-2-immediate/stdout.log")[0]
    ...
    def test_delayed_endpoint_cannot_move_to_a_later_window(self):
        value = V.lines(self.payload / "results/combined-2-delayed/stdout.log")[0]
        t0 = V.load(self.payload / "results/combined-2-test/record.json")["t0"][
            "monotonic_ns"
        ]
        with self.assertRaises(ValueError):
    ...
    def test_native_cases_require_exact_profile_and_complete_release(self):
        for case in V.CASES:
            ...
            for changed in (
                out.replace(f"resources_returned={11 + 3 * self.P.TESTS[case]}", "resources_returned=5"),
                out.replace(self.P.PROFILE_SHA, "0" * 64),
                out.replace("retry=rejected", "retry=accepted"),
                out.replace("striped_engine_placement=0,1", "striped_engine_placement=1,0"),
                out.replace(f"host_delta_bytes={4096 * (self.P.TESTS[case] + 2)}", "host_delta_bytes=0"),
                out.replace(f"host_delta_records={self.P.TESTS[case] + 2}", "host_delta_records=0"),
                out.replace("cursor=striped-initial-0-no-advance-source-qualified", "cursor=native-observed"),
    ...
    def test_native_command_uses_combined_count_and_uid(self):
        for case in V.CASES:
            count = self.P.TESTS[case]
            expected = V.native_command(self.P, case, V.NEW)
            self.assertEqual(expected[-4:], [V.NEW + "/queue-example", "--retained-release-combined-sdma", str(count), self.P.UID])
            actual = V.load(self.payload / f"results/{case}-test/record.json")["command"]
            self.assertEqual(actual, expected)

    def test_ENOSPC_setup_failure_and_absence_are_retained(self):
        self.assertEqual(V.setup_history(V.ROOT)["native_commands"], 0)
        self.assertEqual(V.setup_history(V.ROOT)["disposition"], "ENOSPC-before-directory-creation")
    ...
    def test_calibration_rosters_reject_partial_or_ambiguous_results(self):
        for tag, source, suite in (
            ("local-protocol-tests", self.payload / "test_protocol.py", "ProtocolTests"),
            ("local-controller-wiring-tests", V.ROOT / "control/test_controller.py", "ControllerTests"),
        ):
            text = (self.native / tag / "stderr.log").read_text()
            V.unittest_transcript(text, source, suite, 4)
            for changed in (text + "extra\n", text.replace("Ran 4 tests", "Ran 0 tests"), text.replace(" ... ok", " ... skipped", 1), text[:text.index("Ran 4 tests")], text.replace("\nOK\n", "\nFAILED\n")):
                with self.subTest(tag=tag), self.assertRaises(ValueError):
                    V.unittest_transcript(changed, source, suite, 4)

    def test_refused_delayed_endpoint_rederives_all_raw_facts(self):
        value = V.lines(self.payload / "results/combined-10-delayed/stdout.log")[0]
        t0 = V.load(self.payload / "results/combined-10-test/record.json")["t0"]["monotonic_ns"]
        V.refused_endpoint(self.P, value, t0)
        for fault in ("admitted", "busy", "pid", "reasons", "identity", "capture", "clock"):
            ...
            if fault == "admitted":
                ...
                changed["pci_bdf"] = "0000:46:00.0"
            elif fault == "capture":
                changed["pids"]["exit"] = 1
            else:
                changed["finished"]["monotonic_ns"] = 1
            with self.subTest(fault=fault), self.assertRaises(ValueError):
                V.refused_endpoint(self.P, changed, t0)
        with self.assertRaises(ValueError):
            V.refused_endpoint(self.P, value, t0 - 2_000_000_000)
