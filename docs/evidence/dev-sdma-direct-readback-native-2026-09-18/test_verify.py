...
class ArchiveTests(unittest.TestCase):
    ...
    def test_complete_historical_audit(self):
        ...
        self.assertEqual(value["device"], "passed")
        self.assertEqual(value["host"], "passed")
        self.assertEqual(value["harness_passes"], [1, 1])
        self.assertEqual(value["strict_endpoints"], 6)
    ...
    def test_observer_command_contract(self):
        value = V.load(self.payload / "results/device-preflight/record.json")
    ...
    def test_endpoint_capture_clock_bracket(self):
        row = V.load(self.payload / "results/device-preflight/record.json")
        value = V.lines(self.payload / "results/device-preflight/stdout.log")[0]
    ...
    def test_inventory_cannot_omit_or_replace_retained_files(self):
        ...
        for label in ("missing", "wrong-elf", "extra"):
            with self.subTest(label=label), self.assertRaises(ValueError):
                changed = copy.deepcopy(value)
                if label == "missing":
                    changed["files"].pop("results/host-test/stderr.log")
    ...
    def test_endpoint_rederives_raw_values(self):
        value = V.lines(self.payload / "results/device-immediate/stdout.log")[0]
    ...
    def test_delayed_endpoint_cannot_move_to_a_later_window(self):
        value = V.lines(self.payload / "results/device-delayed/stdout.log")[0]
        t0 = V.load(self.payload / "results/device-test/record.json")["t0"][
            "monotonic_ns"
        ]
        with self.assertRaises(ValueError):
            self.P.endpoint(value, t0=t0 - 2_000_000_000, offset=20)

    def test_native_cases_require_exact_kind_size_hash_and_harness(self):
        for case in ("device", "host"):
            out = (self.payload / f"results/{case}-test/stdout.log").read_text()
            for changed in (
                out.replace("retry_bytes=4096", "retry_bytes=4095"),
                out.replace(self.P.READBACK_SHA, "0" * 64),
                out.replace("1124 filtered", "1121 filtered"),
                out.replace("kind=DeviceLocal", "kind=HostVisible")
                if case == "device"
                else out.replace("kind=HostVisible", "kind=DeviceLocal"),
            ):
                with self.subTest(case=case), self.assertRaises(ValueError):
