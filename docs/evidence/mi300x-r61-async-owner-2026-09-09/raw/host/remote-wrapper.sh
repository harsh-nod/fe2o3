...
stage="$1"
commit="$2"
[[ "$stage" =~ ^/dev/shm/fe2o3-r61-owner\.[A-Za-z0-9]+$ ]]
[[ "$commit" =~ ^[0-9a-f]{40}$ ]]
cleanup() {
  result=$?
  trap - EXIT
...
git clone --no-local --branch codex/r61-distributed-execution-v1 "$stage/source.bundle" "$stage/checkout" > "$stage/clone.log" 2>&1
[[ "$(git -C "$stage/checkout" rev-parse HEAD)" == "$commit" ]]
export PYTHONDONTWRITEBYTECODE=1
python3 "$stage/checkout/benchmarks/runtime_gfx942/run-r61-owner-mi300x.py" \
