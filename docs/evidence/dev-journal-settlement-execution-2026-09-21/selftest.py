...
def main():
    ...
    spec = importlib.util.spec_from_file_location('settlement_execution_selftest_checker', path)
    ...
    for part in ('body', 'invocation', 'definition'):
        ...
        if part == 'invocation':
            ...
            text = 'let writer_returns'
            offset = body.index(text, body.index('macro_rules! settlement_return_admission_body'))
    ...
    branch_rejections = 0
    for name in ('stage_lineage', 'commit_writer_return', 'rejection_identity'):
        control = next(m for m in controls if m.name == name)
        src, gen, origin, stdout, stderr = inputs(name, control)
        diagnostics = [base.unique_json(line) for line in stderr.splitlines()]
        adverse = []
        for index, span in enumerate(diagnostics[1]['spans']):
            for field, value in [('byte_start', 0), ('is_primary', int(span['is_primary'])),
                                 ('file_name', '/foreign.rs'), ('expansion', {})]:
                changed = copy.deepcopy(diagnostics)
                changed[1]['spans'][index][field] = value
                adverse.append(changed)
        changed = copy.deepcopy(diagnostics)
        changed[1]['spans'].append(copy.deepcopy(changed[1]['spans'][0]))
        adverse.append(changed)
        if control.kind == 'post':
            proof_file = checker.EXEC if control.macro else checker.SPEC
            text = gen[proof_file].decode('ascii')
            anchor = 'fn ' + control.function + '('
            offset = text.index(anchor)
            wrong = module.retained_parent.exact_span(text, origin.parent.parent / proof_file,
                offset, text.index('\n', offset), False, 'at the end of the function body')
            changed = copy.deepcopy(diagnostics)
            secondary = next(s for s in changed[1]['spans'] if s['is_primary'] is False)
            secondary.clear()
            secondary.update(wrong)
            adverse.append(changed)
        for changed in adverse:
            try:
                checker.check_result(module, 1, stdout, '\n'.join(map(json.dumps, changed)), src, gen, origin, control)
            except (ValueError, KeyError, TypeError):
                branch_rejections += 1
            else:
                raise ValueError('accepted adverse assertion or local-body result')

    snapshots = {module.HERE.parent / p: (repo / checker.CRATE / p).read_bytes() for p in checker.PINS}
    original_custody = repo / checker.LEGACY_CUSTODY
    snapshots[original_custody] = original_custody.read_bytes()
    original = snapshots[original_custody]
    current = snapshots[module.HERE.parent / checker.CUSTODY]
    checker.custody_transition(original, current)
    transition_changes = [
        current.replace(b'    ensures pending_custody_v1(after),', b'    ensures true,', 1),
        current.replace(b'pub proof fn begin_preserves_member_custody_v1(', b'pub proof fn renamed_member_custody_v1(', 1),
        current + b'\n',
        original,
    ]
    for changed in transition_changes:
        try:
            checker.custody_transition(original, changed)
        except ValueError:
            pass
        else:
            raise ValueError('accepted altered custody transition boundary')
    with tempfile.TemporaryDirectory(prefix='fe2o3-settlement-execution-source-selftest-') as temporary:
        for path in snapshots:
            ...
            try:
                ...
                expected = 'authenticated original custody proof' if path == original_custody else 'pinned settlement source:'
                checker.need(str(error).startswith(expected), 'intended source rejection')
            else:
                raise ValueError('accepted changed settlement input')
    ...
    if args.packet:
        ...
        failures = ['exact staged nested inputs', 'exact settlement postcondition', 'owned solver completion',
                    'exact verification result', 'proof roster', 'proof input identities', 'owned command completion',
                    'exact staged nested inputs']
        with tempfile.TemporaryDirectory(prefix='fe2o3-settlement-execution-packet-selftest-') as temporary:
            for index, failure in enumerate(failures):
                ...
                if index == 0:
                    ...
                    (case / checker.CUSTODY).write_bytes(original)
                ...
                rejected_packets += 1
    print(json.dumps({'adverse_solver_results_rejected': len(invalid) + branch_rejections, 'altered_inputs_rejected': len(snapshots),
                      'altered_transition_boundaries_rejected': len(transition_changes),
