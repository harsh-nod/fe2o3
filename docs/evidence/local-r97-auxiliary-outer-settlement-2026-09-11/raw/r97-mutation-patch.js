...
const mutation = JSON.parse(fs.readFileSync(path.join(root, 'r97-mutations.json')))
  .find(m => m.name === name);
assert(mutation);
const baseline = JSON.parse(fs.readFileSync(path.join(root, 'r97-frozen-source.json')));
