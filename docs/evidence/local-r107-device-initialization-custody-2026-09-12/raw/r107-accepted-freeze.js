...
const environment = {
  ...
  recorded_phase: 'Fresh preallocated-slot cohort before accepted frozen/full/auxiliary/mutation campaigns; after preliminary tests and the preserved inline-candidate GNU/stack failures. No host-load qualification.',
  ...
  runners: ['r107-run.js', 'r107-accepted-source-gate.py', 'r107-accepted-auxiliary-gates.py', 'r107-accepted-freeze.js']
  ...
  prior_artifacts: fs.readdirSync(root).filter(name => /^r107-.*\.(json|log|py|js)$/.test(name)
    && !name.startsWith('r107-accepted-')).sort()
    .map(name => ({name, sha256: hash(fs.readFileSync(path.join(root, name)))})),
};
...
for (const [name, value] of [['frozen-source', frozen], ['environment', environment], ['issue-status', issue]]) {
  fs.writeFileSync(path.join(root, 'r107-accepted-' + name + '.json'), JSON.stringify(value, null, 2) + '\n', {flag: 'wx'});
}
