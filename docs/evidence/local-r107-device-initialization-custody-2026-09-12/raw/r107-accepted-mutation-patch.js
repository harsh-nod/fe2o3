...
const mutation = JSON.parse(fs.readFileSync(path.join(root, 'r107-accepted-mutations.json')))
  .find(m => m.name === name);
assert(mutation, 'named mutation');
const baseline = JSON.parse(fs.readFileSync(path.join(root, 'r107-accepted-frozen-source.json')));
