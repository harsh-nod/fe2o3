...
const mutation = JSON.parse(fs.readFileSync(path.join(root, 'r107-mutations.json')))
  .find(m => m.name === name);
assert(mutation, 'named mutation');
const baseline = JSON.parse(fs.readFileSync(path.join(root, 'r107-frozen-source.json')));
