...
const baseline = read('r107-accepted-frozen-source.json');
...
const originalBaseline = read('r107-frozen-source.json');
assert.strictEqual(Object.keys(originalBaseline).length, 5667);
assert.notDeepStrictEqual(originalBaseline, baseline);
const environment = read('r107-accepted-environment.json');
const historicalPins = new Map(environment.prior_artifacts.map(p => [p.name, p.sha256]));
assert.strictEqual(environment.prior_artifacts.length, 51);
assert.strictEqual(historicalPins.size, 51, 'no duplicate historical artifact names');
assert.strictEqual(hash(JSON.stringify(environment.prior_artifacts.map(p => p.name))),
  '025b5f28701aad792ed6c676474224f5bfea14f103be766c2889777d73d9416e', 'exact historical artifact roster');
for (const [name, digest] of historicalPins) assert.strictEqual(hash(fs.readFileSync(file(name))), digest, name);
...
assert.deepStrictEqual(environment.runners.map(r => r.name), ['r107-run.js', 'r107-accepted-source-gate.py', 'r107-accepted-auxiliary-gates.py', 'r107-accepted-freeze.js']);
...
const issue = read('r107-accepted-issue-status.json');
...
const gates = read('r107-accepted-source-gate.json');
const auxiliary = read('r107-accepted-auxiliary-results.json');
for (const [rows, oldName, count, prefix] of [
  [gates, 'raw/r106-final-source-gate.json', 17, 'r107-accepted'],
  [auxiliary, 'raw/r106-auxiliary-results.json', 10, 'r107-accepted-auxiliary'],
]) {
  ...
  for (const [i, row] of rows.entries()) {
    assert.deepStrictEqual(row.command, old[i].command.map(s => s.replace('r106-production-metadata.log', 'r107-accepted-production-metadata.log')), row.name);
    assert.strictEqual(row.cwd, old[i].cwd);
    assert.strictEqual(row.log, file('r107-accepted-' + row.name + '.log'));
...
const newTests = [
  ...
  'device_initializer_terminal_storage_is_preallocated_and_cannot_overwrite_custody',
...
function checkRun(name, command, returncode, frozen = false) {
  ...
  assert(Object.keys(inputs).length >= 5666, 'nonempty complete historical/source roster');
  if (!name.startsWith('r107-accepted-')) {
    for (const artifact of [name + '.json', name + '.log', name + '-source.json']) {
      assert.strictEqual(hash(fs.readFileSync(file(artifact))), historicalPins.get(artifact), artifact);
    }
  }
  if (frozen) assert(Date.parse(run.started_at) >= Date.parse(environment.recorded_at));
...
for (const [kind, filter, names] of [
  ...
  const run = checkRun('r107-accepted-' + phase + '-' + kind, [...kfd, filter], 0, true);
...
const pinRecord = read('r107-accepted-mutation-tool-pins.json');
assert.deepStrictEqual(Object.keys(pinRecord.pins), ['r107-accepted-mutations.json', 'r107-accepted-mutation-patch.js', 'r107-accepted-check-mutation.js', 'r107-accepted-assert-frozen.js']);
for (const [name, digest] of Object.entries(pinRecord.pins)) assert.strictEqual(hash(fs.readFileSync(file(name))), digest);
const mutations = read('r107-accepted-mutations.json');
const oracleLines = {'drop-terminal-owner': 295, 'drop-source': 171, 'omit-native-admission': 295,
  'late-input-admission': 779, 'omit-gpu-prefix': 669, 'skip-final-map': 432, 'omit-in-place-quarantine': 860,
  'omit-slot-vacancy': 1137, 'omit-slot-reservation': 1102};
...
for (const mutation of mutations) {
  ...
  const name = 'r107-accepted-mut-' + mutation.name;
  ...
  const restored = read('r107-accepted-restoration-' + mutation.name + '.json');
...
for (const [name, result] of Object.entries(tests)) assert.deepStrictEqual(result, {
  ...oldTests[name], passed: oldTests[name].passed + (['gnu-tests', 'musl-tests'].includes(name) ? 11 : 0),
}, name);
...
for (const target of ['gnu', 'musl']) assert.deepStrictEqual(passing(fs.readFileSync(file('r107-accepted-' + target + '-tests.log'), 'utf8')),
...
const originalEnvironment = read('r107-environment.json');
for (const runner of originalEnvironment.runners) {
  assert.strictEqual(hash(fs.readFileSync(file(runner.name))), runner.sha256);
}
const oldMatrix = checkRun('r107-frozen-matrix', [...kfd, 'shared_memory::tests::device_initialization'], 0);
assert.deepStrictEqual(read('r107-frozen-matrix-source.json'), originalBaseline);
assert.deepStrictEqual(read('r107-routing-source.json'), originalBaseline);
assert.deepStrictEqual(passing(log(oldMatrix)), newTests.filter(n => !n.includes('terminal_storage')));
historical.push({name: 'r107-frozen-matrix', returncode: 0, accepted_campaign: false});
const originalGates = read('r107-final-source-gate.json');
assert.strictEqual(originalGates.length, 1);
assert.strictEqual(originalGates[0].name, 'gnu-tests');
assert.strictEqual(originalGates[0].returncode, 101);
assert.deepStrictEqual(originalGates[0].command, gates[0].command);
assert.deepStrictEqual(read('r107-final-source-inputs.json'), originalBaseline);
for (const suffix of ['source-after.json', 'complete.json']) {
  assert(!fs.existsSync(file('r107-final-' + suffix)), 'no fabricated old completion');
}
const stackTest = 'kfd_backend::qualification_drain_capture::tests::caller_authority_and_missing_history_cannot_enter_copy_only_observation';
const stackCommand = ['cargo', '+nightly-2026-04-03', 'test', '--locked', '--offline',
  '-p', 'fe2o3-runtime', '--all-features', '--lib', stackTest, '--', '--exact'];
const reproduction = checkRun('r107-stack-repro', stackCommand, 101);
assert.deepStrictEqual(read('r107-stack-repro-source.json'), originalBaseline);
for (const run of [originalGates[0], reproduction]) {
  const text = log(run);
  for (const marker of [stackTest, 'has overflowed its stack', 'fatal runtime error: stack overflow', 'SIGABRT']) {
    assert(text.includes(marker), marker);
  }
}
assert.strictEqual(totals(log(originalGates[0])).harnesses, 47);
historical.push({name: 'r107-final', returncode: 101, attempted_gates: 1, unstarted_gates: 16,
  accepted_campaign: false, failure: 'runtime harness stack-overflow abort'});
historical.push({name: 'r107-stack-repro', returncode: 101, accepted_campaign: false,
  failure: 'same named runtime test stack-overflow abort'});
const fixedStack = checkRun('r107-accepted-stack', stackCommand, 0, true);
assert.deepStrictEqual(passing(log(fixedStack)), [stackTest]);
assert.deepStrictEqual(totals(log(fixedStack)), {harnesses: 1, passed: 1, failed: 0, ignored: 0});
for (const [name, command, count] of [
  ['r107-slot-stack', stackCommand, 1],
  ['r107-slot-matrix', [...kfd, 'shared_memory::tests::device_initialization'], 11],
  ['r107-slot-clippy', ['cargo', '+nightly-2026-04-03', 'clippy', '--locked', '--offline', '-p', 'fe2o3-kfd', '-p', 'fe2o3-runtime', '-p', 'fe2o3-host', '--all-features', '--all-targets', '--', '-D', 'warnings'], null],
]) {
  const run = checkRun(name, command, 0);
  assert(Date.parse(run.finished_at) <= Date.parse(environment.recorded_at));
  assert.deepStrictEqual(read(name + '-source.json'), baseline);
  if (count !== null) assert.deepStrictEqual(totals(log(run)), {harnesses: 1, passed: count, failed: 0, ignored: 0});
  historical.push({name, returncode: 0, accepted_campaign: false});
}
const format = read('r107-slot-format.json');
assert.strictEqual(format.source_head, parent);
assert.strictEqual(format.returncode, 0);
assert.strictEqual(format.source_unchanged, false);
assert.strictEqual(format.timed_out, false);
assert.strictEqual(format.signal, null);
assert.deepStrictEqual(format.command, ['cargo', '+nightly-2026-04-03', 'fmt', '--all']);
assert(Date.parse(format.finished_at) <= Date.parse(environment.recorded_at));
historical.push({name: 'r107-slot-format', returncode: 0, accepted_campaign: false, expected_source_reformat: true});
for (const run of historical) {
  const snapshot = file(run.name + '-source.json');
  if (fs.existsSync(snapshot)) {
    run.source_snapshot_sha256 = hash(fs.readFileSync(snapshot));
    run.source_identities = Object.keys(JSON.parse(fs.readFileSync(snapshot))).length;
  }
}
assert(fs.readFileSync(file('r107-accepted-python.log'), 'utf8').includes('Ran 151 tests'));
assert(fs.readFileSync(file('r107-accepted-dependency-tests.log'), 'utf8').includes('Ran 8 tests'));
const metadata = auxiliary.find(r => r.name === 'production-metadata');
assert.strictEqual(metadata.stderr_log, file('r107-accepted-production-metadata.stderr.log'));
...
const summary = {publication_parent: parent, accepted_runtime_checkpoint: accepted,
  ...
  focused_tests: {frozen_matrix: 11, restored_matrix: 11, frozen_borrowed: 8, restored_borrowed: 8, frozen_transitions: 30, restored_transitions: 30},
  historical_attempts: historical, initial_formatter_output_retained: false,
  stack_regression: {original_failure: 'r107-stack-repro', accepted_pass: 'r107-accepted-stack',
    command_unchanged: true, runner_controlled_environment_unchanged: true,
    explicit_runner_stack_override: false, original_ambient_stack_limits_recorded: false},
  terminal_storage: 'one fallibly preallocated slot per engine; no retention-time growth; not global allocator-count or total-memory qualification',
