...
RELATIVE_ARCHIVE = Path(
    "docs/evidence/dev-primary-envelope-late-selection-cpu-2026-09-18"
)
...
def script_commands(suffix, relative):
    tag = f"-{suffix}" if suffix else ""
    root = RELATIVE_ARCHIVE if relative else ARCHIVE
    scripts = [str(root / name) for name in ("source.py", "binaries.py", "verify.py")]
    shells = [
        str(root / name)
        for name in ("record.sh", "qualify.sh", "qualify-final.sh", "seal.sh")
    ]
    return {
        f"python-lint{tag}": ["ruff", "check", "--no-cache"] + scripts,
        f"python-format{tag}": ["ruff", "format", "--check", "--no-cache"] + scripts,
        f"shell-syntax{tag}": ["bash", "-n"] + shells,
        f"shellcheck{tag}": ["shellcheck"] + shells,
        f"verify{tag}": ["python3", "-I", str(root / "verify.py")],
    }
...
def verify():
    ...
    assert (
        digest(ARCHIVE / "failed-verifier.py")
        == "bec6d11676b5fee60edbe4ec21f907576baf92faf1ea62842074bd286845dd9d"
    )
    ...
    first_scripts = script_commands("", True)
    final_scripts = script_commands("final", False)
    ordered = (
        [
        ...
        + [
            (name, command, 1 if name == "verify" else 0)
            for name, command in first_scripts.items()
        ]
        + [(name, command, 0) for name, command in final_scripts.items()]
    )
    expected_raw = {
        f"{name}.{suffix}"
    ...
    if pending:
        expected_raw -= {"verify-final.finished", "verify-final.exit"}
        assert (
            shlex.split((ARCHIVE / "raw/verify-final.command").read_text())
            == final_scripts["verify-final"]
        )
    times = []
        ...
        json.dumps(
            {
                ...
                "successful_receipts": len(ordered) - 2,
