...
ARENA_ROOT = MODEL / 'verus/r75_resource_domain_arena_v1.rs'
UNITS = (('planner', ROOT, 19), ('arena', ARENA_ROOT, 21))
VECTOR = MODEL / 'src/resource_vector_bodies.rs'
BODY = MODEL / 'src/r75_resource_domain_bodies.rs'
ARENA = Path('crates/fe2o3-resource-accounting/src/domain/arena_bodies.rs')
...
PROOF_FILES = frozenset((ROOT, VECTOR, BODY, ARENA,
    Path('crates/fe2o3-resource-accounting/src/domain/arena_declarations.rs'),
    ARENA_ROOT,
...
def source_gate(manifest, captured):
    ...
    adapter = captured[Path('crates/fe2o3-resource-accounting/src/domain.rs')].decode()
    for name, arguments in (
        ('resource_domain_node_body_v1', 'nodes,key'),
        ('resource_domain_path_extract_body_v1', 'resource_domain_arena_rust_expr,nodes,profile,leaf,path,next,depth,[],[],[],[]'),
        ('resource_domain_facts_body_v1', 'resource_domain_arena_rust_expr,nodes,path,depth,facts,index,[],[],[]'),
    ):
        need(adapter.count(name + '!') == 1
             and name + '!(' + arguments + ')' in re.sub(r'\s+', '', adapter),
             'one hook-free arena body: ' + name)
...
def mutations(captured):
    roster = [
        ...
        ('ignore-key-generation', ARENA, 'node.key.slot == $key.slot && node.key.generation == $key.generation', 'node.key.slot == $key.slot'),
        ('ignore-key-slot', ARENA, 'node.key.slot == $key.slot && node.key.generation == $key.generation', 'node.key.generation == $key.generation'),
        ('ignore-root-generation', ARENA, '$path[$depth - 1].slot != ROOT.slot || $path[$depth - 1].generation != ROOT.generation', '$path[$depth - 1].slot != ROOT.slot'),
        ('accept-nonroot-terminal', ARENA, '$path[$depth - 1].slot != ROOT.slot || $path[$depth - 1].generation != ROOT.generation', 'false'),
        ('accept-invalid-profile', ARENA, '$profile != MAX_RESOURCE_DOMAIN_DEPTH_V1 && $profile != MAX_RESOURCE_CLASS_DOMAIN_DEPTH_V1', 'false'),
        ('ignore-selected-depth', ARENA, '$depth == $path.len() || $depth == $profile', '$depth == $path.len()'),
        ('stop-before-parent', ARENA, 'Some(node) => node.parent,', 'Some(_node) => None,'),
        ('force-root-parent', ARENA, 'Some(node) => node.parent,', 'Some(_node) => Some(ROOT),'),
        ('substitute-root-leaf', ARENA, 'let mut $next = Some($leaf);', 'let mut $next = Some(ROOT);'),
        ('wrong-path-padding', ARENA, 'let mut $path = [ROOT; MAX_RESOURCE_CLASS_DOMAIN_DEPTH_V1];', 'let mut $path = [$leaf; MAX_RESOURCE_CLASS_DOMAIN_DEPTH_V1];'),
        ('omit-last-fact', ARENA, 'while $index < $depth', 'while $index + 1 < $depth'),
        ('wrong-fact-node', ARENA, 'domain_node_v1($nodes, $path[$index])', 'domain_node_v1($nodes, $path[0])'),
        ('wrong-fact-used', ARENA, 'used: node.used,', 'used: node.capacity,'),
        ('wrong-fact-capacity', ARENA, 'capacity: node.capacity,', 'capacity: node.used,'),
        ('omit-fact-quarantine', ARENA, 'counts: node.counts,', 'counts: [node.counts[0], node.counts[1], 0],'),
        ('wrong-fact-record-limit', ARENA, 'record_limit: node.record_limit,', 'record_limit: 0,'),
    ]
    ...
    need(len({name for name, _, _ in result}) == 31, 'distinct mutation names')
    need(len({(path, data) for _, path, data in result}) == 31, 'distinct mutation bodies')
    return result


def classify(code, log, negative=False, obligations=19):
    need(obligations in (19, 21), 'reviewed proof unit')
    summaries = re.findall(rb'^verification results:: (\d+) verified, (\d+) errors$', log, re.M)
    if not negative:
        need(code == 0 and summaries == [(str(obligations).encode(), b'0')], 'exact positive proof result')
        ...
        need(sum(map(int, summaries[0])) == obligations, 'complete negative obligation roster')
        ...
        allowed = rb'error: (postcondition not satisfied|precondition not satisfied|invariant not satisfied.*|assertion failed|aborting due to \d+ previous errors?(; \d+ warnings? emitted)?)'
...
def main():
    ...
    for name, changed in cases:
        ...
        for label, proof, obligations in UNITS:
            command = [str(verus), '--crate-type', 'lib', '--no-cheating', '--num-threads', '2', '--triggers-mode', 'silent', str(root / proof)]
            code, log = execute(command, root, env, output, name + '-' + label)
            negative = changed is not None and ((changed[0] == ARENA) == (proof == ARENA_ROOT))
            classify(code, log, negative=negative, obligations=obligations)
    ...
    (output / 'result.json').write_text(json.dumps({'status': 'passed', 'obligations_per_unit': {label: count for label, _, count in UNITS},
        'positive_runs': 2, 'logical_mutations': 31, 'source_manifest_sha256': MANIFEST_SHA,
        'proof_unit_runs': len(cases) * len(UNITS),
        'scope': 'shared planner/vector and immutable arena lookup/path/fact extraction; not mutable ledger/lock/native refinement'}, indent=2) + '\n')
    print('R75_SHARED_DOMAIN_PLANNER_OK planner=19 arena=21 positives=2 mutations=31')
