...
impl ResourceCreditAccountV1 {
    ...
    /// Checks this exact account's live retained record and complete charge.
    /// Domain accounts additionally require a valid generation-bound ancestry.
    /// This borrows both owners, takes at most one ledger lock, and neither
    /// allocates nor changes credits, handles, poison flags or retention anchors.
    /// A poisoned mutex or ledger rejects without recovering it.
    ///
    /// This is an accounting observation, not native execution/disposal authority
    /// or a proof of whole-ledger conservation. The adapter must keep the token
    /// with its represented resource for the entire operation. Equal charges in
    /// the same exact account are accounting-equivalent, not allocation identities.
    pub fn matches_retained_charge_v1(
        &self,
