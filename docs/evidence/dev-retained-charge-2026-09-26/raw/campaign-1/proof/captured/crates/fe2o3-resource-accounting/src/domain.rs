...
macro_rules! resource_domain_arena_declarations_v1 {
    ($($items:tt)*) => { $($items)* };
}
...
macro_rules! resource_domain_arena_rust_expr {
    ($body:expr) => {
        $body
    };
}
...
#[inline]
fn domain_node_v1(nodes: &[Option<Node>], key: Key) -> Option<&Node> {
...
fn domain_path_v1(
    ...
    profile: usize,
    leaf: Key,
) -> Option<([Key; MAX_RESOURCE_CLASS_DOMAIN_DEPTH_V1], usize)> {
    resource_domain_path_extract_body_v1!(
        resource_domain_arena_rust_expr,
        nodes,
        profile,
        leaf,
        path,
...
fn domain_facts_v1(
    nodes: &[Option<Node>],
    path: &[Key; MAX_RESOURCE_CLASS_DOMAIN_DEPTH_V1],
    depth: usize,
) -> Option<[R75ResourceDomainFactsV1; R75_RESOURCE_DOMAIN_LEVELS_V1]> {
    resource_domain_facts_body_v1!(
        resource_domain_arena_rust_expr,
        nodes,
        path,
        depth,
...
impl State {
    ...
    fn node(&self, key: Key) -> Result<&Node, ResourceCreditErrorV1> {
        domain_node_v1(&self.nodes, key).ok_or(ResourceCreditErrorV1::Invariant)
    }
    ...
    fn path(
        ...
        domain_path_v1(&self.nodes, self.max_depth, leaf).ok_or(ResourceCreditErrorV1::Invariant)
    }
...
impl DomainAccount {
    ...
    pub(super) fn reserve_into(
        ...
        let result = (|| {
            ...
            let facts = domain_facts_v1(&state.nodes, &path, depth).expect("validated domain path");
