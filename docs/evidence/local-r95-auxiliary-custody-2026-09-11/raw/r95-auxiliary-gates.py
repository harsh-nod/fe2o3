...
commands = [
    ...
    ('production-audit', ['python3', '-B', 'scripts/runtime_pure_rust_audit.py', 'metadata',
        '--input', str(root / 'r95-production-metadata.log'), '--root', 'fe2o3-runtime']),
]
results = []
for name, command in commands:
    log = root / f'r95-{name}.log'
    ...
    results.append(row)
    (root / 'r95-auxiliary-results.json').write_text(json.dumps(results, indent=2) + '\n')
