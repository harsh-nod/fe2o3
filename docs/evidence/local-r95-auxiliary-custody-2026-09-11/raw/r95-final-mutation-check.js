...
for (const mutation of JSON.parse(fs.readFileSync(path.join(root, 'r95-mutations.json')))) {
  const stem = path.join(root, 'r95-mutation-' + mutation.name);
  ...
  assert(log.includes('test ' + mutation.test + ' ... FAILED'));
...
console.log('PASS: 8 compiled negative mutations rejected; ' + names.length + ' source identities exactly restored');
