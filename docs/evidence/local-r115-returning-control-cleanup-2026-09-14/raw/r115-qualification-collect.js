...
const priorFullName = 'r115-preliminary-gnu-all';
...
function checkBaseline() {
  ...
  assert.strictEqual(env.reboot_anchor, p.rebootAnchor[0]);
  ...
  assert.deepStrictEqual(env.prerequisite_validation_inputs.map(x => x.name), [...new Set([
    ...
    ...artifactSuffixes.map(s => p.rebootAnchor[0] + s), ...p.freezeHelpers])].sort());
  checkHistoricalAttempts(env);
  for (const suffix of artifactSuffixes) assert(env.prior_artifacts.some(x => x.name === priorFullName + suffix));
  const priorFull = e.checkRun(priorFullName, p.prerequisites[0][1], map, 'r115-preliminary-clippy.json');
  checkFull(priorFull.log, e.committed('raw/r114-preliminary-gnu-all.log'));
  const [anchorName, anchorCommand, anchorPrevious] = p.rebootAnchor;
  const anchor = e.checkRun(anchorName, anchorCommand, map, anchorPrevious);
  assert.strictEqual(anchor.log, '');
  assert.notStrictEqual(anchor.record.clock.start.boot_id, priorFull.record.clock.source_verified.boot_id, 'fresh boot, not a fabricated causal edge');
  e.ordered(anchor.record.clock.source_verified, env.observation);
  ...
  previous = 'r115-refreshed-musl-all.json';
...
function collect(closed) {
  ...
  const summary = {source_head: p.parent, accepted_parent: p.accepted, source_map_sha256: e.hash(JSON.stringify(map)),
    ...
    reboot_anchor: p.rebootAnchor[0], pre_reboot_full_result: {name: priorFullName, accepted_as_current_boot_prerequisite: false},
...
if (require.main === module) {
  ...
  if (mode === '--archive') {
    const dest = path.join(e.repo, 'docs/evidence/local-r115-returning-control-cleanup-2026-09-14');
    ...
    fs.writeFileSync(path.join(dest, 'README.md'), '# R115 Returning Control Cleanup Evidence\n\n' +
      ...
      'A successful earlier GNU run is retained on its original boot; it is not a current-boot prerequisite. A fresh independent anchor and refreshed GNU/musl runs establish the current qualification timeline without a cross-boot monotonic claim.\n' +
      ...
      'Six pre-reboot helper versions are retained alongside the current helpers; prior observations are not rewritten to imply execution of the updated validation plan.\n' +
