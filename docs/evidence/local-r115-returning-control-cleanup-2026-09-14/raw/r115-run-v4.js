...
assert(/^r115-[a-z0-9-]+$/.test(name) && previousName && command);
...
(async () => {
  ...
  let previous = null;
  if (previousName !== '-') {
    assert(/^r115-[a-z0-9-]+\.json$/.test(previousName));
    ...
    assert.strictEqual(record.clock.contract, 'r115-raw-utc-boot-monotonic-v1');
  ...
  const deadline = ['r115-source-campaign', 'r115-auxiliary-campaign'].includes(name) ? 7200000 : 1800000;
  let timedOut = false;
  ...
  const record = {source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    clock: {contract: 'r115-raw-utc-boot-monotonic-v1', predecessor: previous, start, finish: result.finish, source_verified: verified, error: clockError}};
