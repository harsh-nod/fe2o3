...
const source = e.read('r115-frozen-source.json');
let count = 0;
...
count++; console.log('PASS independent-reboot-anchor');
...
assert.throws(() => e.ordered(e.read('r115-preliminary-gnu-all.json').clock.source_verified, anchor.clock.start), assert.AssertionError);
...
assert.deepStrictEqual(entries['r115-qualified-mut-' + mutation.name + '.json'].command, [...p.test, mutation.test, '--', '--exact']);
...
for (const [label, text] of [
  ['r115-gnu-target-roster', e.bytes('r115-refreshed-gnu-all.log').toString()],
  ['r115-musl-target-roster', e.bytes('r115-refreshed-musl-all.log').toString()],
  ['r114-gnu-target-roster', e.committed('raw/r114-preliminary-gnu-all.log')],
  ['r114-musl-target-roster', e.committed('raw/r114-preliminary-musl-all.log')],
]) {
...
const {checkFull, docNames} = require('./r115-qualification-collect.js');
for (const target of ['gnu', 'musl']) {
  const log = e.bytes('r115-refreshed-' + target + '-all.log').toString();
  const old = e.committed('raw/r114-preliminary-' + target + '-all.log');
...
for (const target of ['gnu', 'musl']) {
  ...
  assert.deepStrictEqual(renamed, e.passing(e.bytes('r115-final-' + target + '-docs.log').toString()));
}
