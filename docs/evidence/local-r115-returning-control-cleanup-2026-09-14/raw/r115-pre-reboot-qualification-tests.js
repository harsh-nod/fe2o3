...
for (const [label, text] of [
  ['r115-gnu-target-roster', e.bytes('r115-preliminary-gnu-all.log').toString()],
  ['r115-musl-target-roster', e.bytes('r115-preliminary-musl-all.log').toString()],
...
for (const target of ['gnu', 'musl']) {
  const log = e.bytes('r115-preliminary-' + target + '-all.log').toString();
