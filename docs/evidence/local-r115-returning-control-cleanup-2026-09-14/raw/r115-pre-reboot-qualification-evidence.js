...
const contract = 'r115-raw-utc-boot-monotonic-v1';
...
function checkRecord(r, name, command, expectedMap, previousName, code, io = {read, bytes}) {
  ...
  assert.strictEqual(r.deadline_ms, ['r115-source-campaign', 'r115-auxiliary-campaign'].includes(name) ? 7200000 : 1800000);
  assert.deepStrictEqual(r.environment, {CARGO_BUILD_JOBS: '4', CARGO_INCREMENTAL: '0', RUST_TEST_THREADS: '4', XDG_RUNTIME_DIR: null});
  assert.deepStrictEqual(r.runner, {name: 'r115-run-v4.js', sha256: hash(io.bytes('r115-run-v4.js'))});
...
function expectedEntries(map, originals) {
  const entries = {};
  let predecessor = 'r115-qualification-manifest.json';
  ...
  for (const mutation of p.mutations) {
    ...
    hashes.add(mapHash);
    const run = 'r115-qualified-mut-' + mutation.name + '.json';
    entries[run] = {kind: 'run', predecessor, source_map_sha256: mapHash,
      command: [...p.test, mutation.test, '--', '--exact'], returncode: 101};
    predecessor = run;
    const restored = 'r115-qualified-restoration-' + mutation.name + '.json';
  ...
  for (const [kind, filter] of p.focused) {
    const run = 'r115-qualified-restored-' + kind + '.json';
    entries[run] = {kind: 'run', predecessor, source_map_sha256: hash(JSON.stringify(map)), command: [...p.test, filter], returncode: 0};
    predecessor = run;
  }
  entries['r115-qualified-collector-validation.json'] = {kind: 'run', predecessor,
    source_map_sha256: hash(JSON.stringify(map)), command: ['node', file('r115-qualification-collect.js'), '--check'], returncode: 0};
...
function historicalNames() {
  const runs = p.mutations.map(m => 'r115-qualified-mut-' + m.name)
    .concat(p.focused.map(([kind]) => 'r115-qualified-restored-' + kind), ['r115-qualified-collector-validation']);
  const excluded = new Set(runs.flatMap(n => ['.json', '.log', '-source.json', '-source-after.json'].map(s => n + s))
    .concat(p.mutations.flatMap(m => ['.json', '-source.json'].map(s => 'r115-qualified-restoration-' + m.name + s)),
      ['r115-qualification-manifest.json']));
  assert.strictEqual(excluded.size, 6 * p.mutations.length + 4 * p.focused.length + 5);
  return fs.readdirSync(root).filter(n => /^r115-.*\.(json|log|js|py)$/.test(n) && !excluded.has(n)).sort();
}
