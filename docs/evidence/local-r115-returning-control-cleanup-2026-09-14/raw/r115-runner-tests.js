...
const runner = path.join(root, 'r115-run-v4.js');
...
async function mocked(name, mode) {
  ...
  const fakeProcess = {
    argv: ['node', runner, 'r115-mocked-' + name, '-', 'fixture'], env: {},
  ...
  const record = JSON.parse(writes.get(path.join(root, 'r115-mocked-' + name + '.json')));
...
(async () => {
  ...
  try {
    ...
    const cases = [
      ...
      ['spawn', '/no-such-r115-qualification-command', [], 127],
    ];
    for (const [name, command, args, expected] of cases) {
      const label = 'r115-runner-v2-fixture-' + name;
      let result;
      try {
