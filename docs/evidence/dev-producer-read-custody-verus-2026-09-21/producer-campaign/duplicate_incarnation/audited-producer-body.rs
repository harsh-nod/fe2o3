...
{
    if contents.reservations.len() > 1 { if let (Some(first), Some(mut second)) = (contents.reservations[0], contents.reservations[1]) { second.reference.incarnation = first.reference.incarnation; contents.reservations.set(1, Some(second)); } }
    return ();
}
}
