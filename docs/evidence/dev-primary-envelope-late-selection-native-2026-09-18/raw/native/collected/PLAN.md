...
- Signed containing commit: `a0db73625c421e26922a1bb8cab4dec49012417c`.
...
- CPU packet: `docs/evidence/dev-primary-envelope-late-selection-cpu-2026-09-18`.
...
The exporter creates and checks local directory mode 0700 before copying files;
SCP must not propagate a less-private local directory mode to remote staging.

Only after preparation/review may root create one fresh remote directory with
`mktemp -d /tmp/fe2o3-r126-primary-a0db7362-20260918.XXXXXXXX`, record its exact
...
Signed source identity, final CPU cohort and final binary identity received.
The local export receipt must independently bind all of them. No SSH, GPU
admission, remote export, native launch or cleanup has been performed for this
plan. Root READY review, a closed previous GPU handoff, a fresh marked remote
path and explicit root execution authorization remain prerequisites.
