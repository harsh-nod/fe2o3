...
def check_finalization(repo, packet):
    ...
    for path in paths:
        ...
        if path.is_relative_to(PACKET):
            # The final report replaces the explicitly captured in-progress README.
            if path.name != 'README.md':
                need(data == ordinary(packet, path.relative_to(PACKET)), 'unchanged executable packet control')
        else:
