...
- Eleven checker-calibration groups, five packet-calibration groups (including
  relocated read-only replay), eight finalizer groups and five inherited cleanup
  groups pass. The independent post-collection calibration is recorded separately
  in `post-collection-tests`; its first run is rejected as described below.
...
```sh
python3 -I -B docs/evidence/dev-mixed-acquisition-proof-2026-09-24/audit.py
```
