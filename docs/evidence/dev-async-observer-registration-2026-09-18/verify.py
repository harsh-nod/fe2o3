...
BASE = "5d70cb0a6e16fb265fe224690274fdb0be2b0055"
MUSL_HIP_WARNING = (
...
def docs(text):
    ...
    assert counts == {"fe2o3_host": [4, 21], "fe2o3_runtime": [3, 42]}, counts
    bundle = [
        kind
...
def verify():
    ...
    rosters = {}
    for target in ("gnu", "musl"):
        for crate, passed, ignored in (("host", 282, 4), ("runtime", 1101, 17)):
            name = target + "-" + crate
    ...
    additions = {
        ...
        if "::registration_tests::observer_registration_" in name
    }
    assert len(additions) == 13 and all(
    ...
    baselines = {
        ...
        "runtime": (
            "e7bde73cd8e2d149e76c8150702f02510753fd9f790ae4fa74130b8270ed7547",
            1088,
            17,
        ),
    }
    for crate, (digest, passed, ignored) in baselines.items():
        raw = (
            ARCHIVE.parent / f"dev-current-thread-owner-2026-09-18/raw/gnu-{crate}.log"
        ).read_bytes()
    ...
    assert current["files"] == after["files"] and len(after["files"]) == 5540
