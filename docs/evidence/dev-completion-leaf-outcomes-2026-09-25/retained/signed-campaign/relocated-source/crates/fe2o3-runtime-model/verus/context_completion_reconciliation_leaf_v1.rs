...
pub proof fn leaf_iteration_shape(initial: CompletionTableV1, current: CompletionTableV1,
    ...
    ensures current.contains(id), custody_valid(current, id),
        !current.node(id).generated_owner,
        current.node(id).state == initial.node(id).state,
        current.node(id).state.is_some(),
        current.node(id).state.unwrap().terminal == Some(BackendPollV1::Succeeded),
        current.node(id).dependencies@.len() == 0,
        current.node(id).input == initial.node(id).input,
        match current.node(id).input { Ok(input) => leaf_input(input), Err(_) => false },
        current.node(id).settlement_failure == initial.node(id).settlement_failure,
        current.node(id).record.status == if iteration == 0 { RuntimeCompletionStatusV1::Pending }
            else { leaf_target(initial, id) },
        iteration == 0 && current.node(id).record.directed_peer_copy ==> pending_roots_result(current, id, 1) == Ok(()),
        iteration == 0 && current.node(id).record.producer_launch ==> pending_roots_result(current, id, 2) == Ok(()),
{
