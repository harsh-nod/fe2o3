...
macro_rules! begin_reserved_body {
...
macro_rules! begin_canonical_body {
...
macro_rules! begin_destinations_body {
    ...
    ($syntax:ident, $journal:ident, $count:ident, $index:ident, [$($annotations:tt)*]) => {
        $syntax!({
                ...
                $($annotations)*
            {
                ...
                if $index > 0 && member == $journal.member_free[$journal.member_free.len() - $index] { return Err(ReadErrorV1::InvalidState); }
                begin_indexed_access_v1($journal);
...
macro_rules! begin_preflight_body {
...
macro_rules! begin_execution_body {
    ($syntax:ident, $journal:ident, $writer:ident, $roster:ident, $reserved_count:ident,
