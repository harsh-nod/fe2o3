...
def render_results(packet):
    performance = read_module(packet / 'performance.py', 'begin_render_performance')
    lines = [
        ...
        performance.table((packet / 'cargo/performance/stdout').read_text()), '',
        '## Limits', '',
...
def audit(repo, packet):
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-audit-') as temporary:
        ...
        performance = read_module(tree / PACKET / 'performance.py', 'begin_offline_performance')
        ...
        for name, command in cpu.commands():
            ...
            if environment is None:
                ...
                need(target.is_absolute() and target.name == 'target' and target.parent.name.startswith('fe2o3-begin-execution-'), 'owned CPU target')
            ...
            if name == 'test':
                need(test_results(out) == [(878, 0, 10, 0, 0), (27, 0, 0, 0, 0)], 'complete unit/doctest result roster')
            if name == 'performance':
                need(test_results(out) == [(1, 0, 0, 0, 887)], 'single release benchmark result')
...
def selftest(repo, packet):
    ...
    performance = read_module(packet / 'performance.py', 'begin_selftest_performance')
    ...
    modifications = [
        ...
        ('decisions', 'proof/positive-before/' + str(check.DECISIONS), b'// changed decisions\n'),
        ('benchmark', 'cargo/performance/stdout', None),
        ('witness', 'proof/positive-before/' + str(check.WITNESS), b'// changed witness\n'),
    ]
    for name, relative, addition in modifications:
        with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-negative-') as temporary:
            ...
            if addition is not None:
                ...
                key = next(key for key in row if key.endswith('/context_version_journal/begin.rs'))
                row[key] = '0' * 64
                target.write_text(json.dumps(row))
            elif name == 'benchmark':
                target.write_text(target.read_text().replace(',shared,', ',baseline,', 1))
                receipt = target.with_name('receipt.json')
                row = base.unique_json(receipt.read_text())
...
def main():
    ...
    if args.selftest:
        ...
        print('PASS: standalone actual-type Begin proof and CPU packet reconstructed from Git objects')
