 ...
 producer_acquire_item_body {
    ($contents:ident, $consumer:ident, $request:ident, $index:ident, $state:ident) => {{
        ...
        let count = match $contents.counts[$request.read.allocation.slot].checked_add(0) {
            Some(count) => count,
