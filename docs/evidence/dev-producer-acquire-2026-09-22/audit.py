...
def audit(repo, packet):
    ...
    need(not git(repo, 'diff', '--name-only', '4c237366268542e1a19542b25c00f7b58d9f24a8', source, '--',
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-audit-') as temporary:
        ...
        for name, command in cpu.commands():
            ...
            if environment is None:
                ...
                need(target.is_absolute() and target.name == 'target' and target.parent.name.startswith('fe2o3-producer-acquire-'), 'owned CPU target')
            ...
            if name == 'test':
                need(test_results(out) == [(914, 0, 16, 0, 0), (27, 0, 0, 0, 0)], 'complete unit/doctest result roster')
            if name == 'performance':
                need(test_results(out) == [(1, 0, 0, 0, 929)], 'exact release producer-acquire benchmark result')
    ...
    for name, relative, addition in modifications:
        with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-negative-') as temporary:
            ...
            if addition is not None:
                ...
                key = next(key for key in row if key.endswith('/context_producer_reads/acquire.rs'))
                row[key] = '0' * 64
                ...
                index = next(i for i, line in enumerate(rows) if 'producer_acquire,' in line)
                del rows[index]
...
def main():
    ...
    if args.selftest:
        ...
        print('PASS: standalone actual producer acquisition proof and instrumented CPU packet reconstructed from Git objects')
