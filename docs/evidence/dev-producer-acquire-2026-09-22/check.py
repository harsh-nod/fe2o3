#!/usr/bin/env python3
"""Actual producer-read acquisition correspondence campaign with scoped controls."""
import argparse
import hashlib
...
PACKET = Path('docs/evidence/dev-producer-acquire-2026-09-22')
ROOT = CRATE / 'verus/context_producer_acquire_historical_v1.rs'
EXECUTION_ROOT = CRATE / 'verus/context_producer_acquire_execution_v1.rs'
RAW_ROOT = CRATE / 'verus/context_journal_begin_execution_v1.rs'
VIEWS = CRATE / 'verus/context_reader_owner_views_v1.rs'
BRIDGE = CRATE / 'verus/context_producer_acquire_correspondence_v1.rs'
LIVE = CRATE / 'verus/context_producer_acquire_witnesses_v1.rs'
DECISIONS = CRATE / 'verus/context_producer_acquire_decisions_v1.rs'
PROOF = CRATE / 'verus/context_producer_acquire_bodies_v1.rs'
BODY = CRATE / 'src/context_producer_reads/acquire_bodies.rs'
COUNT = CRATE / 'src/context_read_leases/count_bodies.rs'
WRITER = CRATE / 'src/context_version_journal/writer_lookup_bodies.rs'
HISTORICAL = CRATE / 'verus/context_producer_acquire_historical_bodies_v1.rs'
...
PROJECTIONS = [(BEGIN, 'audited-begin-bodies.rs', 8),
               ...
               (BODY, 'audited-producer-acquire-bodies.rs', 10),
...
MUTATIONS = [
    ("stable_count", COUNT, "macro_rules! stable_retained_read_count_body {", "production", "ContextReadLeasedJournalV1::retained_read_count", "$contents.leases.len() - $contents.free_reads.len()", "$contents.leases.len()", "execution"),
    ("producer_count", BODY, "macro_rules! producer_retained_count_body {", "production", "ContextProducerReadJournalV1::retained_producer_read_count", "$contents.reservations.len() - $contents.free.len()", "$contents.reservations.len()", "execution"),
    ("combined_count", BODY, "macro_rules! producer_total_read_count_body {", "production", "ContextProducerReadJournalV1::retained_read_count", "$contents.stable.retained_read_count() + $contents.retained_producer_read_count()", "$contents.retained_producer_read_count()", "execution"),
    ("remaining_count", BODY, "macro_rules! producer_remaining_read_slots_body {", "production", "ContextProducerReadJournalV1::remaining_read_slots", "$contents.reservations.len() - $contents.retained_read_count()", "$contents.reservations.len()", "execution"),
    ("stable_budget", BODY, "macro_rules! producer_stable_capacity_body {", "production", "ContextProducerReadJournalV1::validate_read_capacity", "$count > $contents.remaining_read_slots()", "false", "execution"),
    ("stable_epoch", BODY, "macro_rules! producer_stable_capacity_body {", "production", "ContextProducerReadJournalV1::validate_read_capacity", "$contents.stable.validate_read_capacity($count)", "Ok(())", "execution"),
    ("budget_endpoint", BODY, "macro_rules! producer_capacity_body {", "production", "ContextProducerReadJournalV1::validate_producer_read_capacity", "$count > $contents.remaining_read_slots()", "$count >= $contents.remaining_read_slots()", "execution"),
    ("zero_epoch", BODY, "macro_rules! producer_capacity_body {", "production", "ContextProducerReadJournalV1::validate_producer_read_capacity", "$contents.next_incarnation == 0", "false", "execution"),
    ("epoch_overflow", BODY, "macro_rules! producer_capacity_body {", "production", "ContextProducerReadJournalV1::validate_producer_read_capacity", "checked_add($count as u64)", "checked_add(0)", "execution"),
    ("header_context", BODY, "macro_rules! producer_acquire_header_body {", "production", "producer_acquire_header_exec_v1", "$consumer.context_generation != $generation", "false", "execution"),
    ("consumer_zero", BODY, "macro_rules! producer_acquire_header_body {", "production", "producer_acquire_header_exec_v1", "$consumer.local == 0", "false", "execution"),
    ("consumer_kind", BODY, "macro_rules! producer_acquire_header_body {", "production", "producer_acquire_header_exec_v1", "!matches!($consumer.kind, ContextWriterKindV1::Submission)", "false", "execution"),
    ("output_vacancy", BODY, "macro_rules! producer_acquire_header_body {", "production", "producer_acquire_header_exec_v1", "!producer_output_vacant_exec_v1($output)", "false", "execution"),
    ("early_budget", BODY, "macro_rules! producer_acquire_header_body {", "production", "producer_acquire_header_exec_v1", "if $consumer.context_generation", "let _budget = $contents.remaining_read_slots();\n        if $consumer.context_generation", "execution"),
    ("item_error", BODY, "macro_rules! producer_acquire_item_body {", "production", "producer_acquire_item_exec_v1", "Err(error) => return Err(error)", "Err(_error) => return Err(ContextVersionJournalErrorV1::InvalidState)", "execution"),
    ("producer_order", BODY, "macro_rules! producer_acquire_item_body {", "production", "producer_acquire_item_exec_v1", "$request.producer.key.local >= $consumer.local", "$request.producer.key.local > $consumer.local", "execution"),
    ("canonical", BODY, "macro_rules! producer_acquire_item_body {", "production", "producer_acquire_item_exec_v1", "!producer_read_order_less_exec_v1(prior, key)", "false", "execution"),
    ("group_count", BODY, "macro_rules! producer_acquire_item_body {", "production", "producer_acquire_item_exec_v1", "group = $state.group + 1;", "group = 1;", "execution"),
    ("count_overflow", BODY, "macro_rules! producer_acquire_item_body {", "production", "producer_acquire_item_exec_v1", "checked_add(group)", "checked_add(0)", "execution"),
    ("free_tail", BODY, "macro_rules! producer_acquire_item_body {", "production", "producer_acquire_item_exec_v1", "$contents.free[$contents.free.len() - $index - 1]", "$contents.free[$index]", "execution"),
    ("occupied_slot", BODY, "macro_rules! producer_acquire_item_body {", "production", "producer_acquire_item_exec_v1", "$contents.reservations[slot].is_some()", "false", "execution"),
    ("commit_pop", BODY, "macro_rules! producer_acquire_commit_body {", "production", "producer_acquire_commit_exec_v1", "let slot = $contents.free.pop().expect(\"preflighted free reservation\");", "let slot = $contents.free[$contents.free.len() - 1];", "execution"),
    ("commit_count", BODY, "macro_rules! producer_acquire_commit_body {", "production", "producer_acquire_commit_exec_v1", "$contents.counts[allocation] += 1;", "$contents.counts[allocation] += 0;", "execution"),
    ("commit_output", BODY, "macro_rules! producer_acquire_commit_body {", "production", "producer_acquire_commit_exec_v1", "$output[$index] = Some(reference);", "$output[$index] = None;", "execution"),
    ("commit_epoch", BODY, "macro_rules! producer_acquire_commit_body {", "production", "producer_acquire_commit_exec_v1", "$contents.next_incarnation += $requests.len() as u64;", "$contents.next_incarnation += 0;", "execution"),
    ("rejection_frame", BODY, "macro_rules! producer_acquire_execution_body {", "production", "ContextProducerReadJournalV1::acquire_producer_reads", "Err(error) => return Err(error)", "Err(error) => { $contents.next_incarnation = 0; return Err(error); }", "execution"),
    ("skip_actual_acquire", BRIDGE, "fn producer_acquire_historical_exec_v1(", "production", "producer_acquire_historical_exec_v1", "let result = actual.acquire_producer_reads(consumer, requests, output);", "let result: Result<(), ReadErrorV1> = Err(ReadErrorV1::InvalidState);", "execution"),
    ("skip_historical_acquire", BRIDGE, "fn producer_acquire_historical_exec_v1(", "production", "producer_acquire_historical_exec_v1", "let model_result = logical::producer_acquire_exec_v1(model, model_consumer, model_requests, model_output);", "let model_result: Result<(), logical::ReadErrorV1> = Err(logical::ReadErrorV1::InvalidState);", "execution"),
    ("conditional_domain", DECISIONS, "spec fn producer_acquire_domain_v1(", "production", "producer_acquire_bad_budget_prefix_witness_v1", "producer_acquire_prefix_v1(contents, consumer, count, output).is_ok() ==> ", "", "contract"),
    ("output_projection", BRIDGE, "spec fn producer_output_slot_view(", "production", "producer_acquire_header_correspondence", "Some(reference) => Some(producer_reference_view(reference))", "Some(_reference) => None", "projection"),
    ("alias_witness", LIVE, "fn producer_acquire_alias_witness_v1(", "production", "producer_acquire_alias_witness_v1", "actual.free.set(2, 0);", "actual.free.set(2, 1);", "witness"),
    ("raw_issuer_witness", LIVE, "fn producer_acquire_zero_producer_witness_v1(", "production", "producer_acquire_zero_producer_witness_v1", "local: 0, kind: WriterKindV1::Submission", "local: 1, kind: WriterKindV1::Submission", "witness"),
    ("budget_priority_witness", LIVE, "fn producer_acquire_raw_budget_witness_v1(", "production", "producer_acquire_raw_budget_witness_v1", "actual.next_incarnation = 0;", "actual.next_incarnation = 1;", "witness"),
]
...
def check_result(base, status, stdout, stderr, folder, mutation, expected):
    ...
    if mutation:
        ...
        need(same(observed['result'], {
            ...
            'verified': 625, 'errors': 0, 'is-verifying-entire-crate': True,
