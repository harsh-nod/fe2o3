...
class HarnessTests(unittest.TestCase):
    def test_both_real_transcripts(self):
        for target in ("gnu", "musl"):
            self.assertEqual(len(V.harness((V.ARCHIVE / f"raw/{target}-example.log").read_text())), 15)
    ...
    def test_rejects_twelve_malformed_harnesses(self):
        ...
        mutations = [
            ...
            text.replace("running 15 tests", "running 14 tests"),
            text.replace("15 passed", "14 passed"),
    ...
    def test_final_source_and_commands(self):
        self.assertEqual(V.qualify()["example_tests_per_target"], 15)
