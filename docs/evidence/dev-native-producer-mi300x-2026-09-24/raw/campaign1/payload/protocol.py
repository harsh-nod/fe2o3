...
COMMIT = "f3f8206fe4727f1f535ad820721fff7e7bdd4a39"
GPU, UID, BDF = 1, "0xab83d2ffef0d3cdf", "0000:26:00.0"
PREFIX = "/home/harsh/fe2o3-native-producer-20260924."
...
CASE_NAMES = [
    ...
    *[("generated-" + start + "-" + action, GENERATED + "generated_native_" + start + "_" + suffix)
    ...
    ("queued-producer", BASE + "producer_launch::native_runtime_producer_launch_queued_chain"),
    ("published-producer", BASE + "producer_launch::native_runtime_producer_launch_published_chain"),
]
...
def environment(root, case=None):
    ...
    if case is not None and (case.startswith(("generated-", "cold-")) or case.endswith("-producer")):
...
def marker(case):
    if case.endswith("-producer"):
        return ("native_producer_launch=complete published=" + str(case == "published-producer").lower()
                + " authority_calls=2 launches=2 checked_bytes=1048576 peak_readers=4 terminal_readers=0 terminal_writers=0"
                + " public_event=released observation=consumer_first data_path=PersistentDeviceReused materializations=0"
                + " a_sha256=1202ea159669597ce883b2af7745fc94afd5d9de7e1985cacb341cec3ae914a7"
                + " b_sha256=2c1e99dcde81f5dd034e1cda167af61dd9c7109c72c6a0ede80801f2e1d665d8"
                + " c_sha256=564b131bbd3f6eae41a23c72fea11ce4efa9ff0539b16eca66f4aa365d1ad8a7"
                + " d_sha256=275fe5e215ed3701e9f0d694119cf46d89846916ea29269dec0533d56e410925 cleanup=complete")
    if case in TYPED:
...
def transcript(case, stdout, stderr):
    ...
    need(len(summaries) == count and all(re.fullmatch(
        r"test result: ok\. 1 passed; 0 failed; 0 ignored; 0 measured; 1421 filtered out; finished in [0-9]+\.[0-9]+s", line)
