...
CPU = "docs/evidence/dev-native-producer-witness-cpu-2026-09-24"
...
def roster(text):
    ...
    if len(rows) != len(result) or len(result) != 1422 or list(result.values()).count("ok") != 1400:
    ...
    if len(summaries) != 1 or not re.fullmatch(
        r"test result: ok\. 1400 passed; 0 failed; 22 ignored; 0 measured; 0 filtered out; finished in [0-9.]+s",
        summaries[0],
    ):
...
def main():
    ...
    if (output.resolve() != output or output.is_symlink() or output.name != "build"
            or not re.fullmatch(r"/home/harsh/\.codex-tmp/fe2o3-native-producer-20260924-[A-Za-z0-9]{8}", str(output.parent))):
    ...
    prior = rec.run("cpu-source", [*c.K.GIT, "show", COMMIT + ":" + CPU + "/raw/cpu4/inputs-before.json"],
                    30, env=c.K.GIT_ENV)
    ...
    c.H.need(inputs == expected and len(inputs) == 3961, "exact CPU-qualified source inputs")
    prior = rec.run("cpu-roster", [*c.K.GIT, "show", COMMIT + ":" + CPU + "/raw/cpu4/commands/musl-runtime/stdout"],
                    30, env=c.K.GIT_ENV)
