...
def capture(path):
    if not stat.S_ISREG(path.lstat().st_mode):
        raise RuntimeError("ordinary protocol source")
    return path.read_bytes()
...
def validate_build(build, m, p, b):
    ...
    p.need(value["commit"] == p.COMMIT and value["cpu_source_inputs"] == 3961, "fixed qualified build")
...
def main():
    ...
    captured = {name: capture(HERE / name) for name in PROTOCOL}
    initial = {name: hashlib.sha256(raw).hexdigest() for name, raw in captured.items()}
    p, m = [module(HERE / (name + ".py"), "producer_" + name, captured[name + ".py"])
            for name in ("protocol", "prepare")]
    b = p.load_module(REPO / m.HELPER, p.BASE_SHA, "native_matrix_local_recorder")
    n = module(HERE / "native.py", "native_matrix_runner", captured["native.py"])
    for number in b.MANAGED:
        signal.signal(number, b.interrupted)
    build, output = args.build, args.output
    p.need(build.resolve() == build and output.resolve() == output and output.parent == build.parent and output.name == "campaign1",
    ...
    for name in PROTOCOL:
        with (output / "protocol" / name).open("xb") as retained:
            retained.write(captured[name])
    ...
    for name, path in sources.items():
        if name in ("native.py", "protocol.py"):
            with (payload / name).open("xb") as retained:
                retained.write(captured[name])
        else:
