 producer_writer_same_body {
    ($left:ident, $right:ident) => {
        $left.slot == $right.slot && producer_query_consumer_same_exec_v1($left.key, $right.key)
    };
}

 producer_reference_same_body {
    ($left:ident, $right:ident) => {
        $left.slot == $right.slot && $left.incarnation == $right.incarnation
            && producer_query_consumer_same_exec_v1($left.consumer, $right.consumer)
    };
}

 producer_status_body {
    ($journal:expr, $request:ident, $same:path) => {{
        ...
        match journal.lookup_writer($request.producer) { Ok(_) => {}, Err(error) => return Err(error) };
