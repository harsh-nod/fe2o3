...
def owned_absence(pids):
    ...
    for pid in pids:
        ...
        observations.append({"pid": pid, "pid_absent": path_absent, "process_group_absent": group_absent})
    require(all(row["pid_absent"] and row["process_group_absent"] for row in observations), "a recorded owned PID/group remains")
    return observations
...
def scan(owned_pids):
        ...
        try:
            ...
            for entry in entries:
                ...
                if target == str(OWNED) or target.startswith(str(OWNED) + "/"):
                    references.append({"pid": pid, "field": entry.name, "target": target})
    ...
    for pid in sorted({row["pid"] for row in unreadable}):
        ...
        try:
            ...
            require(group not in owned_pids, "unreadable member of recorded owned process group")
...
def report(kind, pids, **extra):
    ...
    row = {
        "record": kind, "utc_ns": time.time_ns(), "owned": str(OWNED),
        "owned_processes": owners, "accessible_references": references,
...
def main():
    require(len(sys.argv) == 3 and sys.argv[1] in ("--cleanup", "--absence"), "exact mode and PID roster required")
    pids = [int(value) for value in sys.argv[2].split(",")]
    require(len(pids) == 29 and len(set(pids)) == 29 and all(pid > 0 for pid in pids), "29 unique recorded owned PIDs required")
    os.chdir("/tmp")
    if sys.argv[1] == "--absence":
        require(not os.path.lexists(OWNED), "owned path remains")
        report("independent-absence", pids, absent=True)
        return
    require(OWNED.is_dir() and not OWNED.is_symlink() and OWNED.resolve() == OWNED, "exact nonsymlink owned path required")
    require(OWNED.stat().st_uid == os.getuid() and (OWNED / "owner").read_text() == MARKER, "ownership marker mismatch")
    rows = [json.loads(path.read_text()) for path in sorted((OWNED / "results/campaign").glob("*.json"))]
    require([row["pid"] for row in rows] == pids and all(row["group_absent"] for row in rows), "PID roster must match original recorded campaign")
    report("before-cleanup-visible", pids)
    size = sum(path.stat().st_size for path in OWNED.rglob("*") if path.is_file() and not path.is_symlink())
    shutil.rmtree(OWNED)
