#!/usr/bin/env python3
"""Bounded CPU qualification of the native directed-owner witness and harness."""
import hashlib
...
def main():
    ...
    commands = [
        ...
        ('gnu', cargo + ['test', '-p', 'fe2o3-runtime', '--example', example], 1200),
        ('musl', cargo + ['test', '-p', 'fe2o3-runtime', '--example', example,
    ...
    env['FE2O3_DIRECTED_OWNER_RUST_BINARY'] = str(ROOT / 'target/x86_64-unknown-linux-musl/debug/examples' / example)
    if len(sys.argv) > 2 and sys.argv[2] == 'focused':
        commands = [('focused', cargo + ['test', '-p', 'fe2o3-runtime', '--example', example], 1200)]
