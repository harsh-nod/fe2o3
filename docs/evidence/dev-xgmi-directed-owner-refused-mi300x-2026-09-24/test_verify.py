...
class EvidenceTests(unittest.TestCase):
    ...
    def test_original(self):
        result = V.verify()
        self.assertTrue(result["native_process_executed"])
        self.assertFalse(result["native_graph_qualified"])
        self.assertEqual(result["planned_directed_copies"], 4)
        ...
        self.assertEqual(result["failure"], "unordered shared-source admission")
        self.assertEqual(result["owner_disposition"], "retained-until-process-exit")
    ...
    def test_semantic_mutations_after_rehash(self):
        ...
        cases += [
            ...
            ("invented-success-receipt", lambda f: (f / "remote/directed-owner/stdout").write_bytes(b"PASS\n")),
            ("claimed-dataflow", lambda f: edit(f, "remote/validated-results.json", lambda r: r.append({"trial": "directed-owner", "result": {"pending_dataflow": "supported"}}))),
            ("failure-erased", lambda f: edit(f, "remote/finished.json", lambda r: r.update(failures=[], native_execution=True))),
            ("workload-exit-promoted", lambda f: edit(f, "remote/directed-owner/receipt.json", lambda r: r.update(exit=0))),
            ("failure-text-changed", lambda f: (f / "remote/directed-owner/stderr").write_bytes(b"different refusal\n")),
            ("cleanup-retention-erased", lambda f: (f / "remote/directed-owner/stderr").write_bytes((f / "remote/directed-owner/stderr").read_bytes().replace(b"RetainedUntilProcessExit", b"Released"))),
            ...
            ("CPU-count", lambda f: (f / "qualification/gnu/stdout.log").write_bytes((f / "qualification/gnu/stdout.log").read_bytes().replace(b"6 passed", b"0 passed"))),
    ...
    def test_cpu_content_checks_independent_of_signed_copy_gate(self):
        ...
        cases = [
            ...
            ("count", lambda f: (f / "gnu/stdout.log").write_bytes((f / "gnu/stdout.log").read_bytes().replace(b"6 passed", b"0 passed"))),
            ("name", lambda f: (f / "gnu/stdout.log").write_bytes((f / "gnu/stdout.log").read_bytes().replace(b"tests::dropped_gate_sender", b"tests::different_gate_sender"))),
