...
def main():
    ...
    spec = importlib.util.spec_from_file_location('unknown_transition_selftest_checker', path)
    ...
    for part in ('body', 'invocation', 'definition'):
        ...
        if part == 'invocation':
            ...
            text = 'let result = shared_retained_chain_v1'
            offset = body.index(text, body.index('macro_rules! retained_unknown_body'))
    ...
    branch_rejections = 0
    for name in ('rejection_write', 'unknown_revalidation', 'rejection_identity'):
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-unknown-transition-source-selftest-') as temporary:
        for path in snapshots:
            ...
            try:
                ...
                checker.need(str(error).startswith('pinned Unknown source:'), 'intended source rejection')
            else:
                raise ValueError('accepted changed Unknown input')
    ...
    if args.packet:
        ...
        failures = ['exact staged nested inputs', 'exact Unknown postcondition', 'owned solver completion',
                    'exact verification result', 'proof roster', 'proof input identities', 'owned command completion']
        with tempfile.TemporaryDirectory(prefix='fe2o3-unknown-transition-packet-selftest-') as temporary:
