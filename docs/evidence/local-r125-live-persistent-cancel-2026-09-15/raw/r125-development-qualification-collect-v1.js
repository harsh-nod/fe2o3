...
const q = JSON.parse(initialPin('r125-development-qualification-plan-v1.json',
...
assert.strictEqual(q.source_changed, 16); assert.strictEqual(q.source_added, 3);
...
assert.deepStrictEqual(JSON.parse(sourcePlan.log), {output: root + 'r125-development-mutations-v2.json',
  sha256: hash(bytes('r125-development-mutations-v2.json')), accepted: false,
...
assert.deepStrictEqual(JSON.parse(snapshot.log), {source_snapshot_only: true,
  output: root + q.source_bundle, files: 16, new_files: 3,
...
assert.deepStrictEqual(q.calibrations.map(spec => [spec.accept, spec.reject]), [[5, 50], [10, 39], [38, 63]]);
...
assert.strictEqual(q.helper_calibrations, 205);
...
console.log(JSON.stringify({development_only: true, publication_pending: true, scope: q.scope,
  ...
  source_bundle: {name: q.source_bundle, sha256: hash(bytes(q.source_bundle)), changed: 16, added: 3},
