...
const helper = root + 'r125-development-check-v3.js';
const helperHash = '308fa81a3d7a475491f48490f8ab77761bcb0c45fb30ffbb77796f61c0e8aaaa';
...
for (const [label, test] of [
  ...
  ['new zero-scan test', C.p.new_tests.kfd.find(name => name.endsWith('::context_zero_check_preserves_every_byte_including_unaligned_partial_pages'))],
...
good('actual GNU 90-minute record', () => validate(baseline.record));
for (const [name, edit] of [
  ['old full deadline', value => value.deadline_ms = 3600000],
...
bad('ordinary record cannot borrow full deadline', () =>
  fixtureValidator(ordinarySpec, {...ordinary, deadline_ms: 5400000}));
...
good('synthetic exact musl ninety-minute declaration', () => fixtureValidator(muslSpec, musl));
for (const deadline of [3600000, 7200000])
...
assert.strictEqual(cases.length, 57);
