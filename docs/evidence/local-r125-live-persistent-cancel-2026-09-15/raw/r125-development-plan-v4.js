...
const old = JSON.parse(oldBytes), runner = 'r125-development-run-v4.js';
assert.strictEqual(hash(fs.readFileSync(path.join(root, runner))), '52a8f0b4413a875757e943a9276b9cb832312ac0fd07a457305660065fdcb1e9');
...
assert.strictEqual(hash(JSON.stringify(map)), '29b127dd3e2f1eaa9cbc3e8bd7bfd7fb26bb8879e58bdf0ab801c291f188dd80');
const declarations = [
  ...
  ['crates/fe2o3-kfd/src/queue_live/construction_primary/integration_platform_tests.rs',
    'queue::live::construction_primary::integration_tests::platform::tests::', [
      'context_zero_check_preserves_every_byte_including_unaligned_partial_pages',
    ]],
];
...
assert.strictEqual(additions.length, 18);
assert.strictEqual(new Set(additions).size, 18);
...
const fullRuns = old.full_runs.map(spec => ({
  name: 'r125-development-' + spec.kind + '-all-v4', kind: spec.kind,
  predecessor: spec.kind === 'gnu' ? 'r125-development-zero-scan-fixture-musl-01.json' : 'r125-development-gnu-check-v3.json',
  command: spec.command, runner, deadline_ms: 5400000,
  accepted_log: archive + spec.name + '.log',
  accepted_log_sha256: hash(committed(spec.name + '.log')),
}));
let previous = 'r125-development-full-check-v3.json';
...
// The boxed-ledger cohort's full GNU pass is historical; its musl timeout never qualified.
const priorNames = ['r125-development-gnu-all-v3', 'r125-development-musl-all-v3'];
for (const name of priorNames) for (const suffix of ['.json', '.log', '-source.json', '-source-after.json'])
  attempted.push(name + suffix);
attempted.push('r125-development-run-v3.js', 'r125-development-full-plan-v3.json',
  'r125-development-check-v2.js');
const previousChecker = require('./r125-development-check-v2.js');
previousChecker.baseline('gnu');
previousChecker.stable();
const timedOut = JSON.parse(fs.readFileSync(path.join(root, 'r125-development-musl-all-v3.json')));
assert.strictEqual(hash(fs.readFileSync(path.join(root, 'r125-development-musl-all-v3.json'))),
  '8aba08320b166f8a86f905ff6da53211ea6f9bf1e99855d082f119c51c2ac26d');
assert.strictEqual(hash(fs.readFileSync(path.join(root, 'r125-development-musl-all-v3.log'))),
  'f0bd4d9272d251f75f8e81e6eee152bac1cb689c00677be78ee847dd3e56415d');
assert.strictEqual(timedOut.returncode, 124);
assert.strictEqual(timedOut.child_returncode, null);
assert.strictEqual(timedOut.signal, 'SIGKILL');
assert.strictEqual(timedOut.timed_out, true);
assert.strictEqual(timedOut.child_closed, true);
assert.strictEqual(timedOut.deadline_ms, 3600000);
assert(timedOut.elapsed_seconds >= 3600);
previousChecker.observeClosure('r125-development-musl-all-v3', timedOut);
const priorMap = JSON.parse(fs.readFileSync(path.join(root, 'r125-development-musl-all-v3-source.json')));
assert.deepStrictEqual(priorMap,
  JSON.parse(fs.readFileSync(path.join(root, 'r125-development-musl-all-v3-source-after.json'))));
assert.strictEqual(hash(JSON.stringify(priorMap)), previousChecker.p.source_map_sha256);
assert.deepStrictEqual(Object.keys(map), Object.keys(priorMap));
assert.deepStrictEqual(Object.keys(map).filter(name => map[name] !== priorMap[name]), [
...
const plan = {
  accepted: false,
  scope: 'R125 exact-byte fixture cohort CPU/runtime regression only; mutation, native, formal, aggregate-memory and performance acceptance remain separate.',
  source_parent: head, accepted_commit: head,
  source_map: 'r125-development-gnu-all-v4-source.json',
  ...
  superseded_attempts: {
    classification: 'GNU v1 intentionally interrupted; GNU v2 failed with runtime stack overflow, independently reproduced. Boxed-ledger GNU v3 passed but musl v3 timed out. None qualifies the successor exact-byte fixture cohort.',
    inputs: Object.fromEntries(attempted.map(name => [name, hash(fs.readFileSync(path.join(root, name)))])),
  },
  acceptance: 'Require exact source endpoints, commands, environment, runner, deadlines, normal child and current-boot group closure, predecessor hashes, complete executable/test/ignored/summary rosters plus the declared 18 KFD and 1 runtime tests, and all 25 leaves. This plan does not accept R125 or establish native execution, formal correspondence or HIP/HSA parity.',
};
fs.writeFileSync(path.join(root, 'r125-development-full-plan-v4.json'), JSON.stringify(plan, null, 2) + '\n', {flag: 'wx'});
