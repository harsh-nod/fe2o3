// R119 transcript parsers remain pinned; only the R125 record policy is adapted.
...
function checkRecord(r, name, command, expectedMap, previousName, code, io, context) {
  assert(/^r125-development-[a-z0-9-]+$/.test(name), 'R125 record namespace');
  ...
  const full = ['r125-development-gnu-all-v4', 'r125-development-musl-all-v4'].includes(name);
  const campaign = ['r125-development-source-campaign', 'r125-development-auxiliary-campaign'].includes(name);
  assert.strictEqual(r.deadline_ms, full ? 5400000 : campaign ? 7200000 : 1800000, 'declared deadline policy');
