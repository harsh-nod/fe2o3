// R125 exact-byte fixture cohort; preserve prior interrupted, failed and superseded evidence.
...
const parent = 'c171d915047daa5153c1f77253e15260e55da8c5';
const [name, previousName, command, ...args] = process.argv.slice(2);
assert(/^r125-development-[a-z0-9-]+$/.test(name) && previousName && command);
...
(async () => {
  ...
  assert.strictEqual(boot(), '74856d82-79a2-4377-b6c4-aaf01159fd19', 'admitted execution boot');
  let previous = null;
  if (previousName !== '-') {
    assert(/^r125-development-[a-z0-9-]+\.json$/.test(previousName));
  ...
  const declaredFullCommand = name === 'r125-development-gnu-all-v4' ? fullCommand
    : name === 'r125-development-musl-all-v4' ? [...fullCommand, '--target', 'x86_64-unknown-linux-musl']
    : null;
  if (declaredFullCommand) {
    ...
    assert.strictEqual(hash(JSON.stringify(before)),
      '29b127dd3e2f1eaa9cbc3e8bd7bfd7fb26bb8879e58bdf0ab801c291f188dd80', 'declared full-test source');
    assert.strictEqual(Object.keys(before).length, 5709, 'declared full-test source count');
  }
  const deadline = declaredFullCommand ? 5400000
    : ['r125-development-source-campaign', 'r125-development-auxiliary-campaign'].includes(name) ? 7200000 : 1800000;
  ...
  assert.strictEqual(git(['rev-parse', 'HEAD']).trim(), parent, 'source HEAD unchanged');
  ...
  const record = {source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    clock: {contract: 'r125-development-raw-utc-boot-monotonic-v1', predecessor: previous, start, finish: result.finish, source_verified: verified, error: clockError}};
  ...
  process.exitCode = !result.observed || !record.source_unchanged || clockError || cleanup.status === 'error' || timeoutCleanup?.status === 'error' ? 125 : record.returncode;
})().catch(error => {
