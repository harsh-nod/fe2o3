...
const q = JSON.parse(pin('r125-development-qualification-plan-v2.json',
...
const collector = 'r125-development-qualification-collect-v2.js';
assert.strictEqual(q.collector, collector);
pin(collector, '33ee25c4cc24a0014d13da25385c21816b63d5a775b240766e920afef7d13163');
bytes(__filename);
...
assert.strictEqual(report.gnu_passed, 2901); assert.strictEqual(report.musl_passed, 2901);
...
assert.strictEqual(report.helper_calibrations, 207);
...
assert.deepStrictEqual(report.source_bundle, {
  name: q.source_bundle, sha256: hash(bytes(q.source_bundle)), changed: 18, added: 3,
});
assert.deepStrictEqual(report.restored_suites, [47, 744]);
assert.deepStrictEqual(report.exclusions, q.exclusions);
const runNames = [q.diagnostic_audit.name, q.source_plan_run.name, q.source_snapshot.name,
...
assert.strictEqual(runNames.length, 92);
...
for (const name of ['r125-development-qualification-plan-v2.json', collector,
...
const summary = {
  ...
  scope: 'Live persistent cancellation, boxed-ledger repair and exact-byte fixture scans: local CPU/test qualification',
  full: {gnu: {harnesses: 48, passed: 2901, failed: 0, ignored: 5},
    musl: {harnesses: 48, passed: 2901, failed: 0, ignored: 5}},
  harnessless_csv_each: 1, full_runs: C.p.full_runs.map(spec => spec.name),
  source_gates: 17, auxiliary_gates: 10, focused: {kfd: 47, runtime: 744, rosters: [47, 744]},
  compiled_negatives: 18, distinct_mutations: 18, helper_calibration_tests: 207,
...
const readme = [
  ...
  'GNU and musl each pass 2,901 tests with five ignored across 48 libtest harnesses and one separately checked CSV target.',
  'All seventeen source gates, ten auxiliary checks, restored 47-test KFD and 744-test runtime suites, and 207 fresh checker-calibration cases pass.',
  ...
  'The mutation inventory does not claim mutation coverage of the added boxed-ledger layout, owner-move tests or zero-check optimization.',
  'The immutable source bundle retains the exact eighteen changed/new source files, including three added files; unchanged sources belong to the accepted R124 parent.',
  ...
  'Earlier interrupted GNU, stack-overflow and musl-timeout attempts, intermediate source cohorts, and development checks remain separate nonqualifying history.',
  'The six-record paired CPU fixture diagnostic is independently checked and replayed; its test timings do not qualify GPU execution or matched HIP/HSA performance.',
