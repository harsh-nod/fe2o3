...
stage="$1"
commit="$2"
[[ "$stage" =~ ^/dev/shm/fe2o3-r65-owner\.[A-Za-z0-9]+$ ]]
[[ "$commit" =~ ^[0-9a-f]{40}$ ]]
[[ -d "$stage" && ! -L "$stage" ]]
...
# Resolve offline dev dependencies only in this task's private Cargo cache.
...
python3 "$stage/checkout/benchmarks/runtime_gfx942/run-r65-drain-versions-mi300x.py" \
