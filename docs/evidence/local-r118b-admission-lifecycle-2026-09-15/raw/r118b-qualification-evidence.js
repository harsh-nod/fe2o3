...
function checkRecord(r, name, command, expectedMap, previousName, code, io = {read, bytes}, context = {
  head: p.parent, cwd: repo, contract, runner: 'r118b-run-v1.js', after: expectedMap,
}) {
  ...
  assert.strictEqual(r.deadline_ms, ['r118b-source-campaign', 'r118b-auxiliary-campaign'].includes(name) ? 7200000 : 1800000);
...
const core = require('./r118b-mutation-core-v1.js');
...
function expectedEntries(map, originals) {
  const entries = {};
  let predecessor = 'r118b-qualification-manifest.json';
  const hashes = [];
  ...
  for (const mutation of p.mutations) {
    ...
    const run = 'r118b-qualified-mut-' + mutation.name + '.json';
    ...
    predecessor = run;
    const restored = 'r118b-qualified-restoration-' + mutation.name + '.json';
  ...
  for (const focused of p.focused) {
    const run = 'r118b-qualified-restored-' + focused[0] + '.json';
    entries[run] = {kind: 'run', predecessor, source_map_sha256: hash(JSON.stringify(map)), command: focusedCommand(focused), returncode: 0};
    predecessor = run;
  }
  entries['r118b-qualified-collector-validation.json'] = {kind: 'run', predecessor,
    source_map_sha256: hash(JSON.stringify(map)), command: ['node', file('r118b-qualification-collect.js'), '--check'], returncode: 0};
...
function historicalNames() {
  const runs = p.mutations.map(mutation => 'r118b-qualified-mut-' + mutation.name)
  ...
  const excluded = new Set(runs.flatMap(name => ['.json', '.log', '-source.json', '-source-after.json'].map(suffix => name + suffix))
    .concat(p.mutations.flatMap(mutation => ['.json', '-source.json'].map(suffix => 'r118b-qualified-restoration-' + mutation.name + suffix)),
      ['r118b-qualification-manifest.json']));
  const local = fs.readdirSync(root).filter(name => /^r118b-.*\.(json|log|js|py|md|rs)$/.test(name) && !excluded.has(name));
  const origin = read('r118b-origin-v2.json');
  return [...new Set([...local, ...origin.historical_artifacts.map(item => item.name),
    ...origin.explicit_inputs.map(item => item.name)])].sort();
}
