...
const runner = path.join(root, 'r118b-run-v1.js');
const source = fs.readFileSync(runner, 'utf8');
const parent = 'a07ec44309e214f2a8ef0e687e610c8e60a36224';
const inputs = JSON.parse(fs.readFileSync(path.join(root, 'r118b-runner-inputs-v1.json'), 'utf8'));
...
assert.deepStrictEqual(inputs.helpers.map(item => item.name), ['r118b-runner-tests.js', 'r118b-run-v1.js']);
...
async function mocked(name, mode) {
  ...
  const fakeProcess = {
    argv: ['node', runner, 'r118b-mocked-' + name, '-', 'fixture'], env: {},
  ...
  const record = JSON.parse(writes.get(path.join(root, 'r118b-mocked-' + name + '.json')));
...
(async () => {
  ...
  try {
    ...
    const cases = [
      ...
      ['spawn', '/no-such-r118b-qualification-command', [], 127],
    ];
    for (const [name, command, args, expected] of cases) {
      const label = 'r118b-runner-v1-fixture-' + name;
      let result;
      try {
