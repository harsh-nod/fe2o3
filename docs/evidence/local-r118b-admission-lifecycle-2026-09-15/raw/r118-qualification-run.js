...
function runEntry(manifest, name, expectedMap, beforeSpawn = () => {}) {
  ...
  beforeSpawn();
  const result = cp.spawnSync(process.execPath, [e.file('r118-run-v1.js'), name, entry.predecessor, ...entry.command],
...
function bindTerminal(record, name, expected, entry, readArtifact, runnerDigest) {
  ...
  assert.deepStrictEqual(record.runner, {name: 'r118-run-v1.js', sha256: runnerDigest}, 'manifest-pinned terminal runner');
  assert.strictEqual(e.hash(readArtifact('r118-run-v1.js')), runnerDigest, 'manifest-pinned live runner');
...
function qualifyMutation({manifest, manifestHash, map, mutation, io = sourceIO,
  ...
  const entry = manifest.entries[name + '.json'];
  assert(entry && entry.kind === 'run');
  const runnerPins = manifest.helpers.filter(helper => helper.name === 'r118-run-v1.js');
  ...
  if (errors.length === 0 && oraclePassed && completed && state.record) {
    try {
      ...
      const restoredName = 'r118-qualified-restoration-' + mutation.name;
  ...
  if (errors.length || !record) {
    ...
    try {write('r118-qualification-failure-' + mutation.name + '.json', diagnostic);}
...
function main() {
  ...
  const all = e.passing(e.bytes('r118-reviewed-gnu-all.log').toString());
  for (const focused of p.focused) {
    const [kind, filter, count, exact] = focused;
    const result = runEntry(manifest, 'r118-qualified-restored-' + kind, map);
  ...
  runEntry(manifest, 'r118-qualified-collector-validation', map);
