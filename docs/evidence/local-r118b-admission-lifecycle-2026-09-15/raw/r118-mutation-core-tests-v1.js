...
const C = require('./r118-mutation-core-v1.js');
const P = require('./r118-mutation-plan-v1.js');
...
test('executed helper bytes match the separately frozen input manifest', () => {
  const inputs = JSON.parse(fs.readFileSync(root + 'r118-mutation-core-inputs-v1.json', 'utf8'));
  ...
  assert.deepStrictEqual(inputs.helpers.map(helper => helper.name), [
    ...
    'r118-mutations-c1-v1.js', 'r118-mutations-c2-v1.js', 'r118-mutations-c3-v1.js', 'r118-run-v1.js',
  ]);
...
test('restoration I/O failure reports exact mixed state and retains recovery information', () => {
  ...
  f.state.manifest = {name: 'r118-qualification-manifest.json', sha256: C.hash('manifest')};
...
for (const [name, change] of [
  ...
  const f = fixture(); const manifest = {name: 'r118-qualification-manifest.json', sha256: C.hash('manifest')};
...
test('real prospective plan derives 78 executions and 74 exact maps without writes', () => {
  const map = JSON.parse(fs.readFileSync(root + 'r118-reviewed-clippy-source.json', 'utf8'));
