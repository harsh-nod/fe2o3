...
assert(/^r118b-[a-z0-9-]+$/.test(name) && previousName && command);
...
(async () => {
  ...
  let previous = null;
  if (previousName !== '-') {
    assert(/^r118b-[a-z0-9-]+\.json$/.test(previousName));
    ...
    assert.strictEqual(record.clock.contract, 'r118b-raw-utc-boot-monotonic-v1');
  ...
  const deadline = ['r118b-source-campaign', 'r118b-auxiliary-campaign'].includes(name) ? 7200000 : 1800000;
  let timedOut = false;
  ...
  const record = {source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    clock: {contract: 'r118b-raw-utc-boot-monotonic-v1', predecessor: previous, start, finish: result.finish, source_verified: verified, error: clockError}};
