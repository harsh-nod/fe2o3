...
const {qualifyMutation} = require('./r118b-qualification-run.js');
const map = e.identities();
assert.strictEqual(e.hash(JSON.stringify(map)), p.sourceMapSha256);
const inputs = e.read('r118b-qualification-lifecycle-inputs-v1.json');
assert.strictEqual(inputs.parent, p.parent);
assert.strictEqual(inputs.source_map_sha256, p.sourceMapSha256);
assert.deepStrictEqual(inputs.helpers.map(pin => pin.name), [...p.helpers, 'r118b-qualification-lifecycle-tests-v1.js']);
...
const runner = e.bytes('r118b-run-v1.js');
...
function fixture(options = {}) {
  const mutation = options.single ? p.mutations[0] : p.mutations.find(mutation => mutation.patches.length === 2);
  assert(mutation);
  ...
  const manifest = {original_sources: originals, helpers: [{name: 'r118b-run-v1.js', sha256: e.hash(runner)}],
    entries: {[name + '.json']: {kind: 'run', command, predecessor: 'r118b-qualification-manifest.json',
      source_map_sha256: e.hash(JSON.stringify(changed)), returncode: 101}},
    clock: {source_verified: observation(1)}};
  const manifestBytes = Buffer.from(JSON.stringify(manifest));
  const bytes = new Map([['r118b-run-v1.js', runner], ['r118b-qualification-manifest.json', manifestBytes]]);
  ...
  const run = (actualName, expected, beforeSpawn) => {
    ...
    rawRecord = {source_head: p.parent, cwd: e.repo, command,
      ...
      runner: {name: 'r118b-run-v1.js', sha256: e.hash(runner)}, child_closed: true, spawn_error: null,
      ...
      clock: {contract: e.contract, source_verified: observation(20), predecessor: {
        name: 'r118b-qualification-manifest.json', sha256: e.hash(manifestBytes), observation: observation(1)}}};
...
function rejected(result, restored, attempted = true) {
  ...
  assert(![...result.emitted.keys()].some(name => name.startsWith('r118b-qualified-restoration-')));
}
test('single-file mutation is applied, checked and restored', () => {
  const result = fixture({single: true});
...
test('combined defense preserves declared two-file application and restoration order', () => {
  ...
  assert.deepStrictEqual(result.writes.map(write => write.kind), ['apply', 'apply', 'restore', 'restore']);
...
test('second application failure restores an owned first-file prefix', () => {
  const result = fixture({beforeWrite: ({kind, writes}) => {if (kind === 'apply' && writes.length === 2) throw new Error('second write failure');}});
  rejected(result, true, false); assert.strictEqual(result.diagnostic.application_attempts.length, 2);
});
test('partial second-file bytes are retained for explicit recovery', () => {
  const result = fixture({beforeWrite: ({kind, writes, values, name}) => {
    if (kind === 'apply' && writes.length === 2) {values.set(name, 'partial bytes'); throw new Error('partial write');}
  }});
...
test('changed live runner bytes cannot authenticate a pinned terminal', () => {
  rejected(fixture({afterRecord: ({bytes}) => {bytes.set('r118b-run-v1.js', Buffer.from('substituted'));}}), false);
});
test('matching substituted runner bytes and record still fail the manifest pin', () => {
  rejected(fixture({record: (record, {bytes}) => {
    const other = Buffer.from('substituted'); bytes.set('r118b-run-v1.js', other); record.runner.sha256 = e.hash(other);
  }}), false);
});
...
test('post-write restoration error prevents acceptance despite complete physical recovery', () => {
  const result = fixture({afterWrite: ({kind, writes}) => {if (kind === 'restore' && writes.length === 4) throw new Error('late restoration I/O');}});
...
test('external source changes during child execution are not overwritten', () => {
  const result = fixture({afterRecord: ({values, files}) => {values.set(files[1].path, 'external bytes');}});
