...
const l = require('./r118-qualification-launch-v1.js');
const prep = require('./r118-qualification-prepare.js');
const cases = [];
const tests = [];
...
function data() {
  ...
  const map = e.read('r118-frozen-source.json');
  ...
  cached = {map, originals, entries: e.expectedEntries(map, originals),
    names: e.passing(e.bytes('r118-reviewed-gnu-all.log').toString()),
    helperBytes: new Map([...p.helpers, 'r118-qualification-launch-v1.js'].map(name => [name, e.bytes(name)]))};
  return cached;
}
...
function fixture() {
  ...
  const previous = 'r118-qualification-contract-tests.json';
  ...
  const launcher = {name: 'r118-qualification-launch-v1.js', sha256: e.hash(values.get('r118-qualification-launch-v1.js'))};
  const base = {map: {...d.map}, helpers, launcher, previous, allNames: d.names, cases,
    env: {}, history: e.read('r118-environment.json').history};
  ...
  const publish = () => values.set('r118-qualification-manifest.json', Buffer.from(JSON.stringify(manifest)));
...
function load(name, overrides) {
  ...
  const localRequire = name => Object.hasOwn(overrides, name) ? overrides[name] : require(name.startsWith('./r118-') ? e.file(name.slice(2)) : name);
...
function launchFixture(options = {}) {
  ...
  io.run = (manifest, name, expected, beforeSpawn) => {
    ...
    const focused = p.focused.find(([kind]) => name === 'r118-qualified-restored-' + kind);
  ...
  io.qualify = ({manifest, map, mutation, run}) => {
    ...
    try {
      ...
      run('r118-qualified-mut-' + mutation.name, state.current, () => {state.callbacks++;});
      record = {mutation: mutation.name};
      values.set('r118-qualified-restoration-' + mutation.name + '.json', Buffer.from(JSON.stringify(record)));
      values.set('r118-qualified-restoration-' + mutation.name + '-source.json', Buffer.from(JSON.stringify(map)));
...
check('launch-exact-78-plus-six-plus-collector-sequence', () => {
  ...
  assert.deepStrictEqual(f.state.spawns, p.mutations.map(m => 'r118-qualified-mut-' + m.name)
...
for (const suffix of ['.json', '.log', '-source.json', '-source-after.json', '-gate-failure.json']) {
  check('occupied-run-output-' + suffix, () => {
    const f = launchFixture({initial: f => f.state.occupied.add('r118-qualified-mut-' + p.mutations[0].name + suffix)});
...
for (const suffix of ['.json', '-source.json']) check('occupied-restoration-' + suffix, () => {
  const f = launchFixture({initial: f => f.state.occupied.add('r118-qualified-restoration-' + p.mutations[0].name + suffix)});
  assert(f.error); assert.strictEqual(f.state.mutations, 0);
});
check('occupied-failure-output', () => {
  const f = launchFixture({initial: f => f.state.occupied.add('r118-qualification-failure-' + p.mutations[0].name + '.json')});
  assert(f.error); assert.strictEqual(f.state.mutations, 0);
});
...
for (const kind of ['identity', 'reservation']) check('helper-change-after-focused-' + kind, () => {
  const f = launchFixture({afterRun: (f, name) => {if (name === 'r118-qualified-restored-' + kind) f.values.set(p.helpers[0], Buffer.from('changed'));}});
  assert(f.error); assert(!f.state.spawns.includes('r118-qualified-collector-validation'));
});
for (const [name, artifact] of [
  ['log', 'r118-qualified-mut-' + p.mutations[0].name + '.log'],
  ['source', 'r118-qualified-mut-' + p.mutations[0].name + '-source.json'],
  ['restoration-predecessor', 'r118-qualified-restoration-' + p.mutations[0].name + '.json'],
]) check('completed-output-change-' + name, () => {
  const f = launchFixture({beforeCallback: (f, run) => {
    if (run === 'r118-qualified-mut-' + p.mutations[1].name) f.values.set(artifact, Buffer.from('changed'));
  }});
...
for (const suffix of ['.json', '-source.json']) check('retain-rejects-restoration-substitution-' + suffix, () => {
  const f = launchFixture({afterMutation: (f, mutation) => f.values.set('r118-qualified-restoration-' + mutation.name + suffix,
...
for (const occupied of [false, true]) check('pre-effect-guard-' + (occupied ? 'occupied-output' : 'changed-completed'), () => {
  const f = launchFixture({logged: f => {
    if (occupied) f.state.occupied.add('r118-qualified-restoration-' + p.mutations[1].name + '.json');
    else f.values.set('r118-qualified-mut-' + p.mutations[0].name + '.log', Buffer.from('changed'));
  }});
...
function prepareFixture(options = {}) {
  const f = fixture(); f.values.delete('r118-qualification-manifest.json');
...
for (const [name, modify] of [
  ...
  ['launcher', f => f.values.set('r118-qualification-launch-v1.js', Buffer.from('changed'))],
  ...
  ['output', f => f.state.occupied.add('r118-qualified-mut-' + p.mutations[0].name + '.log')],
...
function runEntryFixture(options = {}) {
  ...
  const name = 'r118-qualified-mut-fixture';
  const entry = {kind: 'run', command, predecessor: 'r118-qualification-manifest.json', source_map_sha256: e.hash(JSON.stringify(data().map)), returncode: 101};
  const manifest = {entries: {[name + '.json']: entry}};
  const runner = load('r118-qualification-run.js', {
    ...
    child_process: {spawnSync: (executable, args, settings) => {
      ...
      assert.deepStrictEqual(args, [e.file('r118-run-v1.js'), name, entry.predecessor, ...command]);
    ...
    './r118-qualification-evidence.js': {...e, checkRun: () => {events.push('record'); if (options.recordFailure) throw new Error('bad terminal'); return 'checked';}},
  });
  let error = null;
  let result;
...
function collectorFixture() {
  ...
  put('r118-environment.json', base.env); put('r118-frozen-source.json', base.map);
  f.names.push('r118-environment.json', 'r118-frozen-source.json'); f.names.sort();
  manifest.historical_artifacts = f.names.map(name => ({name, sha256: e.hash(values.get(name))}));
  f.publish();
  const manifestHash = e.hash(io.bytes('r118-qualification-manifest.json'));
  let tick = 2;
  const next = () => obs(++tick);
  const run = (name, map, log) => {
    ...
    const record = {source_head: p.parent, cwd: e.repo, command: entry.command,
      ...
      runner: {name: 'r118-run-v1.js', sha256: e.hash(io.bytes('r118-run-v1.js'))},
  ...
  for (const mutation of p.mutations) {
    ...
    put('r118-qualified-restoration-' + mutation.name + '.json', restoration);
    put('r118-qualified-restoration-' + mutation.name + '-source.json', base.map);
  }
  for (const focused of p.focused) run('r118-qualified-restored-' + focused[0], base.map,
  ...
  const collector = load('r118-qualification-collect.js', {'./r118-qualification-evidence.js': io});
  const close = () => {
    const open = collector.collect(false, () => base);
    put('r118-qualified-collector-validation.log', Buffer.from(JSON.stringify(open.summary)));
...
for (const [name, modify] of [
  ['wrong-oracle', f => f.put('r118-qualified-mut-' + p.mutations[0].name + '.log', Buffer.from('compile failure'))],
  ['missing-restoration', f => f.values.delete('r118-qualified-restoration-' + p.mutations[0].name + '.json')],
  ['failed-focused', f => f.put('r118-qualified-restored-identity.log', Buffer.from(positiveLog(f.base.allNames.filter(name => name.startsWith(p.focused[0][1]))) + 'test result: FAILED. 0 passed; 1 failed; 0 ignored;\n'))],
  ['closed-transcript', () => {}],
...
for (const membership of [false, true]) check('collector-rejects-late-' + (membership ? 'membership' : 'cohort-bytes'), () => {
  ...
  assert.throws(() => f.collector.collect(false, () => {
    if (membership) f.names.push('late-history.json');
      ...
      const name = 'r118-qualified-restoration-' + p.mutations[0].name + '.json';
...
function restorationFixture() {
  ...
  const name = 'r118-qualified-mut-' + mutation.name + '.json';
  const restoration = f.io.read('r118-qualified-restoration-' + mutation.name + '.json');
  const files = e.mutationSources(f.base.map, f.manifest.original_sources, mutation);
  const args = [files, f.base.map, e.hash(f.io.bytes('r118-qualification-manifest.json')),
...
check('archive-rejects-missing-captured-artifact-before-writing', () => {
  const f = collectorFixture(); const result = f.close(); result.captured.delete('r118-qualification-manifest.json');
