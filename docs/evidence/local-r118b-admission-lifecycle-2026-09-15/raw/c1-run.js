// Isolated C1 preliminary checks only; not source, native or formal packet acceptance.
...
function identities() {
  ...
  const skipped = new Set(git(['ls-files', '-t', '-z']).split('\0')
    .filter(row => row.startsWith('S ')).map(row => row.slice(2)));
  return Object.fromEntries([...new Set(git(['ls-files', '--cached', '--others', '--exclude-standard', '-z']).split('\0'))]
    .filter(n => n && !n.startsWith('docs/')).sort().map(n => {
      const filename = path.join(repo, n);
      if (fs.existsSync(filename)) return [n, {sha256: hash(fs.readFileSync(filename))}];
      assert(skipped.has(n) && index.has(n), 'missing source must be explicitly unmaterialized: ' + n);
      return [n, {unmaterialized_index_entry: index.get(n)}];
    }));
}
...
(async () => {
  ...
  let previous = null;
  if (previousName !== '-') {
    assert(/^c1-[a-z0-9-]+\.json$/.test(previousName));
    ...
    assert.strictEqual(record.clock.contract, 'c1-raw-utc-boot-monotonic-v1');
  ...
  const deadline = ['c1-source-campaign', 'c1-auxiliary-campaign'].includes(name) ? 7200000 : 1800000;
  let timedOut = false;
  ...
  const record = {scope: 'isolated C1 preliminary checks, not packet acceptance',
    unmaterialized_sources: Object.values(before).filter(v => v.unmaterialized_index_entry).length,
    source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    clock: {contract: 'c1-raw-utc-boot-monotonic-v1', predecessor: previous, start, finish: result.finish, source_verified: verified, error: clockError}};
  ...
  process.exitCode = !record.source_unchanged || !result.observed || clockError || cleanup.status === 'error' || timeoutCleanup?.status === 'error' ? 125 : record.returncode;
})().catch(error => {
