...
const {qualifyMutation} = require('./r118-qualification-run.js');
...
const runner = e.bytes('r118-run-v1.js');
...
function fixture(options = {}) {
  ...
  const manifest = {original_sources: originals, helpers: [{name: 'r118-run-v1.js', sha256: e.hash(runner)}],
    entries: {[name + '.json']: {kind: 'run', command, predecessor: 'r118-qualification-manifest.json',
      source_map_sha256: e.hash(JSON.stringify(changed)), returncode: 101}},
    clock: {source_verified: observation(1)}};
  const manifestBytes = Buffer.from(JSON.stringify(manifest));
  const bytes = new Map([['r118-run-v1.js', runner], ['r118-qualification-manifest.json', manifestBytes]]);
  ...
  const run = (actualName, expected, beforeSpawn) => {
    ...
    rawRecord = {source_head: p.parent, cwd: e.repo, command,
      ...
      runner: {name: 'r118-run-v1.js', sha256: e.hash(runner)}, child_closed: true, spawn_error: null,
      ...
      clock: {contract: e.contract, source_verified: observation(20), predecessor: {
        name: 'r118-qualification-manifest.json', sha256: e.hash(manifestBytes), observation: observation(1)}}};
...
function rejected(result, restored, attempted = true) {
  ...
  assert(![...result.emitted.keys()].some(name => name.startsWith('r118-qualified-restoration-')));
}
...
test('changed live runner bytes cannot authenticate a pinned terminal', () => {
  rejected(fixture({afterRecord: ({bytes}) => {bytes.set('r118-run-v1.js', Buffer.from('substituted'));}}), false);
});
test('matching substituted runner bytes and record still fail the manifest pin', () => {
  rejected(fixture({record: (record, {bytes}) => {
    const other = Buffer.from('substituted'); bytes.set('r118-run-v1.js', other); record.runner.sha256 = e.hash(other);
  }}), false);
});
