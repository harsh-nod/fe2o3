...
const historicalNames = fs.readdirSync(root).filter(name => /^r118-.*\.(json|log|js|py|md)$/.test(name)).sort();
...
for (const name of [initialName, mapName, 'r118b-origin-v1.json']) {
...
const record = {
  ...
  explicit_inputs: ['r118b-prior-completion-tests.rs', initialName, mapName, 'r118b-run-v1.js', 'r118b-origin-v1.js'].map(pin),
  observation,
};
...
fs.writeFileSync(path.join(root, 'r118b-origin-v1.json'), JSON.stringify(record, null, 2) + '\n', {flag: 'wx'});
console.log(JSON.stringify({historical_artifacts: historical.length, source_count: record.source_count,
  initial_source_map_sha256: record.initial_source_map_sha256, origin_sha256: e.hash(read('r118b-origin-v1.json'))}));
