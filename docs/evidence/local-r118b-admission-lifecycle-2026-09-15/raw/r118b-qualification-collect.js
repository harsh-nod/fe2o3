...
const baseline = () => e.read('r118b-frozen-source.json');
...
function checkSupportContracts(map, env) {
  for (const [name, expected] of [
    ['r118b-freeze-inputs-v1.json', p.freezeHelpers],
    ['r118b-qualification-lifecycle-inputs-v1.json', [...p.helpers, 'r118b-qualification-lifecycle-tests-v1.js']],
  ]) {
    ...
    assert.deepStrictEqual(inputs.helpers.map(pin => pin.name), expected);
    for (const pin of inputs.helpers) assert.strictEqual(e.hash(e.bytes(pin.name)), pin.sha256, pin.name);
  }
  const frozen = e.checkRun('r118b-freeze-contracts-v1',
    ['node', e.file('r118b-freeze-tests.js')], map, 'r118b-runner-contracts-v1.json');
  const names = require('./r118b-freeze-tests.js').cases;
  assert.strictEqual(names.length, p.freezeContractCount);
  contractTranscript(frozen.log, 'freeze', names);
  assert(frozen.log.endsWith('PASS: ' + p.freezeContractCount + ' freeze contract tests\n'));
  const lifecycle = e.checkRun('r118b-qualification-lifecycle-contracts-v1',
    ['node', '--test', e.file('r118b-qualification-lifecycle-tests-v1.js')], map, 'r118b-freeze-contracts-v1.json');
  const lifecycleNames = require('./r118-history-v3.js').tapNames(lifecycle.log, 22);
  ...
  return 'r118b-qualification-lifecycle-contracts-v1.json';
}
function checkGateRows(map, name, priorName, count, prefix, sourcePrefix) {
  ...
  for (const [index, row] of rows.entries()) {
    assert.deepStrictEqual(row.command, oldRows[index].command.map(argument =>
      argument.replace('r117-production-metadata.log', 'r118b-production-metadata.log')));
    ...
    const oldLog = e.committed('raw/' + prefix.replace('r118b', 'r117') + '-' + row.name + '.log');
    ...
    if (row.name === 'production-metadata') {
      assert.strictEqual(row.stderr_log, e.file('r118b-production-metadata.stderr.log'));
      e.bytes('r118b-production-metadata.stderr.log');
...
function checkBaseline() {
  ...
  const env = e.read('r118b-environment.json');
  ...
  const inputNames = [...new Set([...h.inputNames(), ...support.inputNames(),
  ...
  const history = h.checkCurrentHistory(map);
  assert.deepStrictEqual(env.history, history);
  assert.deepStrictEqual(env.support_history, support.check(map, history));
  for (const [name] of p.prerequisites) e.ordered(e.read(name + '.json').clock.source_verified, env.observation);
  e.ordered(e.read(env.support_history.previous).clock.source_verified, env.observation);
  const previousContract = checkSupportContracts(map, env);
  const source = e.checkRun('r118b-source-campaign', ['python3', '-B', e.file('r118b-source-gate.py'), 'r118b-final'], map, previousContract);
  e.ordered(env.observation, source.record.clock.start);
  const auxiliary = e.checkRun('r118b-auxiliary-campaign', ['python3', '-B', e.file('r118b-auxiliary-gates.py')], map, 'r118b-source-campaign.json');
  checkCampaignTranscript(source.log, e.read('r118b-final-source-gate.json'), false);
  checkCampaignTranscript(auxiliary.log, e.read('r118b-auxiliary-results.json'), true);
  checkGateRows(map, 'r118b-final-source-gate.json', 'r117-final-source-gate.json', 15, 'r118b-final', 'r118b-final');
  checkGateRows(map, 'r118b-auxiliary-results.json', 'r117-auxiliary-results.json', 10, 'r118b', 'r118b-auxiliary');
  const allNames = e.passing(e.bytes('r118b-reviewed-gnu-all.log').toString());
  let previous = 'r118b-auxiliary-campaign.json';
  for (const focused of p.focused) {
    const name = 'r118b-frozen-' + focused[0];
    checkFocused(e.checkRun(name, e.focusedCommand(focused), map, previous), focused, allNames);
    previous = name + '.json';
  }
  const tests = e.checkRun('r118b-qualification-contract-tests', ['node', e.file('r118b-qualification-tests.js')], map, previous);
  const cases = require('./r118b-qualification-tests.js').cases;
  ...
  return {map, env, history, allNames, helpers, launcher, cases, previous: 'r118b-qualification-contract-tests.json'};
}
function validateManifest(manifest, baseline, io = e) {
  ...
  assert.strictEqual(launcher.name, 'r118b-qualification-launch-v1.js');
...
function collect(closed, getBaseline = checkBaseline) {
  ...
  const mutationNames = p.mutations.flatMap(mutation => [
    ...suffixes.map(suffix => 'r118b-qualified-mut-' + mutation.name + suffix),
    ...['.json', '-source.json'].map(suffix => 'r118b-qualified-restoration-' + mutation.name + suffix),
  ]);
  const focusedNames = p.focused.flatMap(([kind]) => suffixes.map(suffix => 'r118b-qualified-restored-' + kind + suffix));
  const collectorNames = closed ? suffixes.map(suffix => 'r118b-qualified-collector-validation' + suffix) : [];
  ...
  for (const mutation of p.mutations) {
    ...
    previous = name + '.json';
    const restoredName = 'r118b-qualified-restoration-' + mutation.name + '.json';
  ...
  for (const focused of p.focused) {
    const name = 'r118b-qualified-restored-' + focused[0];
    checkFocused(e.checkRun(name, e.focusedCommand(focused), map, previous), focused, allNames);
    previous = name + '.json';
  }
  const collector = 'r118b-qualified-collector-validation';
  const command = ['node', e.file('r118b-qualification-collect.js'), '--check'];
  ...
  const summary = {source_head: p.parent, accepted_parent: p.accepted, source_map_sha256: p.sourceMapSha256,
    ...
    corrected_history: history, stopped_prior: history.stopped_prior, source_gates: 17, subsequent_source_gates: 15, auxiliary_gates: 10,
    ...
    core_contract_tests: 48, runner_contract_tests: 9, prior_history_contract_tests: 45, current_history_contract_tests: 27,
...
function archive({summary, manifest, names, captured}, io = {
  ...
  const destination = path.join(e.repo, 'docs/evidence/local-r118b-admission-lifecycle-2026-09-15');
  ...
  const artifacts = [...new Set([...manifest.historical_artifacts.map(item => item.name), 'r118b-qualification-manifest.json', ...cohort])].sort();
  for (const name of [...e.historicalNames(), ...p.helpers, 'r118b-environment.json', 'r118b-frozen-source.json']) {
  ...
  io.write(path.join(destination, 'README.md'), '# R118B Admission And Lifecycle Evidence\n\n' +
    ...
    'Stopped R118, including 72 qualified negatives and rejected attempt 73, is retained as unaccepted history. The corrected preliminary regression does not count toward the new 78-execution campaign. Original source maps and failed helper attempts retain their original contexts. Old-boot history remains independent. Endpoint equality does not establish continuous immutability.\n' +
