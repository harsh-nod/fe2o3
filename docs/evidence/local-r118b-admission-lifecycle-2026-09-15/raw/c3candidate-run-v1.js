...
(async () => {
  ...
  let previous = null;
  if (previousName !== '-') {
    assert(/^c3candidate-[a-z0-9-]+\.json$/.test(previousName));
    ...
    assert.strictEqual(record.clock.contract, 'c3candidate-raw-utc-boot-monotonic-v1');
  ...
  const deadline = ['c3candidate-source-campaign', 'c3candidate-auxiliary-campaign'].includes(name) ? 7200000 : 1800000;
  let timedOut = false;
  ...
  const record = {source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    clock: {contract: 'c3candidate-raw-utc-boot-monotonic-v1', predecessor: previous, start, finish: result.finish, source_verified: verified, error: clockError}};
