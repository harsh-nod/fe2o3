...
const runner = path.join(root, 'r118-run-v1.js');
...
async function mocked(name, mode) {
  ...
  const fakeProcess = {
    argv: ['node', runner, 'r118-mocked-' + name, '-', 'fixture'], env: {},
  ...
  const record = JSON.parse(writes.get(path.join(root, 'r118-mocked-' + name + '.json')));
...
(async () => {
  ...
  try {
    ...
    const cases = [
      ...
      ['spawn', '/no-such-r118-qualification-command', [], 127],
    ];
    for (const [name, command, args, expected] of cases) {
      const label = 'r118-runner-v1-fixture-' + name;
      let result;
      try {
