...
(async () => {
  ...
  await test('foreign cohort and missing predecessors reject', async () => {
    assert.throws(() => c.predecessor('r108-repeat-mut-slot-only.json', 'finished_at'), /clock cohort/);
    assert.throws(() => c.predecessor('r108-clock-does-not-exist.json', 'finished_at'), /ENOENT/);
    assert.throws(() => c.predecessor('r108-clock-does-not-exist.json', 'anything'), /predecessor field/);
  });
  const predecessorName = 'r108-clock-test-parent.json';
  ...
  await test('wrong event source cohort bytes or outcome rejects', async () => {
    for (const patch of [{name: 'r108-clock-other.json'}, {field: 'verified_at'}, {kind: 'restoration'},
