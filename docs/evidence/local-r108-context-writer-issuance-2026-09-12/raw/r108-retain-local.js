...
const auxiliary = read('r108-auxiliary-results.json');
for (const [rows, oldName, count, prefix, logPrefix] of [
  [gates, 'raw/r107-accepted-source-gate.json', 17, 'r108-final', 'r108-final'],
  [auxiliary, 'raw/r107-accepted-auxiliary-results.json', 10, 'r108-auxiliary', 'r108'],
]) {
  ...
  for (const [i, row] of rows.entries()) {
    assert.deepStrictEqual(row.command, old[i].command.map(s => s.replace('r107-accepted-production-metadata.log', 'r108-production-metadata.log')), row.name);
    ...
    if (row.name === 'production-metadata') {
      assert.strictEqual(row.stderr_log, file('r108-production-metadata.stderr.log'));
...
const auxCampaign = checkRun('r108-auxiliary-campaign', ['python3', '-B', file('r108-auxiliary-gates.py')]);
...
for (const [kind, filter, names] of [
  ...
  assert(names.length);
  const run = checkRun('r108-' + phase + '-' + kind, [...model, filter]);
...
let lastRestoration = Math.max(time(sourceCampaign.finished_at), time(auxCampaign.finished_at),
  ...focused.frozen.map(r => time(r.finished_at)), time(pins.recorded_at));
...
for (const mutation of mutations) {
  ...
  const name = 'r108-mut-' + mutation.name;
  ...
  const restoration = read('r108-restoration-' + mutation.name + '.json');
...
const validation = checkRun('r108-collector-validation', ['node', file('r108-retain-local.js'), '--check']);
