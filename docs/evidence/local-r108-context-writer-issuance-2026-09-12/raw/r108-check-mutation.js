...
const mutation = read('r108-mutations.json').find(m => m.name === name);
assert(mutation, 'known mutation');
const run = read('r108-' + prefix + 'mut-' + name + '.json');
...
assert.strictEqual(run.source_head, 'f767fad35d24f585e8ecb95558bce97a9d543cfe');
assert.deepStrictEqual(run.command, ['cargo', '+nightly-2026-04-03', 'test', '--locked',
  '--offline', '-p', 'fe2o3-runtime-model', '--lib', mutation.test, '--', '--exact']);
...
const inputs = read('r108-' + prefix + 'mut-' + name + '-source.json');
