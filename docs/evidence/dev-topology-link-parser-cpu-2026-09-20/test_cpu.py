...
spec = importlib.util.spec_from_file_location("link_cpu", path)
...
class Calibration(unittest.TestCase):
    def test_commands(self):
        ...
        for name, command, _ in commands:
            if name in {target + "-tests" for target in Q.CONFIGURATIONS}:
                self.assertEqual(command[-4:], list(Q.FILTERS))
    ...
    def test_test_output(self):
        ...
        for names in [Q.NEW_TESTS, {f"topology::tests::substituted_{index}" for index in range(93)}]:
            with self.assertRaises(RuntimeError):
