...
def main():
    ...
    commands = [
        ...
        ('gnu', cargo + ['test'] + packages + ['--all-features', '--lib'], 1200),
        ('musl', cargo + ['test'] + packages + ['--all-features', '--lib', '--target', 'x86_64-unknown-linux-musl'], 1200),
    ...
    if len(sys.argv) > 2 and sys.argv[2] == 'focused':
        commands = [('focused', cargo + ['test'] + packages + ['--all-features', '--lib', 'xgmi_'], 1200)]
    try:
