#!/usr/bin/env python3
"""Bounded, serialized CPU qualification of native XGMI backing budgets."""
import hashlib
...
def main():
    ...
    commands = [
        ...
        ('gnu', cargo + ['test'] + packages + ['--all-features', '--lib', '--', '--test-threads=4'], 7200),
        ('musl', cargo + ['test'] + packages + ['--all-features', '--lib', '--target', 'x86_64-unknown-linux-musl', '--', '--test-threads=4'], 7200),
    ...
    if len(sys.argv) > 2 and sys.argv[2] == 'focused':
        commands = [('focused', cargo + ['test'] + packages + ['--all-features', '--lib', 'xgmi_', '--', '--test-threads=4'], 1200)]
    try:
