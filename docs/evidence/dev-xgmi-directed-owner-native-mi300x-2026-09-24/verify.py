...
ROOT = HERE.parents[2]
SOURCE = "87a9a4c90af12f09457249608dcbb215e7028847"
EXECUTION_ROOT = "/home/harsh/.codex-tmp/fe2o3-xgmi-shared-read-20260924-YGPsyc0Y/source"
LOCAL_OUTPUT = "/home/harsh/.codex-tmp/fe2o3-xgmi-directed-owner-results-ney9nh1_"
...
def verify(folder=HERE, sealed=True):
    ...
    need(binding["payload"]["kfd-owner"] == "60ee9afb8f77cc72c0b691ff38ccbf79862bf7cf61c37b1a4490547c1c19d502", "exact qualified ELF")
    ...
    parsed = [{"trial": "directed-owner", "result": R.parse_receipt((remote / "directed-owner/stdout").read_bytes(), unique_ids=[int(d[2], 16) for d in DEVICES])}]
    ...
    return {"source_commit": SOURCE, "trials": 1, "directed_copies": 4, "endpoint_observations": 6,
            "runtime_tests_each": 1341, "runtime_ignored_each": 20, "doctests": 46,
            "example_tests": 6, "python_tests": 14, "checked_bytes": 327680,
            "pending_dataflow": "supported", "owned_cleanup": True,
...
def verify_cpu_archive(archive, local, campaign_started):
    relative = "docs/evidence/dev-xgmi-shared-source-cpu-2026-09-24"
...
def verify_cpu_contents(archive, local, campaign_started):
    ...
    commands = {
        ...
        "gnu": cargo + ["test", "-p", "fe2o3-runtime", "--all-features", "--lib"],
        "musl": cargo + ["test", "-p", "fe2o3-runtime", "--all-features", "--lib", "--target", "x86_64-unknown-linux-musl"],
        "docs": cargo + ["test", "-p", "fe2o3-runtime", "--all-features", "--doc"],
        "gnu-example": cargo + ["test", "-p", "fe2o3-runtime", "--example", EXAMPLE],
        "musl-example": cargo + ["test", "-p", "fe2o3-runtime", "--example", EXAMPLE, "--target", "x86_64-unknown-linux-musl"],
    ...
    for name, command in commands.items():
        ...
        if name in ("gnu", "musl", "docs"):
            counts = re.findall(rb"test result: ok\. (\d+) passed; (\d+) failed; (\d+) ignored; (\d+) measured; (\d+) filtered out;", (folder / "stdout.log").read_bytes())
            need(counts == ([(b"4", b"0", b"0", b"0", b"0"), (b"42", b"0", b"0", b"0", b"0")] if name == "docs" else [(b"1341", b"0", b"20", b"0", b"0")]), "full CPU runtime tests")
        if name in ("gnu", "musl"):
            suites = {
                "kfd_backend::xgmi_batch::tests::shared_sources::": [
                    "complete_shared_roster_is_healthy_but_busy_after_custody_validation",
                    "frontier_custody_allows_only_unpublished_siblings_or_dependent_later_owners",
                    "healthy_sharing_does_not_hide_a_later_writer_or_unauthenticated_root",
                    "production_owner_roster_checks_both_sibling_orientations_and_corrupt_indexes"],
                "kfd_backend::xgmi_directed::tests::shared_sources::": [
                    "exact_siblings_form_a_single_mapping_prefix_and_flush_rejects_before_publication",
                    "independent_destination_writes_still_reject",
                    "legacy_and_readwrite_roots_do_not_gain_the_shared_read_exception",
                    "owner_and_root_corruption_are_not_busy_or_permission",
                    "producer_write_still_requires_an_explicit_dependency",
                    "owner_capacity_is_bounded_and_hazards_keep_precedence",
                    "production_owner_gate_admits_the_unchanged_diamond_without_sibling_dependency",
                    "publication_never_skips_a_reader_or_masks_later_corruption",
                    "publication_window_stays_bounded_and_flush_capacity_precedes_selection",
                    "restored_mapping_allows_the_next_reader_without_a_new_dependency",
                    "shared_source_provenance_and_prefix_work_in_both_directions"],
                "kfd_backend::xgmi_progress::tests::": [
                    "shared_source_progress_waits_for_the_first_reader_and_retries_after_recovery"],
            }
            expected = {prefix + test for prefix, tests in suites.items() for test in tests}
            raw = (folder / "stdout.log").read_text()
            split_name = "kfd_backend::tests::sdma_promotion_tests::sdma_promotion_native_diagnostic_stays_typed_until_rooted_formatting"
            fragment = "test " + split_name + " ... \nrunning 1 test\nok\n"
            need(raw.count(fragment) == int(name == "musl"), "exact child-banner interleaving")
            raw = raw.replace(fragment, "test " + split_name + " ... ok\n")
            passed = re.findall(r"^test ([a-z0-9_:]+) \.\.\. ok$", raw, re.MULTILINE)
            need(len(passed) == 1341 and len(set(passed)) == 1341 and expected <= set(passed), "named runtime regressions")
