...
def timing_rows(kind, text):
    ...
    for line in text.splitlines():
        ...
        expected_fields = 7 if kind == 'search' else 6
    ...
    sizes = {'ordering': [8, 64, 512, 4096], 'search': [0, 1, 2, 8, 64, 512, 4096], 'enrollment': [8, 64, 512]}[kind]
    patterns = {'ordering': ['ascending', 'descending', 'shuffled', 'duplicates'], 'search': ['repeated', 'mixed'],
                'enrollment': ['fresh', 'half', 'replay_first', 'replay_last', 'capacity', 'alias']}[kind]
    expected = {}
    for size in sizes:
        for pattern in patterns:
            for query in (['key', 'slot'] if kind == 'search' else [None]):
                for round_number in range(7):
                    for flag in ('true', 'false'):
                        key = (size, pattern, *([query] if query else []), str(round_number), flag)
                        expected[key] = max(1_000_000 // size, 100) if kind == 'ordering' else (131072 if kind == 'search' else 1024)
...
def render_results(packet):
    data = {kind: timing_rows(kind, (packet / 'cargo' / kind / 'stdout').read_text())
            for kind in ('ordering', 'search', 'enrollment')}

    def ratio(kind, *coordinates):
        def median(flag):
            return statistics.median(elapsed / count for key, (count, elapsed) in data[kind].items()
                                     if key[:-2] == coordinates and key[-1] == flag)
    ...
    for size in [8, 64, 512, 4096]:
        lines.append('| ' + str(size) + ' | ' + ' | '.join(ratio('ordering', size, pattern) for pattern in ('ascending', 'descending', 'shuffled', 'duplicates')) + ' |')
    lines += ['', '## Production Searches', '',
              ...
              '| --- | --- | --- | --- | --- |']
    for size in [0, 1, 2, 8, 64, 512, 4096]:
        lines.append('| ' + str(size) + ' | ' + ' | '.join(ratio('search', size, pattern, kind) for kind, pattern in [('key', 'repeated'), ('key', 'mixed'), ('slot', 'repeated'), ('slot', 'mixed')]) + ' |')
    lines += ['', '## Complete Enrollment', '',
    ...
    for size in [8, 64, 512]:
        lines.append('| ' + str(size) + ' | ' + ' | '.join(ratio('enrollment', size, scenario) for scenario in ('fresh', 'half', 'replay_first', 'replay_last', 'capacity', 'alias')) + ' |')
    lines += ['', 'The remaining architectural gate is still actual-type lifecycle composition: enrollment',
...
def audit(repo, packet):
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-audit-') as temporary:
        ...
        for name, mutation in [('positive-before', None), *[(m[0], m) for m in check.MUTATIONS], ('positive-after', None)]:
            ...
            stripped = (folder / check.BODY).read_text().replace('macro_rules!', '').encode()
            need((folder / 'audited-shared-bodies.rs').read_bytes() == stripped, 'exact policy projection')
            policy.scan(folder / check.PROOF)
            policy.scan(folder / 'audited-shared-bodies.rs')
            roster.add('proof/' + name + '/audited-shared-bodies.rs')
        ...
        for name, command in cpu.commands():
            ...
            if environment is None:
                ...
                need(target.is_absolute() and target.name == 'target' and target.parent.name.startswith('fe2o3-enrollment-ordering-'), 'owned CPU target')
            ...
            if name == 'test':
                need(test_results(out) == [(866, 0, 5, 0, 0), (27, 0, 0, 0, 0)], 'complete unit/doctest result roster')
            if name in ('ordering', 'search', 'enrollment'):
                need(test_results(out) == [(1, 0, 0, 0, 870)], 'executed benchmark and fixture assertions')
...
def selftest(repo, packet):
    ...
    modifications = [
        ...
        ('timing', 'cargo/search/stdout', None),
    ]
    for name, relative, addition in modifications:
        with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-negative-') as temporary:
            ...
            if addition is not None:
                ...
                first = next(i for i, line in enumerate(lines) if 'search,' in line)
                del lines[first]
                target.write_text('\n'.join(lines) + '\n')
...
def main():
    ...
    if args.selftest:
        ...
        print('PASS: standalone ordering proof and CPU packet reconstructed from Git objects')
