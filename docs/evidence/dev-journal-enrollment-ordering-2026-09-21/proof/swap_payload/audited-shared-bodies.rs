...
#[allow(unused_macros)]
 enrollment_swap_body {
    ($values:ident, $left:ident, $right:ident) => {{
        let a = $values[$left];
        let b = $values[$right];
        $values[$left] = b;
        $values[$right] = match a { Some(mut value) => { value.key.local = 0; Some(value) }, None => None };
    }};
}

#[allow(unused_macros)]
 enrollment_sorted_body {
