#!/usr/bin/env python3
"""Reject altered solver diagnostics and unauthenticated settlement execution inputs."""
import argparse
import copy
...
def main():
    ...
    spec = importlib.util.spec_from_file_location('settlement_commit_diagnostic_selftest', path)
    ...
    source = (case / target).read_text()
    record = json.loads((case / 'solver/record.json').read_text())
    original = Path(record['command'][-1]).with_name(target)
    ...
    for field, value in [('file_name', '/foreign.rs'), ('file_name', record['command'][-1]),
                         ('byte_start', 0), ('line_start', 1), ('is_primary', 1),
                         ('label', 'different clause'), ('expansion', {})]:
        changed = copy.deepcopy(diagnostics)
    ...
    originals = {name: (good / name).read_text() for name in checker.PREFIXES}
    with tempfile.TemporaryDirectory(prefix='fe2o3-settlement-commit-source-selftest-') as temporary:
        for index in range(6):
            folder = Path(temporary) / str(index)
            shutil.copytree(good, folder)
            changed = dict(originals)
            if index == 1:
                changed[checker.COMMIT] = changed[checker.COMMIT].replace('include!("context_version_journal_settlement_v1.rs");', 'include!("foreign.rs");')
            elif index == 2:
                changed[checker.SOURCE] += '\ninclude!("foreign.rs");\n'
            elif index == 3:
                path = folder / module.commit_parent.SOURCE
                path.write_bytes(path.read_bytes() + b'\n')
            elif index == 4:
                path = folder / module.custody_checker.GUARDS
                path.write_bytes(path.read_bytes().replace(b'<= 2_097_152', b'<= 2_097_153'))
            elif index == 5:
    ...
    print(json.dumps({'adverse_solver_results_rejected': len(invalid), 'adverse_source_inputs_rejected': 5,
