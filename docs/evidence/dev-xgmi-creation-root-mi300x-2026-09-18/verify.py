...
def verify():
    ...
    for depth in (1, 16):
        for backend in ("kfd", "hsa", "hip"):
            ...
            N.need(
                remote_rows[name + "-delayed-gpu1"]["started_ns"]
                - remote_rows[name + "-after-gpu2"]["finished_ns"]
                >= 20 * 10**9,
                "twenty-second delayed postflight",
            )
    N.need(
        results == read(remote / "validated-results.json") and len(results) == 8,
