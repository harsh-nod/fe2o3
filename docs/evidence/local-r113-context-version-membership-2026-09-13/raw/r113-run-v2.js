...
(async () => {
  ...
  const result = await new Promise(resolve => child.on('close', (code, signal) => {
    const finish = sample();
    clearTimeout(timer); resolve({code, signal, finish});
  }));
  ...
  const record = {source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    child_returncode: result.code, spawn_error: spawnError,
  ...
  process.exitCode = clockError || cleanup.status === 'error' || timeoutCleanup?.status === 'error' ? 125 : record.returncode;
})().catch(error => {
