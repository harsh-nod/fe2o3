...
const runner = path.join(root, 'r113-run-v2.js');
...
async function mocked(name, mode) {
  ...
  const fakeTimer = (callback, ms) => {
    assert.strictEqual(ms, 1800000);
    if (mode === 'timeout') setImmediate(callback);
    return 1;
  };
  ...
  await vm.runInNewContext(source, {
    ...
    process: fakeProcess, __filename: runner, setTimeout: fakeTimer, clearTimeout: () => {},
    console: {log: () => {}, error: () => {}},
  }, {filename: runner});
  const record = JSON.parse(writes.get(path.join(root, 'r113-mocked-' + name + '.json')));
  assert.strictEqual(record.child_returncode, mode === 'timeout' ? null : 0);
  assert.strictEqual(record.signal, mode === 'timeout' ? 'SIGKILL' : null);
  assert.strictEqual(record.timed_out, mode === 'timeout');
  assert.strictEqual(record.returncode, mode === 'timeout' ? 124 : 0);
  assert.strictEqual(record.clock.error, null);
  assert.strictEqual(fakeProcess.exitCode, mode === 'eperm' ? 125 : mode === 'timeout' ? 124 : 0);
  assert.strictEqual(record.process_group_cleanup.close.status, mode === 'eperm' ? 'error' : 'absent');
  assert(signals.length === (mode === 'timeout' ? 2 : 1));
...
(async () => {
  ...
  try {
    ...
    for (const [name, command, args, expected] of cases) {
      const label = 'r113-runner-fixture-' + name;
      const result = cp.spawnSync(process.execPath, [runner, label, '-', command, ...args], {encoding: 'utf8', timeout: 30000, maxBuffer: 1024 * 1024});
