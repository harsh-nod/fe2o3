...
[[ $(sha256sum "$archive/source-files.sha256" | awk '{print $1}') == 35aadbd8b642c57095f0fdaf9437bb888c218ba0b38c68943ce3262f4855f810 ]]
[[ $(sha256sum "$archive/source.patch" | awk '{print $1}') == 5e74f1b3a7193ad6d076ac77c9784cd86c7f1f36b90251925f6a4b7b26ebd0d3 ]]
...
scope=(. ':(exclude)docs/evidence/dev-v4j4-reader-invariant-2026-09-17' ':(exclude)docs/evidence/dev-v4j4-reader-invariant-preliminary-2026-09-17')
...
[[ $(sha256sum "$archive/../dev-v4j4-reader-invariant-preliminary-2026-09-17/SHA256SUMS" | awk '{print $1}') == d2b55db01a1d3673ba817884c22a46ec35203884c753d8dde208ad6a58a75017 ]]
(cd -- "$archive/../dev-v4j4-reader-invariant-preliminary-2026-09-17" && sha256sum --check --quiet SHA256SUMS)
cd -- "$archive"
