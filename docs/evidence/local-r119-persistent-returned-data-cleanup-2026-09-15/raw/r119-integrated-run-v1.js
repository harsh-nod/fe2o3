...
assert(/^r119-integrated-[a-z0-9-]+$/.test(name) && previousName && command);
...
(async () => {
  ...
  let previous = null;
  if (previousName !== '-') {
    assert(/^r119-integrated-[a-z0-9-]+\.json$/.test(previousName));
    ...
    assert.strictEqual(record.clock.contract, 'r119-integrated-raw-utc-boot-monotonic-v1');
  ...
  const deadline = ['r119-integrated-source-campaign', 'r119-integrated-auxiliary-campaign'].includes(name) ? 7200000 : 1800000;
  let timedOut = false;
  ...
  const record = {source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    clock: {contract: 'r119-integrated-raw-utc-boot-monotonic-v1', predecessor: previous, start, finish: result.finish, source_verified: verified, error: clockError}};
