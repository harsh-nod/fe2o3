"""Remaining fifteen source gates; exact full GNU/musl prerequisites run separately."""
import hashlib
import json
import os
...
root = repo.parent
attempt = sys.argv[1]
assert re.fullmatch(r'r119-integrated-[a-z0-9]+', attempt)
...
env = dict(os.environ, CARGO_BUILD_JOBS='4', CARGO_INCREMENTAL='0', RUST_TEST_THREADS='4', PYTHONDONTWRITEBYTECODE='1')
...
after = identities()
with (root / f'{attempt}-source-after.json').open('x') as output:
    json.dump(after, output, indent=2)
assert after == before, 'source changed during final gates'
with (root / f'{attempt}-complete.json').open('x') as output:
