...
const runner = path.join(root, 'r119-integrated-run-v1.js');
const source = fs.readFileSync(runner, 'utf8');
const parent = '8e2c8532cb60918de523c6cab1b861bcd61fc319';
const inputs = JSON.parse(fs.readFileSync(path.join(root, 'r119-integrated-runner-inputs-v1.json'), 'utf8'));
assert.strictEqual(inputs.parent, parent);
assert.strictEqual(inputs.source_map_sha256, '542110b3429161394392f24a398929aba37aa1d74950d0a86204adda2d0959d2');
assert.deepStrictEqual(inputs.helpers.map(item => item.name), ['r119-integrated-runner-tests-v1.js', 'r119-integrated-run-v1.js']);
for (const pin of inputs.helpers) {
  assert.strictEqual(require('crypto').createHash('sha256').update(fs.readFileSync(path.join(root, pin.name))).digest('hex'), pin.sha256);
}
let passed = 0;
...
async function mocked(name, mode) {
  ...
  const fakeProcess = {
    argv: ['node', runner, 'r119-integrated-mocked-' + name, '-', 'fixture'], env: {},
  ...
  const record = JSON.parse(writes.get(path.join(root, 'r119-integrated-mocked-' + name + '.json')));
...
(async () => {
  ...
  try {
    ...
    const cases = [
      ...
      ['spawn', '/no-such-r119-integrated-qualification-command', [], 127],
    ];
    for (const [name, command, args, expected] of cases) {
      const label = 'r119-integrated-runner-v1-fixture-' + name;
      let result;
      try {
  ...
  for (const name of require('./r119-integrated-runner-evidence-v1.js').fixtureNames) {
    const bytes = fs.readFileSync(path.join(root, name));
    console.log('R119_RUNNER_FIXTURE ' + name + ' ' + require('crypto').createHash('sha256').update(bytes).digest('hex'));
  }
