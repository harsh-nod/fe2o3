...
const supportRuns = [
  ['history-contracts-v1', ['node', '--test', e.file('r119-integrated-history-tests-v1.js')], 24],
  ['history-validation-v1', ['node', e.file('r119-integrated-history-v1.js')], null],
  ['runner-contracts-v1', ['node', e.file('r119-integrated-runner-tests-v1.js')], null],
  ['freeze-contracts-v1', ['node', '--test', e.file('r119-integrated-freeze-tests-v1.js')], 26],
];
function checkSupport(map, history, io) {
  ...
  for (const [kind, command, count] of supportRuns) {
    ...
    if (count !== null) {
      const names = kind === 'history-contracts-v1' ? v.historyNames : v.freezeNames;
      assert.strictEqual(names.length, count);
      v.checkTap(log, names);
    } else if (kind === 'history-validation-v1') {
  ...
  const pins = io.read('r119-integrated-runner-inputs-v1.json');
  assert.strictEqual(pins.parent, p.parent);
  assert.strictEqual(pins.source_map_sha256, p.sourceMapSha256);
  assert.deepStrictEqual(pins.helpers.map(pin => pin.name), ['r119-integrated-runner-tests-v1.js', 'r119-integrated-run-v1.js']);
  for (const pin of pins.helpers) assert.strictEqual(e.hash(io.bytes(pin.name)), pin.sha256);
  return {history_contracts: 24, runner_contracts: 9, freeze_boundary_contracts: 17, validation_contracts: 9, previous};
}
const defaults = {
