...
if (require.main === module) require('./r119-integrated-validation-v2.js').guard();
...
const priorValidation = require('./r119-integrated-validation-v1.js');
const priorRuns = ['r119-integrated-history-reject-timeout-v1', 'r119-integrated-preliminary-freeze-contracts-v1'];
const suffixes = ['.json', '.log', '-source.json', '-source-after.json'];
const integrated = ['format', 'persistent-focused', 'control-focused', 'gnu-all', 'clippy', 'musl-all', 'musl-retry1'];
...
function inputNames(io = e) {
  return [...new Set([originName, 'r119-integrated-run-v1.js',
    ...
    ...integrated.flatMap(kind => suffixes.map(suffix => 'r119-integrated-' + kind + suffix)),
    priorValidation.manifestName, ...priorValidation.helperNames,
    ...priorRuns.flatMap(name => suffixes.map(suffix => name + suffix))])].sort();
}
...
function checkRecords(map, io = e) {
  ...
  const rejectedTimeout = timeout.checkHistory(map, io);
  const musl = current('musl-retry1', [...p.fullTest, '--target', 'x86_64-unknown-linux-musl'], 'r119-integrated-musl-all.json');
  const full = {
  ...
  e.ordered(clippy.record.clock.source_verified, io.read('r119-integrated-musl-all.json').clock.start);
  checkPriorHelpers(map, io, musl.record.clock.start);
  ...
  return {accepted: false, source_map_sha256: p.sourceMapSha256, source_identities: p.sourceCount,
    isolated_artifacts: 24, isolated_zero_test_run: 'compile-only', rejected_timeout: rejectedTimeout, full,
    persistent_tests: 15, control_tests: 46, kfd_tests: 1149, integrated_clippy: true,
    previous: 'r119-integrated-musl-retry1.json'};
}
function checkPriorHelpers(map, io, retryStart) {
  const pins = priorValidation.inputs(io);
  assert.strictEqual(pins.sha256, '5998a625873e4bc4aa7e4b320c77e3eeb9f6165aaa8c31a9ecba6d611101c0bc');
  const [rejection, contracts] = priorRuns;
  e.checkRecord(io.read(rejection + '.json'), rejection, ['node', e.file('r119-integrated-history-v1.js')],
    map, 'r119-integrated-musl-all.json', 1, io);
  const rejectionLog = priorValidation.transcript(io.bytes(rejection + '.log').toString(), pins.sha256);
  assert(rejectionLog.includes('124 !== 0'));
  assert(rejectionLog.includes('r119-integrated-qualification-evidence-v1.js:44:10'));
  assert.strictEqual(e.hash(io.bytes(rejection + '.log')), '9075a66e872c1564fbc2dfa888ca83831e342cb58f4e9f1b3c92654680b20593');
  assert.strictEqual(e.hash(io.bytes(rejection + '.json')), 'f049423dd589130442fbb1ed2d1379f6e96400a84466fab76320380b70342712');
  e.checkRecord(io.read(contracts + '.json'), contracts, ['node', '--test', e.file('r119-integrated-freeze-tests-v1.js')],
    map, rejection + '.json', 0, io);
  priorValidation.checkTap(priorValidation.transcript(io.bytes(contracts + '.log').toString(), pins.sha256, true), priorValidation.freezeNames);
  assert.strictEqual(e.hash(io.bytes(contracts + '.log')), '5e7f80df1b2b64a15a680a11002830222e03f8d1c81bfc7ae35b878d2aabc8e0');
  assert.strictEqual(e.hash(io.bytes(contracts + '.json')), '72f1cc9d1e453322689e5f6f78048778636e68d64f9e36eb517de9b7c0903c03');
  e.ordered(io.read(contracts + '.json').clock.source_verified, retryStart);
}
