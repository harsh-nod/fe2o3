...
function runEntry(manifest, name, expectedMap, beforeSpawn = () => {}) {
  ...
  beforeSpawn();
  const result = cp.spawnSync(process.execPath, [e.file('r119-integrated-run-v1.js'), name, entry.predecessor, ...entry.command],
...
function bindTerminal(record, name, expected, entry, readArtifact, runnerDigest) {
  ...
  assert.deepStrictEqual(record.runner, {name: 'r119-integrated-run-v1.js', sha256: runnerDigest}, 'manifest-pinned terminal runner');
  assert.strictEqual(e.hash(readArtifact('r119-integrated-run-v1.js')), runnerDigest, 'manifest-pinned live runner');
...
function qualifyMutation({manifest, manifestHash, map, mutation, io = sourceIO,
  ...
  const entry = manifest.entries[name + '.json'];
  assert(entry && entry.kind === 'run');
  const runnerPins = manifest.helpers.filter(helper => helper.name === 'r119-integrated-run-v1.js');
  ...
  if (errors.length === 0 && oraclePassed && completed && state.record) {
    try {
      ...
      const restoredName = 'r119-integrated-qualified-restoration-' + mutation.name;
  ...
  if (errors.length || !record) {
    ...
    try {write('r119-integrated-qualification-failure-' + mutation.name + '.json', diagnostic);}
...
module.exports = {bindTerminal, runEntry, qualifyMutation};
