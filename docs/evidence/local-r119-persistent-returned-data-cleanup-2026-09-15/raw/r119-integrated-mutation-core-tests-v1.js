...
const C = require('./r119-integrated-mutation-core-v1.js');
...
test('executed helper bytes match the separately frozen input manifest', () => V.inputs());
function fixture() {
...
test('restoration I/O failure reports exact mixed state and retains recovery information', () => {
  ...
  f.state.manifest = {name: 'r119-integrated-qualification-manifest.json', sha256: C.hash('manifest')};
...
for (const [name, change] of [
  ...
  const f = fixture(); const manifest = {name: 'r119-integrated-qualification-manifest.json', sha256: C.hash('manifest')};
...
test('real prospective plan derives 37 executions and 30 exact maps without writes', () => {
  const map = JSON.parse(fs.readFileSync(root + 'r119-integrated-frozen-source.json', 'utf8'));
  ...
  assert.strictEqual(new Set(P.newTests).size, 15);
  ...
  for (const mutation of P.mutations) {
    ...
    assert(/assert(?:_eq|_ne)?!\(|panic!\(|\.unwrap\(\)|\.expect\(/.test(fs.readFileSync(repo + mutation.oracle_path, 'utf8').split('\n')[mutation.oracle_line - 1]), 'named behavioral panic site: ' + mutation.name);
  ...
  assert.deepStrictEqual(shared, P.mutationSourceGroups.filter(group => group.length > 1));
  assert.deepStrictEqual(counts, {production: 37});
  assert.strictEqual(P.mutations.filter(m => m.patches.length === 2).length, 0);
});
