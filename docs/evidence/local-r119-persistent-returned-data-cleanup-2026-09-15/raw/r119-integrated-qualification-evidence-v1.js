...
const contract = 'r119-integrated-raw-utc-boot-monotonic-v1';
...
function checkRecord(r, name, command, expectedMap, previousName, code, io = {read, bytes}, context = {
  head: p.parent, cwd: repo, contract, runner: 'r119-integrated-run-v1.js', after: expectedMap,
}) {
  ...
  assert.strictEqual(r.deadline_ms, ['r119-integrated-source-campaign', 'r119-integrated-auxiliary-campaign'].includes(name) ? 7200000 : 1800000);
...
function assertPassing(text) {
  assert.deepStrictEqual(failed(text), [], 'no contradictory failed test rows');
  assert(!/^test result: FAILED\./m.test(text), 'no contradictory failed test summary');
}
...
function checkMutation(log, mutation) {
  ...
  assert(!p.retainedSourceGuards.includes(mutation.test));
  ...
  assert(!/^test result: ok\./m.test(log), 'negative run has a failed summary');
  assert(!/^error\[E\d+\]:/m.test(log), 'not a compile failure');
  for (const marker of ['panic in a destructor during cleanup', 'thread caused non-unwinding panic', 'SIGABRT']) {
    assert(!log.includes(marker), 'not a destructor abort: ' + marker);
  }
...
function focusedNames([, filter, count, exact], allNames) {
  // Rust's non-exact libtest filter is a substring, including pristine_abort.
  const names = allNames.filter(name => exact ? name === filter : name.includes(filter));
  assert.strictEqual(names.length, count, 'exact selected focused roster');
  return names;
}
...
function expectedEntries(map, originals) {
  const entries = {};
  let predecessor = 'r119-integrated-qualification-manifest.json';
  const hashes = [];
  ...
  for (const mutation of p.mutations) {
    ...
    const run = 'r119-integrated-qualified-mut-' + mutation.name + '.json';
    ...
    predecessor = run;
    const restored = 'r119-integrated-qualified-restoration-' + mutation.name + '.json';
  ...
  for (const focused of p.focused) {
    const run = 'r119-integrated-qualified-restored-' + focused[0] + '.json';
    entries[run] = {kind: 'run', predecessor, source_map_sha256: hash(JSON.stringify(map)), command: focusedCommand(focused), returncode: 0};
    predecessor = run;
  }
  entries['r119-integrated-qualified-collector-validation.json'] = {kind: 'run', predecessor,
    source_map_sha256: hash(JSON.stringify(map)), command: ['node', file('r119-integrated-qualification-collect-v1.js'), '--check'], returncode: 0};
...
function historicalNames() {
  const runs = p.mutations.map(mutation => 'r119-integrated-qualified-mut-' + mutation.name)
  ...
  const excluded = new Set(runs.flatMap(name => ['.json', '.log', '-source.json', '-source-after.json'].map(suffix => name + suffix))
    .concat(p.mutations.flatMap(mutation => ['.json', '-source.json'].map(suffix => 'r119-integrated-qualified-restoration-' + mutation.name + suffix)),
      ['r119-integrated-qualification-manifest.json']));
  const local = fs.readdirSync(root).filter(name => /^r119-integrated-.*\.(json|log|js|py|md|rs)$/.test(name) && !excluded.has(name));
  const origin = read('r119-integrated-origin-v1.json');
  return [...new Set([...local, ...origin.isolated_artifacts.map(item => item.name)])].sort();
}
...
module.exports = {root, repo, contract, file, hash, bytes, read, git, committed, identities,
  sample, ordered, checkRecord, checkRun, passing, ignored, failed, assertPassing, totals, executables, checkFull, checkMutation,
  core, originalSources, mutationSources, mutationMap, focusedCommand, focusedNames, checkMutationGroups, expectedEntries,
