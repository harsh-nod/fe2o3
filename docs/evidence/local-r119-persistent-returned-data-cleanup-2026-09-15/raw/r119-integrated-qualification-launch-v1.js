...
const r = require('./r119-integrated-qualification-run-v1.js');
...
function outputNames(entries) {
  return Object.entries(entries).flatMap(([name, entry]) => entryOutputs(name, entry))
    .concat(p.mutations.map(mutation => 'r119-integrated-qualification-failure-' + mutation.name + '.json'));
}
function launch(io = {...e, stat, baseline: c.checkBaseline, qualify: r.qualifyMutation, run: r.runEntry, log: console.log}) {
  ...
  for (const [index, mutation] of p.mutations.entries()) {
    const names = ['r119-integrated-qualified-mut-' + mutation.name + '.json', 'r119-integrated-qualified-restoration-' + mutation.name + '.json'];
    const outputs = names.flatMap(name => entryOutputs(name, manifest.entries[name]))
      .concat('r119-integrated-qualification-failure-' + mutation.name + '.json');
  ...
  for (const focused of p.focused) {
    const result = run('r119-integrated-qualified-restored-' + focused[0], base.map);
