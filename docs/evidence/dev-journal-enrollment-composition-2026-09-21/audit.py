...
def timing_rows(kind, text):
    ...
    need(kind in ('sort_enrollment', 'composition_enrollment'), 'known benchmark')
    patterns.append('prefix_alias')
    if kind == 'composition_enrollment':
        patterns += ['half', 'replay_first', 'replay_last', 'capacity']
    expected = {}
    for size in sizes:
        for pattern in patterns:
            for round_number in range(7):
                for flag in ('true', 'false'):
                    ...
                    expected[key] = min(4096, max(128, 262_144 // size))
...
def render_results(packet):
    kinds = ('sort_enrollment', 'composition_enrollment')
    data = {kind: timing_rows(kind, (packet / 'cargo' / kind / 'stdout').read_text()) for kind in kinds}
    ...
    sizes = [8, 16, 17, 19, 20, 21, 64, 127, 128, 129, 255, 256, 257, 512, 4096]
    patterns = ('ascending', 'descending', 'shuffled', 'duplicates', 'prefix_alias')

    def table(kind, labels, cases):
        result = ['| Batch | ' + ' | '.join(labels) + ' |', '| --- | ' + ' | '.join('---' for _ in labels) + ' |']
        for size in sizes:
            result.append('| ' + str(size) + ' | ' + ' | '.join(ratio(kind, size, pattern) for pattern in cases) + ' |')
        return result

    labels = ('Ascending', 'Descending', 'Shuffled', 'Duplicate Slot', 'Prefix Alias')
    lines += table('sort_enrollment', labels, patterns)
    lines += ['', '## Matched Adaptive-Sort Baseline', '',
    ...
    lines += table('composition_enrollment', labels, patterns)
    lines += ['', '## Occupied and Rejection Paths', '',
    ...
    lines += table('composition_enrollment', ('Half Occupied', 'Replay First', 'Replay Last', 'Capacity'),
                   ('half', 'replay_first', 'replay_last', 'capacity'))
    lines += ['', 'Gate 1 advances actual-type raw batch enrollment, including its caller-precondition derivation.',
...
def audit(repo, packet):
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-audit-') as temporary:
        ...
        for name, mutation in [('positive-before', None), *[(m[0], m) for m in check.MUTATIONS], ('positive-after', None)]:
            ...
            policy.scan(folder / check.ADAPTIVE_PROOF)
        ...
        for name, command in cpu.commands():
            ...
            if environment is None:
                ...
                need(target.is_absolute() and target.name == 'target' and target.parent.name.startswith('fe2o3-enrollment-composition-'), 'owned CPU target')
            ...
            if name in ('sort_enrollment', 'composition_enrollment'):
                need(test_results(out) == [(1, 0, 0, 0, 882)], 'executed benchmark and fixture assertions')
...
def selftest(repo, packet):
    ...
    modifications = [
        ...
        ('timing', 'cargo/sort_enrollment/stdout', None),
        ('composition', 'cargo/composition_enrollment/stdout', None),
    ]
    for name, relative, addition in modifications:
        with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-negative-') as temporary:
            ...
            if addition is not None:
                ...
                marker = {'timing': 'sort_enrollment,', 'composition': 'composition_enrollment,'}[name]
...
def main():
    ...
    if args.selftest:
        ...
        print('PASS: standalone enrollment composition proof and CPU packet reconstructed from Git objects')
