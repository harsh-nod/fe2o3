// Shared enrollment execution. Annotation hooks contain only proof/loop metadata.
 ...
 enrollment_duplicate_slots_body {
 ...
 enrollment_selected_vacant_body {
    ($syntax:ident, $journal:ident, $remaining:ident, $index:ident, [$($invariants:tt)*]) => {
        $syntax!({
                ...
                $($invariants)*
            {
                ...
                if slot >= $journal.allocations.len() || $journal.allocations[slot].is_none() { return false; }
                $index += 1;
            }
            true
        })
 ...
 enrollment_retained_clear_body {
