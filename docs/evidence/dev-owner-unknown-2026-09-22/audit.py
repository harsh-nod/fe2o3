...
CHECK = Path('crates/fe2o3-runtime-model/verus/check-owner-unknown.py')
...
def audit(repo, packet, quiet=False):
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-unknown-audit-') as temporary:
        tree = Path(temporary)
        ...
        check = module(tree / CHECK, 'unknown_record_check')
        legacy = module(tree / check.LEGACY, 'unknown_record_legacy')
        base = module(tree / legacy.BASE, 'unknown_record_base')
        expected = set(legacy.SOURCES) | set(check.NEW) | set(legacy.PINS) | {
            check.LEGACY, CHECK, check.CRATE / 'verus/context_version_journal_settlement_v1.rs',
        ...
        parse = module(target, 'unknown_cpu_result_parser').test_results
        need(parse((records / 'test/stdout.log').read_text()) == [(935, 0, 18, 0, 0), (27, 0, 0, 0, 0)], 'unit and doctest results')
...
def selftest(repo, packet):
    for case in ('probe', 'source_roster', 'positive_count', 'positive_type', 'negative_scope', 'cpu_count',
                 'closure_transcript', 'receipt_order', 'auditor_identity'):
        with tempfile.TemporaryDirectory(prefix='fe2o3-unknown-audit-test-') as temporary:
            ...
            if case in ('probe', 'source_roster'):
                ...
                path = records / 'journal_noop/record.json'
                row = read(path)
                index = row['command'].index('--verify-function') + 1
                row['command'][index] = 'shared_retained_unknown_v1'
                path.write_text(json.dumps(row))
            elif case == 'cpu_count':
                path = records / 'test/stdout.log'
                text = path.read_text()
                need(text.count('935 passed;') == 1, 'CPU selftest anchor')
                path.write_text(text.replace('935 passed;', '934 passed;'))
    ...
    print('PASS: nine rehashed malformed record sets rejected')
