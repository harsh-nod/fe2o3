...
for (const m of mutations) {
  ...
  const restored = read('r105-restoration-' + m.name + '.json');
  ...
  mutationTiming.push({started: Date.parse(run.started_at), restored: verifiedAt});
}
...
for (let i = 1; i < mutationTiming.length; i++) {
  assert(mutationTiming[i].started >= mutationTiming[i - 1].restored, 'restore before next mutation');
}
