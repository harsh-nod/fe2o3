...
SOURCE = '8ff8efee5d24bd6bbf7c5ef03ed3e542c214387b'
CHECK = Path('crates/fe2o3-runtime-model/verus/check-owner-settlement-historical.py')
...
def audit(repo, packet, quiet=False):
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-settlement-history-audit-') as temporary:
        tree = Path(temporary)
        ...
        settlement = module(tree / check.PREVIOUS, 'settlement_record_raw')
        ...
        inherited = {Path(p) for p in frozen['inputs']} | {check.FROZEN_INPUTS, *check.ORIGINALS}
        ...
        need({p for p in committed if p.is_relative_to(check.CRATE / 'src')}
             == {p for p in inherited if p.is_relative_to(check.CRATE / 'src')}, 'committed runtime source roster')
        ...
        check.authenticate(tree, inherited, writer, git_repo=repo)
        ...
        cases = [('positive-before', None), *[(m[0], m) for m in check.mutations(settlement)], ('positive-after', None), ('raw-regression', None)]
        ...
        for name, mutation in cases:
            ...
            receipts[name] = row
            root = settlement.RAW if name == 'raw-regression' else check.ROOT
            ...
            if mutation:
                ...
                need(legacy.same(observed['result'], {'encountered-error': False, 'encountered-vir-error': False,
                    'success': True, 'verified': 306 if name == 'raw-regression' else check.HISTORICAL_VERIFIED,
...
def selftest(repo, packet):
    cases = ('probe', 'source_roster', 'positive_count', 'positive_type', 'negative_scope', 'cpu_count',
             ...
             'negative_type', 'negative_count', 'negative_roster', 'historical_root', 'historical_input')
    for case in cases:
        with tempfile.TemporaryDirectory(prefix='fe2o3-settlement-history-audit-test-') as temporary:
            ...
            if case in ('probe', 'source_roster'):
                ...
                path.write_text(json.dumps(row))
            elif case == 'historical_root':
                path = records / 'positive-before/record.json'
                row = read(path)
                row['command'][-1] = row['command'][-1].replace(
                    'context_owner_settlement_historical_v1.rs', 'context_owner_settlement_execution_v1.rs')
                path.write_text(json.dumps(row))
            elif case == 'historical_input':
                path = records / 'source.json'
                row = read(path)
                del row['inputs']['crates/fe2o3-runtime-model/verus/context_version_journal_settlement_custody_v1.rs']
                path.write_text(json.dumps(row))
                (records / 'inputs-after.json').write_text(json.dumps(row['inputs']))
            elif case == 'cpu_count':
