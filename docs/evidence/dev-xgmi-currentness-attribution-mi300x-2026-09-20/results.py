#!/usr/bin/env python3
"""Strict parser for depth-one KFD full-currentness host attribution."""
...
_DIAGNOSTIC_FIXED = {
    "schema": "fe2o3.xgmi-aggregate-currentness-attribution.v1",
    "backend": "kfd",
    "authority": "none",
    "teardown": "explicit",
...
_PAIR_PHASES = (
    "source_before_ns",
    "peer_before_ns",
    "topology_discovery_ns",
    "route_and_equality_ns",
    "source_after_ns",
    "peer_after_ns",
)
_TOPOLOGY_PHASES = (
...
_NESTED = (*_PAIR_PHASES, "pair_total_ns", *_TOPOLOGY_PHASES, "topology_total_ns")
_DIAGNOSTIC_KEYS = {
    *_DIAGNOSTIC_FIXED,
    "ordinal",
    "backend_submission",
    ...
    *_PHASES,
    "total_ns",
    *(f"{edge}_{field}" for edge in ("opening", "closing") for field in _NESTED),
}
...
def _summary(line: bytes, mode: str, uids: tuple[str, str]) -> dict[str, str]:
    ...
    if mode == "on":
        constants["diagnostic"] = "aggregate-currentness-attribution"
...
def _diagnostic(line: bytes, ordinal: int, uids: tuple[str, str]) -> dict[str, object]:
    fields = _tokens(line)
    ...
    for edge in ("opening", "closing"):
        nested = {key: _u64(fields[f"{edge}_{key}"]) for key in _NESTED}
        for phases, total_key in (
            (_PAIR_PHASES, "pair_total_ns"),
            (_TOPOLOGY_PHASES, "topology_total_ns"),
        ):
            summed = sum(nested[key] for key in phases)
            if summed > _U64_MAX or summed > nested[total_key]:
                raise ValueError("nested phases exceed their total or u64")
        if nested["topology_total_ns"] > nested["topology_discovery_ns"]:
            raise ValueError("topology total exceeds pair discovery span")
        if nested["pair_total_ns"] > durations[edge + "_currentness_ns"]:
            raise ValueError("pair total exceeds aggregate currentness span")
        currentness[edge] = nested
...
def parse_result(data: bytes, backend: str, unique_ids: list[str]) -> dict[str, object]:
    ...
    if backend == "on":
        diagnostics = [
            _diagnostic(line, ordinal, uids) for ordinal, line in enumerate(lines[:-1])
        ]
