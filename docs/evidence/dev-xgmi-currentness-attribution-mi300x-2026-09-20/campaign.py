#!/usr/bin/env python3
"""Collect signed-source, owned-directory full-currentness host attribution."""
...
ROOT = HERE.parents[2]
CPU = ROOT / "docs/evidence/dev-xgmi-currentness-capture-cpu-2026-09-20"
...
NATIVE_SHA = "293a25fd764cbf3fbe17b9e241f70c44fa5aee15f04926707037c54189b7cab8"
NATIVE_TIMEOUT = 6000
SIGNERS = (
...
def native_module():
    path = HERE / "native.py"
    need(path.is_file() and not path.is_symlink(), "ordinary native controller dependency")
    need(sha(path) == NATIVE_SHA, "authenticate native controller dependency")
    return load(path, "aggregate_campaign_native")
...
def run():
    ...
    try:
        ...
        need(sha(N.HOT_SOURCE) == N.HOT_SHA, "pinned endpoint helper")
        shutil.copyfile(N.HOT_SOURCE, payload / "hot.py")
