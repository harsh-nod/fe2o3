#!/usr/bin/env python3
"""Replay full-currentness attribution, endpoint admission, and collection."""
...
def verify():
    ...
    need(
        ...
        and binding["source_files"] == cpu["source"]["files"]
    ...
    need(binding["payload"]["hot.py"] == N.HOT_SHA, "transported endpoint helper")
    signed_tar = subprocess.check_output(
        [
            ...
            commit,
            "--",
            *C.source_paths(cpu["source"]["files"]),
        ],
        cwd=ROOT,
    )
    ...
    for name, backend, command, trial_env in N.trial_specs(owned):
        for index, bdf, uid in N.DEVICES:
            ...
            N.H.parse_endpoint((remote / stage / "stdout").read_bytes(), index, bdf, uid)
        trial = check(name, command, 300, trial_env)
    ...
    need(
        ...
        "replayed all four result transcripts",
    )
