...
const script = path.join(root, 'r114-freeze.js');
...
function fixture(name, mutate, rejects, openingHead = parent, prerequisite = value => value) {
  ...
  for (const index of [5, 7, 9]) delete previousMap[delta[index]];
  const binaries = ['cargo', 'rustc'].map(name => ({name, path: '/fixture/bin/' + name, sha256: hash(name)}));
  const artifacts = new Map(helpers);
  const prior = {source_head: parent, clock: {contract: 'r114-raw-utc-boot-monotonic-v1', error: null, source_verified: observation(0)}};
  artifacts.set('r114-device-oracle-format.json', JSON.stringify(prior));
  for (const [index, [run, command, previous]] of p.prerequisites.entries()) {
    ...
    const record = prerequisite({source_head: parent, source_map_sha256: hash(JSON.stringify(expectedMap)), cwd: repo, command,
      ...
      runner: {name: 'r114-run-v4.js', sha256: hash(helpers.get('r114-run-v4.js'))},
      process_group_cleanup: {timeout: null, close: {status: 'absent', pgid: 42, error: 'Error: kill ESRCH', live_members: [], observation: close}},
      clock: {contract: 'r114-raw-utc-boot-monotonic-v1', error: null, start, finish, source_verified: verified,
  ...
  const fakeFs = {
    ...
    readFileSync: file => {
      if (file.includes('/raw/r113-environment.json')) return JSON.stringify({binaries});
      if (file.includes('/raw/r113-frozen-source.json')) return JSON.stringify(previousMap);
  ...
  function localRequire(name) {
    ...
    if (!name.startsWith('./r114-')) return require(name);
  ...
  if (rejects) {
    ...
    assert.deepStrictEqual(writes.map(([file]) => file), ['frozen-source', 'environment', 'issue-status'].map(n => path.join(root, 'r114-' + n + '.json')));
...
fixture('deleted-untracked-member-test', ({files, listed}) => {files.delete(delta[5]); listed.delete(delta[5]);}, true);
...
fixture('same-count-replacement', ({files, listed}) => {
  files.delete(delta[5]); listed.delete(delta[5]); files.set('scripts/replacement.rs', 'candidate'); listed.add('scripts/replacement.rs');
}, true);
...
fixture('prerequisite-artifact-substitution', ({artifacts}) => artifacts.set('r114-preliminary-gnu-all.log', 'substituted'), true);
