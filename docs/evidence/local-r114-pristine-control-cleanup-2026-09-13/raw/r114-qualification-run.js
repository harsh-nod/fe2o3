...
const map = e.read('r114-frozen-source.json');
...
function run(name, expectedMap) {
  ...
  const result = cp.spawnSync(process.execPath, [e.file('r114-run-v4.js'), name, entry.predecessor, ...entry.command],
...
for (const [index, mutation] of p.mutations.entries()) {
  ...
  try {
    ...
    if (!restoration.restored) {
      fs.writeFileSync(e.file('r114-qualification-failure-' + mutation.name + '.json'), JSON.stringify({
    ...
    if (completed) {
      ...
      const restoredName = 'r114-qualified-restoration-' + mutation.name;
  ...
  if (failure) {
    fs.writeFileSync(e.file('r114-qualification-failure-' + mutation.name + '.json'),
...
for (const [kind, filter, count] of p.focused) {
  const result = run('r114-qualified-restored-' + kind, map);
  assert.strictEqual(e.passing(result.log).length, count);
  console.log('PASS restored ' + kind + ': ' + count);
}
run('r114-qualified-collector-validation', map);
