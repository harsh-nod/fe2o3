...
const parent = 'd85d6d7dc4f3b065dce9dc6505eac065c48cb0fa';
const [name, previousName, command, ...args] = process.argv.slice(2);
assert(/^r114-[a-z0-9-]+$/.test(name) && previousName && command);
...
(async () => {
  ...
  let previous = null;
  if (previousName !== '-') {
    assert(/^r114-[a-z0-9-]+\.json$/.test(previousName));
    ...
    assert.strictEqual(record.clock.contract, 'r114-raw-utc-boot-monotonic-v1');
  ...
  const deadline = ['r114-source-campaign', 'r114-auxiliary-campaign'].includes(name) ? 7200000 : 1800000;
  let timedOut = false;
  ...
  const record = {source_head: parent, source_map_sha256: hash(JSON.stringify(before)),
    ...
    clock: {contract: 'r114-raw-utc-boot-monotonic-v1', predecessor: previous, start, finish: result.finish, source_verified: verified, error: clockError}};
