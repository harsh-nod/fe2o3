...
PREFIX = "/home/harsh/fe2o3-xgmi-settled-20260918."
DEVICES = [
...
def settled_postflight(observe, name, failure=None, *, sleep=time.sleep):
...
def run_native(marker):
    ...
    need(
        ...
        "predeclared settled observation policy",
    )
    need(
        ...
        for depth in (1,):
            ...
            for backend in ("kfd", "hsa", "hip"):
                ...
                try:
                    ...
                    failure = error
                failure = settled_postflight(observe, name, failure)
                if failure is not None:
                    raise failure
    finally:
        check_identities()
        write_json(rec.output / "source-after.json", inventory(source))
    need(len(results) == 4, "complete four-row result roster")
