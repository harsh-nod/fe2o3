...
VERIFY_SHA = "8aacaff8fb1e8880dfa74067e8bf9d5f010add1c74039db09ae34e57d3951f4f"
...
def remove_cache(target, receipt_path, result_path, allocated, write_json):
    V.need(receipt_path != result_path, "distinct immutable cleanup receipts")
    V.need(not result_path.exists() and not result_path.is_symlink(), "unused final cleanup receipt")
    ...
    try:
        ...
        try:
            write_json(result_path, cleanup)
...
def main():
    ...
    cleanup = remove_cache(target, raw / "cleanup.json", raw / "cleanup-result.json", allocated, c.B.write_json)
