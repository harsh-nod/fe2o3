...
V = load(Path(__file__).with_name("verify.py"),
         "8aacaff8fb1e8880dfa74067e8bf9d5f010add1c74039db09ae34e57d3951f4f", "producer_launch_verify")
C = load(Path(__file__).with_name("collect.py"),
         "7d1c9ef12a62da30de7c2a43d823a77e0977ccc60a52df40f267ec42f61023e9", "producer_launch_collect")
...
class EvidenceTests(unittest.TestCase):
    ...
    def test_bootstrap_rejects_substituted_helpers_before_execution(self):
        ...
        for script, helper in (("verify.py", "runner.py"), ("collect.py", "verify.py"),
                               ("audit.py", "verify.py"), ("recover.py", "verify.py")):
    ...
    def test_cleanup_preserves_primary_failure_and_records_success(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            result = C.remove_cache(target, folder / "receipt", folder / "result", 4096, write)
            ...
            with patch.object(C.shutil, "rmtree", side_effect=primary):
                with self.assertRaises(OSError) as caught:
                    C.remove_cache(target, folder / "receipt", folder / "result", 4096, failing_write)
            ...
            with patch.object(C.shutil, "rmtree") as remove:
                ...
                with self.assertRaises(RuntimeError) as caught:
                    C.remove_cache(target, folder / "receipt", folder / "result", 4096, initial_failure)
                self.assertIs(caught.exception, secondary)
                remove.assert_not_called()
            self.assertTrue(target.is_dir())

    def test_immutable_writer_reproduces_original_failure_and_checks_fix(self):
        path = Path(__file__).resolve().parent / "raw/collection-failure/collect.py"
        raw = path.read_bytes()
        self.assertEqual(hashlib.sha256(raw).hexdigest(), V.FAILED_COLLECTION["collect.py"])
        tree = ast.parse(raw, str(path))
        body = [node for node in tree.body if isinstance(node, ast.FunctionDef) and node.name == "remove_cache"]
        self.assertEqual(len(body), 1)
        namespace = {"shutil": shutil, "V": V}
        exec(compile(ast.Module(body=body, type_ignores=[]), str(path), "exec"), namespace)
        writer = C.R.helpers().B.write_json
        with tempfile.TemporaryDirectory() as temporary:
            folder = Path(temporary)
            old_target = folder / "old-target"
            old_target.mkdir()
            old_receipt = folder / "old-receipt.json"
            with self.assertRaises(FileExistsError):
                namespace["remove_cache"](old_target, old_receipt, 4096, writer)
            self.assertFalse(old_target.exists())
            self.assertIs(V.read(old_receipt)["absent"], False)
            target = folder / "target"
            target.mkdir()
            initial, final = folder / "initial.json", folder / "final.json"
            C.remove_cache(target, initial, final, 4096, writer)
            self.assertIs(V.read(initial)["absent"], False)
            self.assertIs(V.read(final)["absent"], True)
            self.assertFalse(target.exists())
            target.mkdir()
            with self.assertRaisesRegex(RuntimeError, "unused final"), patch.object(C.shutil, "rmtree") as remove:
                C.remove_cache(target, folder / "unused.json", final, 4096, writer)
            remove.assert_not_called()
            self.assertTrue(target.is_dir())
            self.assertFalse((folder / "unused.json").exists())

    def test_cleanup_recovery_cannot_replace_the_failed_attempt(self):
        initial = {"path": str(V.R.PRIVATE / "target"), "allocated_bytes": 4096, "absent": False}
        final = initial | {"absent": True}
        recovery = {
            "cache_path": initial["path"], "initial_receipt_sha256": V.INITIAL_CLEANUP_SHA,
        ...
        mutations = (
            (0, "absent", True), (0, "allocated_bytes", 0), (0, "path", "/tmp/foreign"),
            (1, "absent", False), (1, "allocated_bytes", 4096.0), (1, "extra", "unrecorded"),
            (2, "deletion_repeated", True), (2, "command_groups_absent", False),
            (2, "initial_receipt_sha256", "0" * 64), (2, "original_collection", "pass"),
        )
        for index, key, value in mutations:
            rows = copy.deepcopy([initial, final, recovery])
            rows[index][key] = value
            with self.subTest(index=index, key=key), self.assertRaises(RuntimeError):
                V.cleanup_recovery(*rows)
