...
def main():
    ...
    need({path.name for path in raw.iterdir()} == {"iterations", "campaign1", "campaign1.log", "cleanup.json"},
    ...
    need(set(cleanup) == {"path", "allocated_bytes", "absent"} and cleanup["path"] == str(target)
         and type(cleanup["allocated_bytes"]) is int and cleanup["allocated_bytes"] > 0
         and cleanup["absent"] is True, "owned cache cleanup receipt")
