...
fn reader_guards_live_exercise_v1(actual: &mut ContextProducerReadJournalV1, model: &mut logical::ProducerReadContentsV1,
    ...
    requires producer_represents(*old(actual), *old(model)),
        ...
        begin_preflight_decision_v1(old(actual).stable.journal, writer, roster@) == Ok(0),
        logical::begin_issued_decision_v1(*old(model), model_writer, model_roster@) == Ok(()),
        producer_unread_writes_v1(*old(actual), roster@, 0) == Ok(()),
        stable_unread_writes_v1(old(actual).stable, roster@, 0) == Ok(()),
        ...
        old(actual).stable.journal.writers@.len() == 2, old(actual).stable.journal.allocations@.len() == 3,
        old(actual).stable.journal.allocations@[1].is_some(), old(actual).stable.journal.allocations@[1].unwrap().attempt_epoch == 0,
        old(actual).stable.journal.members@.len() == 3,
        old(actual).stable.journal.member_free@ == seq![2usize, 1],
    ensures result,
{
    proof {
    ...
    let result = producer_begin_historical_exec_v1(actual, model, writer, model_writer,
        roster, model_roster, Ghost(storage), Ghost(history));
    assert(result.0 == Ok(()) && result.1 == Ok(()));
    assert(actual.stable.journal.allocations@[1].unwrap().attempt_epoch == 1);
    assert(actual.stable.journal.allocations@[1].unwrap().pending_member == Some(1));
    assert(actual.stable.journal.members@[1].unwrap().writer == writer);
    assert(actual.stable.journal.allocations@[0] == before.stable.journal.allocations@[0]);
    assert(actual.stable.journal.members@[0] == before.stable.journal.members@[0] && actual.stable.journal.writers@[0] == before.stable.journal.writers@[0]);
    ...
    let replay = producer_begin_historical_exec_v1(actual, model, writer, model_writer,
    ...
    // Combined unread rejection precedes even an invalid writer, with each owner executing its own guard.
    ...
    let blocked_result = producer_begin_historical_exec_v1(actual, model, bad_writer, model_bad_writer,
        &blocked, &model_blocked, Ghost(storage), Ghost(history));
    assert(blocked_result.0 == Err(ReadErrorV1::AllocationBusy) && blocked_result.1 == Err(logical::ReadErrorV1::AllocationBusy));
...
#[verifier::spinoff_prover]
fn reader_guards_live_witness_v1() -> (result: bool)
    ensures result,
{
    ...
    let journal = JournalContentsV1 { context_generation: 7, allocation_capacity: 3, writer_capacity: 2,
    ...
    let actual_reservation = ReservationV1 {
        reference: ContextProducerReadReferenceV1 { slot: 0, incarnation: 1, consumer: writer.key },
        request: ContextProducerReadV1 { producer, read: ContextAllocationReadV1 {
            allocation, device: protected.device, byte_extent: 16, byte_offset: 0, byte_len: 8,
            attempt_epoch: 1, content_lineage: 0 } },
    };
    let mut actual = ContextProducerReadJournalV1 {
        stable: ContextReadLeasedJournalV1 { journal, leases: vec![None, None], free_reads: vec![1usize, 0],
            readers: vec![0usize, 0, 0], next_incarnation: 1 },
        reservations: vec![Some(actual_reservation), None], free: vec![1usize], counts: vec![1usize, 0, 0],
        next_incarnation: 2,
    };
    ...
    proof {
        ...
        assert(journal_view(actual.stable.journal).free =~= model.stable.journal.free@);
        assert(journal_view(actual.stable.journal).allocation_free =~= model.stable.journal.allocation_free@);
        assert(journal_view(actual.stable.journal).member_free =~= model.stable.journal.member_free@);
        ...
        assert(stable_represents(actual.stable, model.stable));
        assert(producer_represents(actual, model));
    }
    assert(logical::producer_status_v1(model.stable.journal, request) == Some(logical::ProducerStatusV1::Pending));
    ...
    proof {
        ...
        begin_preflight_correspondence(actual.stable.journal, model.stable.journal, writer, roster@);
        producer_unread_correspondence(actual, model, roster@, 0);
        stable_unread_correspondence(actual.stable, model.stable, roster@, 0);
    }
    reader_guards_live_exercise_v1(&mut actual, &mut model, writer, model_writer, &roster, &model_roster,
