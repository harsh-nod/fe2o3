...
def render_results(packet):
    ...
    performance = read_module(packet / 'performance.py', 'guards_render_performance')
    lines = [
...
def audit(repo, packet):
    ...
    need(not git(repo, 'diff', '--name-only', 'e8134e770a30d12aa4abd190bd29c5083ea5b3b5', source, '--',
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-audit-') as temporary:
        ...
        for name, command in cpu.commands():
            ...
            if environment is None:
                ...
                need(target.is_absolute() and target.name == 'target' and target.parent.name.startswith('fe2o3-begin-guards-'), 'owned CPU target')
            ...
            if name == 'test':
                need(test_results(out) == [(887, 0, 12, 0, 0), (27, 0, 0, 0, 0)], 'complete unit/doctest result roster')
            if name == 'performance':
                need(test_results(out) == [(2, 0, 0, 0, 897)], 'exact two release benchmark results')
    ...
    for name, relative, addition in modifications:
        with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-negative-') as temporary:
            ...
            if addition is not None:
                ...
                index = next(i for i, line in enumerate(rows) if 'begin_guards,' in line)
                del rows[index]
...
def main():
    ...
    if args.selftest:
        ...
        print('PASS: standalone actual Begin guard proof and instrumented CPU packet reconstructed from Git objects')
