#!/usr/bin/env python3
"""Actual stable-reader release correspondence campaign with scoped controls."""
import argparse
import hashlib
...
PACKET = Path('docs/evidence/dev-stable-release-2026-09-22')
ROOT = CRATE / 'verus/context_stable_release_historical_v1.rs'
EXECUTION_ROOT = CRATE / 'verus/context_stable_release_execution_v1.rs'
RAW_ROOT = CRATE / 'verus/context_journal_begin_execution_v1.rs'
VIEWS = CRATE / 'verus/context_reader_owner_views_v1.rs'
BRIDGE = CRATE / 'verus/context_stable_release_correspondence_v1.rs'
LIVE = CRATE / 'verus/context_stable_release_witnesses_v1.rs'
DECISIONS = CRATE / 'verus/context_stable_release_decisions_v1.rs'
PROOF = CRATE / 'verus/context_stable_release_bodies_v1.rs'
BODY = CRATE / 'src/context_read_leases/release_bodies.rs'
...
PROJECTIONS = [(BEGIN, 'audited-begin-bodies.rs', 8),
               ...
               (BODY, 'audited-release-bodies.rs', 8)]
...
MUTATIONS = [
    ("reference_incarnation", BODY, "macro_rules! stable_read_reference_same_body {", "production", "stable_read_reference_same_exec_v1", "$left.incarnation == $right.incarnation", "true", "execution"),
    ("reference_consumer", BODY, "macro_rules! stable_read_reference_same_body {", "production", "stable_read_reference_same_exec_v1", "stable_read_consumer_same_exec_v1($left.consumer, $right.consumer)", "true", "execution"),
    ("lookup_validation_error", BODY, "macro_rules! stable_lease_lookup_body {", "production", "ContextReadLeasedJournalV1::lookup_read", "Err(error) => Err(error)", "Err(_error) => Ok(entry.request)", "execution"),
    ("evidence_before_empty", BODY, "macro_rules! stable_release_header_body {", "production", "stable_release_header_exec_v1", "if !stable_read_consumer_same_exec_v1($evidence, $consumer)", "if $count != 0 && !stable_read_consumer_same_exec_v1($evidence, $consumer)", "execution"),
    ("capacity_equality", BODY, "macro_rules! stable_release_header_body {", "production", "stable_release_header_exec_v1", "if count > $observe_capacity", "if count >= $observe_capacity", "execution"),
    ("arena_headroom", BODY, "macro_rules! stable_release_header_body {", "production", "stable_release_header_exec_v1", "if count > $contents.leases.len()", "if false", "execution"),
    ("incarnation_order", BODY, "macro_rules! stable_release_order_less_body {", "production", "stable_release_order_less_exec_v1", "$left.3 < $right.3", "$left.3 <= $right.3", "execution"),
    ("group_count", BODY, "macro_rules! stable_release_item_body {", "production", "stable_release_item_exec_v1", "group = $state.group + 1;", "group = 1;", "execution"),
    ("take_clear", BODY, "macro_rules! stable_release_commit_body {", "production", "stable_release_commit_exec_v1", ".take().expect(\"preflighted lease\")", ".expect(\"preflighted lease\")", "execution"),
    ("reader_decrement", BODY, "macro_rules! stable_release_commit_body {", "production", "stable_release_commit_exec_v1", "$contents.readers[$allocation] -= 1;", "$contents.readers[$allocation] -= 0;", "execution"),
    ("free_push", BODY, "macro_rules! stable_release_commit_body {", "production", "stable_release_commit_exec_v1", "$contents.free_reads.push($reference.slot);", "$contents.free_reads.push(0);", "execution"),
    ("reject_mutates_owner", BODY, "macro_rules! stable_release_execution_body {", "production", "ContextReadLeasedJournalV1::release_reads_observed_v1", "Err(error) => return Err(error),", "Err(error) => { $contents.next_incarnation = 0; return Err(error); },", "execution"),
    ("reject_frame", DECISIONS, "fn stable_release_execution_relation_v1(", "production", "stable_release_paired_transition", "Err(_) => after == before,", "Err(_) => true,", "contract"),
    ("skip_actual", BRIDGE, "fn stable_release_historical_exec_v1(", "production", "stable_release_historical_exec_v1", "let result = actual.release_reads_observed_v1(consumer, references, evidence, observed_free_capacity);", "let result: Result<(), ReadErrorV1> = Ok(());", "execution"),
    ("skip_historical", BRIDGE, "fn stable_release_historical_exec_v1(", "production", "stable_release_historical_exec_v1", "let model_result = logical::release_verified_v1(model, model_consumer, model_references, model_evidence, observed_free_capacity);", "let model_result: Result<(), logical::ReadErrorV1> = Ok(());", "execution"),
    ("roster_projection", BRIDGE, "fn stable_references_view(", "production", "stable_release_decision_correspondence", "references.map(|_i, reference| read_reference_view(reference))", "Seq::<logical::ReadReferenceV1>::empty()", "projection"),
    ("scan_projection", BRIDGE, "fn stable_release_scan_view(", "production", "stable_release_item_correspondence", "group: state.group", "group: 0", "projection"),
    ("short_capacity_witness", LIVE, "fn stable_release_live_witness_v1(", "production", "stable_release_live_witness_v1", "&refs, &model_refs, &evidence, model_consumer, 1);", "&refs, &model_refs, &evidence, model_consumer, 4);", "witness"),
    ("late_undercount_witness", LIVE, "fn stable_release_late_undercount_witness_v1(", "production", "stable_release_late_undercount_witness_v1", "actual.readers.set(0, 1);", "actual.readers.set(0, 2);", "witness"),
    ("raw_prefix_witness", LIVE, "fn stable_release_raw_prefix_witness_v1(", "production", "stable_release_raw_prefix_witness_v1", "actual.free_reads.set(1, 0);", "actual.free_reads.set(1, 1);", "witness"),
]
...
def check_result(base, status, stdout, stderr, folder, mutation, expected):
    ...
    if mutation:
        ...
        need(same(observed['result'], {
            ...
            'verified': 522, 'errors': 0, 'is-verifying-entire-crate': True,
