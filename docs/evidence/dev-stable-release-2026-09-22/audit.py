...
def render_results(packet):
    ...
    performance = read_module(packet / 'performance.py', 'release_render_performance')
    lines = [
...
def audit(repo, packet):
    ...
    need(not git(repo, 'diff', '--name-only', '455d3c62a03f1033b5c63e40634592a583e42484', source, '--',
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-audit-') as temporary:
        ...
        for name, command in cpu.commands():
            ...
            if environment is None:
                ...
                need(target.is_absolute() and target.name == 'target' and target.parent.name.startswith('fe2o3-stable-release-'), 'owned CPU target')
            ...
            if name == 'test':
                need(test_results(out) == [(901, 0, 14, 0, 0), (27, 0, 0, 0, 0)], 'complete unit/doctest result roster')
            if name == 'performance':
                need(test_results(out) == [(1, 0, 0, 0, 914)], 'exact release release benchmark result')
    ...
    for name, relative, addition in modifications:
        with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-negative-') as temporary:
            ...
            if addition is not None:
                ...
                key = next(key for key in row if key.endswith('/context_read_leases/release.rs'))
                row[key] = '0' * 64
                ...
                index = next(i for i, line in enumerate(rows) if 'stable_release,' in line)
                del rows[index]
...
def main():
    ...
    if args.selftest:
        ...
        print('PASS: standalone actual stable release proof and instrumented CPU packet reconstructed from Git objects')
