// Capacity is observed only after the original early-return header prefix.
 stable_read_reference_same_body {
 ...
 stable_release_order_less_body {
    ($left:ident, $right:ident) => {
     ...
     [$($selected:tt)*], [$($count:tt)*], [$($step:tt)*]) => {
        $syntax!({
                ...
                $($invariant)*
            {
                ...
                $($selected)*
                let $entry = $contents.leases[$reference.slot].expect("preflighted lease");
