...
def render_results(packet):
    ...
    performance = read_module(packet / 'performance.py', 'stable_render_performance')
    lines = [
...
def audit(repo, packet):
    ...
    need(not git(repo, 'diff', '--name-only', '30aac7720c4a2eef34da3e3682ec5b74784ed61d', source, '--',
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-audit-') as temporary:
        ...
        for name, command in cpu.commands():
            ...
            if environment is None:
                ...
                need(target.is_absolute() and target.name == 'target' and target.parent.name.startswith('fe2o3-producer-stable-'), 'owned CPU target')
            ...
            if name == 'test':
                need(test_results(out) == [(930, 0, 18, 0, 0), (27, 0, 0, 0, 0)], 'complete unit/doctest result roster')
            if name == 'performance':
                need(test_results(out) == [(1, 0, 0, 0, 947)], 'exact release producer-release benchmark result')
    ...
    for name, relative, addition in modifications:
        with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-negative-') as temporary:
            ...
            if addition is not None:
                ...
                key = next(key for key in row if key.endswith('/context_producer_reads/stable_wrappers.rs'))
                row[key] = '0' * 64
                ...
                index = next(i for i, line in enumerate(rows) if 'producer_stable_' in line)
                del rows[index]
...
def main():
    ...
    if args.selftest:
        ...
        print('PASS: standalone actual producer stable-wrapper proof and instrumented CPU packet reconstructed from Git objects')
