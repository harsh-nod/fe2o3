...
const environment = {
  ...
  runners: ['r110-run.js', 'r110-campaign-run.js', 'r110-source-gate.py', 'r110-auxiliary-gates.py', 'r110-freeze.js']
  ...
  campaign_envelope: {previous_deadline_ms: 1800000, deadline_ms: 7200000, individual_gate_deadline_ms: 1800000, reason: 'Expanded CPU test matrices increase the total of seventeen serialized gates; no individual test or gate deadline is weakened.'},
  ambient_stack: {RUST_MIN_STACK: process.env.RUST_MIN_STACK ?? null,
    command: ['bash', '-c', 'ulimit -s'], output: execute(['bash', '-c', 'ulimit -s'])},
  prior_artifacts: fs.readdirSync(root).filter(name => /^r110-.*\.(json|log)$/.test(name)).sort()
    .map(name => ({name, sha256: hash(fs.readFileSync(path.join(root, name)))})),
};
const previous = JSON.parse(fs.readFileSync(path.join(repo,
  'docs/evidence/local-r109-live-device-insertion-2026-09-12/raw/r109-environment.json')));
...
for (const [name, value] of [['frozen-source', frozen], ['environment', environment], ['issue-status', issue]]) {
  fs.writeFileSync(path.join(root, 'r110-' + name + '.json'), JSON.stringify(value, null, 2) + '\n', {flag: 'wx'});
}
