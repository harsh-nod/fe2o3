...
const clock = require('./r110-exact-clock-evidence.js');
const p = require('./r110-exact-qualification-plan.js');
const root = '/home/harsh/.codex-tmp';
const repo = path.join(root, 'fe2o3-r61-execution');
const dest = path.join(repo, 'docs/evidence/local-r110-live-coherent-insertion-2026-09-12');
...
const environment = read('r110-environment.json');
...
assert.deepStrictEqual(environment.binaries, JSON.parse(committed('raw/r109-environment.json')).binaries);
...
assert.deepStrictEqual(environment.runners.map(r => r.name),
  ['r110-run.js', 'r110-campaign-run.js', 'r110-source-gate.py', 'r110-auxiliary-gates.py', 'r110-freeze.js']);
...
assert.deepStrictEqual(environment.campaign_envelope, {previous_deadline_ms: 1800000, deadline_ms: 7200000,
  individual_gate_deadline_ms: 1800000, reason: 'Expanded CPU test matrices increase the total of seventeen serialized gates; no individual test or gate deadline is weakened.'});
const preliminary = ['initial-format', 'initial-build', 'coherent-build', 'oracle-build', 'oracle-tests',
  'matrix-format', 'matrix-tests', 'matrix-fixed-tests', 'review-format', 'review-clippy', 'clean-clippy',
  'reviewed-tests', 'public-preflight-tests', 'prefreeze-format', 'final-format'];
assert.deepStrictEqual(environment.prior_artifacts.map(r => r.name), preliminary.flatMap(name =>
  ['.json', '.log', '-source.json'].map(suffix => 'r110-' + name + suffix)).sort());
...
for (const name of preliminary) {
  const run = read('r110-' + name + '.json');
  const rejected = ['coherent-build', 'matrix-tests', 'matrix-fixed-tests', 'review-clippy', 'reviewed-tests', 'prefreeze-format'].includes(name);
  assert.strictEqual(run.returncode, name === 'prefreeze-format' ? 1 : rejected ? 101 : 0);
  assert.strictEqual(run.signal, null);
  assert.strictEqual(run.timed_out, false);
  assert.strictEqual(run.source_head, p.parent);
  assert.strictEqual(run.source_unchanged, !['initial-format', 'matrix-format', 'review-format', 'final-format'].includes(name));
...
for (const name of ['coherent-build', 'matrix-tests']) assert(output(read('r110-' + name + '.json')).includes('error[E'));
for (const marker of ['forget_non_drop', 'type_complexity', 'unnecessary_unwrap']) assert(output(read('r110-review-clippy.json')).includes(marker));
assert.deepStrictEqual(totals(output(read('r110-matrix-fixed-tests.json'))),
  {harnesses: 1, passed: 14, failed: 2, ignored: 0});
assert.deepStrictEqual(totals(output(read('r110-reviewed-tests.json'))),
  {harnesses: 1, passed: 18, failed: 1, ignored: 0});
const publicPreflight = p.newTests.find(n => n.endsWith('all_public_entrypoints_preserve_preflight_precedence'));
assert.deepStrictEqual(failed(output(read('r110-reviewed-tests.json'))), [publicPreflight]);
assert.deepStrictEqual(passing(output(read('r110-public-preflight-tests.json'))), [publicPreflight]);
assert.deepStrictEqual(passing(output(read('r110-reviewed-tests.json'))), p.newTests.filter(n => n !== publicPreflight));
const issue = read('r110-issue-status.json');
...
const manifest = read('r110-exact-clock-manifest.json');
...
const historicalNames = fs.readdirSync(root).filter(n => /^r110-.*\.(json|log|py|js)$/.test(n) && !n.startsWith('r110-exact-clock-')).sort();
...
const mutations = read('r110-exact-mutations.json');
...
const plannedNames = mutations.flatMap(m => ['r110-exact-clock-mut-' + m.name + '.json', 'r110-exact-clock-restoration-' + m.name + '.json'])
  .concat(p.focused.map(([name]) => 'r110-exact-clock-restored-' + name + '.json'), ['r110-exact-clock-collector-validation.json']);
assert.deepStrictEqual(Object.keys(manifest.entries), plannedNames);
let expectedPredecessor = {name: 'r110-exact-clock-manifest.json', field: 'recorded_at'};
for (const name of plannedNames) {
  ...
  const restoration = name.startsWith('r110-exact-clock-restoration-');
...
function checkClock(name, record, restoration = false) {
  ...
  assert.strictEqual(record.manifest_sha256, hash(fs.readFileSync(file('r110-exact-clock-manifest.json'))));
...
const gates = read('r110-final-source-gate.json');
const auxiliary = read('r110-auxiliary-results.json');
for (const [rows, oldName, count, prefix, logPrefix, oldPrefix] of [
  [gates, 'raw/r109-final-source-gate.json', 17, 'r110-final', 'r110-final', 'r109-final'],
  [auxiliary, 'raw/r109-auxiliary-results.json', 10, 'r110-auxiliary', 'r110', 'r109'],
]) {
  ...
  for (const [index, row] of rows.entries()) {
    assert.deepStrictEqual(row.command, old[index].command.map(s => s.replace('r109-production-metadata.log', 'r110-production-metadata.log')), row.name);
    ...
    if (row.name === 'production-metadata') {
      assert.strictEqual(row.stderr_log, file('r110-production-metadata.stderr.log'));
...
for (const target of ['gnu', 'musl']) {
  ...
  const old = committed('raw/r109-final-' + target + '-tests.log');
  ...
  assert.deepStrictEqual(expected, {harnesses: 48, passed: 2638, failed: 0, ignored: 5});
  full[target] = expected;
}
const sourceCampaign = checkRun('r110-source-campaign', ['python3', '-B', file('r110-source-gate.py'), 'r110-final']);
const auxCampaign = checkRun('r110-auxiliary-campaign', ['python3', '-B', file('r110-auxiliary-gates.py')]);
...
const clockTests = checkRun('r110-exact-clock-contract-tests', ['node', file('r110-exact-clock-evidence-tests.js')]);
assert.deepStrictEqual(manifest.contract_tests.map(r => r.name),
  ['r110-exact-clock-contract-tests.json', 'r110-exact-clock-contract-tests.log', 'r110-exact-clock-contract-tests-source.json']);
...
const oldNames = passing(committed('raw/r109-final-gnu-tests.log'));
...
for (const [kind, filter, count] of p.focused) for (const phase of ['frozen', 'restored']) {
  ...
  const run = checkRun((phase === 'restored' ? 'r110-exact-clock-' : 'r110-') + phase + '-' + kind, [...p.tests, filter]);
...
for (const mutation of mutations) {
  ...
  const name = 'r110-exact-clock-mut-' + mutation.name;
  ...
  const restored = read('r110-exact-clock-restoration-' + mutation.name + '.json');
  checkClock('r110-exact-clock-restoration-' + mutation.name, restored, true);
...
const summary = {
  scope: 'N3-L2 initialized-coherent insertion at named CPU/shared-sequencer and concrete-facade boundaries; no formal, native or performance qualification',
  ...
  dynamic_new_tests: p.newTests.filter(n => !n.endsWith('production_routing_preserves_root_and_terminal_owner')),
  ...
  accepted_mutation_cohort: 'r110-exact-clock-mut-',
  limits: ['constructed native leaves are scripted, not Linux GPU execution',
    ...
    'uninitialized insertion, cleanup custody, generated adoption and the remaining roadmap stay open'],
};
const validationText = JSON.stringify(summary, null, 2) + '\n'
  + 'PASS: full/focused/auxiliary rosters, 10 compiled negatives, exact restoration and chronology\n';
if (process.argv[2] === '--check') {
  process.stdout.write(validationText);
  process.exit(0);
}
assert.strictEqual(process.argv.length, 2);
const validation = checkRun('r110-exact-clock-collector-validation', ['node', file('r110-exact-clock-retain-local.js'), '--check']);
...
const names = fs.readdirSync(root).filter(n => /^r110-.*\.(json|log|py|js)$/.test(n)).sort();
