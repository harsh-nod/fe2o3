...
const contract = 'r110-exact-observed-utc-floor-v1';
...
function predecessor(name, field, expected, io = fs) {
  assert(/^r110-exact-clock-[a-z0-9-]+\.json$/.test(name), 'clock cohort predecessor');
  ...
  if (expected.kind === 'run') {
    ...
    assert.strictEqual(record.source_unchanged, true);
    assert.strictEqual(record.source_identities, 5675);
  }
...
function plan(name) {
  const manifestName = 'r110-exact-clock-manifest.json';
