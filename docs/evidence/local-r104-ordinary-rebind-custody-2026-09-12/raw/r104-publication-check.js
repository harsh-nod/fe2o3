...
const evidence = 'docs/evidence/local-r104-ordinary-rebind-custody-2026-09-12';
const root = path.join(repo, evidence);
const mode = process.argv[2];
assert(['manifest', 'staged'].includes(mode));
const git = args => cp.execFileSync('git', args, {cwd: repo, maxBuffer: 32 * 1024 * 1024});
assert.strictEqual(git(['rev-parse', 'HEAD']).toString().trim(),
  '5cdeedd8290fac0bf01ca53b01cc12e828a2e20e');
const hash = bytes => crypto.createHash('sha256').update(bytes).digest('hex');
const baseline = JSON.parse(fs.readFileSync(path.join(root, 'raw/r104-frozen-source.json')));
...
const docs = ['docs/runtime-a1-a2-swarm-current.md', 'docs/runtime-a1-a2-next-wave.md',
  'docs/runtime-primary-queue-construction-custody-v1.md',
  'docs/runtime-persistent-bind-settlement-v1.md',
  'docs/runtime-ordinary-rebind-custody-v1.md', evidence + '/README.md'];
let links = 0;
...
if (mode === 'staged') {
  ...
  const expected = [...new Set([...summary.changed_source_files, ...docs,
