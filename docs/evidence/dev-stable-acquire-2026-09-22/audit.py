...
def render_results(packet):
    ...
    performance = read_module(packet / 'performance.py', 'acquire_render_performance')
    lines = [
...
def audit(repo, packet):
    ...
    need(not git(repo, 'diff', '--name-only', '72bcb1da989ea79c2c48085f5a6c9ba3ce1633f1', source, '--',
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-audit-') as temporary:
        ...
        for name, command in cpu.commands():
            ...
            if environment is None:
                ...
                need(target.is_absolute() and target.name == 'target' and target.parent.name.startswith('fe2o3-stable-acquire-'), 'owned CPU target')
            ...
            if name == 'test':
                need(test_results(out) == [(893, 0, 13, 0, 0), (27, 0, 0, 0, 0)], 'complete unit/doctest result roster')
            if name == 'performance':
                need(test_results(out) == [(1, 0, 0, 0, 905)], 'exact release acquisition benchmark result')
    ...
    for name, relative, addition in modifications:
        with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-negative-') as temporary:
            ...
            if addition is not None:
                ...
                key = next(key for key in row if key.endswith('/context_read_leases/acquire.rs'))
                row[key] = '0' * 64
                ...
                index = next(i for i, line in enumerate(rows) if 'stable_acquire,' in line)
                del rows[index]
...
def main():
    ...
    if args.selftest:
        ...
        print('PASS: standalone actual stable acquisition proof and instrumented CPU packet reconstructed from Git objects')
