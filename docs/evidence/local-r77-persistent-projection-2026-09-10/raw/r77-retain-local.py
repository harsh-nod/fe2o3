"""Retain exact Persistent projection source gates, not native or proof acceptance."""
import hashlib
import json
...
root = repo / 'docs/evidence/local-r77-persistent-projection-2026-09-10'
raw = root / 'raw'
attempt = sys.argv[1]
...
assert base == 'e43e6a98dfc625374f8badee24174e2bcb8ebd95'
...
inventory = (temporary / 'r77-proof-inventory.log').read_text()
assert 'PASS:' in inventory and 'FAIL:' not in inventory
metadata = (temporary / 'r77-production-audit.log').read_text()
...
summary.update(base_commit=base, final_gate_attempt=attempt,
    ...
    production_metadata_packages=43, new_cpu_test_functions=12,
    new_runtime_compile_fail_doctests=2, new_host_negative_binaries=0,
    new_host_compile_only_examples=0, nonexecuting_projection_validated=True,
...
files = list(temporary.glob('r77-*.log'))
files += list(temporary.glob('r77-*-source-gate.json'))
files += list(temporary.glob('r77-*-source-inputs.json'))
files += [temporary / name for name in ('r77-source-gate.py', 'r77-retain-local.py',
    'r77-production-metadata.json', 'r77-auxiliary-gates.py', 'r77-doc-links.py')]
for path in files:
