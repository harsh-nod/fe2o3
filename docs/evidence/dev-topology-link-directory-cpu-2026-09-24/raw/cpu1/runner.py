...
def helpers():
    ...
    if sha(path) != "a6b2f42beb687b82bc9f8207acc034df6003c598eb7dc8308447024ef421f940":
...
def main():
    ...
    old = json.loads(PRIOR.read_bytes())["source"]
    ...
    c.B.write_json(output / "prior-delta.json", {"prior_sha256": sha(PRIOR), "changed": sorted(changed)})
