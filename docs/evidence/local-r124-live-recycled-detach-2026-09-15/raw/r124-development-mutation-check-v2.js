...
const checker = root + 'r124-development-check-v2.js';
const checkerHash = '8fd3e10f8240bb49fae000a1b6b1aa2340704df74399ddfea6878ca57459973d';
...
const p = JSON.parse(C.pin('r124-development-mutations-v2.json', '71756d631b65e715b7202c7dcc82a80c4456b958c70d6480d594ff3b3f7ec460'));
...
assert.strictEqual(p.mutations.length, 19);
for (const [index, mutation] of p.mutations.entries())
  assert.strictEqual(mutation.predecessor, index === 0 ? 'r124-development-gate-check-all-v2.json'
    : 'r124-development-restored-' + p.mutations[index - 1].id + '-v1.json');
assert.strictEqual(new Set(p.mutations.map(mutation => mutation.expected_source_map_sha256)).size, 19);
...
function negative(mutation) {
  ...
  C.baseline();
  const name = 'r124-development-negative-' + mutation.id + '-v1';
