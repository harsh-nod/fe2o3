...
const helper = '/home/harsh/.codex-tmp/r124-development-gate-check-v1.js';
const helperHash = '29276457e534266a342fa7e2bc577286f4ec2d11f9f91fd3ebb88f4d1f5a12aa';
assert.strictEqual(crypto.createHash('sha256').update(fs.readFileSync(helper)).digest('hex'), helperHash);
const G = require(helper);
const {C, gates, accepted, full, leaf, relocate, linux, linuxNames, fullGnu} = G;
...
const correctedLinux = replaceOne(currentLinux, '0 measured; 1189 filtered out;', '0 measured; 1206 filtered out;');
...
for (const relocation of gates.doc_relocations) {
  const name = E.passing(relocated).find(n => n.startsWith(relocation.path + ' - ') &&
    n.includes('(line ' + relocation.to + ')'));
  assert(name, 'declared R124 doctest');
  bad('GNU stale doctest ' + relocation.path + ':' + relocation.to, () => leaf(docsSpec, replaceOne(relocated,
    name + ' ... ok', name.replace('(line ' + relocation.to + ')', '(line ' + relocation.from + ')') + ' ... ok')));
  bad('GNU missing doctest ' + relocation.path + ':' + relocation.to, () => leaf(docsSpec, replaceOne(relocated,
    'test ' + name + ' ... ok\n', '')));
}
...
assert.strictEqual(acceptedLockLog.split(waitLine).length, 1, 'accepted R123 has no cache-wait row');
...
assert.strictEqual(cases.length, 43);
