...
const q = JSON.parse(initialPin('r124-development-qualification-plan-v2.json',
...
const readmissionReport = C.readmission();
assert.deepStrictEqual(q.resumption, p.resumption);
assert.strictEqual(q.retained_helper_calibrations, 175);
for (const spec of q.historical_bootstrap) record(spec);
...
const anchorCheck = json('r124-development-anchor-check-01.json');
let admission = null;
for (const spec of q.bootstrap) {
  const result = record(spec), report = JSON.parse(result.log);
  if (spec.cases) calibration(spec, result);
  else {
    assert.strictEqual(spec.name, p.resumption.admission.name);
    assert.deepStrictEqual(report, readmissionReport);
...
for (const spec of G.gates.full_runs) {
  ...
  if (spec.kind === 'musl') {
    assert(admission, 'independent current-boot admission');
    E.ordered(admission.clock.source_verified, result.record.clock.start);
  }
}
...
assert.strictEqual(calibrationCount, 197);
...
console.log(JSON.stringify({development_only: true, publication_pending: true, scope: q.scope,
  ...
  helper_calibrations: 197, retained_helper_calibrations: 175,
  execution_boots: {historical: C.resumption.prior_boot, current: C.resumption.current_boot},
  interrupted_run: {name: C.resumption.interrupted, qualification: false}, restored_suites: [158], runs, mutations, raw_artifacts: raw,
  retained_other_artifacts: inventory.filter(name => !qualified.has(name)), historical_runs: historicalRuns,
  owned_group_observations: [...groups.values()].sort((a, b) => a.boot_id.localeCompare(b.boot_id) || a.pgid - b.pgid),
