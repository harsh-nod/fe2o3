// R119 transcript parsers remain pinned; only the R124 record policy is adapted.
...
function checkRecord(r, name, command, expectedMap, previousName, code, io, context) {
  assert(/^r124-development-[a-z0-9-]+$/.test(name), 'R124 record namespace');
  ...
  const full = ['r124-development-gnu-all-v1', 'r124-development-musl-all-v1', 'r124-development-musl-all-v2'].includes(name);
  const campaign = ['r124-development-source-campaign', 'r124-development-auxiliary-campaign'].includes(name);
...
// Historical numeric PGIDs may have been reassigned after reboot.
function observeClosure(record, policy, scan = E.liveGroupMembers) {
  const start = record.clock.start, finish = record.clock.finish;
  const close = record.process_group_cleanup.close;
  ordered(start, finish); ordered(finish, close.observation);
  ordered(close.observation, record.clock.source_verified);
  assert.strictEqual(record.child_closed, true);
  assert(['absent', 'signaled'].includes(close.status));
  assert(Number.isSafeInteger(close.pgid) && close.pgid > 1);
  assert.deepStrictEqual(close.live_members, []);
  assert.strictEqual(policy.current_boot, policy.admitted_boot, 'current admitted boot');
  assert.notStrictEqual(policy.historical_boot, policy.admitted_boot);
  if (start.boot_id === policy.current_boot) {
    assert.deepStrictEqual(scan(close.pgid), [], 'current-boot group absent');
    return {boot_id: start.boot_id, pgid: close.pgid, closure: 'current_boot_observed_absent'};
  }
  assert.strictEqual(start.boot_id, policy.historical_boot, 'declared historical boot');
  assert(typeof policy.historical_sha256 === 'string' && /^[0-9a-f]{64}$/.test(policy.historical_sha256),
    'predeclared historical record');
  assert.strictEqual(policy.record_sha256, policy.historical_sha256, 'exact historical record bytes');
  return {boot_id: start.boot_id, pgid: close.pgid, closure: 'historical_recorded_closure'};
}

module.exports = {...E, checkRecord, observeClosure};
