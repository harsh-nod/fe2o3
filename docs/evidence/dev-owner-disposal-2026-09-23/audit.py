#!/usr/bin/env python3
"""Audit source-bound developer disposal records; does not rerun Verus."""
import argparse
import hashlib
...
SOURCE = '2cfff18d91c1e0b182adc974bceee67b45c1bd05'
CHECK = Path('crates/fe2o3-runtime-model/verus/check-owner-disposal.py')
DOCUMENT = Path('docs/runtime-producer-read-reservations-v1.md')
DOCUMENT_SHA256 = '902ad997745fcc8f0447711dbff76bcdc05d71780cbafbfa246bf9d374f3a7d2'
SOURCE_COUNT = 405
NEGATIVE_COUNT = 37
COMMAND_COUNT = 49
RECORD_COUNT = 151
RECORDER_TRANSCRIPT = (
    ...
    '37 scoped mutation anchors authenticated\n'
)
SELFTEST_CASES = (
    ...
    'extra_stage', 'extra_case_directory', 'diagnostic_wrong_case', 'diagnostic_resource',
    'diagnostic_frontend', 'diagnostic_abort_only', 'document_hash', 'diagnostic_initial_typo',
    'raw_count', 'retirement_count', 'negative_terminal', 'incomplete_group',
)
...
def source_tree(repo, source, tree):
    ...
    check = module(tree / CHECK, 'disposal_record_check')
    retirement = module(tree / check.PREVIOUS, 'disposal_record_retirement')
    scalar = module(tree / retirement.PREVIOUS, 'disposal_record_scalar')
    history = module(tree / scalar.PREVIOUS, 'disposal_record_history')
    settlement = module(tree / history.PREVIOUS, 'disposal_record_settlement')
    writer = module(tree / settlement.PREVIOUS, 'disposal_record_writer')
    previous = module(tree / writer.PREVIOUS, 'disposal_record_enrollment')
    module(tree / previous.PREVIOUS, 'disposal_record_unknown')
    legacy = module(tree / settlement.LEGACY, 'disposal_record_legacy')
    base = module(tree / legacy.BASE, 'disposal_record_base')
    ...
    check.configuration()
    need(check.PAIRED_VERIFIED == 1085 and check.RAW_VERIFIED == 356 and check.RETIREMENT_VERIFIED == 999,
         'pinned proof inventories')
    need(check.CPU_TEST_RESULTS == [(996, 0, 18, 0, 0), (27, 0, 0, 0, 0)], 'pinned CPU inventory')
    ...
    return check, retirement, scalar, legacy, base, inherited
...
def audit(repo, packet, quiet=False):
    ...
    need(len(files) == 154 and set(manifest) == set(files), 'manifest roster')
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-disposal-audit-') as temporary:
        tree = Path(temporary)
        check, retirement, scalar, legacy, base, _ = source_tree(repo, source, tree)
        ...
        need(len(cases) == 41 and set(results) == {name for name, _ in cases}, 'exact proof case roster')
        ...
        for name, change in cases:
            ...
            receipts[name] = row
            root = check.RAW if name == 'raw-regression' else retirement.ROOT if name == 'retirement-regression' else check.ROOT
            ...
            if change:
                ...
                count = 356 if name == 'raw-regression' else 999 if name == 'retirement-regression' else 1085
        ...
        need(str(old_audit) in source['inputs'], 'CPU parser is source bound')
        parse = module(tree / old_audit, 'disposal_cpu_result_parser').test_results
        need(parse((records / 'test/stdout.log').read_text()) == [(996, 0, 18, 0, 0), (27, 0, 0, 0, 0)], 'unit and doctest results')
    ...
    if not quiet:
        print('PASS: 405 Git source identities, 49 terminal commands, 37 scoped disposal controls, and CPU receipts')


def document_pin_selftest(repo, source):
    with tempfile.TemporaryDirectory(prefix='fe2o3-disposal-document-test-') as temporary:
        tree = Path(temporary)
        check, _, scalar, _, _, inherited = source_tree(repo, source, tree)
...
def selftest(repo, packet):
    for case in SELFTEST_CASES:
        with tempfile.TemporaryDirectory(prefix='fe2o3-disposal-audit-test-') as temporary:
            ...
            if case in ('probe', 'source_roster', 'model_input', 'document_hash'):
                ...
                if case == 'probe':
                    ...
                    removed = str(DOCUMENT) if case == 'source_roster' else 'crates/fe2o3-runtime-model/verus/context_owner_disposal_model_v1.rs'
            ...
            elif case in ('positive_count', 'positive_type', 'raw_count', 'retirement_count'):
                name = 'raw-regression' if case == 'raw_count' else 'retirement-regression' if case == 'retirement_count' else 'positive-before'
                ...
                path = records / 'unknown_only/stdout.log'
                row = read(path)
                ...
                path = records / 'results.json'
                results = read(path)
                results['unknown_only']['result'] = row['verification-results']
                path.write_text(json.dumps(results))
            elif case.startswith('diagnostic_'):
                name = 'unknown_only' if case in ('diagnostic_wrong_case', 'diagnostic_initial_typo') else 'unchecked_return'
                ...
                root = 'crates/fe2o3-runtime-model/verus/context_owner_disposal_paired_v1.rs'
                ...
                path = records / 'unknown_only/record.json'
                row = read(path)
                if case == 'negative_scope':
                    row['command'][row['command'].index('--verify-function') + 1] = 'owner_disposal_live_witness_v1'
                ...
                row['command'][-1] = row['command'][-1].replace(
                    'context_owner_disposal_paired_v1.rs', 'context_owner_disposal_execution_v1.rs')
                path.write_text(json.dumps(row))
            elif case == 'cpu_count':
                path = records / 'test/stdout.log'
                text = path.read_text()
                need(text.count('996 passed;') == 1, 'CPU selftest anchor')
                path.write_text(text.replace('996 passed;', '995 passed;'))
    ...
    print(f'PASS: {len(SELFTEST_CASES)} rehashed malformed record sets and 1 source-document pin mutation rejected')
