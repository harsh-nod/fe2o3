...
CAMPAIGN = Path('/home/harsh/.codex-tmp/fe2o3-producer-issuance-qualification-20260921-a/producer-campaign')
ARCHIVE = ROOT / 'docs/evidence/dev-producer-journal-issuance-verus-2026-09-21'
...
QUALIFY_SHA = '6cec065ce5ced2c67641d988f6776f3374d88c5d99125ad6177c4b390d5d5858'
PINNED = {
    PROOF / 'context_producer_journal_issuance_v1.rs': 'CONTEXT_PRODUCER_JOURNAL_ISSUANCE_SHA256',
    PROOF / 'check-producer-journal-issuance.py': 'PRODUCER_JOURNAL_ISSUANCE_CHECKER_SHA256',
...
FINISHED = {'source_commit': SOURCE, 'qualified': True,
            ...
            'constructor_to_pending_reachability': False,
...
COMMANDS = [
    ...
    ('lint', ['/home/harsh/.local/bin/ruff', 'check', '--no-cache',
              str(PROOF / 'check-producer-journal-issuance.py'), str(PROOF / 'check-negative-quality.py'),
              str(PROOF / 'check-read-commit.py'), str(PROOF / 'check-read-invariant.py'),
              str(PROOF / 'check-producer-read-invariant.py'), str(PROOF / 'check-producer-read-lifecycle.py')], 60),
    ('diff-check', ['git', 'diff', '--check', SOURCE + '^', SOURCE], 30),
    ('producer-campaign', ['python3', '-I', '-B', str(PROOF / 'check-producer-journal-issuance.py'),
                           str(PROOF / 'context_producer_journal_issuance_v1.rs'), str(VERUS), '180', str(CAMPAIGN)], 5400),
...
def signed_sources(repo, temporary):
    ...
    checker = temporary / PROOF / 'check-producer-journal-issuance.py'
...
def check_times(record):
    ...
    # Receipts use realtime timestamps, not a monotonic performance clock.
...
def validate_campaign(campaign, module, contents, temporary):
    ...
    expected_result = {'qualified': True, 'obligations': 201, 'inherited': 181,
                       'executable_mutations': 13, 'inputs': inputs}
    need(result == expected_result and all(type(result[key]) is type(value)
         for key, value in expected_result.items()), 'exact dedicated result')
    source = contents[PROOF / 'context_producer_journal_issuance_v1.rs'].decode('ascii')
    mutations = module.mutations()
    need(len(mutations) == 13 and len({item.name for item in mutations}) == 13,
         'exact unique mutation inventory')
    cases = [('positive_before', None), *[(item.name, item) for item in mutations], ('positive_after', None)]
    need(len(cases) == 15, 'exact dedicated case count')
    ...
    for phase in ['before', 'after']:
        ...
        for name, mutation in cases:
            ...
            candidate = original_folder / 'context_producer_journal_issuance_v1.rs'
            expected = module.BASE.mutate(source, mutation) if mutation else source
            generated = {'context_producer_journal_issuance_v1.rs': expected.encode('ascii'),
            ...
            rows.append({'case': name, 'exit': record['status'], 'verified': 200 if mutation else 201,
