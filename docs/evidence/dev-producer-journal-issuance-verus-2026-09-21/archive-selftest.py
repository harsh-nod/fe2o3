...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-evidence-selftest-') as owned:
        ...
        api['need'](len(campaign_check()) == 15, 'baseline complete campaign')
        adverse('floating obligation count', campaign / 'finished.json',
                change(lambda d: d.update(obligations=201.0)), campaign_check)
        ...
        adverse('changed candidate', case / 'context_producer_journal_issuance_v1.rs', lambda b: b + b'\n', campaign_check)
        ...
        if not args.campaign_only:
            ...
            qualification_check()
        api['need'](len(campaign_check()) == 15, 'restored campaign')
