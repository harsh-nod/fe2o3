...
MANIFEST_SHA = '4d6a594f9307334d0edf557318c3faefe007ab170b29a9f365bfec59a629c351'
...
def mutations(captured):
    ...
    need(len({name for name, _, _ in result}) == 29, 'distinct mutation names')
    need(len({(path, data) for _, path, data in result}) == 29, 'distinct mutation bodies')
    return result
...
def classify(code, log, negative=False):
    ...
    if not negative:
        need(code == 0 and summaries == [(b'31', b'0')], 'exact positive proof result')
        ...
        need(sum(map(int, summaries[0])) == 31, 'complete negative obligation roster')
...
def main():
    ...
    (output / 'result.json').write_text(json.dumps({'status': 'passed', 'obligations_per_positive': 31,
        'positive_runs': 2, 'logical_mutations': 29, 'source_manifest_sha256': MANIFEST_SHA,
    ...
    print('R75_SHARED_DOMAIN_PLANNER_OK obligations=31 positives=2 mutations=29')
