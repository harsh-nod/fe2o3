...
MANIFEST_SHA = 'bdc8f18bc9f2fd76991b1b1e3f994481fadf514a29dcc3bf3003d694526b8c69'
...
PROOF_FILES = frozenset((ROOT, VECTOR, BODY, ARENA,
    ...
    MODEL / 'verus/r75_resource_domain_arena_v1.rs',
...
def classify(code, log, negative=False):
    ...
    if not negative:
        need(code == 0 and summaries == [(b'32', b'0')], 'exact positive proof result')
        ...
        need(sum(map(int, summaries[0])) == 32, 'complete negative obligation roster')
...
def main():
    ...
    (output / 'result.json').write_text(json.dumps({'status': 'passed', 'obligations_per_positive': 32,
    ...
    print('R75_SHARED_DOMAIN_PLANNER_OK obligations=32 positives=2 mutations=31')
