...
const environment = {
  ...
  runners: ['r112-run.js', 'r112-campaign-run.js', 'r112-source-gate.py', 'r112-auxiliary-gates.py', 'r112-freeze.js']
  ...
  campaign_envelope: {previous_deadline_ms: 7200000, deadline_ms: 7200000, individual_gate_deadline_ms: 1800000, reason: 'Retain R111\'s serialized campaign envelope; individual gate and test deadlines are unchanged.'},
  ambient_stack: {RUST_MIN_STACK: process.env.RUST_MIN_STACK ?? null,
    command: ['bash', '-c', 'ulimit -s'], output: execute(['bash', '-c', 'ulimit -s'])},
  prior_artifacts: fs.readdirSync(root).filter(name => /^r112-.*\.(json|log)$/.test(name)).sort()
    .map(name => ({name, sha256: hash(fs.readFileSync(path.join(root, name)))})),
};
const previous = JSON.parse(fs.readFileSync(path.join(repo,
  'docs/evidence/local-r111-uninitialized-coherent-insertion-2026-09-12/raw/r111-environment.json')));
...
for (const [name, value] of [['frozen-source', frozen], ['environment', environment], ['issue-status', issue]]) {
  fs.writeFileSync(path.join(root, 'r112-' + name + '.json'), JSON.stringify(value, null, 2) + '\n', {flag: 'wx'});
}
