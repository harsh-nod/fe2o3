...
(async () => {
  ...
  const baseline = JSON.parse(fs.readFileSync(c.root + 'r112-frozen-source.json'));
  const planned = c.plan('r112-exact-clock-restoration-' + name + '.json');
  const predecessor = c.predecessor('r112-exact-clock-mut-' + name + '.json', 'finished_at', planned.expected);
  ...
  fs.writeFileSync(c.root + 'r112-exact-clock-restoration-' + name + '.json', JSON.stringify(record, null, 2) + '\n', {flag: 'wx'});
  console.log('PASS: all ' + paths.length + ' source identities restored with observed UTC floor');
})().catch(error => {
  fs.writeFileSync(c.root + 'r112-exact-clock-restoration-' + name + '-failure.json', JSON.stringify({accepted: false, error: String(error), clock_failure: error.clock_failure || null}, null, 2) + '\n', {flag: 'wx'});
