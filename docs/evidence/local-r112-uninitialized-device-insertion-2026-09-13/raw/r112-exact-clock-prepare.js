...
const c = require('./r112-exact-clock-evidence.js');
...
(async () => {
  cp.execFileSync('node', [c.root + 'r112-assert-frozen.js']);
  ...
  const testLog = fs.readFileSync(c.root + 'r112-exact-clock-contract-tests.log', 'utf8');
  ...
  const testRun = read('r112-exact-clock-contract-tests.json');
  assert.strictEqual(testRun.returncode, 0);
  assert.strictEqual(testRun.source_unchanged, true);
  const entries = {};
  let predecessor = {name: 'r112-exact-clock-manifest.json', field: 'recorded_at'};
  for (const m of p.mutations) {
    ...
    const runName = 'r112-exact-clock-mut-' + m.name + '.json';
    ...
    const restorationName = 'r112-exact-clock-restoration-' + m.name + '.json';
  ...
  for (const [kind, filter] of p.focused) {
    const name = 'r112-exact-clock-restored-' + kind + '.json';
  ...
  entries['r112-exact-clock-collector-validation.json'] = {kind: 'run', source_map_sha256: sourceHash,
    command: ['node', c.root + 'r112-exact-clock-retain-local.js', '--check'], returncode: 0, predecessor};
  const names = fs.readdirSync(c.root).filter(n => /^r112-.*\.(json|log|py|js)$/.test(n) && !n.startsWith('r112-exact-clock-')).sort();
  const floorRoster = [
  ...
  const manifest = {recorded_at: new Date(observation.samples.at(-1).utc_ms).toISOString(),
    ...
    contract_tests: ['r112-exact-clock-contract-tests.json', 'r112-exact-clock-contract-tests.log', 'r112-exact-clock-contract-tests-source.json']
  ...
  fs.writeFileSync(c.root + 'r112-exact-mutations.json', JSON.stringify(p.mutations, null, 2) + '\n', {flag: 'wx'});
  manifest.historical_artifacts.push({name: 'r112-exact-mutations.json', sha256: c.hash(fs.readFileSync(c.root + 'r112-exact-mutations.json'))});
  manifest.historical_artifacts.sort((a, b) => a.name < b.name ? -1 : a.name > b.name ? 1 : 0);
  fs.writeFileSync(c.root + 'r112-exact-clock-manifest.json', JSON.stringify(manifest, null, 2) + '\n', {flag: 'wx'});
