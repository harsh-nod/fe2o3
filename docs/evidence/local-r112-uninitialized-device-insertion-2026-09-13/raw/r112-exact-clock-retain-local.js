...
for (const [kind, filter, count] of p.focused) for (const phase of ['frozen', 'restored']) {
  ...
  const run = checkRun((phase === 'restored' ? 'r112-exact-clock-' : 'r112-') + phase + '-' + kind, [...p.tests, filter]);
...
const validationText = JSON.stringify(summary, null, 2) + '\n'
  + 'PASS: full/focused/auxiliary rosters, 16 compiled negatives, exact restoration and chronology\n';
if (process.argv[2] === '--check') {
  process.stdout.write(validationText);
  process.exit(0);
}
assert.strictEqual(process.argv.length, 2);
const validation = checkRun('r112-exact-clock-collector-validation', ['node', file('r112-exact-clock-retain-local.js'), '--check']);
