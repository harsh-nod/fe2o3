...
const mutation = JSON.parse(fs.readFileSync(path.join(root, 'r96-mutations.json')))
  .find(m => m.name === name);
assert(mutation);
const baseline = JSON.parse(fs.readFileSync(path.join(root, 'r96-frozen-source.json')));
