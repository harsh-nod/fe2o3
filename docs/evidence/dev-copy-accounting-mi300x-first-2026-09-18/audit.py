...
STAGE = "/home/harsh/.codex-tmp/kfd-accounting-20260918.jTLdmGXF"
...
assert not (ROOT / "runtime-test").exists(), "ELF omitted from published packet"
input_paths = [
    "runtime-test",
    "copy-host-observe.py",
    "source-qualified-before.log",
    "record.sh",
    "source-check.sh",
    "create.sh",
    "run.sh",
...
local_inputs = dict(
    ...
    for line in read("raw/local-inputs.stdout").splitlines()
)
assert (
    len(read("raw/local-inputs.stdout").splitlines())
    == len(local_inputs)
    == len(input_paths)
)
assert set(local_inputs) == set(input_paths)
assert local_inputs.pop("runtime-test") == BINARY
for path, expected in local_inputs.items():
    assert digest(path) == expected, path
remote_inputs = dict(
    line.split(maxsplit=1)[::-1]
    for line in read("raw/remote-inputs.stdout").splitlines()
)
assert len(read("raw/remote-inputs.stdout").splitlines()) == len(remote_inputs) == 4
assert remote_inputs == {
    OWNED + "/runtime-test": BINARY,
    **{
        OWNED + "/" + p: digest(p)
        for p in ("copy-host-observe.py", "run.sh", "inspect.sh")
    },
}
...
commands = {
    "source-before": ["bash", "source-check.sh"],
    "source-after": ["bash", "source-check.sh"],
    "script-syntax": ["bash", "-n", "run.sh", "inspect.sh", "cleanup.sh", "absence.sh"],
    "local-inputs": ["sha256sum", *input_paths],
    "upload": scp
    + [
        "runtime-test",
        "copy-host-observe.py",
    ...
    + [
        OWNED + "/" + p
        for p in ("runtime-test", "copy-host-observe.py", "run.sh", "inspect.sh")
    ],
    "campaign": ssh + ["bash", OWNED + "/run.sh"],
    "collect": ["scp", "-r", *scp[1:]]
    + [
        "mi300x:" + OWNED + "/" + p
        for p in ("results", "owner", "run.sh", "inspect.sh")
    ]
    + ["returned/"],
    "audit": ["python3", "-B", "audit.py"],
}
for name in ("create", "cleanup", "absence"):
    commands[name] = [
        "bash",
        "-c",
...
assert commands.keys() == local.keys()
for name, command in commands.items():
    assert shlex.split(read(f"raw/{name}.command")) == command, name
assert (
    digest("audit-staging.py")
    == "c194acc1fc340d089bb6af49acd0b52a8c86d9e17a173b31d375a88ba36441bf"
)
assert (
    read("raw/audit.stdout")
    == "PASS: one failed native attempt; 11 original local and 7 remote closed receipts; identities, retained refusal, and owned cleanup verified; no copy/refund acceptance\n"
)
