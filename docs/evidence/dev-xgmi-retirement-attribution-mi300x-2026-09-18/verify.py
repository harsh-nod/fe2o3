...
CAMPAIGN_SHA = "94acb8893839768bf2a6247a81f8504f6d90c35729dc6670ebda4dd474f1acab"
...
def verify(root=HERE):
    ...
    need(
        re.fullmatch(
            r"/home/harsh/\.codex-tmp/fe2o3-xgmi-retirement-attribution-20260918\.[A-Za-z0-9_]+",
            str(payload),
        ),
        "private local payload shape",
    )
    commands = C.local_commands(payload, marker, execution_root=EXECUTION_ROOT)
    ...
    for name, count in (("calibration", 21), ("observer-tests", 19)):
        need(
            re.fullmatch(
                r"\.{"
                + str(count)
    ...
    need(
        ...
        and cpu_report["source_files"] == 5568
