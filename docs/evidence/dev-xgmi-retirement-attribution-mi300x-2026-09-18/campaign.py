...
ROOT = HERE.parents[2]
QUALIFIED_SOURCE_COMMIT = "54b75b774fbbb510a0480646535835b7431cda67"
CPU = ROOT / "docs/evidence/dev-xgmi-retirement-cpu-2026-09-18"
CPU_SEAL = "b2d79602fd7200ab9a50553b0eeed4b40be901e3df6099264a1e61723805533b"
...
PREFIX = "/home/harsh/fe2o3-xgmi-retirement-attribution-20260918."
SIGNERS = (
...
def local_commands(payload, marker, *, execution_root=None):
    execution_root = ROOT if execution_root is None else execution_root
    run_here = execution_root / HERE.relative_to(ROOT)
    run_cpu = execution_root / CPU.relative_to(ROOT)
    run_selector = execution_root / SELECTOR.relative_to(ROOT)
    serialized = json.dumps(marker, sort_keys=True, separators=(",", ":"))
    result = [
        (
            "calibration",
            ["python3", "-I", str(run_here / "test_campaign.py")],
            60,
            None,
        ),
        (
        ...
        (
            "cpu-verify",
            ["python3", "-I", str(run_cpu / "verify.py"), "--live"],
            60,
            None,
        ),
        (
        ...
        ("source", ["python3", "-I", str(run_selector)], 60, None),
        (
            "source-ancestor",
            [
                "git",
...
def run_local(indices):
    ...
    payload = Path(
        tempfile.mkdtemp(
            prefix="fe2o3-xgmi-retirement-attribution-20260918.",
            dir="/home/harsh/.codex-tmp",
        )
    )
    state = {
        "created": False,
