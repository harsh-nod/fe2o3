...
SOURCE = 'de34cb456cbf0bfd21f006b04ce77f0219b24177'
CHECK = Path('crates/fe2o3-runtime-model/verus/check-owner-scalar-enrollment.py')
...
def audit(repo, packet, quiet=False):
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-scalar-enrollment-audit-') as temporary:
        tree = Path(temporary)
        ...
        history = module(tree / check.PREVIOUS, 'scalar_record_history')
        ...
        check.authenticate(tree, inherited, history, git_repo=repo)
        ...
        cases = [('positive-before', None), *[(m[0], m) for m in check.MUTATIONS], ('positive-after', None), ('raw-regression', None), ('settlement-regression', None)]
        ...
        for name, mutation in cases:
            ...
            receipts[name] = row
            root = check.RAW if name == 'raw-regression' else history.ROOT if name == 'settlement-regression' else check.ROOT
            ...
            check.check_verifier(observed['verus'])
            if mutation:
                ...
                need(legacy.same(observed['result'], {'encountered-error': False, 'encountered-vir-error': False,
                    'success': True, 'verified': 310 if name == 'raw-regression' else 916 if name == 'settlement-regression' else check.PAIRED_VERIFIED,
        ...
        need(parse((records / 'test/stdout.log').read_text()) == [(966, 0, 18, 0, 0), (27, 0, 0, 0, 0)], 'unit and doctest results')
...
def selftest(repo, packet):
    ...
    for case in cases:
        with tempfile.TemporaryDirectory(prefix='fe2o3-scalar-enrollment-audit-test-') as temporary:
            ...
            if case in ('probe', 'source_roster'):
                ...
                path = records / 'header_order/stdout.log'
                row = read(path)
                ...
                path = records / 'results.json'
                results = read(path)
                results['header_order']['result'] = row['verification-results']
                path.write_text(json.dumps(results))
            elif case == 'negative_scope':
                path = records / 'header_order/record.json'
                row = read(path)
                index = row['command'].index('--verify-function') + 1
                row['command'][index] = 'owner_scalar_enrollment_live_witness_v1'
                path.write_text(json.dumps(row))
            elif case == 'paired_root':
                path = records / 'positive-before/record.json'
                row = read(path)
                row['command'][-1] = row['command'][-1].replace(
                    'context_owner_scalar_enrollment_paired_v1.rs', 'context_owner_scalar_enrollment_execution_v1.rs')
                path.write_text(json.dumps(row))
            elif case == 'model_input':
                path = records / 'source.json'
                row = read(path)
                del row['inputs']['crates/fe2o3-runtime-model/verus/context_owner_scalar_enrollment_model_v1.rs']
                path.write_text(json.dumps(row))
                (records / 'inputs-after.json').write_text(json.dumps(row['inputs']))
            elif case == 'cpu_count':
                path = records / 'test/stdout.log'
                text = path.read_text()
                need(text.count('966 passed;') == 1, 'CPU selftest anchor')
                path.write_text(text.replace('966 passed;', '965 passed;'))
