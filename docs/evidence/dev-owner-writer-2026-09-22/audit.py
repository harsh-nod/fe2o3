...
SOURCE = 'e83a6247de1ab97dc28ac19500200945788683a4'
CHECK = Path('crates/fe2o3-runtime-model/verus/check-owner-writer.py')
...
def audit(repo, packet, quiet=False):
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-writer-audit-') as temporary:
        tree = Path(temporary)
        ...
        previous = module(tree / check.PREVIOUS, 'writer_record_previous')
        ancestor = module(tree / previous.PREVIOUS, 'writer_record_ancestor')
        legacy = module(tree / check.LEGACY, 'enrollment_record_legacy')
        base = module(tree / legacy.BASE, 'enrollment_record_base')
        expected = set(legacy.SOURCES) | set(ancestor.NEW) | set(previous.NEW) | set(check.NEW) | set(check.CPU_INPUTS) | set(legacy.PINS) | {
            check.LEGACY, check.PREVIOUS, previous.PREVIOUS, CHECK, check.CRATE / 'verus/context_version_journal_settlement_v1.rs',
        ...
        cases = [('raw', None), ('positive-before', None), *[(m[0], m) for m in check.MUTATIONS], ('positive-after', None)]
        ...
        for name, mutation in cases:
            ...
            receipts[name] = row
            root = check.RAW if name == 'raw' else check.ROOT
        ...
        need(parse((records / 'test/stdout.log').read_text()) == [(952, 0, 18, 0, 0), (27, 0, 0, 0, 0)], 'unit and doctest results')
...
def selftest(repo, packet):
    for case in ('probe', 'source_roster', 'positive_count', 'positive_type', 'negative_scope', 'cpu_count',
                 'closure_transcript', 'closure_prefix', 'receipt_order', 'auditor_identity'):
        with tempfile.TemporaryDirectory(prefix='fe2o3-writer-audit-test-') as temporary:
            ...
            if case in ('probe', 'source_roster'):
                ...
                path = records / 'register_id/record.json'
                row = read(path)
                index = row['command'].index('--verify-function') + 1
                row['command'][index] = 'owner_register_historical_exec_v1'
                path.write_text(json.dumps(row))
            elif case == 'cpu_count':
                path = records / 'test/stdout.log'
                text = path.read_text()
                need(text.count('952 passed;') == 1, 'CPU selftest anchor')
                path.write_text(text.replace('952 passed;', '951 passed;'))
    ...
    print('PASS: ten rehashed malformed record sets rejected')
