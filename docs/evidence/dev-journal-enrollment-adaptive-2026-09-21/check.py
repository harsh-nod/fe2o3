#!/usr/bin/env python3
"""Standalone actual-type adaptive sorting campaign with executable controls."""
import argparse
import hashlib
...
PROJECTIONS = [(BODY, 'audited-shared-bodies.rs', 11),
...
MUTATIONS = [
    ...
    ('reverse_pair', 'enrollment_reverse_body', 'enrollment_reverse',
     '$len - 1 - $index', '$index'),
    ...
    ('pivot_median', 'enrollment_pivot_body', 'enrollment_pivot',
...
def check_result(base, status, stdout, stderr, folder, mutation, expected):
    ...
    if mutation:
        ...
        need(same(observed['result'], {
            ...
            'verified': 57, 'errors': 0, 'is-verifying-entire-crate': True,
