...
PACKET = Path('docs/evidence/dev-journal-enrollment-adaptive-2026-09-21')
...
def timing_rows(kind, text):
    ...
    sizes = [8, 16, 17, 64, 255, 256, 257, 512, 4096]
        ...
        for pattern in patterns:
            for round_number in range(7):
                for flag in ('true', 'false'):
                    ...
                    expected[key] = max(1_000_000 // size, 100) if kind == 'adaptive' else min(256, max(8, 16_384 // size))
...
def render_results(packet):
    ...
    lines += ['', '## Complete Enrollment Sort Selection', '',
    ...
    lines += ['', 'The remaining architectural gate is actual-type lifecycle composition: construction, full enrollment',
    ...
    with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-audit-') as temporary:
        ...
        for name, command in cpu.commands():
            ...
            if environment is None:
                ...
                need(target.is_absolute() and target.name == 'target' and target.parent.name.startswith('fe2o3-enrollment-introsort-'), 'owned CPU target')
            ...
            if name == 'test':
                need(test_results(out) == [(870, 0, 7, 0, 0), (27, 0, 0, 0, 0)], 'complete unit/doctest result roster')
            if name in ('adaptive', 'sort_enrollment'):
                need(test_results(out) == [(1, 0, 0, 0, 876)], 'executed benchmark and fixture assertions')
    ...
    for name, relative, addition in modifications:
        with tempfile.TemporaryDirectory(prefix='fe2o3-ordering-negative-') as temporary:
            ...
            if addition is not None:
                ...
                first = next(i for i, line in enumerate(lines) if 'adaptive,' in line)
                del lines[first]
                target.write_text('\n'.join(lines) + '\n')
...
def main():
    ...
    if args.selftest:
        ...
        print('PASS: standalone adaptive proof and CPU packet reconstructed from Git objects')
