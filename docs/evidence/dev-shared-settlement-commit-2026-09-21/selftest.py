#!/usr/bin/env python3
"""Reject substituted commit diagnostics, shared inputs and rehashed packets."""
import argparse
import copy
...
def main():
    ...
    spec = importlib.util.spec_from_file_location('shared_commit_diagnostic_selftest', path)
    ...
    for item in mutations:
        case_inputs(item)
    mutation = next(m for m in mutations if m.name == 'writer_clear')
    ...
    def target(rows, primary):
        return next(s for s in rows[0]['spans'] if s['is_primary'] is primary)
    ...
    for primary in (False, True):
        for field, value in [('file_name', '/foreign.rs'), ('byte_start', 0), ('line_start', 1.0),
            ...
            changed = copy.deepcopy(diagnostics)
            target(changed, primary)[field] = value
            invalid.append((1, out, '\n'.join(map(json.dumps, changed))))
        for field, value in [('text', 'foreign excerpt'), ('highlight_start', True), ('highlight_end', 0)]:
            changed = copy.deepcopy(diagnostics)
            target(changed, primary)['text'][0][field] = value
            invalid.append((1, out, '\n'.join(map(json.dumps, changed))))

    def relocate_exit(rows):
        secondary = target(rows, False)
        start = source.index('settlement_prefix_shapes_v1', source.index('pub fn shared_settlement_commit_v1'))
        secondary.clear()
        secondary.update(spans_helper.exact_span(source, original, start, start + len('settlement_prefix_shapes_v1'),
            False, 'at the end of the function body'))
    ...
    for item in mutations:
        ...
        try:
            ...
            checker.need(str(error) == 'exact commit loop invariant span', 'exact invariant target rejection')
        else:
    ...
    rejected_sources = 0
    with tempfile.TemporaryDirectory(prefix='fe2o3-shared-commit-source-selftest-') as temporary:
        for index in range(10):
            folder = Path(temporary) / str(index)
            shutil.copytree(good, folder)
            changed_source, changed_body = good_source, good_body
            if index == 1:
                changed_source = changed_source.replace('../src/context_version_journal/settlement_commit_body.rs', 'foreign.rs')
            elif index == 2:
                changed_source += '\ninclude!("foreign.rs");\n'
            elif index == 3:
                changed_body = changed_body.replace('$journal:ident', '$journal:expr', 1)
            elif index == 4:
                changed_body = changed_body.replace('let mut $index', 'assume(false); let mut $index', 1)
            elif index == 5:
                changed_body = changed_body.replace('let mut $index', '#[cfg(test)] let bypass = 0; let mut $index', 1)
            elif index == 6:
                changed_body += '\nmacro_rules! bypass { () => { true }; }\n'
            elif index == 7:
                path = folder / 'verus' / module.shared_commit_parent.SOURCE
                path.write_bytes(path.read_bytes() + b'\n')
            elif index == 8:
                changed_source = changed_source.replace(checker.SCRATCH, 'let bypass = true;')
            elif index == 9:
                changed_source = changed_source.replace('settlement_commit_body!(verus_exec_expr,', 'settlement_commit_body!(foreign_expr,')
            try:
    ...
    if args.packet:
        command = [sys.executable, '-I', '-B', str(Path(__file__).resolve().with_name('audit.py')),
                   '--repo', str(args.repo.resolve()), '--packet']
        ...
        expected_failures = ['exact staged nested inputs', 'exact commit postcondition and wrapper exit',
            'owned solver completion', 'owned solver completion', 'exact whole-crate result', 'proof roster',
            'proof input identities', 'owned command completion', 'exact commit loop invariant span']
        with tempfile.TemporaryDirectory(prefix='fe2o3-shared-commit-packet-selftest-') as temporary:
            for index, failure in enumerate(expected_failures):
                packet = Path(temporary) / str(index)
                shutil.copytree(args.packet, packet)
                if index == 0:
                    path = packet / 'proof/writer_clear' / checker.BODY
                    path.write_text(path.read_text().replace('let _ = $writer.slot;', '$journal.writers[$writer.slot] = None;'))
                elif index == 1:
                    path = packet / 'proof/writer_clear/solver/stderr.log'
                    rows = [json.loads(line) for line in path.read_text().splitlines()]
                    relocate_exit(rows)
                    path.write_text('\n'.join(map(json.dumps, rows)) + '\n')
                elif index in (2, 3):
                    path = packet / 'proof/writer_clear/solver/record.json'
                    ...
                    item = next(m for m in mutations if m.name == 'lineage')
                    text, _, location, _, _ = case_inputs(item)
                    path = packet / 'proof/lineage/solver/stderr.log'
                    rows = [json.loads(line) for line in path.read_text().splitlines()]
                    relocate_invariant(rows, text, location, item)
                    path.write_text('\n'.join(map(json.dumps, rows)) + '\n')
