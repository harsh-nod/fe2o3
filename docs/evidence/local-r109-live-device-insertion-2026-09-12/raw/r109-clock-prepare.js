...
const c = require('./r109-clock-evidence.js');
...
(async () => {
  ...
  const testLog = fs.readFileSync(c.root + 'r109-clock-contract-tests.log', 'utf8');
  ...
  const testRun = read('r109-clock-contract-tests.json');
  assert.strictEqual(testRun.returncode, 0);
  assert.strictEqual(testRun.source_unchanged, true);
  const entries = {};
  let predecessor = {name: 'r109-clock-manifest.json', field: 'recorded_at'};
  for (const m of p.mutations) {
    ...
    const runName = 'r109-clock-mut-' + m.name + '.json';
    ...
    const restorationName = 'r109-clock-restoration-' + m.name + '.json';
  ...
  for (const [kind, filter] of p.focused) {
    const name = 'r109-clock-restored-' + kind + '.json';
  ...
  entries['r109-clock-collector-validation.json'] = {kind: 'run', source_map_sha256: sourceHash,
    command: ['node', c.root + 'r109-clock-retain-local.js', '--check'], returncode: 0, predecessor};
  const names = fs.readdirSync(c.root).filter(n => /^r109-.*\.(json|log|py|js)$/.test(n) && !n.startsWith('r109-clock-')).sort();
  const floorRoster = [
  ...
  const manifest = {recorded_at: new Date(observation.samples.at(-1).utc_ms).toISOString(),
    ...
    contract_tests: ['r109-clock-contract-tests.json', 'r109-clock-contract-tests.log', 'r109-clock-contract-tests-source.json']
  ...
  fs.writeFileSync(c.root + 'r109-mutations.json', JSON.stringify(p.mutations, null, 2) + '\n', {flag: 'wx'});
  manifest.historical_artifacts.push({name: 'r109-mutations.json', sha256: c.hash(fs.readFileSync(c.root + 'r109-mutations.json'))});
  manifest.historical_artifacts.sort((a, b) => a.name < b.name ? -1 : a.name > b.name ? 1 : 0);
  fs.writeFileSync(c.root + 'r109-clock-manifest.json', JSON.stringify(manifest, null, 2) + '\n', {flag: 'wx'});
