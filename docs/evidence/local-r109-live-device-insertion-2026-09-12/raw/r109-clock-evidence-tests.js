...
const helperNames = ['r109-clock-check-mutation.js', 'r109-clock-evidence-tests.js', 'r109-clock-evidence.js',
...
(async () => {
  ...
  await test('foreign cohort and missing predecessors reject', async () => {
    ...
    assert.throws(() => c.predecessor('r109-clock-does-not-exist.json', 'finished_at'), /ENOENT/);
    assert.throws(() => c.predecessor('r109-clock-does-not-exist.json', 'anything'), /predecessor field/);
  });
  const predecessorName = 'r109-clock-test-parent.json';
  ...
  await test('wrong event source cohort bytes or outcome rejects', async () => {
    for (const patch of [{name: 'r109-clock-other.json'}, {field: 'verified_at'}, {kind: 'restoration'},
