...
const clock = require('./r109-clock-evidence.js');
...
const manifest = read('r109-clock-manifest.json');
...
const historicalNames = fs.readdirSync(root).filter(n => /^r109-.*\.(json|log|py|js)$/.test(n) && !n.startsWith('r109-clock-')).sort();
...
const floorRoster = [
  ...
  ['r109-environment.json', 'recorded_at'], ['r109-clock-contract-tests.json', 'finished_at'],
];
...
const mutations = read('r109-mutations.json');
...
const plannedNames = mutations.flatMap(m => ['r109-clock-mut-' + m.name + '.json', 'r109-clock-restoration-' + m.name + '.json'])
  .concat(p.focused.map(([name]) => 'r109-clock-restored-' + name + '.json'), ['r109-clock-collector-validation.json']);
assert.deepStrictEqual(Object.keys(manifest.entries), plannedNames);
let expectedPredecessor = {name: 'r109-clock-manifest.json', field: 'recorded_at'};
for (const name of plannedNames) {
  ...
  const restoration = name.startsWith('r109-clock-restoration-');
...
function checkClock(name, record, restoration = false) {
  ...
  assert.strictEqual(record.manifest_sha256, hash(fs.readFileSync(file('r109-clock-manifest.json'))));
...
const clockTests = checkRun('r109-clock-contract-tests', ['node', file('r109-clock-evidence-tests.js')]);
assert.deepStrictEqual(manifest.contract_tests.map(r => r.name),
  ['r109-clock-contract-tests.json', 'r109-clock-contract-tests.log', 'r109-clock-contract-tests-source.json']);
...
for (const [kind, filter, count] of p.focused) for (const phase of ['frozen', 'restored']) {
  ...
  const run = checkRun((phase === 'restored' ? 'r109-clock-' : 'r109-') + phase + '-' + kind, [...p.tests, filter]);
...
for (const mutation of mutations) {
  ...
  const name = 'r109-clock-mut-' + mutation.name;
  ...
  const restored = read('r109-clock-restoration-' + mutation.name + '.json');
  checkClock('r109-clock-restoration-' + mutation.name, restored, true);
...
const summary = {
  ...
  accepted_mutation_cohort: 'r109-clock-mut-',
...
const validation = checkRun('r109-clock-collector-validation', ['node', file('r109-clock-retain-local.js'), '--check']);
