...
const clock = require('./r109-exact-clock-evidence.js');
const p = require('./r109-exact-qualification-plan.js');
const root = '/home/harsh/.codex-tmp';
const repo = path.join(root, 'fe2o3-r61-execution');
const dest = path.join(repo, 'docs/evidence/local-r109-live-device-insertion-2026-09-12');
...
assert.strictEqual(git(['rev-parse', p.parent + '^']).trim(), p.accepted);
assert.deepStrictEqual(git(['diff', '--name-only', p.accepted, p.parent]).trim().split('\n'),
...
const prior = JSON.parse(committed('raw/r108-frozen-source.json'));
...
const environment = read('r109-environment.json');
...
assert.deepStrictEqual(environment.binaries, JSON.parse(committed('raw/r108-environment.json')).binaries);
...
assert.deepStrictEqual(environment.runners.map(r => r.name),
  ['r109-run.js', 'r109-source-gate.py', 'r109-auxiliary-gates.py', 'r109-freeze.js']);
...
const preliminary = ['initial-format', 'initial-initializer', 'composition-format', 'initial-composition',
  'prefix-format', 'prefix-composition', 'oracles-format', 'oracles-build', 'holes-format', 'expanded-composition',
  'review-format', 'reviewed-composition', 'forwarders-format', 'initial-clippy'];
assert.deepStrictEqual(environment.prior_artifacts.map(r => r.name), preliminary.flatMap(name =>
  ['.json', '.log', '-source.json'].map(suffix => 'r109-' + name + suffix)).sort());
...
for (const name of preliminary) {
  const run = read('r109-' + name + '.json');
  const rejected = ['initial-composition', 'expanded-composition'].includes(name);
  ...
  assert.strictEqual(run.source_unchanged, !['initial-format', 'composition-format', 'prefix-format',
    'oracles-format', 'holes-format', 'review-format'].includes(name));
...
assert(output(read('r109-initial-composition.json')).includes('error[E'));
assert.deepStrictEqual(totals(output(read('r109-expanded-composition.json'))),
  {harnesses: 1, passed: 13, failed: 1, ignored: 0});
assert.deepStrictEqual(passing(output(read('r109-reviewed-composition.json'))), p.newTests);
const issue = read('r109-issue-status.json');
...
const manifest = read('r109-exact-clock-manifest.json');
...
const historicalNames = fs.readdirSync(root).filter(n => /^r109-.*\.(json|log|py|js)$/.test(n) && !n.startsWith('r109-exact-clock-')).sort();
...
const mutations = read('r109-exact-mutations.json');
...
for (const [type, oldLine, newLine] of [
  ['Gfx942ComputeDependencyEventV1', 3825, 3827],
  ['Gfx942ComputeDependencyDispatchV1', 3880, 3882],
  ['ComputeAqlQueueLaneDispatchV1', 4364, 4366],
]) {
...
const plannedNames = mutations.flatMap(m => ['r109-exact-clock-mut-' + m.name + '.json', 'r109-exact-clock-restoration-' + m.name + '.json'])
  .concat(p.focused.map(([name]) => 'r109-exact-clock-restored-' + name + '.json'), ['r109-exact-clock-collector-validation.json']);
assert.deepStrictEqual(Object.keys(manifest.entries), plannedNames);
let expectedPredecessor = {name: 'r109-exact-clock-manifest.json', field: 'recorded_at'};
for (const name of plannedNames) {
  ...
  const restoration = name.startsWith('r109-exact-clock-restoration-');
...
function checkClock(name, record, restoration = false) {
  ...
  assert.strictEqual(record.manifest_sha256, hash(fs.readFileSync(file('r109-exact-clock-manifest.json'))));
...
const gates = read('r109-final-source-gate.json');
const auxiliary = read('r109-auxiliary-results.json');
for (const [rows, oldName, count, prefix, logPrefix, oldPrefix] of [
  [gates, 'raw/r108-final-source-gate.json', 17, 'r109-final', 'r109-final', 'r108-final'],
  [auxiliary, 'raw/r108-accepted-auxiliary-results.json', 10, 'r109-auxiliary', 'r109', 'r108-accepted'],
]) {
  ...
  for (const [index, row] of rows.entries()) {
    assert.deepStrictEqual(row.command, old[index].command.map(s => s.replace('r108-accepted-production-metadata.log', 'r109-production-metadata.log')), row.name);
    ...
    if (row.name === 'production-metadata') {
      assert.strictEqual(row.stderr_log, file('r109-production-metadata.stderr.log'));
...
for (const target of ['gnu', 'musl']) {
  ...
  const old = committed('raw/r108-final-' + target + '-tests.log');
  ...
  assert.deepStrictEqual(expected, {harnesses: 48, passed: 2619, failed: 0, ignored: 5});
  full[target] = expected;
}
const sourceCampaign = checkRun('r109-source-campaign', ['python3', '-B', file('r109-source-gate.py'), 'r109-final']);
const rejectedChecker = checkRun('r109-original-mutation-checker',
  ['node', file('r109-clock-check-mutation.js'), 'omit-reservation'], 1);
assert(output(rejectedChecker).includes('exact behavioral assertion location'));
const originalNegative = read('r109-clock-mut-omit-reservation.json');
assert.strictEqual(originalNegative.returncode, 101);
assert.strictEqual(originalNegative.source_unchanged, true);
assert.strictEqual(originalNegative.signal, null);
assert.strictEqual(originalNegative.timed_out, false);
assert.strictEqual(originalNegative.clock.error, null);
const originalMutations = read('r109-mutations.json');
assert.strictEqual(originalMutations.length, p.mutations.length);
for (const [index, mutation] of p.mutations.entries()) {
  assert.deepStrictEqual({...mutation, oracle_path: path.posix.normalize(mutation.oracle_path)}, originalMutations[index]);
}
const originalManifest = read('r109-clock-manifest.json');
assert.strictEqual(hash(fs.readFileSync(file('r109-clock-manifest.json'))), originalNegative.manifest_sha256);
for (const artifact of [...originalManifest.helpers, ...originalManifest.historical_artifacts, ...originalManifest.contract_tests]) {
  assert.strictEqual(hash(fs.readFileSync(file(artifact.name))), artifact.sha256, 'original pinned artifact preserved');
}
const originalMutation = originalMutations[0];
assert.strictEqual(originalMutation.name, 'omit-reservation');
assert.strictEqual(originalMutation.oracle_line, p.mutations[0].oracle_line);
assert.strictEqual(path.posix.normalize(p.mutations[0].oracle_path), originalMutation.oracle_path);
assert.deepStrictEqual(failed(output(originalNegative)), [originalMutation.test]);
assert(output(originalNegative).includes(p.mutations[0].oracle_path + ':774:'));
assert(!output(originalNegative).includes(originalMutation.oracle_path + ':774:'));
const originalRestoration = read('r109-clock-restoration-omit-reservation.json');
assert.strictEqual(originalRestoration.source_unchanged, true);
assert.strictEqual(originalRestoration.source_identities, paths.length);
assert.strictEqual(originalRestoration.source_map_sha256, hash(JSON.stringify(baseline)));
assert(time(originalRestoration.verified_at) >= time(originalNegative.finished_at));
const auxCampaign = checkRun('r109-auxiliary-campaign', ['python3', '-B', file('r109-auxiliary-gates.py')]);
assert(time(auxCampaign.started_at) >= time(sourceCampaign.finished_at));
const clockTests = checkRun('r109-exact-clock-contract-tests', ['node', file('r109-exact-clock-evidence-tests.js')]);
assert.deepStrictEqual(manifest.contract_tests.map(r => r.name),
  ['r109-exact-clock-contract-tests.json', 'r109-exact-clock-contract-tests.log', 'r109-exact-clock-contract-tests-source.json']);
...
const oldNames = passing(committed('raw/r108-final-gnu-tests.log'));
...
for (const [kind, filter, count] of p.focused) for (const phase of ['frozen', 'restored']) {
  ...
  const run = checkRun((phase === 'restored' ? 'r109-exact-clock-' : 'r109-') + phase + '-' + kind, [...p.tests, filter]);
...
for (const mutation of mutations) {
  ...
  const name = 'r109-exact-clock-mut-' + mutation.name;
  ...
  const restored = read('r109-exact-clock-restoration-' + mutation.name + '.json');
  checkClock('r109-exact-clock-restoration-' + mutation.name, restored, true);
...
const summary = {
  scope: 'N3-L1 initialized-device insertion at named CPU/shared-sequencer and concrete-facade boundaries; no formal, native or performance qualification',
  ...
  original_mutation_cohort: {accepted: false, name: 'r109-clock-mut-', checker_returncode: 1,
    reason: 'Exact compiler-reported path contains the construction_primary/../ module path; canonical filename oracle rejected it',
    source_restored: true, original_oracle_path: originalMutation.oracle_path,
    compiler_reported_oracle_path: p.mutations[0].oracle_path},
  ...
  accepted_mutation_cohort: 'r109-exact-clock-mut-',
  limits: ['constructed native leaves are scripted, not Linux GPU execution',
    ...
    'concrete session/facade dynamic coverage exercises preflight and missing-engine failure, not native success',
    'configured accounting only for new composition; prior lower initializer tests retain configured/unconfigured coverage',
    ...
    'coherent/uninitialized insertion, cleanup custody, generated adoption and the remaining roadmap stay open'],
};
const validationText = JSON.stringify(summary, null, 2) + '\n'
  + 'PASS: full/focused/auxiliary rosters, 11 compiled negatives, exact restoration and chronology\n';
if (process.argv[2] === '--check') {
  process.stdout.write(validationText);
  process.exit(0);
}
assert.strictEqual(process.argv.length, 2);
const validation = checkRun('r109-exact-clock-collector-validation', ['node', file('r109-exact-clock-retain-local.js'), '--check']);
...
const names = fs.readdirSync(root).filter(n => /^r109-.*\.(json|log|py|js)$/.test(n)).sort();
