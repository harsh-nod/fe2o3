...
const helperNames = ['r109-exact-clock-check-mutation.js', 'r109-exact-clock-evidence-tests.js', 'r109-exact-clock-evidence.js',
  'r109-exact-clock-prepare.js', 'r109-exact-clock-restoration.js', 'r109-exact-clock-retain-local.js', 'r109-exact-clock-run.js',
  'r109-exact-qualification-plan.js'];
...
(async () => {
  ...
  await test('foreign cohort and missing predecessors reject', async () => {
    assert.throws(() => c.predecessor('r109-repeat-mut-slot-only.json', 'finished_at'), /clock cohort/);
    assert.throws(() => c.predecessor('r109-exact-clock-does-not-exist.json', 'finished_at'), /ENOENT/);
    assert.throws(() => c.predecessor('r109-exact-clock-does-not-exist.json', 'anything'), /predecessor field/);
  });
  const predecessorName = 'r109-exact-clock-test-parent.json';
  ...
  await test('wrong event source cohort bytes or outcome rejects', async () => {
    for (const patch of [{name: 'r109-exact-clock-other.json'}, {field: 'verified_at'}, {kind: 'restoration'},
