...
(async () => {
  ...
  const planned = c.plan('r109-exact-clock-restoration-' + name + '.json');
  const predecessor = c.predecessor('r109-exact-clock-mut-' + name + '.json', 'finished_at', planned.expected);
  ...
  fs.writeFileSync(c.root + 'r109-exact-clock-restoration-' + name + '.json', JSON.stringify(record, null, 2) + '\n', {flag: 'wx'});
  console.log('PASS: all ' + paths.length + ' source identities restored with observed UTC floor');
})().catch(error => {
  fs.writeFileSync(c.root + 'r109-exact-clock-restoration-' + name + '-failure.json', JSON.stringify({accepted: false, error: String(error), clock_failure: error.clock_failure || null}, null, 2) + '\n', {flag: 'wx'});
