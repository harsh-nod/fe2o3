...
const mutation = JSON.parse(fs.readFileSync(path.join(root, 'r109-exact-mutations.json')))
  .find(m => m.name === name);
assert(mutation, 'named mutation');
const baseline = JSON.parse(fs.readFileSync(path.join(root, 'r109-frozen-source.json')));
