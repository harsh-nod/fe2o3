...
(async () => {
  ...
  const baseline = JSON.parse(fs.readFileSync(c.root + 'r109-frozen-source.json'));
  const planned = c.plan('r109-clock-restoration-' + name + '.json');
  const predecessor = c.predecessor('r109-clock-mut-' + name + '.json', 'finished_at', planned.expected);
  ...
  fs.writeFileSync(c.root + 'r109-clock-restoration-' + name + '.json', JSON.stringify(record, null, 2) + '\n', {flag: 'wx'});
  console.log('PASS: all ' + paths.length + ' source identities restored with observed UTC floor');
})().catch(error => {
  fs.writeFileSync(c.root + 'r109-clock-restoration-' + name + '-failure.json', JSON.stringify({accepted: false, error: String(error), clock_failure: error.clock_failure || null}, null, 2) + '\n', {flag: 'wx'});
