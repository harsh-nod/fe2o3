...
| T1 semantic map | elementwise, collective, and tiled kernels round-trip source to ISA and ISA to source; optimization shapes and hostile substitution tested | bounded collection inspection preserves a six-cell gfx942/gfx950 target and structural-count matrix plus typed unavailable/error outcomes and rejects hostile bytes; production ordinary-source evidence for all six elementwise/collective/tiled target cells, general schedule/LLVM/ISA intervals, and optimization-shape round trips remains incomplete |
