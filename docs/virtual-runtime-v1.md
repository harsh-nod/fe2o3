...
The V3 binary32 rows are compile-time exact IEEE input/result bit tables; the
oracle does not execute host floating-point operations. The matrix is
deterministic CPU semantic conformance, not hardware observation or CPU
performance prediction. Cross-width casts, wider/narrower float edge matrices,
and unsupported F32 functions remain outside its declared capability.
