...
`fe2o3-sim-runtime` adapts exact V3 and V4 bundles to the ordinary
...
padding. Adjusted, cast, indirect, pointer-containing, ambiguous, reordered,
expanded, unsupported wrapper, and non-global forms fail typed admission.
...
| C3 wave/workgroup | Wave32/Wave64 masks, ballot/any/all/integer shuffle, exact V9/V10 software-IEEE f32 XOR-butterfly sum/maximum and bit-preserving broadcast, static scalar LDS, barriers, integer atomics/fences, race/HB exploration, exact Wave64 16x16 XOR4 matrix LDS load/store, and exact V9/V10 gfx950 FP4/FP8 Current/Stage/Publish/Read transpose semantics. The transpose reads actual initialized LDS bytes, publishes through the normal workgroup epoch, retains schedule/debug evidence, and covers guarded overflow-to-zero behavior. An 8x8 LDS GEMM acceptance case uses only ordinary software-IEEE f32 multiply/add; it does not claim MFMA equivalence. | KIR requires full uniform waves and reports partial launch tails exactly. General divergence/reconvergence, scans, dynamic/general LDS, and remaining intrinsics stay typed unsupported. BF16 and scaled MFMA are rejected as `unsupported_numerical_contract` until an executable target contract fixes rounding, association, nonfinite, denormal, and signed-zero behavior; the simulator does not approximate those instructions. |
| C4 virtual runtime | This crate and CLI cover bounded allocation/copy/queue/dependency/dispatch/completion plus pre-publication cancellation, typed timeout ambiguity, early-release, quiesced recovery, and atomic full-generation reset. `fe2o3-sim-runtime` additionally runs exact admitted V3/V4 bundles through the normal typed `RuntimeContextV1` lifecycle, including event dependencies, output copyback, and V4 `Direct`/`Pair`/`Ignore` aggregate leaf materialization, without GPU probing or fallback | #182-defined multi-device plans and unsupported aggregate ABI forms remain outside this single-device adapter |
| C5 debugger/agent | Bounded JSONL simulator debugger, semantic scopes, replay/reverse inspection, race/source evidence, direct V3/V4 bundle admission, a one-shot agent-readable typed layout/region query with hostile correspondence substitution rejection, and exact canonical/seeded failure reduction with bounded canonical reports and context-bound reproducer replay. Public `sim --bundle-v4` runs compiler-flattened aggregate parameters | Broader query families and unsupported aggregate ABI forms remain incomplete |
| C6 differential | The bounded V1/V2 CPU harnesses retain their existing GPU-runtime-free contracts. The opt-in `fe2o3-sim-physical-differential` crate adds a single-use library state machine joining Bundle V4, the admitted V8-production-to-V7-simulator structural bridge, a canonical simulator request/schedule/limits snapshot, compiler-generated address-free argument packing, and a sealed observation minted only after authorized Worker V3 direct-KFD completion. Its agent-readable `physical-capabilities-v1` command reports the exact current blocker and zero hardware/parity passes; generated hosts query `differential_availability` on the exact prepared invocation. An unavailable run is 0 hardware/0 parity, while a completed mismatch is 1 hardware/0 parity. | The normal protected Worker V3 application verifier remains unwired, so no physical parity observation can currently be produced. Runtime failure or ambiguous completion cannot mint an observation and is not converted into a mismatch. The legacy handwritten LLVM vecadd fixture is excluded; unavailable hardware or authority never increments parity. Broad semantic-family physical qualification remains open. |
