...
| H3: [module load/unload](../crates/fe2o3-runtime/src/kfd_backend.rs), `load_module_v1`, `unload_module_v1`; [residency](../crates/fe2o3-runtime/src/kfd_backend/residency.rs) | Retained validated host image plus parser metadata, symbols, kernel records and materialization caches | Optional MEM-4A module-image payload accounting is implemented in development: reserve exact Rust byte capacity plus one record before copying/parsing; aliases retain one debit until final disposal. Defaults, parser/kernel metadata, generated/transport copies and aggregate domains remain unaccounted. Native materialization is separately N8. Failed unload is not a refund witness; native qualification remains open |
