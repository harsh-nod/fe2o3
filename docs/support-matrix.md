...
| Directional persistent local SDMA adapter | One promoted pooled or exact-size device buffer plus one ordinary host buffer on the exact gfx942 engine-1 H2D/engine-0 D2H pair | Experimental single-flight custody boundary | R19 binds the parent occurrence, distinct child IDs, pool and attachment generations, logical and physical extents, storage identities, logical copy ranges, and exact planned ticket. Exact frontier retirement is required before arbitrary repeated or mixed-direction reuse. Packet checks use bounded operational currentness and the single-request lower path is stack-sized. The packet cap remains `0x003f_ffe0` bytes; there is no batched large-copy, H2H, D2D, runtime-facade, hardware, executable-refinement, or performance-parity claim. Independent model/Verus totals are 305 obligations and 179 rejected mutations. |
