...
| Same-host multi-GPU peer copy | Exact admitted KFD devices | Experimental bounded lanes | Generic routing uses bounded host staging; a separate exact two-device copy-only backend retains native XGMI mappings and publishes a deterministic FIFO ready queue in directional prefixes of at most 63. Scalar flush is nonwaiting and requires the complete ready set to fit one window; overflow or a busy nonempty window rejects before publication. Scalar poll/wait do not publish. The separate [scalar progress method](runtime-xgmi-scalar-progress-v1.md) advances one FIFO publication, ticket observation or failed-dependent settlement, retaining the root independently of its dependencies. It does not add Context pending-producer reads or Worker transport. Opt-in background progress applies only through Send-capable Worker V4/V5 adapters; direct KFD remains thread-affine. Benchmark `outstanding_depth` is queued work on one ordered directional SDMA engine, not engine concurrency. |
...
| Two-native logical SDMA mux V2 | Exact gfx942 engine-0/engine-1 native pair and 2, 4, 8, 14, or 16 logical lanes | Experimental ordered mux custody boundary | One call admits 2 through 126 requests, stable-filters logical cursor order onto exactly two nonempty native shards of at most 63 requests, prepares both before publication, publishes once per native queue, waits on two exact tails, and retires only after a full original-order audit. Cursor commit requires both publications, closing currentness, successful live-model retake, and the restored owner; retake failure leaves terminal two-publication audit custody and no cursor advance. A lower-layer-classified first-shard no-effect failure returns the original ordered requests; any confirmed or indeterminate shard or currentness loss is terminal audit-only custody. Ordinary returned failures preserve these typed paths. For an unwind reaching the facade catch, the restoring helper resumes only into that catch, which poisons local and process authority and aborts. A nested unwind after wait moves submission ownership into the lower abort-only retirement suffix instead aborts immediately without any guarantee of explicit owner restoration or poisoning. Neither unwind route continues or returns typed custody. Logical lanes sharing a native queue gain cross-lane order, so this is not HIP stream independence, scheduling, priority, event, capture, or per-stream synchronization parity. Host fault and structural tests exist. One exact-source MI300X engineering screen validated the buffers and observed configuration-specific host-wall deltas, but the queue topology and ordering semantics differ, each configuration has one process replicate, and no causal speedup or HIP/HSA parity follows. Typed panic recovery, general hardware correctness, performance qualification, and formal refinement are unclaimed. |
...
| Persistent compute storage bridge | Exact full-allocation device-local storage on one primary gfx942 queue; either the R25 one-binding profile or the R57 exact three-binding Read/Read/Write profile; one fixed `WaitForPrior` packet | Experimental runtime-integrated bounded custody | R25 reuses one mapped HBM allocation through metadata-inspected compute without a second user-data allocation or host round trip. R57 additionally admits exactly two initialized read inputs A/B and one distinct initialized write-only output C with equal full extents on the same device and VM, and routes that shape through typed `RuntimeContextV1::launch`. All three inputs require authenticated H2D or fully initialized persistent replay custody. Write metadata alone is not full-write coverage authority, so a fresh uninitialized C rejects without changing custody. Recoverable clean prepublication rejection or explicit prepared cancellation restores all three owners. A terminal `RejectedBeforeSideEffect`, consuming native-preparation failure, or consuming unwind retains or quarantines all three and poisons authority even when publication was impossible; ambiguity after possible publication has the same terminal policy. Successful completion preserves A/B content generations, advances only C's effect/content generation, reports `PersistentDeviceReused` with zero user-data materializations, releases incompatible retained control, and keeps the device allocations reusable for a later exact transaction. A separately signature-bound, exact `hardware-qualification` lane exercised the intended uninitialized-C rejection and two-launch `A+B -> C`, `C+B -> D` sequence twice on one idle MI300X GPU, with identical full four-buffer readback digests and sole PASS records; see the [bounded evidence](evidence/mi300x-r57-n3-qualification-2026-09-08.md). This supplies native numerical evidence only for that fixed sequence. It does not provide generic binding arity, control replay for the three-binding shape, partial/padded ranges, auxiliary lanes, overlap, XGMI compute, a general full-write initialization certificate, Rust/native refinement, or a HIP/HSA performance or parity claim. The earlier R26-R31 single-binding measurements do not measure R57. |
