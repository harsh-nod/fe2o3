...
#[cfg(test)]
mod physical_row_split_controls {
    use super::*;
    use std::io::{BufReader, Cursor};
    #[test]
    fn every_snapshot_sized_split_reassembles_one_exact_line_without_new_transport() {
        let mut raw = b"=fe2o3-owned-one-stop-v2,s=\"11\",p=\"8\",reg=\"df9b5713\",mem=\"".to_vec();
        raw.extend(std::iter::repeat_n(b'a', 544));
        raw.extend_from_slice(b"\"\n");
        for split in 1..=raw.len() {
            let mut input = BufReader::with_capacity(split, Cursor::new(&raw));
            assert_eq!(
                line(&mut input, &ReadBudget::default()).unwrap(),
                Some(raw.clone())
            );
            assert_eq!(line(&mut input, &ReadBudget::default()).unwrap(), None);
        }
        raw.pop();
        assert!(
            line(
                &mut BufReader::with_capacity(1, Cursor::new(raw)),
                &ReadBudget::default()
            )
            .is_err()
        );
