...
fn snapshot_payload() -> String {
    ...
    for lane in 0..64_u32 {
        ...
        bytes[at..at + 4].copy_from_slice(&(0x1357_9bdf_u32 ^ lane).to_le_bytes());
    }
    bytes[264..].copy_from_slice(&0xfedc_ba98_7654_3210_u64.to_le_bytes());
    bytes.iter().map(|b| format!("{b:02x}")).collect()
}
fn row(s: u64, p: u64) -> Vec<u8> {
    let mut row = format!(
        "=fe2o3-owned-one-stop-v2,s=\"{s}\",p=\"{p}\",pid=\"4242\",i=\"1\",a=\"7\",t=\"1\",start=\"100\""
    );
    if p == 8 {
        row.push_str(&format!(",g=\"2\",w=\"3\",wg=\"4\",d=\"5\",q=\"6\",ag=\"7\",ar=\"8\",pc=\"2097232\",c=\"262144\",cp=\"327680\",co=\"9\",entry=\"2097152\",desc=\"2092992\",kernarg=\"393216\",out=\"458752\",regid=\"10\",dwarf=\"44\",rbytes=\"4\",mbytes=\"272\",status=\"available\",reg=\"df9b5713\",mem=\"{}\"",snapshot_payload()));
    }
    row.push('\n');
    row.into_bytes()
}
...
struct Fake {
    ...
    late_snapshot_parse: bool,
    checks_after_snapshot: Option<u8>,
}
impl Fake {
    fn new(lines: Vec<Vec<u8>>) -> Self {
        Self {
            ...
            late_snapshot_parse: false,
            checks_after_snapshot: None,
        }
    }
}
impl Peer for Fake {
    ...
    fn next(&mut self) -> Result<Option<Vec<u8>>, Refusal> {
        ...
        if self.late_snapshot_parse && line.as_ref().is_some_and(|x| *x == row(11, 8)) {
            self.checks_after_snapshot = Some(0);
        }
        Ok(line)
    }
    ...
    fn elapsed_ns(&mut self) -> Result<u64, Refusal> {
        if let Some(n) = &mut self.checks_after_snapshot {
            *n += 1;
            if *n == 3 {
                self.now = DEADLINE_NS;
            }
...
fn closed_positive_is_an_inert_observation_not_authority() {
    ...
    assert!(r.later_target_4096_output_validation_observed);
    assert!(!r.post_completion_raw_bytes_recaptured);
...
fn every_transition_row_is_required() {
    ...
    for i in 0..baseline.len() {
        if baseline[i].starts_with(b"=fe2o3-owned-one-stop-v2") {
