...
Error validateCompilerImports(const Request &RequestValue,
...
inspectNormalizedCompilerModule(const Request &RequestValue,
  ...
  if (Error E = validateCompilerImports(RequestValue, IrSymbols, ""))
    return E;
  if (Error E =
          validateCompilerImports(RequestValue, EmittedSymbols, "object"))
    return E;
...
inspectOutput(ArrayRef<uint8_t> Bytes, const ElfContract &Expected,
  ...
  if (StaticPublicDefinitions != ExpectedSymbols)
    return pipelineError(
        Twine("post_link.check=static_exports status=failed expected=") +
  ...
  if (!ExpectedDescriptors.empty() &&
    ...
    for (const KernelLaunchContract &Kernel : Metadata->Kernels) {
      ...
      if (ExpectedLaunch == CompilerLaunchContracts->end())
        return pipelineError(
            Twine("post_link.check=compiler_launch_contract status=failed "
                  "kernel=") +
            diagnosticAtom(Kernel.Name) + " field=compiler_kernel_presence");
      if (!Kernel.RequiredWorkgroupSize ||
        ...
        return pipelineError(
            Twine("post_link.check=compiler_launch_contract status=failed "
                  "kernel=") +
            diagnosticAtom(Kernel.Name) + " field=reqd_workgroup_size");
      if (Kernel.MaxFlatWorkgroupSize !=
          ExpectedLaunch->second.MaxFlatWorkgroupSize)
        return pipelineError(
            Twine("post_link.check=compiler_launch_contract status=failed "
                  "kernel=") +
            diagnosticAtom(Kernel.Name) + " field=max_flat_workgroup_size");
