// Opt-in normal-worker native observation of unchanged V21-emitted LLVM.
...
namespace {
#include "PhysicalGlobalCopyInput.inc"
#include "PhysicalGlobalCopyMetadata.inc"
#include "PhysicalGlobalCopyRelation.inc"
#include "PhysicalGlobalCopyControls.inc"
void globalCopyPhysicalReport(json::Object Report) {
...
int main(int Argc,char **Argv) {
  alarm(90);
  require(Argc==5,"usage: physical-global-copy-canonical-candidate ABS_LLVM ABS_EXPECTATION O0|O3 ABS_FRESH_DIRECTORY");
  ...
  auto Parsed=parseGlobalCopyInput(Sidecar.Bytes);require(Parsed.has_value(),"canonical physical expectation grammar");
  require(globalCopyInputJoin(*Parsed,Llvm),"canonical expectation/unchanged LLVM content join");
  auto Expected=globalCopyPhysicalRows(*Parsed);require(Expected.has_value(),"closed canonical primitive/native row profile");
  const auto Level=LevelName=="O0"?OptimizationLevel::O0:OptimizationLevel::O3;
  auto SidecarNegatives=globalCopyPhysicalSidecarNegatives(Sidecar.Bytes,Llvm);
  ...
  auto InputNegatives=globalCopyPhysicalInputNegatives(Llvm,Parsed->Entry,Level);
  ...
  if(auto Failure=globalCopyPhysicalRelation(*Parsed,*Expected,Built.Evidence,Payload))fail(*Failure);
  auto Metadata=globalCopyMetadata(Payload,Parsed->Entry);require(Metadata.has_value(),"canonical exact four/thirteen ABI metadata");
  ...
  auto Mutations=globalCopyPhysicalMutations(*Parsed,*Expected,Built,*Descriptor);
  ...
  globalCopyPhysicalReport(json::Object{
    ...
    {"runtime_pointer_validity_proved",false},{"runtime_input_bounds_proved",false},
    {"runtime_output_bounds_proved",false},{"initialized_input_proved",false},
    {"host_input_output_disjointness_proved",false},{"live_rust_source_abi_qualified",false},{"owner_contract_accepted",false},{"milestone_completion",false}});
  alarm(0);return 0;
}
