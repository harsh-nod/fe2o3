...
export const RELATION_PREFIX_V22 = "FE2O3_PHYSICAL_LDS_EXCHANGE_HANDOFF_RELATION_V22 ";
const schema = "fe2o3-physical-lds-exchange-inert-handoff-relation-v22";
...
const keys = ["schema", ...digests, ...unavailable, "lds_bytes", "workgroup_participants_required", "lds_execution_observed"].sort();
const hash = bytes => createHash("sha256").update(bytes).digest("hex");
export function observedHandoffRelationV22(stderr, canonicalLLVM, handoff) {
  ...
  const rows = stderr.split("\n").filter(line => line.startsWith(RELATION_PREFIX_V22));
  assert.equal(rows.length, 1, "exactly one V22 diagnostic handoff relation");
  const raw = rows[0].slice(RELATION_PREFIX_V22.length);
  ...
  assert.equal(row.lds_bytes, 512);
  assert.equal(row.workgroup_participants_required, 128);
  assert.equal(row.lds_execution_observed, false);
