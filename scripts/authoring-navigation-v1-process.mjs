// Private bounded stdin transport for the navigation capture, not compiler authority.
import { spawn } from 'node:child_process';

export function runNavigationCommand({ executable, args, cwd, env, input = Buffer.alloc(0),
  ...
  if (!Buffer.isBuffer(input) || input.length > 4 * 1024 * 1024
    ...
    throw new Error('navigation process bounds');
  }
