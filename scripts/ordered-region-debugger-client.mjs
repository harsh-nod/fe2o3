...
export const BOUNDS = Object.freeze({
  milliseconds: 30000, drainMilliseconds: 1000, commands: 192, lineBytes: 262144,
  stdoutBytes: 8388608, stderrBytes: 65536, requestBytes: 1048576,
  kirBytes: 1048576, simulationRequestBytes: 65536, executableBytes: 536870912,
  resourceItems: 16, resourceScanned: 128, resourcePages: 64,
});
...
/** Owns one capped byte buffer, never incoming chunk views or their backings.
 * Each explicit snapshot is a separate owned allocation bounded by its cap;
 * these storage bounds are not a combined process/RSS claim.
 */
export class BoundedByteCapture {
  ...
  #maximum;
  constructor(maximum) {
    integer(maximum, BOUNDS.stdoutBytes);
    this.#maximum = maximum;
    this.#storage = Buffer.allocUnsafeSlow(maximum);
  }
  get byteLength() { return this.#bytes; }
  get backingByteLength() { return this.#storage.buffer.byteLength; }
  append(chunk) {
    assert(chunk instanceof Uint8Array, 'byte chunk required');
    const copied = Math.min(chunk.byteLength, this.#maximum - this.#bytes);
    if (copied > 0) this.#storage.set(chunk.subarray(0, copied), this.#bytes);
    this.#bytes += copied;
    return chunk.byteLength > copied;
  }
  snapshot(maximum) {
    integer(maximum, this.#maximum);
    assert(this.#bytes <= maximum, 'snapshot byte bound');
    const result = Buffer.allocUnsafeSlow(this.#bytes);
    this.#storage.copy(result, 0, 0, this.#bytes);
    return result;
  }
}

export function parseExactJson(raw) {
  assert(typeof raw === 'string' && Buffer.byteLength(raw) <= BOUNDS.lineBytes, 'JSON line bound');
  const result = JSON.parse(raw, (_key, value, context) => {
    if (typeof value !== 'number') return value;
    assert(context && /^(?:0|[1-9][0-9]*)$/.test(context.source), 'noncanonical unsigned JSON integer');
    if (Number.isSafeInteger(value)) return value;
    const exact = BigInt(context.source);
    assert(exact <= 0xffffffffffffffffn, 'JSON integer exceeds u64');
    return exact;
  });
  let count = 0;
  ...
  while (queue.length) {
    ...
    assert(++count <= 32768 && depth <= 32, 'JSON graph bound');
    if (value && typeof value === 'object') {
      const children = Object.values(value);
      assert(children.length <= 4096, 'JSON collection bound');
      for (const child of children) queue.push([child, depth + 1]);
    }
  }
  return result;
}
export function encodeExactJson(value) {
  assert.equal(typeof JSON.rawJSON, 'function', 'Node22 lossless JSON required');
  return JSON.stringify(value, (_key, item) => {
    if (typeof item === 'bigint') {
      assert(item >= 0n && item <= 0xffffffffffffffffn, 'JSON u64 bound');
      return JSON.rawJSON(item.toString());
    }
    if (typeof item === 'number') integer(item);
    return item;
  });
}
...
class OrdinaryClient {
  constructor(binary, args) {
    ...
    assert.equal(process.platform, 'linux', 'ordered-region smoke process-group policy requires Linux');
...
export function validateResourceRequestRevision(request) {
  equal(request.expected_revision, request.expected_snapshot.cursor.state_revision,
...
async function exercise(client, e) {
  ...
  const breakpointSpec = { client_label: 'whole-region-before', enabled: true,
  ...
  assert(diagnosis.error.message.includes('cannot represent raw canonical KIR V16'));
  ...
  equal(observations.after.cursor.event_sequence, observations.before.cursor.event_sequence + 1, 'whole-region pair not adjacent');
...
export async function runDebuggerQualification(options) {
  ...
  try {
    ...
    const args = ['sim', '--diagnostic-kir-v16', o.kirPath, '--request', o.requestPath, '--wave-width', '64', '--protocol', 'jsonl'];
    ...
    report = {
      schema: 'fe2o3-ordered-region-debugger-observation-v1', ...report,
