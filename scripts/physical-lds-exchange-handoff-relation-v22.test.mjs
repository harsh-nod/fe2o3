...
import { observedHandoffRelationV22 as observe, RELATION_PREFIX_V22 as prefix } from "./physical-lds-exchange-handoff-relation-v22.mjs";
...
const example = () => ({
  ...
  lds_bytes: 512, workgroup_participants_required: 128, lds_execution_observed: false,
...
test("frame/publication declarations remain exact and are not execution observations", () => {
  for (const change of [{lds_bytes: 0}, {lds_bytes: 508}, {workgroup_participants_required: 64}, {lds_execution_observed: true}])
    assert.throws(() => observe(line({...example(), ...change}), llvm, handoff));
});
