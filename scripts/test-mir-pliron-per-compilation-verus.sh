...
readonly verus_args=(--crate-type lib --triggers-mode silent --no-cheating --num-threads 1)
...
printf '%s  %s\n' \
    ...
    18e00e30529a50beb541b573ed30289e3c74ca9c526ee2d49a89a5d25ed4b5a4 "$generated" \
    3a2b9e22ef4ad01a7bd2fab605fe7c38f408de08afefe99b64cab1ca2a5bc404 "$generated_multi" \
    ...
    08f364ebe3ac969c8bf5d731ad2c111e2490137abb7b3074716529c7d2bd9dea "$negative_dir/mir_pliron_per_compilation_multi_output_substitution_v1.rs" \
    | sha256sum -c -
...
do
    ...
    if timeout "$timeout_seconds" "$verus" "${verus_args[@]}" "$file" \
...
if timeout "$timeout_seconds" "$verus" "${verus_args[@]}" "$multi_substitution" \
