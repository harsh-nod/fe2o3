...
printf '%s  %s\n' \
    ...
    25ac7ce744e3d489e8cd4c89489abbedc08484a53d6440dfdef4434d1bba3559 "$generated" \
...
for token in 'assume(' 'admit(' '#[verifier::external_body]' '#[verifier::external]'; do
    if grep -F "$token" "$template" "$generated" >/dev/null; then
...
grep -F 'verification results:: 8 verified, 0 errors' "$tmp/generated.log" >/dev/null
...
printf 'per-compilation MIR/PLIRON composition: 4 template and 4 generated obligations, 2 expected rejections\n'
