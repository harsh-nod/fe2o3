...
if (process.argv.length !== 4 || !Object.hasOwn(cases, process.argv[2])) {
  throw new Error("usage: node scripts/physical-global-copy-source-v21.mjs one|registers|wrong-launch|missing-lgkm|missing-vm|wrong-carry|wrong-store|wrong-offset|reserved-register NEW_OUTPUT_DIRECTORY");
}
...
try {
  ...
  saveJson("inputs.json", { schema: "fe2o3-physical-global-copy-source-inputs-v21",
  ...
  for (const stage of ["diagnostic"]) {
    ...
    const envName = "FE2O3_EXTRACT_DIAGNOSTIC_PHYSICAL_GLOBAL_COPY_DIRECTORY_V21";
    ...
    if (refusal) {
      ...
      assert.ok(stderr.includes("actual Rust -> MIR38 -> pre-ranked exact KIR21"));
      assert.ok(stderr.includes("ranked/formal/descriptor continuation unavailable"));
      assert.deepEqual(readdirSync(path).sort(), ["canonical-v21.bin", "canonical.ll", "native-observation-input-v21.txt"].sort());
      observations.at(-1).outputs = [
        ["canonical-v21.bin", MiB], ["canonical.ll", 64 * 1024], ["native-observation-input-v21.txt", 32 * 1024]
      ...
      const sidecar = new TextDecoder("utf-8",{fatal:true}).decode(readBounded(join(path,"native-observation-input-v21.txt"),32*1024));
      const canonical = readBounded(join(path,"canonical-v21.bin"),MiB);
      assert.ok(canonical.length>0 && llvm.includes("asm sideeffect") && llvm.includes("unreachable"));
      assert.ok(!llvm.includes("ret void"));
      assert.ok(sidecar.startsWith("FE2O3_PHYSICAL_GLOBAL_COPY_V21_NATIVE_OBSERVATION_INPUT_V1\n"));
  ...
  saveJson("receipt.json", { schema: "fe2o3-physical-global-copy-source-command-v21", feature, observations,
  ...
  console.log("Physical global-copy pre-ranked source " + (refusal ? "exact refusal" : "inert KIR/LLVM/native-observation diagnostics") + " observed: " + output);
} catch (error) {
  saveJson("failure.json", { schema: "fe2o3-physical-global-copy-source-command-failure-v21", error: String(error),
