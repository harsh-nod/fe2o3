...
import { observedHandoffRelationV21 } from "./physical-global-copy-handoff-relation-v21.mjs";
...
const cases = {
  "one": ["physical-global-copy-one-v21", null],
  "registers": ["physical-global-copy-registers-v21", null],
  "wrong-launch": ["physical-global-copy-wrong-launch-v21", "physical-global-copy requires exact authored launch64 and max_grid2"],
  "missing-lgkm": ["physical-global-copy-missing-lgkm-v21", "global copy requires exact initial kernarg loads/wait"],
  "missing-vm": ["physical-global-copy-missing-vm-v21", "global copy read requires immediate VM wait"],
  "wrong-carry": ["physical-global-copy-wrong-carry-v21", "global copy pointer high provenance"],
  "wrong-store": ["physical-global-copy-wrong-store-v21", "global copy store must consume actual loaded U32"],
  "wrong-offset": ["physical-global-copy-wrong-offset-v21", "global copy repeated kernarg component"],
  "reserved-register": ["physical-global-copy-reserved-register-v21", "physical-global-copy instruction descriptor is outside the closed profile"],
};
if (process.argv.length !== 4 || !Object.hasOwn(cases, process.argv[2])) {
  throw new Error("usage: node scripts/physical-global-copy-checked-v21.mjs one|registers|wrong-launch|missing-lgkm|missing-vm|wrong-carry|wrong-store|wrong-offset|reserved-register NEW_OUTPUT_DIRECTORY");
}
...
const bin = resolve(process.env.FE2O3_PHYSICAL_GLOBAL_COPY_BIN_DIR_V21 ??
...
const observations = [];
const sources = [
  "Cargo.toml", "src/lib.rs", "src/physical_global_copy_v21.rs",
...
try {
  ...
  saveJson("inputs.json", { schema: "fe2o3-physical-global-copy-checked-source-inputs-v21",
  ...
  for (const stage of ["llvm", "handoff"]) {
    ...
    if (refusal) {
      ...
      assert.ok(stderr.includes("authenticated Rust -> MIR38 -> ranked/formal checked KIR21"));
      for (const condition of ["full-EXEC", "initialized 128-u32 prefix (512 bytes)", "output requires 512 writable bytes", "input/output and output/kernarg disjointness remain runtime obligations", "live readable immutable 32-byte prefix aligned to 8 bytes"]) assert.ok(stderr.includes(condition));
      ...
      if (stage === "llvm") {
        ...
        const relation = observedHandoffRelationV21(stderr, readBounded(join(output,"canonical.ll"),64*1024), readBounded(path,MiB));
        observations.at(-1).diagnostic_relation = relation;
        saveJson("inert-handoff-relation.json", relation);
      }
    }
    observeFreshDisk();
  ...
  saveJson("receipt.json", { schema: "fe2o3-physical-global-copy-checked-source-command-v21", feature, observations,
    ...
    runtime_abi_conditions: "input full-EXEC 128-u32 prefix readable/initialized; both global minima 512 bytes; input/output and output/kernarg disjoint; kernarg live/readable/immutable32B aligned8",
    public_handoff_relation: refusal === null ? "observed diagnostic hashes match unchanged canonical LLVM and serialized inert handoff" : "no output",
  ...
  console.log("Physical global-copy checked source " + (refusal ? "exact refusal" : "normal inert LLVM/handoff; runtime ABI conditions unresolved") + " observed: " + output);
} catch (error) {
  saveJson("failure.json", { schema: "fe2o3-physical-global-copy-checked-source-command-failure-v21", error: String(error),
