...
fn matrix_module() -> Module {
    ...
    let parameters = vec![
        pointer(bf16.clone(), AccessMode::ReadOnly),
        pointer(bf16.clone(), AccessMode::ReadOnly),
        pointer(Type::F32, AccessMode::ReadWrite),
        Type::F32,
        Type::F32,
        Type::F32,
        Type::F32,
    ...
    let load_a = MatrixOperation::lds_load(ValueId(0), MatrixElement::Bf16);
    let load_b = MatrixOperation::lds_load(ValueId(1), MatrixElement::Bf16);
    let mma = MatrixOperation::multiply_accumulate(
        ...
        [ValueId(3), ValueId(4), ValueId(5), ValueId(6)],
    );
...
fn operation_mut(module: &mut Module, index: usize) -> &mut MatrixOperation {
    ...
    let OperationKind::Matrix(matrix) = &mut body.blocks[0].operations[index].kind else {
...
fn exact_matrix_and_lds_profiles_verify_with_explicit_effects() {
    ...
    assert_eq!(
        operations[0].memory_effects(),
        vec![MemoryEffect::Read(AddressSpace::Workgroup)]
    );
    assert!(operations[2].effect_summary().is_pure());
    assert_eq!(
        operations[3].memory_effects(),
...
fn pointer_value_and_result_type_mutations_fail_closed() {
    ...
    global.functions[0].signature.parameters[0] = Type::pointer(
    ...
    read_only_store.functions[0].signature.parameters[2] = pointer(Type::F32, AccessMode::ReadOnly);
    assert!(
    ...
    wrong_accumulator.functions[0].signature.parameters[3] = Type::Scalar(ScalarType::Bf16);
    assert!(
    ...
    wrong_result.functions[0].body.as_mut().unwrap().blocks[0].operations[2].results[0].ty =
        Type::Scalar(ScalarType::Bf16);
    assert!(
