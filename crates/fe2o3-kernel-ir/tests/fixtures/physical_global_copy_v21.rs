...
use super::physical_global_copy_fixture_ir::*;
...
pub(crate) fn site(n: u8) -> Site {
    Site {
        ...
        semantic_block_index: 33 - u32::from(n),
        semantic_block_identity: [n + 1; 32],
        semantic_callable_index: 7,
    }
}
pub(crate) const ROWS: [(Opcode, u8, u8, u8, u32); 23] = [
    ...
    (Opcode::GlobalStoreDword, 0, 10, 8, 0),
    (Opcode::WaitVm0, 0, 0, 0, 0),
    (Opcode::RestoreExec, 0, 18, 0, 0),
];
pub(crate) fn module() -> Module {
    module_with_extra_zero(false)
}
pub(crate) fn module_with_extra_zero(extra: bool) -> Module {
    ...
    if extra {
        rows.insert(8, (Opcode::VectorMove32, 30, 255, 0, 0));
    }
    for (ordinal, (opcode, d, a, b, immediate)) in rows.into_iter().enumerate() {
        let instruction = Gfx942PhysicalGlobalCopyInstructionV1 {
            opcode,
            destination: d,
            source0: a,
            source1: b,
        ...
        block.operations.push(Operation::new(
            results,
            OperationKind::Gfx942PhysicalGlobalCopyStep(Gfx942PhysicalGlobalCopyStepV1 {
    ...
    let declaration = Gfx942PhysicalGlobalCopyDeclarationV1 {
    ...
    let mut module = Module::new("inert_physical_global_copy_fixture");
