//! Exact immutable V17 custody through the shared allocation-budgeted verifier.
...
pub const VERIFIED_CANONICAL_KERNEL_IR_V17_IDENTITY_DOMAIN_V1: &[u8] =
    b"FE2O3/VERIFIED-CANONICAL-KERNEL-IR/V17\0";
pub const VERIFIED_CANONICAL_KERNEL_IR_V17_IDENTITY_POLICY_V1: u16 = 1;

#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct VerifiedCanonicalKernelIrIdentityV17 {
    digest: [u8; 32],
    canonical_length: u64,
}
impl VerifiedCanonicalKernelIrIdentityV17 {
...
/// Read-only canonical bytes retained inseparably by the V17 module owner.
#[derive(Debug, Eq, PartialEq)]
pub struct VerifiedCanonicalKernelIrV17 {
    canonical_bytes: Vec<u8>,
    identity: VerifiedCanonicalKernelIrIdentityV17,
}
impl VerifiedCanonicalKernelIrV17 {
    pub fn canonical_bytes(&self) -> &[u8] {
        &self.canonical_bytes
    }
    pub const fn identity(&self) -> &VerifiedCanonicalKernelIrIdentityV17 {
        &self.identity
    }
}

/// Move-only custody of one exact V17 encoding and the actual freshly decoded,
...
/// requires_clone::<VerifiedCanonicalKernelIrModuleV17>();
/// ```
/// ```compile_fail
/// use fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV17;
/// fn mutate(owner: &mut VerifiedCanonicalKernelIrModuleV17) { owner.module().functions.clear(); }
/// ```
/// ```compile_fail
/// use fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV17;
/// fn separate(owner: VerifiedCanonicalKernelIrModuleV17) { let _ = owner.into_parts(); }
/// ```
#[derive(Debug, Eq, PartialEq)]
pub struct VerifiedCanonicalKernelIrModuleV17 {
    canonical: VerifiedCanonicalKernelIrV17,
    module: Module,
}

impl VerifiedCanonicalKernelIrModuleV17 {
    ...
    pub fn from_module_ref_with_verification_budget_v17(
        module: &Module,
        budget: &mut CanonicalKernelIrVerificationResourceBudgetV1<'_>,
    ) -> Result<(Self, CanonicalKernelIrReplayStorageV17), CanonicalKernelIrReplayAdmissionErrorV17>
    {
        let floor = budget.storage_checkpoint();
        let result = (|| {
            let inverse_inline = std::mem::size_of::<Self>()
                .checked_sub(std::mem::size_of::<VerifiedCanonicalKernelIrV17>())
                .ok_or(CanonicalKernelIrVerificationResourceErrorV1::Arithmetic)?;
            let inverse = verified_inverse(
                module,
                budget,
                Profile::V17,
                std::mem::size_of::<VerifiedCanonicalKernelIrV17>(),
                inverse_inline,
            )
            .map_err(CanonicalKernelIrReplayAdmissionErrorV17::from_inverse)?;
            ...
            Ok((
                ...
                CanonicalKernelIrReplayStorageV17 { retained },
            ))
        })();
    ...
    pub fn from_canonical_bytes_with_verification_budget_v17(
        bytes: &[u8],
        budget: &mut CanonicalKernelIrVerificationResourceBudgetV1<'_>,
    ) -> Result<(Self, CanonicalKernelIrReplayStorageV17), CanonicalKernelIrReplayAdmissionErrorV17>
    {
        ...
        let result = (|| {
            ...
            let module = crate::wire::decode_module_v17_with_allocation_budget_v1(bytes, budget)
                .map_err(CanonicalKernelIrReplayAdmissionErrorV17::Decode)?;
            verify_exact_decoded_module_with_budget_v1(&module, None, budget).map_err(|error| {
                match error {
                    MeteredKernelIrVerificationErrorV1::Verification(error) => {
                        CanonicalKernelIrReplayAdmissionErrorV17::Verification(error)
                    }
                    MeteredKernelIrVerificationErrorV1::Resource(error) => {
                        CanonicalKernelIrReplayAdmissionErrorV17::Resource(error)
                    }
                }
            })?;
    ...
    pub const fn canonical(&self) -> &VerifiedCanonicalKernelIrV17 {
        &self.canonical
    }
    pub fn canonical_bytes(&self) -> &[u8] {
...
pub struct CanonicalKernelIrReplayStorageV17 {
    retained: usize,
}
impl CanonicalKernelIrReplayStorageV17 {
...
#[derive(Debug)]
pub enum CanonicalKernelIrReplayAdmissionErrorV17 {
...
impl CanonicalKernelIrReplayAdmissionErrorV17 {
...
impl From<CanonicalKernelIrVerificationResourceErrorV1>
    for CanonicalKernelIrReplayAdmissionErrorV17
{
    fn from(error: CanonicalKernelIrVerificationResourceErrorV1) -> Self {
        Self::Resource(error)
    }
}
impl fmt::Display for CanonicalKernelIrReplayAdmissionErrorV17 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Encode(error) => write!(f, "V17 canonical encode failed: {error}"),
            Self::Decode(error) => write!(f, "V17 canonical decode failed: {error}"),
            Self::Verification(error) => error.fmt(f),
            Self::Resource(error) => error.fmt(f),
            Self::CanonicalMismatch => f.write_str("V17 inverse differs from its exact subject"),
        }
    }
}
impl Error for CanonicalKernelIrReplayAdmissionErrorV17 {}

fn identity(
    bytes: &[u8],
    budget: &mut CanonicalKernelIrVerificationResourceBudgetV1<'_>,
) -> Result<VerifiedCanonicalKernelIrIdentityV17, CanonicalKernelIrReplayAdmissionErrorV17> {
    let domain = VERIFIED_CANONICAL_KERNEL_IR_V17_IDENTITY_DOMAIN_V1;
    ...
    hash.update(domain);
    hash.update(VERIFIED_CANONICAL_KERNEL_IR_V17_IDENTITY_POLICY_V1.to_le_bytes());
    hash.update(length.to_le_bytes());
    hash.update(bytes);
    Ok(VerifiedCanonicalKernelIrIdentityV17 {
...
#[cfg(test)]
#[path = "canonical_kir_v17_tests.rs"]
mod tests;
