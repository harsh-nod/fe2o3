...
#[test]
fn base_getters_reject_same_slot_replacement_and_allow_restored_reentry() {
    let module = slot();
    ...
    for query in 0..2 {
        ...
        with_checked_local_frame_function_v1(verified, 0, &mut budget, |checked, budget| {
            swapped_query(budget, foreign, |budget| {
                if query == 0 {
                    checked.allocations(budget).map(|_| ())
                } else {
                    checked.accesses(budget).map(|_| ())
                }
            })
        })
        .unwrap();
...
#[test]
fn base_no_getter_replacement_precedes_callback_results_and_foreign_cleanup() {
    let module = slot();
    let verified = verify_module_ref(&module).unwrap();
    for mode in 0..3 {
        let mut original_work = CanonicalKernelIrWorkBudgetV1::new(usize::MAX);
        ...
        let outcome = catch_unwind(AssertUnwindSafe(|| {
            with_checked_local_frame_function_v1(verified, 0, &mut budget, |_, budget| {
                entered = true;
                full = budget.storage();
...
#[test]
fn base_entry_and_foreign_getter_have_isolated_work_first_boundaries() {
    let module = slot();
    let verified = verify_module_ref(&module).unwrap();
    let header_total = FLOOR + header_bytes();
    for limit in [3, 4] {
    ...
    with_checked_local_frame_function_v1(verified, 0, &mut budget, |checked, budget| {
        ...
        for limit in [4, 5] {
            ...
            let result = checked.allocations(&mut foreign);
            if limit == 4 {
                assert!(matches!(result,
                    Err(LocalFrameErrorV1::Resource(ResourceError::Work(error)))
