...
fn derive_alias_requirements(
    ...
    for index in 0..entries.len() {
        ...
        if count != 0 && entries[count - 1].0 == allocation {
            ...
            envelope.range = match (envelope.range, next.range) {
                (FormalAliasRegionV1::FixedBytes(a), FormalAliasRegionV1::FixedBytes(b)) => {
                    FormalAliasRegionV1::FixedBytes(FormalByteRange {
                        start: a.start.min(b.start),
                        end_exclusive: a.end_exclusive.max(b.end_exclusive),
                    })
                }
                _ => FormalAliasRegionV1::WholeFormalAllocation,
            };
