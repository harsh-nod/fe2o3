...
use crate::{
    ...
    ValueDef, ValueId, WorkgroupSize, decode_module_v13,
};

fn bundle() -> VerifiedSimulationBundleV8 {
    let mut module = crate::Module::new("bundle_v8_test");
    ...
    .unwrap();
    let semantic = b"exact-production-semantic-mir-v8-fixture".to_vec();
...
#[test]
fn v8_round_trips_exact_v13_with_pointer_restriction_without_authority() {
    let bundle = bundle();
    let decoded =
        VerifiedSimulationBundleV8::from_canonical_bytes(bundle.canonical_bytes().to_vec())
            .unwrap();
    ...
    assert_eq!(decoded.canonical_kir_v13()[8..10], 13_u16.to_le_bytes());
...
#[test]
fn v8_round_trips_exact_v13_without_semantic_drift() {
    let bundle = bundle();
    let decoded =
        VerifiedSimulationBundleV8::from_canonical_bytes(bundle.canonical_bytes().to_vec())
            .unwrap();
    assert_eq!(decoded.production_kir_identity().version(), 13);
    let module = decode_module_v13(decoded.canonical_kir_v13()).unwrap();
    let production = VerifiedCanonicalKernelIrV13::from_module(module).unwrap();
    assert_eq!(
...
#[test]
fn v8_identity_binds_epoch_and_exact_context_global_bytes() {
    let first = bundle();
    let second = bundle();
    ...
    assert!(matches!(
        VerifiedSimulationBundleV8::from_canonical_bytes(stale_epoch),
        Err(SimulationBundleErrorV8::SubjectIdentityMismatch)
    ));

    let mut zero_epoch = first.canonical_bytes().to_vec();
    zero_epoch[56..64].fill(0);
    assert!(matches!(
        VerifiedSimulationBundleV8::from_canonical_bytes(zero_epoch),
        Err(SimulationBundleErrorV8::InvalidFinalGraphEpoch)
    ));
    ...
    for marker in [[0x44_u8; 32], [0x48_u8; 32]] {
        ...
        let offset = mutated
            ...
            .expect("context/global marker is retained in exact V13 bytes");
        assert!(offset >= first.kir_range.start && offset < first.kir_range.end);
        mutated[offset] ^= 1;
        assert!(VerifiedSimulationBundleV8::from_canonical_bytes(mutated).is_err());
    }
}

#[test]
fn v8_rejects_version_bridge_section_and_trailing_substitution() {
    let bundle = bundle();
    ...
    assert!(matches!(
        VerifiedSimulationBundleV8::from_canonical_bytes(wrong_production_version),
        Err(SimulationBundleErrorV8::InvalidProductionKirIdentity)
    ));
    ...
    assert!(matches!(
        VerifiedSimulationBundleV8::from_canonical_bytes(wrong_canonical_version),
        Err(SimulationBundleErrorV8::UnsupportedCanonicalKirVersion(9))
    ));

    let mut exact_v12_body = bundle.canonical_bytes().to_vec();
    exact_v12_body[bundle.kir_range.start + 8..bundle.kir_range.start + 10]
        .copy_from_slice(&12_u16.to_le_bytes());
    assert!(matches!(
        VerifiedSimulationBundleV8::from_canonical_bytes(exact_v12_body),
        Err(SimulationBundleErrorV8::CanonicalKir(
            crate::VerifiedCanonicalKernelIrErrorV13::NotExactV13 { version: 12 }
        ))
    ));
    ...
    for offset in [
        ...
        hostile[offset] ^= 1;
        assert!(VerifiedSimulationBundleV8::from_canonical_bytes(hostile).is_err());
    }
    let mut trailing = bundle.canonical_bytes().to_vec();
    trailing.push(0);
    assert!(matches!(
        VerifiedSimulationBundleV8::from_canonical_bytes(trailing),
        Err(SimulationBundleErrorV8::TrailingOrMissingBytes)
    ));
}

#[test]
fn v8_map_decoders_reject_unknown_fields_and_subject_substitution() {
    let bundle = bundle();
    ...
    assert!(matches!(
        SemanticStorageMapV8::from_canonical_json_bytes(&serde_json::to_vec(&storage).unwrap()),
        Err(SimulationBundleErrorV8::InvalidStorageMap)
    ));

    let mut hostile: SemanticAggregateStorageMapV8 =
        serde_json::from_slice(bundle.aggregate_storage_map()).unwrap();
    hostile.bundle_subject_identity = [0x11; 32];
    let hostile = SemanticAggregateStorageMapV8::from_canonical_json_bytes(
    ...
    hostile_bundle[352..384]
        .copy_from_slice(&domain_hash(AGGREGATE_MAP_IDENTITY_DOMAIN_V8, &hostile_map));
    assert!(matches!(
        VerifiedSimulationBundleV8::from_canonical_bytes(hostile_bundle),
        Err(SimulationBundleErrorV8::StorageMapBindingMismatch)
    ));
}

#[test]
fn v8_aggregate_map_reuses_the_exact_v2_hostile_layout_boundary() {
    ...
    let map = |components| {
        SemanticAggregateStorageMapV8::new(
            [0x31; 32],
            [0x32; 32],
            123,
            vec![SemanticKernelStorageV2::new(
    ...
    SemanticAggregateStorageMapV8::from_canonical_json_bytes(
    ...
    for hostile in [
        ...
        assert!(matches!(
            map(hostile),
            Err(SimulationBundleErrorV8::InvalidAggregateStorageMap)
        ));
    }
    assert!(matches!(
        SemanticAggregateStorageMapV8::new([0; 32], [0x32; 32], 123, exact.kernels().to_vec(),),
        Err(SimulationBundleErrorV8::InvalidAggregateStorageMap)
    ));
    assert!(matches!(
        SemanticAggregateStorageMapV8::new([0x31; 32], [0; 32], 123, exact.kernels().to_vec(),),
        Err(SimulationBundleErrorV8::InvalidAggregateStorageMap)
    ));
    assert!(matches!(
        SemanticAggregateStorageMapV8::new([0x31; 32], [0x32; 32], 0, exact.kernels().to_vec(),),
        Err(SimulationBundleErrorV8::InvalidAggregateStorageMap)
    ));
}
