//! Exact immutable V16 custody through the shared allocation-budgeted verifier.
...
pub const VERIFIED_CANONICAL_KERNEL_IR_V16_IDENTITY_DOMAIN_V1: &[u8] =
    b"FE2O3/VERIFIED-CANONICAL-KERNEL-IR/V16\0";
pub const VERIFIED_CANONICAL_KERNEL_IR_V16_IDENTITY_POLICY_V1: u16 = 1;

#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct VerifiedCanonicalKernelIrIdentityV16 {
    digest: [u8; 32],
    canonical_length: u64,
}
impl VerifiedCanonicalKernelIrIdentityV16 {
...
/// Read-only canonical bytes retained inseparably by the V16 module owner.
#[derive(Debug, Eq, PartialEq)]
pub struct VerifiedCanonicalKernelIrV16 {
    canonical_bytes: Vec<u8>,
    identity: VerifiedCanonicalKernelIrIdentityV16,
}
impl VerifiedCanonicalKernelIrV16 {
    pub fn canonical_bytes(&self) -> &[u8] {
        &self.canonical_bytes
    }
    pub const fn identity(&self) -> &VerifiedCanonicalKernelIrIdentityV16 {
        &self.identity
    }
}

/// Move-only custody of one exact V16 encoding and the actual freshly decoded,
/// semantically verified Module. Only complete budgeted admission constructs it.
///
/// ```compile_fail
/// use fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV16;
/// fn requires_clone<T: Clone>() {}
/// requires_clone::<VerifiedCanonicalKernelIrModuleV16>();
/// ```
/// ```compile_fail
/// use fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV16;
/// fn mutate(owner: &mut VerifiedCanonicalKernelIrModuleV16) { owner.module().functions.clear(); }
/// ```
/// ```compile_fail
/// use fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV16;
/// fn separate(owner: VerifiedCanonicalKernelIrModuleV16) { let _ = owner.into_parts(); }
/// ```
#[derive(Debug, Eq, PartialEq)]
pub struct VerifiedCanonicalKernelIrModuleV16 {
    canonical: VerifiedCanonicalKernelIrV16,
    module: Module,
}

impl VerifiedCanonicalKernelIrModuleV16 {
    /// Fresh encode/inverse-decode/verification/equality/hash with one cumulative
    /// ledger. The borrowed source is not cloned or retained. All Result exits
    ...
    pub fn from_module_ref_with_verification_budget_v16(
        module: &Module,
        budget: &mut CanonicalKernelIrVerificationResourceBudgetV1<'_>,
    ) -> Result<(Self, CanonicalKernelIrReplayStorageV16), CanonicalKernelIrReplayAdmissionErrorV16>
    {
        let floor = budget.storage_checkpoint();
        let result = (|| {
            let inverse_inline = std::mem::size_of::<Self>()
                .checked_sub(std::mem::size_of::<VerifiedCanonicalKernelIrV16>())
                .ok_or(CanonicalKernelIrVerificationResourceErrorV1::Arithmetic)?;
            let inverse = verified_inverse(
                module,
                budget,
                Profile::V16,
                std::mem::size_of::<VerifiedCanonicalKernelIrV16>(),
                inverse_inline,
            )
            .map_err(CanonicalKernelIrReplayAdmissionErrorV16::from_inverse)?;
            ...
            Ok((
                ...
                CanonicalKernelIrReplayStorageV16 { retained },
            ))
        })();
    ...
    pub fn from_canonical_bytes_with_verification_budget_v16(
        bytes: &[u8],
        budget: &mut CanonicalKernelIrVerificationResourceBudgetV1<'_>,
    ) -> Result<(Self, CanonicalKernelIrReplayStorageV16), CanonicalKernelIrReplayAdmissionErrorV16>
    {
        ...
        let result = (|| {
            ...
            let module = crate::wire::decode_module_v16_with_allocation_budget_v1(bytes, budget)
                .map_err(CanonicalKernelIrReplayAdmissionErrorV16::Decode)?;
            verify_exact_decoded_module_with_budget_v1(&module, None, budget).map_err(|error| {
                match error {
                    MeteredKernelIrVerificationErrorV1::Verification(error) => {
                        CanonicalKernelIrReplayAdmissionErrorV16::Verification(error)
                    }
                    MeteredKernelIrVerificationErrorV1::Resource(error) => {
                        CanonicalKernelIrReplayAdmissionErrorV16::Resource(error)
                    }
                }
            })?;
    ...
    pub const fn canonical(&self) -> &VerifiedCanonicalKernelIrV16 {
        &self.canonical
    }
    pub fn canonical_bytes(&self) -> &[u8] {
...
pub struct CanonicalKernelIrReplayStorageV16 {
    retained: usize,
}
impl CanonicalKernelIrReplayStorageV16 {
...
#[derive(Debug)]
pub enum CanonicalKernelIrReplayAdmissionErrorV16 {
...
impl CanonicalKernelIrReplayAdmissionErrorV16 {
...
impl From<CanonicalKernelIrVerificationResourceErrorV1>
    for CanonicalKernelIrReplayAdmissionErrorV16
{
    fn from(error: CanonicalKernelIrVerificationResourceErrorV1) -> Self {
        Self::Resource(error)
    }
}
impl fmt::Display for CanonicalKernelIrReplayAdmissionErrorV16 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Encode(error) => write!(f, "V16 canonical encode failed: {error}"),
            Self::Decode(error) => write!(f, "V16 canonical decode failed: {error}"),
            Self::Verification(error) => error.fmt(f),
            Self::Resource(error) => error.fmt(f),
            Self::CanonicalMismatch => f.write_str("V16 inverse differs from its exact subject"),
        }
    }
}
impl Error for CanonicalKernelIrReplayAdmissionErrorV16 {}

fn identity(
    bytes: &[u8],
    budget: &mut CanonicalKernelIrVerificationResourceBudgetV1<'_>,
) -> Result<VerifiedCanonicalKernelIrIdentityV16, CanonicalKernelIrReplayAdmissionErrorV16> {
    let domain = VERIFIED_CANONICAL_KERNEL_IR_V16_IDENTITY_DOMAIN_V1;
    ...
    hash.update(domain);
    hash.update(VERIFIED_CANONICAL_KERNEL_IR_V16_IDENTITY_POLICY_V1.to_le_bytes());
    hash.update(length.to_le_bytes());
    hash.update(bytes);
    Ok(VerifiedCanonicalKernelIrIdentityV16 {
...
#[cfg(test)]
#[path = "canonical_kir_v16_tests.rs"]
mod tests;
