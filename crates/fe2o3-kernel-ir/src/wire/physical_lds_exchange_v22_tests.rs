...
use super::*;
use crate::canonical_kir_v22::tests::fixture;
fn declaration() -> Declaration {
    let module = fixture::module();
    let crate::OperationKind::Gfx942PhysicalLdsExchangeDeclaration(value) =
...
fn step() -> Step {
    let module = fixture::module();
    let crate::OperationKind::Gfx942PhysicalLdsExchangeStep(value) =
...
fn declaration_bytes() -> Vec<u8> {
    let mut writer = Writer::new(KERNEL_IR_VERSION_V22, None);
...
fn decode_d(bytes: &[u8]) -> Result<Declaration, KernelIrDecodeError> {
    ...
    reader.version = KERNEL_IR_VERSION_V22;
...
#[test]
fn exact_474_and_80_payloads_retain_origins_two_roots_and_instruction() {
    let d = declaration_bytes();
    let s = step_bytes();
    assert_eq!(
        d.len(),
        crate::GFX942_PHYSICAL_LDS_EXCHANGE_DECLARATION_BYTES_V22
    );
    assert_eq!(s.len(), crate::GFX942_PHYSICAL_LDS_EXCHANGE_STEP_BYTES_V22);
    ...
    assert_eq!(d[342], 32);
    assert_eq!(&d[343..355], &[128, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0]);
    assert_eq!(&d[355..367], &[1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0]);
    assert_eq!(&d[367..371], &0u32.to_le_bytes());
    assert_eq!(&d[371..375], &512u32.to_le_bytes());
    assert_eq!(&d[375..379], &4u32.to_le_bytes());
    assert_eq!(d[379], 1);
    assert_eq!(d[380], 0);
    assert_eq!(d[426], 4);
    assert_eq!(&d[472..474], &[1, 31]);
    assert_eq!(s[46], 0);
...
#[test]
fn all_twenty_descriptors_are_closed_and_global_load_requires_exact_shape() {
    ...
    let end = Instruction {
        opcode: crate::Gfx942PhysicalLdsExchangeOpcodeV1::Endpgm0,
        destination: 0,
        source0: 0,
        source1: 0,
        immediate: 0,
    ...
    for tag in 20..=u8::MAX {
        let mut changed = [0; 8];
        changed[0] = tag;
        assert!(Instruction::from_descriptor(changed).is_err());
    }
    let load = Instruction {
        opcode: crate::Gfx942PhysicalLdsExchangeOpcodeV1::GlobalLoadDword,
        destination: 8,
        source0: 6,
        source1: 0,
        immediate: 0,
