//! Exact allocated KIR21 payloads. No old-version interpretation or source authority.
#[cfg(test)]
#[path = "physical_global_copy_v21_tests.rs"]
mod tests;
use super::{KERNEL_IR_VERSION_V21, KernelIrDecodeError, KernelIrEncodeError, Reader, Writer};
use crate::{
...
pub(super) fn decode_declaration(
    ...
    let encoding = match r.u8()? {
        ...
        tag => {
            return Err(KernelIrDecodeError::UnknownTag {
                kind: "global copy terminal encoding",
                tag,
            });
        }
    };
    ...
    let native_ordinal = match (r.u8()?, r.u8()?) {
        ...
        (tag, _) => {
            return Err(KernelIrDecodeError::UnknownTag {
                kind: "global copy terminal ordinal",
                tag,
            });
        }
    };
...
pub(super) fn decode_step(r: &mut Reader<'_, '_>) -> Result<Step, KernelIrDecodeError> {
    ...
    for operand in &mut operands {
        *operand = match (r.u8()?, r.u32()?) {
            ...
            (tag, _) => {
                return Err(KernelIrDecodeError::UnknownTag {
                    kind: "global copy operand presence",
                    tag,
                });
            }
        };
