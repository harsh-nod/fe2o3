//! Exact immutable V21 custody through the shared allocation-budgeted verifier.
//! No source authentication, V12 conversion, optimizer or final artifact authority.

pub type VerifiedCanonicalPhysicalGlobalCopyKernelIrModuleV1 = VerifiedCanonicalKernelIrModuleV21;

#[cfg(test)]
#[path = "canonical_kir_inert_view_v21_tests.rs"]
mod inert_view_tests;
#[cfg(test)]
#[path = "canonical_kir_v21_tests.rs"]
pub(crate) mod tests;
...
pub const VERIFIED_CANONICAL_KERNEL_IR_V21_IDENTITY_DOMAIN_V1: &[u8] =
    b"FE2O3/VERIFIED-CANONICAL-KERNEL-IR/V21\0";
pub const VERIFIED_CANONICAL_KERNEL_IR_V21_IDENTITY_POLICY_V1: u16 = 1;

#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
pub struct VerifiedCanonicalKernelIrIdentityV21 {
    digest: [u8; 32],
    canonical_length: u64,
}
impl VerifiedCanonicalKernelIrIdentityV21 {
...
/// Read-only canonical bytes retained inseparably by the V21 module owner.
#[derive(Debug, Eq, PartialEq)]
pub struct VerifiedCanonicalKernelIrV21 {
    canonical_bytes: Vec<u8>,
    identity: VerifiedCanonicalKernelIrIdentityV21,
}
impl VerifiedCanonicalKernelIrV21 {
    pub fn canonical_bytes(&self) -> &[u8] {
        &self.canonical_bytes
    }
    pub const fn identity(&self) -> &VerifiedCanonicalKernelIrIdentityV21 {
        &self.identity
    }
}

/// Move-only custody of one exact V21 encoding and the actual freshly decoded,
/// semantically verified Module. Only complete budgeted admission constructs it.
///
/// ```compile_fail
/// use fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV21;
/// fn requires_clone<T: Clone>() {}
/// requires_clone::<VerifiedCanonicalKernelIrModuleV21>();
/// ```
/// ```compile_fail
/// use fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV21;
/// fn mutate(owner: &mut VerifiedCanonicalKernelIrModuleV21) { owner.module().functions.clear(); }
/// ```
/// ```compile_fail
/// use fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV21;
/// fn separate(owner: VerifiedCanonicalKernelIrModuleV21) { let _ = owner.into_parts(); }
/// ```
#[derive(Debug, Eq, PartialEq)]
pub struct VerifiedCanonicalKernelIrModuleV21 {
    canonical: VerifiedCanonicalKernelIrV21,
    module: Module,
}

impl VerifiedCanonicalKernelIrModuleV21 {
    ...
    pub fn from_module_ref_with_verification_budget_v21(
        module: &Module,
        budget: &mut CanonicalKernelIrVerificationResourceBudgetV1<'_>,
    ) -> Result<(Self, CanonicalKernelIrReplayStorageV21), CanonicalKernelIrReplayAdmissionErrorV21>
    {
        let floor = budget.storage_checkpoint();
        budget.with_prepaid_scope(floor, 0, 0, 0, |budget| {
            let inverse_inline = std::mem::size_of::<Self>()
                .checked_sub(std::mem::size_of::<VerifiedCanonicalKernelIrV21>())
                .ok_or(CanonicalKernelIrVerificationResourceErrorV1::Arithmetic)?;
            let inverse = verified_inverse(
                module,
                budget,
                Profile::V21,
                std::mem::size_of::<VerifiedCanonicalKernelIrV21>(),
                inverse_inline,
            )
            .map_err(CanonicalKernelIrReplayAdmissionErrorV21::from_inverse)?;
            ...
            Ok((
                ...
                CanonicalKernelIrReplayStorageV21 { retained },
            ))
        })
    }
    ...
    pub fn from_canonical_bytes_with_verification_budget_v21(
        bytes: &[u8],
        budget: &mut CanonicalKernelIrVerificationResourceBudgetV1<'_>,
    ) -> Result<(Self, CanonicalKernelIrReplayStorageV21), CanonicalKernelIrReplayAdmissionErrorV21>
    {
        ...
        budget.with_prepaid_scope(floor, 0, 0, 0, |budget| {
            ...
            let module = crate::wire::decode_module_v21_with_allocation_budget_v1(bytes, budget)
                .map_err(CanonicalKernelIrReplayAdmissionErrorV21::Decode)?;
            verify_exact_decoded_module_with_budget_v1(&module, None, budget).map_err(|error| {
                match error {
                    MeteredKernelIrVerificationErrorV1::Verification(error) => {
                        CanonicalKernelIrReplayAdmissionErrorV21::Verification(error)
                    }
                    MeteredKernelIrVerificationErrorV1::Resource(error) => {
                        CanonicalKernelIrReplayAdmissionErrorV21::Resource(error)
                    }
                }
            })?;
    ...
    pub fn decoded_inert_view_with_verification_budget_v21(
        &self,
        budget: &mut CanonicalKernelIrVerificationResourceBudgetV1<'_>,
    ) -> Result<
        (Module, CanonicalKernelIrInertViewStorageV21),
        CanonicalKernelIrReplayAdmissionErrorV21,
    > {
        let floor = budget.storage_checkpoint();
        budget.with_prepaid_scope(floor, 0, 0, 0, |budget| {
            budget.reserve_storage(std::mem::size_of::<Module>())?;
            let module = crate::wire::decode_module_v21_with_allocation_budget_v1(
                self.canonical_bytes(),
                budget,
            )
            .map_err(CanonicalKernelIrReplayAdmissionErrorV21::Decode)?;
            let retained = budget
                ...
                .checked_sub(floor)
                .ok_or(CanonicalKernelIrVerificationResourceErrorV1::Accounting)?;
            Ok((module, CanonicalKernelIrInertViewStorageV21 { retained }))
        })
    }
    ...
    pub const fn canonical(&self) -> &VerifiedCanonicalKernelIrV21 {
        &self.canonical
    }
    pub fn canonical_bytes(&self) -> &[u8] {
...
pub struct CanonicalKernelIrInertViewStorageV21 {
    retained: usize,
}
impl CanonicalKernelIrInertViewStorageV21 {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct CanonicalKernelIrReplayStorageV21 {
    retained: usize,
}
impl CanonicalKernelIrReplayStorageV21 {
    pub const fn retained_storage(self) -> usize {
        self.retained
    }
}
...
#[derive(Debug)]
pub enum CanonicalKernelIrReplayAdmissionErrorV21 {
...
impl CanonicalKernelIrReplayAdmissionErrorV21 {
...
impl From<CanonicalKernelIrVerificationResourceErrorV1>
    for CanonicalKernelIrReplayAdmissionErrorV21
{
    fn from(error: CanonicalKernelIrVerificationResourceErrorV1) -> Self {
        Self::Resource(error)
    }
}
impl fmt::Display for CanonicalKernelIrReplayAdmissionErrorV21 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Encode(error) => write!(f, "V21 canonical encode failed: {error}"),
            Self::Decode(error) => write!(f, "V21 canonical decode failed: {error}"),
            Self::Verification(error) => error.fmt(f),
            Self::Resource(error) => error.fmt(f),
            Self::CanonicalMismatch => f.write_str("V21 inverse differs from its exact subject"),
        }
    }
}
impl Error for CanonicalKernelIrReplayAdmissionErrorV21 {}

fn identity(
    bytes: &[u8],
    budget: &mut CanonicalKernelIrVerificationResourceBudgetV1<'_>,
) -> Result<VerifiedCanonicalKernelIrIdentityV21, CanonicalKernelIrReplayAdmissionErrorV21> {
    let domain = VERIFIED_CANONICAL_KERNEL_IR_V21_IDENTITY_DOMAIN_V1;
    ...
    hash.update(bytes);
    Ok(VerifiedCanonicalKernelIrIdentityV21 {
