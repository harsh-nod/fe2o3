...
for (const [index, edit] of manifest.edits.entries()) {
  ...
  if (start < 0 || file.original.indexOf(edit.before, start + 1) >= 0) {
    errors.push(`Nonunique or stale anchor ${index}: ${edit.path}`);
    continue;
  }
...
const patch = [];
const overlayOption = process.argv.indexOf('--overlay');
const requestedOverlay = overlayOption >= 0 ? process.argv[overlayOption + 1] : undefined;
if (overlayOption >= 0 && !files.has(requestedOverlay)) throw new Error('Unknown overlay path');
for (const [filename, file] of files) {
  ...
  if (requestedOverlay) {
    if (filename === requestedOverlay) process.stdout.write(overlay);
    continue;
  }
  if (process.argv.includes('--parse')) {
    const parsed = spawnSync(process.env.RUSTFMT || 'rustfmt', ['--edition', '2024', '--emit', 'stdout', '--config', 'skip_children=true'], {
      input: overlay, encoding: 'utf8', maxBuffer: 16 * 1024 * 1024, timeout: 60000,
    });
    if (parsed.error || parsed.status !== 0) throw new Error(`Rust syntax check ${filename}: ${parsed.stderr || parsed.error}`);
  }
...
if (requestedOverlay) { /* The requested overlay has already been written. */ }
else if (process.argv.includes('--patch')) process.stdout.write(patch.join('\n') + '\n');
