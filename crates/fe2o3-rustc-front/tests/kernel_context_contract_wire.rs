...
#[test]
fn context_wire_has_a_frozen_golden() {
    const GOLDEN: &str = "";
    let bytes = encode_kernel_context_frontend_contract_v1(&fixture());
    let hex = bytes
        .iter()
...
#[test]
fn maximum_distinct_item_names_reach_the_exact_wire_bound() {
    let n = MAX_KERNEL_CONTEXT_GENERATED_ITEM_NAME_BYTES_V1;
    ...
    assert_eq!(bytes.len(), MAX_KERNEL_CONTEXT_FRONTEND_CONTRACT_BYTES_V1);
    assert_eq!(
        decode_kernel_context_frontend_contract_v1(&bytes).unwrap(),
        contract
    );
}
...
fn every_reserved_byte_is_rejected() {
    let bytes = encode_kernel_context_frontend_contract_v1(&fixture());
    let offsets = entry_offsets(&bytes);
    let mut reserved = vec![26, 27, 28, 29, 30, 31];
    for offset in offsets {
        reserved.extend([offset + 2, offset + 3, offset + 38, offset + 39]);
    }
    for index in reserved {
        ...
        assert!(
            matches!(
                decode_kernel_context_frontend_contract_v1(&bad),
...
#[test]
fn zero_digests_invalid_utf8_and_oversized_internal_text_reject() {
    use KernelContextFrontendContractDecodeErrorV1 as Error;
    let bytes = encode_kernel_context_frontend_contract_v1(&fixture());
    for offset in entry_offsets(&bytes) {
        let mut bad = bytes.clone();
        bad[offset + 4..offset + 36].fill(0);
        assert_eq!(
            decode_kernel_context_frontend_contract_v1(&bad),
            Err(Error::Validation(
                KernelContextFrontendContractValidationErrorV1::ZeroItemIdentity
            ))
        );
        let mut bad = bytes.clone();
        bad[offset + 40] = 0xff;
        ...
        let mut bad = bytes.clone();
        bad[offset + 36..offset + 38].copy_from_slice(&u16::MAX.to_le_bytes());
        assert!(matches!(
            decode_kernel_context_frontend_contract_v1(&bad),
