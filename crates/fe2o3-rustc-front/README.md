...
## Logical kernel-context sidecar V1

`KernelContextFrontendContractV1` is an additive wire domain for a macro that
erases one logical `KernelContext` argument from the physical kernel ABI. It
does not change the `KernelFrontendContractV1` format or bytes. Its magic is
`FE2O3KC\0`, its version is 1, and its maximum encoded length is bounded by
`MAX_KERNEL_CONTEXT_FRONTEND_CONTRACT_BYTES_V1`.

The fixed 32-byte header records the total length, exactly three item bindings,
context source ordinal 0, required issuance count 1, and the exact
`fe2o3_device_kernel_context_issue_v1` diagnostic at version 1. The diagnostic
identity follows the header. Three role-ordered records then bind the physical
kernel root, generated logical helper, and nominal kernel marker. Each record
contains a role tag, a 32-byte identity, and a bounded canonical ASCII Rust
identifier.

Generated-item identities are SHA-256 commitments over a domain separator,
the role tag, and the exact generated name, with every digest field length
prefixed. They provide deterministic metadata identity without depending on a
rustc-private `DefId`, arena address, or printer output. They are not proof that
the named item exists or was generated by the macro.

`encode_generated_kernel_context_frontend_contract_v1` is the narrow producer
API for the proc macro. The decoder bounds lengths before allocation, rejects
unknown roles, flags, versions, reserved fields, malformed names, duplicate
roles or names, identity/name substitution, wrong ordinals or counts,
truncation, trailing bytes, and noncanonical ordering. A rustc collector must
still resolve the sidecar's exact function and marker anchors, compare their
independently observed identities and names with every getter on the decoded
record, and prove that exactly one authenticated issuance call connects the
physical root to the logical helper. Decoding this sidecar grants no runtime or
compiler authority.
