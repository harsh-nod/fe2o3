...
/// Historical production LLVM V1 layout and exact built-in rustc target spelling.
///
/// Existing MIR, LLVM handoff, external-library manifest, and replay V1 contracts retain this
/// byte spelling. It must not be reinterpreted as the physical LLVM 22 Worker layout.
...
/// Exact data layout reported by the pinned built-in rustc AMDGPU target.
pub const PRODUCTION_AMDHSA_RUSTC_DATA_LAYOUT_V1: &str = PRODUCTION_AMDHSA_LLVM_DATA_LAYOUT_V1;
/// Exact physical layout returned by the ROCm 7.2.4 LLVM 22 AMDGPU target machine.
///
/// The gfx942 and gfx950 Worker profiles share this layout. Worker V3 requires byte-for-byte
/// equality with `TargetMachine::createDataLayout`, which omits rustc's `m:e` component.
pub const PRODUCTION_AMDHSA_LLVM22_WORKER_DATA_LAYOUT_V1: &str = "e-p:64:64-p1:64:64-p2:32:32-p3:32:32-p4:64:64-p5:32:32-p6:32:32-p7:160:256:256:32-p8:128:128:128:48-p9:192:256:256:32-i64:64-v16:16-v24:32-v32:32-v48:64-v96:128-v192:256-v256:256-v512:512-v1024:1024-v2048:2048-n32:64-S32-A5-G1-ni:7:8:9";
...
mod tests {
    ...
    #[test]
    fn production_amdhsa_layouts_retain_the_distinct_measured_contracts() {
        assert!(PRODUCTION_AMDHSA_LLVM_DATA_LAYOUT_V1.starts_with("e-m:e-p:64:64-"));
        assert!(PRODUCTION_AMDHSA_RUSTC_DATA_LAYOUT_V1.starts_with("e-m:e-p:64:64-"));
        assert!(PRODUCTION_AMDHSA_LLVM22_WORKER_DATA_LAYOUT_V1.starts_with("e-p:64:64-"));
        for required in [
            "p3:32:32",
            "p7:160:256:256:32",
            "p8:128:128:128:48",
            ...
            assert!(
                PRODUCTION_AMDHSA_LLVM22_WORKER_DATA_LAYOUT_V1.contains(required),
                "missing Worker target-machine layout component {required}"
            );
            assert!(
                PRODUCTION_AMDHSA_RUSTC_DATA_LAYOUT_V1.contains(required),
                "missing rustc layout component {required}"
            );
        }
        assert_eq!(
            PRODUCTION_AMDHSA_LLVM_DATA_LAYOUT_V1.strip_prefix("e-m:e-"),
            PRODUCTION_AMDHSA_LLVM22_WORKER_DATA_LAYOUT_V1.strip_prefix("e-")
        );
        assert_eq!(
