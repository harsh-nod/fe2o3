...
use crate::{PlironIrStructuralIdentityV1, derive_pliron_ir_structural_identity_v1};
...
fn collect_model(
    ...
    // Close every use before a dialect verifier follows an operand's type or
    // native dominance follows a reverse-use/predecessor list.
    ...
    use crate::pliron_function_inventory::{
        BoundedPlironFunctionInventoryV1, ClosedPlironSsaErrorV1, preflight_closed_pliron_ssa_v1,
    };
    charge(work, live_blocks.len() + operation_count)?;
    let inventory = BoundedPlironFunctionInventoryV1::collect(context, function)
        .map_err(|_| Error::Limit("inventory"))?;
    let closed =
        preflight_closed_pliron_ssa_v1(context, function, &inventory, |n| charge(work, n).is_ok())
            .map_err(|error| match error {
                ClosedPlironSsaErrorV1::ResourceLimit => Error::Limit("work"),
                _ => Error::Malformed("SSA custody or order"),
            })?;
    // The native tree is constructed only after the DAG/reachability proof.
    charge(
        work,
        live_blocks
            .len()
            .saturating_mul(closed.edges)
            .saturating_mul(4),
    )?;
    let arguments = entry
        .deref(context)
...
/// Derives conditional coverage from the exact live graph. Unsupported forms
/// return an error, never a weakened ownership contract or runtime assumption.
pub fn derive_pliron_conditional_prefix_coverage_v1(
    context: &Context,
    function: &FuncOp,
    ...
    let epoch = context
        ...
        .map_err(|_| Error::MutationEpochUnavailable)?
        .value();
    let mut work = 0;
    ...
    let graph_identity = derive_pliron_ir_structural_identity_v1(context, function)
        .map_err(|_| Error::StructuralIdentityUnavailable)?;
    if context
        .ir_mutation_attempt_epoch()
        .map_err(|_| Error::MutationEpochUnavailable)?
        .value()
...
/// Re-derives the closed theorem as well as checking byte-exact graph identity
/// and epoch. A mutate-then-restore attempt therefore invalidates the record.
pub fn revalidate_pliron_conditional_prefix_coverage_v1(
    context: &Context,
    function: &FuncOp,
    record: &ConditionalPrefixCoverageV1,
) -> Result<(), ConditionalPrefixDerivationErrorV1> {
    if derive_pliron_conditional_prefix_coverage_v1(context, function)? != *record {
        return Err(ConditionalPrefixDerivationErrorV1::StaleGraph);
    }
    Ok(())
}
