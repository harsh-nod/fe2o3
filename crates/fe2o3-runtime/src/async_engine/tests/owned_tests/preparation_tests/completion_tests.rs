...
fn reserved_completion_isolates_discard_order_and_abandoned_observers() {
    for order in [[0, 1], [1, 0]] {
        for abandoned in [false, true] {
            ...
            let observed_wakes = observations.lock().unwrap().clone();
            ...
            let (no_issues, no_flushes) = {
                let state = h.state.lock().unwrap();
                (state.issues.is_empty(), state.flush_calls.is_empty())
            };
...
fn reserved_stop_notifies_latest_wakers_before_disposal_and_contains_wake_panic() {
    for panics in [false, true] {
        ...
        assert!(!context.is_terminal());
        let old_wakes = old.lock().unwrap().clone();
        assert!(old_wakes.is_empty());
        let expected: Vec<_> = (0..2)
            ...
            .collect();
        let latest_wakes = latest.lock().unwrap().clone();
        assert_eq!(latest_wakes, expected);
...
fn unpublished_retirement_preserves_exact_failed_owner_and_untouched_neighbor() {
    for mode in [0, 1, 2] {
        ...
        h.advance();
        assert!(!h.context.is_terminal());
        let completed = h.state.lock().unwrap().adoption_completed.clone();
        assert_eq!(
            completed,
        ...
        for repetition in 0..5 {
            ...
            let (retire_attempts, payload_drops, no_issues, no_flushes) = {
                let state = h.state.lock().unwrap();
                (
                    state.adoption_retire_attempts.clone(),
                    state.adoption_payload_drops.clone(),
                    state.issues.is_empty(),
                    state.flush_calls.is_empty(),
                )
            };
            assert_eq!(retire_attempts, attempts);
...
fn owned_shutdown_retires_in_order_and_retains_failed_and_unvisited_owners() {
    for mode in [0, 1, 2] {
        ...
        let completed = state.lock().unwrap().adoption_completed.clone();
        assert_eq!(completed, streams.map(|stream| (stream, owner)));
        assert_eq!(
            drops.each_ref().map(|v| v.load(Ordering::SeqCst)),
            [0, 0, 0]
        );
