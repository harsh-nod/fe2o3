//! Public-runtime-facade benchmark for large directional persistent SDMA copies.
...
const USAGE: &str =
    "usage: gfx942-runtime-directional-window-benchmark <unique-id> <bytes> <warmups> <samples>";
...
// SAFETY: This authority never admits a launch. It exists only because the
// copy-only benchmark uses the production constructor without exercising compute.
...
fn drive_copy_to_completion_v1(
    ...
    // A prior SDMA result may leave the next copy ready but unpublished.
    driver.flush()?;
    loop {
...
fn main() -> BenchmarkResult<()> {
    ...
    for round in 0..rounds {
        ...
        if observed.iter().any(|byte| *byte != expected) {
            return Err(format!("directional SDMA copy mismatch at round {round}").into());
        }
        if round >= warmups {
            h2d.push(h2d_ns);
            d2h.push(d2h_ns);
    ...
    let max_window_packets = fe2o3_kfd::GFX942_PERSISTENT_DIRECTIONAL_SDMA_MAX_WINDOW_PACKETS_V1;
    ...
    println!(
        "backend=kfd schema=fe2o3.async-copy-benchmark.v1 unique_id={unique_id:016x} profile=directional bytes={copy_bytes} depth=1 queue_depth=1 batch_size=1 direction=h2d-then-d2h concurrency=1 configured_queues=1 doorbells_per_batch={window_count} packets_per_transfer={packet_count} windows_per_transfer={window_count} max_packets_per_window={max_window_packets} warmups={warmups} samples={samples} h2d_engine_index=1 d2h_engine_index=0 h2d_p50_ns={h2d_p50} h2d_p95_ns={h2d_p95} h2d_p50_GBps={:.3} d2h_p50_ns={d2h_p50} d2h_p95_ns={d2h_p95} d2h_p50_GBps={:.3} completion=facade-wait-and-explicit-continuation-flush teardown=explicit",
        copy_bytes as f64 / h2d_p50 as f64,
        copy_bytes as f64 / d2h_p50 as f64,
    );
    Ok(())
}
...
mod tests {
    ...
    fn sixty_three_plus_two_window_copy_flushes_without_full_timeout_wait() {
        ...
        assert_eq!(
            packet_count
                .div_ceil(fe2o3_kfd::GFX942_PERSISTENT_DIRECTIONAL_SDMA_MAX_WINDOW_PACKETS_V1),
            2
        );
