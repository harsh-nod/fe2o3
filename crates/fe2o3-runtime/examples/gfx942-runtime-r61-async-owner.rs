...
fn run(unique_id: u64) -> ResultV1<()> {
    ...
    let upload_future = handle.copy_async(
        stream,
    ...
    let download_result = await_bounded(handle.copy_async(
        stream,
...
fn main() -> ResultV1<()> {
    ...
    if arguments.len() != 2 {
        return Err("usage: gfx942-runtime-r61-async-owner <unique-id>".into());
    }
