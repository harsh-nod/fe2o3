...
fn safe_general_gemm_mir_reaches_kir_and_two_semantic_failures_are_diagnostic() {
    ...
    assert!(
        ...
        ) && impostor_stderr.contains("outside the reviewed fe2o3-device source root"),
    ...
    for (bin, code, property) in [
        ("missing-publish", "0x46470103", "initialized"),
        ("duplicate-store", "0x46470106", "output_region_injective"),
    ] {
        ...
        assert!(
            stderr.contains(&format!(
                "authenticated general GEMM semantic KIR rejected: general GEMM {property} counterexample"
