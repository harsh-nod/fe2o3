...
fn run_extraction(target: &ScratchTarget) -> String {
    ...
    assert!(
        stderr.contains("semantic importer authenticated rustc target \"amdgcn-amd-amdhsa\"")
            ...
            && stderr.contains(
                "retaining 11 sorted rustc type producer(s), 1 stable source file identity producer(s), 24 canonical source provenance producer(s), and 1 canonical body ID table(s)",
            )
