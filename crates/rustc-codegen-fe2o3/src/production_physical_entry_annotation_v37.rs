...
use crate::production_physical_entry_call_v37::{ActualPhysicalEntryCallV37, is_physical};
use crate::production_physical_entry_census_v37::{observe_root, require_body_bounds};
...
const MAX: usize = SEMANTIC_PHYSICAL_ENTRY_MAX_OCCURRENCES_V37;
...
pub(crate) const PREPAID_CONSTRUCTION_WORK_V37: usize = 2_200_000;
...
struct Slot<'tcx> {
    ...
    occurrence: u8,
    actual: ActualPhysicalEntryCallV37<'tcx>,
...
pub(crate) struct PhysicalEntryAnnotationsV37<'tcx> {
    root: Instance<'tcx>,
...
pub(crate) fn prepare<'tcx>(
    ...
    budget: &mut Budget<'_>,
) -> Result<PhysicalEntryAnnotationsV37<'tcx>, String> {
    ...
    let [function] = plan.roots() else {
        return Err("physical-entry requires one root".into());
    };
    let [root] = plan.function_producers() else {
        return Err("physical-entry excludes helpers".into());
    };
    let [retained] = plan.body_producers() else {
        return Err("physical-entry retained body absent".into());
    };
    if function.index() != 0
    ...
    {
        return Err("physical-entry actual root/call roster differs".into());
    }
    ...
    {
        return Err("physical-entry terminal roster contains a foreign or excessive marker".into());
    }
    let target = crate::semantic_layout_bridge::rustc_semantic_layout_target_v1(tcx)
        .map_err(|_| "physical-entry active target absent")?;
    if target.llvm_target() != "amdgcn-amd-amdhsa"
        || !target.has_exact_codegen_profile("gfx942", "-wavefrontsize32,+wavefrontsize64,-xnack")
    {
        return Err("physical-entry requires exact gfx942 xnack-off Wave64".into());
    }
    let frontend = root
        .frontend_contract
        .as_ref()
        .ok_or("physical-entry frontend contract absent")?;
    let launch = frontend
        .contract()
        .launch()
        .ok_or("physical-entry explicit source launch absent")?;
    ...
    {
        return Err("physical-entry requires exact authored launch64 and max_grid2".into());
    }
    if frontend.resource_contract().is_some_and(|r| {
        r.static_shared_memory_bytes() != 0 || r.max_dynamic_shared_memory_bytes() != 0
    }) {
        return Err("physical-entry source cannot declare workgroup storage".into());
    }
    let export = root
        .export_name
        .as_deref()
        .ok_or("physical-entry source export absent")?;
    if export.is_empty()
        || export.len() > 128
        || !export
            .bytes()
    ...
    {
        return Err("physical-entry export is outside the closed ASCII symbol grammar".into());
    }
    ...
    {
        return Err("physical-entry current source bytes absent or oversized".into());
    }
    for amount in [
        transcript.len(),
    ...
    budget
        .charge_work(
            items
                .checked_mul(4)
                .ok_or("physical-entry source work overflow")?,
        )
        .map_err(|e| e.to_string())?;
    let census = observe_root(tcx, root.instance, body).map_err(str::to_owned)?;
    if census.len() != recipes.len() {
        return Err(
            "physical-entry retained recipes do not cover the complete source census".into(),
        );
    }
    let identities = canonical_function_identities_v1(tcx, root.instance);
    if identities != root.identities {
        return Err("physical-entry current full root Instance identity differs".into());
    }
    ...
    for (raw, semantic) in retained.raw_to_semantic_locals.iter().enumerate() {
        ...
        {
            return Err("physical-entry raw-to-semantic local mapping differs".into());
        }
    }
    let (signature, abi) =
        crate::production_physical_entry_source_abi_v37::check(plan).map_err(str::to_owned)?;
    let common = Common {
        axes: [
    ...
    for (occurrence, observed) in census.calls().enumerate() {
        ...
        let (recipe_index, recipe) = matching
            .next()
            .ok_or("physical-entry current occurrence recipe absent")?;
        if matching.next().is_some() || used_recipes[recipe_index] {
            return Err("physical-entry source occurrence recipe is ambiguous".into());
        }
        let terminal = terminals
            .get(recipe.terminal as usize)
            .ok_or("physical-entry source terminal association absent")?;
        ...
        {
            return Err("physical-entry current exact callee/const/ABI occurrence differs".into());
        }
        ...
        .map_err(|_| "physical-entry source provenance is unavailable or overbound")?;
        if source.provenance() != row.terminator.provenance {
            return Err("physical-entry retained source occurrence provenance differs".into());
        }
        ...
        if count == 5 {
            for (index, raw) in actual.argument_locals().into_iter().enumerate() {
                semantic_locals[index] = *retained
                    ...
                    .get(raw as usize)
                    .ok_or("physical-entry actual begin local mapping absent")?;
            }
        }
        let callee = recipe
            .terminal
            ...
            .ok_or("physical-entry callable index overflow")?;
    ...
    {
        return Err("physical-entry contains an unconsumed terminal or call recipe".into());
    }
    Ok(PhysicalEntryAnnotationsV37 {
        root: root.instance,
        function: *function,
        common,
        slots,
...
impl<'tcx> PhysicalEntryAnnotationsV37<'tcx> {
    ...
    pub(crate) fn attach(
        ...
        let index = self.slots[..self.count]
            ...
            .ok_or("physical-entry source occurrence missing or already consumed")?;
        let slot = self.slots[index]
            .as_ref()
            .ok_or("physical-entry source occurrence disappeared")?;
        let count = if matches!(
            actual.operation(),
            SemanticCompilerIntrinsicOperationV1::Gfx942PhysicalEntryBegin
        ) {
        ...
        {
            return Err("physical-entry current raw/semantic occurrence join differs");
        }
        for (index, operand) in call.arguments().iter().enumerate() {
            let (place, moved) = match operand {
                ...
                _ => return Err("physical-entry emitted begin input is not direct transport"),
            };
            if !place.projections().is_empty()
                || place.local() != slot.semantic_locals[index]
                || moved != (actual.moved_arguments() & (1 << index) != 0)
            {
                return Err("physical-entry emitted begin input changed identity or move mode");
            }
        }
        let source = SemanticPhysicalEntrySourceV37::new(
            self.common.axes,
            self.common.body,
