        ...
        for mutation in [false, true] {
            for profile in [Profile::Gfx942, Profile::Gfx950] {
                ...
                match short.error {
                    ...
                    other => panic!("exact final composed LLVM comparison Work refusal: {other:?}"),
                }
...
#[test]
fn refined_forwarding_native_factory_exact_and_short_work_keep_final_comparison_phase() {
    exact_work_cases(false);
}
#[test]
fn refined_forwarding_native_replay_exact_and_short_work_keep_final_comparison_phase() {
    exact_work_cases(true);
}

// Stop before the native emitter: replay the actual composed source owner and
// original P7 history under the same paid owner/sibling floor as the measurement.
fn replay_prefix_reference(
    ...
    with_prefix(erased, profile, mutation, |prefix, parent| {
        ...
        let mut work = Work::new(1_000_000_000);
        {
            ...
            let ledger = budget.work_ledger_identity_v1();
            match &value.owner {
                Composed::Direct(owner) => owner.verify_equivalence(&mut budget).unwrap(),
                Composed::Erased(owner) => owner.verify_equivalence(&mut budget).unwrap(),
            }
            assert_eq!(budget.storage(), floor);
        ...
        for mutation in [false, true] {
            for profile in [Profile::Gfx942, Profile::Gfx950] {
                ...
                match short.error {
                    ...
                    other => {
                        panic!("exact final composed native scratch Storage refusal: {other:?}")
                    }
                }
                ...
                if replay {
                    ...
                    let candidate_header = size_of::<PreparedRefinedForwardingNativeOutputV1>()
                        .checked_sub(owner_header)
                        .and_then(|bytes| bytes.checked_sub(size_of::<Policy7ExecutionWitnessV1>()))
                        .unwrap();
                    // Both native scratch requests are identical; only the second
                    // coexists with retained LLVM capacity and completed headers.
...
#[test]
fn refined_forwarding_native_factory_storage_short_observes_first_denial() {
    storage_cases(false);
}
#[test]
fn refined_forwarding_native_replay_storage_short_observes_first_denial() {
    storage_cases(true);
}

#[test]
fn refined_forwarding_native_success_keeps_prior_denials_live_receipts_and_ledger() {
    for erased in [false, true] {
        for mutation in [false, true] {
...
#[test]
fn refined_forwarding_native_scope_refuses_foreign_ledger_after_real_preparation() {
...
#[test]
fn refined_forwarding_native_partial_error_and_unwind_drop_actual_both_stage_owner() {
        ...
        for unwind in [false, true] {
            with_prefix(erased, Profile::Gfx942, true, |prefix, budget| {
                ...
                let result: Result<()> = scoped(floor, budget, |budget| {
                    ...
                    budget.reserve_storage(receipt.retained_storage()).unwrap();
                    shape(&owner, erased, 2, 2, budget);
                    let _live = (owner, Dropped(dropped.clone()));
                    if unwind {
                        std::panic::panic_any(73u32);
                    }
                    Err(mismatch("injected with actual composed native owner"))
                });
                if unwind {
                    assert!(matches!(
                        result,
                        ...
                        Err(ProductionPipelineError::InductionRefinementNativeStage(
                            InductionRefinementNativeStageErrorV1::ForwardingComposition(
                                RefinedForwardingNativeStageErrorV1::Mismatch(
                                    "injected with actual composed native owner"
                                )
                            )
                        ))
                    ));
...
#[test]
fn refined_forwarding_native_forwarding_refusal_consumes_real_refined_owner_without_fallback() {
    for erased in [false, true] {
        with_prefix(erased, Profile::Gfx942, true, |prefix, budget| {
            let floor = budget.storage();
            let ledger = budget.work_ledger_identity_v1();
            let mut actual = 0;
            let result: Result<()> = scoped(floor, budget, |budget| {
                let (licm, _history, _, _, _, _) = prepare_licm_source_prefix_v1(prefix, budget)?;
                let (refined, _) = Refined::continue_once(licm, Limits::default(), budget)?;
                assert_eq!(
                    refined
                        .origins()
                        .iter()
                ...
                actual = count(refined.output());
                let mut limits = ForwardingLimits::default();
                limits.memory.operations = 0;
                Composed::continue_once(refined, limits, budget)
                    .map(|_| panic!("no R fallback or F owner on refusal"))
            });
            match result {
                Err(ProductionPipelineError::InductionRefinementNativeStage(
                    InductionRefinementNativeStageErrorV1::ForwardingComposition(
                        RefinedForwardingNativeStageErrorV1::Admission(error),
                    ),
                )) => match *error {
                    CompositionAdmissionError::Continuation(
                        fe2o3_kernel_opt::OwnedCrossBlockForwardingErrorV1::Memory(
                            fe2o3_kernel_analysis::CanonicalKirMemorySsaErrorV1::InputLimit {
                                resource,
                                actual: found,
                                limit,
                            },
                        ...
                        assert_eq!(
                            resource,
                            fe2o3_kernel_analysis::CanonicalKirMemorySsaResourceV1::Operations
                        );
                        assert_eq!((found, limit), (actual, 0));
                    }
                    other => panic!("exact actual R MemorySSA input refusal: {other:?}"),
                },
                other => panic!("closed composed error custody: {other:?}"),
            }
            assert!(actual > 0);
            assert_eq!(budget.storage(), floor);
            assert!(budget.work_ledger_identity_v1() == ledger);
