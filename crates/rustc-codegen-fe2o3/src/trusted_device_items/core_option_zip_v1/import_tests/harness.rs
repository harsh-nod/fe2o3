...
const CRATE_NAME: &str = "discriminant_import_source";
const METADATA: &str = "fe2o3-discriminant-import-source-v1";
const CHILD_ENV: &str = "FE2O3_DISCRIMINANT_IMPORT_CHILD";
...
impl Scratch {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "fe2o3-discriminant-import-{}-{nonce}",
            std::process::id()
        ));
        ...
        std::fs::write(scratch.0.join("Cargo.toml"), format!(
            "[package]\nname = \"discriminant-import-source\"\nversion = \"0.0.0\"\nedition = \"2024\"\n[workspace]\n[dependencies]\nfe2o3-device = {{ path = {device:?} }}\n"
        )).unwrap();
        scratch
    }
}
...
pub(super) fn run(case: Case) {
    ...
    if std::env::var(CHILD_ENV).ok().as_deref() != Some(child.as_str()) {
        ...
        let output = crate::process_execution::capture_output(
            std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env("CARGO_PKG_NAME", "discriminant-import-source")
        ...
        assert!(
            ...
            "actual discriminant child failed:\n{}\n{}",
    ...
    let args = vec![
        ...
        format!("-Ctarget-cpu={}", case.profile()),
