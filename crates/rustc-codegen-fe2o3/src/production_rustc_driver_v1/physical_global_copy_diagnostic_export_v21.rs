...
/// Produces diagnostic KIR21, unchanged canonical LLVM and a bounded native
/// observation sidecar from one live authenticated MIR38 source preparation.
///
...
pub fn run_diagnostic_physical_global_copy_extraction_driver_v21(
    args: &[String],
    output: &Path,
) -> Result<(), String> {
    ...
    if fatal.is_some() {
        return Err("diagnostic physical-global-copy V21 rustc analysis failed".into());
    }
    if callbacks.calls != 1 {
        return Err(
            "diagnostic physical-global-copy V21 requires exactly one actual rustc callback".into(),
        );
    }
    callbacks.result.unwrap_or_else(|| {
        Err("diagnostic physical-global-copy V21 callback produced no result".into())
    })
}
fn extract(tcx: TyCtxt<'_>, output: &Path) -> Result<(), String> {
    ...
    if !transaction.has_authenticated_physical_global_copy_v21() {
        return Err(
            "diagnostic V21 requires actual authenticated physical-global-copy markers".into(),
        );
    }
    ...
    eprintln!(
        "fe2o3 diagnostic physical-global-copy V21: actual Rust -> MIR38 -> pre-ranked exact KIR21 -> canonical LLVM; ranked/formal/descriptor continuation unavailable; serialized outputs are inert diagnostic observations, not source/compiler custody; native execution, hardware observation and artifact/launch authority false"
    );
    Ok(())
}
pub(super) fn publish(
    owner:&crate::production_pipeline::physical_global_copy_diagnostic_v21::AuthenticatedPhysicalGlobalCopyDiagnosticV21,
    output: &Path,
) -> Result<(), String> {
    ...
    std::fs::create_dir(output)
        .map_err(|e| format!("fresh physical-global-copy diagnostic directory: {e}"))?;
    for (name, bytes, limit) in [
        ...
        (
            "native-observation-input-v21.txt",
...
mod tests {
    use super::*;
    #[test]
    fn physical_global_copy_diagnostic_requires_explicit_overflow_before_rustc() {
        let error = run_diagnostic_physical_global_copy_extraction_driver_v21(
            &["rustc".into()],
            Path::new("/unopened-physical-v21"),
        )
        .unwrap_err();
