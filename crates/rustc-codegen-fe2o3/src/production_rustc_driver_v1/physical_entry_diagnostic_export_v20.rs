...
/// Produces diagnostic KIR20, unchanged canonical LLVM and a bounded native
/// observation sidecar from one live authenticated MIR37 source preparation.
///
...
pub fn run_diagnostic_physical_entry_extraction_driver_v20(
    args: &[String],
    output: &Path,
) -> Result<(), String> {
    ...
    if fatal.is_some() {
        return Err("diagnostic physical-entry V20 rustc analysis failed".into());
    }
    if callbacks.calls != 1 {
        return Err(
            "diagnostic physical-entry V20 requires exactly one actual rustc callback".into(),
        );
    }
    callbacks
        .result
        .unwrap_or_else(|| Err("diagnostic physical-entry V20 callback produced no result".into()))
}
fn extract(tcx: TyCtxt<'_>, output: &Path) -> Result<(), String> {
    ...
    if !transaction.has_authenticated_physical_entry_v20() {
        return Err("diagnostic V20 requires actual authenticated physical-entry markers".into());
    }
    ...
    eprintln!(
        "fe2o3 diagnostic physical-entry V20: actual Rust -> MIR37 -> pre-ranked exact KIR20 -> canonical LLVM; ranked/formal/descriptor continuation unavailable; serialized outputs are inert diagnostic observations, not source/compiler custody; native execution, hardware observation and artifact/launch authority false"
    );
    Ok(())
}
pub(super) fn publish(
    owner:&crate::production_pipeline::physical_entry_diagnostic_v20::AuthenticatedPhysicalEntryDiagnosticV20,
    output: &Path,
) -> Result<(), String> {
    ...
    std::fs::create_dir(output)
        .map_err(|e| format!("fresh physical-entry diagnostic directory: {e}"))?;
    for (name, bytes, limit) in [
        ...
        (
            "native-observation-input-v20.txt",
...
mod tests {
    use super::*;
    #[test]
    fn physical_entry_diagnostic_requires_explicit_overflow_before_rustc() {
        let error = run_diagnostic_physical_entry_extraction_driver_v20(
            &["rustc".into()],
            Path::new("/unopened-physical-v20"),
        )
        .unwrap_err();
