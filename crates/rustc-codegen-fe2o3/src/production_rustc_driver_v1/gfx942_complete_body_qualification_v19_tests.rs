//! Actual Rust -> normal checked MIR36/KIR19 -> CPU and inert worker handoff.
...
//! Native LLVM, protected artifact finalization, and GPU execution are NOT run.
...
#[path = "gfx942_complete_body_observation_v19_tests.rs"]
mod observation;

const OUTPUT_ENV: &str = "FE2O3_TEST_COMPLETE_BODY_OUTPUT_V19";
const CHILD_ENV: &str = "FE2O3_TEST_COMPLETE_BODY_INPUTS_V19";
const FEATURE_ENV: &str = "FE2O3_TEST_COMPLETE_BODY_FEATURE_V19";
const PACKAGE: &str = "fe2o3-production-extraction-fixture";
const CRATE_NAME: &str = "fe2o3_production_extraction_fixture";
const CHILD: &str = "production_rustc_driver_v1::gfx942_complete_body_qualification_v19_tests::actual_complete_body_source_child";
const PREFIX: &str = "FE2O3_COMPLETE_BODY_OBSERVATION_V19 ";
const MODE_ENV: &str = "FE2O3_TEST_COMPLETE_BODY_MODE_V19";
const MODES: [&str; 3] = ["observe", "llvm", "handoff"];
const FEATURES: [&str; 7] = [
    "complete-body-one-v19",
    "complete-body-diamond-v19",
    "complete-body-wrong-launch-v19",
    "complete-body-reserved-register-v19",
    "complete-body-foreign-input-v19",
    "complete-body-undefined-merge-v19",
    "complete-body-dynamic-grid-v19",
];
...
fn require_current_source() {
    for (relative, expected) in [
        ...
        (
            "src/complete_body_v19.rs",
            include_bytes!(
                "../../tests/fixtures/production-extraction-device/src/complete_body_v19.rs"
            )
            .as_slice(),
        ),
        (
...
fn checked_feature(feature: &str) -> Result<(), &'static str> {
    FEATURES
        .contains(&feature)
        .then_some(())
        .ok_or("unknown or combined complete-body feature")
}
...
fn derive_record(directory: &Path, feature: &str) -> PreparedInvocation {
    ...
    PreparedInvocation {
        schema: "fe2o3-test-source-complete-body-invocation-v19".into(),
        feature: feature.into(),
        args,
        crate_binding,
        cargo_observation,
        source_sha256: source_hash(&fixture.join("src/complete_body_v19.rs")),
...
fn rejection_fragment(feature: &str) -> Result<&'static str, &'static str> {
    match feature {
        "complete-body-wrong-launch-v19" => {
            Ok("complete body requires required and maximum 64x1x1")
        }
        "complete-body-reserved-register-v19" => {
            Ok("complete body roles require distinct v8..v63 outside the reserved prefix")
        }
        "complete-body-foreign-input-v19" => {
            Ok("complete body marker operands differ from exact root argument order")
        }
        "complete-body-undefined-merge-v19" => {
            Ok("OutputNotDefined { label: Gfx942CompleteBodyLabelV1(4) }")
        }
        "complete-body-dynamic-grid-v19" => {
            Ok("complete-body source requires an explicit finite max_grid")
        }
    ...
    fn after_analysis<'tcx>(&mut self, _compiler: &Compiler, tcx: TyCtxt<'tcx>) -> Compilation {
        ...
        self.result = Some((|| {
            ...
            if !transaction.has_authenticated_complete_body_v19() {
                return Err("actual complete-body source marker was not authenticated".into());
            }
...
#[test]
#[ignore = "isolated actual AMD rustc child; use actual_complete_body_source_ladder"]
fn actual_complete_body_source_child() {
    let directory =
    ...
    let result = match mode.as_str() {
        ...
        "llvm" => {
            ...
            super::run_production_amdgpu_llvm_extraction_driver_v1(&actual.args, &path).map(|()| {
                let bytes = read_bounded(&path, 16 * 1024).unwrap();
                assert!(!bytes.is_empty());
                json!({"stage": "public_normal_llvm_driver", "sha256": digest(&bytes)})
            })
        }
        "handoff" => {
            let path = output.join("handoff-v2.bin");
            super::run_production_amdgpu_compiler_handoff_extraction_driver_v1(&actual.args, &path)
                .map(|()| {
                    let bytes = read_bounded(&path, 1024 * 1024).unwrap();
                    let handoff =
                        fe2o3_compiler_ffi::CompilerModuleHandoffV2::decode(&bytes).unwrap();
                    ...
                    json!({"stage": "public_normal_handoff_driver", "sha256": digest(&bytes)})
                })
        }
        _ => unreachable!(),
    };
    ...
    println!(
        ...
        serde_json::to_string(&json!({
            "schema": "fe2o3-test-source-complete-body-observation-v19",
...
fn actual_complete_body_source_ladder() {
    ...
    for feature in FEATURES {
        ...
        if FEATURES[..2].contains(&feature) {
            for (mode, file, cap) in [
                ...
                .unwrap();
                let public =
                    read_bounded(&directory.join(format!("{feature}.{mode}")).join(file), cap)
                        .unwrap();
                assert_eq!(
                    observed, public,
                    "normal public {mode} output differs from same-source checked continuation"
                );
            }
        }
    }
    ...
    fs::write(directory.join("observation.json"), serde_json::to_vec_pretty(&json!({
        "schema": "fe2o3-test-source-complete-body-ladder-v19", "observations": observations,
        "public_driver_output_relation": "exact_bytes_same_current_source",
        "cpu_positive_cases": 384, "exact_negative_runs": 15,
        "formal_bounds_scope": "conservative LaunchEnvelope; CPU tail results do not weaken launch requirements",
        "protected_finalizer_admitted": false, "native_llvm_executed": false,
        "grants_artifact_or_launch_authority": false, "hardware_observed": false,
    })).unwrap()).unwrap();
    eprintln!(
        "complete-body actual source observations retained at {}",
        directory.display()
    );
}

#[test]
fn complete_body_source_controls_reject_wrong_stage_or_feature() {
    ...
    assert!(checked_feature("complete-body-one-v19,complete-body-diamond-v19").is_err());
