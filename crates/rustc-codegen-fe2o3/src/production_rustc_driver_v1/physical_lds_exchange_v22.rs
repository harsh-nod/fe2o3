...
pub(super) fn extract_llvm<'tcx>(
    ...
    publish_new_inert_output(
        ...
        dialect_amdgcn::GFX942_PHYSICAL_LDS_EXCHANGE_LLVM_BYTES_V22,
        "physical-lds-exchange checked canonical LLVM",
    )?;
    eprintln!(
        "fe2o3 production extraction: authenticated Rust -> MIR39 -> ranked/formal checked KIR22 -> exact four-slot compiler ABI and authored 512-byte LDS reservation -> gfx942:xnack- inert LLVM; {} authored kernarg reads; {}; artifact/launch authority false",
...
pub(super) fn extract_handoff<'tcx>(
    ...
    let prepared =
        crate::production_worker_handoff::prepare_physical_lds_exchange_worker_handoff_v22(lowered)
            .map_err(|e| e.to_string())?;
    ...
    publish_new_inert_output(
        ...
        "physical-lds-exchange inert compiler-module handoff",
    )?;
    ...
    eprintln!(
        "FE2O3_PHYSICAL_LDS_EXCHANGE_HANDOFF_RELATION_V22 {}",
        serde_json::json!({
            "schema": "fe2o3-physical-lds-exchange-inert-handoff-relation-v22",
            ...
            "lds_bytes": 512,
            "workgroup_participants_required": 128,
            "lds_execution_observed": false,
    ...
    eprintln!(
        "fe2o3 production extraction: authenticated Rust -> MIR39 -> ranked/formal checked KIR22 -> exact source/compiler ABI descriptor -> inert worker handoff; {}; {} handoff byte(s); protected/native/host admission unavailable; artifact/launch authority false",
        conditions,
