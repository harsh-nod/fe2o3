...
use super::*;
use fe2o3_kernel_ir::{
    FormalMemoryAccessKind, Gfx942PhysicalLdsExchangeOpcodeV1 as Opcode, OperationKind,
};
use fe2o3_mir_model::semantic_mir_v1::SemanticMirWireVersionV1;

pub(super) fn observe(
    mut target: crate::production_pipeline::AuthenticatedPhysicalLdsExchangeTargetModuleV22,
    feature: &str,
    output: &Path,
) -> Result<Value, String> {
    let abi_controls = target.qualify_actual_abi_controls_v22();
    ...
    let cpu_cases = target.observe_with_budget(|checked, budget| {
        ...
        super::super::gfx942_physical_lds_exchange_qualification_v22_tests::observation::observe_cpu(
            checked.executable(),
            budget,
        )
    });
    assert_eq!(cpu_cases, 32);
    ...
    let OperationKind::Gfx942PhysicalLdsExchangeDeclaration(declaration) =
        ...
        panic!("actual physical LDS-exchange declaration");
    };
    ...
    assert!(body.blocks[0].operations.iter().any(|operation|matches!(operation.kind,
        OperationKind::Gfx942PhysicalLdsExchangeStep(step) if
    ...
    assert_eq!(formal.input_read().result(), formal.lds_write().value());
    assert_eq!(formal.lds_read().value(), formal.output_store().value());
    assert_ne!(formal.input_read().result(), formal.output_store().value());
    assert_eq!(formal.lds_frame().descriptor(), declaration.lds_frame);
    assert_eq!(declaration.lds_frame.byte_length, 512);
    assert_eq!(formal.lds_frame().peer_xor_mask(), 64);
    assert_eq!(formal.lds_frame().byte_scale(), 4);
    assert_eq!(formal.publication().participant_count(), 128);
    assert_eq!(
        formal.publication().memory_scope(),
        fe2o3_kernel_ir::SynchronizationScope::Workgroup
    );
    assert_eq!(
        formal.publication().ordering(),
        fe2o3_kernel_ir::MemoryOrdering::AcquireRelease
    );
    assert!(!formal.publication().completes_pending_accesses());
    assert!(!formal.publication().grants_global_happens_before());
    assert_eq!(runtime.required_workgroup(), [128, 1, 1]);
    assert_eq!(runtime.required_workgroups(), [1, 1, 1]);
    assert!(runtime.requires_all_workgroup_invocations());
    let lds = json!({
        "frame_bytes":declaration.lds_frame.byte_length,
        "frame_alignment":declaration.lds_frame.alignment,
        "epoch":formal.publication().publication_epoch(),
        "participants":formal.publication().participant_count(),
        "write_operation":formal.lds_write().location().operation_index,
        "write_complete_operation":formal.lds_write().complete_at().operation_index,
        "publication_operation":formal.publication().location().operation_index,
        "read_operation":formal.lds_read().location().operation_index,
        "read_complete_operation":formal.lds_read().complete_at().operation_index,
        "global_happens_before":false,"runtime_conditions_discharged":false,
    });
    assert_eq!(
    ...
    let prepared =
        crate::production_worker_handoff::prepare_physical_lds_exchange_worker_handoff_v22(target)
    ...
    let relation = crate::kernel_ir_codegen::exact_inert_descriptor_extension_v22;
    ...
    for (name, bytes, cap) in [
        ("canonical-v22.bin", canonical.as_slice(), 1024 * 1024),
    ...
    Ok(json!({
        "stage":"actual_source_mir39_checked_kir22_cpu_normal_inert_handoff",
        ...
        "entry_symbol":entry_symbol,"authored_blocks":1,"native_instruction_count":32,
        ...
        "cpu_cases":cpu_cases,"cpu_negative_controls":6,"cpu_grids":[128],
        "lds":lds,"cpu_observed_barrier_cases":1,
