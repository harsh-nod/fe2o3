...
pub(super) fn current_sources() -> Vec<FilePin> {
    ...
    for (relative, expected) in [
        ...
        (
            "crates/rustc-codegen-fe2o3/tests/fixtures/production-extraction-device/src/ordered_composition_v1.rs",
            include_bytes!(
                "../../tests/fixtures/production-extraction-device/src/ordered_composition_v1.rs"
            )
            .as_slice(),
        ),
        (
        ...
        assert_eq!(
            bytes, expected,
            "rebuild the test binary after changing {relative}"
        );
        rows.push(FilePin {
            path: relative.into(),
...
pub(super) fn derive_record(directory: &Path, feature: &str) -> PreparedInvocation {
    ...
    PreparedInvocation {
        schema: "fe2o3-test-ordered-composition-invocation-v1".into(),
...
#[test]
fn composition_metadata_requires_real_feature_not_lib_table_or_cfg() {
