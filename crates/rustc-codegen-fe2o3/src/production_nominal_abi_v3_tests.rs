...
#[test]
fn nominal_descriptor_errors_retain_their_typed_source() {
    use std::error::Error;
    let error = E::Resource(Resource::Arithmetic);
    assert!(error.source().unwrap().is::<Resource>());
    let mismatch = E::Mismatch("source ABI");
    assert_eq!(
        mismatch.to_string(),
        "nominal source descriptor: source ABI"
    );
...
fn nominal_descriptor_declared_target_sensitive_requirements_are_not_dropped() {
    for capability in [
        ...
        TargetCapability::WaveWidth(fe2o3_kernel_ir::WaveWidth::Wave32),
        ...
        run(|budget| {
            ...
            for target_context in [false, true] {
                assert!(matches!(
                    inert_capabilities(&module, target_context, budget),
...
#[test]
fn nominal_descriptor_diagnostics_require_retained_target_context() {
    use fe2o3_kernel_ir as kir;
    for declared in [false, true] {
        run(|budget| {
            let trap = kir::AmdGpuDiagnosticOperation::Trap;
            let module = if declared {
                let mut module = Module::new("diagnostics");
                module
                ...
                operation_module(
                    ...
                    vec![Operation {
                        ...
                        kind: OperationKind::Call {
                            callee: trap.intrinsic_function_id(),
                            arguments: vec![],
                        },
                    }],
                )
            ...
            for target_tag in [
                None,
                Some(kir::AMDGPU_GFX942_XNACK_MINUS_TARGET_CAPABILITY_NAME),
                Some(kir::AMDGPU_GFX950_XNACK_MINUS_TARGET_CAPABILITY_NAME),
            ] {
                ...
                if let Some(name) = target_tag {
                    module
                        ...
                        .insert(TargetCapability::Extension {
                            namespace: kir::AMDGPU_EXACT_TARGET_CAPABILITY_NAMESPACE.to_owned(),
                            name: name.to_owned(),
                        });
                }
                assert!(matches!(
                    inert_capabilities(&module, false, budget),
                    Err(E::UnsupportedRequirements)
                ));
                let (capabilities, count) = inert_capabilities(&module, true, budget)?;
                assert_eq!(&capabilities[..count], &[CapabilityV1::AmdWave]);
            }
            Ok(())
        });
    }
...
#[test]
fn nominal_descriptor_wave64_operations_derive_deduplicated_subgroups() {
    use fe2o3_kernel_ir as kir;
    for name in ["wave_helper", "renamed_helper"] {
        run(|budget| {
            let operations = (0..2)
                .map(|id| Operation {
                    results: vec![kir::ValueDef::new(
                        kir::ValueId(id),
                        kir::Type::Scalar(kir::ScalarType::U32),
                    )],
                    kind: OperationKind::Wave(kir::WaveOperation::full(
                        kir::WaveOperationKind::LaneId,
                        kir::WaveWidth::Wave64,
                    )),
                })
                .collect();
            let mut module = operation_module(name, operations);
            for declared in [false, true] {
                if declared {
                    module
                        .required_capabilities
                    ...
                    module.functions[0]
                        .required_capabilities
                        .insert(TargetCapability::SubgroupSize(64));
                }
                let (capabilities, count) = inert_capabilities(&module, false, budget)?;
                assert_eq!(
                    &capabilities[..count],
...
#[test]
fn nominal_descriptor_wave32_operation_is_refused() {
    use fe2o3_kernel_ir as kir;
    let module = operation_module(
        "wave32",
        vec![Operation {
            results: vec![kir::ValueDef::new(
                kir::ValueId(0),
...
#[test]
fn nominal_descriptor_synchronization_is_not_hidden_by_empty_declarations() {
    use fe2o3_kernel_ir as kir;
    let semantics = kir::BarrierSemantics::new(
        kir::MemoryOrdering::AcquireRelease,
        [kir::AddressSpace::Global],
    );
    for (kind, expected_visits) in [
        (
            OperationKind::Fence(kir::Fence {
                memory_scope: kir::SynchronizationScope::Device,
                semantics: semantics.clone(),
            }),
            0,
        ),
        (
            OperationKind::Barrier(kir::Barrier {
                execution_scope: kir::SynchronizationScope::Subgroup,
                memory_scope: kir::SynchronizationScope::Device,
                semantics,
            }),
            1,
        ...
        let module = operation_module("synchronization", vec![operation]);
        run(|budget| {
            for target_context in [false, true] {
                assert!(matches!(
                    inert_capabilities(&module, target_context, budget),
...
fn operation_module(name: &str, operations: Vec<Operation>) -> Module {
    use fe2o3_kernel_ir as kir;
    let mut module = Module::new(name);
    module.functions.push(kir::Function::internal_helper(
        name,
        ...
        vec![kir::BasicBlock {
            ...
            parameters: vec![],
            operations,
            terminator: Some(kir::Terminator::Return { values: vec![] }),
        }],
    ));
    module
}
