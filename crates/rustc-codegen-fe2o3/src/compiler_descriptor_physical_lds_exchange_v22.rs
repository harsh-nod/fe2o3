...
#[path = "compiler_descriptor_physical_lds_exchange_abi_v22.rs"]
mod abi;
#[cfg(test)]
pub(crate) use abi::qualify_actual_owner_abi_controls_v22;
pub(crate) use abi::{PhysicalLdsExchangePreparedAbiV22, prepare_physical_lds_exchange_abi_v22};
...
pub(super) struct PhysicalLdsExchangeDescriptorAdmissionV22<'a> {
    module: &'a Module,
    _checked: &'a Checked,
    _abi: &'a PhysicalLdsExchangePreparedAbiV22,
}
impl PhysicalLdsExchangeDescriptorAdmissionV22<'_> {
    pub(super) fn admits(&self, module: &Module, capability: &TargetCapability) -> bool {
        std::ptr::eq(self.module, module)
            && matches!(capability,
            TargetCapability::Extension{namespace,name}
            if namespace==fe2o3_kernel_ir::AMDGPU_GFX942_PHYSICAL_LDS_EXCHANGE_CAPABILITY_NAMESPACE_V22
                &&name==fe2o3_kernel_ir::AMDGPU_GFX942_PHYSICAL_LDS_EXCHANGE_CAPABILITY_NAME_V22)
    }
}
fn mismatch(detail: &'static str) -> CompilerDescriptorError {
    CompilerDescriptorError::ProductionDescriptorMismatch(detail)
}
pub(crate) fn construct_physical_lds_exchange_descriptor_source_v22(
    ...
    checked
        ...
        .map_err(|error| CompilerDescriptorError::PhysicalLdsExchangeV22(Box::new(error)))?;
    abi::validate_prepared_abi_v22(checked, typed_roots, abi, budget)?;
    ...
    if geometry.rank() != 1
        || geometry.workgroup() != [128, 1, 1]
        || geometry.max_grid() != [1, 1, 1]
        || geometry.max_flat_workgroup_size() != 128
    ...
    // Source annotations are still exactly zero. Only this retained verified
    // frame supplies the internal static descriptor reservation; generic
    // WorkgroupMemory capability admission remains disabled.
    let frame = abi.lds_frame();
    if frame != checked.memory_obligations().lds_frame().descriptor()
        || frame.validate_shape().is_err()
    {
        return Err(mismatch("physical descriptor same-owner LDS frame"));
    }
    let profiles = [DescriptorConstructionProfileV1 {
        ...
        static_shared_memory_bytes: frame.byte_length,
        allow_exact_tiled_matrix: false,
        allow_workgroup_memory: false,
        producer_version: "physical-lds-exchange-v22-gfx942-cov6-inert-v1",
    }];
    let admission = PhysicalLdsExchangeDescriptorAdmissionV22 {
        module,
        _checked: checked,
        _abi: abi,
    };
    construct_compiler_descriptor_source_with_profiles_and_admission_v1(
        ...
        typed_roots,
        &profiles,
        DescriptorCapabilityAdmissionV1::PhysicalLdsExchangeV22(&admission),
    )?
    .ok_or_else(|| mismatch("physical descriptor typed closure"))
}
#[cfg(test)]
#[path = "compiler_descriptor_physical_lds_exchange_v22_tests.rs"]
mod tests;
