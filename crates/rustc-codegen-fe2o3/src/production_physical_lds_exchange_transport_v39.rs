//! Closed argument transport for the physical-lds-exchange whole-root source census.
...
#[cfg(test)]
#[path = "production_physical_lds_exchange_transport_v39_tests.rs"]
mod tests;
...
impl ArgumentTransport {
    pub(super) fn new(locals: usize) -> Result<Self, &'static str> {
        if !(3..=MAX_LOCALS).contains(&locals) {
            return Err("physical-lds-exchange source local bound exceeded");
        }
        let mut state = Self {
    ...
    fn temporary(&self, index: usize) -> Result<(), &'static str> {
        if index < 3 || index >= self.locals {
            return Err("physical-lds-exchange source mutates a root argument or invalid local");
        }
        Ok(())
    }
    ...
    pub(super) fn assign(
        ...
        let origin = self
            ...
            .ok_or("physical-lds-exchange source transport reads an undefined or foreign value")?;
        if origin == 1 && !moved {
            return Err("physical-lds-exchange output owner cannot be copied");
        }
    ...
    pub(super) fn consume_marker(
        ...
        if self.marker_consumed || moved & 2 == 0 || moved & !0b11 != 0 {
            return Err("physical-lds-exchange marker repeated or output owner not moved");
        }
        for (index, local) in locals.into_iter().enumerate() {
            ...
            if actual != Some(index as u8) {
                return Err(
                    "physical-lds-exchange marker operands differ from exact root argument order",
                );
            }
        }
        for (index, local) in locals.into_iter().enumerate() {
    ...
    pub(super) fn require_marker(&self) -> Result<(), &'static str> {
        if self.marker_consumed {
            ...
            Err("physical-lds-exchange source never consumed its marker")
        }
    }
}
