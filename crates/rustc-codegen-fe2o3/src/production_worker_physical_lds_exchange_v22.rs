...
use super::*;
use crate::compiler_descriptor::physical_lds_exchange_v22::PhysicalLdsExchangePreparedAbiV22;
...
pub(crate) struct PreparedPhysicalLdsExchangeWorkerHandoffV22 {
...
impl PreparedPhysicalLdsExchangeWorkerHandoffV22 {
...
pub(crate) fn prepare_physical_lds_exchange_worker_handoff_v22(
    authenticated: crate::production_pipeline::AuthenticatedPhysicalLdsExchangeTargetModuleV22,
) -> Result<PreparedPhysicalLdsExchangeWorkerHandoffV22, ProductionWorkerHandoffError> {
    ...
    let prepared = ledger.with_budget(|budget| {
        ...
        if target.to_string() != "gfx942:xnack-" || emission.canonical_identity() != canonical.identity()
            || *emission.lds_frame() != abi.lds_frame()
            || abi.lds_frame() != checked.memory_obligations().lds_frame().descriptor() {
            return Err(ProductionWorkerHandoffError::MissingProductionBindings);
        }
        validate_exact_target_binding(target, module)?;
        let compiler_module =
            crate::kernel_ir_codegen::retain_verified_physical_lds_exchange_compiler_module_text_v22(canonical, emission)
        ...
        let descriptor = crate::compiler_descriptor::physical_lds_exchange_v22::construct_physical_lds_exchange_descriptor_source_v22(
    ...
    Ok(PreparedPhysicalLdsExchangeWorkerHandoffV22 {
        prepared,
        _checked: checked,
        abi,
