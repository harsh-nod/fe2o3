//! Actual source to expanded-final LLVM/V3. No signed, loader or GPU credit.
#![allow(
...
const NOOP_CHILD: &str = "production_rustc_driver_v1::checked_output_source_v1_tests::expanded_native_v3::expanded_noop_source_child";
const REWRITE_CHILD: &str = "production_rustc_driver_v1::checked_output_source_v1_tests::expanded_native_v3::expanded_rewrite_source_child";
    ...
    fn after_analysis<'tcx>(&mut self, _: &Compiler, tcx: TyCtxt<'tcx>) -> Compilation {
        ...
        self.result = Some((|| {
            ...
            output
                ...
                .map_err(|e| fail("negative native", e))?;
...
fn run_child(case: Case) {
    ...
    .unwrap();
    println!(
        "EXPANDED_NATIVE_V3_RESPONSE_BEGIN bytes={} sha256={:?}",
        bytes.len(),
        digest(&bytes)
    );
    println!("{}", std::str::from_utf8(&bytes).unwrap());
    println!("EXPANDED_NATIVE_V3_RESPONSE_END");
...
fn ordinary_rust_expanded_native_v3_preserves_normal_noop() {
    parent(Case::Noop);
}
#[test]
#[ignore = "pinned rust-src and actual Erased nonvacuous scalar-final source on both AMD profiles"]
fn ordinary_rust_expanded_native_v3_rewrites_before_first_llvm() {
    parent(Case::Rewrite);
}
...
#[path = "production_rustc_driver_expanded_history_v1_tests.rs"]
