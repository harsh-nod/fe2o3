    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::TargetOptimization(error) => write!(
                formatter,
                "production compilation target-bound Kernel IR optimization failed: {error}"
            ),
...
impl TargetLoweredProductionCompilation {
    ...
    pub(crate) fn retained_identity_and_transaction_binding_count(&self) -> usize {
        let _ = (
            ...
            self.source_mir_kir.eligible_candidates(),
            self.source_mir_kir.covered_candidates(),
            self.source_mir_kir.status(),
...
mod tests {
    ...
    fn production_target_lowering_uses_shared_replayable_transforms() {
        ...
        let identity = transaction
            ...
            .expect("optimized target-KIR identity");
