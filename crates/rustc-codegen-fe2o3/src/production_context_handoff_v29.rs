...
pub(crate) fn with_projected_execution_source_v29<'work, R>(
    ...
    match result {
        Ok(result) => match cleanup {
            Ok(()) => result,
            Err(error) => {
                let rejected_panic = catch_unwind(AssertUnwindSafe(|| drop(result))).err();
                // A payload destructor may still unwind, but the accounting
                // decision precedes all rejected-result payload destruction.
                drop(rejected_panic);
                Err(error.into())
            }
        },
