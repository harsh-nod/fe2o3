//! Existing V12 simulator executes the exact scheduled source-produced owners.
use super::*;
use fe2o3_kernel_ir::{
    AccessMode, AddressSpace, LaunchDomain, LaunchExtent, VerifiedCanonicalKernelIrV12,
};
use fe2o3_kir_sim::{
...
const BYTE_CAP: usize = 256 * 1024;
const MAX_RUNS: usize = 60;
const MAX_STEPS: u64 = 8_000_000;

#[derive(Default, Serialize)]
pub(super) struct OracleLedger {
    runs: usize,
    steps: u64,
...
impl OracleLedger {
    fn charge(&mut self, bytes: usize) -> Result<(), String> {
        let total = self
            ...
            .checked_add(bytes)
            .ok_or("local-order oracle payload overflow")?;
        if total > 2 * 1024 * 1024 {
            return Err("local-order oracle host payload limit".into());
        }
    ...
    fn next(&mut self) -> Result<SimulationLimitsV1, String> {
        if self.runs >= MAX_RUNS || self.steps >= MAX_STEPS {
            return Err("local-order cumulative oracle limit".into());
        }
        self.runs += 1;
...
pub(super) fn observe(owner: &Owner, ledger: &mut OracleLedger) -> Result<Value, String> {
    let bytes = owner.canonical().canonical_bytes();
    if bytes.len() > BYTE_CAP {
        return Err("local-order oracle canonical byte limit".into());
    }
    ledger.charge(
        bytes
            .len()
            .checked_mul(2)
            .ok_or("local-order oracle byte overflow")?,
    )?;
    // Exact V12 bytes from the live scheduled V12 owner; not V8 relabeling and
    // never a source/compiler owner. This copy exists solely for model execution.
    let canonical = VerifiedCanonicalKernelIrV12::from_canonical_bytes(bytes.to_vec())
        .map_err(|e| e.to_string())?;
    if canonical.identity() != owner.canonical().identity() || canonical.canonical_bytes() != bytes
    {
        return Err("local-order exact simulator input identity changed".into());
    }
    let identity = SimulationKernelIrIdentityV1::from(*canonical.identity());
    let module =
        AdmittedSimulationModuleV1::admit_v12(canonical, limits()).map_err(|e| e.to_string())?;
    if module.identity() != &identity || module.grants_execution_authority() {
        return Err("local-order simulator identity or authority changed".into());
    }
    let [kernel] = module.module().kernels.as_slice() else {
        return Err("local-order oracle root roster".into());
    };
    let entry = module
        .module()
        .function(&kernel.entry)
        .ok_or("local-order oracle entry missing")?;
    let [Type::Slice(output), a, b, c, d] = entry.signature.parameters.as_slice() else {
        return Err("local-order oracle requires output slice and four scalar inputs".into());
    };
    ...
    {
        return Err("local-order oracle source ABI or launch changed".into());
    }
    ...
    for [a, b, c, d] in [
        [0, 0, 0, 0],
        [u32::MAX, 0, 0, u32::MAX],
        [0xaaaa_aaaa, 0x5555_5555, 0xf0f0_f0f0, 0x0f0f_0f0f],
        [0x8000_0000, 1, 0x8000_0000, 2],
        [0x1234_5678, 0x8765_4321, 0x4444_4444, 0x1111_1111],
    ] {
        for len in [0usize, 1, 65] {
            ...
            let expected = backing(len, (a ^ b) & (c | d), true)?.1;
            let mut arguments = vec![output];
            arguments.extend(
                [a, b, c, d].map(|value| SimulationArgumentV1::Scalar(ScalarBitsV1::u32(value))),
            );
            ...
            let execution = module
                ...
                .map_err(|e| format!("local-order oracle execution: {e:?}"))?;
            ledger.steps = ledger
                .steps
                .checked_add(execution.steps_executed())
                .ok_or("local-order oracle step overflow")?;
            ...
            {
                return Err(
                    "local-order oracle output, initialization, canaries or identity mismatch"
                        .into(),
                );
            }
            match execution.conflict_assessment() {
                ...
                other => return Err(format!("local-order observed conflict: {other:?}")),
            }
            match execution.race_assessment() {
                ...
                other => return Err(format!("local-order observed race: {other:?}")),
            }
            let repeated = module
                .simulate(&request, TARGET, ledger.next()?)
                .map_err(|e| format!("local-order oracle replay: {e:?}"))?;
            ledger.steps = ledger
                .steps
                .checked_add(repeated.steps_executed())
                .ok_or("local-order oracle step overflow")?;
            if request != before || execution != repeated {
                return Err("local-order simulator mutated input or replay diverged".into());
            }
        }
    }
    Ok(
        ...
        "oracle":"host_u32_xor_and_or_exact","simulator_version":"V12",
        "simulation_is_proof":false,"hardware_observed":false}),
    )
}

#[test]
fn source_local_order_oracle_budget_is_cumulative_and_failed_charge_preserves_it() {
