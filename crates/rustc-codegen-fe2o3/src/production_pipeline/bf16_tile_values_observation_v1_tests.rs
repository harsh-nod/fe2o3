...
pub(crate) struct PhaseObservation {
    ...
    pub(crate) occurrence_storage: Option<usize>,
    pub(crate) normal_attempted: bool,
...
impl<'tcx> ProductionCompilation<'tcx, CollectedRustStage<'tcx>> {
    pub(crate) fn observe_bf16_tile_values_source_for_test_v1<R: Copy + 'static>(
        self,
        ...
        let result = self
            .materialize_with_bf16_tile_values_inspection_v1(inspect)
            .map(|(ordinary, value)| {
                drop(ordinary);
                value
            });
