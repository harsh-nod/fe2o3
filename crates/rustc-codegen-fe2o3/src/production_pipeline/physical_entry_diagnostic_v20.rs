//! Pre-ranked diagnostic-only actual-source continuation for MIR37/KIR20.
...
use fe2o3_lower_mir_kernel::ProductionPhysicalEntryPreRankedKirOwnerV20 as Materialized;

#[derive(Debug)]
pub(crate) enum PhysicalEntryStageErrorV20 {
...
impl std::fmt::Display for PhysicalEntryStageErrorV20 {
...
impl std::error::Error for PhysicalEntryStageErrorV20 {
...
fn stage_error(error: PhysicalEntryStageErrorV20) -> ProductionPipelineError {
    ProductionPipelineError::PhysicalEntryDiagnosticStage(Box::new(error))
}
fn resource(error: Resource) -> ProductionPipelineError {
    stage_error(PhysicalEntryStageErrorV20::Resource(error))
}
...
pub(crate) struct AuthenticatedPhysicalEntryDiagnosticV20 {
...
impl AuthenticatedPhysicalEntryDiagnosticV20 {
...
impl<'tcx> ProductionCompilation<'tcx, CollectedRustStage<'tcx>> {
    ...
    pub(crate) fn has_authenticated_physical_entry_v20(&self) -> bool {
        self.stage
            .closure
            .contains_physical_entry_terminal_v20(self.stage.tcx)
    }

    pub(crate) fn lower_physical_entry_diagnostic_v20(
        self,
    ) -> Result<AuthenticatedPhysicalEntryDiagnosticV20, Box<ProductionPipelineError>> {
        let ssa = self
            .import_semantic_mir()?
            ...
            != fe2o3_mir_model::semantic_mir_v1::SemanticMirWireVersionV1::V37
        {
            return Err(Box::new(stage_error(
                PhysicalEntryStageErrorV20::WrongSourceProfile,
            )));
        }
        ...
        {
            return Err(Box::new(stage_error(
                PhysicalEntryStageErrorV20::WrongTarget,
            )));
        }
        if !ssa
            .stage
        ...
        {
            return Err(Box::new(stage_error(
                PhysicalEntryStageErrorV20::UnsupportedRefinementBindings,
            )));
        }
        ...
        {
            return Err(Box::new(stage_error(
                PhysicalEntryStageErrorV20::ProtectedPublicationUnavailable,
            )));
        }
        if ssa
            .stage
        ...
        let prepared = ledger.with_budget(|budget| {
            materialize_prepared_with_budget_v29(
                ...
                |source, launch, budget| {
                    ...
                    .map_err(|error| stage_error(PhysicalEntryStageErrorV20::Source(error)))?;
        ...
        let emission =
            ledger.with_budget(|budget| {
                ...
                dialect_amdgcn::lower_canonical_v20_compiler_module_to_gfx942_xnack_minus_llvm_ir(
                    materialized.executable(), budget)
                    .map_err(|error| stage_error(PhysicalEntryStageErrorV20::Emission(error)))?;
                budget
        ...
        let native_observation = ledger.with_budget(|budget| {
            let (text, storage) = dialect_amdgcn::physical_entry_native_observation_input_v20(
        ...
        Ok(AuthenticatedPhysicalEntryDiagnosticV20 {
            materialized,
            emission,
            native_observation,
            _bindings: bindings,
