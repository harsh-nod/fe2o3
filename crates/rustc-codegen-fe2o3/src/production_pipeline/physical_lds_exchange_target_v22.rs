//! Normal exact-source continuation for the fixed MIR39/KIR22 body profile.
//! No diagnostic observation is converted into source or output custody.
//! Full-prefix input readability/initialization, global alias, full workgroup
//! participation, LDS completion/publication and compiler-ABI conditions remain typed and unresolved
...
use fe2o3_lower_mir_kernel::{
    ProductionPhysicalLdsExchangeCheckedKirOwnerV22 as Checked,
    ProductionPhysicalLdsExchangePreRankedKirOwnerV22 as Materialized,
};

#[derive(Debug)]
pub(crate) enum PhysicalLdsExchangeTargetStageErrorV22 {
    Source(fe2o3_lower_mir_kernel::ProductionPhysicalLdsExchangeSourceErrorV22),
    Checks(fe2o3_lower_mir_kernel::ProductionPhysicalLdsExchangeCheckErrorV22),
...
impl std::fmt::Display for PhysicalLdsExchangeTargetStageErrorV22 {
...
impl std::error::Error for PhysicalLdsExchangeTargetStageErrorV22 {
...
fn stage_error(error: PhysicalLdsExchangeTargetStageErrorV22) -> ProductionPipelineError {
    ProductionPipelineError::PhysicalLdsExchangeTargetStage(Box::new(error))
}
fn resource(error: Resource) -> ProductionPipelineError {
    stage_error(PhysicalLdsExchangeTargetStageErrorV22::Resource(error))
}
...
pub(crate) struct AuthenticatedPhysicalLdsExchangeTargetModuleV22 {
    checked: Checked,
    emission: dialect_amdgcn::Gfx942PhysicalLdsExchangeCanonicalEmissionV22,
    abi: crate::compiler_descriptor::physical_lds_exchange_v22::PhysicalLdsExchangePreparedAbiV22,
    bindings: AuthenticatedProductionBindings,
    ledger: Ledger,
}
impl AuthenticatedPhysicalLdsExchangeTargetModuleV22 {
    ...
    #[cfg(test)]
    pub(crate) fn qualify_actual_abi_controls_v22(&mut self) -> usize {
        self.ledger.with_budget(|budget| {
            crate::compiler_descriptor::physical_lds_exchange_v22::qualify_actual_owner_abi_controls_v22(
                &self.checked,
    ...
    pub(crate) fn into_parts(
        ...
        dialect_amdgcn::Gfx942PhysicalLdsExchangeCanonicalEmissionV22,
        Vec<crate::compiler_descriptor::TypedDescriptorRootV1>,
        Option<fe2o3_compiler_ffi::CompilerFfiEnvelopeV1>,
        crate::compiler_descriptor::physical_lds_exchange_v22::PhysicalLdsExchangePreparedAbiV22,
        Ledger,
    ) {
        (
            self.checked,
...
impl<'tcx> ProductionCompilation<'tcx, CollectedRustStage<'tcx>> {
    pub(crate) fn lower_physical_lds_exchange_target_v22(
        self,
    ) -> Result<AuthenticatedPhysicalLdsExchangeTargetModuleV22, Box<ProductionPipelineError>> {
        let ssa = self
            .import_semantic_mir()?
            ...
            != fe2o3_mir_model::semantic_mir_v1::SemanticMirWireVersionV1::V39
        {
            return Err(Box::new(stage_error(
                PhysicalLdsExchangeTargetStageErrorV22::WrongSourceProfile,
            )));
        }
        ...
        {
            return Err(Box::new(stage_error(
                PhysicalLdsExchangeTargetStageErrorV22::WrongTarget,
            )));
        }
        if !ssa
            .stage
        ...
        {
            return Err(Box::new(stage_error(
                PhysicalLdsExchangeTargetStageErrorV22::UnsupportedRefinementBindings,
            )));
        }
        ...
        {
            return Err(Box::new(stage_error(
                PhysicalLdsExchangeTargetStageErrorV22::ProtectedPublicationUnavailable,
            )));
        }
        if ssa
            .stage
        ...
        let prepared = ledger.with_budget(|budget| {
            materialize_prepared_with_budget_v29(
                ...
                |source, launch, budget| {
                    ...
                    .map_err(|error| {
                        stage_error(PhysicalLdsExchangeTargetStageErrorV22::Source(error))
                    })?;
        ...
        let checked = ledger.with_budget(|budget| {
            let owner = Checked::try_check(materialized, budget).map_err(|error| {
                stage_error(PhysicalLdsExchangeTargetStageErrorV22::Checks(error))
            })?;
            let added = owner
                .retained_storage()
        ...
        let abi = ledger.with_budget(|budget| {
            let abi =
                crate::compiler_descriptor::physical_lds_exchange_v22::prepare_physical_lds_exchange_abi_v22(
                    &checked,
                    &bindings.typed_descriptor_roots,
                    budget,
                )
                .map_err(|error| {
                    stage_error(PhysicalLdsExchangeTargetStageErrorV22::Descriptor(error))
                })?;
            budget
        ...
        let emission = ledger.with_budget(|budget| {
            let (emission, storage) =
                ...
                .map_err(|error| {
                    stage_error(PhysicalLdsExchangeTargetStageErrorV22::Emission(error))
                })?;
            budget
        ...
        Ok(AuthenticatedPhysicalLdsExchangeTargetModuleV22 {
            checked,
            emission,
            abi,
            bindings,
