...
use crate::production_physical_lds_exchange_call_v39::{
    ActualPhysicalLdsExchangeCallV39, is_physical,
};
use crate::production_physical_lds_exchange_census_v39::{observe_root, require_body_bounds};
...
const MAX: usize = SEMANTIC_PHYSICAL_LDS_EXCHANGE_MAX_OCCURRENCES_V39;
...
pub(crate) const PREPAID_CONSTRUCTION_WORK_V39: usize = 2_200_000;
...
struct Slot<'tcx> {
    ...
    occurrence: u8,
    actual: ActualPhysicalLdsExchangeCallV39<'tcx>,
...
pub(crate) struct PhysicalLdsExchangeAnnotationsV39<'tcx> {
    root: Instance<'tcx>,
...
pub(crate) fn prepare<'tcx>(
    ...
    budget: &mut Budget<'_>,
) -> Result<PhysicalLdsExchangeAnnotationsV39<'tcx>, String> {
    ...
    let [function] = plan.roots() else {
        return Err("physical-lds-exchange requires one root".into());
    };
    let [root] = plan.function_producers() else {
        return Err("physical-lds-exchange excludes helpers".into());
    };
    let [retained] = plan.body_producers() else {
        return Err("physical-lds-exchange retained body absent".into());
    };
    if function.index() != 0
    ...
    {
        return Err("physical-lds-exchange actual root/call roster differs".into());
    }
    ...
    {
        return Err(
            "physical-lds-exchange terminal roster contains a foreign or excessive marker".into(),
        );
    }
    let target = crate::semantic_layout_bridge::rustc_semantic_layout_target_v1(tcx)
        .map_err(|_| "physical-lds-exchange active target absent")?;
    if target.llvm_target() != "amdgcn-amd-amdhsa"
        || !target.has_exact_codegen_profile("gfx942", "-wavefrontsize32,+wavefrontsize64,-xnack")
    {
        return Err("physical-lds-exchange requires exact gfx942 xnack-off Wave64".into());
    }
    let frontend = root
        .frontend_contract
        .as_ref()
        .ok_or("physical-lds-exchange frontend contract absent")?;
    let launch = frontend
        .contract()
        .launch()
        .ok_or("physical-lds-exchange explicit source launch absent")?;
    if launch.required().map(|n| n.as_array()) != Some([128, 1, 1])
        || launch.maximum().map(|n| n.as_array()) != Some([128, 1, 1])
        || launch.max_grid().map(|n| n.as_array()) != Some([1, 1, 1])
    {
        return Err("physical-lds-exchange requires exact authored launch128 and max_grid1".into());
    }
    if frontend.resource_contract().is_some_and(|r| {
        r.static_shared_memory_bytes() != 0 || r.max_dynamic_shared_memory_bytes() != 0
    }) {
        return Err(
            "physical-lds-exchange source resource annotations cannot override the typed LDS frame"
                .into(),
        );
    }
    let export = root
        .export_name
        .as_deref()
        .ok_or("physical-lds-exchange source export absent")?;
    if export.is_empty()
        || export.len() > 128
        || !export
            .bytes()
    ...
    {
        return Err(
            "physical-lds-exchange export is outside the closed ASCII symbol grammar".into(),
        );
    }
    ...
    {
        return Err("physical-lds-exchange current source bytes absent or oversized".into());
    }
    for amount in [
        transcript.len(),
    ...
    budget
        .charge_work(
            items
                .checked_mul(4)
                .ok_or("physical-lds-exchange source work overflow")?,
        )
        .map_err(|e| e.to_string())?;
    let census = observe_root(tcx, root.instance, body).map_err(str::to_owned)?;
    if census.len() != recipes.len() {
        return Err(
            "physical-lds-exchange retained recipes do not cover the complete source census".into(),
        );
    }
    let identities = canonical_function_identities_v1(tcx, root.instance);
    if identities != root.identities {
        return Err("physical-lds-exchange current full root Instance identity differs".into());
    }
    ...
    {
        return Err("physical-lds-exchange retained body coordinate census differs".into());
    }
    for (raw, semantic) in retained.raw_to_semantic_blocks.iter().enumerate() {
        let row = retained
            .blocks
            .get(semantic.index() as usize)
            .ok_or("physical-lds-exchange block mapping leaves roster")?;
        if row.rustc_block != raw as u32
            || row.identity != rustc_block_identity_v1(identities.function(), body_hash, raw as u32)
        {
            return Err(
                "physical-lds-exchange current raw-to-semantic block identity differs".into(),
            );
        }
    }
    for (raw, semantic) in retained.raw_to_semantic_locals.iter().enumerate() {
        ...
        {
            return Err("physical-lds-exchange raw-to-semantic local mapping differs".into());
        }
    }
    let (signature, abi) = crate::production_physical_lds_exchange_source_abi_v39::check(plan)
    ...
    for (occurrence, observed) in census.calls().enumerate() {
        ...
        let (recipe_index, recipe) = matching
            .next()
            .ok_or("physical-lds-exchange current occurrence recipe absent")?;
        if matching.next().is_some() || used_recipes[recipe_index] {
            return Err("physical-lds-exchange source occurrence recipe is ambiguous".into());
        }
        let terminal = terminals
            .get(recipe.terminal as usize)
            .ok_or("physical-lds-exchange source terminal association absent")?;
        ...
        {
            return Err(
                "physical-lds-exchange current exact callee/const/ABI occurrence differs".into(),
            );
        }
        ...
        .map_err(|_| "physical-lds-exchange source provenance is unavailable or overbound")?;
        if source.provenance() != row.terminator.provenance {
            return Err(
                "physical-lds-exchange retained source occurrence provenance differs".into(),
            );
        }
        ...
        if count == 2 {
            for (index, raw) in actual.argument_locals().into_iter().enumerate() {
                semantic_locals[index] = *retained
                    ...
                    .get(raw as usize)
                    .ok_or("physical-lds-exchange actual begin local mapping absent")?;
            }
        }
        let callee = recipe
            .terminal
            ...
            .ok_or("physical-lds-exchange callable index overflow")?;
    ...
    {
        return Err("physical-lds-exchange contains an unconsumed terminal or call recipe".into());
    }
    Ok(PhysicalLdsExchangeAnnotationsV39 {
        root: root.instance,
        function: *function,
        common,
        slots,
...
impl<'tcx> PhysicalLdsExchangeAnnotationsV39<'tcx> {
    ...
    pub(crate) fn attach(
        ...
        let index = self.slots[..self.count]
            ...
            .ok_or("physical-lds-exchange source occurrence missing or already consumed")?;
        let slot = self.slots[index]
            .as_ref()
            .ok_or("physical-lds-exchange source occurrence disappeared")?;
        let count = if matches!(
            actual.operation(),
            SemanticCompilerIntrinsicOperationV1::Gfx942PhysicalLdsExchangeBegin(_)
        ) {
        ...
        if current_root != self.root
            ...
            || call.physical_lds_exchange_source_v39().is_some()
            || call.physical_entry_source_v37().is_some()
        {
            return Err("physical-lds-exchange current raw/semantic occurrence join differs");
        }
        for (index, operand) in call.arguments().iter().enumerate() {
            let (place, moved) = match operand {
                ...
                _ => {
                    return Err(
                        "physical-lds-exchange emitted begin input is not direct transport",
                    );
                }
            };
            ...
            {
                return Err(
                    "physical-lds-exchange emitted begin input changed identity or move mode",
                );
            }
        }
        let source = SemanticPhysicalLdsExchangeSourceV39::new(
            self.common.axes,
            self.common.body,
        ...
        Ok(call.with_physical_lds_exchange_source_v39(source))
    }
    pub(crate) fn require_drained(&self) -> Result<(), &'static str> {
        if self.slots[..self.count].iter().any(Option::is_some) {
            Err("physical-lds-exchange source occurrence was not consumed")
        } else {
            Ok(())
        }
    }
