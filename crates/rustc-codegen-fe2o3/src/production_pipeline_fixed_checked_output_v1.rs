...
use super::checked_output_policy5_v1::{
    CheckedOutputPolicy5StageErrorV1, CheckedOutputTargetProductionCompilation as Direct,
};
use super::erased_checked_output_policy5_v1::ErasedCheckedOutputTargetProductionCompilationV1 as Erased;
...
#[cfg(test)]
#[path = "production_pipeline_fixed_checked_output_v1_tests.rs"]
mod tests;
...
pub(crate) struct FixedCheckedOutputProductionCompilationV1 {
    stage: Stage,
...
fn additional_header(source: SourcePolicy) -> Result<usize, ProductionPipelineError> {
    ...
    std::mem::size_of::<FixedCheckedOutputProductionCompilationV1>()
        .checked_sub(moved)
...
impl RankedVerifiedProductionCompilation {
    ...
    pub(crate) fn lower_fixed_checked_output_v1(
        self,
    ) -> Result<FixedCheckedOutputProductionCompilationV1, ProductionPipelineError> {
        ...
        let stage = transfer_header(source, &mut budget, || match source {
            ...
            SourcePolicy::RawEmpty => self.lower_checked_output_policy5_v1().map(Stage::Direct),
            SourcePolicy::UnitLocal => self
                .lower_silent_unit_checked_output_policy5_v1()
                .map(Stage::Erased),
        })?;
        ...
        Ok(FixedCheckedOutputProductionCompilationV1 {
            stage,
...
impl FixedCheckedOutputProductionCompilationV1 {
    ...
    pub(crate) fn checked_output(
        &self,
    ) -> &fe2o3_kernel_opt::CheckedCanonicalKernelIrOwnerPolicy5V1 {
        match &self.stage {
