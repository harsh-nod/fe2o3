...
use fe2o3_lower_mir_kernel::ProductionCheckedOutputOwnerPolicy5V1 as Admitted;

#[cfg(test)]
pub(crate) use super::checked_output_policy4_v1::snapshots;

#[derive(Debug)]
pub(crate) enum CheckedOutputPolicy5StageErrorV1 {
    Resource(Resource),
    Canonical(fe2o3_kernel_ir::CanonicalKernelIrReplayAdmissionErrorV12),
    Optimization(Box<fe2o3_kernel_opt::CanonicalPolicy5OptimizationErrorV1>),
    Admission(Box<fe2o3_lower_mir_kernel::ProductionCheckedOutputAdmissionErrorPolicy5V1>),
    NativeSource(Box<crate::production_native_source_lineage_v1::NativeSourceLineageErrorV1>),
    NativePublicationUnavailable,
}

impl fmt::Display for CheckedOutputPolicy5StageErrorV1 {
...
impl std::error::Error for CheckedOutputPolicy5StageErrorV1 {
...
fn resource(error: Resource) -> ProductionPipelineError {
    ProductionPipelineError::CheckedOutputPolicy5Stage(CheckedOutputPolicy5StageErrorV1::Resource(
        error,
    ))
}

pub(super) fn admission(
    error: fe2o3_lower_mir_kernel::ProductionCheckedOutputAdmissionErrorPolicy5V1,
) -> ProductionPipelineError {
    ProductionPipelineError::CheckedOutputPolicy5Stage(CheckedOutputPolicy5StageErrorV1::Admission(
        Box::new(error),
    ))
}
...
impl PreparedCheckedOutputArtifactsV1 {
    pub(crate) fn native_worker_output_v1(
        ...
        super::native_checked_output_handoff_v1::OutputInputsV1 {
            owner: super::native_checked_output_handoff_v1::OutputOwnerV1::Direct5(&self.admitted),
...
/// Additional retained logical payload receipt, not the input N/B/C/S/O receipt.
...
> {
    ...
    let (parts, retained) = prepare_checked_artifact_parts_v1(
        CheckedArtifactsOwnerRefV1::Direct5(&admitted),
        profile,
        typed_roots,
        source_envelope,
...
/// bindings alongside N/B/C/S/O and the actually replayed native handoff.
    ...
    pub(crate) fn lower_checked_output_policy5_v1(
        ...
        let (bound, bound_storage) =
            Owner::from_module_ref_with_verification_budget_v12(binding.module(), &mut budget)
                .map_err(|e| {
                    ProductionPipelineError::CheckedOutputPolicy5Stage(
                        CheckedOutputPolicy5StageErrorV1::Canonical(e),
                    )
                })?;
        budget
        ...
        let checked =
            fe2o3_kernel_opt::optimize_checked_canonical_kernel_ir_policy5_v1(&bound, &mut budget)
                .map_err(|e| {
                    ProductionPipelineError::CheckedOutputPolicy5Stage(
                        CheckedOutputPolicy5StageErrorV1::Optimization(Box::new(e)),
                    )
                })?;
        budget
...
impl CheckedOutputTargetProductionCompilation {
    ...
    > {
        ...
        let (source_lineage, storage) =
            ...
            .map_err(|error| {
                ProductionPipelineError::CheckedOutputPolicy5Stage(
                    CheckedOutputPolicy5StageErrorV1::NativeSource(Box::new(error)),
                )
            })?;
        Ok((
            NativeSourceCheckedOutputProductionCompilationV1 {
    ...
    > {
        if !self
            ...
            .is_extraction_only()
        {
            return Err(ProductionPipelineError::CheckedOutputPolicy5Stage(
