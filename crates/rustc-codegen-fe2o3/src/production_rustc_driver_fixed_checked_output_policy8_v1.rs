//! Literal fixed Policy8 extraction of actual K, without protected authority.
use super::*;
...
const DIAGNOSTIC_POLICY: u16 = 8;
#[cfg(test)]
use crate::production_pipeline::checked_output_policy8_v1::source_observation;

struct FixedCheckedOutputCallbacksPolicy8V1<'a> {
    output: &'a Path,
    ...
    #[cfg(test)]
    stage_observer: Option<source_observation::Observer>,
}

impl Callbacks for FixedCheckedOutputCallbacksPolicy8V1<'_> {
    fn after_analysis<'tcx>(&mut self, _compiler: &Compiler, tcx: TyCtxt<'tcx>) -> Compilation {
        #[cfg(test)]
        let result =
            fixed_census_invocation_observer_v1_tests::in_callback(self.observer.take(), || {
                source_observation::in_callback(self.stage_observer.take(), || {
...
fn extract_in_active_session(
    ...
    ranked
        .with_fixed_checked_output_policy8_extraction_v1(
            &mut budget,
            |observed, handoff, descriptor| {
                ...
                eprintln!(
                    "fe2o3 fixed checked-output extraction: original N {}, distinct E {:?}, historical I {}, historical J {}, actual K {}, fixed policy {}, target {:?}, {} ordered kernel(s), {} LLVM byte(s), retained logical floor {}; extraction-only observation, proof/compiler/artifact/load/launch authority false",
                    lower_hex_v1(&observed.original),
                    observed.erased.as_ref().map(|digest| lower_hex_v1(digest)),
                    lower_hex_v1(&observed.historical_i),
                    lower_hex_v1(&observed.historical_j),
...
/// Extracts actual final-K LLVM through the compiled Policy8 schedule.
...
#[doc(hidden)]
pub fn run_production_fixed_checked_output_policy8_extraction_driver_v1(
    args: &[String],
    output: &Path,
) -> Result<(), String> {
    ...
    let mut callbacks = FixedCheckedOutputCallbacksPolicy8V1 {
        output,
        result: None,
        census,
        #[cfg(test)]
        ...
        #[cfg(test)]
        stage_observer: source_observation::take_for_invocation(),
    };
    let fatal =
...
#[cfg(test)]
#[path = "production_rustc_driver_fixed_checked_output_policy8_v1_tests.rs"]
mod tests;
