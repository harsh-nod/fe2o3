//! Explicit inert preparation only; this wrapper deliberately has no native,
//! protected/finalizer or host-admission conversion.
...
use super::*;
use crate::compiler_descriptor::physical_entry_v20::PhysicalEntryPreparedAbiV20;
...
pub(crate) struct PreparedPhysicalEntryWorkerHandoffV20 {
...
impl PreparedPhysicalEntryWorkerHandoffV20 {
    pub(crate) fn unresolved_abi_conditions(&self) -> &'static str {
        self.abi.unresolved_conditions()
    }
    /// Explicit demotion only: returns inert protocol bytes/table, not checked
    /// native output or discharged runtime/allocation/launch authority.
...
pub(crate) fn prepare_physical_entry_worker_handoff_v20(
    authenticated: crate::production_pipeline::AuthenticatedPhysicalEntryTargetModuleV20,
) -> Result<PreparedPhysicalEntryWorkerHandoffV20, ProductionWorkerHandoffError> {
    ...
    let prepared=ledger.with_budget(|budget|{
        ...
        let compiler_module=crate::kernel_ir_codegen::retain_verified_physical_entry_compiler_module_text_v20(canonical,emission)
            .map_err(ProductionWorkerHandoffError::CompilerModule)?;
        let envelope=derive_production_compiler_ffi_envelope(target,module,&compiler_module,observed_source_envelope,
        ...
        let descriptor=crate::compiler_descriptor::physical_entry_v20::construct_physical_entry_descriptor_source_v20(
    ...
    // Unlike legacy preparation, exact source+combined memory+ABI conditions
    // survive until the one explicit inert-extraction demotion above.
    Ok(PreparedPhysicalEntryWorkerHandoffV20 {
        prepared,
        _checked: checked,
        abi,
