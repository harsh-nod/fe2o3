//! Typed final-J receipt custody. The underlying codecs are unchanged inert bytes.
use super::*;
...
/// A distinct type prevents accidental use of a final-I admission endpoint.
/// It grants no authority and never constructs a signed native owner.
pub(crate) struct NativeFinalOutputReceiptsPolicy7V1(Payloads);
impl NativeFinalOutputReceiptsPolicy7V1 {
...
pub(super) fn require_pair(left: usize, right: usize, budget: &mut Budget<'_>) -> Result7<usize> {
...
pub(super) fn finish_pair<T, A, B>(
    ...
    Ok((retained, Policy7NativeStorageV1(delta)))
}

fn check(
    native: &PreparedNativeCheckedOutputWorkerHandoffPolicy7V1,
    receipts: &NativeFinalOutputReceiptsPolicy7V1,
    budget: &mut Budget<'_>,
) -> Result7<usize> {
    scoped(0, budget, |budget| {
        ...
        // boundary. Full signed N/source/native replay precedes final-J joins.
...
/// Retains actual signed N/(E), checked history through J, and exact J receipts.
/// This is not a signed final refinement proof or heterogeneous wire admission.
pub(crate) struct PreparedNativeFinalOutputPolicy7V1 {
    pub(super) native: PreparedNativeCheckedOutputWorkerHandoffPolicy7V1,
    receipts: NativeFinalOutputReceiptsPolicy7V1,
...
impl PreparedNativeCheckedOutputWorkerHandoffPolicy7V1 {
    ...
    pub(crate) fn try_admit_final_output_receipts_v1(
        ...
        receipts: NativeFinalOutputReceiptsPolicy7V1,
        budget: &mut Budget<'_>,
    ) -> Result7<(PreparedNativeFinalOutputPolicy7V1, Policy7NativeStorageV1)> {
        scoped(0, budget, move |budget| {
            ...
            let (retained_floor, receipt) = finish_pair::<
                ...
                Self,
                NativeFinalOutputReceiptsPolicy7V1,
            >(incoming, budget)?;
            Ok((
                PreparedNativeFinalOutputPolicy7V1 {
                    native: self,
...
impl PreparedNativeFinalOutputPolicy7V1 {
    ...
    pub(crate) fn receipts(&self) -> &NativeFinalOutputReceiptsPolicy7V1 {
        &self.receipts
    }
