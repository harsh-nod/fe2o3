//! Genuine unsigned source/K component checks, never signed owner construction.
use super::*;
...
fn subject(graph: &Graph, artifacts: &PreparedPolicy8ArtifactsV1) -> Subject {
...
fn envelope(graph: &Graph, artifacts: &PreparedPolicy8ArtifactsV1) -> Vec<u8> {
...
fn reports(artifacts: &PreparedPolicy8ArtifactsV1, historical: bool) -> &[FormalMemoryObligations] {
...
fn formal(artifacts: &PreparedPolicy8ArtifactsV1, ranked: &Ranked, fault: Fault) -> Vec<u8> {
...
fn check_case(
    ...
    match expected {
        ...
        Some(expected) => assert!(
            matches!(result,
            Err(ProductionPipelineError::CheckedOutputPolicy8Stage(CheckedOutputPolicy8StageErrorV1::Native(ref e)))
...
pub(crate) fn exercise_final_k_components(
    artifacts: &PreparedPolicy8ArtifactsV1,
    ranked: &Ranked,
    typed: &[TypedDescriptorRootV1],
    budget: &mut Budget<'_>,
    ...
    );
    {
        ...
        historical_view.owner = match &artifacts.admitted {
            Admitted8::Direct(v) => OutputOwnerV1::Direct7(v.prefix()),
            Admitted8::Erased(v) => OutputOwnerV1::Erased7(v.prefix()),
        };
        assert!(matches!(
            joins::check_policy8(historical_view, ranked, &payloads, typed, budget),
            Err(NativeOutputHandoffErrorV1::Mismatch(
                "fixed Policy8 receipt owner"
            ))
        ));
        drop(payloads);
        budget.release_storage(storage.retained_storage()).unwrap();
    ...
    for (fault, expected) in [
        ...
        (Fault::Formal, "fresh final-K formal payload"),
    ] {
        check_case(
            artifacts,
            ranked,
    ...
    let (old_o, erased) = match &artifacts.admitted {
        ...
        Admitted8::Erased(v) => (
            v.prefix()
                ...
                .intermediate_policy5()
                .owner(),
            Some(v.prefix().prefix().erased()),
        ),
    };
    for old in [
        Some(artifacts.admitted.original()),
        Some(artifacts.admitted.historical_j()),
        Some(old_o),
        erased,
    ]
    .into_iter()
    ...
    if artifacts.test_deletion_count_v1() != 0 {
        ...
        assert_ne!(
            artifacts.admitted.historical_j().canonical().identity(),
            artifacts.output().canonical().identity(),
            "actual commutative substitution must still change the graph"
        );
        let expected = match &artifacts.admitted {
            Admitted8::Direct(_) => {
                ...
                assert_eq!(
                    ...
                    "pure bitwise removal preserves external obligations"
                );
                None
            }
            Admitted8::Erased(_) => {
                // Its genuine global Store precedes the bitwise additions.
                // Formal equality alone cannot distinguish these two graphs.
                assert!(
                    reports(artifacts, false)
                        .iter()
                        .chain(reports(artifacts, true))
                ...
                assert_eq!(
                    historical, good_formal,
                    "earlier global Store coordinates remain unchanged"
                );
                None
            }
        };
