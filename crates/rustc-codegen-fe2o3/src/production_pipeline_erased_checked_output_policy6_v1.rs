...
use super::checked_output_policy6_v1::CheckedOutputPolicy6StageErrorV1;
#[cfg(test)]
...
use fe2o3_lower_mir_kernel::ProductionUnitLocalErasedCheckedOutputOwnerPolicy6V1 as Admitted;
...
/// final fixed Policy6 custody. No conversion to the historical direct owner.
...
impl PreparedErasedCheckedOutputArtifactsV1 {
    pub(crate) fn native_worker_output_v1(
        ...
        super::native_checked_output_handoff_v1::OutputInputsV1 {
            owner: super::native_checked_output_handoff_v1::OutputOwnerV1::Erased6(&self.admitted),
...
/// Additional artifact payload only. Original source/N/E, B and checked C/S/O/I
...
> {
    let (parts, retained) = prepare_checked_artifact_parts_v1(
        CheckedArtifactsOwnerRefV1::Erased6(&admitted),
        profile,
        typed_roots,
        source_envelope,
...
/// with N/E/B/C/S/O/I and actual native preparation; none is replaced by inert rows.
...
/// source/N/E proof, E/B, checked C/S/O/I and the actual-I native artifacts. The
...
impl NativeSourceErasedCheckedOutputProductionCompilationV1 {
    pub(super) fn native_original_envelope_v1(&self) -> &[u8] {
        self.source_lineage.original_native_module()
    }
...
pub(super) struct AdmittedErasedPolicy6StageV1 {
...
impl RankedVerifiedProductionCompilation {
    ...
    #[allow(dead_code)]
    pub(crate) fn lower_silent_unit_checked_output_policy6_v1(
        self,
        ...
        let AdmittedErasedPolicy6StageV1 {
            admitted,
            ranked_verification,
            bindings,
        } = self.prepare_admitted_erased_policy6_v1(&mut budget)?;
    ...
    pub(super) fn prepare_admitted_erased_policy6_v1(
        self,
        budget: &mut Budget<'_>,
    ) -> Result<AdmittedErasedPolicy6StageV1, ProductionPipelineError> {
            ...
            Owner::from_module_ref_with_verification_budget_v12(binding.module(), budget).map_err(
                |error| {
                    ProductionPipelineError::CheckedOutputPolicy6Stage(
                        CheckedOutputPolicy6StageErrorV1::Canonical(error),
                    )
                },
            )?;
        budget
        ...
        let checked =
            fe2o3_kernel_opt::optimize_checked_canonical_kernel_ir_policy5_v1(&bound, budget)
                .map_err(|error| {
                    ProductionPipelineError::CheckedOutputPolicy6Stage(
                        CheckedOutputPolicy6StageErrorV1::Prefix(Box::new(error)),
                    )
                })?;
        budget
            .reserve_storage(checked.retained_storage())
            .map_err(resource)?;
        // The original N/E and one Policy5 prefix remain owned; only I is emitted.
        ...
        #[cfg(test)]
        phase.complete();
        Ok(AdmittedErasedPolicy6StageV1 {
            admitted,
            ranked_verification,
            bindings,
        })
    ...
    > {
        ...
        let (source_lineage, storage) =
            crate::production_native_source_lineage_v1::try_prepare_erased_native_source_lineage_v1(
                ...
                self.ranked_verification,
                budget,
            ).map_err(|error| ProductionPipelineError::CheckedOutputPolicy6Stage(
