//! Fixed Policy6 actual-I observation, without protected publication authority.
use super::*;
use crate::collector::source_census_v1::ExtractionMode as CensusMode;

const DIAGNOSTIC_POLICY: u16 = 6;

#[cfg(test)]
#[path = "production_rustc_driver_fixed_checked_output_policy6_v1_tests.rs"]
mod tests;

struct FixedCheckedOutputCallbacksPolicy6V1<'a> {
    output: &'a Path,
...
impl Callbacks for FixedCheckedOutputCallbacksPolicy6V1<'_> {
...
fn extract_in_active_session(
    ...
    .and_then(|ranked| ranked.lower_fixed_checked_output_policy6_v1())
    ...
    eprintln!(
        "fe2o3 fixed checked-output extraction: original N {}, distinct E {:?}, actual I {}, fixed policy {}, target {:?}, {} ordered kernel(s), {} LLVM byte(s), retained logical floor {}; extraction-only observation, proof/compiler/artifact/load/launch authority false",
...
/// Extracts actual final-I LLVM through one compiled checked schedule.
...
pub fn run_production_fixed_checked_output_policy6_extraction_driver_v1(
    args: &[String],
    output: &Path,
) -> Result<(), String> {
    ...
    let mut callbacks = FixedCheckedOutputCallbacksPolicy6V1 {
        output,
        result: None,
        census,
        #[cfg(test)]
