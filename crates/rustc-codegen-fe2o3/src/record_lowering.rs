...
fn slice_access_from_memory(
    ...
    if matches!(access.place.projection.as_slice(), [projection] if projection == "deref")
        && let Some(access) = disjoint_refs.get(&access.place.local)
    {
...
fn bind_disjoint_call_output(
    ...
    let index = if callee == TrustedDeviceItem::DisjointSliceGetMut {
        operands
            .get(1)
            .and_then(|operand| RecordPlaceRef::parse(operand))
            .filter(|place| {
    ...
    } else if callee == TrustedDeviceItem::DisjointSliceGetMutAt {
        operands
            .get(1)
