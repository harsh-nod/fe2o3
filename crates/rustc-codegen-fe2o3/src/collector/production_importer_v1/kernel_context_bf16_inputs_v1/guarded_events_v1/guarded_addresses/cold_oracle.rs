...
fn conjuncts(p: &Formula, root: Id, facts: &mut [bool; NODES], f: Charge<'_>) -> Result<()> {
...
fn guarded(
    ...
    for (id, included) in facts.iter().enumerate().take(p.len()) {
        charge(f, 1)?;
        if !included {
            continue;
        }
...
fn range(p: &Formula, index: Id, facts: &[bool; NODES], f: Charge<'_>) -> Result<()> {
...
pub(super) fn check(p: &Formula, f: Charge<'_>) -> Result<()> {
    ...
    charge(
        f,
        (4 * std::mem::size_of::<[bool; NODES]>() + std::mem::size_of::<[Option<Range>; NODES]>())
    ...
    for event in p.events {
        ...
        let mut facts = [false; NODES];
        conjuncts(p, event.guard, &mut facts, f)?;
        let mut extent = false;
        for (id, included) in facts.iter().enumerate().take(p.len()) {
            charge(f, 1)?;
            extent |= *included
                && matches!(p.node(id as Id), Ok(Node::Compare(Compare::Less, actual, 7)) if actual == event.index);
        }
        if !extent {
            return Err(Error::Changed);
        }
