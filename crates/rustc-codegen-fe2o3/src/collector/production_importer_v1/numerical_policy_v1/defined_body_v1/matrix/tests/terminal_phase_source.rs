...
pub fn matrix_terminal_phase_abis(
    ...
    context.with_workgroup(|workgroup| {
        ...
        phases.with_phase(|phase| {
            let result = {
                ...
                subgroup.with_matrix(phase.epoch(), |matrix, lane| {
                    let bound = matrix.with_numerical_policy(&policy);
                    let narrowed = bound.gfx950();
                    let Ok(a4) = narrowed.fp4_a_global_row_major(&bits, 0, rows, columns, stride)
                    else {
                        return Err(KernelError::InvalidArgument);
                    };
                    let Ok(b4) = narrowed.fp4_b_global_row_major(&bits, 0, rows, columns, stride)
                    else {
                        return Err(KernelError::InvalidArgument);
                    };
                    let Ok(a8) = narrowed.fp8_a_global_row_major(&bits, 0, rows, columns, stride)
                    else {
                        return Err(KernelError::InvalidArgument);
                    };
                    let Ok(b8) = narrowed.fp8_b_global_row_major(&bits, 0, rows, columns, stride)
                    else {
                        return Err(KernelError::InvalidArgument);
                    };
            ...
            (phase.finish_reusable_phase(), result)
        })
    })
}
