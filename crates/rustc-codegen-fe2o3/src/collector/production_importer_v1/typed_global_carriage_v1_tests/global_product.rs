//! Source-owned multi-capture retention through the existing process-isolated
//! actual AMD importer. This stops before memory equivalence or final lowering.
use super::*;
use fe2o3_pliron::{
...
macro_rules! root {
    ($name:ident) => {
        ...
        pub fn $name(mut context: KernelContext<'_>, left: Global<'_, u8, ReadOnly>, right: Global<'_, u8, ReadOnly>) {
...
impl Callbacks for ProductProbe {
    fn config(&mut self, config: &mut Config) {
        config.input = Input::Str {
            name: FileName::Custom("global_product_source.rs".into()),
            input: SOURCE.into(),
        };
    }
    ...
    fn after_analysis<'tcx>(&mut self, _: &Compiler, tcx: TyCtxt<'tcx>) -> Compilation {
        ...
        .expect("collect original registered Global binds and closure calls");
        assert!(
            closure
                .collection
                .functions
        ...
        .expect("actual multi-capture source must import without a collector exception");
        ...
        let make_owner = |mir| {
            ...
            .expect("actual source closure must reach the checked execution SSA view")
        };
        ...
        for (index, root) in roots.iter().copied().enumerate() {
            ...
            let entry = imported
                ...
                .unwrap()
                .expect("actual source entry relation is retained");
            crate::production_ranked_projection_v1::global_product_source_checks::check(
                &owner,
                &foreign,
                root,
                roots[1 - index],
...
fn global_product_source_gfx942() {
    run("gfx942", "global_product::global_product_source_gfx942");
}

#[test]
#[ignore = "requires cached actual AMD metadata; no Cargo or dependency rebuild"]
fn global_product_source_gfx950() {
    run("gfx950", "global_product::global_product_source_gfx950");
}
