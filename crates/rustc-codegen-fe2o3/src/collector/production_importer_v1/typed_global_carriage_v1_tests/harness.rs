...
const CRATE_NAME: &str = "typed_global_carriage_source";
const METADATA: &str = "fe2o3-typed-global-carriage-v17";
const CHILD_ENV: &str = "FE2O3_TYPED_GLOBAL_CARRIAGE_CHILD";
...
impl Scratch {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "fe2o3-typed-global-carriage-{}-{nonce}",
            std::process::id()
        ));
        ...
        std::fs::write(scratch.0.join("Cargo.toml"), format!(
            "[package]\nname = \"typed-global-carriage-source\"\nversion = \"0.0.0\"\nedition = \"2024\"\n[workspace]\n[dependencies]\nfe2o3-device = {{ path = {device:?} }}\n"
        )).unwrap();
        scratch
    }
}
...
pub(super) fn run(cpu: &'static str, name: &str) {
    let mut probe = Probe { cpu, completed: false };
    if run_probe(cpu, name, &mut probe) {
        assert!(probe.completed, "must import canonical V17 and complete every carriage negative");
    }
}
...
pub(super) fn run_probe(
    ...
    if std::env::var(CHILD_ENV).ok().as_deref() != Some(child_key.as_str()) {
        ...
        let test =
            format!("collector::production_importer_v1::typed_global_carriage_v1_tests::{name}");
        let output = crate::process_execution::capture_output(
            std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env("CARGO_PKG_NAME", "typed-global-carriage-source")
                .env(
        ...
        assert!(
            ...
            "typed-global full-import child failed:\n{}\n{}",
...
#[test]
fn typed_global_registration_uses_session_derived_binding() {
    ...
    assert_ne!(
        registration_binding_v1(),
        derive_crate_binding_id_v1("different_typed_global_crate", [METADATA])
    );
    assert_ne!(
        registration_binding_v1(),
        derive_crate_binding_id_v1(CRATE_NAME, ["different_typed_global_metadata"])
    );
    assert!(!super::SOURCE.contains("namespace"));
}
