...
const CRATE_NAME: &str = "exclusive_reference_source";
const METADATA: &str = "fe2o3-exclusive-reference-v1";
const CHILD_ENV: &str = "FE2O3_EXCLUSIVE_REFERENCE_CHILD";
...
impl Scratch {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "fe2o3-exclusive-reference-{}-{nonce}",
            std::process::id()
        ));
        ...
        std::fs::write(scratch.0.join("Cargo.toml"), format!(
            "[package]\nname = \"exclusive-reference-source\"\nversion = \"0.0.0\"\nedition = \"2024\"\n[workspace]\n[dependencies]\nfe2o3-device = {{ path = {device:?} }}\n"
        )).unwrap();
        scratch
    }
}
...
pub(super) fn run_probe(
    ...
    if std::env::var(CHILD_ENV).ok().as_deref() != Some(child_key.as_str()) {
        ...
        let test = format!("collector::exclusive_reference_v1::compiler_tests::{name}");
        let output = crate::process_execution::capture_output(
            std::process::Command::new(std::env::current_exe().unwrap())
                ...
                .env("CARGO_PKG_NAME", "exclusive-reference-source")
                .env(
        ...
        assert!(
            ...
            "exclusive-reference source child failed:\n{}\n{}",
...
#[test]
fn exclusive_reference_registration_uses_session_derived_binding() {
