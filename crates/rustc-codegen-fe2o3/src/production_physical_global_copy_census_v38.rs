//! Whole actual Rust MIR census for declarative physical-global-copy primitives.
//! The Rust body is one acyclic marker chain; this does not execute native steps.
use crate::production_physical_global_copy_call_v38::{ActualPhysicalGlobalCopyCallV38, observe};
use fe2o3_mir_model::semantic_mir_v1::{
    SEMANTIC_PHYSICAL_GLOBAL_COPY_MAX_OCCURRENCES_V38 as MAX_OCCURRENCES,
...
#[path = "production_physical_global_copy_transport_v38.rs"]
mod transport;
...
pub(crate) struct Occurrence<'tcx> {
    ...
    pub(crate) actual: ActualPhysicalGlobalCopyCallV38<'tcx>,
}
...
pub(crate) fn require_body_bounds(body: &Body<'_>) -> Result<usize, &'static str> {
    ...
    {
        return Err("physical-global-copy source body/local bound exceeded");
    }
    ...
    for block in body.basic_blocks.iter() {
        items = items
            ...
            .ok_or("physical-global-copy source item overflow")?;
        if items > MAX_SOURCE_ITEMS {
            return Err("physical-global-copy source item bound exceeded");
        }
    }
    Ok(items)
}
...
pub(crate) fn observe_root<'tcx>(
    ...
    if body.arg_count != 2
        ...
        || !crate::production_physical_global_copy_call_v38::valid_root_signature(tcx, &signature)
    {
        return Err("physical-global-copy root signature differs");
    }
    for (index, ty) in signature.inputs().iter().enumerate() {
        ...
        {
            return Err("physical-global-copy root MIR argument type differs");
        }
    }
    ...
    loop {
        let block = body
            .basic_blocks
            .get(next)
            .ok_or("physical-global-copy source edge leaves body")?;
        if visited[next.index()] || block.is_cleanup {
            return Err("physical-global-copy source chain cycles or enters cleanup");
        }
            ...
            match &statement.kind {
                ...
                StatementKind::Assign(assignment) => {
                    ...
                    if !destination.projection.is_empty() {
                        return Err("physical-global-copy source writes a projection");
                    }
                    let Rvalue::Use(operand) = value else {
                        return Err(
                            "physical-global-copy source computes outside authored instructions",
                        );
                    };
                    if destination.local.as_u32() == 0 {
                        if !matches!(operand,Operand::Constant(value) if value.const_.ty().is_unit())
                        {
                            return Err("physical-global-copy return assignment is not unit");
                        }
                        continue;
                    }
                    if census.count != 0 {
                        return Err(
                            "physical-global-copy source performs argument transport after begin",
                        );
                    }
                    let (source, moved) = match operand {
                        ...
                        _ => {
                            return Err(
                                "physical-global-copy source contains a foreign argument assignment",
                            );
                        }
                    };
                    ...
                    {
                        return Err(
                            "physical-global-copy source argument transport projects or changes type",
                        );
                    }
                    transport.assign(destination.local.index(), source.local.index(), moved)?;
                }
                _ => return Err("physical-global-copy source has an unadmitted statement"),
            }
        }
        next = match &block.terminator().kind {
            ...
            TerminatorKind::Call {
                ...
                {
                    return Err(
                        "physical-global-copy marker count or unit call destination differs",
                    );
                }
                let actual = match args.len() {
                    0 => observe(tcx, root, body, func, &[])?,
                    2 => observe(tcx, root, body, func, &[&args[0].node, &args[1].node])?,
                    _ => return Err("physical-global-copy marker runtime arity differs"),
                };
                match actual.operation() {
                    Operation::Gfx942PhysicalGlobalCopyBegin if census.count == 0 => {
                        transport
                            .consume_marker(actual.argument_locals(), actual.moved_arguments())?;
                    }
                    Operation::Gfx942PhysicalGlobalCopyLabel(0) if census.count == 1 => {
                        labels += 1;
                        if labels > 1 {
                            return Err("physical-global-copy source exceeds one label");
                        }
                    }
                    Operation::Gfx942PhysicalGlobalCopyStep(step)
                        if census.count > 1 && labels != 0 =>
                    {
                        if ended {
                            return Err("physical-global-copy source step follows endpgm");
                        }
                        native += 1;
                        ended = step.opcode() == 14;
                        if native > 32 {
                            return Err("physical-global-copy native instruction bound exceeded");
                        }
                    }
                    _ => return Err("physical-global-copy begin/label/step source order differs"),
                }
            ...
            TerminatorKind::Return => {
                ...
                {
                    return Err(
                        "physical-global-copy source labels, instructions or complete chain census differ",
                    );
                }
                return Ok(census);
            }
            _ => {
                return Err(
                    "physical-global-copy source has a foreign call, Rust branch or hidden effect",
                );
            }
        };
    }
