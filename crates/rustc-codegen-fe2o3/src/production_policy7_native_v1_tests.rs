...
use final_receipts::{
    NativeFinalOutputReceiptsPolicy7V1 as Final, PreparedNativeFinalOutputPolicy7V1,
};
use input_association::{
    NativeOriginalInputAssociationReceiptsPolicy7V1 as Input,
...
#[path = "production_policy7_native_receipt_components_v1_tests.rs"]
mod components;
pub(crate) use components::exercise_final_j_components;

fn is_accounting(result: &Result7<()>) -> bool {
    matches!(
        result,
...
#[test]
fn policy7_native_source_requires_real_signature_without_emitting_native_i() {
    for profile in [Profile::Gfx942, Profile::Gfx950] {
        for duplicate in [false, true] {
            let check = |admitted: Admitted7, ranked: Ranked, budget: &mut Budget<'_>| {
                // Only a lowerer owner exists here. No I or J native artifact
                // constructor has run, including during the actual source call.
                let n = admitted.original() as *const Graph;
                let j = admitted.output() as *const Graph;
                ...
                assert!(matches!(result,
                    Err(ProductionPipelineError::CheckedOutputPolicy7Stage(
                        CheckedOutputPolicy7StageErrorV1::NativeSource(ref e)))
                ...
                assert_eq!(j, admitted.output() as *const Graph);
            };
            with_backend_policy7_direct_prefix_v1(profile, duplicate, |prefix, ranked, budget| {
                let (admitted, receipt) =
                    prefix.continue_redundant_private_stores_v1(budget).unwrap();
                budget.reserve_storage(receipt.retained_storage()).unwrap();
                check(Admitted7::Direct(admitted), ranked, budget);
                budget.release_storage(receipt.retained_storage()).unwrap();
            });
            with_backend_policy7_erased_prefix_v1(profile, duplicate, |prefix, ranked, budget| {
                let (admitted, receipt) =
                    prefix.continue_redundant_private_stores_v1(budget).unwrap();
                budget.reserve_storage(receipt.retained_storage()).unwrap();
                check(Admitted7::Erased(admitted), ranked, budget);
                budget.release_storage(receipt.retained_storage()).unwrap();
            });
        }
    }
...
#[test]
fn policy7_native_header_credits_ranked_once_and_retains_old_envelope() {
    for header in [
        ...
        let credited = size_of::<CheckedOutputTargetProductionCompilationPolicy7V1>() + header
            - size_of::<Ranked>();
        assert!(credited + delta >= size_of::<PreparedNativeCheckedOutputWorkerHandoffPolicy7V1>());
        assert_eq!(
            delta,
            size_of::<PreparedNativeCheckedOutputWorkerHandoffPolicy7V1>().saturating_sub(credited)
        );
        let run = |limit| {
...
#[test]
fn policy7_native_pair_floor_and_wrapper_deltas_are_exact() {
    ...
    let first = scoped(128, &mut budget, |budget| {
        final_receipts::finish_pair::<
            PreparedNativeFinalOutputPolicy7V1,
...
#[test]
fn policy7_native_opaque_receipt_copy_exact_work_storage_and_no_admission() {
    for input in [false, true] {
        let run = |work_limit, storage_limit| {
            ...
            let result = if input {
                ...
                Final::try_from_preimages_v1(b"not a J envelope", b"unsigned formal", &mut budget)
                    .map(|(v, s)| {
                        assert_eq!(v.kernel_ir().canonical_preimage(), b"not a J envelope");
                        assert_eq!(v.formal_memory().canonical_preimage(), b"unsigned formal");
                        assert_eq!(v.retained_storage(), s.retained_storage());
                        drop(v);
                    })
...
#[test]
fn policy7_native_pair_scope_drops_failed_owners_and_keeps_work() {
    for panics in [false, true] {
        let mut work = Work::new(100);
        let mut budget = Budget::new(&mut work, 1000);
        budget.reserve_storage(71).unwrap();
        let drops = Arc::new(AtomicUsize::new(0));
        let result: Result7<()> = scoped(71, &mut budget, |budget| {
            ...
            >(71, budget)?;
            if panics {
                panic!("native7 failure after receipt growth");
            }
            Err(execution_error("native7 failure after receipt growth"))
        });
...
#[test]
fn policy7_native_pair_scope_never_refunds_foreign_ledger() {
    for mode in 0..3 {
        let mut work = Work::new(100);
