...
pub(crate) fn check(
    ...
    let [abi] = plan.function_abi_producers() else {
        return Err("physical-global-copy current root ABI roster differs");
    };
