...
use fe2o3_kernel_ir::{
    AccessMode, BasicBlock, BinaryOp, CastKind, CheckedBinaryOperator, ComparePredicate, Constant,
    IntrinsicOperation, MatrixOperation, Operation, OperationKind, ScalarType, Terminator, Type,
    WorkgroupSize,
};
...
pub(super) fn lower_call(
    ...
    match call.item {
        ...
        SessionRecognizedSemanticItem::TrustedDevice(
            TrustedDeviceItem::Bf16MfmaFragmentFromBits,
        ) => lower_bf16_fragment_from_bits(lowerer, call, block),
        SessionRecognizedSemanticItem::TrustedDevice(
            TrustedDeviceItem::F32AccumulatorFragmentFromValues,
        ) => lower_f32_fragment_from_values(lowerer, call),
        SessionRecognizedSemanticItem::TrustedDevice(
            TrustedDeviceItem::F32AccumulatorFragmentIntoValues,
...
fn lower_bf16_fragment_from_bits(
    lowerer: &mut FunctionLowerer<'_, '_>,
    call: SessionRecognizedSemanticCall<'_>,
    block: &mut BasicBlock,
) -> Result<Terminator, TranslationDiagnostic> {
    let bits = require_fixed_array4(
        lowerer,
        call,
        &MirTypeShape::U16,
    ...
    require_shape(
        ...
        "Bf16MfmaFragment::from_bits destination",
        call.location,
    )?;
    let bf16 = Type::Scalar(ScalarType::Bf16);
    let mut values = Vec::with_capacity(4);
    for value in bits {
        values.push(lowerer.emit_result(
            block,
            bf16.clone(),
            OperationKind::Cast {
    ...
    let values: [_; 4] = values.try_into().map_err(|_| {
        diagnostic(
            TranslationDiagnosticCode::MalformedMir,
            call.location.clone(),
            "authenticated BF16 fragment did not lower to four bit-preserving lanes",
        )
    })?;
    lowerer.bind_local(
        call.destination.local,
        LocalBinding::Bf16MfmaFragment(values),
        call.location.clone(),
    )?;
    branch_to_call_target(lowerer, call)
}

fn lower_f32_fragment_from_values(
    lowerer: &mut FunctionLowerer<'_, '_>,
    call: SessionRecognizedSemanticCall<'_>,
) -> Result<Terminator, TranslationDiagnostic> {
    lowerer.require_strict_float_policy(call.location)?;
    let values = require_fixed_array4(
        lowerer,
        call,
        &MirTypeShape::F32,
    ...
    require_shape(
        ...
        "F32AccumulatorFragment::from_values destination",
        call.location,
    )?;
    lowerer.bind_local(
        call.destination.local,
        LocalBinding::F32AccumulatorFragment(values),
        call.location.clone(),
    )?;
    branch_to_call_target(lowerer, call)
}
...
fn require_fixed_array4(
        ...
        )
    {
        return Err(diagnostic(
            ...
            call.location.clone(),
            format!("{role} requires one exact four-element scalar array"),
        ));
    }
    match lowerer.locals.get(&place.local).copied() {
        Some(LocalBinding::FixedArray4(values)) => Ok(values),
        _ => Err(diagnostic(
            TranslationDiagnosticCode::UnsupportedType,
            call.location.clone(),
            format!("{role} array does not originate from authenticated scalar elements"),
        )),
    }
}
