...
fn metadata_refusal(result: Result8<()>) {
    assert!(matches!(
        result,
    ...
    for limit in [7 + scan, 7 + scan - 1] {
        ...
        if limit == 7 + scan {
            ...
            assert!(matches!(
                result,
                Err(ProductionPipelineError::CheckedOutputPolicy8Stage(
                    CheckedOutputPolicy8StageErrorV1::Resource(Resource::Work(_))
                ))
            ));
        }
...
#[test]
fn copied_metadata_scan_keeps_fixed7_checks_and_charge_sequence_identical() {
    fn scan(source: &str) -> String {
        source
            .split_once("fn check_extraction_metadata(")
            .unwrap()
            .1
            .split_once("\nimpl CheckedOutputTargetProductionCompilationPolicy")
            .unwrap()
            .0
            .replace("Result7", "Result8")
            .split_whitespace()
            .collect::<String>()
    }
    // Source parity deliberately complements, rather than substitutes for, the
    // exact/one-short and hostile runtime controls above. No old API is widened.
    assert_eq!(
        scan(include_str!("production_policy7_extraction_v1.rs")),
        scan(include_str!("production_policy8_extraction_v1.rs")),
    );
}

#[test]
fn extraction_panic_drops_outputs_then_restores_inherited_floor_before_payload_drop() {
    struct Payload;
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        let _: Result8<()> = scoped(13, &mut budget, |budget| {
            budget.charge_work(5).unwrap();
            budget.reserve_storage(11).unwrap();
            let _output = Dropped(Rc::clone(&dropped));
