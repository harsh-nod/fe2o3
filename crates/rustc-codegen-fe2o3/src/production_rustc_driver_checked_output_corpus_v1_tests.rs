...
#[test]
fn attention_configurations_retain_distinct_features_and_source_members() {
    let bytes = include_bytes!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../../config/tutorial-kernel-manifest-v1.json"
    ));
    let all = fixtures(bytes).unwrap();
    for name in ["pipelined-attention", "scalar-attention"] {
        let id = format!("gfx950-gpt-oss-{name}");
        let fixture = all.iter().find(|fixture| fixture.fixture_id == id).unwrap();
        let input = &fixture.compiler_input;
        assert_eq!(fixture.target, "gfx950");
        assert!(!input.default_features);
        assert_eq!(input.features, [format!("kernel-gpt-oss-decode-{name}")]);
        assert_eq!(
            input.source_paths,
            [format!(
                "examples/gfx950_gpt_oss_decode/src/kernel_{}.rs",
                name.replace('-', "_")
            )]
        );
        assert_eq!(
            input.kernel_symbols,
            ["gfx950_gpt_oss_120b_decode_megakernel_v1"]
        );
        let mut missing: serde_json::Value = serde_json::from_slice(bytes).unwrap();
        missing["compilerFixtures"]
            .as_array_mut()
            .unwrap()
            .retain(|row| row["fixtureId"] != id);
        assert!(fixtures(&serde_json::to_vec(&missing).unwrap()).is_err());
    }
}
