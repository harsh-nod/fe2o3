...
pub(crate) enum NominalDescriptorErrorV3 {
    ...
    Unroll(super::checked_output_policy3_v1::refined_forwarding_v1::loop_unroll_v1::LoopUnrollDescriptorErrorV1),
...
impl fmt::Display for E {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "nominal source descriptor: {self:?}")
    }
}
...
pub(super) fn nominal_kind_matches(
    kind: DescriptorArgumentKindV1,
    rust: SemanticRustTypeKindV1,
) -> bool {
    match kind {
...
fn inert_capabilities(
    ...
    // Source-only uses the retained planned profile without claiming N is bound.
    // The final-U caller separately replays its actual target-binding history.
    // Neither route derives diagnostic permission from an arbitrary IR marker.
    fn visit(
        ...
        if let TargetCapabilityRefV1::Extension { namespace, name } = capability {
            // Prepay the two local guards and the classifier's bounded string
            // comparisons before inspecting names, including hostile long names.
            let work = namespace
                .len()
            ...
            if namespace == fe2o3_kernel_ir::AMDGPU_GFX942_DIAGNOSTICS_CAPABILITY_NAMESPACE
                && name.matches(fe2o3_kernel_ir::AMDGPU_GFX942_DIAGNOSTICS_CAPABILITY_NAME)
                && profile != ProductionAmdTargetProfileV1::Gfx942
            {
                return Err(E::UnsupportedRequirements);
            }
        }
        let diagnostic_target = matches!(
            profile,
    ...
    for cap in &module.required_capabilities {
        visit(TargetCapabilityRefV1::from_owned(cap), profile, budget)?;
    }
            ...
            for block in &body.blocks {
                ...
                for operation in &block.operations {
                    ...
                    operation.try_visit_required_capabilities_v1(|capability| {
                        visit(capability, profile, budget)
                    })?;
                }
            }
        }
    }
    Ok([CapabilityV1::AmdWave])
}
...
// Only the source owner above and the actual-U descriptor child call this
// private encoder. It produces content bytes, never an authenticated owner.
#[allow(
    clippy::too_many_arguments,
    reason = "Explicit borrowed source/final subject inputs."
)]
    ...
    scoped(budget, |budget| {
        ...
        budget.reserve_storage(
            DESCRIPTOR_QUERY_STORAGE_V3
                ...
                .and_then(|v| v.checked_add(size_of::<[CapabilityV1; 1]>()))
                .ok_or(Resource::Arithmetic)?,
        )?;
        ...
        let capabilities = inert_capabilities(module, profile, budget)?;
