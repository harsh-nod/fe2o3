...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum CheckedReferenceSourceV1 {
    GuardedAccess(usize),
    ProjectedSharedBorrow,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct CheckedReferenceOriginV1 {
    source: CheckedReferenceSourceV1,
    availability: Option<CapabilityAvailabilityV1>,
}

struct CheckedReferencesV1 {
    ...
    for (block_index, block) in function.blocks().iter().enumerate() {
        let guard = match block.terminator().kind() {
            SemanticTerminatorKindV1::Assert {
                ...
                BoundsGuardV1 {
                    condition_local: simple_operand_local(condition).ok_or(
                    ...
                    access_block: target.target().index() as usize,
            ...
            SemanticTerminatorKindV1::SwitchInt {
                ...
                if length_definition.length_source.is_none() {
                    continue;
                }
                ...
                let access_block = match target.value() {
                    0 => targets.otherwise().target().index() as usize,
                    1 => target.edge().target().index() as usize,
                    _ => {
                        return Err(ProductionRankedProjectionErrorV1::Incomplete(
...
fn project_uniform_inductions_v1(
    ...
    let address_escaped = assertion_definition_inventory(function)?.address_escaped;
...
#[derive(Clone, Copy, Debug)]
struct PureUniformIndexValueV1 {
    ranked: ProductionRankedValueV1,
    maximum: u64,
    exact: Option<u64>,
}
...
mod tests {
    ...
    fn merged_guard_with_post_edge_definition(
        ...
        projection_function_with_locals(
            vec![
                ...
                block(180, vec![], SemanticTerminatorKindV1::Return),
            ],
            vec![
                local(173, SCALAR_TYPE, SemanticLocalRoleV1::Return),
                local(174, U64_TYPE, SemanticLocalRoleV1::Temporary),
                local(175, U64_TYPE, SemanticLocalRoleV1::Temporary),
        ...
        projection_function_with_locals(
            vec![
                ...
                block(
                    ...
                    SemanticTerminatorKindV1::Assert {
                        condition: typed_operand(5, BOOL_TYPE),
                        expected: false,
                        message: SemanticAssertMessageV1::DivisionByZero(typed_operand(
                            4, U64_TYPE,
                        )),
            ...
            vec![
                local(191, SCALAR_TYPE, SemanticLocalRoleV1::Return),
    ...
    fn project_test_inductions(
        ...
        ProductionRankedProjectionErrorV1,
    > {
        project_test_inductions_with_types(function, &projection_types())
    }
    ...
    fn pure_argument_derived_induction_bound_remains_uniform() {
        ...
        let (inductions, operations, _) =
            project_test_inductions_with_types(&function, &types).unwrap();
        assert_eq!(inductions.len(), 1);
        assert!(operations.iter().any(|operation| matches!(
            operation,
            ProductionRankedOperationV1::IndexBinary {
    ...
    fn derived_induction_bound_that_can_wrap_its_source_type_fails_closed() {
        ...
        assert_incomplete(
            project_test_inductions_with_types(&function, &types),
            "a uniform induction bound is not an exact total unsigned index expression",
        );
    }
