...
pub(crate) fn with_backend_checked_output_policy6_owned_v1(
...
pub(crate) fn with_backend_erased_output_policy6_owned_v1(
    ...
    with_backend_erased_load_roster_v1(
        ...
        |source, bound, ranked, budget| {
            ...
            budget.reserve_storage(storage).unwrap();
            let floor = budget.storage();
            let admitted = fe2o3_lower_mir_kernel::ProductionUnitLocalErasedCheckedOutputOwnerPolicy6V1::try_admit_v1(source, bound, checked, budget).unwrap();
            next(admitted, ranked, budget);
            assert_eq!(budget.storage(), floor);
            budget.release_storage(storage).unwrap();
        },
