...
type Expression = ProductionSemanticExpressionV2;
...
fn argument_indices(
    ...
    meter.work(1)?;
    if depth > DEPTH {
        return Err("template argument exceeds depth limit");
    }
    let next = depth
        .checked_add(1)
        .ok_or("template argument depth overflow")?;
    ...
    match expression {
        ...
        Expression::Unary { operand, .. } | Expression::Cast { operand, .. } => {
            argument_indices(operand, next, meter)
        }
        Expression::Binary { lhs, rhs, .. } | Expression::Compare { lhs, rhs, .. } => sum(
            argument_indices(lhs, next, meter)?,
            argument_indices(rhs, next, meter)?,
        ),
        Expression::Select {
            condition,
            when_true,
            ...
            let first = argument_indices(condition, next, meter)?;
            let second = argument_indices(when_true, next, meter)?;
            sum(
                sum(first, second)?,
                argument_indices(when_false, next, meter)?,
            )
        }
    }
}
    ...
    pub(super) fn instantiate(
        ...
        let prepared = (|| {
            for (argument, scalar) in arguments.iter().zip(&self.parameters) {
                ...
                let indices = argument_indices(argument, 0, meter)?;
...
#[cfg(test)]
#[path = "helper_value_template_v1_tests.rs"]
mod tests;
