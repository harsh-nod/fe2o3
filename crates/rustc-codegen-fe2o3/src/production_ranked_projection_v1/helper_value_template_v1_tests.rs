...
fn nested_templates_keep_each_call_arguments_separate() {
    ...
    assert_ne!(lhs, rhs);
    assert_eq!(actual.validate().unwrap().nodes, 7);
    release_expression(actual, bytes, &mut meter);
...
#[test]
fn repeated_load_parameters_pay_boxed_indices_and_clone_work_before_emit() {
    let mut setup = TestMeter::generous();
    let mut template = Template::new(&[U32], &mut setup).unwrap();
    let parameter = template.push(U32, Kind::Parameter(0), &mut setup).unwrap();
    let result = template
        .push(
            U32,
            Kind::Binary(
        ...
        .unwrap();
    template.finish(result, U32).unwrap();
    let input = Expression::Load(ProductionSemanticLoadV2 {
        block: 0,
        operation: 1,
        scalar: U32,
        allocation_origin: 1,
        ...
        indices: (0..8)
            .map(ProductionRankedValueV1::Argument)
            .collect::<Vec<_>>()
            .into_boxed_slice(),
    });
    let input_storage =
        std::mem::size_of::<Expression>() + 8 * std::mem::size_of::<ProductionRankedValueV1>();
    setup.reserve(input_storage).unwrap();
    let floor = setup.storage;
    setup.peak = floor;
    let start = setup.work;
    let (actual, output_storage) = template
        .instantiate(std::slice::from_ref(&input), &mut setup)
        .unwrap();
    let work = setup.work - start;
    let peak = setup.peak;
    assert_eq!(
        output_storage,
        3 * std::mem::size_of::<Expression>() + 16 * std::mem::size_of::<ProductionRankedValueV1>()
    );
    let Expression::Binary { lhs, rhs, .. } = &actual else {
        panic!("missing expanded binary");
    };
    let (Expression::Load(left), Expression::Load(right)) = (lhs.as_ref(), rhs.as_ref()) else {
        panic!("missing loads");
    };
    assert_eq!(left, right);
    assert!(!std::ptr::eq(left.indices.as_ptr(), right.indices.as_ptr()));
    release_expression(actual, output_storage, &mut setup);
    for (work_limit, storage_limit, succeeds) in [
        (work, peak, true),
        (work - 1, peak, false),
        (work, peak - 1, false),
        // Omitting either copy of the boxed payload must not suffice.
        (
            work,
            floor + output_storage - 8 * std::mem::size_of::<ProductionRankedValueV1>(),
            false,
        ),
    ] {
        let mut meter = TestMeter {
        ...
        let result = template.instantiate(std::slice::from_ref(&input), &mut meter);
        assert_eq!(result.is_ok(), succeeds);
        if let Ok((actual, storage)) = result {
            release_expression(actual, storage, &mut meter);
        }
        assert_eq!(meter.storage, floor);
    }
    drop(input);
    setup.release(input_storage).unwrap();
    template.destroy(&mut setup).unwrap();
