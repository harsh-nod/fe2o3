//! Closed argument transport for the pending whole-root source census.
...
impl ArgumentTransport {
    pub(super) fn new(locals: usize) -> Result<Self, &'static str> {
        if !(6..=MAX_LOCALS).contains(&locals) {
            return Err("complete body source local bound exceeded");
        }
        let mut state = Self {
    ...
    fn temporary(&self, index: usize) -> Result<(), &'static str> {
        if index < 6 || index >= self.locals {
            return Err("complete body source mutates a root argument or invalid local");
        }
        Ok(())
    }
    ...
    pub(super) fn assign(
        ...
        let origin = self
            ...
            .ok_or("complete body source transport reads an undefined or foreign value")?;
        if origin == 0 && !moved {
            return Err("complete body output owner cannot be copied");
        }
    ...
    pub(super) fn consume_marker(
        ...
        if self.marker_consumed || moved & 1 == 0 || moved & !0b1_1111 != 0 {
            return Err("complete body marker repeated or output owner not moved");
        }
        for (index, local) in locals.into_iter().enumerate() {
            ...
            if actual != Some(index as u8) {
                return Err("complete body marker operands differ from exact root argument order");
            }
        }
        for (index, local) in locals.into_iter().enumerate() {
            if moved & (1 << index) != 0 {
    ...
    pub(super) fn require_marker(&self) -> Result<(), &'static str> {
        if self.marker_consumed {
            ...
            Err("complete body source never consumed its marker")
        }
    }
}

#[cfg(test)]
#[path = "production_complete_body_transport_vnext_tests.rs"]
mod tests;
