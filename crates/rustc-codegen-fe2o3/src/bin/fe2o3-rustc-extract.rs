...
const EXTRACT_DIAGNOSTIC_KIR_PATH_ENV_V19: &str = "FE2O3_EXTRACT_DIAGNOSTIC_KIR_PATH_V19";
const EXTRACT_PHYSICAL_ENTRY_DIRECTORY_ENV_V20: &str =
...
fn main() {
    ...
    if let Err(error) = require_disjoint_physical_entry_diagnostic_v20(
        physical_entry_v20.is_some(),
        [
            diagnostic_kir_v16.is_some(),
            diagnostic_kir_v17.is_some(),
    ...
    if let Err(error) = require_disjoint_diagnostic_outputs_v19(
        diagnostic_kir_v16.is_some(),
        diagnostic_kir_v17.is_some(),
        diagnostic_kir_v19.is_some(),
    ) {
    ...
    .and_then(|prepared| select_diagnostic_kir_v19_mode(prepared, diagnostic_kir_v19))
...
fn require_disjoint_physical_entry_diagnostic_v20(
    ...
    if v20 && older.into_iter().any(|selected| selected) {
        Err(
            "pre-ranked physical-entry V20 diagnostics are mutually exclusive with V16/V17/V19"
                .into(),
        )
    } else {
        Ok(())
...
fn select_physical_entry_diagnostic_v20_mode(
    ...
    if output.is_empty() {
        return Err(format!(
            "{EXTRACT_PHYSICAL_ENTRY_DIRECTORY_ENV_V20} must not be empty"
        ));
    }
    if let PreparedExtractionV1::Selected(selected) = &mut prepared {
        ...
        {
            return Err(format!(
                "{EXTRACT_PHYSICAL_ENTRY_DIRECTORY_ENV_V20} is mutually exclusive with all other diagnostic, ranked, LLVM, compiler-handoff, simulation-bundle and crate-binding outputs"
            ));
        }
        selected.mode = ExtractionModeV1::PhysicalEntryDiagnosticV20(output);
    }
    Ok(prepared)
}

fn require_disjoint_physical_global_copy_diagnostic_v21(
    v21: bool,
    older: [bool; 4],
) -> Result<(), String> {
    if v21 && older.into_iter().any(|selected| selected) {
        Err(
            "pre-ranked physical-global-copy V21 diagnostics are mutually exclusive with V16/V17/V19/V20"
                .into(),
        )
    } else {
        Ok(())
...
fn select_physical_global_copy_diagnostic_v21_mode(
    mut prepared: PreparedExtractionV1,
    output: Option<OsString>,
) -> Result<PreparedExtractionV1, String> {
    let Some(output) = output else {
    ...
    if output.is_empty() {
        return Err(format!(
            "{EXTRACT_PHYSICAL_GLOBAL_COPY_DIRECTORY_ENV_V21} must not be empty"
        ));
    }
    if let PreparedExtractionV1::Selected(selected) = &mut prepared {
        if !matches!(selected.mode, ExtractionModeV1::KernelIr)
        ...
        {
            return Err(format!(
                "{EXTRACT_PHYSICAL_GLOBAL_COPY_DIRECTORY_ENV_V21} is mutually exclusive with all other diagnostic, ranked, LLVM, compiler-handoff, simulation-bundle and crate-binding outputs"
            ));
        }
...
fn require_disjoint_diagnostic_outputs_v19(v16: bool, v17: bool, v19: bool) -> Result<(), String> {
    require_disjoint_diagnostic_outputs(v16, v17)?;
    if v19 && (v16 || v17) {
        return Err("diagnostic KIR V19 is mutually exclusive with V16/V17".to_owned());
    }
    Ok(())
}

fn select_diagnostic_kir_v19_mode(
    mut prepared: PreparedExtractionV1,
    output: Option<OsString>,
) -> Result<PreparedExtractionV1, String> {
    let Some(output) = output else {
    ...
    if output.is_empty() {
        return Err(format!(
            "{EXTRACT_DIAGNOSTIC_KIR_PATH_ENV_V19} must not be empty"
        ));
    }
    if let PreparedExtractionV1::Selected(selected) = &mut prepared {
        if !matches!(selected.mode, ExtractionModeV1::KernelIr)
        ...
        {
            return Err(format!(
                "{EXTRACT_DIAGNOSTIC_KIR_PATH_ENV_V19} is mutually exclusive with V16/V17, ranked, LLVM, compiler-handoff, simulation-bundle and crate-binding outputs"
            ));
        }
        selected.mode = ExtractionModeV1::DiagnosticKirV19(output);
    }
    Ok(prepared)
}
...
fn execute_selected(selected: SelectedExtractionV1) -> Result<i32, String> {
    ...
    match selected.mode {
        ...
        ExtractionModeV1::PhysicalEntryDiagnosticV20(output) => {
            rustc_codegen_fe2o3::run_diagnostic_physical_entry_extraction_driver_v20(
                &selected.args,
                std::path::Path::new(&output),
            )?;
        }
        ExtractionModeV1::PhysicalGlobalCopyDiagnosticV21(output) => {
            rustc_codegen_fe2o3::run_diagnostic_physical_global_copy_extraction_driver_v21(
                &selected.args,
                std::path::Path::new(&output),
            )?;
        }
