#[test]
fn physical_lds_exchange_diagnostic_selector_retains_exact_invocation_and_bindings() {
    ...
    let prepared = select_physical_lds_exchange_diagnostic_v22_mode(
    ...
    assert!(
        matches!(selected.mode,ExtractionModeV1::PhysicalLdsExchangeDiagnosticV22(path)if path=="fresh-diagnostics")
    );
}
#[test]
fn physical_lds_exchange_diagnostic_selector_cannot_mix_output_authorities() {
    ...
    for mode in [
        ...
        ExtractionModeV1::PhysicalLdsExchangeDiagnosticV22(path()),
    ] {
        let mut selected = selected_compile("unit", &["metadata"]);
        selected.mode = mode;
        assert!(
            select_physical_lds_exchange_diagnostic_v22_mode(
                PreparedExtractionV1::Selected(selected),
                Some(path())
            )
            .unwrap_err()
    ...
    for older in [
        ...
        assert_eq!(
            require_disjoint_physical_lds_exchange_diagnostic_v22(true, older).is_ok(),
            !older.into_iter().any(|v| v)
        );
    }
}
#[test]
fn physical_lds_exchange_diagnostic_selector_is_explicit_and_preserves_dependencies() {
    let selected = || PreparedExtractionV1::Selected(selected_compile("unit", &["metadata"]));
    assert!(matches!(
        select_physical_lds_exchange_diagnostic_v22_mode(selected(), None).unwrap(),
    ...
    assert!(
        select_physical_lds_exchange_diagnostic_v22_mode(selected(), Some(OsString::new()))
            .is_err()
    );
    ...
    assert!(
        select_physical_lds_exchange_diagnostic_v22_mode(passthrough(), Some(OsString::new()))
            .is_err()
    );
    assert!(
        matches!(select_physical_lds_exchange_diagnostic_v22_mode(passthrough(),Some("fresh".into())).unwrap(),PreparedExtractionV1::Passthrough{executable,forwarded_args}if executable=="actual-rustc"&&forwarded_args==[OsString::from("--version")])
    );
}

#[test]
fn physical_lds_exchange_cannot_replace_a_selected_v20_and_v20_cannot_replace_v22() {
    let selected = selected_compile("unit", &["metadata"]);
    let v22 = select_physical_lds_exchange_diagnostic_v22_mode(
        PreparedExtractionV1::Selected(selected),
        Some("v22".into()),
    )
    .unwrap();
    assert!(
        select_physical_entry_diagnostic_v20_mode(v22, Some("v20".into()))
            .unwrap_err()
            .contains("mutually exclusive")
    );
}
