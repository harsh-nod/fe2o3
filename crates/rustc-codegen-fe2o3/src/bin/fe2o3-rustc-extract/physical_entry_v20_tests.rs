#[test]
fn physical_entry_diagnostic_selector_retains_exact_invocation_and_bindings() {
    ...
    let prepared = select_physical_entry_diagnostic_v20_mode(
    ...
    assert!(
        matches!(selected.mode,ExtractionModeV1::PhysicalEntryDiagnosticV20(path)if path=="fresh-diagnostics")
    );
}
#[test]
fn physical_entry_diagnostic_selector_cannot_mix_output_authorities() {
    ...
    for mode in [
        ...
        selected.mode = mode;
        assert!(
            select_physical_entry_diagnostic_v20_mode(
                PreparedExtractionV1::Selected(selected),
                Some(path())
            )
            .unwrap_err()
    ...
    for older in [
        ...
        assert_eq!(
            require_disjoint_physical_entry_diagnostic_v20(true, older).is_ok(),
            !older.into_iter().any(|v| v)
        );
    }
}
#[test]
fn physical_entry_diagnostic_selector_is_explicit_and_preserves_dependencies() {
    let selected = || PreparedExtractionV1::Selected(selected_compile("unit", &["metadata"]));
    assert!(matches!(
        select_physical_entry_diagnostic_v20_mode(selected(), None).unwrap(),
        PreparedExtractionV1::Selected(SelectedExtractionV1 {
            mode: ExtractionModeV1::KernelIr,
            ..
        })
    ));
    assert!(select_physical_entry_diagnostic_v20_mode(selected(), Some(OsString::new())).is_err());
    let passthrough = || PreparedExtractionV1::Passthrough {
        executable: "actual-rustc".into(),
        forwarded_args: vec!["--version".into()],
    };
    assert!(
        select_physical_entry_diagnostic_v20_mode(passthrough(), Some(OsString::new())).is_err()
    );
    assert!(
        matches!(select_physical_entry_diagnostic_v20_mode(passthrough(),Some("fresh".into())).unwrap(),PreparedExtractionV1::Passthrough{executable,forwarded_args}if executable=="actual-rustc"&&forwarded_args==[OsString::from("--version")])
    );
}
