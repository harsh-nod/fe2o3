...
fn report_fixture() -> Observation7 {
    let case = Case {
        integer: Integer::U32,
    ...
    serde_json::from_value(json!({
        ...
        "j_traps": 3, "k_traps": 3,
        "entry_work": 0, "stage_work": 10, "tail_work": 20, "replay_work": 10,
        "stage_floor": 100, "tail_floor": 200,
        ...
        "compared_scenarios": SCENARIOS, "baseline_policy": 7, "baseline_subject": ([4u8; 32]),
        "baseline_llvm_sha256": ([11u8; 32]), "baseline_llvm_bytes": 100, "grants_authority": false
    })).unwrap()
}

#[test]
fn actual_j_report_binds_mutation_source_fresh_reports_and_historical_native_subject() {
    let report = report_fixture();
    validate7(&report, report.case).unwrap();
    let value = serde_json::to_value(&report).unwrap();
    let changes: &[fn(&mut Value)] = &[
        |v| v["changed"] = false.into(),
        |v| v["proved_pairs"] = 0.into(),
        |v| v["historical_j"] = v["final_k"].clone(),
        |v| v["baseline_subject"] = v["final_k"].clone(),
        |v| v["baseline_policy"] = 8.into(),
        |v| v["grants_authority"] = true.into(),
        |v| v["stage_work"] = v["entry_work"].clone(),
        |v| v["tail_work"] = v["stage_work"].clone(),
        |v| v["tail_floor"] = v["stage_floor"].clone(),
        |v| v["k_traps"] = 0.into(),
    ...
    for change in changes {
        ...
        change(&mut changed);
        let changed: Observation7 = serde_json::from_value(changed).unwrap();
        assert!(validate7(&changed, report.case).is_err());
    }
}

#[test]
fn actual_j_report_requires_both_exact_sim_subjects_and_matching_experiments() {
    let report = report_fixture();
    validate7(&report, report.case).unwrap();
    let value = serde_json::to_value(&report).unwrap();
