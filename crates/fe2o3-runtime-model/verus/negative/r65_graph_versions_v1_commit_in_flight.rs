...
pub open spec fn commit_write_v1(phase: PhaseV1, pending: Option<nat>, output: nat, current: Option<nat>) -> bool {
    (phase == PhaseV1::InFlight || phase == PhaseV1::Planned) && pending == Some(output) && current == None
}
pub proof fn mutated_commit_in_flight_v1(phase: PhaseV1, pending: Option<nat>, output: nat, current: Option<nat>)
