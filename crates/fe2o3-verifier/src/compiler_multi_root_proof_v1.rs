...
mod tests {
    ...
    fn formal_component_payload(
        ...
        if width == fe2o3_kernel_ir::FormalIndexWidth::Bits32 {
            // This is deliberately inert wire data, not a live Bits32 proof.
            let mut cursor = 20;
            for _ in 0..2 {
                let length =
            ...
            bytes[cursor] = 1;
            let receipt = InertFormalMemoryReceiptFormatV3::decode_current(bytes).unwrap();
            assert_eq!(receipt.metadata().index_width(), width);
            assert!(!receipt.grants_authority());
            bytes = receipt.into_canonical_bytes();
        }
        bytes
    }
