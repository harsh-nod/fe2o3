//! Existing source/output correspondence over borrowed inert CPU records.
use super::*;
...
pub(super) fn check_source_identity(
    ...
    let signature = binding.replay.signature_preimage;
    charge(
        budget,
        signature
            .reference_inputs()
    ...
    require(
        relations.len() == binding.replay.effect_ir.relations.len()
            && signature.reference_inputs().len()
                == binding.replay.effect_ir.argument_count as usize,
        "conditional CPU signature arity",
    )?;
    for (raw, relation) in binding.replay.effect_ir.relations.iter().enumerate() {
        require(
...
pub(super) fn semantic_expression<'a>(
...
fn check_contract_roles(
    ...
    let mut prefix = [0; 8];
    prefix.copy_from_slice(&binding.replay.effect_ir_sha256[..8]);
    require(
...
/// Checks the existing source/adjusted origin pair without granting authority.
pub fn require_read_origins(
...
fn with_reference_expression<R>(
    ...
    let roles = binding
        ...
        .relations
        .len()
        .checked_mul(binding.replay.effect_ir.relations.len())
