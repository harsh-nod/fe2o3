...
//! includes the reviewed scalar source model. Its retained execution closes request-to-transcript
//! binding, but source-model-to-KIR and emitted-machine refinement remain explicit obligations.
...
fn generate_source(
    ...
    let mut generated = String::with_capacity(SOURCE_MODEL.len() + 4096);
...
mod tests {
    ...
    fn generated_source_binds_challenge_and_every_compiler_axis() {
        ...
        assert!(source.ends_with(std::str::from_utf8(SOURCE_MODEL).unwrap()));
