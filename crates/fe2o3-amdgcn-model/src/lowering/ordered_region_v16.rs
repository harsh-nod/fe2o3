//! Exact-owner entry and closed emission for one direct-root ordered region.
//! LLVM text is not source authentication, a final descriptor, or artifact authority.

use super::*;
use fe2o3_kernel_ir::{
    Gfx942OrderedRegionProfileV1, Gfx942OrderedRegionV1, VerifiedCanonicalKernelIrModuleV16,
...
/// Lowers the exact retained V16 executable using the existing complete-module
...
pub fn lower_canonical_v16_compiler_module_to_gfx942_xnack_minus_llvm_ir(
    owner: &VerifiedCanonicalKernelIrModuleV16,
...
pub(super) fn validate_owner_context(
    ...
    if !std::ptr::eq(module, owner.module()) || target != LoweringTarget::Gfx942XnackMinusV1 {
        return Err(reject(
            ...
            "ordered-region lowering requires the actual V16 owner and exact gfx942:xnack- target",
        ));
    }
    let [kernel] = module.kernels.as_slice() else {
        return Err(reject(
            LoweringLocation::module(module),
            "initial ordered-region profile requires exactly one kernel root",
        ));
    };
            ...
            for (ordinal, operation) in block.operations.iter().enumerate() {
                    ...
                    || function.role != FunctionRole::KernelEntry
                {
                    return Err(reject(
                        ...
                        "initial ordered-region profile allows only one direct kernel-root region",
                    ));
                }
                for capabilities in [
                    ...
                    {
                        return Err(reject(LoweringLocation::device_operation(module, function, block.id, ordinal), "ordered region requires all three profile capabilities on module, kernel and entry without a conflicting wave declaration"));
                    }
                }
            }
        }
    ...
    if count != 1 {
        return Err(reject(
            LoweringLocation::module(module),
            "ordered-region owner entry requires exactly one region",
        ));
    }
    Ok(())
}
...
impl FunctionLowerer<'_> {
    pub(super) fn validate_ordered_region_v16(
        &self,
        operation: &Operation,
        location: &LoweringLocation,
    ) -> Result<(), LoweringErrors> {
        if !self.ordered_region_v16
            ...
            || self.workgroup_size != Some(WorkgroupSize::new(64, 1, 1))
        {
            return Err(reject(
                location.clone(),
                "ordered region requires V16 root context, gfx942:xnack-, wave64 and required workgroup64x1x1",
            ));
        }
        ...
        .map_err(|error| {
            LoweringErrors::one(
                location.clone(),
                LoweringDiagnosticCode::UnsupportedInlineAssembly,
                format!("invalid ordered region: {error}"),
            )
        })?;
        let target_block = location.block.ok_or_else(|| {
            reject(
                location.clone(),
                "ordered region has no exact block location",
            )
        })?;
        let body = self.body("ordered region requires a retained function body")?;
        let Some(first) = body.blocks.first() else {
            return Err(reject(
                location.clone(),
                "ordered region requires a function entry block",
            ));
        };
        ...
        for position in 0..body.blocks.len() {
                ...
                != Some(usize::from(position != 0))
            {
                return Err(reject(
                    location.clone(),
                    "ordered region entry prefix cannot have bypass or loop edges",
                ));
            }
            ...
            let Some(Terminator::Branch { target, .. }) = &self.block(cursor)?.terminator else {
                return Err(reject(
                    location.clone(),
                    "ordered region must precede conditional control flow on the unconditional entry prefix",
                ));
            };
            cursor = *target;
        }
        Err(reject(
            location.clone(),
            "ordered region is not on the bounded unconditional entry prefix",
        ))
    }

    pub(super) fn emit_ordered_region_v16(
        &self,
        ...
        writeln!(output, "  ; ordered-region-v16 vgpr-high-water={} (binding extent; final descriptor unverified)", registers.vgpr_high_water()).unwrap();
        match region.profile() {
            Gfx942OrderedRegionProfileV1::XorAddU32E32 => {
                writeln!(output, "  {result} = call i32 asm sideeffect \"v_xor_b32_e32 v{scratch}, $1, $2\\0A\\09v_add_u32_e32 $0, v{scratch}, $3\", \"=&{{v{destination}}},{{v{input0}}},{{v{input1}}},{{v{input2}}},~{{v{scratch}}}\"(i32 {a}, i32 {b}, i32 {c})").unwrap();
            }
        }
    }
}
