...
use fe2o3_kernel_ir::{
    ...
    Gfx942PhysicalGlobalCopyInstructionV1 as Instruction,
    Gfx942PhysicalGlobalCopyOpcodeV1 as Opcode, OperationKind, Terminator,
    gfx942_physical_global_copy_declaration_v21,
};
use std::fmt::Write as _;
type EmitError = Gfx942PhysicalGlobalCopyCanonicalEmissionErrorV21;
...
fn llvm(symbol: &str, assembly: &str, clobbers: &[bool; 131]) -> Result<String> {
    let mut llvm = Text::new(GFX942_PHYSICAL_GLOBAL_COPY_LLVM_BYTES_V21)?;
    llvm.format(format_args!(r#"; Exact canonical V21 physical-global-copy body; inert until normal worker qualification.
...
pub(super) fn render(
...
) -> Result<Gfx942PhysicalGlobalCopyCanonicalEmissionV21> {
    let declaration = gfx942_physical_global_copy_declaration_v21(owner)
        .ok_or(invalid("owner has no exact physical-global-copy profile"))?;
    ...
    let body = function
        .body
        .as_ref()
        .ok_or(invalid("physical global-copy body absent"))?;
    ...
    let mut assembly = Text::new(GFX942_PHYSICAL_GLOBAL_COPY_ASSEMBLY_BYTES_V21)?;
    let mut operations = [None; 32];
    ...
    for (block_ordinal, block) in body.blocks.iter().enumerate() {
        ...
        for (ordinal, operation) in block.operations.iter().enumerate() {
            ...
            *slot = Some(Gfx942PhysicalGlobalCopyOperationCorrespondenceV21 {
                block: block.id,
        ...
        blocks[block_ordinal] = Some(Gfx942PhysicalGlobalCopyBlockCorrespondenceV21 {
            block: block.id,
    ...
    Ok(Gfx942PhysicalGlobalCopyCanonicalEmissionV21 {
