...
use fe2o3_kernel_ir::{
    ...
    TargetCapability, TensorInstructionProfileV1, Terminator, TiledGemmLdsEdgesV1Error,
...
pub const GFX942_UPSTREAM_LLVM_DATA_LAYOUT_V1: &str =
    fe2o3_llvm_handoff::GFX942_AMDHSA_DATA_LAYOUT_V1;
...
/// Exact target-bound scalar GEMM V1 LLVM output.
///
/// The code-object version remains an authenticated downstream build input;
/// textual LLVM IR cannot itself encode COV6 authority.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ScalarGemmGfx942LlvmV1 {
    llvm_ir: String,
    requirements: ScalarGemmTargetRequirementsV1,
}

impl ScalarGemmGfx942LlvmV1 {
    ...
    /// Requirements retained for downstream artifact inspection. This does
    /// not claim that a COV6 code object has been emitted or inspected.
    pub const fn requirements(&self) -> ScalarGemmTargetRequirementsV1 {
        self.requirements
    }
}

#[derive(Debug)]
pub enum ScalarGemmLoweringErrorV1 {
    Profile(ScalarGemmV1Error),
    Lowering(LoweringErrors),
    NonCanonicalOutput(&'static str),
}

impl fmt::Display for ScalarGemmLoweringErrorV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::NonCanonicalOutput(reason) => {
                write!(
                    formatter,
                    "scalar GEMM V1 LLVM output failed closed: {reason}"
                )
            }
        }
    }
...
impl Error for ScalarGemmLoweringErrorV1 {
...
/// Verifies and lowers only the canonical scalar GEMM V1 graph for
/// gfx942:xnack-/COV6. This performs no linking or code-object construction.
pub fn lower_scalar_gemm_v1_to_gfx942_llvm_ir(
    module: &Module,
    requirements: ScalarGemmTargetRequirementsV1,
) -> Result<ScalarGemmGfx942LlvmV1, ScalarGemmLoweringErrorV1> {
    verify_scalar_gemm_v1_module(module, requirements)
        .map_err(ScalarGemmLoweringErrorV1::Profile)?;
    ...
    .map_err(ScalarGemmLoweringErrorV1::Lowering)?;
    audit_scalar_gemm_v1_llvm(&llvm_ir)?;
    Ok(ScalarGemmGfx942LlvmV1 {
        llvm_ir,
        requirements,
    })
}

fn audit_scalar_gemm_v1_llvm(llvm: &str) -> Result<(), ScalarGemmLoweringErrorV1> {
    let exact_layout = format!("target datalayout = \"{GFX942_UPSTREAM_LLVM_DATA_LAYOUT_V1}\"");
    if !llvm.contains(&exact_layout) {
        return Err(ScalarGemmLoweringErrorV1::NonCanonicalOutput(
            "missing exact upstream LLVM 22 gfx942 data layout",
        ));
    }
    let required = [
        (
        ...
        ("udiv i64", "missing unsigned row division"),
        ("urem i64", "missing unsigned column remainder"),
        ("fmul float", "missing sequential f32 multiply"),
        ("fadd float", "missing sequential f32 accumulation"),
    ];
    for (needle, reason) in required {
        if !llvm.contains(needle) {
            return Err(ScalarGemmLoweringErrorV1::NonCanonicalOutput(reason));
        }
    }
    for forbidden in [
        "llvm.fma",
        ...
        if llvm.contains(forbidden) {
            return Err(ScalarGemmLoweringErrorV1::NonCanonicalOutput(
                "forbidden contraction, reassociation, or COMGR marker",
            ));
        }
    }
    let multiply = llvm.find("fmul float").expect("required above");
    let accumulate = llvm.find("fadd float").expect("required above");
    if multiply >= accumulate {
        return Err(ScalarGemmLoweringErrorV1::NonCanonicalOutput(
            "f32 accumulation does not follow its multiply",
        ));
    }
    if llvm.matches("load float").count() != 2 || llvm.matches("store float").count() != 1 {
        return Err(ScalarGemmLoweringErrorV1::NonCanonicalOutput(
...
fn audit_tiled_gemm_lds_v1_llvm(llvm: &str) -> Result<(), TiledGemmLdsLoweringErrorV1> {
    ...
    let exact_counts = [
        ...
        ("define ", 1, "definition cardinality"),
...
/// Exact target-bound two-phase LDS-tiled GEMM Slice 2 LLVM output.
///
/// This value authenticates only the reviewed textual LLVM lowering. It does
/// not establish source correspondence, code-object identity, or execution.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TiledGemmLdsK32Gfx942LlvmV2 {
    llvm_ir: String,
    profile: TiledGemmLdsK32V2Profile,
}

impl TiledGemmLdsK32Gfx942LlvmV2 {
    pub fn as_str(&self) -> &str {
        &self.llvm_ir
    }
    ...
    /// Profile retained for downstream COV6 construction and inspection.
    pub const fn profile(&self) -> &TiledGemmLdsK32V2Profile {
        &self.profile
    }
}

#[derive(Debug)]
pub enum TiledGemmLdsK32LoweringErrorV2 {
    Profile(TiledGemmLdsK32V2Error),
    Lowering(LoweringErrors),
    NonCanonicalOutput(&'static str),
}

impl fmt::Display for TiledGemmLdsK32LoweringErrorV2 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Profile(error) => error.fmt(formatter),
            Self::Lowering(error) => error.fmt(formatter),
            Self::NonCanonicalOutput(reason) => write!(
                formatter,
                "tiled GEMM LDS K32 V2 LLVM output failed closed: {reason}"
            ),
        }
    }
}

impl Error for TiledGemmLdsK32LoweringErrorV2 {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            Self::Profile(error) => Some(error),
            Self::Lowering(error) => Some(error),
...
/// Verifies and lowers only the canonical two-iteration K=32 LDS GEMM graph
/// for gfx942:xnack-/COV6.
///
/// The upstream-LLVM input contains a real SSA loop. Its four FP32
/// accumulators are loop-carried, both LDS allocations are reused by the two
/// K=16 phases, and physical barriers guard reads and subsequent overwrites.
/// This function performs no COMGR operation, linking, loading, or launching.
pub fn lower_tiled_gemm_lds_k32_v2_to_gfx942_llvm_ir(
    module: &Module,
    profile: TiledGemmLdsK32V2Profile,
) -> Result<TiledGemmLdsK32Gfx942LlvmV2, TiledGemmLdsK32LoweringErrorV2> {
    verify_tiled_gemm_lds_k32_v2_module(module, &profile)
        .map_err(TiledGemmLdsK32LoweringErrorV2::Profile)?;
    let llvm_ir = lower_compiler_module_to_llvm_ir_for_target(
        module,
    ...
    .map_err(TiledGemmLdsK32LoweringErrorV2::Lowering)?;
    audit_tiled_gemm_lds_k32_v2_llvm(&llvm_ir)?;
    Ok(TiledGemmLdsK32Gfx942LlvmV2 { llvm_ir, profile })
}

fn audit_tiled_gemm_lds_k32_v2_llvm(llvm: &str) -> Result<(), TiledGemmLdsK32LoweringErrorV2> {
    let required = [
        (
            "target triple = \"amdgcn-amd-amdhsa\"",
            "missing AMDGPU HSA target triple",
        ...
        (
            ...
            "missing canonical K32 LDS kernel definition",
        ),
        (
            "\"target-cpu\"=\"gfx942\"",
            "missing gfx942 processor binding",
        ...
        ("icmp ult i64", "missing bounded unsigned phase comparison"),
        ("br i1", "missing bounded phase-loop branch"),
    ];
    for (needle, reason) in required {
        if !llvm.contains(needle) {
            return Err(TiledGemmLdsK32LoweringErrorV2::NonCanonicalOutput(reason));
        }
    }
    ...
    let lds_declarations = llvm
        ...
        .collect::<Vec<_>>();
    if lds_declarations.len() != 2 || lds_declarations[0] == lds_declarations[1] {
        return Err(TiledGemmLdsK32LoweringErrorV2::NonCanonicalOutput(
    ...
    let exact_counts = [
        ...
        (
            "call void asm sideeffect \"s_barrier\", \"\"()",
            2,
            "pre-read and pre-reuse barrier cardinality",
        ),
        (
            "call i32 @llvm.amdgcn.mbcnt.lo",
            4,
    ...
    let mut barriers = llvm
        ...
        .map(|(offset, _)| offset);
    let pre_read_barrier = barriers.next().expect("required by exact cardinality");
    let pre_reuse_barrier = barriers.next().expect("required by exact cardinality");
    let first_lds_load = llvm
        .find(" = load i16, ptr addrspace(3)")
        .expect("required by exact cardinality");
    let mfma_call = llvm.find(&mfma).expect("required by exact cardinality");
    ...
    let first_output_store = llvm
        .find("store float ")
        .expect("required by exact cardinality");
    if !(first_lds_store < pre_read_barrier
        && pre_read_barrier < first_lds_load
        && first_lds_load < mfma_call
        && mfma_call < pre_reuse_barrier
        && pre_reuse_barrier < phase_increment
        && phase_increment < first_output_store)
    {
        return Err(TiledGemmLdsK32LoweringErrorV2::NonCanonicalOutput(
            "two-phase LDS store/barrier/load/MFMA/reuse/output ordering is not exact",
        ));
    }
    ...
    for forbidden in [
        ...
        if llvm.contains(forbidden) {
            return Err(TiledGemmLdsK32LoweringErrorV2::NonCanonicalOutput(
                "forbidden floating-point, atomic, scratch, external-call, or COMGR marker",
            ));
        }
    }
    Ok(())
...
/// Exact target-bound padded-stride LDS grid GEMM Slice 3 LLVM output.
///
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct TiledGemmLdsGridGfx942LlvmV1 {
    llvm_ir: String,
    profile: TiledGemmLdsGridV1Profile,
}

impl TiledGemmLdsGridGfx942LlvmV1 {
    pub fn as_str(&self) -> &str {
        &self.llvm_ir
    }
    ...
    /// Profile retained for downstream COV6 construction and inspection.
    pub const fn profile(&self) -> &TiledGemmLdsGridV1Profile {
        &self.profile
    }
}

#[derive(Debug)]
pub enum TiledGemmLdsGridLoweringErrorV1 {
    Profile(TiledGemmLdsGridV1Error),
    Lowering(LoweringErrors),
    NonCanonicalOutput(&'static str),
}

impl fmt::Display for TiledGemmLdsGridLoweringErrorV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Profile(error) => error.fmt(formatter),
            Self::Lowering(error) => error.fmt(formatter),
            Self::NonCanonicalOutput(reason) => write!(
                formatter,
                "tiled GEMM LDS grid V1 LLVM output failed closed: {reason}"
            ),
        }
    }
}

impl Error for TiledGemmLdsGridLoweringErrorV1 {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            Self::Profile(error) => Some(error),
            Self::Lowering(error) => Some(error),
...
/// Verifies and lowers only the canonical M64/N48/K16 padded-stride 3x4 LDS
/// grid graph for gfx942:xnack-/COV6.
///
/// The emitted upstream-LLVM input derives both tile origins from workgroup
/// IDs, preserves lda=33/ldb=79/ldc=96 address arithmetic, uses two distinct
/// 512-byte XOR4 LDS tiles and one physical barrier, executes one BF16 MFMA,
/// and writes four C elements per lane. It performs no COMGR operation,
/// optimization, linking, loading, or launching.
pub fn lower_tiled_gemm_lds_grid_v1_to_gfx942_llvm_ir(
    module: &Module,
    profile: TiledGemmLdsGridV1Profile,
) -> Result<TiledGemmLdsGridGfx942LlvmV1, TiledGemmLdsGridLoweringErrorV1> {
    verify_tiled_gemm_lds_grid_v1_module(module, &profile)
        .map_err(TiledGemmLdsGridLoweringErrorV1::Profile)?;
    let llvm_ir = lower_compiler_module_to_llvm_ir_for_target(
        module,
    ...
    .map_err(TiledGemmLdsGridLoweringErrorV1::Lowering)?;
    audit_tiled_gemm_lds_grid_v1_llvm(&llvm_ir)?;
    Ok(TiledGemmLdsGridGfx942LlvmV1 { llvm_ir, profile })
}

fn audit_tiled_gemm_lds_grid_v1_llvm(llvm: &str) -> Result<(), TiledGemmLdsGridLoweringErrorV1> {
    let required = [
        (
            "target triple = \"amdgcn-amd-amdhsa\"",
            "missing AMDGPU HSA target triple",
        ...
        (
            "%v16 = mul i64 %v9, 16",
            "missing workgroup-X tile-column origin",
        ),
        (
            "%v17 = mul i64 %v10, 16",
            "missing workgroup-Y tile-row origin",
        ),
    ];
    for (needle, reason) in required {
        if !llvm.contains(needle) {
            return Err(TiledGemmLdsGridLoweringErrorV1::NonCanonicalOutput(reason));
        }
    }
    ...
    let lds_declarations = llvm
        ...
        .collect::<Vec<_>>();
    if lds_declarations.len() != 2 || lds_declarations[0] == lds_declarations[1] {
        return Err(TiledGemmLdsGridLoweringErrorV1::NonCanonicalOutput(
            "LLVM does not contain exactly two distinct aligned 512-byte BF16 LDS tiles",
        ));
    }
    ...
    for (stride, expected, reason) in [
        (33, 1, "lda=33 address arithmetic cardinality"),
        (79, 4, "ldb=79 address arithmetic cardinality"),
        (96, 4, "ldc=96 address arithmetic cardinality"),
    ] {
                    ...
                    ) =>
                {
                    let (intrinsic, result) = match intrinsic.kind {
                        IntrinsicKind::InvocationIndex {
                            ...
                            axis: Axis::X,
                        } => (AmdgcnIntrinsic::WorkItemId(Dim::X), "i32"),
                        IntrinsicKind::InvocationIndex {
                            kind: IndexKind::Workgroup,
                            axis: Axis::X,
                        } => (AmdgcnIntrinsic::WorkGroupId(Dim::X), "i32"),
                        IntrinsicKind::InvocationIndex {
                            kind: IndexKind::Workgroup,
                            axis: Axis::Y,
                        } => (AmdgcnIntrinsic::WorkGroupId(Dim::Y), "i32"),
                        _ => continue,
                    };
                    insert_intrinsic(
                        &mut declarations,
