...
use fe2o3_kernel_ir::{
    ...
    Gfx942PhysicalEntryRegisterV20 as Register, OperationKind, Terminator,
    gfx942_physical_entry_declaration_v20,
};
use std::fmt::Write as _;
type EmitError = Gfx942PhysicalEntryCanonicalEmissionErrorV20;
...
fn instruction(text: &mut Text, value: Instruction) -> Result<u16> {
    ...
    match opcode {
        ...
        Opcode::LoadKernargDword => text.line(format_args!("s_load_dword s{d}, s[0:1], {i}")),
        ...
        Opcode::ScalarCompareEqZero => text.line(format_args!("s_cmp_eq_u32 s{a}, 0")),
        ...
        Opcode::BranchScc1 | Opcode::Branch | Opcode::Endpgm0 | Opcode::Fallthrough => {
            Err(invalid("physical terminator opcode stored as operation"))
        }
    }
}
...
const ABSENCE_ATTRIBUTES: [&str; 14] = [
...
fn llvm(symbol: &str, assembly: &str, clobbers: &[bool; 131]) -> Result<String> {
    let mut llvm = Text::new(GFX942_PHYSICAL_ENTRY_LLVM_BYTES_V20)?;
    llvm.format(format_args!(r#"; Exact canonical V20 physical-entry body; inert until normal worker qualification.
...
entry:
  call void asm sideeffect ""#,
    ...
    llvm.format(format_args!(r#"attributes #0 = {{ nounwind "target-cpu"="gfx942" "target-features"="-xnack,-wavefrontsize32,+wavefrontsize64" "amdgpu-flat-work-group-size"="64,64" "amdgpu-max-num-workgroups"="2,1,1""#))?;
...
pub(super) fn render(
...
) -> Result<Gfx942PhysicalEntryCanonicalEmissionV20> {
    let declaration = gfx942_physical_entry_declaration_v20(owner)
        .ok_or(invalid("owner has no exact physical-entry profile"))?;
    ...
    let mut assembly = Text::new(GFX942_PHYSICAL_ENTRY_ASSEMBLY_BYTES_V20)?;
    let mut operations = [None; 65];
    ...
    for (block_ordinal, block) in body.blocks.iter().enumerate() {
        let contract = declaration
            .blocks
            .get(block_ordinal)
            .ok_or(invalid("physical block correspondence bound"))?;
        let label_line =
            assembly.line(format_args!(".Lfe2o3_physical_${{:uid}}_b{}:", block.id.0))?;
        for (ordinal, operation) in block.operations.iter().enumerate() {
            ...
            *slot = Some(Gfx942PhysicalEntryOperationCorrespondenceV20 {
                block: block.id,
        ...
        let terminal = match (&block.terminator, contract.encoding) {
            (Some(Terminator::ConditionalBranch { then_target, .. }), Encoding::Scc1) => {
                Some(assembly.line(format_args!(
                    "s_cbranch_scc1 .Lfe2o3_physical_${{:uid}}_b{}",
                    then_target.0
                ))?)
            }
            (Some(Terminator::Branch { target, .. }), Encoding::Jump) => Some(assembly.line(
                format_args!("s_branch .Lfe2o3_physical_${{:uid}}_b{}", target.0),
            )?),
            (Some(Terminator::Branch { .. }), Encoding::Fallthrough) => None,
        ...
        blocks[block_ordinal] = Some(Gfx942PhysicalEntryBlockCorrespondenceV20 {
            block: block.id,
    ...
    let llvm = llvm(function.id.as_str(), &assembly.value, &clobbers)?;
    Ok(Gfx942PhysicalEntryCanonicalEmissionV20 {
