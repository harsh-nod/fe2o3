...
use super::*;
use fe2o3_kernel_ir::*;
fn admit(module: &Module) -> (VerifiedCanonicalKernelIrModuleV21, usize) {
...
fn edited(extra: bool) -> Module {
    let mut module = fixture::module_with_extra_zero(extra);
    for operation in &mut module.functions[0].body.as_mut().unwrap().blocks[0].operations {
        if let OperationKind::Gfx942PhysicalGlobalCopyStep(step) = &mut operation.kind {
            match step.instruction.opcode {
                Gfx942PhysicalGlobalCopyOpcodeV1::GlobalLoadDword => {
                    step.instruction.destination = 22
                }
                Gfx942PhysicalGlobalCopyOpcodeV1::GlobalStoreDword => step.instruction.source1 = 22,
                _ => {}
            }
        }
    }
...
fn exact_owner_llvm_four_slot_and_actual_step_grammar() {
    for module in [fixture::module(), edited(false), edited(true)] {
        ...
        let (text, receipt) =
            physical_global_copy_native_observation_input_v21(&owner, &emission, &mut budget)
                .unwrap();
        ...
        assert!(
            text.contains("entry physical_global_copy_fixture\nlaunch 64 1 1 2 1 1\nblocks 1\n")
        );
...
fn mismatched_owner_or_inert_corrupted_correspondence_refuses() {
    ...
    let (other, other_retained) = admit(&edited(false));
    let mut work = CanonicalKernelIrWorkBudgetV1::new(16_000_000);
    let mut budget = Budget::new(&mut work, 16_000_000);
    budget
    ...
    let floor = budget.storage();
    assert!(matches!(
        physical_global_copy_native_observation_input_v21(&other, &emission, &mut budget),
...
#[test]
fn store_must_use_actual_load_ssa_not_another_well_typed_u32() {
    ...
    let OperationKind::Gfx942PhysicalGlobalCopyStep(store) = &mut block.operations[21].kind else {
        panic!("store")
    };
    ...
    assert!(matches!(
        ...
        Err(CanonicalKernelIrReplayAdmissionErrorV21::Verification(_))
    ));
...
fn exact_one_below_and_denial_history_preserve_owner_emission_floor() {
    ...
    drop(
        physical_global_copy_native_observation_input_v21(&owner, &emission, &mut budget).unwrap(),
    );
    ...
    for case in 0..3 {
        ...
        match (case, result) {
            ...
            (
                1,
                Err(Gfx942PhysicalGlobalCopyCanonicalEmissionErrorV21::Resource(Resource::Work(e))),
            ) => assert_eq!(e.actual(), e.limit() + 1),
            (
                2,
                Err(Gfx942PhysicalGlobalCopyCanonicalEmissionErrorV21::Resource(
                    Resource::Storage(e),
                )),
            ) => {
                assert_eq!(e.actual(), e.limit() + 1);
...
#[test]
#[ignore = "explicit inert V21 export only; no source or native authority; fresh directory required"]
fn export_inert_global_copy_and_register_site_edits_for_native_observer() {
    use std::{fs, io::Write, path::Path};
    let directory =
        std::env::var("FE2O3_PHYSICAL_V21_NATIVE_FIXTURE_DIR").expect("fresh directory");
    assert!(
        directory.len() <= 4000
    ...
    for (name, module) in [
        ("copy", fixture::module()),
        ("copy-v22", edited(false)),
        ("copy-extra-v22", edited(true)),
    ] {
        let (owner, retained) = admit(&module);
        let mut work = CanonicalKernelIrWorkBudgetV1::new(16_000_000);
        let mut budget = Budget::new(&mut work, 16_000_000);
    ...
    eprintln!(
        "inert V21 same-owner export only; no source/native/hardware/protected execution or milestone closure"
    );
}
