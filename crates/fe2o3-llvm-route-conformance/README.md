...
public worker-admission V1 boundary. The Pliron lowering crate remains an
API-unavailable gap at this revision, so the corpus does not assume its
unfinished surface.

Successful fixture construction and round trips establish canonical handoff
representation only. They do not establish LLVM lowering, intrinsic or atomic
semantics, emitted machine correspondence, code-object correctness, hardware
behavior, device-library contents, or publication authority. The crate does
not invoke a compiler, linker, runtime, device, or subprocess.
