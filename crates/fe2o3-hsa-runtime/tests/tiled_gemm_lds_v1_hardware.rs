//! Observational MI300X evidence for fixed LDS-tiled GEMM Slice 1.
//!
...
impl MetadataFacts {
    fn expected() -> Self {
        ...
        Self {
            ...
            kernel_count: 1,
            kernel_name: TILED_GEMM_LDS_V1_KERNEL_ID.to_owned(),
            descriptor_symbol: format!("{TILED_GEMM_LDS_V1_KERNEL_ID}.kd"),
...
fn validate_metadata(facts: &MetadataFacts) -> Result<(), BoxError> {
    require(
        ...
        format!(
            "Slice 1 metadata differs from exact gfx942:xnack-/COV6/3-slice/\
...
impl BoundKernelEntry {
    fn end(self) -> Result<u64, BoxError> {
        self.address
            ...
            .ok_or_else(|| "Slice 1 entry address overflow".into())
    }
}
...
fn inspect_metadata(bytes: &[u8]) -> Result<BoundKernelEntry, BoxError> {
    require(
        TILED_GEMM_LDS_V1_STATIC_LDS_BYTES as u64 == STATIC_LDS_BYTES,
    ...
    let kernel = inspection
        .kernels()
        .first()
        .ok_or("Slice 1 HSACO declares no kernel")?;
    let binding = bound
        .bindings()
        .first()
        .ok_or("Slice 1 HSACO has no descriptor binding")?;
    ...
    require(entry.size != 0, "Slice 1 ELF entry is empty")?;
    let _ = entry.end()?;
    Ok(entry)
}
...
fn validate_isa(
    ...
    require(
        header_symbol == TILED_GEMM_LDS_V1_KERNEL_ID && header_address == entry.address,
        "disassembly header differs from the bound Slice 1 ELF entry",
    )?;
    ...
    require(!instructions.is_empty(), "Slice 1 disassembly is empty")?;
    ...
    require(
        count("s_barrier") == 1,
        "final ISA must contain one s_barrier",
    )?;
    require(
...
impl Toolchain {
    fn from_environment() -> Result<Self, BoxError> {
        require(
            std::env::var("FE2O3_RUN_GFX942_TILED_GEMM_LDS_V1_HARDWARE").as_deref() == Ok("1"),
            "set FE2O3_RUN_GFX942_TILED_GEMM_LDS_V1_HARDWARE=1 to opt in",
        )?;
    ...
    fn new() -> Result<Self, BoxError> {
        ...
        for attempt in 0..64_u32 {
            let path = std::env::temp_dir().join(format!(
                "fe2o3-tiled-gemm-lds-v1-{}-{nonce}-{attempt}",
                std::process::id()
            ));
...
fn generate_observational_artifact(tools: &Toolchain) -> Result<ObservationalArtifact, BoxError> {
    let llvm = lower_tiled_gemm_lds_v1_to_gfx942_llvm_ir(
    ...
    let input = directory.join("slice1.ll");
    let object = directory.join("slice1.o");
    let hsaco = directory.join("slice1.hsaco");
    ...
    checked_tool_output(
        ...
        "upstream llc Slice 1 compilation",
    )?;
    checked_tool_output(
        &tools.lld,
        vec![
        ...
        "upstream ld.lld Slice 1 link",
    )?;
    tools.verify()?;
    let bytes = std::fs::read(&hsaco)?;
    require(
        (1..=fe2o3_hsaco::MAX_HSACO_BYTES).contains(&bytes.len()),
        "generated Slice 1 HSACO has an invalid bounded length",
    )?;
    for forbidden in [b"amd_comgr".as_slice(), b"libamd_comgr".as_slice()] {
        require(
            ...
            "generated Slice 1 HSACO contains a COMGR reference",
        )?;
    }
    let entry = inspect_metadata(&bytes)?;
    let disassembly_output = tools.objdump.output([
        std::ffi::OsString::from(format!(
            "--disassemble-symbols={TILED_GEMM_LDS_V1_KERNEL_ID}"
        )),
    ...
    require(
        ...
        format!(
            "upstream llvm-objdump rejected generated Slice 1: {}",
...
fn hardware_cases() -> Vec<HardwareCase> {
    ...
    let mut identity_a = vec![0; ELEMENTS];
    let mut identity_b = vec![0; ELEMENTS];
    let identity_palette = [0x3f80, 0xbf80, 0x3f00, 0xbf00, 0x4000, 0xc000];
    for row in 0..TILE {
        identity_a[row * TILE + row] = 0x3f80;
        for column in 0..TILE {
            identity_b[row * TILE + column] =
                identity_palette[(7 * row + 3 * column) % identity_palette.len()];
        }
    }
    ...
    let mut dyadic_a = vec![0; ELEMENTS];
    let mut dyadic_b = vec![0; ELEMENTS];
    for row in 0..TILE {
        for column in 0..TILE {
            let index = row * TILE + column;
            dyadic_a[index] = dyadic_palette[(row + 3 * column) % dyadic_palette.len()];
            dyadic_b[index] = dyadic_palette[(2 * row + column + 1) % dyadic_palette.len()];
        }
    }
    ...
    let random = HardwareCase {
        ...
        a: (0..ELEMENTS).map(|_| next_random()).collect(),
        b: (0..ELEMENTS).map(|_| next_random()).collect(),
        exact: false,
    };

    let mut cancellation_a = vec![0; ELEMENTS];
    let mut cancellation_b = vec![0; ELEMENTS];
    for row in 0..TILE {
        for depth in 0..TILE {
            cancellation_a[row * TILE + depth] = if (row + depth).is_multiple_of(2) {
                0x4100
            } else {
                0xc100
            };
    ...
    let mut adversarial_a = vec![0; ELEMENTS];
    let mut adversarial_b = vec![0; ELEMENTS];
    for row in 0..TILE {
        for depth in 0..TILE {
            ...
            adversarial_a[row * TILE + depth] = magnitude ^ sign;
        }
    }
    for depth in 0..TILE {
        for column in 0..TILE {
...
fn gemm_oracle(case: &HardwareCase) -> Result<Vec<OracleValue>, BoxError> {
    require(
        case.a.len() == ELEMENTS && case.b.len() == ELEMENTS,
        format!("{} does not contain exact 16x16 inputs", case.name),
    )?;
    ...
    for row in 0..TILE {
        for column in 0..TILE {
            ...
            for depth in 0..TILE {
                let lhs = f64::from(bf16_to_f32(case.a[row * TILE + depth]));
            ...
            let rounding_allowance = if case.exact {
                ...
                // orders over 16 exact BF16 products with ample audit margin.
                64.0 * f64::from(f32::EPSILON) * absolute_sum + f64::from(f32::MIN_POSITIVE)
            };
...
fn validate_output(
    ...
    require(
        observed.len() == ELEMENTS && oracle.len() == ELEMENTS,
...
fn split_guarded<T>(values: &[T]) -> Result<GuardedParts<'_, T>, BoxError> {
    require(
        values.len() == ELEMENTS + 2 * CANARY_ELEMENTS,
        "guarded allocation has the wrong extent",
    )?;
    let (prefix, rest) = values.split_at(CANARY_ELEMENTS);
    let (body, suffix) = rest.split_at(ELEMENTS);
...
fn verify_guarded_u16(
    ...
    let (actual_prefix, actual_body, actual_suffix) = split_guarded(actual)?;
    require(
...
fn verify_output_allocation(
    ...
    let (prefix, body, suffix) = split_guarded(actual)?;
    require(
        prefix
            .iter()
...
impl RuntimeKernarg {
    fn new() -> Result<Self, BoxError> {
        ...
        let pointer = std::ptr::NonNull::new(unsafe { std::alloc::alloc_zeroed(layout) })
            .ok_or("failed to allocate aligned Slice 1 kernarg")?;
...
unsafe fn dispatch_one_workgroup(
    ...
    require(
        ...
        "Slice 1 launch must be exactly one static-LDS WG64",
    )?;
    ...
    unsafe {
        ...
        require(completion.completed(), "Slice 1 dispatch did not complete")?;
    }
    Ok(())
}
...
fn execute_case(
    ...
    let c_host = guarded_f32(&[C_POISON; ELEMENTS], C_PREFIX, C_SUFFIX);
...
fn run_observational_hardware_evidence() -> Result<(), BoxError> {
    ...
    require(
        ...
        "Slice 1 hardware evidence requires a gfx942:xnack- physical device",
    )?;
    ...
    let execution = (|| -> Result<(), BoxError> {
        require(
            ...
            "loaded bytes differ from generated Slice 1 artifact",
        )?;
        // SAFETY: inspection admitted one exact symbol and descriptor.
        let (kernels, resolutions) =
            unsafe { adapter.resolve_kernel_set(&executable, [TILED_GEMM_LDS_V1_KERNEL_ID]) }?;
        require(
            kernels.len() == 1
                && resolutions.len() == 1
                && resolutions[0].export_symbol() == TILED_GEMM_LDS_V1_KERNEL_ID
                && resolutions[0].executable_object() == executable_identity,
            "runtime resolved a substituted Slice 1 kernel",
        )?;
        let kernel = kernels.get(0).ok_or("runtime omitted Slice 1 kernel")?;
    ...
    require(
        ...
        "reviewed HSA unload did not release the exact Slice 1 executable",
    )?;
    tools.verify()?;
    execution
}

/// Generates and executes the fixed LDS-tiled `16x16x16` Slice 1 on MI300X.
///
...
/// or publisher authority. Guard values detect changed in-allocation values,
/// but cannot detect beyond-guard accesses, value-preserving writes, or races.
///
/// ```text
...
///   --test tiled_gemm_lds_v1_hardware \
///   gfx942_tiled_gemm_lds_v1_observational_hardware_evidence \
///   -- --ignored --exact --nocapture
/// ```
#[cfg(feature = "hardware-test-hooks")]
#[test]
#[ignore = "observational: requires SHA-pinned upstream LLVM 22 and gfx942:xnack-"]
fn gfx942_tiled_gemm_lds_v1_observational_hardware_evidence() -> Result<(), BoxError> {
...
mod tests {
    ...
    fn valid_disassembly() -> String {
        format!(
            "slice1.hsaco:\tfile format elf64-amdgpu\n\n\
             Disassembly of section .text:\n\n\
             0000000000001000 <{TILED_GEMM_LDS_V1_KERNEL_ID}>:\n\
             ...
             \tglobal_store_dwordx4 v[0:1], v[0:3], off // 000000001018: DEADBEEF\n\
             \ts_endpgm // 00000000101C: DEADBEEF\n"
        )
    }
    ...
    fn launch_and_kernarg_are_exactly_one_wg64_and_three_slices() {
        ...
        for (index, address) in [0x11_u64, 0x22, 0x33].into_iter().enumerate() {
            let offset = index * 16;
    ...
    #[test]
    fn isa_validator_requires_lds_barrier_mfma_and_rejects_atom_calls_scratch() {
    ...
    fn corpus_covers_required_finite_classes_and_all_outputs() {
        ...
        for case in cases {
            assert_eq!(case.a.len(), ELEMENTS);
            assert_eq!(case.b.len(), ELEMENTS);
            assert!(
                case.a
                    .iter()
                    .chain(&case.b)
