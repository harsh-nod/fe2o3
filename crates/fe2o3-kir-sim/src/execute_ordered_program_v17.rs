...
pub(super) fn execute(
    ...
    let validated = fe2o3_kernel_ir::validate_gfx942_ordered_program_v1(operation, |value| {
        match values.get(&value) {
            Some(RuntimeValue::Scalar(scalar)) => Some(scalar.ty()),
            _ => None,
        }
    })
    .map_err(|_| invariant("ordered program contract changed after preflight"))?;
    ...
    for (index, value) in validated.inputs().iter().enumerate() {
        inputs[index] = u32::try_from(scalar_value(engine, values, *value, site)?.bits())
            .map_err(|_| invariant("ordered program U32 input bits"))?;
    }
    // At most sixteen internal value steps remain one logical operation. The
    // checked evaluator reads each step's inputs before writing its destination;
    // no intermediate values are inserted into the debugger's SSA namespace.
    let bits = validated.program().evaluate(inputs);
    let scalar = ScalarBitsV1::new(ScalarType::U32, u128::from(bits), engine.target)
        .map_err(|_| invariant("ordered program U32 result bits"))?;
