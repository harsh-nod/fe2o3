//! Observational CPU admission for the fixed ordered-region value abstraction.
...
use fe2o3_kernel_ir::{
    ...
    Kernel, Module, TargetCapability, VerifiedCanonicalKernelIrModuleV16, WaveWidth, WorkgroupSize,
    decode_module_v16, encode_module_v16,
};
...
impl AdmittedSimulationModuleV1 {
    /// Borrows exact immutable V16 custody and retains an independent, bounded
    ...
    pub fn admit_v16(
        canonical: &VerifiedCanonicalKernelIrModuleV16,
        ...
        let module =
            decode_module_v16(bytes).map_err(SimulationAdmissionErrorV1::DecodeAfterAdmission)?;
        let reencoded =
            encode_module_v16(&module).map_err(SimulationAdmissionErrorV1::EncodeAfterAdmission)?;
        if reencoded != bytes {
        ...
        if peak > limits.max_resident_bytes {
            return Err(SimulationAdmissionErrorV1::ResidentBytesLimit {
                phase: "post-decode V16 canonical admission",
                actual: peak,
