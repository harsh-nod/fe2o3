//! Inert V22 owner execution only; no Rust custody, launch or native authority.
...
fn owner(module: &Module) -> (Owner, usize) {
    ...
    let (owner, receipt) =
        Owner::from_module_ref_with_verification_budget_v22(module, &mut budget).unwrap();
...
fn admitted() -> AdmittedSimulationModuleV1 {
    ...
    AdmittedSimulationModuleV1::admit_v22_with_verification_budget(
        &owner,
...
fn exact_owner_and_admission_ledger_keep_nonzero_floors_and_one_short_refusals() {
    ...
    assert_eq!(view.identity().wire_version(), 22);
    ...
    for case in 0..3 {
        ...
        match (case, result) {
            (
                1,
                Err(PhysicalLdsExchangeSimulationAdmissionErrorV22::Resource(Resource::Work(e))),
            ) => assert_eq!(e.limit() + 1, e.actual()),
            (
                2,
                Err(PhysicalLdsExchangeSimulationAdmissionErrorV22::Resource(Resource::Storage(e)))
                | Err(PhysicalLdsExchangeSimulationAdmissionErrorV22::CanonicalView(
                    fe2o3_kernel_ir::CanonicalKernelIrReplayAdmissionErrorV22::Resource(
                        Resource::Storage(e),
                    ),
                ))
                | Err(PhysicalLdsExchangeSimulationAdmissionErrorV22::CanonicalView(
                    fe2o3_kernel_ir::CanonicalKernelIrReplayAdmissionErrorV22::Decode(
...
fn ordinary_owner_and_canonical_byte_limit_refuse_without_erasing_accounting() {
    ...
    for _ in 0..2 {
        assert!(matches!(
            ...
            Err(PhysicalLdsExchangeSimulationAdmissionErrorV22::Profile)
        ));
    }
    ...
    assert!(matches!(
        ...
        Err(PhysicalLdsExchangeSimulationAdmissionErrorV22::Admission(
...
fn full_input_reads_and_zero_partial_full_output_masks_preserve_canaries() {
    ...
    for grid in [128u64] {
        for len in [0, 1, 63, 64, 65, 127, 128, 129] {
            ...
            assert_eq!(run.steps_executed(), grid * 33);
...
fn actual_read_and_write_events_use_the_canonical_load_and_store_sites() {
    let admitted = admitted();
    let req = request(128, 13, 128, None);
    ...
    for lane in 0..128 {
        ...
        assert_eq!(reads[1].site.operation, Some(21));
        assert_eq!(reads[0].site.operation, Some(13));
        assert_eq!(writes.len(), 1 + usize::from(lane < 13));
        assert_eq!(writes[0].site.operation, Some(16));
        if let Some(write) = writes.get(1) {
            assert_eq!(write.site.operation, Some(29));
        }
    }
}
#[test]
fn inactive_output_does_not_suppress_input_initialization_or_bounds_checks() {
    ...
    for (req, uninitialized) in [
        (request(128, 0, 128, Some(127)), true),
        (request(127, 0, 128, None), false),
    ] {
...
fn same_backing_is_refused_before_execution_even_for_nonoverlapping_views() {
    ...
    let target = SimulationTargetV1::amdgpu_64();
    for offset in [8, 520] {
        let mut req = request(128, 128, 128, None);
...
fn execution_step_and_resident_one_below_limits_preserve_request() {
    let admitted = admitted();
    let req = request(128, 128, 128, None);
    let original = req.clone();
    let target = SimulationTargetV1::amdgpu_64();
    let limits = SimulationLimitsV1::default();
...
#[test]
fn capability_matrix_names_only_the_closed_v22_profile_and_keeps_older_rows() {
    ...
    for row in rows {
        let expected = row.profile == SimulationCapabilityProfileV1::Gfx942XnackMinus
            && matches!(
                row.operation,
                SimulationOperationSurfaceV1::PhysicalLdsExchangeDeclaration
                    | SimulationOperationSurfaceV1::PhysicalLdsExchangeStep
...
#[test]
fn register_variant_retains_real_two_wave_exchange() {
    let module = fixtures::module_with_registers(true);
    let (owner, retained) = owner(&module);
    let mut work = Work::new(16_000_000);
    let mut budget = Budget::new(&mut work, 16_000_000);
    budget.reserve_storage(retained + 73).unwrap();
    let (admitted, receipt) = AdmittedSimulationModuleV1::admit_v22_with_verification_budget(
        &owner,
        SimulationLimitsV1::default(),
        &mut budget,
    )
...
#[test]
fn full_workgroup_barrier_is_after_all_real_lds_stores_and_before_all_peer_reads() {
    let admitted = admitted();
    let mut events = Events::default();
    admitted
        .simulate_observed_with_sink(
    ...
    assert_eq!(release.site.operation, Some(18));
    let writes: Vec<_> = events
        .0
        .iter()
        .enumerate()
        .filter(|(_, e)| {
            e.site.operation == Some(16)
                && matches!(e.kind, SimulationEventKindV1::MemoryWrite { .. })
        })
        .collect();
    let reads: Vec<_> = events
        ...
        .enumerate()
        .filter(|(_, e)| {
            e.site.operation == Some(21)
                && matches!(e.kind, SimulationEventKindV1::MemoryRead { .. })
        })
        .collect();
    assert_eq!(writes.len(), 128);
    assert_eq!(reads.len(), 128);
    assert!(writes.iter().all(|(i, _)| *i < release_index));
    assert!(reads.iter().all(|(i, _)| *i > release_index));
    let mut lanes = std::collections::BTreeSet::new();
    for (_, event) in writes {
        assert!(lanes.insert(event.invocation.local[0]));
    }
    assert_eq!(lanes, (0..128).collect());
}
