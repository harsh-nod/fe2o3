...
use fe2o3_semantic_query::{
    ...
    run_agent_profiler_variant_jsonl_v3, run_agent_runtime_causality_jsonl_v1,
};
...
fn main() -> ExitCode {
    ...
    match arguments.as_slice() {
        ...
        [mode] if mode == "variant-v3-jsonl" => {
            let stdin = io::stdin();
            let mut input = BufReader::new(stdin.lock());
            let stdout = io::stdout();
            let mut output = stdout.lock();
            match run_agent_profiler_variant_jsonl_v3(&mut input, &mut output) {
                Ok(()) => ExitCode::SUCCESS,
                Err(_) => ExitCode::from(1),
            }
        }
        ...
        [mode] if mode == "decoded-att-source-isa-jsonl" => {
            let stdin = io::stdin();
            let mut input = BufReader::new(stdin.lock());
            let stdout = io::stdout();
            let mut output = stdout.lock();
            match run_agent_decoded_att_source_isa_jsonl_v1(&mut input, &mut output) {
                Ok(()) => ExitCode::SUCCESS,
                Err(_) => ExitCode::from(1),
            }
        }
        [mode] if mode == "runtime-causality-jsonl" => {
            let stdin = io::stdin();
            let mut input = BufReader::new(stdin.lock());
            let stdout = io::stdout();
            let mut output = stdout.lock();
            match run_agent_runtime_causality_jsonl_v1(&mut input, &mut output) {
                Ok(()) => ExitCode::SUCCESS,
                Err(_) => ExitCode::from(1),
            }
        }
