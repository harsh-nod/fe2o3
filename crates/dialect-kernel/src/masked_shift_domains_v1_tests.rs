...
type Expr = SemanticTypedExpressionV1;
type Scalar = SemanticTypedScalarV1;
type Op = SemanticTypedBinaryKindAttr;
type Overflow = SemanticOverflowAttr;
type Cast = SemanticTypedCastKindAttr;
type Error = SemanticTypedExpressionErrorV1;

fn integer(signed: bool, bits: u16) -> Scalar {
    let kind = if signed {
        crate::SemanticScalarKindAttr::SignedInteger
    } else {
        crate::SemanticScalarKindAttr::UnsignedInteger
    };
    Scalar::new(kind, bits).unwrap()
}
