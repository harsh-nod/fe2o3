...
fn stored_values_preserve_rank_indices_and_round_trip_plain_and_atomic_accesses() {
    for rank in [1, MAX_RANKED_MEMORY_RANK] {
        ...
        .unwrap();
        let rmw = RankedAccessOp::new_atomic_value(
            context,
        ...
        .unwrap();
        for access in [plain, atomic, rmw] {
        ...
        assert_eq!(
            accesses[2].kind(context),
            Some(AccessKindAttr::AtomicReadModifyWrite)
        );
        assert_eq!(
            accesses[2].atomic_ordering(context),
            Some(AtomicOrderingAttr::AcquireRelease)
        );
        assert_eq!(
            accesses[2].atomic_scope(context),
            Some(AtomicScopeAttr::Device)
        );
    }
}
