...
//! ranked kernel, deterministic ranked IR, and the five fixed clean analysis
...
use dialect_kernel::{
    AccessKindAttr, IndexBinaryKindAttr, MemorySpaceAttr, SemanticBinaryKindAttr,
};
...
pub enum ProductionMiddleEndAssuranceV3 {
    /// fe2o3's five internal analyses completed cleanly; no Verus proof is claimed.
    InternalChecksOnly,
}
...
impl ProductionMiddleEndEvidencePassV3 {
    const fn tag(self) -> u8 {
        match self {
            Self::MemoryBounds => 1,
            Self::RaceFreedom => 2,
            Self::BarrierConvergence => 3,
            Self::WorkgroupMemory => 4,
            Self::SemanticRefinement => 5,
        }
    }
...
/// revalidated the live semantic owner, ranked structure, and all five reports.
