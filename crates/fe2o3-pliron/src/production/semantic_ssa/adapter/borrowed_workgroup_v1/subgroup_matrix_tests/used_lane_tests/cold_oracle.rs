...
#[path = "oracle_lane_consumer_v1.rs"]
...
pub(super) fn sites_with_consumers(
    ...
    // Terminal checks never restore validity. Reject already-dead components
    // before auditing unrelated statements; every survivor still receives the
    // complete statement audit. Only exact closed lane leaf consumers can pass.
    ...
    if !viable { return Ok(BTreeMap::new()) }
    for (block, body) in function.blocks().iter().enumerate() {
        for (statement, source) in body.statements().iter().enumerate() {
            budget.charge(2)?;
    ...
    for (root, mut relation) in roots {
        if let Ok(members) = borrow_components_v1::members(&candidates, &children, root, budget, |_| true)? {
