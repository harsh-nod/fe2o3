...
/// A closed original getter/bridge result, not a Math issuer or ambient ZST.
/// Only replay of the complete Defined call chain can construct this record.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct ProductionSemanticMathBridgeResultV1 {
...
impl ProductionSemanticMathBridgeResultV1 {
    ...
    pub const fn math(&self) -> SemanticTypeIdV1 {
        self.math
    }
}

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub(super) struct DefinedMathResultsV1 {
    view_identity: Option<[u8; 32]>,
    entries: Box<[ProductionSemanticMathBridgeResultV1]>,
    resources: SemanticSsaAuxiliaryResourcesV1,
}

impl DefinedMathResultsV1 {
    pub(super) fn derive(
        ...
        if !semantic.functions().iter().any(|function| matches!(function.defined_capability_contract(),
            Some(SemanticDefinedCapabilityContractV1::KernelMathDerive(record)) if record.provenance().root() == view.root())) {
        ...
        for binding in bindings {
            charge(1, 0)?;
            let SemanticDefinedCapabilityContractV1::KernelMathDerive(record) = binding.contract()
            else {
                continue;
            };
            ...
            for (body, origin) in view.body().blocks().iter().zip(view.block_origins()) {
                ...
                if let SemanticTerminatorKindV1::Call(call) = body.terminator().kind() {
                    ...
                    if call.callee() != record.current_callable()
                        ...
                        || destination.ty() != record.types().unbranded_math
            ...
            if getter_assignment.destination() != binding.destination()
                || !matches!(getter_assignment.value().kind(), SemanticRvalueKindV1::Use(SemanticOperandV1::Move(source))
                    if source.local() == binding.callee_return() && source.projections().is_empty() && source.ty() == record.types().math)
            {
                return Err(mismatch());
            }
            charge(128, 32)?;
            entries.push(ProductionSemanticMathBridgeResultV1 {
    ...
    pub(super) fn entries(&self) -> &[ProductionSemanticMathBridgeResultV1] {
        &self.entries
    }
    ...
    pub(super) fn verify_markers(
        ...
        for entry in &self.entries {
            ...
            if value.destination().local() != entry.destination
                ...
                || value.destination().ty() != entry.math
                || !matches!(value.value().kind(), SemanticRvalueKindV1::Use(SemanticOperandV1::Move(place))
                    if place.local() == entry.return_local && place.projections().is_empty() && place.ty() == entry.math)
            {
                return Err(ProductionSemanticSsaErrorV1::ReplayMismatch);
            }
            ...
            if !matches!(getter_value.value().kind(), SemanticRvalueKindV1::Use(SemanticOperandV1::Move(place))
                if place.local() == entry.destination && place.projections().is_empty() && place.ty() == entry.math)
            {
                return Err(ProductionSemanticSsaErrorV1::ReplayMismatch);
            }
        }
    ...
    pub(super) fn hash_into(&self, digest: &mut Sha256) {
        ...
        digest.update(b"fe2o3.execution-defined-math-results.v1\0");
        digest.update(
            self.view_identity
...
#[cfg(test)]
#[path = "defined_math_results/tests.rs"]
pub(super) mod tests;
