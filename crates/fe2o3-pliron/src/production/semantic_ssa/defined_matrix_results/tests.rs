...
pub(crate) fn fixture_source() -> AdmittedInertSemanticMirV1 {
...
fn checked_result_occurrences_keep_distinct_getter_and_bridge_instances() {
    ...
    let relation = DefinedMatrixResultsV1::derive(
        &source,
        &expansion,
        view,
        ProductionSemanticSsaLimitsV1::default(),
...
#[test]
fn absent_metadata_does_not_initialize_an_ignored_matrix_return() {
...
fn checked_owner_preserves_original_return_move_and_frame_kills() {
    ...
    assert_eq!(plan.defined_matrix_results().len(), 2);
    ...
    let (input, implicit, _) = adapter::semantic_function_ssa_input_with_defined_results_v1(
        view.body(),
        ...
        &plan.defined_matrix_results,
        &plan.defined_reusable_lds_results,
        &plan.defined_reusable_phase_results,
        &plan.guarded_grid_results,
        |block, statement, range| {
            if let Some(entry) = plan.defined_matrix_results().iter().find(|entry| {
...
fn result_relation_rejects_foreign_view_and_changed_call_coordinates() {
    ...
    let expansion =
        SemanticCallExpansionV1::try_new(&source, SemanticCallExpansionLimitsV1::default())
            .unwrap();
    let view = expansion.root(root()).unwrap();
    let expected = DefinedMatrixResultsV1::derive(
        &source,
        &expansion,
        view,
        ProductionSemanticSsaLimitsV1::default(),
...
fn result_relation_resource_limits_are_inclusive() {
    ...
    for (storage, work, expected) in [
        ...
        assert!(
            matches!(DefinedMatrixResultsV1::derive(&source, &expansion, view, limits(storage, work)),
