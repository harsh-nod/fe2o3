...
pub type ArgumentBudgetV1<'a> = fe2o3_kernel_ir::CanonicalKernelIrVerificationResourceBudgetV1<'a>;
pub type ArgumentLedgerV1 = fe2o3_kernel_ir::CanonicalKernelIrWorkLedgerIdentityV1;
pub type ArgumentResourceV1 = fe2o3_kernel_ir::CanonicalKernelIrVerificationResourceErrorV1;

impl From<ArgumentResourceV1> for ProductionSourceArgumentErrorV1 {
...
#[derive(Clone, Copy)]
pub struct ArgumentTraceV1<'a> {
    pub direct: &'a [SemanticKirParameterBindingV1],
    pub components: &'a [SemanticKirParameterComponentBindingV1],
    pub ignored: &'a [SemanticKirIgnoredParameterBindingV1],
}

#[derive(Clone, Copy)]
pub enum PhysicalArgumentTraceV1<'a> {
...
pub struct IndexedArgumentTraceV1<'a> {
    pub trace: PhysicalArgumentTraceV1<'a>,
    pub used: bool,
}
...
pub fn sort_argument_trace_v1(rows: &mut [IndexedArgumentTraceV1<'_>], bit: u32) {
    sort_correspondence_keys_v1(rows, bit, &|row| u64::from(row.trace.value().0));
}

pub fn sort_correspondence_keys_v1<T>(rows: &mut [T], bit: u32, key: &impl Fn(&T) -> u64) {
    if rows.len() < 2 {
        return;
    }
...
pub fn argument_sum_v1(values: &[usize]) -> Result<usize, ArgumentResourceV1> {
...
pub fn argument_product_v1(left: usize, right: usize) -> Result<usize, ArgumentResourceV1> {
    left.checked_mul(right)
        .ok_or(ArgumentResourceV1::Arithmetic)
}

pub fn argument_vec_v1<T>(length: usize) -> Result<Vec<T>, ArgumentResourceV1> {
...
pub fn validate_parameter_correspondence_v1(
    ...
    with_parameter_correspondence_v1(semantic, instance, target, trace, budget, |_, _| Ok(()))
}

pub fn with_parameter_correspondence_v1<'w, R>(
    semantic: &AdmittedInertSemanticMirV1,
    instance: &SemanticKirFunctionCorrespondenceV1,
    target: &Function,
    trace: ArgumentTraceV1<'_>,
...
fn check_argument_trace_v1<'w, R>(
    ...
    view.visit_nodes(budget, |_| Ok(()))?;
    // Prepay the post-callback custody check before the consumer runs.
    budget.charge_work(5)?;
    let custody = view.data;
    let result = use_view(view, budget);
    // Validate before dropping the still-charged indices, including when the
    // consumer returns an error without making another query.
    custody.require_live_v1(budget)?;
    result
}

pub fn prepay_argument_shape_v1(
    semantic: &AdmittedInertSemanticMirV1,
    ty: SemanticTypeIdV1,
    budget: &mut ArgumentBudgetV1<'_>,
) -> Result<(), ProductionSourceArgumentErrorV1> {
...
pub fn count_argument_shape_nodes_v1(
    types: &[SemanticTypeDeclV1],
    ty: SemanticTypeIdV1,
    nodes: &mut usize,
    budget: &mut ArgumentBudgetV1<'_>,
