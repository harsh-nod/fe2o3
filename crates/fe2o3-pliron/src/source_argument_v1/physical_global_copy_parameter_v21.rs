//! Narrow ABI-planner recognition for an actual MIR38 whole-body marker.
...
pub fn physical_global_copy_slice_parameter_v21(
    ...
    if root.index() != 0 || !(2..=35).contains(&callables.len()) {
        return None;
    }
    ...
    for block in function.blocks() {
        ...
        {
            let source = call.physical_global_copy_source_v38()?;
    ...
    occurrence?;
    let (element, length) = physical_global_copy_marker_slice_abi_v21(types, binding.abi(), ty)?;
    Some((element, length, AccessMode::ReadWrite))
}

fn physical_global_copy_marker_slice_abi_v21(
    types: &[SemanticTypeDeclV1],
    ...
    let length = physical_global_copy_slice_fields_v21(types, ty, element)?;
    Some((element, length))
}
...
/// with the caller's retained MIR38 owner.
pub fn shared_u32_element(
    types: &[SemanticTypeDeclV1],
    ty: SemanticTypeIdV1,
...
fn physical_global_copy_slice_fields_v21(
    types: &[SemanticTypeDeclV1],
    ty: SemanticTypeIdV1,
    ...
    // MIR38 deliberately retains the ordinary integer representation; V35
