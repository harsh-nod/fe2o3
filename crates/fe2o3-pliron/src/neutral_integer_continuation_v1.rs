//! Explicit Policy6 integer/DCE native continuation. Historical V1 entrypoints never
...
use crate::{
    IntegerContinuationExecutionWitnessV1, KirBridgeOptimizedReceiptV1,
    KirOptimizationMapIntegerContinuationV12, PlironOptimizationReportV1,
...
/// Actual two-pass output with immutable execution evidence; not yet checked
/// for semantic preservation. The actual Policy5 O remains borrowed without copying.
pub struct KirNeutralOptimizationOutputIntegerContinuationV1<'input> {
    parts: ObservedParts<
        'input,
...
/// The same actual I after independent F2NTR1 local-rule checking, with distinct
/// trusted execution evidence. No V1 owner conversion or old-policy cast exists.
pub struct CheckedNeutralKernelIrOwnerIntegerContinuationV1 {
    parts: CheckedParts<
        KirOptimizationMapIntegerContinuationV12,
        IntegerContinuationExecutionWitnessV1,
    >,
...
fn wrapper_remainder<T>() -> Option<usize> {
    ...
    for header in [
        ...
        size_of::<KirOptimizationMapIntegerContinuationV12>(),
...
fn observed_wrapper() -> Option<usize> {
    wrapper_remainder::<KirNeutralOptimizationOutputIntegerContinuationV1<'_>>()
}
fn checked_wrapper() -> Option<usize> {
    wrapper_remainder::<CheckedNeutralKernelIrOwnerIntegerContinuationV1>()
}

impl<'input> KirNeutralOptimizationOutputIntegerContinuationV1<'input> {
    ...
    pub const fn map(&self) -> &KirOptimizationMapIntegerContinuationV12 {
        &self.parts.map
    }
    pub const fn occurrences(&self) -> &KirNeutralOccurrenceRowsV1 {
        &self.parts.occurrences
    }
    pub const fn execution(&self) -> &IntegerContinuationExecutionWitnessV1 {
        &self.parts.extra
    }
    ...
    ///
    /// ```compile_fail
    /// use fe2o3_pliron::KirNeutralOptimizationOutputIntegerContinuationV1;
    /// use fe2o3_kernel_ir::CanonicalKernelIrVerificationResourceBudgetV1 as Budget;
    /// fn escape_view(observed: KirNeutralOptimizationOutputIntegerContinuationV1<'_>, budget: &mut Budget<'_>) {
    ...
    /// use fe2o3_pliron::KirNeutralOptimizationOutputIntegerContinuationV1;
    /// use fe2o3_kernel_ir::CanonicalKernelIrVerificationResourceBudgetV1 as Budget;
    /// fn escape_input(observed: KirNeutralOptimizationOutputIntegerContinuationV1<'_>, budget: &mut Budget<'_>) {
        ...
        >,
    {
        ...
        .map(|(parts, origin, receipt)| {
            (
                CheckedNeutralKernelIrOwnerIntegerContinuationV1 { parts },
                origin,
                receipt,
            )
        })
...
impl CheckedNeutralKernelIrOwnerIntegerContinuationV1 {
    pub const fn owner(&self) -> &Owner {
        &self.parts.owner
    }
    pub const fn report(&self) -> &PlironOptimizationReportV1 {
...
/// Executes the literal Policy6 integer-continuation roster on one private imported candidate and
/// extracts the actual output once. Prepaid capture/profile bounds and dynamic
/// integer-identity charges all use this same ledger. Reserve the returned transfer receipt
/// before further allocation; the original input remains borrowed and excluded.
pub fn optimize_native_neutral_kernel_ir_integer_continuation_v1<'input>(
    input: &'input Owner,
    budget: &mut Budget<'_>,
) -> Result<KirNeutralOptimizationOutputIntegerContinuationV1<'input>, Error> {
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        ...
        let limits = graph
            ...
            .and_then(|limits| limits.for_integer_continuation())
        ...
        let capture = graph
            .begin_neutral_occurrence_capture_for_policy_v1(limits, FixedPolicy::Integer6)
            .map_err(Error::Occurrences)?;
        let execution = graph.execute_native_integer_continuation_v1(budget, &capture, limits);
        ...
        let (owner, bridge, map, extracted) = graph
            .extract_admitted_canonical_with_integer_continuation_map_v1(budget, &witness)
        ...
        let occurrences = capture
            .finish_integer_continuation(&graph.session.context, &roster, &map, owner.module())
            .map_err(Error::Occurrences)?;
        ...
        let execution = IntegerContinuationExecutionWitnessV1::from_execution(
            input,
            &owner,
            &report,
            &map,
        ...
        let output = KirNeutralOptimizationOutputIntegerContinuationV1 {
            parts: ObservedParts {
                input,
                owner,
                report,
