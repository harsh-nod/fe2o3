...
use dialect_kernel::{
    BranchOp, DIALECT_NAME, RequireEquivalentOp, ReturnOp, SemanticBinaryKindAttr,
    SemanticBinaryOp, SemanticExceptionalValueAttr, SemanticExpressionCommitmentOp,
    SemanticIeeeRoundingAttr, SemanticNumericalContractV1, SemanticNumericalPolicyAttr,
    SemanticOverflowAttr, SemanticScalarKindAttr, SemanticScalarType, SemanticSymbolOp,
    ...
    SemanticTypedSymbolOp, register_dialect,
};
use dialect_proof::{
...
#[test]
fn empty_semantic_subset_still_retains_nested_progress_rejection() {
    let context = &mut setup();
    let function = function(context, "empty_semantic_progress_cycle", 0);
    ...
    assert!(
        !report.is_clean(),
        "the semantic-local empty fast path must retain the nested progress report"
    );
}
