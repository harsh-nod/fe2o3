...
use dialect_kernel::{
    AccessKindAttr, AllocationEffectOp, BranchOp, DIALECT_NAME, DimensionAttr,
    ...
    SemanticScalarKindAttr, SemanticSymbolOp, SemanticTypedScalarV1, SemanticTypedSymbolOp,
    neutral_workgroup_allocation_contract_v1, register_dialect,
};
use pliron::{
...
#[test]
fn typed_data_identity_rejects_unadmitted_nested_pointer_pointee() {
    let context = &mut setup();
    let hierarchy: TypeHandle = HierarchyIndexType::get(context, HierarchyAttr::Lane).into();
    let pointer: TypeHandle = dialect_gpu::optimization_v1::PointerType::get(
        context,
        hierarchy,
    ...
    .into();
    let slice: TypeHandle = dialect_gpu::optimization_v1::SliceType::get(
        context,
        pointer,
        dialect_gpu::AddressSpaceAttr::Global,
        dialect_gpu::optimization_v1::AccessModeAttr::ReadOnly,
    ...
    .into();
    let function = empty_function(context, "nested_unadmitted_data", vec![slice]);
    let ret = ReturnOp::new(context);
    append(context, function.get_entry_block(context), &ret);
    let error = derive_pliron_ir_structural_identity_v1(context, &function).unwrap_err();
    assert!(matches!(
...
#[test]
fn typed_data_pointer_cannot_launder_an_existing_semantic_carrier() {
    let context = &mut setup();
    let semantic: TypeHandle = dialect_kernel::SemanticScalarType::get(context).into();
    let pointer: TypeHandle = dialect_gpu::optimization_v1::PointerType::get(
        context,
        semantic,
        dialect_gpu::AddressSpaceAttr::Global,
    ...
    .into();
    let function = empty_function(context, "nondata_pointee", vec![pointer]);
    let ret = ReturnOp::new(context);
    append(context, function.get_entry_block(context), &ret);
    assert!(matches!(
...
#[test]
fn typed_data_identity_preserves_the_existing_nested_type_limit() {
    let context = &mut setup();
    let mut ty: TypeHandle = UnitType::get(context).into();
    for _ in 0..crate::MAX_PLIRON_IDENTITY_TYPE_NESTING_V1 {
        ty = dialect_gpu::optimization_v1::PointerType::get(
            context,
            ty,
    ...
    let function = empty_function(context, "deep_pointer_data", vec![ty]);
    let ret = ReturnOp::new(context);
    append(context, function.get_entry_block(context), &ret);
    let error = derive_pliron_ir_structural_identity_v1(context, &function).unwrap_err();
    assert!(matches!(
...
#[test]
fn live_identity_census_classifies_actual_resource_relevant_operations() {
    let context = &mut setup();
    let function = empty_function(context, "resource_census", vec![]);
    let entry = function.get_entry_block(context);
    let view_type = RankedViewType::new(context, 32, true, vec![1]).unwrap();
    let global_view =
        RankedViewOp::new_in_space(context, view_type, vec![], MemorySpaceAttr::Global).unwrap();
    let workgroup_view =
        RankedViewOp::new_in_space(context, view_type, vec![], MemorySpaceAttr::Workgroup).unwrap();
    let zero = IndexConstantOp::new(context, 0);
    let global_access = RankedAccessOp::new(
        context,
        AccessKindAttr::Read,
        global_view.result(context),
        vec![zero.result(context)],
    )
    .unwrap();
    let workgroup_access = RankedAccessOp::new(
        context,
        AccessKindAttr::Read,
        workgroup_view.result(context),
        vec![zero.result(context)],
    )
    .unwrap();
    ...
    .unwrap();
    let semantic_symbol = SemanticSymbolOp::new(context, 7);
    let semantic_requirement = RequireEquivalentOp::new(
        context,
        semantic_symbol.result(context),
        semantic_symbol.result(context),
    );
    let (neutral_origin, neutral_class) = neutral_workgroup_allocation_contract_v1([37; 32]);
    assert_ne!(
        neutral_class,
    ...
    assert!(
        ...
        .is_err(),
        "a mismatched reserved origin is rejected before identity census"
    );
    let non_reserved = AllocationEffectOp::new(
        context,
        AccessKindAttr::Read,
        MemorySpaceAttr::Workgroup,
        neutral_origin,
    ...
    .unwrap();
    // A lone reserved read cannot satisfy the complete write/barrier/read
    // lifecycle. Census it before the later semantic rejection.
    let malformed_reserved_candidate = AllocationEffectOp::new(
        context,
        AccessKindAttr::Read,
        MemorySpaceAttr::Workgroup,
        GFX950_TRANSPOSE_FP4_WORKGROUP_ALLOCATION_ORIGIN_V1,
    ...
    let ret = ReturnOp::new(context);
    for operation in [
        global_view.get_operation(),
        workgroup_view.get_operation(),
        zero.get_operation(),
        global_access.get_operation(),
        workgroup_access.get_operation(),
        ownership.get_operation(),
        semantic_symbol.get_operation(),
        semantic_requirement.get_operation(),
        non_reserved.get_operation(),
        malformed_reserved_candidate.get_operation(),
        ret.get_operation(),
    ] {
    ...
    let preservation = crate::production_analysis::pliron_pass_contract::begin_production_pliron_pass_contract_session_v1(
        crate::production_analysis::pliron_ir_identity::LivePlironStructuralIdentityProviderV1::new(
            context,
            &function,
        ),
    )
    ...
    assert_eq!(census.operations, 11);
    assert_eq!(census.ranked_accesses, 2);
    assert_eq!(census.workgroup_ranked_accesses, 1);
    assert_eq!(census.allocation_effects, 2);
    assert_eq!(census.collective_transpose_candidates, 1);
    assert_eq!(census.ownership_contracts, 1);
    assert_eq!(census.semantic_definitions, 1);
    assert_eq!(census.semantic_refinement_contracts, 2);
}

#[test]
fn semantic_census_predicate_counts_malformed_typed_candidates_before_verification() {
    let context = &mut setup();
    let scalar = SemanticTypedScalarV1::new(SemanticScalarKindAttr::UnsignedInteger, 32).unwrap();
    let malformed = SemanticTypedSymbolOp::new(context, 9, scalar);
    malformed.set_attr_kernel_semantic_typed_symbol_bit_width(context, DimensionAttr(0));
    let operation = pliron::operation::Operation::get_op_dyn(malformed.get_operation(), context);
    assert!(crate::production_analysis::is_semantic_refinement_definition_v1(&*operation));
    assert!(!crate::production_analysis::is_semantic_refinement_contract_v1(&*operation));
}
