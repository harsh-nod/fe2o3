...
fn native_data_textual_pliron_fixtures_v3() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/production_analysis/tests/lit");
    for fixture in [
        "native_data_checked_bool.pliron",
        "native_data_physical_index.pliron",
        "native_data_float.pliron",
        "native_data_wrong_binary_type.pliron",
        "native_data_wrong_overflow_type.pliron",
        "native_data_wrong_index_cast.pliron",
    ] {
        run_fixture(&root.join(fixture));
    }
}
...
#[test]
fn boolean_control_textual_pliron_fixtures_v3() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/production_analysis/tests/lit");
    for fixture in [
        "boolean_mixed_branch.pliron",
        "boolean_duplicate_edges.pliron",
        "boolean_wrong_condition.pliron",
        "boolean_wrong_type.pliron",
        "boolean_wrong_space.pliron",
        "boolean_wrong_access.pliron",
        "boolean_wrong_arity.pliron",
        "boolean_no_bounds_assumption.pliron",
        "boolean_uniform_tensor.pliron",
    ] {
        run_fixture(&root.join(fixture));
    }
}
...
#[test]
fn typed_ssa_signature_textual_pliron_fixtures() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/production_analysis/tests/lit");
    for fixture in [
        "typed_ssa_mixed_branch.pliron",
        "typed_ssa_wrong_type.pliron",
        "typed_ssa_wrong_space.pliron",
        "typed_ssa_wrong_access.pliron",
        "typed_ssa_wrong_arity.pliron",
    ] {
        run_fixture(&root.join(fixture));
    }
}
...
fn run_fixture(path: &Path) {
    ...
    let local_rejections = source
        .lines()
        .filter(|line| *line == "// EXPECT-LOCAL: REJECT")
        .count();
    assert!(
        local_rejections <= 1,
        "{} has duplicate local expectation",
        path.display()
    );
    if local_rejections == 1 {
        assert!(
            rejected,
            "{} local rejection requires EXPECT: REJECT",
            path.display()
        );
        let error = verify_operation(operation, &context)
            .expect_err("fixture must fail actual local verification");
        let output = format!("LOCAL-REJECT\n{error}");
        for check in checks {
            assert!(
                output.contains(check),
                "{} missing local diagnostic {check:?}: {output}",
                path.display()
            );
        }
        return;
