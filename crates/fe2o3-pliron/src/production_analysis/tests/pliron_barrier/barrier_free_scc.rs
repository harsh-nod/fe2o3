use super::*;
use crate::{KernelCheckStatusV1, PlironBarrierReportV1, run_pliron_progress_check_v1};
use dialect_kernel::{
...
#[test]
fn barrier_free_scc_retains_the_full_fresh_production_pipeline() {
    let mut context = setup();
    let function = branchy_loop(&mut context, Case::Memory);
    let report = crate::production_analysis::pliron_pipeline::require_production_pliron_checks_before_lowering_v2(
        &context, &function,
    );
    assert!(report.is_ok(), "fresh mandatory pipeline: {report:?}");
}
