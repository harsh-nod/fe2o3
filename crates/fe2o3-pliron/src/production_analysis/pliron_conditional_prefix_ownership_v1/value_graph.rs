...
use super::*;
use crate::production_analysis::pliron_ranked_bounds::paired_read_access_v1;
use dialect_kernel::{
...
pub(super) fn validate(
        ...
        for (position, pointer) in operations[block].iter().copied().enumerate() {
            ...
            if let Some(access) = op.downcast_ref::<RankedAccessOp>() {
                if access.kind(context) == Some(AccessKindAttr::Write) {
                    let value = access
                        .stored_value(context)
                        .ok_or(Error::Unsupported("write has no retained scalar RHS"))?;
                    let scalar = values
                        .get(&value)
            ...
            let scalar = if let Some(symbol) = op.downcast_ref::<SemanticTypedSymbolOp>() {
                ...
                .ok_or(Error::Malformed("unpaired read"))?;
    ...
    // The scoped owner input supplies native dominance, not this topological visit.
    Ok(())
}
