...
pub fn main() -> ExitCode {
    ...
    if arguments.first().is_some_and(|value| {
        value == OsStr::new("live-rocgdb-kfd-v4")
            || value == OsStr::new("live-rocgdb-kfd-v5")
            || value == OsStr::new("capture-rocgdb-kfd-resources-v1")
    }) {
        #[cfg(all(target_os = "linux", target_arch = "x86_64"))]
        {
    ...
    if arguments.first().is_some_and(|v| v == "sim")
        && arguments.iter().any(|v| v == "--diagnostic-kir-v22")
    {
        #[cfg(target_os = "linux")]
        {
            return diagnostic_kir_v20::run_v22(arguments);
        }
        #[cfg(not(target_os = "linux"))]
        {
            write_bootstrap_error(
                "platform",
                "kir_v22_debug_platform_unavailable",
                "typed physical-LDS-exchange CPU debugger requires Linux",
            );
            return ExitCode::FAILURE;
        }
    }
