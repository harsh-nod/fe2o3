...
use fe2o3_debug_protocol::{
    ...
    DiagnosisClassV2, DiagnosisFactV2, DiagnosisResponseV2, DiagnosisViewV2, ProtocolLimitsV1,
...
use fe2o3_semantic_query::{
    ...
    AGENT_PROFILER_REQUEST_SCHEMA_V1, AGENT_PROFILER_VARIANT_COMPARISON_SCHEMA_V1,
    AGENT_PROFILER_VARIANT_REQUEST_SCHEMA_V1, MAX_AGENT_PROFILER_REQUEST_BYTES_V1,
...
fn main() -> ExitCode {
    match run() {
        Ok(report) => {
            ...
            bytes.push(b'\n');
            if bytes.len() > MAX_REFERENCE_REPORT_BYTES_V1
...
fn diagnose_simulator(
    ...
    executable.revalidate("debugger executable")?;
    let mut child = Command::new(executable.proc_path())
        .args(["sim", "--kir-v7"])
        .arg(kernel.path())
        .arg("--request")
        .arg(request.path())
        ...
        .stderr(Stdio::piped())
        .spawn()
        .map_err(|_| "could not spawn debugger")?;
    if let Err(error) = executable.revalidate("debugger executable") {
        let _ = child.kill();
        let _ = child.wait();
        return Err(error);
    }
    ...
    {
        let mut input = child.stdin.take().ok_or("debugger stdin unavailable")?;
        for request in requests {
            write_json_line(&mut input, &request, 64 * 1024)?;
        }
    }
    let output = child
        .wait_with_output()
        .map_err(|_| "debugger wait failed")?;
    executable.revalidate("debugger executable")?;
    validate_child(&output.status, &output.stderr, "debugger")?;
    let lines = jsonl_lines(
        &output.stdout,
        ProtocolLimitsV1::default().max_response_line_bytes,
    )?;
    if lines.len() != 3 {
        return Err("debugger returned an unexpected response count".into());
    }
    let capabilities = decode_response_line_v1(lines[0], ProtocolLimitsV1::default())
        .map_err(|_| "invalid debugger capability response")?;
    let DebugResponseV1::Ok { result, .. } = capabilities else {
    ...
    decode_response_line_v1(lines[1], ProtocolLimitsV1::default())
        .map_err(|_| "invalid debugger control response")?;
    let diagnosis = decode_diagnosis_response_line_v2(lines[2], ProtocolLimitsV1::default())
...
fn compare_variants(
    ...
    let mut session = JsonlChildV1::spawn(executable, "variant-jsonl")?;
    let capabilities = session.exchange(
        &json!({
        ...
        MAX_AGENT_PROFILER_VARIANT_REQUEST_BYTES_V1,
...
fn plan_next_capture(
    ...
    let mut session = JsonlChildV1::spawn(executable, "jsonl")?;
    let capabilities = session.exchange(
        &json!({
            "operation":"discover_capabilities","schema":AGENT_PROFILER_REQUEST_SCHEMA_V1,"request_id":1
        }),
        MAX_AGENT_PROFILER_REQUEST_BYTES_V1,
        MAX_AGENT_PROFILER_RESPONSE_BYTES_V1,
    )?;
    ...
    if capture.is_null() {
        return Err("opened capture identity missing".into());
    }
    let first = session.exchange(
        &json!({
    ...
    let cursor = first["value"]["page"]["next_cursor"].clone();
    if cursor.is_null() {
        return Err("first dispatch page was not paged".into());
    }
    let dispatch = first["value"]["page"]["items"][0]["dispatch"]["identity"].clone();
    if dispatch.is_null() {
        return Err("first dispatch page did not expose a content-bound selector".into());
    }
    let second = session.exchange(
        &json!({
...
impl<'a> JsonlChildV1<'a> {
    fn spawn(executable: &'a PinnedExecutableV1, mode: &str) -> Result<Self, String> {
        executable.revalidate("profiler service executable")?;
        let mut child = Command::new(executable.proc_path())
            .arg(mode)
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()
            .map_err(|_| "could not spawn profiler service")?;
        if let Err(error) = executable.revalidate("profiler service executable") {
            let _ = child.kill();
            let _ = child.wait();
            return Err(error);
        }
        let input = child.stdin.take().ok_or("profiler stdin unavailable")?;
        let output = BufReader::new(child.stdout.take().ok_or("profiler stdout unavailable")?);
        Ok(Self {
            child,
            input: Some(input),
            output,
    ...
    fn exchange(
        ...
        write_json_line(
            self.input.as_mut().ok_or("profiler input closed")?,
            request,
            max_request,
        )?;
        ...
        self.output
            .read_until(b'\n', &mut line)
            .map_err(|_| "profiler response read failed")?;
        if line.is_empty() || line.len() as u64 > max_response || !line.ends_with(b"\n") {
            return Err("profiler response violated line bound".into());
        }
        ...
        serde_json::from_slice(&line).map_err(|_| "invalid profiler JSON response".into())
    }
    ...
    fn finish(mut self) -> Result<(), String> {
        ...
        let output = self
            .child
            .wait_with_output()
            .map_err(|_| "profiler wait failed")?;
        self.executable.revalidate("profiler service executable")?;
        validate_child(&output.status, &output.stderr, "profiler service")
    }
}

fn write_json_line(output: &mut impl Write, value: &Value, max: u64) -> Result<(), String> {
    let mut bytes = serde_json::to_vec(value).map_err(|_| "request encoding failed")?;
    bytes.push(b'\n');
    if bytes.len() as u64 > max {
        return Err("request exceeds protocol bound".into());
    }
    output
        .write_all(&bytes)
    ...
    output
        .flush()
        .map_err(|_| "request flush failed".to_owned())
}
...
fn validate_child(
    ...
    if stderr.len() as u64 > MAX_CHILD_STDERR_BYTES_V1 {
        return Err(format!("{label} stderr exceeded bound"));
    }
    if !status.success() {
...
fn jsonl_lines(bytes: &[u8], max_line: usize) -> Result<Vec<&[u8]>, String> {
    if bytes.last() != Some(&b'\n') {
        return Err("child output lacks line terminator".into());
    }
    let lines = bytes
        .split_inclusive(|byte| *byte == b'\n')
        .collect::<Vec<_>>();
    if lines.iter().any(|line| line.len() > max_line) {
        return Err("child response exceeds line bound".into());
    }
    Ok(lines)
}
