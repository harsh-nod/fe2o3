...
fn all_program_lengths_and_instruction_forms_have_one_logical_before_after_pair_per_lane() {
    for case in fixture::cases() {
        for used in [true, false] {
            for arguments in [[19, 23, 42], [u32::MAX, 1, 2]] {
                ...
                for record in backend
                    ...
                    .records()
                    .iter()
                    .filter(|record| record.site.operation == 0)
                {
                    match record.kind {
                        ...
                        SimulationDebugRecordKindV1::Checkpoint {
                            ...
                            assert_eq!(
                                scalar(record, 4),
                                Some((case.oracle)(arguments)),
                                "{}",
                                case.name
                            );
                        }
                        _ => panic!(
                            "internal declared steps must not become physical/instruction observations"
                        ),
                    }
                }
                assert_eq!((before, after), (64, 64));
                ...
                assert_eq!(
                    memory[0].bytes,
                    fixture::output(if used {
                        (case.oracle)(arguments)
                    } else {
                        arguments[0]
                    })
                );
...
fn configuration_requires_exact_v17_owner_and_binds_declared_program_source_and_registers() {
    ...
    let old_input = fe2o3_kir_sim_cli::load_debug_simulation_input_bytes_v16(
        old.canonical_bytes(),
    ...
    for variant in 0..5 {
        ...
        let mut registers = program.registers();
        let mut declared = *program.program();
        match variant {
            0 => {}
            1 => source.statement = [99; 32],
            2 => {
                registers =
                    fe2o3_kernel_ir::Gfx942OrderedProgramRegistersV1::new(40, 41, [42, 43, 44])
                        .unwrap()
            }
            3 => declared = fixture::program(&[8]),
            4 => declared = fixture::program(&[0, 8]),
            _ => unreachable!(),
        }
        operation.kind = fe2o3_kernel_ir::OperationKind::Gfx942OrderedProgram(
        ...
        .unwrap();
        assert!(identities.insert(identity(&input).as_bytes()));
    }
    assert_eq!(identities.len(), 5);
}

#[test]
fn typed_shared_buffer_view_and_wave_fields_bind_configuration_without_rewriting_raw_request() {
    ...
    admitted.request.arguments[3] = SimulationArgumentV1::BufferView(
    ...
    let raw = admitted.request_sha256;
    ...
    for field in 0..9 {
        ...
        match field {
            0..=4 => {
                admitted.request.arguments[3] = SimulationArgumentV1::BufferView(
                    BufferViewArgumentV1::new(
                        BufferBackingIdV1(if field == 0 { 18 } else { 17 }),
                        ScalarType::U32,
                        if field == 1 {
                            AccessMode::ReadOnly
                        } else {
                        ...
                        if field == 2 { 8 } else { 4 },
                        if field == 3 { 4 } else { 0 },
                        if field == 4 { 63 } else { 64 },
                        admitted.simulation_target(),
                    )
                    .unwrap(),
                );
            }
            5 => admitted.request.shared_buffers[0].id = BufferBackingIdV1(18),
            6..=8 => {
                let buffer = &original.shared_buffers[0].buffer;
                ...
                admitted.request.shared_buffers[0].buffer = BufferArgumentV1::new(
                    buffer.element(),
                    buffer.access(),
                    if field == 8 { 8 } else { buffer.alignment() },
                    bytes,
                    initialized,
                    admitted.simulation_target(),
                )
        ...
        assert_eq!(admitted.request_sha256, raw);
        let actual = identity(&admitted);
        assert_ne!(actual, base, "field={field}");
        assert!(distinct.insert(actual.as_bytes()));
    }
    admitted.request = original;
    let wave32 = configuration_identity(
        &admitted,
    ...
    .unwrap();
    assert_ne!(wave32, base);
    // Hash coverage is not option admission: the actual session still refuses wave32.
    assert!(require_supported_options(DebugWaveWidthV1::Wave32, false, false).is_err());
}
#[path = "../../fe2o3-kir-sim-cli/tests/fixtures/diagnostic_kir_v17.rs"]
mod fixture;

fn input(used: bool, arguments: [u32; 3]) -> AdmittedSimulationInputV1 {
    let owner = fixture::owner(&fixture::module(used));
    load_debug_simulation_input_bytes_v17(owner.canonical_bytes(), &fixture::request(arguments))
        .unwrap()
}
...
#[test]
fn diagnostic_options_select_only_explicit_v17_path_and_wave64() {
    ...
    for arguments in [
        vec!["sim", "--diagnostic-kir-v17", "/missing/first"],
        vec![
            "sim",
            "--diagnostic-kir-v17",
            "/missing/first",
...
fn unsupported_options_refuse_during_argument_parsing_before_path_admission() {
    ...
    for extras in [
        ...
        assert!(
            error.contains("diagnostic KIR V17 requires wave64"),
            "{error}"
        );
    }
...
fn bindings(record: &SimulationDebugRecordV1) -> &[SimulationDebugBindingV1] {
    let SimulationDebugRecordKindV1::Checkpoint {
        ...
        panic!("expected captured whole-program checkpoint");
    };
...
#[test]
fn canonical_owner_retains_logical_whole_program_pairs_without_source_or_physical_claims() {
    for used in [true, false] {
        for arguments in [[19, 23, 42], [u32::MAX, 0, 1], [0, 0, 0]] {
            ...
            assert_eq!(backend.module.identity().wire_version(), 17);
            ...
            for record in backend.session.transcript().records() {
                ...
                match record.kind {
                    ...
                    _ => panic!(
                        "No memory event or instruction microstep belongs inside this NoMemory program"
                    ),
                }
...
#[test]
fn diagnosis_v2_refuses_v17_instead_of_serializing_a_v7_evidence_label() {
    let mut backend =
...
fn post_admission_typed_request_changes_bind_even_if_original_raw_hash_is_unchanged() {
    ...
    for field in 0..10 {
        ...
        match field {
            ...
            3 => admitted.request.kernel = "other_program".into(),
        ...
        for index_width in [IndexWidthV1::Bits64, IndexWidthV1::Bits32] {
            ...
            if variant < 3 {
                ...
                module.functions[0]
                    ...
                    .parameters
                    .push(ValueId(7));
                admitted.module = AdmittedSimulationModuleV1::admit_v17(
