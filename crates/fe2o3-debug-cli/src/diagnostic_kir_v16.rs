//! Raw V16 is diagnostic CPU input, never a serialized source owner.
use super::*;

pub(super) const DIAGNOSIS_UNAVAILABLE: &str = "diagnosis V2 cannot represent raw canonical KIR V16; logical debugger/resource queries remain available";
...
pub(super) fn require_supported_options(
    ...
    if wave != DebugWaveWidthV1::Wave64 || source_map || replay {
        return Err("diagnostic KIR V16 requires wave64 and does not support source maps or persisted schedule replay".to_owned());
    }
    Ok(())
}
