fn runtime_v8_semantic_fixture() -> (Vec<u8>, u16, [u8; 32], [u8; 32]) {
    ...
    .with_kernel_entry(SemanticKernelEntryV1::new(
        SemanticLinkSymbolV1::new(b"bundle_v8_entry".to_vec()).unwrap(),
...
fn runtime_v8_bundle_fixture() -> (VerifiedSimulationBundleV8, [u8; 32]) {
    let scalar = Type::Scalar(ScalarType::U32);
    let physical = Type::slice(scalar.clone(), AddressSpace::Global, AccessMode::ReadOnly);
    let context = KernelContextTypeV1::new("bundle_v8_entry", [0x81; 32], [0x82; 32], [0x83; 32]);
    ...
    let mut module = Module::new("sim-runtime-bundle-v8-test");
    ...
    let source_map = DebugSourceMapDocumentV2::new(
        ...
        vec![DebugSourceMapFileV1::new([0x7f; 32], 16, "runtime-bundle-v8.rs".into()).unwrap()],
        vec![],
    ...
    .unwrap();
    let (semantic, semantic_version, semantic_digest, abi_identity) = runtime_v8_semantic_fixture();
...
#[test]
fn bundle_v8_executes_exact_v13_context_and_global_capabilities_without_downgrade() {
    let (bundle, abi_identity) = runtime_v8_bundle_fixture();
    ...
    .unwrap();
    assert_eq!(parsed.admitted.identity().wire_version(), 13);
    assert_eq!(
    ...
    let module = backend
        ...
        .unwrap();
    let kernel = backend
        .resolve_kernel_v1(module, "bundle_v8_kernel", abi_identity)
        .unwrap();
