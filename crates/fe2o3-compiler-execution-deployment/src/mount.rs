...
impl MountedCompilerExecutionQualificationV1 {
    ...
    fn cleanup_internal(&mut self) -> Result<(), DeploymentVerificationErrorV1> {
        ...
        if self.root_attached {
            unmount(
                descriptor_path(staged.directory_descriptor("root")),
                UnmountFlags::empty(),
            )
            .map_err(|source| io_error("unmount qualification overlay root", source))?;
            self.root_attached = false;
            self.mounted_root.take();
        }
        if self.base_attached {
            unmount(
                descriptor_path(staged.directory_descriptor("base")),
                UnmountFlags::empty(),
            )
            .map_err(|source| io_error("unmount qualification SquashFS base", source))?;
...
pub fn attach_compiler_execution_qualification_mounts_v1(
    ...
    if let Err(error) = attach_base(&mut mounted) {
        return Err(mounted.cleanup_or(error));
    }
    if let Err(error) = attach_overlay(&mut mounted) {
        return Err(mounted.cleanup_or(error));
    }
