...
#[test]
fn same_ledger_admission_revalidation_and_terminal_release_preserve_exact_custody() {
    let fixture = Fixture::new();
    let baseline = fixture.references();
    assert!(baseline >= 1);
    let stat = rustix::fs::fstat(&fixture.witness).unwrap();
    let mut work = Work::new(Client::ADMISSION_WORK + Client::REVALIDATION_WORK);
    let floor = EXTRA + Client::FD_STORAGE;
    let mut budget = Budget::new(&mut work, EXTRA + Client::RETAINED + Client::IO_STORAGE);
    budget.reserve_storage(floor).unwrap();
