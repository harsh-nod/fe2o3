...
impl FakeRetained {
    fn new(drops: Arc<AtomicUsize>) -> Self {
        let bytes = b"exact-finalized-row-softmax-v1".to_vec();
...
impl RetainedProtectedRowSoftmaxV1 for FakeRetained {
...
unsafe impl ReviewedProtectedRowSoftmaxV1RuntimeAdapterV1 for FakeAdapter {
...
fn load(
    state: &FakeState,
    drops: Arc<AtomicUsize>,
) -> Result<LoadedState<FakeRetained, FakeAdapter>, ProtectedRowSoftmaxV1LoadErrorV1<FakeError>> {
...
fn load_error_names_the_hsa_context() {
    let error: ProtectedRowSoftmaxV1LoadErrorV1<FakeError> =
        ProtectedRowSoftmaxV1LoadErrorV1::ContextIdentity;
