...
fn nominal_host_v3_argument_bounds_output_extents_and_empty_kernel_are_closed() {
    with_plan(&[Shape::Usize], "gfx942:xnack-", false, |plan, budget| {
        ...
        ));
        {
            ...
            let mut bytes = [0xa5; 9];
            for length in [0, 7, 9] {
                assert!(
                    matches!(plan.pack(&inputs,&mut bytes[..length],budget).err().unwrap(),GeneratedNominalPackErrorV3::OutputLength{expected:8,actual} if actual==length)
                );
                assert_eq!(bytes, [0xa5; 9]);
            }
        }
        budget.release_storage(budget.storage() - floor).unwrap();
    });
