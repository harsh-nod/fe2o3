...
use crate::{
    MoeCompletedRoutingExpertBridgeV2, MoeExpertWeightArtifactBindingV2,
...
/// V2 completed-readback binding with a pre-joined expert-weight artifact.
///
/// V1 remains available as the host-observed compatibility API. V2 cannot be
/// prepared in safe production code until protected router/readback and expert
/// artifact lifecycles issue both required opaque capabilities.
...
>
    ...
    >
{
    /// Retains a completed V2 routing bridge and the exact pre-bound weight artifact.
    pub fn prepare(
        observed: &ObservedContext,
        ...
        validate_observed_target(observed.device().target())
            .map_err(GeneratedMoeExpertV2HostAdapterErrorV2::RegionAdmission)?;
        ...
        for (role, context) in MoeExpertV1BufferRoleV1::ALL.into_iter().zip(contexts) {
            if !observed.is_for_context(context) {
                return Err(GeneratedMoeExpertV2HostAdapterErrorV2::RegionAdmission(
                    GeneratedMoeExpertV1HostAdapterErrorV1::WrongContext { role },
                ));
            }
        }
        let prepared = prepare_regions([
...
/// Rejection before the V2 completed expert binding is retained.
#[derive(Debug)]
#[non_exhaustive]
pub enum GeneratedMoeExpertV2HostAdapterErrorV2 {
    RequestBatchIdentity,
    WeightArtifactBinding,
    RegionAdmission(GeneratedMoeExpertV1HostAdapterErrorV1),
}
...
impl Error for GeneratedMoeExpertV2HostAdapterErrorV2 {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            Self::RegionAdmission(error) => Some(error),
            _ => None,
        }
    }
}
