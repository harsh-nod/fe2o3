...
fn prepare_physical_kernarg_parts(
    ...
    let mut storage = ScalarGemmAlignedKernarg::new(SCALAR_GEMM_V1_TOTAL_KERNARG_BYTES, alignment)?;
    storage.bytes_mut()[..SCALAR_GEMM_V1_EXPLICIT_KERNARG_BYTES].copy_from_slice(packed.bytes());
    Ok(storage)
}
...
impl ScalarGemmAlignedKernarg {
    fn new(byte_len: usize, alignment: usize) -> Result<Self, ScalarGemmV1PhysicalKernargError> {
...
mod tests {
    ...
    fn plan(seed: u8) -> GeneratedArgumentPackingPlanV1 {
        let generated =
            CompilerGeneratedArgumentLayoutV1::new(64, 8, PointerWidth::Bits64, fields()).unwrap();
