...
use fe2o3_hsaco_finalize::{
    ContentIdentityV1, FinalizedMoeTop2V1HsacoIdentityV1, FinalizedWorkerV2HsacoIdentityV1,
    PreparedFinalizedMoeTop2V1HsacoV1,
};
...
const EXPORT_SYMBOL: &str = "moe_top2_route_f32_t8_e4_k2_c4_v1";
const GRID: [u32; 3] = [1, 1, 1];
const WORKGROUP: [u32; 3] = [64, 1, 1];
const EXPLICIT_KERNARG_BYTES: usize = 128;
const COMPLETE_KERNARG_BYTES: usize = 384;
...
const UNLOAD_IDENTITY_DOMAIN_V1: &[u8] = b"FE2O3/MOE-TOP2/UNLOAD/V1\0";

/// Runtime-reported resource facts for the exact MoeTop2 kernel object.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct MoeTop2V1KernelResourceObservationV1 {
...
impl MoeTop2V1KernelResourceObservationV1 {
    pub const fn new(
        executable_object: HsaExecutableObjectIdentityV1,
        kernel_object: HsaKernelObjectIdentityV1,
        group_segment_size: u32,
...
/// [`ReviewedHsaImplicitKernargAdapterV1`].
pub unsafe trait ReviewedMoeTop2V1RuntimeAdapterV1:
    ReviewedHsaImplicitKernargAdapterV1
{
...
#[non_exhaustive]
pub enum MoeTop2V1JoinErrorV1 {
    ReceiptField(&'static str),
    HostField(&'static str),
}

impl fmt::Display for MoeTop2V1JoinErrorV1 {
...
impl Error for MoeTop2V1JoinErrorV1 {}

/// Recoverable load rejection before dispatch publication.
#[derive(Debug)]
#[non_exhaustive]
pub enum MoeTop2V1LoadErrorV1<E> {
    ContextIdentity,
...
impl<E: fmt::Display> fmt::Display for MoeTop2V1LoadErrorV1<E> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl<E: Error + 'static> Error for MoeTop2V1LoadErrorV1<E> {
...
#[derive(Debug)]
#[non_exhaustive]
pub enum MoeTop2V1DispatchErrorV1<E> {
    ImplicitAdapter(E),
...
impl<E: fmt::Display> fmt::Display for MoeTop2V1DispatchErrorV1<E> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
...
impl<E: Error + 'static> Error for MoeTop2V1DispatchErrorV1<E> {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
...
#[must_use = "the joined MoeTop2 request must enter its one-shot lifecycle"]
pub struct JoinedMoeTop2V1<
    'logits,
    'top2,
    'requested,
    'admitted,
    ...
    'inverse,
> {
    receipt: PreparedFinalizedMoeTop2V1HsacoV1,
    host: GeneratedMoeTop2V1HostAdapterV1<
        'logits,
        'top2,
        'requested,
        'admitted,
...
impl fmt::Debug for JoinedMoeTop2V1<'_, '_, '_, '_, '_, '_, '_, '_> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("JoinedMoeTop2V1")
            .field("receipt", &self.receipt.identity())
            .field("artifact", &self.receipt.structural_finalization_identity())
            .finish_non_exhaustive()
    }
}

/// Consumes the opaque finalization receipt and exact generated binding into one linear join.
pub fn join_moe_top2_v1<
    'logits,
    'top2,
    'requested,
    'admitted,
...
>(
    ...
    JoinedMoeTop2V1<
        ...
        'inverse,
    >,
    MoeTop2V1JoinErrorV1,
> {
    validate_join(&receipt, &host)?;
    Ok(JoinedMoeTop2V1 { receipt, host })
}

impl<'logits, 'top2, 'requested, 'admitted, 'offsets, 'slots, 'permutation, 'inverse>
    JoinedMoeTop2V1<'logits, 'top2, 'requested, 'admitted, 'offsets, 'slots, 'permutation, 'inverse>
{
    pub const fn receipt_identity(&self) -> FinalizedMoeTop2V1HsacoIdentityV1 {
        self.receipt.identity()
    }

    pub const fn finalized_artifact_identity(&self) -> FinalizedWorkerV2HsacoIdentityV1 {
        self.receipt.structural_finalization_identity()
    }

    pub fn load<A: ReviewedMoeTop2V1RuntimeAdapterV1>(
        self,
        mut adapter: A,
    ) -> Result<
        LoadedMoeTop2V1<
    ...
    > {
        ...
        {
            return Err(MoeTop2V1LoadErrorV1::ContextIdentity);
        }
        let state = load_after_context_match(self, adapter)?;
        Ok(LoadedMoeTop2V1 { state })
    }
}

trait RetainedMoeTop2V1 {
...
impl RetainedMoeTop2V1 for JoinedMoeTop2V1<'_, '_, '_, '_, '_, '_, '_, '_> {
    fn target_v1(&self) -> &str {
        self.host.target()
    }
    ...
    fn with_finalized_bytes_v1<T, F: FnOnce(&[u8], ContentIdentityV1) -> T>(&self, load: F) -> T {
        ...
        unsafe {
            self.receipt
                .with_exact_finalized_bytes_for_reviewed_moe_top2_runtime_v1(load)
        }
    }
}

struct LoadedState<R, A: ReviewedMoeTop2V1RuntimeAdapterV1> {
    retained: Option<R>,
    adapter: Option<A>,
    environment: HsaEnvironmentObservationV1,
    load: HsaCodeObjectLoadObservationV1,
    resolution: HsaKernelResolutionObservationV1,
    resources: MoeTop2V1KernelResourceObservationV1,
    executable: Option<A::Executable>,
    kernel: Option<A::Kernel>,
    kernarg: CompleteKernargV1,
}

impl<R, A: ReviewedMoeTop2V1RuntimeAdapterV1> Drop for LoadedState<R, A> {
    fn drop(&mut self) {
        self.kernel.take();
...
#[must_use = "the loaded MoeTop2 lifecycle must dispatch or terminally unload"]
pub struct LoadedMoeTop2V1<
    'logits,
    'top2,
    'requested,
    'admitted,
    ...
    'permutation,
    'inverse,
    A: ReviewedMoeTop2V1RuntimeAdapterV1,
> {
    state: LoadedState<
        JoinedMoeTop2V1<
            'logits,
...
impl<A: ReviewedMoeTop2V1RuntimeAdapterV1> fmt::Debug
    for LoadedMoeTop2V1<'_, '_, '_, '_, '_, '_, '_, '_, A>
{
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("LoadedMoeTop2V1")
    ...
    >
{
    pub fn dispatch_and_wait(
        self,
    ) -> Result<CompletedMoeTop2V1<A>, MoeTop2V1DispatchErrorV1<A::Error>> {
        ...
        let receipt = CompletionReceiptV1 {
            ...
            finalized_artifact_identity: retained.receipt.structural_finalization_identity(),
            finalized_output_identity: retained.receipt.finalized_output_identity(),
        };
        Ok(CompletedMoeTop2V1 {
...
struct QuiescentState<R, A: ReviewedMoeTop2V1RuntimeAdapterV1> {
    retained: Option<R>,
    adapter: Option<A>,
    environment: HsaEnvironmentObservationV1,
    load: HsaCodeObjectLoadObservationV1,
...
impl<R, A: ReviewedMoeTop2V1RuntimeAdapterV1> Drop for QuiescentState<R, A> {
    fn drop(&mut self) {
        let Some(executable) = self.executable.take() else {
            return;
        };
...
impl<R, A: ReviewedMoeTop2V1RuntimeAdapterV1> QuiescentState<R, A> {
...
struct CompletedState<A: ReviewedMoeTop2V1RuntimeAdapterV1> {
    adapter: Option<A>,
    environment: HsaEnvironmentObservationV1,
    load: HsaCodeObjectLoadObservationV1,
    resolution: HsaKernelResolutionObservationV1,
...
impl<A: ReviewedMoeTop2V1RuntimeAdapterV1> Drop for CompletedState<A> {
    fn drop(&mut self) {
        let Some(executable) = self.executable.take() else {
            return;
        };
...
struct CompletionReceiptV1 {
    receipt_identity: FinalizedMoeTop2V1HsacoIdentityV1,
...
#[must_use = "the completed MoeTop2 lifecycle must be terminally unloaded"]
pub struct CompletedMoeTop2V1<A: ReviewedMoeTop2V1RuntimeAdapterV1> {
    state: CompletedState<A>,
    receipt: CompletionReceiptV1,
}

impl<A: ReviewedMoeTop2V1RuntimeAdapterV1> CompletedMoeTop2V1<A> {
    pub const fn receipt_identity(&self) -> FinalizedMoeTop2V1HsacoIdentityV1 {
    ...
    pub fn unload(mut self) -> UnloadedMoeTop2V1 {
...
pub struct MoeTop2V1UnloadIdentityV1([u8; 32]);

impl MoeTop2V1UnloadIdentityV1 {
...
pub struct UnloadedMoeTop2V1 {
    ...
    kernel_object: HsaKernelObjectIdentityV1,
    dispatch_identity: [u8; 16],
    unload_identity: MoeTop2V1UnloadIdentityV1,
}

impl UnloadedMoeTop2V1 {
    pub const fn receipt_identity(&self) -> FinalizedMoeTop2V1HsacoIdentityV1 {
    ...
    pub const fn unload_identity(&self) -> MoeTop2V1UnloadIdentityV1 {
...
impl<R, A: ReviewedMoeTop2V1RuntimeAdapterV1> LoadedState<R, A> {
    fn dispatch_and_wait(
        mut self,
    ) -> Result<QuiescentState<R, A>, MoeTop2V1DispatchErrorV1<A::Error>> {
    ...
    A: ReviewedMoeTop2V1RuntimeAdapterV1,
{
    ...
    let (kernel, resolution) = match reviewed_adapter_call(|| unsafe {
        ...
        Err(error) => {
            terminal_unload(&mut adapter, executable, &environment, &load);
            return Err(MoeTop2V1LoadErrorV1::KernelAdapter(error));
        }
    };
    if let Err(field) = validate_resolution(&load, &resolution) {
        drop(kernel);
        terminal_unload(&mut adapter, executable, &environment, &load);
        return Err(MoeTop2V1LoadErrorV1::KernelObservation(field));
    }

    let resources = match reviewed_adapter_call(|| unsafe {
        adapter.observe_moe_top2_v1_kernel_resources(&executable, &kernel)
    }) {
        Ok(value) => value,
        Err(error) => {
            drop(kernel);
            terminal_unload(&mut adapter, executable, &environment, &load);
            return Err(MoeTop2V1LoadErrorV1::ResourceAdapter(error));
        }
    };
    if let Err(field) = validate_resources(&load, &resolution, resources) {
        drop(kernel);
        terminal_unload(&mut adapter, executable, &environment, &load);
        return Err(MoeTop2V1LoadErrorV1::ResourceObservation(field));
    }
    ...
    if !(kernarg.bytes.as_ptr() as usize).is_multiple_of(HSA_KERNARG_ALIGNMENT as usize) {
        drop(kernel);
        terminal_unload(&mut adapter, executable, &environment, &load);
        return Err(MoeTop2V1LoadErrorV1::KernelObservation(
...
) -> Result<(), MoeTop2V1JoinErrorV1> {
    for (matches, field) in [
        (receipt.identity().as_bytes() != &[0; 32], "identity"),
        (
            receipt.structural_finalization_identity().as_bytes() != &[0; 32],
            "artifact identity",
        ),
        (
        ...
        (
            receipt.canonical_digest().as_bytes() != &[0; 32],
            "canonical digest",
        ),
        (
            receipt.exact_source_kir_compiler_profile_was_checked(),
            "source/KIR/compiler profile",
        ),
        (
            receipt.direct_upstream_llvm_lld_exchange_was_checked(),
            "upstream LLVM/LLD exchange",
        ),
    ] {
        if !matches {
            return Err(MoeTop2V1JoinErrorV1::ReceiptField(field));
        }
    }
    for (matches, field) in [
        (host.target() == TARGET, "target"),
        (host.profile() == [8, 4, 2, 4], "T/E/K/C profile"),
        (host.grid() == GRID, "grid"),
        ...
        (host.is_deterministic_routing_only(), "routing-only policy"),
    ] {
        if !matches {
            return Err(MoeTop2V1JoinErrorV1::HostField(field));
        }
    }
    Ok(())
}
...
fn terminal_unload<A: ReviewedMoeTop2V1RuntimeAdapterV1>(
    adapter: &mut A,
...
fn validate_resources(
    ...
    let expected = MoeTop2V1KernelResourceObservationV1::new(
        load.executable_object(),
        resolution.kernel_object(),
        STATIC_LDS_BYTES,
...
fn unload_identity(
    ...
    MoeTop2V1UnloadIdentityV1(digest.finalize().into())
}

#[cfg(test)]
#[path = "moe_top2_v1_lifecycle_tests.rs"]
mod tests;
