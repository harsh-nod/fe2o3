...
impl FakeRetained {
    fn new(drops: Arc<AtomicUsize>) -> Self {
        let bytes = b"exact-finalized-FlashAttention-v1".to_vec();
...
impl RetainedFlashAttentionV1 for FakeRetained {
...
unsafe impl ReviewedFlashAttentionV1RuntimeAdapterV1 for FakeAdapter {
    ...
    unsafe fn observe_flash_attention_v1_kernel_resources(
        ...
        Ok(FlashAttentionV1KernelResourceObservationV1::new(
            executable_observation,
            kernel_observation,
...
fn load(
    state: &FakeState,
    drops: Arc<AtomicUsize>,
) -> Result<LoadedState<FakeRetained, FakeAdapter>, FlashAttentionV1LoadErrorV1<FakeError>> {
...
fn load_error_names_the_hsa_context() {
    let error: FlashAttentionV1LoadErrorV1<FakeError> =
        FlashAttentionV1LoadErrorV1::ContextIdentity;
