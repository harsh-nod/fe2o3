use crate::{
    ...
    generated_wave64_collectives_v1_lifecycle::{
        CompletedWave64CollectivesV1, ReviewedWave64CollectivesRuntimeAdapterV1,
        Wave64CollectivesDispatchErrorV1, Wave64CollectivesKernelResourceObservationV1,
        Wave64CollectivesLoadErrorV1,
        test_support::{
            load_test_lifecycle_v1, test_complete_bytes_v1, test_explicit_bytes_v1,
            test_group_segment_v1, test_implicit_bytes_v1, test_runtime_alignment_v1,
        },
    },
};
...
#[allow(dead_code)]
fn completion_type_has_no_buffer_lifetimes<A: ReviewedWave64CollectivesRuntimeAdapterV1>(
    completed: CompletedWave64CollectivesV1<A>,
) -> CompletedWave64CollectivesV1<A> {
    completed
}
...
unsafe impl ReviewedHsaExecutableLifecycleAdapterV1 for FakeAdapter {
    ...
    unsafe fn launch_and_wait(
        ...
        self.aligned.store(
            (kernarg.as_ptr() as usize).is_multiple_of(test_runtime_alignment_v1() as usize),
            Ordering::SeqCst,
        );
        self.explicit_preserved.store(
            kernarg[..test_explicit_bytes_v1()]
                .iter()
                .copied()
                .eq((0..test_explicit_bytes_v1()).map(|index| index as u8)),
            Ordering::SeqCst,
        );
...
unsafe impl ReviewedWave64CollectivesRuntimeAdapterV1 for FakeAdapter {
    ...
    unsafe fn observe_wave64_collectives_resources_v1(
        ...
        Ok(Wave64CollectivesKernelResourceObservationV1::new(
            executable_observation,
            kernel_observation,
            if self.fault == Fault::ResourceGroup {
                test_group_segment_v1() + 1
            } else {
                test_group_segment_v1()
            },
...
fn successful_lifecycle_has_exact_order_alignment_spans_and_retention() {
    ...
    // Quiescent completion releases the admission and all four buffer leases;
    ...
    assert_eq!(test_explicit_bytes_v1(), 72);
    assert_eq!(test_implicit_bytes_v1(), 256);
    assert_eq!(test_complete_bytes_v1(), 328);
    assert_eq!(test_runtime_alignment_v1(), 16);
...
fn context_and_environment_rejections_create_no_native_authority() {
    ...
    assert!(matches!(
        ...
        Err(Wave64CollectivesLoadErrorV1::ContextIdentity)
    ));
    ...
    for fault in [
        ...
        match fault {
            Fault::ImplicitAdapter => assert!(matches!(
                error,
                Wave64CollectivesDispatchErrorV1::ImplicitAdapter(_)
            )),
            Fault::ExplicitMutation => assert!(matches!(
                error,
                Wave64CollectivesDispatchErrorV1::ExplicitKernargMutation
            )),
            Fault::DispatchAdapter => assert!(matches!(
                error,
                Wave64CollectivesDispatchErrorV1::DispatchAdapter(_)
            )),
            _ => {}
        }
        assert_eq!(state.unloads.load(Ordering::SeqCst), 1, "{fault:?}");
...
fn adapter_panic_and_unload_ambiguity_are_process_terminal() {
    const CHILD: &str = "FE2O3_WAVE64_COLLECTIVES_TERMINAL_CHILD";
    ...
    for mode in ["panic", "unload"] {
        let mut child = fe2o3_artifact_transaction::with_test_artifact_fork_exec_barrier_v1(|| {
            Command::new(std::env::current_exe().unwrap())
                .arg("--exact")
                .arg(
                    "generated_wave64_collectives_v1_lifecycle_tests::\
