//! Fixed per-occurrence MIR39 primitive descriptor. This is inert source data,
//! not an executable program or a source-authentication token.
use super::*;

pub const SEMANTIC_PHYSICAL_LDS_EXCHANGE_MAX_OCCURRENCES_V39: usize = 42;

/// Fixed source-owned static frame constants. Inert bytes do not authenticate a caller.
#[derive(Clone, Copy, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub struct SemanticPhysicalLdsExchangeFrameV39 {
    byte_offset: u32,
    byte_length: u32,
    alignment: u32,
    publication_epoch: u8,
}
impl SemanticPhysicalLdsExchangeFrameV39 {
    pub fn new(
        byte_offset: u32,
        byte_length: u32,
        alignment: u32,
        ...
        if (byte_offset, byte_length, alignment, publication_epoch) != (0, 512, 4, 1) {
...
#[derive(Clone, Copy, Debug, Eq, Ord, PartialEq, PartialOrd)]
pub struct SemanticPhysicalLdsExchangeInstructionV39 {
    descriptor: [u8; 8],
}
impl SemanticPhysicalLdsExchangeInstructionV39 {
    pub fn new(
        opcode: u8,
        destination: u8,
        source0: u8,
    ...
    pub fn from_descriptor(descriptor: [u8; 8]) -> Result<Self, SemanticMirErrorV1> {
        ...
        let valid = match op {
            ...
            1 | 9 | 14 | 19 => d == 0 && s0 == 0 && s1 == 0 && imm == 0,
            2 => sdst && s0 <= 63 && s1 == 0 && imm == 7,
            ...
            15 => vdst && s0 <= 63 && s1 == 0 && imm == 2,
            16 => vdst && s0 <= 63 && s1 == 0 && imm == 64,
            17 => d == 0 && s0 <= 63 && s1 <= 63 && imm == 0,
            18 => vdst && s0 <= 63 && s1 == 0 && imm == 0,
            _ => false,
        };
        if !valid {
            return Err(SemanticMirErrorV1::InvalidPhysicalLdsExchangeV39);
...
pub(super) fn uses_v39(request: &InertSemanticMirRequestV1) -> bool {
...
pub(super) fn validate_call_source(
    ...
    let physical = matches!(callee,Some(SemanticCallableDeclV1::CompilerIntrinsic{operation,..})
        if is_physical(*operation));
    if !physical && call.physical_lds_exchange_source_v39().is_none() {
        return Ok(());
    }
    let refuse = || SemanticMirErrorV1::InvalidPhysicalLdsExchangeV39;
    ...
    let source = call.physical_lds_exchange_source_v39().ok_or_else(refuse)?;
    let count = if matches!(
        operation,
        SemanticCompilerIntrinsicOperationV1::Gfx942PhysicalLdsExchangeBegin(_)
    ) {
...
pub(super) fn encode_operation(
    ...
    if version != SemanticMirWireVersionV1::V39 {
    ...
    match operation {
        SemanticCompilerIntrinsicOperationV1::Gfx942PhysicalLdsExchangeBegin(frame) => {
            writer.u8(99)?;
            writer.u8(0)?;
            writer.u32(frame.byte_offset())?;
            writer.u32(frame.byte_length())?;
            writer.u32(frame.alignment())?;
            writer.u8(frame.publication_epoch())
        }
        SemanticCompilerIntrinsicOperationV1::Gfx942PhysicalLdsExchangeLabel(0) => {
            writer.u8(100)?;
            writer.u8(0)?;
            writer.u8(0)
        }
        SemanticCompilerIntrinsicOperationV1::Gfx942PhysicalLdsExchangeStep(step) => {
            SemanticPhysicalLdsExchangeInstructionV39::from_descriptor(step.descriptor())?;
            writer.u8(101)?;
            writer.u8(0)?;
        ...
        _ => Err(SemanticMirErrorV1::InvalidPhysicalLdsExchangeV39),
    }
}
pub(super) fn encode_source(
    writer: &mut CanonicalWriterV1,
    source: Option<SemanticPhysicalLdsExchangeSourceV39>,
) -> Result<(), SemanticMirErrorV1> {
