//! Fixed source-call record for the exact MIR38 grammar.
...
pub struct SemanticPhysicalGlobalCopySourceV38 {
    root_axes: [[u8; 32]; 5],
    mir_body: [u8; 32],
...
impl SemanticPhysicalGlobalCopySourceV38 {
    ...
    pub fn new(
        ...
        if usize::from(occurrence)
            >= super::physical_global_copy_v38::SEMANTIC_PHYSICAL_GLOBAL_COPY_MAX_OCCURRENCES_V38
        {
            return Err("physical-global-copy occurrence exceeds closed bound");
        }
        if root_axes
            .into_iter()
            .chain([
        ...
        {
            return Err("physical-global-copy source record has an empty identity");
        }
        Ok(Self {
            root_axes,
            mir_body,
