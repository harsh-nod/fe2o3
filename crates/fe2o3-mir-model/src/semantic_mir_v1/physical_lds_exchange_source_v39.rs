//! Fixed source-call record for the exact MIR39 grammar.
...
pub struct SemanticPhysicalLdsExchangeSourceV39 {
    root_axes: [[u8; 32]; 5],
    mir_body: [u8; 32],
...
impl SemanticPhysicalLdsExchangeSourceV39 {
    ...
    pub fn new(
        ...
        if usize::from(occurrence)
            >= super::physical_lds_exchange_v39::SEMANTIC_PHYSICAL_LDS_EXCHANGE_MAX_OCCURRENCES_V39
        {
            return Err("physical-lds-exchange occurrence exceeds closed bound");
        }
        if root_axes
            .into_iter()
            .chain([
        ...
        {
            return Err("physical-lds-exchange source record has an empty identity");
        }
        Ok(Self {
            root_axes,
            mir_body,
