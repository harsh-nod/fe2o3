...
mod physical_lds_exchange_source_v39;
...
pub use complete_body_source_vnext::{
...
pub use physical_lds_exchange_source_v39::SemanticPhysicalLdsExchangeSourceV39;
pub use physical_lds_exchange_v39::{
    SEMANTIC_PHYSICAL_LDS_EXCHANGE_MAX_OCCURRENCES_V39, SemanticPhysicalLdsExchangeFrameV39,
...
enum PhysicalSourceTailV1 {
    ...
    LdsExchangeV39(SemanticPhysicalLdsExchangeSourceV39),
    Mixed,
}
#[derive(Clone, Debug, Eq, PartialEq)]
...
impl SemanticDirectCallV1 {
    ...
    /// Attaches inert occurrence references; admission checks the exact caller and callee kind.
    pub fn with_inline_assembly_source_v30(
        mut self,
        source: SemanticInlineAssemblySourceV30,
    ) -> Self {
        self.inline_assembly_source_v30 = Some(source);
        self
    }

    pub const fn inline_assembly_source_v30(&self) -> Option<SemanticInlineAssemblySourceV30> {
        self.inline_assembly_source_v30
    }

    /// Attaches inert occurrence references; admission checks the exact caller and callee kind.
    pub fn with_ordered_region_source_v31(
        mut self,
        source: SemanticOrderedRegionSourceV31,
    ) -> Self {
        self.ordered_region_source_v31 = Some(source);
        self
    }
    ...
    /// Inert source observations only; actual source custody remains compiler-owned.
    pub fn with_complete_body_source_vnext(
        mut self,
        source: SemanticCompleteBodySourceVNext,
    ) -> Self {
        self.complete_body_source_vnext = Some(source);
        self
    }
    ...
    pub fn with_physical_lds_exchange_source_v39(
        mut self,
        source: SemanticPhysicalLdsExchangeSourceV39,
    ) -> Self {
        self.physical_source = Some(match self.physical_source {
            None | Some(PhysicalSourceTailV1::LdsExchangeV39(_)) => {
                PhysicalSourceTailV1::LdsExchangeV39(source)
            }
            _ => PhysicalSourceTailV1::Mixed,
        });
        self
    }
    pub const fn physical_lds_exchange_source_v39(
        &self,
    ) -> Option<SemanticPhysicalLdsExchangeSourceV39> {
        match self.physical_source {
            Some(PhysicalSourceTailV1::LdsExchangeV39(source)) => Some(source),
            _ => None,
        }
    }
    /// Inert observations only; decoded records do not grant source custody.
...
pub enum SemanticCompilerIntrinsicOperationV1 {
    ...
    /// Closed typed integer ISA semantics; authenticated occurrence references are per call.
    ...
    Gfx942PhysicalLdsExchangeBegin(SemanticPhysicalLdsExchangeFrameV39),
    Gfx942PhysicalLdsExchangeLabel(u8),
    Gfx942PhysicalLdsExchangeStep(SemanticPhysicalLdsExchangeInstructionV39),
...
impl InertSemanticMirRequestV1 {
    ...
    /// Admits the inert V31 ordered-region grammar, never V29 execution capabilities.
    pub fn admit_exact_v31(
        self,
        limits: SemanticMirLimitsV1,
    ) -> Result<AdmittedInertSemanticMirV1, SemanticMirErrorV1> {
    ...
    /// Exact inert body grammar. Structural admission is not source/execution authority.
    pub fn admit_exact_v36(
        self,
        limits: SemanticMirLimitsV1,
    ) -> Result<AdmittedInertSemanticMirV1, SemanticMirErrorV1> {
    ...
    pub fn admit_exact_v39(
        self,
        limits: SemanticMirLimitsV1,
    ) -> Result<AdmittedInertSemanticMirV1, SemanticMirErrorV1> {
        self.admit_for_wire_version(SemanticMirWireVersionV1::V39, limits)
    }
    /// Explicit-only two-slice source grammar; inert data is not source custody.
    ...
    pub fn admit_current_production(
        ...
        if physical_lds_exchange_v39::uses_v39(&self) {
...
impl fmt::Display for SemanticMirErrorV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::InvalidInlineAssemblyV30 => {
                formatter.write_str("typed gfx942 ISA contract or source occurrence is invalid")
            }
            ...
            Self::InvalidPhysicalLdsExchangeV39 => formatter.write_str(
                "invalid bounded physical-lds-exchange V39 primitive or source occurrence",
            ),
            ...
            Self::InvalidCompleteBodyV36 => {
                write!(
                    formatter,
                    "invalid bounded complete-body V36 grammar or source call"
                )
            }
    ...
    for (index, callable) in context.request.callables.iter().enumerate() {
        ...
        let (tag, binding) = match callable {
            ...
            SemanticCallableDeclV1::CompilerIntrinsic {
                ...
                if physical_lds_exchange_v39::is_physical(*operation) {
                    charge_validation_work(context, physical_lds_exchange_v39::VALIDATION_WORK)?;
                }
...
fn record_intrinsic_capability_claims(
    ...
    match operation {
        ...
        | SemanticCompilerIntrinsicOperationV1::Gfx942PhysicalLdsExchangeStep(_) => true,
...
fn compiler_intrinsic_signature_matches(
    ...
    if physical_lds_exchange_v39::is_physical(operation) {
        return physical_lds_exchange_v39::signature_matches(request, operation, abi);
    }
    if physical_global_copy_v38::is_physical(operation) {
    ...
    match operation {
        ...
        SemanticCompilerIntrinsicOperationV1::Gfx942InlineU32(assembly) => {
            inputs.len() == assembly.input_count()
                && inputs.iter().all(|input| *input == output)
        ...
        SemanticCompilerIntrinsicOperationV1::Gfx942CompleteBody(packed) => {
            complete_body_v36::validate_packing(packed).is_ok()
                && complete_body_v36::signature_matches(request, abi)
        }
        SemanticCompilerIntrinsicOperationV1::ThreadIndex(_)
        | SemanticCompilerIntrinsicOperationV1::WorkgroupIndex(_)
        | SemanticCompilerIntrinsicOperationV1::WorkgroupDimension(_)
...
fn validate_call(
    ...
    if call.physical_lds_exchange_source_v39().is_some()
        || matches!(context.request.callables.get(call.callee.0 as usize), Some(SemanticCallableDeclV1::CompilerIntrinsic { operation, .. }) if physical_lds_exchange_v39::is_physical(*operation))
    {
        charge_validation_work(context, physical_lds_exchange_v39::VALIDATION_WORK)?;
    }
    ...
    physical_lds_exchange_v39::validate_call_source(context.request, function, location, call)?;
    ...
    complete_body_v36::validate_call_source(context.request, function, location, call)?;
    ...
    {
        charge_validation_work(context, 64)?;
    }
    gfx942_ordered_region_v31::validate_call_source(context.request, function, call)?;
    gfx942_inline_v30::validate_call_source(context.request, function, call)?;
    if call.inline_assembly_source_v30.is_some() {
        charge_validation_work(context, 5)?;
    }
    context.totals.charge(
        SemanticMirResourceV1::CallArguments,
        call.arguments.len(),
...
fn minimum_wire_version_without_nominal(
    ...
    if physical_lds_exchange_v39::uses_v39(request) {
        return SemanticMirWireVersionV1::V39;
    }
...
fn encode_function(
    ...
    for block in &function.blocks {
        ...
        encode_terminator(writer, &block.terminator.kind, wire_version)?;
    }
    Ok(())
}
...
fn encode_compiler_intrinsic_operation(
    ...
    if physical_lds_exchange_v39::is_physical(operation) {
        return physical_lds_exchange_v39::encode_operation(writer, operation, wire_version);
    }
    if physical_global_copy_v38::is_physical(operation) {
