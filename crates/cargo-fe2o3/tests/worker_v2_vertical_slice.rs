...
impl OuterHarness {
    fn start(directory: &TestDirectory, config: Option<&Path>) -> Result<Self, Output> {
        let control = directory.0.join("target/vertical-control");
        ...
        // Debug fixtures are large enough that authentication and sealing can exceed
        // 30 seconds on a busy remote test host before fake Cargo publishes readiness.
