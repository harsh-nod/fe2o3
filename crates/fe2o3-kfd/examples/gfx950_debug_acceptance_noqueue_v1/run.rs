//! Sibling registration observer with fixed HOST rendezvous points.
//! The points do not attest debugger acceptance and cannot create a GPU stop.
...
#[path = "../gfx950_debug_noqueue_v1/isolation.rs"]
mod isolation;
use isolation::EntryFence;

const USAGE: &str = "observe_gfx950_debug_acceptance_noqueue_v1 --allow-vm-mapping --retain-until-process-exit --acknowledge-isolated-noqueue-activation --acknowledge-reviewed-host-debugger-control HSACO BYTES SHA256 KERNEL NODE UNIQUE_ID GPU_ID DEVICE_PROFILE_SHA256";
const SCHEMA: &str = "diagnostic-gfx950-debug-acceptance-noqueue-producer-v1";
const MAX_OBSERVED_ARTIFACT_BYTES: usize = 64 * 1024;
...
fn parse(arguments: impl Iterator<Item = OsString>) -> Result<Options, Failure> {
    ...
    for value in arguments {
        if values.len() == 12 || value.as_os_str().as_bytes().len() > MAX_ARGUMENT_BYTES {
            return Err(fail());
        }
    ...
    if bytes == 0
        || bytes > MAX_OBSERVED_ARTIFACT_BYTES
        || !path.is_absolute()
        || kernel.is_empty()
        || kernel.len() > 128
        || kernel
...
fn observe(options: Options, entry: EntryFence) -> Result<Observation, Failure> {
    ...
    // dependency closure, clean environment and exclusion of uncontrolled injection.
    ...
    // The supervised debugger may stop this HOST thread at the fixed marker.
    // No callback pointer or GPU execution capability is exported.
    rendezvous::fe2o3_gfx950_noqueue_pre_activation_v1();
    retained.recheck().map_err(Failure::possible)?;
    entry.recheck_prepared().map_err(Failure::possible)?;
    ...
    // The actual move-only owner remains live across the HOST rendezvous.
    // Its publication was acknowledged; this does not prove debugger acceptance.
    rendezvous::fe2o3_gfx950_noqueue_post_publication_v1();
    retained.recheck().map_err(Failure::possible)?;
    entry.recheck_prepared().map_err(Failure::possible)?;
    if owner.preparation_facts() != facts || owner.registered_metadata_version() != 11 {
        return Err(Failure::new("post_rendezvous_facts", "owner_identity_or_version").possible());
    }
    let result = Observation {
        ...
        host_process_entry_rendezvous_reached: true,
        host_pre_activation_rendezvous_reached: true,
        host_post_publication_rendezvous_reached: true,
...
pub fn main() -> ExitCode {
    // The controller must stop and establish launch-owned process identity here,
    // before this observer can acquire any GPU FD, VM or native resource.
    // A reached marker is not itself ownership or debugger acceptance.
    rendezvous::fe2o3_gfx950_noqueue_process_entry_v1();
    ...
    let value = match result {
        Ok(observation) => serde_json::json!({
            "schema": SCHEMA, "status": "registered_no_queue_host_rendezvous", "observation": observation,
        }),
