...
pub const GFX942_DEVICE_MEMORY_INITIALIZATION_MANIFEST_V1: &str = concat!(
    "profile=fe2o3-mi300x-gfx942-device-memory-initialization-r5-v1\n",
...
pub const GFX942_DEVICE_MEMORY_INITIALIZATION_MANIFEST_SHA256_V1: &str =
    "cee13384c9216974d0ffa4cdcb223cdca4bb92dff17718e1a7808082e00d284e";
...
impl<B: MemoryBackend> SharedMemoryEngine<B> {
    ...
    fn observe_i64_acquire_in_current_scope<P: MutableGpuGttProfileV1>(
        &mut self,
        token: &mut SharedGttAllocationV1<P, GttGpuAccessibleMutableV1>,
        offset: usize,
    ) -> Result<i64, MemorySessionError> {
        ...
        let mapping = self.allocations[index]
            ...
            .as_mut()
            .ok_or(MemorySessionError::InvalidAllocationAuthority)?;
        B::observe_i64_acquire(mapping, requested, offset)
    }
...
impl SharedGttMemorySessionV1 {
    ...
    /// from this allocation's owned mapping. Source validation consumes the
    /// owned bytes, preventing safe mutation or substitution before the mapped
    /// copy. The ordinary uninitialized device-local path remains a distinct
    /// API and flag profile.
    ...
    pub(crate) fn gpu_id(&self) -> u32 {
        self.engine.backend.gpu_id()
    }
    ...
    pub(crate) fn mapped_gfx942_device_memory_facts(
        &self,
        lease: &Gfx942DeviceMemoryLeaseV1<Gfx942DeviceMemoryMappedV1>,
    ) -> Result<Gfx942DeviceMemoryDispatchFactsV1, MemorySessionError> {
        let index = self
    ...
    pub(crate) fn overwrite_mapped_host_visible_subrange_in_current_scope(
        ...
        self.engine
            .overwrite_mapped_host_visible_subrange_in_current_scope(token, offset, source)
    }

    pub(crate) fn copy_mapped_host_visible_subrange(
        &mut self,
        token: &SharedGttAllocationV1<HostVisibleCoherentGttV1, GttGpuAccessibleMutableV1>,
        offset: u64,
        byte_len: u64,
    ...
    pub(crate) fn observe_mapped_host_visible_i64_at_in_current_scope(
        &mut self,
        token: &mut SharedGttAllocationV1<HostVisibleCoherentGttV1, GttGpuAccessibleMutableV1>,
        offset: u64,
    ) -> Result<i64, MemorySessionError> {
...
mod tests {
    ...
    fn shared_slot_reuse_still_rejects_collision_with_a_later_live_record() {
        ...
        engine
            ...
            .unwrap();
        let later_live_va =
            engine.allocations[engine.allocation_record_slots[&tokens[0].id]].gpu_va;
    ...
    #[test]
    fn arbitrary_source_has_one_preflight_hash_and_retains_mapped_readback_hash() {
        let source = include_str!("shared_memory.rs");
        ...
        let mapped_preflight = source
            ...
            .unwrap();
        assert_eq!(
            mapped_preflight.matches("Sha256::digest(mapped)").count(),
            1
        );
