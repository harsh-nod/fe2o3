...
const ROLE: CapabilityRole = CapabilityRole {
    name: "compiler-execution external-anchor deployment capability",
    memfd_name: "fe2o3-compiler-execution-external-anchor-deployment-v1",
};
const LENGTH: ImageLength =
    ImageLength::Exact(COMPILER_EXECUTION_EXTERNAL_ANCHOR_DEPLOYMENT_BYTES_V1);

/// Reserved descriptor carrying trusted deployment configuration into the external anchor.
pub const COMPILER_EXECUTION_EXTERNAL_ANCHOR_DEPLOYMENT_FD_V1: RawFd = 221;

/// Move-only immutable descriptor capability for one external-anchor deployment manifest.
///
/// ```compile_fail
/// use fe2o3_compiler_closure_capability::CompilerExecutionExternalAnchorDeploymentCapabilityV1;
/// fn require_clone<T: Clone>() {}
/// require_clone::<CompilerExecutionExternalAnchorDeploymentCapabilityV1>();
/// ```
///
/// ```compile_fail
/// use std::os::fd::AsFd;
...
/// require_as_fd::<CompilerExecutionExternalAnchorDeploymentCapabilityV1>();
/// ```
pub struct CompilerExecutionExternalAnchorDeploymentCapabilityV1 {
    deployment: CompilerExecutionExternalAnchorDeploymentV1,
    image: SealedCapabilityImage,
}

impl CompilerExecutionExternalAnchorDeploymentCapabilityV1 {
    pub fn create(deployment: CompilerExecutionExternalAnchorDeploymentV1) -> Result<Self, String> {
    ...
    pub fn from_inherited() -> Result<Self, String> {
        Self::from_inherited_at(COMPILER_EXECUTION_EXTERNAL_ANCHOR_DEPLOYMENT_FD_V1)
    }
    ...
    pub fn revalidate(&self) -> Result<(), String> {
        if decode(&self.image.read_exact_bytes()?)? != self.deployment {
            return Err("compiler-execution external-anchor deployment capability changed".into());
        }
        Ok(())
    }
...
fn decode(bytes: &[u8]) -> Result<CompilerExecutionExternalAnchorDeploymentV1, String> {
    CompilerExecutionExternalAnchorDeploymentV1::decode(bytes).map_err(|error| {
        format!("compiler-execution external-anchor deployment is not canonical: {error}")
    })
}
...
mod tests {
    ...
    fn exact_deployment_is_sealed_transferred_and_revalidated() {
        ...
        let capability =
            CompilerExecutionExternalAnchorDeploymentCapabilityV1::create(expected.clone())
                .unwrap();
        let recovered = CompilerExecutionExternalAnchorDeploymentCapabilityV1::from_file(
    ...
    fn mutable_ordinary_file_is_rejected() {
        let path = std::env::temp_dir().join(format!(
            "fe2o3-external-anchor-deployment-capability-hostile-{}",
            std::process::id()
        ));
        fs::write(&path, deployment().canonical_bytes()).unwrap();
        fs::set_permissions(&path, fs::Permissions::from_mode(0o400)).unwrap();
        assert!(
            CompilerExecutionExternalAnchorDeploymentCapabilityV1::from_file(
    ...
    #[test]
    fn inherited_deployment_is_retained_privately() {
        ...
        let retained =
            CompilerExecutionExternalAnchorDeploymentCapabilityV1::from_inherited().unwrap();
        drop(installed);
