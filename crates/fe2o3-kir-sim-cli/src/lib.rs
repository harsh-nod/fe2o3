...
/// Executes an already captured exact canonical KIR V6 image with the same
...
pub fn run_captured_kir_v6(
    ...
    #[cfg(not(target_os = "linux"))]
    {
        let _ = (canonical_kir_v6, request, output);
        unsupported_platform()
    }
}
...
pub fn run_captured_kir_v6_with_bound_request(
    canonical_kir_v6: &[u8],
    request: &Path,
    ...
    #[cfg(not(target_os = "linux"))]
    {
        let _ = (canonical_kir_v6, request, expected_request, output);
        unsupported_platform()
    }
}
