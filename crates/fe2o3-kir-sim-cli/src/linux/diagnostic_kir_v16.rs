//! Exact diagnostic V16 ingress, shared by the simulator and headless debugger.
...
use fe2o3_kernel_ir::{
    CanonicalKernelIrReplayAdmissionErrorV16, CanonicalKernelIrVerificationResourceBudgetV1,
    CanonicalKernelIrVerificationResourceErrorV1, CanonicalKernelIrWorkBudgetV1,
    KernelIrDecodeError, KernelIrEncodeError, VerifiedCanonicalKernelIrModuleV16,
};

use super::*;

const MAX_CANONICAL_WORK_V16: usize = 1 << 27;
const MAX_CANONICAL_STORAGE_V16: usize = 256 * 1024 * 1024;

pub(crate) fn load_debug_simulation_input_v16(
    kir: &Path,
    request: &Path,
) -> Result<crate::AdmittedSimulationInputV1, crate::SimulationInputErrorV1> {
    load_admitted_kir_v16(kir, request).map_err(input_error)
}

pub(crate) fn load_debug_simulation_input_bytes_v16(
    kir: &[u8],
    request: &[u8],
) -> Result<crate::AdmittedSimulationInputV1, crate::SimulationInputErrorV1> {
    let result = (|| {
...
pub(super) fn load_admitted_kir_v16(
    kir: &Path,
    request: &Path,
...
fn load_bytes(
    ...
    let mut work = CanonicalKernelIrWorkBudgetV1::new(MAX_CANONICAL_WORK_V16);
    let mut budget =
        CanonicalKernelIrVerificationResourceBudgetV1::new(&mut work, MAX_CANONICAL_STORAGE_V16);
...
fn admit_with_budget(
    ...
    let result =
        (|| {
            ...
            let (canonical, storage) = VerifiedCanonicalKernelIrModuleV16::
            from_canonical_bytes_with_verification_budget_v16(kir, budget)
            ...
            // admit_v16 bounds decode by wire limits and checks CPU resident storage
            // afterwards; CPU allocations are NOT charged to this canonical ledger.
            let admitted =
                AdmittedSimulationModuleV1::admit_v16(&canonical, limits).map_err(|error| {
                    Failure::new(
            ...
            if admitted.identity().wire_version() != 16
                ...
                || admitted.identity().canonical_length() != canonical.identity().canonical_length()
            {
                return Err(kir_failure(
                    ErrorKind::KirV16IdentityMismatch,
                    "diagnostic V16 identity changed during CPU admission",
                ));
            }
            drop(canonical);
            budget
...
fn kir_failure(kind: ErrorKind, message: impl Into<String>) -> Failure {
    ...
    failure.0.input = Some(InputCode::KirV16);
    failure
}
...
fn resource_failure(error: CanonicalKernelIrVerificationResourceErrorV1) -> Failure {
    ...
    let kind = match error {
        E::Work(_) => ErrorKind::KirV16WorkLimit,
        E::Storage(_) => ErrorKind::KirV16StorageLimit,
        E::Allocation => ErrorKind::KirV16AllocationFailed,
        E::Accounting => ErrorKind::KirV16ResourceAccounting,
        E::Arithmetic => ErrorKind::KirV16ResourceArithmetic,
    };
    kir_failure(kind, bounded_display(&error))
}

fn canonical_failure(error: CanonicalKernelIrReplayAdmissionErrorV16) -> Failure {
    use CanonicalKernelIrReplayAdmissionErrorV16 as E;
    let kind = match &error {
        E::Resource(error) | E::Decode(KernelIrDecodeError::Resource(error)) => {
            return resource_failure(*error);
        }
        E::Decode(KernelIrDecodeError::WorkLimit(_)) => ErrorKind::KirV16WorkLimit,
        E::Decode(KernelIrDecodeError::Encode(error)) | E::Encode(error) => match error {
            KernelIrEncodeError::WorkLimit(_) => ErrorKind::KirV16WorkLimit,
            KernelIrEncodeError::Allocation => ErrorKind::KirV16AllocationFailed,
            KernelIrEncodeError::Overflow { .. } => ErrorKind::KirV16ResourceArithmetic,
            KernelIrEncodeError::NonCanonical { .. } => ErrorKind::KirV16RoundTripMismatch,
            _ => ErrorKind::KirV16EncodeFailed,
        },
        E::Decode(KernelIrDecodeError::UnknownVersion(_)) => ErrorKind::KirV16WrongVersion,
        E::Decode(KernelIrDecodeError::NonCanonical) | E::CanonicalMismatch => {
            ErrorKind::KirV16RoundTripMismatch
        }
        E::Decode(_) => ErrorKind::KirV16DecodeFailed,
        E::Verification(_) => ErrorKind::KirV16VerificationFailed,
    };
    kir_failure(kind, bounded_display(&error))
}
