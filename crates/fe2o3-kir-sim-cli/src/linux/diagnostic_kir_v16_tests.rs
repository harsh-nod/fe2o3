use super::*;
use fe2o3_kernel_ir::{Module, OperationKind, ScalarType, Type, encode_module_v16};
use std::fs;
...
fn exact_owner_identity_request_identity_and_oracle_survive_shared_ingress() {
    for used in [true, false] {
        ...
        for inputs in [
            ...
            let input =
                load_debug_simulation_input_bytes_v16(owner.canonical_bytes(), &request).unwrap();
            assert_eq!(input.module.identity().wire_version(), 16);
...
fn independently_counted_canonical_budget_restores_nonzero_floor() {
    let bytes = encode_module_v16(&Module::new("m")).unwrap();
    assert_eq!(bytes.len(), 37);
    // Existing exact V16 byte-owner formula: work233, owner inline + bytes37
    // + one module-name byte. This helper adds input extent37 and caller floor7.
    let peak = 7 + 37 + size_of::<VerifiedCanonicalKernelIrModuleV16>() + 37 + 1;
    for (work_limit, storage_limit, expected) in [
        (233, peak, None),
        (232, peak, Some(ErrorKind::KirV16WorkLimit)),
        (0, peak, Some(ErrorKind::KirV16WorkLimit)),
        (233, peak - 1, Some(ErrorKind::KirV16StorageLimit)),
    ] {
        ...
        match expected {
            None => {
                assert_eq!(result.unwrap().identity().wire_version(), 16);
...
fn input_capacity_and_prior_failure_are_not_lost_when_restoring_floor() {
    ...
    assert_eq!(error.0.kind, ErrorKind::KirV16StorageLimit);
    assert_eq!(budget.storage(), 9);
    ...
    .unwrap_err();
    assert_eq!(error.0.kind, ErrorKind::KirV16ResourceAccounting);
    assert_eq!(budget.storage(), 9);
    assert_eq!(arithmetic().0.kind, ErrorKind::KirV16ResourceArithmetic);
}
...
fn exact_version_payload_and_semantic_rejections_never_fall_back() {
    ...
    for version in [1_u16, 7, 9, 10, 11, 12, 13, 14, 15, 17] {
        let mut bytes = owner.canonical_bytes().to_vec();
        bytes[8..10].copy_from_slice(&version.to_le_bytes());
        assert_eq!(
            load_debug_simulation_input_bytes_v16(&bytes, &request)
                .unwrap_err()
                .code,
    ...
    let mut bytes = owner.canonical_bytes().to_vec();
    bytes.push(0);
    assert!(load_debug_simulation_input_bytes_v16(&bytes, &request).is_err());
    assert!(load_debug_simulation_input_bytes_v16(&bytes[..10], &request).is_err());
    ...
    let malformed = encode_module_v16(&module).unwrap();
    assert_eq!(
        load_debug_simulation_input_bytes_v16(&malformed, &request)
            .unwrap_err()
            .code,
        "kir_v16_verification_failed"
    );
}
...
fn both_byte_caps_apply_before_decode_and_request_json_remains_strict() {
    ...
    assert_eq!(
        load_debug_simulation_input_bytes_v16(&[], &oversized)
            .unwrap_err()
            .code,
        "input_too_large"
    );
    ...
    for request in [
        ...
        let error =
            load_debug_simulation_input_bytes_v16(owner.canonical_bytes(), request).unwrap_err();
...
fn target_launch_and_argument_refusals_still_come_from_cpu_preflight() {
    ...
    let input = load_debug_simulation_input_bytes_v16(wrong.canonical_bytes(), &request).unwrap();
    assert!(
        input
            .module
            .preflight(
...
fn programmatic_schedule_paths_refuse_before_open_or_publication_without_panicking() {
    ...
    let owner = fixture::owner(&fixture::module(true));
    let request = fixture::request([7, 11, 13]);
    let input = load_debug_simulation_input_bytes_v16(owner.canonical_bytes(), &request).unwrap();
    assert_eq!(
...
fn filesystem_ingress_reuses_regular_no_symlink_and_stable_capture_checks() {
    ...
    let input = crate::load_debug_simulation_input_v16(&kir, &request).unwrap();
    assert_eq!(input.module.identity().digest(), owner.identity().digest());
    let link = directory.path("link");
    symlink(&kir, &link).unwrap();
    assert_eq!(
        crate::load_debug_simulation_input_v16(&link, &request)
            .unwrap_err()
            .code,
        "input_open_failed"
    );
    ...
    assert_eq!(
        crate::load_debug_simulation_input_v16(&ancestor.join("program.kir"), &request)
            .unwrap_err()
            .code,
        "input_open_failed"
    );
    assert_eq!(
        crate::load_debug_simulation_input_v16(&directory.0, &request)
            .unwrap_err()
            .code,
        "input_not_regular"
    );
    ...
    assert_eq!(
        crate::load_debug_simulation_input_v16(&fifo, &request)
            .unwrap_err()
            .code,
        "input_not_regular"
    );
    ...
    File::create(&oversized)
        ...
        .unwrap();
    assert_eq!(
        crate::load_debug_simulation_input_v16(&oversized, &request)
            .unwrap_err()
            .code,
        "input_too_large"
    );
