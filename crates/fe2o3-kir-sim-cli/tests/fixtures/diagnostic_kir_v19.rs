...
pub(crate) fn owner(module: &Module) -> fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV19 {
    let mut work = fe2o3_kernel_ir::CanonicalKernelIrWorkBudgetV1::new(1 << 27);
    let mut budget = fe2o3_kernel_ir::CanonicalKernelIrVerificationResourceBudgetV1::new(
        &mut work,
        256 * 1024 * 1024,
    );
    let (owner, _) = fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV19::from_module_ref_with_verification_budget_v19(module, &mut budget).unwrap();
    owner
}
pub(crate) fn request(selector: u32) -> Vec<u8> {
    let mut arguments = vec![serde_json::json!({
        "kind": "buffer", "element": "u32", "access": "read_write", "alignment": 4,
        "bytes": format!("0x{}", "5a".repeat(66 * 4)),
    })];
    arguments.extend([19, 23, 42, selector].map(|value| {
        serde_json::json!({
            "kind": "scalar", "type": "u32", "bits": format!("0x{value:08x}"),
        })
    }));
    serde_json::to_vec(&serde_json::json!({
        "schema": "fe2o3-simulation-request-v1", "kernel": "synthetic_kernel",
        "grid": [64, 1, 1], "workgroup": [64, 1, 1], "arguments": arguments,
    }))
    .unwrap()
}
pub(crate) fn output(value: u32) -> Vec<u8> {
    let mut bytes = value.to_le_bytes().repeat(64);
