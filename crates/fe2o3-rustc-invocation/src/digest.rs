...
use crate::{DigestError, RustcInvocationDescriptorV1, encode_descriptor_v1};

/// Domain separator for V1 rustc build-invocation coordination identities.
///
/// The terminating NUL is part of the normative byte sequence.
pub const INVOCATION_DIGEST_DOMAIN_V1: &[u8] = b"FE2O3/RUSTC-BUILD-INVOCATION/V1\0";

/// A nonzero SHA-256 coordination identity for one canonical V1 descriptor.
///
...
impl InvocationDigest {
    /// Calculates the V1 domain-separated digest of a descriptor.
    pub fn calculate(descriptor: &RustcInvocationDescriptorV1) -> Result<Self, DigestError> {
        let encoded = encode_descriptor_v1(descriptor)?;
        let mut hasher = Sha256::new();
        hasher.update(INVOCATION_DIGEST_DOMAIN_V1);
...
impl fmt::Display for InvocationDigest {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(&self.to_hex())
    }
}

impl FromStr for InvocationDigest {
