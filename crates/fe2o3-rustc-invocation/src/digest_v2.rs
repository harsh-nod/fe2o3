...
use crate::{DigestError, RustcInvocationDescriptorV2, encode_descriptor_v2};

/// Domain separator for V2 rustc build-invocation coordination identities.
///
/// The terminating NUL is part of the normative byte sequence.
pub const INVOCATION_DIGEST_DOMAIN_V2: &[u8] = b"FE2O3/RUSTC-BUILD-INVOCATION/V2\0";

/// A nonzero SHA-256 coordination identity for one canonical V2 descriptor.
///
/// This digest is not artifact evidence or launch authority.
#[derive(Clone, Copy, Debug, Eq, Hash, Ord, PartialEq, PartialOrd)]
#[repr(transparent)]
pub struct InvocationDigestV2([u8; 32]);

impl InvocationDigestV2 {
    /// Calculates the V2 domain-separated digest of a descriptor.
    pub fn calculate(descriptor: &RustcInvocationDescriptorV2) -> Result<Self, DigestError> {
        let encoded = encode_descriptor_v2(descriptor)?;
        let mut hasher = Sha256::new();
        hasher.update(INVOCATION_DIGEST_DOMAIN_V2);
...
impl fmt::Display for InvocationDigestV2 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(&self.to_hex())
    }
}

impl FromStr for InvocationDigestV2 {
