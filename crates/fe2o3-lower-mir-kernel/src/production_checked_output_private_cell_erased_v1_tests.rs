...
fn prefix8(roots: usize) -> (crate::ProductionOwnedUnitLocalCommutativeContinuationV1, usize) {
...
fn private_cell_unit_local_keeps_actual_n_e_p8_and_all_external_effects_for_each_root() {
    ...
    for roots in [1, 2] {
        ...
        assert_eq!(owner.additional_retained_storage_v1(), added.retained_storage());
        budget.reserve_storage(added.retained_storage()).unwrap();
        assert_eq!(owner.retained_input_storage_floor_v1().unwrap(), minimum + added.retained_storage());
        assert_eq!(original, pointer(owner.prefix().prefix().prefix().original_source().executable()));
        assert_eq!(erased, pointer(owner.prefix().prefix().prefix().erased()));
        assert_eq!(actual_k, pointer(owner.prefix().output()));
        assert_ne!(owner.prefix().prefix().prefix().original_source().executable().canonical().canonical_bytes(),
            owner.prefix().prefix().prefix().erased().canonical().canonical_bytes());
        assert!(owner.continuation().selected_allocations().len() >= roots * 2);
        assert_ne!(owner.output().canonical().canonical_bytes(), owner.prefix().output().canonical().canonical_bytes());
...
fn private_cell_unit_local_exact_short_and_unreserved_result_limits_abort_without_fallback() {
    ...
    assert!(matches!(owner.verify_equivalence(&mut short),
