impl<'a> ProductionCanonicalCallsV1<'a> {
    fn build(
        ...
        let (groups, calls) =
            build_canonical_call_index_v1(CanonicalCallSubjectV1::owner(owner), inventory, budget)?;
...
fn build_canonical_call_index_v1<'a>(
    subject: CanonicalCallSubjectV1<'a>,
    inventory: &'a CanonicalKirInventoryV1<'a>,
    budget: &mut ArgumentBudgetV1<'_>,
) -> Result<
    (
        Vec<CanonicalCallGroupV1<'a>>,
        Vec<CanonicalCallBindingV1<'a>>,
    ),
    ProductionSemanticKirErrorV1,
> {
    let rows = subject.correspondence;
    ...
    budget.reserve_storage(argument_sum_v1(&[
        std::mem::size_of::<ProductionCanonicalCallsV1<'_>>(),
        argument_product_v1(
    ...
    let targets =
        CallTargetIndexV1::new(subject.executable.module(), &rows.lowered_functions, budget)?;
    ...
    for group in &groups {
        ...
        for index in group.function.canonical.calls.clone() {
            ...
            calls.push(bind_canonical_call_v1(
                subject, inventory, &groups, &targets, group, index, budget,
            )?);
        }
