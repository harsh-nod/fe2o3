...
impl<'a> ExecutionAvailabilityV29<'a> {
    fn new(
        ...
        let occurrences = instances
            ...
            .ok_or_else(execution_availability_error_v29)?;
        let mut index = borrowed_aggregate_vec_v1(occurrences.events().len(), budget)?;
        ...
        let mut claimed = borrowed_aggregate_vec_v1(index.len(), budget)?;
        let mut current = borrowed_aggregate_vec_v1(row.declaration().locals().len(), budget)?;
        let mut seen = borrowed_aggregate_vec_v1(row.declaration().locals().len(), budget)?;
        let mut visited = borrowed_aggregate_vec_v1(row.declaration().blocks().len(), budget)?;
