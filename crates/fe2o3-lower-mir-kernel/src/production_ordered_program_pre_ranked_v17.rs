// Additive exact-V17 pre-ranked custody. Existing V12 materialized/Policy3 and
// Policy4 consumers are intentionally unchanged and cannot borrow this as V12.

#[cfg(test)]
#[path = "production_ordered_program_pre_ranked_v17_tests.rs"]
mod ordered_program_pre_ranked_v17_tests;

/// Failure of closed-region context, ordinary lowering, or exact V17 custody.
#[derive(Debug)]
pub enum ProductionOrderedProgramPreRankedErrorV17 {
...
impl fmt::Display for ProductionOrderedProgramPreRankedErrorV17 {
...
impl Error for ProductionOrderedProgramPreRankedErrorV17 {}
impl From<ProductionSemanticKirErrorV1> for ProductionOrderedProgramPreRankedErrorV17 {
    fn from(error: ProductionSemanticKirErrorV1) -> Self {
        Self::Lowering(error)
    }
}
impl From<ArgumentResourceV1> for ProductionOrderedProgramPreRankedErrorV17 {
    fn from(error: ArgumentResourceV1) -> Self {
        Self::Lowering(error.into())
    }
}

/// One normal semantic-to-KIR materialization for the closed V32 source profile.
///
/// The sole executable Module is freshly decoded and verified under V17. Source
...
///
/// ```compile_fail
/// use fe2o3_lower_mir_kernel::ProductionOrderedProgramPreRankedKirOwnerV17;
/// fn cloned<T: Clone>() {}
/// cloned::<ProductionOrderedProgramPreRankedKirOwnerV17>();
/// ```
/// ```compile_fail
/// use fe2o3_lower_mir_kernel::ProductionOrderedProgramPreRankedKirOwnerV17;
/// fn mutate(owner: &mut ProductionOrderedProgramPreRankedKirOwnerV17) {
///     owner.executable().module().functions.clear();
/// }
/// ```
#[derive(Debug)]
#[must_use = "dropping this owner abandons source/executable correspondence"]
pub struct ProductionOrderedProgramPreRankedKirOwnerV17 {
    semantic_ssa: ProductionSemanticSsaOwnerV1,
    source_launch: crate::ProductionSourceLaunchRosterV1,
    executable: fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV17,
    executable_storage: fe2o3_kernel_ir::CanonicalKernelIrReplayStorageV17,
...
impl ProductionOrderedProgramPreRankedKirOwnerV17 {
    ...
    /// the full allocation-budgeted V17 inverse. The cumulative ledger covers
    ...
    ) -> Result<Self, ProductionOrderedProgramPreRankedErrorV17> {
        let floor = budget.storage();
        let result = (|| {
            validate_ordered_program_context_v32(
            ...
            let (executable, executable_storage) =
                fe2o3_kernel_ir::VerifiedCanonicalKernelIrModuleV17::
                    from_module_ref_with_verification_budget_v17(&module, budget)
                    .map_err(ProductionOrderedProgramPreRankedErrorV17::Canonical)?;
    ...
    pub const fn executable_storage(&self) -> fe2o3_kernel_ir::CanonicalKernelIrReplayStorageV17 {
...
fn validate_ordered_program_context_v32(
    ...
    let function = semantic
        ...
        .ok_or_else(|| fail("ordered program root is out of range"))?;
    budget.charge_work(16)?;
    let source_bounds = function
        .kernel_entry()
        .and_then(|entry| entry.source_contract().launch());
    if !source_bounds.is_some_and(|bounds| {
        bounds.required().map(|size| size.as_array()) == Some([64, 1, 1])
            && bounds.maximum().map(|size| size.as_array()) == Some([64, 1, 1])
    }) {
        return Err(fail(
            "ordered program source requires exact required and maximum 64x1x1 bounds",
        ));
    }
    ...
    {
        return Err(fail(
            "ordered program requires bounded direct-root wave64/full64 launch geometry",
        ));
    }
    ...
    for (index, block) in function.blocks().iter().enumerate() {
        ...
        {
            // Fixed precharge covers the eight operand/ABI checks, source and
            // binding shape, and all sixteen descriptor/padding/init checks.
            ...
            semantic
                ...
                .map_err(|_| {
                    fail("ordered program call no longer matches the immutable semantic owner")
                })?;
            if selected.replace(index as usize).is_some() {
                return Err(fail(
                    "ordered program profile admits exactly one occurrence",
                ));
            }
        }
    }
    let selected = selected.ok_or_else(|| fail("ordered program profile has no occurrence"))?;
    let floor = budget.storage();
    let result = (|| {
        ...
        loop {
            ...
            let slot = prefix
                .get_mut(current)
                .ok_or_else(|| fail("ordered program prefix leaves the CFG"))?;
            if *slot != usize::MAX {
                return Err(fail("ordered program prefix is cyclic"));
            }
            *slot = ordinal;
            ...
            for statement in block.statements() {
                ...
                match statement.kind() {
                    SemanticStatementKindV1::Assign(assign)
                        if ordered_program_scalar_prefix_assignment_v32(
                            semantic.types(),
                            assign,
                        ) => {}
                    ...
                    _ => {
                        return Err(fail(
                            "ordered program prefix contains a memory/control effect",
                        ));
                    }
                }
            }
            ...
            let edge = match block.terminator().kind() {
                    ...
                    ) =>
                {
                    ...
                    if !destination.place().projections().is_empty()
                        ...
                        || call.arguments().iter().any(|operand| {
                            !ordered_program_direct_scalar_operand_v32(semantic.types(), operand)
                        })
                    {
                        return Err(fail(
                ...
                _ => {
                    return Err(fail(
                        "ordered program is not on an unconditional entry prefix",
                    ));
                }
            };
        ...
        for (index, block) in function.blocks().iter().enumerate() {
            ...
            block.terminator().kind().try_for_each_edge(|edge| {
                ...
                let destination = *prefix
                    ...
                    .ok_or_else(|| fail("ordered program edge leaves the CFG"))?;
                ...
                {
                    return Err(fail(
                        "ordered program prefix has an alternate entry or backedge",
                    ));
                }
                Ok(())
            })?;
...
fn ordered_program_scalar_prefix_assignment_v32(
    ...
    let operand =
        |value: &SemanticOperandV1| ordered_program_direct_scalar_operand_v32(types, value);
...
fn ordered_program_direct_scalar_operand_v32(
    types: &[SemanticTypeDeclV1],
