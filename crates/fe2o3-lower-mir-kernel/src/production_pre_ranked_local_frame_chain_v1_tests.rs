...
fn edge_copy_work_has_derived_partial_binding_and_final_boundaries() {
    with_physical_component(edge_component(), |executable, inventory, effects| {
        // Core: header/entry16 + signature1 + census24 + reserves14 + publish35
        // + definition sort/window103 + block sort24 + visits/lookups38 + epochs35
        // + operations153 + dispatch12 + simultaneous edges83 + return7
        // + block census3 + cells34 = 582. Eight definition keys, three blocks.
        const CORE: usize = 16 + 1 + 24 + 14 + 35 + 103 + 24 + 38 + 35 + 153 + 12 + 83 + 7 + 3 + 34;
        const EXACT: usize = 35 + 12 + 1 + CORE + 12 + 26 + 3 + 8 + 9 + 12 + 5;
        assert_eq!((CORE, EXACT), (582, 705));
        for limit in [699, EXACT - 1, EXACT] {
...
fn new_producer_queries_preserve_exact_guard_and_census_work_boundaries() {
    with_physical_component(local_component(false), |executable, _, _| {
        for (control, exact) in [(true, 286), (false, 285)] {
            for limit in [exact - 1, exact] {
                ...
                if limit == exact {
                    ...
                    assert_eq!(budget.work(), if control { 285 } else { 281 });
                }
                assert_eq!(budget.storage(), FLOOR);
            }
        }
