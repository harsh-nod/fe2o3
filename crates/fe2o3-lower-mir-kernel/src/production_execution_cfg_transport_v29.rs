...
impl<'a> ExecutionCfgV29<'a> {
    fn new(
        ...
        let mut nominal_locals = borrowed_aggregate_vec_v1(function.locals().len(), budget)?;
        ...
        let mut edges = borrowed_aggregate_vec_v1(occurrences.successors().len(), budget)?;
        let mut incoming = borrowed_aggregate_vec_v1(blocks, budget)?;
        let mut arrived = borrowed_aggregate_vec_v1(blocks, budget)?;
        ...
        let mut entries = borrowed_aggregate_vec_v1(entry_count, budget)?;
        let mut leaves = borrowed_aggregate_vec_v1(leaf_count, budget)?;
        let mut ranges = borrowed_aggregate_vec_v1(blocks, budget)?;
...
fn clone_execution_cfg_parameters_v29(
    ...
    for (local, values) in parameters {
        let mut components = borrowed_aggregate_vec_v1(values.len(), budget)?;
        for value in values {
...
fn clone_execution_cfg_arguments_v29(
    ...
    let mut output = borrowed_aggregate_vec_v1(arguments.len(), budget)?;
