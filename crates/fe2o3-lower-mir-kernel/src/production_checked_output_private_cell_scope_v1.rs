...
pub(super) struct PromotionBinding {
    ledger: CanonicalKernelIrWorkLedgerIdentityV1,
    slot: usize,
    floor: usize,
}
impl PromotionBinding {
    pub(super) fn check(&self, budget: &AssertOriginBudgetV1<'_>) -> PResult<()> {
...
// Only private phases enter here. Scratch is dropped before scope-owned credit
// is released; inherited or foreign-ledger storage is never refunded.
pub(super) fn promotion_scoped<'w, T>(
    required: usize,
    budget: &mut AssertOriginBudgetV1<'w>,
    run: impl FnOnce(&mut AssertOriginBudgetV1<'w>, &PromotionBinding) -> PResult<T>,
) -> PResult<T> {
