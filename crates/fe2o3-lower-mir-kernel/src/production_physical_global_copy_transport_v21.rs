...
struct PhysicalGlobalCopyArgumentTransportV21 {
    origin: [u8; PHYSICAL_GLOBAL_COPY_SOURCE_LOCAL_LIMIT],
    locals: usize,
    return_local: usize,
    arguments: [usize; 2],
    consumed: bool,
}
impl PhysicalGlobalCopyArgumentTransportV21 {
    fn with_roles(
        locals: usize,
        return_local: usize,
        arguments: [usize; 2],
        ...
        if !(3..=PHYSICAL_GLOBAL_COPY_SOURCE_LOCAL_LIMIT).contains(&locals) {
            return Err(physical_global_copy_refusal(
                "physical-global-copy semantic local bound",
            ));
        }
        ...
        {
            return Err(physical_global_copy_refusal(
                "physical-global-copy semantic role mapping differs",
            ));
        }
        let mut result = Self {
            origin: [u8::MAX; PHYSICAL_GLOBAL_COPY_SOURCE_LOCAL_LIMIT],
            locals,
            return_local,
            arguments,
            consumed: false,
    ...
    fn temporary(&self, local: usize) -> Result<(), ProductionSemanticKirErrorV1> {
        if local >= self.locals || local == self.return_local || self.arguments.contains(&local) {
            Err(physical_global_copy_refusal(
                "physical-global-copy mutates argument/invalid temporary",
            ))
        } else {
            Ok(())
        }
    ...
    fn assign(
        ...
        let origin = self
            ...
            .ok_or_else(|| {
                physical_global_copy_refusal("physical-global-copy undefined argument transport")
            })?;
        if origin == 1 && !moved {
            return Err(physical_global_copy_refusal(
                "physical-global-copy copies output owner",
            ));
        }
        if moved {
    ...
    fn consume_marker(
        ...
        if self.consumed || moved & 2 == 0 || moved & !3 != 0 {
            return Err(physical_global_copy_refusal(
                "physical-global-copy repeated marker/non-moved output",
            ));
        }
        for (ordinal, local) in args.into_iter().enumerate() {
            ...
            {
                return Err(physical_global_copy_refusal(
                    "physical-global-copy swapped/foreign argument transport",
                ));
            }
        }
        for (ordinal, local) in args.into_iter().enumerate() {
    ...
    fn require_marker(&self) -> Result<(), ProductionSemanticKirErrorV1> {
        if self.consumed {
            ...
            Err(physical_global_copy_refusal(
                "physical-global-copy marker absent",
            ))
        }
    }
}
