...
//! Its 128-element input AND output prefixes and noalias classes are conditional
//! on the complete typed runtime report retained beside this recipe by the checked
//! source owner. The immutable executable still uses its actual output-length mask.
use super::*;
...
use fe2o3_kernel_ir::{
    ...
    PhysicalLdsExchangeKernargSlotV22 as Slot, PhysicalLdsExchangeMemoryObligationsV22,
    Terminator as End, VerifiedCanonicalKernelIrModuleV22,
};
use fe2o3_pliron::{
...
const LDS: IdR = IdR::new(3);
#[path = "production_physical_lds_exchange_ranked_memory_v22.rs"]
mod memory;
const OPERATIONS: usize = 256;
fn invalid(s: &'static str) -> PhysicalLdsExchangeAuxErrorV22 {
    PhysicalLdsExchangeAuxErrorV22::Relation(s)
}
fn vector<T>(count: usize) -> Result<Vec<T>, PhysicalLdsExchangeAuxErrorV22> {
    let mut v = Vec::new();
...
fn copy<T: Copy>(values: &[T]) -> Result<Vec<T>, PhysicalLdsExchangeAuxErrorV22> {
    let mut v = vector(values.len())?;
    v.extend_from_slice(values);
    Ok(v)
}
fn storage_bound() -> Result<usize, PhysicalLdsExchangeAuxErrorV22> {
    Ok(argument_sum_v1(&[
        std::mem::size_of::<Recipe>(),
        3 * std::mem::size_of::<Block>(),
        (OPERATIONS + 1) * std::mem::size_of::<OpR>(),
        (40 * 5 * 5 + 96) * std::mem::size_of::<ValueR>(),
...
impl Values {
    ...
    fn insert(
        ...
        {
            return Err(invalid(
                "LDS exchange ranked duplicate or excessive SSA definition",
            ));
        }
    ...
    fn get(&self, source: ValueId) -> Result<ValueR, PhysicalLdsExchangeAuxErrorV22> {
        self.rows[..self.count]
            .iter()
            .flatten()
            .find_map(|(id, value)| (*id == source).then_some(*value))
            .ok_or_else(|| invalid("LDS exchange ranked undefined actual SSA input"))
    }
}
struct Operations {
    rows: Vec<OpR>,
...
impl Operations {
    fn new() -> Result<Self, PhysicalLdsExchangeAuxErrorV22> {
        Ok(Self {
            rows: vector(OPERATIONS)?,
            next: 4,
        })
    }
    fn id(&mut self) -> Result<IdR, PhysicalLdsExchangeAuxErrorV22> {
    ...
    fn push(&mut self, op: OpR) -> Result<(), PhysicalLdsExchangeAuxErrorV22> {
        if self.rows.len() == OPERATIONS {
            return Err(invalid("LDS exchange ranked operation bound"));
        }
        self.rows.push(op);
        Ok(())
    }
    fn constant(&mut self, value: u64) -> Result<ValueR, PhysicalLdsExchangeAuxErrorV22> {
        let result = self.id()?;
        self.push(OpR::IndexConstant { result, value })?;
        Ok(ValueR::Local(result))
    }
    fn unknown(&mut self) -> Result<ValueR, PhysicalLdsExchangeAuxErrorV22> {
        let result = self.id()?;
    ...
    fn joined(&mut self, inputs: &[ValueR]) -> Result<ValueR, PhysicalLdsExchangeAuxErrorV22> {
        if inputs.is_empty() || inputs.len() > 5 {
            return Err(invalid("LDS exchange ranked dependency arity"));
        }
        let result = self.id()?;
    ...
    fn invocation(&mut self) -> Result<ValueR, PhysicalLdsExchangeAuxErrorV22> {
        let result = self.id()?;
...
pub(super) fn required_conditions(report: &PhysicalLdsExchangeMemoryObligationsV22) -> bool {
    ...
    runtime.input() == read.allocation()
        ...
        && memory::required_conditions(report)
}
fn setup(ops: &mut Operations, grid: u64) -> Result<(), PhysicalLdsExchangeAuxErrorV22> {
    ...
    // A conditional WRITABLE PREFIX, not a replacement for the actual ABI length.
    // required_conditions has joined this owner to the retained minimum512-byte
    // output obligation. This conservative effect domain includes every actual
    // masked store; it does not claim tail-exact behavior for short output slices.
    ops.push(OpR::ViewInSpace {
        result: OUTPUT,
        element_width: 32,
        writable: true,
...
fn entry_values(
    ...
    let [base_low, base_high, group, lane, exec] = op.results.as_slice() else {
        return Err(invalid("LDS exchange ranked entry live-ins"));
    };
    values.insert(base_low.id, ops.unknown()?)?;
    values.insert(base_high.id, ops.unknown()?)?;
    let gid = ops.invocation()?;
    let width = ops.constant(128)?;
    values.insert(group.id, ops.binary(Binary::Divide, gid, width)?)?;
    // The same-owner profile and required_conditions prove exactly one complete
    // workgroup128: globalX == localX for every admitted invocation. Preserve the
    // actual localX SSA identity while using its exact affine value. Introducing
    // a redundant gid%128 here hides peer (localX+64)%128 from sparse arithmetic.
    values.insert(lane.id, gid)?;
    values.insert(exec.id, ops.constant(u64::MAX)?)?;
    Ok(())
}
pub(super) fn physical_lds_exchange_ranked_recipe_v22(
...
) -> Result<Recipe, PhysicalLdsExchangeAuxErrorV22> {
    let [root] = launch.roots() else {
        return Err(invalid("LDS exchange ranked source root roster"));
    };
    let layout = root.layout();
    if root.source_rank() != 1
        || root.source_launch().max_grid() != [1, 1, 1]
    ...
    {
        return Err(invalid(
            "LDS exchange ranked actual source geometry differs",
        ));
    }
...
fn recipe(
    ...
    let [function] = owner.module().functions.as_slice() else {
        return Err(invalid("LDS exchange ranked one function"));
    };
    let [kernel] = owner.module().kernels.as_slice() else {
        return Err(invalid("LDS exchange ranked one kernel"));
    };
    ...
    let body = function
        .body
        .as_ref()
        .ok_or_else(|| invalid("LDS exchange ranked body missing"))?;
    let [block] = body.blocks.as_slice() else {
        return Err(invalid("LDS exchange ranked one actual block"));
    };
    if body.parameters.len() != 2 || !block.parameters.is_empty() {
        return Err(invalid("LDS exchange ranked exact real parameters"));
    }
    let mut values = Values::new();
    let mut ops = Operations::new()?;
    setup(&mut ops, grid)?;
    memory::join_actual_rows(owner, formal)?;
    let mut memory_state = memory::Census::default();
    ...
    for (operation_index, operation) in block.operations.iter().enumerate() {
        ...
        let step = match operation.kind {
            Op::Gfx942PhysicalLdsExchangeDeclaration(_) if operation_index == 0 => {
                entry_values(operation, &mut values, &mut ops)?;
                continue;
            }
            Op::Gfx942PhysicalLdsExchangeStep(step) => step,
            _ => {
                return Err(invalid(
                    "LDS exchange ranked unexpected executable operation",
                ));
            }
        };
        if operation_index > 32 {
            return Err(invalid("LDS exchange ranked native operation bound"));
        }
        ...
        match step.instruction.opcode {
            Opcode::LoadKernargPair => {
                let read = formal
                    .kernarg_reads()
                    .get(abi_cursor)
                    .ok_or_else(|| invalid("LDS exchange ranked missing kernarg row"))?;
                abi_cursor += 1;
                ...
                {
                    return Err(invalid(
                        "LDS exchange ranked exact kernarg operation SSA join",
                    ));
                }
            ...
            Opcode::ScalarLshl32 => {
                let multiplier = ops.constant(128)?;
            ...
            Opcode::GlobalLoadDword => {
                ...
                {
                    return Err(invalid("LDS exchange ranked actual full input read join"));
                }
            ...
            Opcode::GlobalStoreDword => {
                ...
                if !read_seen
                    ...
                    || store.value() != formal.lds_read().value()
                ...
                {
                    return Err(invalid(
                        "LDS exchange ranked actual ready input to output join",
                    ));
                }
                let index = values.get(access.index()[0])?;
                let _data = values.get(store.value())?;
                if store_index.replace(index).is_some() {
                    return Err(invalid("LDS exchange ranked duplicate output effect"));
                }
            }
            ...
            | Opcode::WorkgroupPublishBarrier => {
                memory::Projection {
                    ...
                    ops: &mut ops,
                    special: &mut special,
                    census: &mut memory_state,
                }
                .project(step, operation, location)?;
            }
            Opcode::WaitLgkm0
            | Opcode::WaitVm0
            | Opcode::VectorMove32
            ...
            | Opcode::RestoreExec => {}
            Opcode::Endpgm0 => {
                return Err(invalid("LDS exchange ranked end belongs to actual CFG"));
            }
        }
    ...
    if abi_cursor != 4 || !read_seen || !memory_state.complete() {
        return Err(invalid("LDS exchange ranked complete read census"));
    }
    let index = store_index.ok_or_else(|| invalid("LDS exchange ranked actual output absent"))?;
    if !matches!(block.terminator.as_ref(),Some(End::Return{values}) if values.is_empty()) {
        return Err(invalid("LDS exchange ranked actual return differs"));
    }
    // This is the conditional safety-prefix extent, not an authored instruction
    // or a fabricated runtime value. Preserve the tail edge/argument transport;
    // all LDS effects already occurred under full participation. The canonical
    // compare/mask/restore and their actual ABI-length SSA remain unchanged.
    let length = ops.id()?;
    ...
    Recipe::new(function.id.as_str(), 2, blocks)
        .map_err(PhysicalLdsExchangeAuxErrorV22::RankedRecipe)
}
#[cfg(test)]
#[path = "production_physical_lds_exchange_ranked_v22_tests.rs"]
mod tests;
