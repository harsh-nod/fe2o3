//! Bounded source-event to actual KIR21 SSA construction. No interpreter,
...
use fe2o3_kernel_ir::{
    ...
    Gfx942PhysicalGlobalCopyOpcodeV1 as Opcode, Gfx942PhysicalGlobalCopyStepV1 as Step, Operation,
...
#[cfg(test)]
#[path = "physical_global_copy_materialization_v21_tests.rs"]
pub(crate) mod tests;
...
fn parse(input: &Input<'_>) -> Result<Contract, Error> {
    ...
    if !input.begin.is_complete() || input.begin.occurrence != 0 || input.events.len() > 33 {
...
/// KIR21 owner admission checks general SSA/types and exact pointer/readiness
