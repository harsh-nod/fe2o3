...
fn array_header_and_rows_remain_reserved_across_real_catalog_success_and_failure() {
    ...
    with_output(
        array_owner(ArrayCase::Write { sparse: false }),
        |view, live_budget| {
            // Literal source census: two assignments and one Return span,
            // one private write and zero eligible Global effects. No assertions
            // or pipelines; two empty catalog canonical encodings remain owned.
            assert_eq!(
                view.source.correspondence.statement_operation_spans().len(),
                2
            );
            assert_eq!(
                view.source
...
#[path = "production_private_array_ranked_output_v1_tests.rs"]
mod private_array_ranked_output_v1_tests;

#[test]
fn memory_free_source_reserves_no_global_effect_rows() {
    let seed = array_owner(ArrayCase::Write { sparse: false });
    let semantic = seed.semantic_ssa().source_semantic();
    ...
    .unwrap();
    assert_eq!(operations(source.executable()), 0);
    with_output(source, |view, budget| {
        ...
        assert_eq!(view.global_spans.len(), 1);
        assert!(view.global_accesses.is_empty());
        assert_eq!(view.global_accesses.capacity(), 0);
        let expected = std::mem::size_of::<ProductionSourceOutputOccurrencesV1<'_, '_>>()
            + std::mem::size_of::<SourceOutputBlockRowV1>()
        ...
        assert_eq!(view.storage.retained_storage(), expected);
        assert!(matches!(
            view.global_access(ARRAY_ROOT, ARRAY_ROOT, 0, None, 0, budget),
            Err(ProductionSourceOutputErrorV1::Invalid(
                "global source access ordinal is absent"
            ))
        ));
    });
}
