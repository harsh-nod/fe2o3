...
/// The caller has already replayed this exact source/N owner. Require complete
/// source membership; native empty effects never invent a source association.
pub(super) fn check_source(
    ...
    let mut membership = scratch::<u8>(semantic.functions().len(), budget)?;
    charge(budget, semantic.functions().len())?;
    membership.resize(semantic.functions().len(), 0);
    ...
    for row in owner.correspondence.lowered_functions.iter() {
        charge(budget, 5)?;
        if membership
            .get(row.correspondence_owner.index() as usize)
            .is_none_or(|mark| *mark & 1 == 0)
        {
            return Err(refused("scalar helpers", "exact source root association"));
        }
        let mark = membership
            .get_mut(row.semantic_function.index() as usize)
            .ok_or_else(|| refused("scalar helpers", "source helper coordinate"))?;
        match row.role {
            SemanticKirFunctionRoleV1::KernelEntry
                if row.semantic_function == row.correspondence_owner && *mark == 1 =>
            {
                *mark = 3
            }
            SemanticKirFunctionRoleV1::InternalHelper if *mark == 0 || *mark == 4 => *mark = 4,
            _ => {
                return Err(refused(
                    "scalar helpers",
                    "exact source function role membership",
                ));
            }
        }
    }
    for (ordinal, (function, mark)) in semantic.functions().iter().zip(&membership).enumerate() {
        charge(budget, 3)?;
        match (function.role(), *mark) {
            (SemanticFunctionRoleV1::KernelRoot, 3) => {}
            (SemanticFunctionRoleV1::InternalHelper, 4) => {
                let ordinal = u32::try_from(ordinal).map_err(|_| arithmetic())?;
                if !source_helper_abi(semantic, SemanticFunctionIdV1::from_index(ordinal), budget)?
                {
