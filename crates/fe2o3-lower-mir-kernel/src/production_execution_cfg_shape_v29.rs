...
fn execution_cfg_types_v29(
    ...
    fn append(
        ...
        if let Some((count, fields, element)) = execution_cfg_fields_v29(types, ty)? {
            ...
            borrowed_aggregate_push_v1(output, ty, budget)?;
        }
        Ok(())
    }
...
fn execution_cfg_values_v29(
    ...
    match binding {
        ...
        SemanticValueBindingV1::Value { id, ty } => {
            ...
            borrowed_aggregate_push_v1(values, ValueDef::new(*id, ty), budget)?;
        }
...
fn clone_execution_cfg_binding_v29(
    ...
    Ok(match binding {
        ...
        SemanticValueBindingV1::Aggregate(fields) => {
            let mut output = borrowed_aggregate_vec_v1(fields.len(), budget)?;
            for field in fields {
...
fn rebuild_execution_cfg_binding_v29(
    ...
    if let Some((count, fields, element)) = execution_cfg_fields_v29(types, ty)? {
        let mut output = borrowed_aggregate_vec_v1(count, budget)?;
        for index in 0..count {
