...
mod guarded_policy_tests {
    use super::*;
    use fe2o3_kernel_ir::{
        AccessMode, AddressSpace, BasicBlock, BlockId, ExplicitLaunchExtent1d, Function, Kernel,
        KernelId, LaunchDomain, LaunchExtent, Module, ScalarType, Signature, Terminator, Type,
        ValueId, VerifiedCanonicalKernelIrV8, VerifiedCanonicalKernelIrV9,
    ...
    fn fixed_receipt(
        ...
        if width == FormalIndexWidth::Bits32 {
            // Legacy inert bytes can represent a width the live analysis refuses.
            assert_eq!(
                report.incomplete_reasons(),
                &[fe2o3_kernel_ir::FormalMemoryIncompleteReason::UnsupportedIndexWidth { width }]
            );
        } else {
