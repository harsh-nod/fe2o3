...
pub enum PhysicalLdsExchangeSourceTerminatorV22 {
...
pub struct PhysicalLdsExchangeSourceBlockV22 {
    ...
    pub terminator: PhysicalLdsExchangeSourceTerminatorV22,
}
#[derive(Debug, Eq, PartialEq)]
/// Immutable full source-block and logical-parameter correspondence.
pub struct PhysicalLdsExchangeSourceCorrespondenceV22 {
    semantic_sha256: [u8; 32],
    root: SemanticFunctionIdV1,
    source_blocks: Vec<PhysicalLdsExchangeSourceBlockV22>,
    parameters: [(SemanticLocalIdV1, ValueId); 2],
}
impl PhysicalLdsExchangeSourceCorrespondenceV22 {
    /// Every original semantic block, including eliminated transport.
    pub fn source_blocks(&self) -> &[PhysicalLdsExchangeSourceBlockV22] {
        &self.source_blocks
    }
    ...
    fn capture(
        ...
        let bytes = argument_product_v1(
            ...
            std::mem::size_of::<PhysicalLdsExchangeSourceBlockV22>(),
        )?;
        ...
        for (ordinal, block) in context.function.blocks().iter().enumerate() {
            let terminator = match block.terminator().kind() {
                SemanticTerminatorKindV1::Goto(edge) => {
                    PhysicalLdsExchangeSourceTerminatorV22::TransportGoto {
                        successor: edge.target(),
                    }
                }
                SemanticTerminatorKindV1::Call(call) => {
                    let source = call
                        .physical_lds_exchange_source_v39()
                        .ok_or(ProductionSemanticKirErrorV1::CorrespondenceMismatch)?;
                    PhysicalLdsExchangeSourceTerminatorV22::Marker {
                        successor: call
                            .destination()
                            .ok_or(ProductionSemanticKirErrorV1::CorrespondenceMismatch)?
                            .edge()
                ...
                SemanticTerminatorKindV1::Return => {
                    PhysicalLdsExchangeSourceTerminatorV22::UnitReturn
                }
                _ => return Err(ProductionSemanticKirErrorV1::CorrespondenceMismatch),
            };
            source_blocks.push(PhysicalLdsExchangeSourceBlockV22 {
