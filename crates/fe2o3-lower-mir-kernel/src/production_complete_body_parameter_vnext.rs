//! Narrow ABI-planner recognition for an actual MIR36 whole-body marker.
...
pub(super) fn complete_body_slice_parameter_vnext(
    ...
    let [SemanticCallableDeclV1::Defined { function: root }, marker] = callables else {
        return None;
    };
    if root.index() != 0 {
        return None;
    ...
    for block in function.blocks() {
        if let SemanticTerminatorKindV1::Call(call) = block.terminator().kind()
            && call.callee().index() == 1
        {
            let source = call.complete_body_source_vnext()?;
            if occurrence.replace(()).is_some()
    ...
    occurrence?;
    let (element, length) = complete_body_marker_slice_abi_vnext(types, binding.abi(), ty)?;
    Some((element, length, AccessMode::ReadWrite))
}

fn complete_body_marker_slice_abi_vnext(
    types: &[SemanticTypeDeclV1],
    ...
    for (index, (input, adjusted)) in inputs.iter().zip(abi.adjusted_arguments()).enumerate() {
        ...
        if index == 0 {
            ...
            if !matches!(types.get(input.index() as usize)?.shape(),
                ...
                || (index < 5 && *input != inputs[1])
            {
                return None;
            }
        }
    }
    let length = complete_body_slice_fields_vnext(types, ty, inputs[1])?;
...
fn complete_body_slice_fields_vnext(
    types: &[SemanticTypeDeclV1],
    ty: SemanticTypeIdV1,
    ...
    // MIR36 deliberately retains the ordinary integer representation; V35
...
#[cfg(test)]
#[path = "production_complete_body_parameter_vnext_tests.rs"]
mod tests;
