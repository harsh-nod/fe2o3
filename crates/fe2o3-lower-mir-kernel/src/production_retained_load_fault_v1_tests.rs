...
thread_local! {
    static PENDING: std::cell::RefCell<Option<Vec<Site>>> = const { std::cell::RefCell::new(None) };
}
...
pub(super) fn with_exact_sites<T>(sites: &[Site], build: impl FnOnce() -> T) -> T {
    ...
    PENDING.with_borrow_mut(|pending| *pending = Some(ordered));
    let reset = Reset;
    let output = build();
    assert!(
        PENDING.with_borrow(|pending| pending.as_ref().unwrap().is_empty()),
        "every injected site must fail the normal initialization check exactly once"
    );
    drop(reset);
    output
}
...
pub(super) fn inject_failed_direct_load(
    ...
    let injected = PENDING.with_borrow_mut(|pending| {
        ...
        let Ok(index) = sites.binary_search(&site) else {
            return false;
        };
        sites.remove(index);
        true
