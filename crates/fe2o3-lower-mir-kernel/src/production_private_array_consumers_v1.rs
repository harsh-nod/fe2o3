...
fn private_array_ranked_write_values_v1<W: PrivateArrayChargeV1>(
    operation: &ProductionRankedOperationV1,
    source_access: PrivateArrayAccessV1,
    physical_access: dialect_kernel::AccessKindAttr,
    atomic: Option<NormalizedAtomicContractV1>,
    work: &mut W,
) -> Result<Option<(ProductionRankedValueV1, ProductionRankedValueV1)>, W::Error> {
    ...
    if source_access != PrivateArrayAccessV1::Write
        || *kind != dialect_kernel::AccessKindAttr::Write
        || physical_access != *kind
        || atomic.is_some()
    {
        return Ok(None);
    }
...
fn private_array_ranked_view_matches_v1<W: PrivateArrayChargeV1>(
    ...
    let Some(origin) = (1u64 << 63)
        .checked_add(u64::from(slot.local))
        .and_then(|n| n.checked_add(1))
    else {
        return Ok(false);
    };
    let Some(width) = slot.element_facts.size.checked_mul(8) else {
        return Ok(false);
    };
    Ok(*memory_space == dialect_kernel::MemorySpaceAttr::Private
        && *writable
        && *extent == slot.length
...
fn private_array_ranked_index_matches_v1<W: PrivateArrayChargeV1>(
    offset: u64,
    operation: &ProductionRankedOperationV1,
    work: &mut W,
) -> Result<bool, W::Error> {
    let ProductionRankedOperationV1::IndexConstant { value, .. } = operation else {
        return Ok(false);
    };
    work.charge_private_array_work(1)?;
    Ok(*value == offset)
...
impl<'a> PrivateArrayFinalRelationV1<'a> {
    ...
    fn check(
        ...
        let operation = lowering
            ...
            .ok_or_else(mismatch)?;
        let (view, ranked_index) = private_array_ranked_write_values_v1(
            operation,
            effect.access,
            consumer.access,
            source.atomic,
        ...
        .ok_or_else(mismatch)?;
        let view = self.definition(view, &mut work)?.ok_or_else(mismatch)?;
        if !private_array_ranked_view_matches_v1(slot, view, &mut work)? {
            return Err(mismatch());
        }
        let index = self
            .definition(ranked_index, &mut work)?
            .ok_or_else(mismatch)?;
        if !private_array_ranked_index_matches_v1(offset, index, &mut work)? {
            return Err(mismatch());
        }
        Ok(())
    }
