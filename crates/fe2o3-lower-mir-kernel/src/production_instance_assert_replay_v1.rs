...
#[derive(Clone, Copy)]
struct InstanceAssertReplaySubjectV1<'a> {
...
struct ReplayedInstanceAssertV1 {
...
fn replay_pending_instance_asserts_v1(
    ...
    let result = (|| {
        let subject = InstanceAssertReplaySubjectV1 {
        ...
        replay_checked_instance_asserts_v1(subject, instances, &graph, budget, &mut |_, _| Ok(()))?;
        graph.release(budget)?;
        Ok(())
    })();
    // All temporary indices are dropped before their scratch reservation is released.
    let extra = budget
        .storage()
        .checked_sub(floor)
...
fn check_instance_assert_source_v1(
    ...
    budget.charge_work(argument_sum_v1(&[2, subject.coordinates.seeds.rows.len()])?)?;
    ...
    let root_seed = subject
        ...
        .ok_or_else(execution_call_error_v29)?;
    assert_origin_string_work_v1(&root_seed.function_name, function.id.as_str(), budget)?;
...
fn replay_instance_asserts_in_functions_v1(
    subject: InstanceAssertReplaySubjectV1<'_>,
    instances: &ProductionCallInstancePlanV1<'_>,
    graph: &AssertGraphIndexV1<'_>,
    budget: &mut ArgumentBudgetV1<'_>,
    visit: &mut impl FnMut(
        ReplayedInstanceAssertV1,
        &mut ArgumentBudgetV1<'_>,
    ) -> Result<(), ProductionSemanticKirErrorV1>,
) -> Result<(), ProductionSemanticKirErrorV1> {
    check_instance_assert_source_v1(subject, instances, budget)?;
    replay_checked_instance_asserts_v1(subject, instances, graph, budget, visit)
}
...
fn replay_checked_instance_asserts_v1(
    ...
    let function = functions
        .get(subject.function_ordinal)
        .ok_or_else(execution_call_error_v29)?;
    let root_coordinate = graph.function(function.id.as_str(), budget)?;
    ...
    for (index, sidecar) in subject.sidecars.rows.iter().enumerate() {
        ...
        budget.charge_work(subject.coordinates.seeds.rows.len())?;
        let seed = subject
            .coordinates
            .seeds
            .rows
        ...
        for block in row.ssa().plan().reverse_postorder() {
            ...
            let site =
                SemanticKirAssertSiteV1::new(capture.source.root, capture.function, semantic_block);
            let bad =
            ...
            assert_origin_string_work_v1(&recorded.emitted_function, &seed.function_name, budget)?;
            if recorded.emitted_function != seed.function_name {
                return Err(bad().into());
            }
            let coordinates = subject.coordinates;
            ...
            let relocated = match subject.slot_relocation {
            ...
            // Lifecycle events belong to source calls/returns, never to an
            // assertion or its generated failure trap. Other blocks may shift.
            budget.charge_work(subject.insertions.len())?;
            if subject.insertions.iter().any(|insertion| {
                insertion.after.block == recorded.block
                    || (matches!(recorded.outcome, PendingAssertOutcomeV1::Emitted { .. })
                        && insertion.after.block == failure)
            }) {
                return Err(bad().into());
            }
            if matches!(recorded.outcome, PendingAssertOutcomeV1::Emitted { .. }) {
                budget
                    .charge_work(coordinates.spans.rows.len() + coordinates.controls.rows.len())?;
