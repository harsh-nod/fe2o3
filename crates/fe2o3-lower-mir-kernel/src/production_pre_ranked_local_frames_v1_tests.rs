...
fn physical_copy_meter_has_literal_partial_copy_and_final_check_boundaries() {
    with_physical_component(local_component(false), |executable, inventory, effects| {
        ...
        // + producer227 + callback8 + queries6/7 + copies3/4/4 + final3.
        const EXACT: usize = 4 + 1 + 4 + 1 + 12 + 6 + 1 + 227 + 8 + 6 + 7 + 3 + 4 + 4 + 3;
        assert_eq!(EXACT, 291);
        for limit in [287, EXACT - 1, EXACT] {
