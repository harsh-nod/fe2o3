...
use super::*;
use ProductionOrderedRegionInspectionAvailabilityV1 as Availability;
use ProductionOrderedRegionInspectionErrorV1 as InspectionError;
use ProductionOrderedRegionInspectionLimitsV1 as Limits;
use fe2o3_kernel_ir::{
    CanonicalKirBlockCoordinateV1, CanonicalKirFunctionCoordinateV1,
    CanonicalKirOperationCoordinateV1 as Coordinate, Gfx942OrderedRegionRegistersV1,
    Gfx942OrderedRegionV1, TargetCapability, VerifiedCanonicalKernelIrModuleV16,
};

const WORK: usize = 100_000_000;
const STORAGE: usize = 32 * 1024 * 1024;

fn make_owner(fixture: &Fixture) -> ProductionOrderedRegionPreRankedKirOwnerV16 {
...
fn owner_floor(owner: &ProductionOrderedRegionPreRankedKirOwnerV16) -> usize {
    17 + owner.executable_storage().retained_storage() + owner.call_correspondence_storage()
}
fn coordinates(owner: &ProductionOrderedRegionPreRankedKirOwnerV16) -> Coordinate {
    for (function, value) in owner.executable().module().functions.iter().enumerate() {
        for (block, value) in value.body.as_ref().unwrap().blocks.iter().enumerate() {
            for (operation, value) in value.operations.iter().enumerate() {
                if matches!(value.kind, OperationKind::Gfx942OrderedRegion(_)) {
                    return Coordinate {
...
fn data_owner(data: [SemanticOperandV1; 3]) -> ProductionOrderedRegionPreRankedKirOwnerV16 {
    let fixture = Fixture {
    ...
    .unwrap()
    .with_ordered_region_source_v31(call.ordered_region_source_v31().unwrap());
...
fn inspector_four_normal_owner_modes_retain_exact_plan_and_no_authority() {
    for consume_result in [false, true] {
        for old_marker in [false, true] {
            ...
            assert_eq!(
                view.region().source().frontend_unit,
                source().frontend_unit()
            );
            assert_eq!(
                view.region().source().function,
                *source().function().as_bytes()
            );
            assert_eq!(view.region().source().contract, source().contract());
            assert_eq!(view.region().source().statement, source().statement());
            assert_eq!(view.region().registers().scratch(), 32);
            assert_eq!(view.region().registers().output(), 33);
            assert_eq!(view.region().registers().inputs(), [34, 35, 36]);
            assert_eq!(view.region().registers().vgpr_high_water(), 37);
            let steps = view.region().registers().steps();
            assert_eq!(steps[0].instruction().mnemonic(), "v_xor_b32_e32");
            assert_eq!(steps[1].instruction().mnemonic(), "v_add_u32_e32");
            assert_eq!(steps[0].output(), steps[1].inputs()[0]);
            assert_eq!(
            ...
            assert_eq!(view.declared_profile(), view.region().profile());
            assert_eq!(
            ...
            assert_eq!(view.region().result(), actual.results[0].id);
            drop(view);
...
fn inspector_genuine_constants_repeated_values_and_nondefault_bindings() {
    for (data, constants, repeated) in [
        ...
        let (view, _) = owner
            ...
            .unwrap();
        assert_eq!(view.region().registers().inputs(), [9, 17, 63]);
        assert_eq!(view.region().registers().vgpr_high_water(), 64);
        ...
        if repeated {
            assert_eq!(view.region().inputs()[0], view.region().inputs()[1]);
            assert_eq!(view.region().inputs()[1], view.region().inputs()[2]);
        }
...
fn change_executable(
    owner: &mut ProductionOrderedRegionPreRankedKirOwnerV16,
            ...
            for operation in &mut module.functions[0].body.as_mut().unwrap().blocks[0].operations {
                if let OperationKind::Gfx942OrderedRegion(region) = operation.kind {
                    ...
                    let registers = if wrong_source {
                        ...
                        Gfx942OrderedRegionRegistersV1::new(1, 5, [9, 17, 63]).unwrap()
                    };
                    operation.kind = OperationKind::Gfx942OrderedRegion(
                        Gfx942OrderedRegionV1::new(source, registers, *region.inputs()).unwrap(),
                    );
                }
            }
        });
        ...
        change_executable(&mut owner, |module| {
            if corruption < 6 {
                ...
                let index = operations
                    .iter()
                    .position(|operation| {
                        matches!(operation.kind, OperationKind::Gfx942OrderedRegion(_))
                    })
                    .unwrap();
                if corruption == 6 {
                    let OperationKind::Gfx942OrderedRegion(region) = operations[index].kind else {
                        unreachable!()
                    };
    ...
    for corruption in 0..4 {
        ...
        assert!(
            matches!(
                ...
                Err(fe2o3_kernel_ir::CanonicalKernelIrReplayAdmissionErrorV16::Verification(_))
            ),
            "case {corruption}"
        );
...
fn expected_work(owner: &ProductionOrderedRegionPreRankedKirOwnerV16) -> usize {
...
fn inspector_exact_work_local_global_and_storage_boundaries() {
    ...
    for (work_limit, local, storage, expected_error) in [
        ...
        let mut budget = CanonicalKernelIrVerificationResourceBudgetV1::new(&mut work, storage);
        budget.reserve_storage(floor).unwrap();
        let result = owner.inspect_ordered_region_v1(
            owner.executable().canonical().identity(),
            None,
...
fn inspector_cumulative_second_query_and_prior_failures_are_preserved() {
    ...
    budget.release_storage(receipt.retained_storage()).unwrap();
    assert!(matches!(
        owner.inspect_ordered_region_v1(
            owner.executable().canonical().identity(),
            None,
            Limits::default(),
            &mut budget
...
fn inspector_invalid_limits_and_missing_input_receipts_fail_without_floor_loss() {
    ...
    for max_work in [
        0,
        MAX_PRODUCTION_ORDERED_REGION_INSPECTION_WORK_V1 + 1,
        usize::MAX,
    ] {
        let mut work = CanonicalKernelIrWorkBudgetV1::new(WORK);
        let mut budget = CanonicalKernelIrVerificationResourceBudgetV1::new(&mut work, STORAGE);
