...
mod ordered_program_inspection_v1 {
    use super::*;
    use fe2o3_kernel_ir::{
        ...
        Gfx942OrderedProgramErrorV1, Gfx942U32ProgramV1, TargetCapability,
        ValidatedGfx942OrderedProgramV1, VerifiedCanonicalKernelIrIdentityV17, WaveWidth,
    ...
    pub const MAX_PRODUCTION_ORDERED_PROGRAM_INSPECTION_WORK_V1: usize = 1_048_576;

    /// Additional local ceiling on the existing cumulative verification ledger.
    #[derive(Clone, Copy, Debug, Eq, PartialEq)]
    pub struct ProductionOrderedProgramInspectionLimitsV1 {
        /// Positive bound no larger than the hard inspection ceiling.
        pub max_work: usize,
    }
    impl Default for ProductionOrderedProgramInspectionLimitsV1 {
        fn default() -> Self {
            Self {
                max_work: MAX_PRODUCTION_ORDERED_PROGRAM_INSPECTION_WORK_V1,
            }
        }
    }
    ...
    #[derive(Clone, Copy, Debug, Eq, PartialEq)]
    pub enum ProductionOrderedProgramInspectionErrorV1 {
        ...
        UnsupportedProfile,
        /// Exactly one ordered program must exist.
        ...
        /// Actual operand definitions or the closed program contract disagree.
        InvalidDescriptor(Gfx942OrderedProgramErrorV1),
    }
    impl fmt::Display for ProductionOrderedProgramInspectionErrorV1 {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            write!(f, "ordered-program inspection: {self:?}")
        }
    }
    impl Error for ProductionOrderedProgramInspectionErrorV1 {}
    impl From<Resource> for ProductionOrderedProgramInspectionErrorV1 {
        fn from(error: Resource) -> Self {
            Self::Resource(error)
        }
    }
    type InspectionError = ProductionOrderedProgramInspectionErrorV1;

    /// Explicit availability, never inferred physical observations or authority.
    #[derive(Clone, Copy, Debug, Eq, PartialEq)]
    pub enum ProductionOrderedProgramInspectionAvailabilityV1 {
    ...
    #[derive(Clone, Copy, Debug, Eq, PartialEq)]
    pub struct ProductionOrderedProgramInspectionStorageV1(usize);
    impl ProductionOrderedProgramInspectionStorageV1 {
    ...
    /// let forged = ProductionOrderedProgramInspectionV1 {};
    /// ```
    /// ```compile_fail
    /// use fe2o3_lower_mir_kernel::ProductionOrderedProgramInspectionV1;
    /// fn escape<'a>(view: ProductionOrderedProgramInspectionV1<'a>)
    ///     -> ProductionOrderedProgramInspectionV1<'static> { view }
    /// ```
    /// ```compile_fail
    /// use fe2o3_lower_mir_kernel::ProductionOrderedProgramInspectionV1;
    /// fn change(view: &mut ProductionOrderedProgramInspectionV1<'_>) {
    ///     *view.ordered_program() = *view.ordered_program();
    /// }
    /// ```
    #[derive(Debug)]
    pub struct ProductionOrderedProgramInspectionV1<'owner> {
        owner: &'owner ProductionOrderedProgramPreRankedKirOwnerV17,
        coordinate: CanonicalKirOperationCoordinateV1,
        span: SemanticKirTerminatorOperationSpanV1,
        region: ValidatedGfx942OrderedProgramV1,
        provenance: SemanticSourceProvenanceV1,
        launch: &'owner crate::ProductionSourceLaunchRootV1,
    }
    const _: () = assert!(std::mem::size_of::<ProductionOrderedProgramInspectionV1<'_>>() <= 1024);

    impl ProductionOrderedProgramInspectionV1<'_> {
        /// Exact V17 digest and length of the borrowed executable owner.
        pub fn canonical_identity(&self) -> &VerifiedCanonicalKernelIrIdentityV17 {
        ...
        /// Exact one-to-sixteen-step plan with logical operands and local VGPR bindings.
        pub const fn ordered_program(&self) -> &ValidatedGfx942OrderedProgramV1 {
            &self.region
        }
        ...
        /// Exact declared instruction sequence, not physical execution observations.
        pub const fn declared_program(&self) -> &Gfx942U32ProgramV1 {
            self.region.program()
        }
        ...
        pub const fn source_association(&self) -> ProductionOrderedProgramInspectionAvailabilityV1 {
            ProductionOrderedProgramInspectionAvailabilityV1::RetainedSemanticCorrespondence
        }
        /// Physical register values are never supplied by a static plan.
        pub const fn physical_values(&self) -> ProductionOrderedProgramInspectionAvailabilityV1 {
            ProductionOrderedProgramInspectionAvailabilityV1::Unavailable
        }
        /// No final-artifact mapping is available.
        pub const fn final_artifact(&self) -> ProductionOrderedProgramInspectionAvailabilityV1 {
            ProductionOrderedProgramInspectionAvailabilityV1::Unavailable
        }
        /// Source insertion/resume is not implemented by this view.
        pub const fn source_insertion(&self) -> ProductionOrderedProgramInspectionAvailabilityV1 {
            ProductionOrderedProgramInspectionAvailabilityV1::Unavailable
        }
    ...
    impl ProductionOrderedProgramPreRankedKirOwnerV17 {
        ...
        pub fn inspect_ordered_program_v1<'owner>(
            &'owner self,
            expected: &VerifiedCanonicalKernelIrIdentityV17,
            requested: Option<CanonicalKirOperationCoordinateV1>,
            limits: ProductionOrderedProgramInspectionLimitsV1,
            budget: &mut Budget<'_>,
        ) -> Result<
            (
                ProductionOrderedProgramInspectionV1<'owner>,
                ProductionOrderedProgramInspectionStorageV1,
            ),
            InspectionError,
        > {
            if limits.max_work == 0
                || limits.max_work > MAX_PRODUCTION_ORDERED_PROGRAM_INSPECTION_WORK_V1
            {
            ...
            let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                ...
                Ok((view, ProductionOrderedProgramInspectionStorageV1(bytes)))
            }));
        ...
        for value in values {
            ...
            match value {
                ...
                TargetCapability::Extension { namespace, name } => {
                    ...
                    if namespace == AMDGPU_GFX942_ORDERED_PROGRAM_CAPABILITY_NAMESPACE
            ...
            if inputs[index] == id {
                if seen[index] {
                    return Err(InspectionError::InvalidDescriptor(
                        Gfx942OrderedProgramErrorV1::InputType,
                    ));
                }
                seen[index] = true;
    ...
    fn inspect<'owner>(
        ...
        requested: Option<CanonicalKirOperationCoordinateV1>,
        meter: &mut InspectionMeter<'_, '_>,
    ) -> Result<ProductionOrderedProgramInspectionV1<'owner>, InspectionError> {
        ...
        if module.kernels.len() != 1
            ...
            || semantic.wire_version() != SemanticMirWireVersionV1::V32
        ...
        // Source/register/result/input checks and all sixteen descriptor/padding/
        // initialization checks are bounded by this fixed precharge.
        meter.charge(4096)?;
        let region = validate_gfx942_ordered_program_v1(operation, |id| {
            payload
                .inputs()
                .iter()
        ...
        // The shared checker inspects eight operands/ABI positions, five bindings,
        // identities and all sixteen descriptors. Include the complete count and
        // descriptor comparison below, before reading those bounded sequences.
        meter.charge(4096)?;
        ...
        if region.source()
            ...
            || region.program().count() != source_call.program().count()
            || region.program().descriptors() != source_call.program().descriptors()
        {
            return Err(InspectionError::InvalidCorrespondence);
        }
        ...
        Ok(ProductionOrderedProgramInspectionV1 {
            owner,
            coordinate,
            span,
            region,
...
pub use ordered_program_inspection_v1::*;
