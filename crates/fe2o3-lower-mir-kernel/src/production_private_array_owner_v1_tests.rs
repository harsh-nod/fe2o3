...
fn array_owner(case: ArrayCase) -> ProductionPreRankedKirOwnerV1 {
    array_owner_at_body(case, false)
}

fn array_owner_at_body(case: ArrayCase, helper: bool) -> ProductionPreRankedKirOwnerV1 {
    let old_types = types();
    ...
    match case {
        ...
        ArrayCase::Initializer { values, repetitions, .. } => {
            for _ in 0..repetitions {
                statements.push(assignment(1, ARRAY_TYPE,
                    SemanticRvalueKindV1::Aggregate(SemanticAggregateRvalueV1::new(
                        SemanticAggregateKindV1::Array,
                        values.into_iter().map(|value| constant(ARRAY_SCALAR, value.into(), 4)).collect(),
                    ).unwrap()),
                ));
            }
            statements.push(SemanticStatementV1::new(source,
                SemanticStatementKindV1::Assign(SemanticAssignmentV1::new(
                    array_place(true), SemanticRvalueV1::new(ARRAY_SCALAR,
                        SemanticRvalueKindV1::Use(constant(ARRAY_SCALAR, 99, 4))),
                )),
            ));
        }
...
#[test]
fn private_array_initializer_uses_exact_sealed_helper_instance_and_promoted_absence() {
