...
pub enum PhysicalGlobalCopySourceTerminatorV21 {
...
pub struct PhysicalGlobalCopySourceBlockV21 {
    ...
    pub terminator: PhysicalGlobalCopySourceTerminatorV21,
}
#[derive(Debug, Eq, PartialEq)]
/// Immutable full source-block and logical-parameter correspondence.
pub struct PhysicalGlobalCopySourceCorrespondenceV21 {
    semantic_sha256: [u8; 32],
    root: SemanticFunctionIdV1,
    source_blocks: Vec<PhysicalGlobalCopySourceBlockV21>,
    parameters: [(SemanticLocalIdV1, ValueId); 2],
}
impl PhysicalGlobalCopySourceCorrespondenceV21 {
    /// Every original semantic block, including eliminated transport.
    pub fn source_blocks(&self) -> &[PhysicalGlobalCopySourceBlockV21] {
        &self.source_blocks
    }
    /// Actual root argument locals and canonical logical parameter IDs.
    pub const fn parameters(&self) -> &[(SemanticLocalIdV1, ValueId); 2] {
        &self.parameters
    }
    ...
    fn capture(
        ...
        if plan.correspondence_owner != context.root
            ...
            || plan.parameter_declarations.len() != 2
            || plan.parameter_local_bindings.len() != 2
        ...
        let mut parameters = [(SemanticLocalIdV1::from_index(0), ValueId(0)); 2];
        ...
        let bytes = argument_product_v1(
            ...
            std::mem::size_of::<PhysicalGlobalCopySourceBlockV21>(),
        )?;
        ...
        for (ordinal, block) in context.function.blocks().iter().enumerate() {
            let terminator = match block.terminator().kind() {
                SemanticTerminatorKindV1::Goto(edge) => {
                    PhysicalGlobalCopySourceTerminatorV21::TransportGoto {
                        successor: edge.target(),
                    }
                }
                SemanticTerminatorKindV1::Call(call) => {
                    let source = call
                        .physical_global_copy_source_v38()
                        .ok_or(ProductionSemanticKirErrorV1::CorrespondenceMismatch)?;
                    PhysicalGlobalCopySourceTerminatorV21::Marker {
                        successor: call
                            .destination()
                            .ok_or(ProductionSemanticKirErrorV1::CorrespondenceMismatch)?
                            .edge()
                ...
                SemanticTerminatorKindV1::Return => {
                    PhysicalGlobalCopySourceTerminatorV21::UnitReturn
                }
                _ => return Err(ProductionSemanticKirErrorV1::CorrespondenceMismatch),
            };
            source_blocks.push(PhysicalGlobalCopySourceBlockV21 {
