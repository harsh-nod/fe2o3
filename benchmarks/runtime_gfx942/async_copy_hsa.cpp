...
int main(int argc, char** argv) {
  ...
  hsa_agent_t cpu = agents.cpus[0], gpu = agents.gpus[gpu_index];
  char uuid[21] = {};
  ...
  PoolSelection host_pool{false}, device_pool{true};
  ...
  std::vector<void*> upload(depth), device(depth), download(depth);
  std::vector<hsa_signal_t> signals(depth);
  for (size_t i = 0; i < depth; ++i) {
    HSA_CHECK(hsa_amd_memory_pool_allocate(host_pool.pool, bytes, 0, &upload[i]));
    HSA_CHECK(hsa_amd_memory_pool_allocate(host_pool.pool, bytes, 0, &download[i]));
    HSA_CHECK(hsa_amd_memory_pool_allocate(device_pool.pool, bytes, 0, &device[i]));
    ...
    HSA_CHECK(hsa_amd_agents_allow_access(1, &gpu, nullptr, device[i]));
  ...
  for (size_t iteration = 0; iteration < workload.total_iterations;
    ...
    for (size_t i = 0; i < depth; ++i)
      HSA_CHECK(hsa_amd_memory_async_copy(device[i], gpu, upload[i], cpu, bytes,
                                          0, nullptr, signals[i]));
    wait_and_reset(signals);
    auto middle = std::chrono::steady_clock::now();
    for (size_t i = 0; i < depth; ++i)
      HSA_CHECK(hsa_amd_memory_async_copy(download[i], cpu, device[i], gpu, bytes,
                                          0, nullptr, signals[i]));
    wait_and_reset(signals);
  ...
  constexpr size_t pool_iterations = 1000;
  auto pool_start = std::chrono::steady_clock::now();
  for (size_t i = 0; i < pool_iterations; ++i) {
    void* ptr = nullptr;
    HSA_CHECK(hsa_amd_memory_pool_allocate(device_pool.pool, bytes, 0, &ptr));
  ...
  for (size_t i = 0; i < depth; ++i) {
    ...
    HSA_CHECK(hsa_amd_memory_pool_free(device[i]));
    HSA_CHECK(hsa_amd_memory_pool_free(upload[i]));
    HSA_CHECK(hsa_amd_memory_pool_free(download[i]));
  }
  HSA_CHECK(hsa_shut_down());
  std::printf(
      "backend=hsa schema=fe2o3.async-copy-benchmark.v1 gpu_index=%zu unique_id=%016llx target=%s xnack=disabled bytes=%zu depth=%zu warmups=%zu samples=%zu h2d_p50_ns=%llu h2d_p95_ns=%llu h2d_p50_GBps=%.3f d2h_p50_ns=%llu d2h_p95_ns=%llu d2h_p50_GBps=%.3f device_pool_allocate_free_pair_ns=%llu\n",
