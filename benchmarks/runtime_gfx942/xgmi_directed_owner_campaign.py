#!/usr/bin/env python3
"""Source-bound directed diamond with fresh admission and exact owned cleanup."""
...
C = module("directed_owner_campaign_helpers", HERE / "xgmi_peer_segments_campaign.py")
N = module("directed_owner_native", HERE / "xgmi_directed_owner_native.py")
B, H = N.B, N.H
...
def main():
    ...
    output = Path(tempfile.mkdtemp(prefix="fe2o3-xgmi-directed-owner-results-", dir="/home/harsh/.codex-tmp"))
    ...
    for name, source in {"native.py": Path(N.__file__), "results.py": HERE / "xgmi_directed_owner_results.py",
    ...
    for name in ["test_xgmi_directed_owner_results.py", "test_xgmi_directed_owner_campaign.py",
                 "test_xgmi_directed_owner_native.py"]:
        rec.run(name, ["/usr/bin/python3", "-I", "-B", str(HERE / name)], 120,
                env={**env, "FE2O3_DIRECTED_OWNER_RUST_BINARY": str(binary)})
