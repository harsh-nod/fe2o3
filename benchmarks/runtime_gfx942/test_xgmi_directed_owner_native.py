...
N = module("directed_native_test", HERE / "xgmi_directed_owner_native.py")
R = module("directed_receipt_test", HERE / "test_xgmi_directed_owner_results.py")
B, H = N.B, N.H
DEVICES = [[5, "0000:a6:00.0", "0xb7baafd0fb173d8e"],
           [6, "0000:c6:00.0", "0x10a254ce4987e716"]]
TRIAL = "directed-owner"
...
class DirectedOwnerNative(unittest.TestCase):
    def exercise(self, *, fail_at=None, secondary=None, malformed=False,
        ...
        with tempfile.TemporaryDirectory(prefix="fe2o3-directed-owner-test-") as temp:
            owned = Path(temp)
            for name, source in {"native.py": Path(N.__file__),
                                 "results.py": HERE / "xgmi_directed_owner_results.py",
    ...
    def test_rehashed_boolean_control_substitution_rejects_before_workload(self):
        calls, sleeps, observations, finished, results = self.exercise(
            controls={**N.CONTROLS, "commands_per_tick": True})
        ...
        self.assertEqual(results, [])
        self.assertFalse(finished["native_execution"])
        self.assertIn("fixed directed correctness controls", finished["failures"][0])
