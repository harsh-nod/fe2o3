...
spec = importlib.util.spec_from_file_location("r63_graph_tests", pathlib.Path(__file__).with_name("run-r63-graph-mi300x.py"))
...
class GraphRunnerTests(unittest.TestCase):
    ...
    def test_both_profile_and_shared_runner_must_match_signed_source(self):
        with tempfile.TemporaryDirectory() as temporary:
            instance = runner.GraphRunner(argparse.Namespace(), pathlib.Path(temporary))
    ...
    def test_profile_keeps_static_musl_and_nonperformance_scope(self):
        ...
        self.assertEqual(runner.GraphRunner.example, "gfx942-runtime-r63-async-graph")
        self.assertEqual(runner.GraphRunner.label, "r63")
        self.assertIn("four-stream-copy-diamond", runner.GraphRunner.claim_scope)
        self.assertIn("host-joins", runner.GraphRunner.claim_scope)

    def test_constructor_failure_cleans_only_r63_owned_stage(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with mock.patch.object(runner.owner.base, "parse_args", return_value=args):
                ...
                self.assertEqual(runner.owner.main(constructor, "r63"), 2)
