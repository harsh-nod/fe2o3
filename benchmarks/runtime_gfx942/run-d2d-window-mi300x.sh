...
readonly gpu_index="${FE2O3_D2D_WINDOW_GPU_INDEX:-0}"
readonly second_gpu_index="${FE2O3_D2D_WINDOW_SECOND_GPU_INDEX:-1}"
readonly bytes="${FE2O3_D2D_WINDOW_BYTES:-268435456}"
readonly warmups="${FE2O3_D2D_WINDOW_WARMUPS:-3}"
readonly samples="${FE2O3_D2D_WINDOW_SAMPLES:-10}"
readonly max_busy="${FE2O3_D2D_WINDOW_MAX_BUSY_PERCENT:-5}"
readonly phase_timeout="${FE2O3_D2D_WINDOW_PHASE_TIMEOUT_SECONDS:-180}"
readonly min_r23_bytes=264239137
build_dir="$(mktemp -d "${TMPDIR:-/tmp}/fe2o3-d2d-window-gfx942.XXXXXX")"
readonly build_dir
...
for value in \
  "${gpu_index}" "${second_gpu_index}" "${bytes}" "${warmups}" \
...
((gpu_index != second_gpu_index)) || {
  printf 'GPU indices must be distinct\n' >&2
  exit 2
}
((bytes >= min_r23_bytes && bytes <= 268435456 && warmups >= 1 && samples >= 1)) || {
  printf 'R23 benchmark requires 264239137..268435456 bytes and nonzero statistics\n' >&2
  exit 2
}
...
[[ -z "$(git -C "${repo_root}" status --porcelain=v1 --untracked-files=all --ignore-submodules=none)" ]] || {
  printf 'D2D-window qualification requires a clean checkout\n' >&2
  exit 2
}
...
result="$(timeout --foreground --signal=TERM --kill-after=5s "${phase_timeout}s" \
  "${build_dir}/target/release/examples/gfx942-runtime-d2d-window-benchmark" \
...
result="$(HSA_XNACK=0 ROCR_VISIBLE_DEVICES="${gpu_index}" \
  ...
  "${build_dir}/d2d-copy-hsa" 0 "${bytes}" 1 "${warmups}" "${samples}" \
  "${unique_id}")"
busy_end="$(require_idle_gpu "${gpu_index}")"
...
result="$(HSA_XNACK=0 HIP_VISIBLE_DEVICES="${gpu_index}" \
  timeout --foreground --signal=TERM --kill-after=5s "${phase_timeout}s" \
  "${build_dir}/d2d-copy-hip" 0 "${bytes}" 1 "${warmups}" "${samples}" \
  "${unique_id}")"
busy_end="$(require_idle_gpu "${gpu_index}")"
...
printf 'context gpu_busy_after_percent=%s,%s\n' \
  "$(require_idle_gpu "${gpu_index}")" \
  "$(require_idle_gpu "${second_gpu_index}")"
