#!/usr/bin/env python3
"""Native backing-budget witness; no reservation, performance or refinement claim."""
...
PREFIX = "/home/harsh/fe2o3-xgmi-backing-budget-20260924."
ORDER = ("backing-budget",)
CONTROLS = dict(allocation_bytes=4097, padded_bytes=8192, allocations=3, streams=2,
...
H = load(hot, HOT_SHA, "backing_budget_admission")
B = H.B
B.PREFIX = PREFIX
...
def trial_specs(owned, devices):
    return [("backing-budget", [str(owned / "kfd-owner"), *(d[2] for d in devices)], H.environment(owned))]
...
def run_native(marker):
    ...
    def bound():
        ...
        need(H.same_json(value["controls"], CONTROLS) and H.same_json(value["order"], list(ORDER)), "fixed backing-budget correctness controls")
    ...
    try:
        ...
        binding = bound()
        parser = load(owned / "results.py", binding["payload"]["results.py"], "backing_budget_results")
        source.mkdir()
        (owned / "tmp").mkdir()
