...
static bool lifecycle() {
  for (auto warmups : {"0", "2"}) {
    for (int failure = -1; failure < 4; ++failure) {
      peer::WorkloadShape shape;
      EXPECT(peer::parse_workload_shape("7", "1", warmups, "3", 1, &shape));
  ...
  EXPECT(!peer::run_peer_persistent_hot(
      invalid, [&](std::size_t) { ++calls; return true; },
      [&](std::size_t) { ++calls; return std::uint64_t{0}; },
      [&](std::size_t) { ++calls; return true; },
