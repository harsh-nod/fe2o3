...
SPEC = importlib.util.spec_from_file_location("owner_campaign", HERE / "xgmi_segments_owner_campaign.py")
...
class OwnerCampaign(unittest.TestCase):
    ...
    def test_one_copy_only_trial_and_private_controller_prefix(self):
        ...
        self.assertEqual(C.N.ORDER, ("owner",))
        self.assertEqual(C.N.CONTROLS, {"allocation_bytes": 65536, "descriptor_count": 65, "ordered_lists": 2})
        self.assertEqual(C.N.trial_specs(owned, devices), [("owner", [str(owned / "kfd-owner"), devices[0][2], devices[1][2]], C.H.environment(owned))])
