...
| Direct-KFD compute dependencies | [R48 session-owned native occurrence lifecycle](crates/fe2o3-kfd/docs/r48-native-compute-dependency-lifecycle-v1.md): addressless two-lane source events, B37 target publication, a preallocated 128-active-target session-owner bound, and exact completion-gated reader/event release; native ordering and completion truth remain contracted and unmeasured |
...
| Runtime | Pure-Rust KFD/AQL foundations, bounded persistent H2D/D2H/D2D storage, exact one-binding and two-read/one-write full-allocation persistent compute transactions, and MI300X execution diagnostics; general binding arity, public application authorization, unified multi-device compute, and parity qualification are incomplete |
